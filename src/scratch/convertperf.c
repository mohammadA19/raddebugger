...
entry_point(CmdLine* cmdline)
{
  ...
  string list_data = os_data_from_file_path(arena, list_path);
  U8 splits[] = {'\n'};
  ...
  {
    string dll_path = n.string;
    ProfScope("kick off %.*s", str8_varg(dll_path))
    {
      string dll_path_no_ext = str8_chop_last_dot(dll_path);
      string dll_name = str8_skip_last_slash(dll_path_no_ext);
      string pdb_path = push_str8f(arena, "%S.pdb", dll_path_no_ext);
      string rdi_path = push_str8f(arena, "dump/%S.rdi", dll_name);
