...
void
print_inline_binary_annotations(StringView binary_annots)
{
  uint32 code_offset = 0;
  int32 line_offset = 0;

  for (uint64 cursor = 0; cursor < binary_annots.size; ) {
    ...
    case CV_InlineBinaryAnnotation_ChangeRangeKind: {
      ...
      StringView range_kind_str = cv_string_from_inline_range_kind(range_kind);
    ...
    case CV_InlineBinaryAnnotation_ChangeCodeLengthAndCodeOffset: {
      uint32 code_length_delta = 0;
      uint32 code_offset_delta = 0;
...
entry_point(CmdLine* cmdl)
{
  ...
  if (single_comp_unit_mode) {
    StringView comp_unit_str = cmd_line_string(cmdl, ("comp_unit"));
  ...
  // -base_addr
  uint64 base_addr = 0;
  StringView base_str = cmd_line_string(cmdl, ("base_addr"));
  ...
  // read PDB from disk
  StringView pdb_data = os_data_from_file_path(arena, pdb_name);
  if (pdb_data.size == 0) {
  ...
  StringView        ipi_leaf_data   = pdb_leaf_data_from_tpi(ipi);
  ...
  // find comp units
  StringView            comp_units_data = pdb_data_from_dbi_range(dbi, PDB_DbiRange_ModuleInfo);
  ...
  StringView  now_time_str       = push_date_time_string(arena, &now_time_local);
  ...
  for (; comp_unit_idx < comp_unit_count; ++comp_unit_idx) {
    ...
    StringView       symbol_data   = pdb_data_from_unit_range(msf, comp_unit, PDB_DbiCompUnitRange_Symbols);
    StringView       c13_data      = pdb_data_from_unit_range(msf, comp_unit, PDB_DbiCompUnitRange_C13);

    // parse $$
    CV_DebugS debug_s = cv_parse_debug_s_c13(arena, c13_data);

    // find $$FILE_CKSMS
    StringView file_chksms = cv_file_chksms_from_debug_s(debug_s);
      ...
      for (String8Node* raw_lines_node = raw_lines_list.first; raw_lines_node != 0; raw_lines_node = raw_lines_node.next) {
        ...
        for(CV_C13LinesHeaderNode* header_node = parsed_list.first; header_node != 0; header_node = header_node.next) {
          if (0 < header_node.v.sec_idx && header_node.v.sec_idx <= sections.count) {
            ...
            uint64 sec_voff = sections.v[header_node.v.sec_idx - 1].voff;
    ...
    for (CV_SymbolNode* symbol_n = symbol_list.first; symbol_n != 0; symbol_n = symbol_n.next) {
      ...
      if (symbol.kind == CV_SymKind_LPROC32 || symbol.kind == CV_SymKind_GPROC32) {
        ...
        StringView                      binary_annots       = str8_skip(symbol.data, sizeof(*inline_site));
        ...
        for (uint64 lines_idx = 0; lines_idx < binary_annots_parse.lines_count; lines_idx += 1) {
          ...
          StringView file_name = pdb_strtbl_string_from_off(strtbl, checksum.name_off);

          fprintf(stdout, "\t\t%.*s\n", str8_varg(file_name));
          for (uint64 line_idx = 0; line_idx < lines.line_count; ++line_idx) {
