...
internal String8
rdi_string_from_reg_code(RDI_Arch arch, ulong reg_code)
{
  switch (arch) {
...
internal String8
rdi_format_reg_code(Arena *arena, RDI_Arch arch, ulong reg_code)
{
  String8 result = {0};
...
internal void
rdi_stringize_data_sections(Arena *arena, String8List *out, RDI_Parsed *rdi, uint indent_level)
{
...
internal void
rdi_stringize_top_level_info(Arena *arena, String8List *out, RDI_Parsed *rdi, RDI_TopLevelInfo *tli, uint indent_level)
{
...
internal void
rdi_stringize_binary_section(Arena *arena, String8List *out, RDI_Parsed *rdi, RDI_BinarySection *bin_section, uint indent_level)
{
  String8 name = {0};
...
internal void
rdi_stringize_file_path(Arena *arena, String8List *out, RDI_Parsed *rdi, RDI_FilePathBundle *bundle, RDI_FilePathNode *file_path, uint indent_level)
{
  String8 name = {0};
  name.str = rdi_string_from_idx(rdi, file_path->name_string_idx, &name.size);
  uint this_idx = (uint)(file_path - bundle->file_paths);
...
internal void
rdi_stringize_source_file(Arena *arena, String8List *out, RDI_Parsed *rdi, RDI_SourceFile *source_file, uint indent_level)
{
  // file path node idx
...
internal void
rdi_stringize_line_table(Arena *arena, String8List *out, RDI_Parsed *rdi, RDI_LineTable *line_table, uint indent_level)
{
...
internal void
rdi_stringize_source_line_map(Arena *arena, String8List *out, RDI_Parsed *rdi, RDI_SourceLineMap *map, uint indent_level)
{
  RDI_ParsedSourceLineMap line_map = {0};
  rdi_parsed_from_source_line_map(rdi, map, &line_map);
  str8_list_pushf(arena, out, "%.*slines:\n", indent_level, rdi_stringize_spaces);
  
  for(uint i = 0; i < line_map.count; i += 1)
  {
    uint line_num = line_map.nums[i];
    uint digit_count = 1;
    if(line_num > 0)
    {
      uint x = line_num;
    ...
    uint first = line_map.ranges[i];
    uint opl_raw = line_map.ranges[i + 1];
    uint opl = ClampTop(opl_raw, line_map.voff_count);
    for(uint j = first; j < opl; j += 1)
    {
      if(j == first)
      {
...
internal void
rdi_stringize_unit(Arena *arena, String8List *out, RDI_Parsed *rdi, RDI_Unit *unit, uint indent_level)
{
  String8 unit_name = {0};
...
rdi_stringize_type_node(Arena *arena, String8List *out, RDI_Parsed *rdi,
                        RDI_TypeNode *type, uint indent_level){
  ...
  if (RDI_TypeKind_FirstBuiltIn <= kind &&
    ...
    if (type->kind == RDI_TypeKind_Function ||
      ...
      uint run_first = type->constructed.param_idx_run_first;
      uint run_count_raw = type->constructed.count;
      
      uint run_count = 0;
      uint *run = rdi_idx_run_from_first_count(rdi, run_first, run_count_raw, &run_count);
      
      uint this_type_idx = 0;
      ...
      if (run_count > 0){
        uint last = run_count - 1;
        for (uint j = 0; j < last; j += 1){
...
rdi_stringize_udt(Arena *arena, String8List *out, RDI_Parsed *rdi,
  ...
  if (udt->flags & RDI_UDTFlag_EnumMembers){
    uint first_raw = udt->member_first;
    uint opl_raw = first_raw + udt->member_count;
    uint opl = ClampTop(opl_raw, member_bundle->enum_member_count);
    uint first = ClampTop(first_raw, opl);
    
    if (first < opl){
      str8_list_pushf(arena, out, "%.*smembers={\n", indent_level, rdi_stringize_spaces);
      RDI_EnumMember *enum_member = member_bundle->enum_members + first;
      for (uint i = first; i < opl; i += 1, enum_member += 1){
        String8 name = {0};
  ...
  // field members
    ...
    uint first_raw = udt->member_first;
    uint opl_raw = first_raw + udt->member_count;
    uint opl = ClampTop(opl_raw, member_bundle->member_count);
    uint first = ClampTop(first_raw, opl);
    
    if (first < opl){
      str8_list_pushf(arena, out, "%.*smembers={\n", indent_level, rdi_stringize_spaces);
      RDI_Member *member = member_bundle->members + first;
      for (uint i = first; i < opl; i += 1, member += 1){
        str8_list_pushf(arena, out, "%.*s {\n", indent_level, rdi_stringize_spaces);
...
rdi_stringize_global_variable(Arena *arena, String8List *out, RDI_Parsed *rdi,
                              RDI_GlobalVariable *global_variable, uint indent_level){
  String8 name = {0};
...
rdi_stringize_procedure(Arena *arena, String8List *out, RDI_Parsed *rdi,
                        RDI_Procedure *proc, uint indent_level){
  String8 name = {0};
...
rdi_stringize_scope(Arena *arena, String8List *out, RDI_Parsed *rdi, RDI_Arch arch,
                    RDI_ScopeBundle *bundle, RDI_Scope *scope, uint indent_level)
{
  Temp scratch = scratch_begin(&arena, 1);
  
  uint this_idx = (uint)(scope - bundle->scopes);
  ...
  // voff ranges
  {
    uint voff_range_first_raw = scope->voff_range_first;
    uint voff_range_opl_raw = scope->voff_range_opl;
    uint voff_range_opl_clamped = ClampTop(voff_range_opl_raw, bundle->scope_voff_count);
    uint voff_range_first = ClampTop(voff_range_first_raw, voff_range_opl_clamped);
    uint voff_range_opl = voff_range_opl_clamped;
    if ((voff_range_opl - voff_range_first) % 2 == 1){
      voff_range_opl -= 1;
    }
    
    ulong *voff_ptr = bundle->scope_voffs + voff_range_first;
    
    if (voff_range_opl - voff_range_first > 2){
      str8_list_pushf(arena, out, "%.*s voff_ranges={\n",
                      indent_level, rdi_stringize_spaces);
      for (uint i = voff_range_first; i < voff_range_opl; i += 2, voff_ptr += 2){
  ...
  // locals
  {
    uint local_first = scope->local_first;
    uint local_opl_raw = local_first + scope->local_count;
    uint local_opl = ClampTop(local_opl_raw, bundle->local_count);
    
    if (local_first < local_opl){
      RDI_Local *local_ptr = bundle->locals + local_first;
      for (uint i = local_first; i < local_opl; i += 1, local_ptr += 1){
        ...
        uint location_first = local_ptr->location_first;
        uint location_opl_raw = local_ptr->location_opl;
        uint location_opl = ClampTop(location_opl_raw, bundle->location_block_count);
        ...
        if (location_first < location_opl){
          ...
          for (uint j = location_first; j < location_opl; j += 1, block_ptr += 1){
  ...
  for (uint child = scope->first_child_scope_idx;
       child != 0;){
    // get scope for child
...
internal void
rdi_stringize_inline_site(Arena *arena, String8List *out, RDI_Parsed *rdi, RDI_InlineSite *inline_site, uint indent_level)
{
  String8 name = {0};
