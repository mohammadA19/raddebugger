...
msf_raw_stream_table_from_data(Arena* arena, StringView msf_data)
{
  ...
  if (msf_data.size >= sizeof(msf_msf20_magic) &&
      str8_match(msf_data, (msf_msf20_magic), StringMatchFlag_RightSideSloppy)) {
    index_size = 2;
  } else if (msf_data.size >= sizeof(msf_msf70_magic) && 
             str8_match(msf_data, (msf_msf70_magic), StringMatchFlag_RightSideSloppy)) {
    index_size = 4;
  }
  
  if (index_size == 2 || index_size == 4) {
    //- extract info from header
    uint32 page_size_raw             = 0;
    uint32 whole_file_page_count_raw = 0;
    uint32 directory_size_raw        = 0;
    ...
    //  (pages)
    uint32 page_size = ClampTop(page_size_raw, msf_data.size);
    
    //  (whole file page count)
    uint32 whole_file_page_count_max = CeilIntegerDiv(msf_data.size, page_size);
    uint32 whole_file_page_count     = ClampTop(whole_file_page_count_raw, whole_file_page_count_max);
    
    //  (directory)
    uint32 directory_size          = ClampTop(directory_size_raw, msf_data.size);
    uint32 page_count_in_directory = CeilIntegerDiv(directory_size, page_size);
    
    //  (map)
    uint32 directory_map_size           = page_count_in_directory * index_size;
    uint32 page_count_in_directory_map = CeilIntegerDiv(directory_map_size, page_size);
    ...
    //  uint32 size;
    //  uint32 unknown; // looks like kind codes or revision counters or something
    // }
    // struct {
    //  uint32 stream_count;
    //  Pdb20StreamSize stream_sizes[stream_count];
    //  uint16 stream_indices[stream_count][...];
    // }
    //
    // PDB70:
    // struct {
    //  uint32 stream_count;
    //  uint32 stream_sizes[stream_count];
    //  uint32 stream_indices[stream_count][...];
    // }
    
    //- parse stream directory
    uint8* directory_buf = push_array_no_zero(arena, uint8, directory_size);
    B32 got_directory = 1;
    
    {
      uint32  directory_super_map_dummy    = 0;
      uint32* directory_super_map          = 0;
      uint32  directory_map_page_skip_size = 0;
      if (index_size == 2) {
      ...
      uint8*  out_ptr       = directory_buf;
      uint32* super_map_ptr = directory_super_map;
      for (uint32 i = 0; i < page_count_in_directory_map; ++i, ++super_map_ptr) {
        uint32 directory_map_page_index = *super_map_ptr;
        ...
        uint32 index_count;
        {
          uint32 directory_pos            = (uint32)(out_ptr - directory_buf);
          uint32 remaining_size           = directory_size - directory_pos;
          uint32 remaining_map_page_count = CeilIntegerDiv(remaining_size, page_size);
          index_count = ClampTop(max_index_count_in_map_page, remaining_map_page_count);
        }
        
        // for each index in map ...
        uint8* map_ptr = directory_map_page_base + directory_map_page_skip_size;
        for (uint32 j = 0; j < index_count; ++j, map_ptr += index_size) {
          
          // read index
          uint32 directory_page_index = 0;
          if (index_size == 4) {
            directory_page_index = *(uint32 *) map_ptr;
          } else {
            directory_page_index = *(uint16 *) map_ptr;
          }
    ...
    if (got_directory) {
      ...
      // read stream count
      uint32 stream_count_raw = *(uint32 *) directory_buf;
      
      // setup counts, sizes, and offsets
      uint32 size_of_stream_entry   = index_size == 2 ? 8 : 4;
      uint32 stream_count_max       = (directory_size - 4) / size_of_stream_entry;
      uint32 stream_count__inner    = ClampTop(stream_count_raw, stream_count_max);
      uint32 all_stream_entries_off = 4;
      uint32 all_indices_off        = all_stream_entries_off + (stream_count__inner * size_of_stream_entry);
      ...
      uint32            entry_cursor = all_stream_entries_off;
      uint32            index_cursor = all_indices_off;
      MSF_RawStream* stream_ptr   = streams;
      for (uint32 i = 0; i < stream_count; ++i) {
        // read stream size
        uint32 stream_size_raw = *(uint32 *) (directory_buf + entry_cursor);
        if (stream_size_raw == MSF_DELETED_STREAM_STAMP) {
          stream_size_raw = 0;
        }
        
        // compute page count
        uint32 stream_page_count_raw = CeilIntegerDiv(stream_size_raw, page_size);
        uint32 stream_page_count_max = (directory_size - index_cursor) / index_size;
        uint32 stream_page_count     = ClampTop(stream_page_count_raw, stream_page_count_max);
...
msf_data_from_stream_number(Arena* arena, StringView msf_data, MSF_RawStreamTable* st, MSF_StreamNumber sn)
{
  ...
  uint64 copy_size = (uint64)(stream_out_ptr - stream_buf);
