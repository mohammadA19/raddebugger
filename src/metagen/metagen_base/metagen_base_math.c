...
internal int dim_1s32(Rng1S32 r)                                {int c = r.max-r.min; return c;}
...
internal long dim_1s64(Rng1S64 r)                                {long c = r.max-r.min; return c;}
...
internal float dim_1f32(Rng1F32 r)                                {float c = r.max-r.min; return c;}
...
internal Vec2S16 dim_2s16(Rng2S16 r)                            {Vec2S16 dim = {(short)(r.max.x-r.min.x), (short)(r.max.y-r.min.y)}; return dim;}
