...
#elif OS_LINUX
# if ARCH_X64
#  define ins_atomic_u64_inc_eval(x) __sync_fetch_and_add((volatile uint64 *)(x), 1)
# else
#  error Atomic intrinsics not defined for this operating system / architecture combination.
# endif
#else
...
uint64 count_bits_set16(uint16 val);
...
uint64 bit_size_from_arch(Architecture arch);
uint64 max_instruction_size_from_arch(Architecture arch);
