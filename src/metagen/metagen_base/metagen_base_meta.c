  ...
  for(RebaseTypeTask *t = first_task; t != 0; t = t->next)
  {
    switch(t->type->kind)
    {
      ...
      case TypeKind_Array:
      {
        ...
        {
          RebaseTypeTask *task = push_array(RebaseTypeTask, 1);
    ...
    for(SerializeTypeTask *t = first_task; t != 0; t = t->next)
    {
      switch(t->type->kind)
      {
        ...
        case TypeKind_Ptr:
        {
          ...
          {
            SerializeTypeTask *task = push_array(SerializeTypeTask, 1);
            task->type  = t->type;
            task->count = t->count;
            task->src   = t->src;
...
deserialized_from_typed_data(Arena *arena, Type *type, String8 data, TypeSerializeParams *params)
{
  ...
  result.str  = push_array(U8, result.size);
  {
    Temp scratch = scratch_begin(&arena, 1);
    ...
    for(DeserializeTypeTask *t = first_task; t != 0; t = t->next)
    {
      ...
      switch(t->type->kind)
      {
        ...
        case TypeKind_Ptr:
        {
          ...
          {
            DeserializeTypeTask *task = push_array(DeserializeTypeTask, 1);
            task->type  = t->type;
            task->count = t->count;
            task->dst   = t->dst;
            task->containing_type = t->containing_type;
          ...
          else if(t->type->count_delimiter_name.size != 0 && t->is_post_header)
          {
            ...
            U8 *ptr_dest_buffer = push_array(U8, ptr_dest_buffer_size);
