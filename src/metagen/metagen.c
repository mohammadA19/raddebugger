...
@(rodata) global String8 mg_str_expr_op_symbol_string_table[MG_StrExprOp_COUNT] =
{
  str8_lit_comp(""),
...
@(rodata) global i8 mg_str_expr_op_precedence_table[MG_StrExprOp_COUNT] =
{
  0,
  20, // MG_StrExprOp_Dot
...
@(rodata) global MG_StrExprOpKind mg_str_expr_op_kind_table[MG_StrExprOp_COUNT] =
{
...
mg_str_expr_parse_from_first_opl__min_prec(Arena *arena, MD_Node *first, MD_Node *opl, i8 min_prec)
{
...
mg_eval_table_expand_expr__numeric(MG_StrExpr *expr, MG_TableExpandInfo *info)
{
  ...
  switch(op)
  {
    ...
    {
      i64 left_val = mg_eval_table_expand_expr__numeric(expr->left, info);
      i64 right_val = mg_eval_table_expand_expr__numeric(expr->right, info);
      switch(op)
      {
        default:break;
    ...
    {
      i64 right_val = mg_eval_table_expand_expr__numeric(expr->left, info);
      switch(op)
      {
        default:break;
        case MG_StrExprOp_BitwiseNegate: result = (i64)(~((u64)right_val)); break;
...
mg_eval_table_expand_expr__string(Arena *arena, MG_StrExpr *expr, MG_TableExpandInfo *info, String8List *out)
{
  ...
  switch(op)
  {
    default:
    {
      ...
      {
        i64 numeric_eval = mg_eval_table_expand_expr__numeric(expr, info);
    ...
    {
      i64 bool_value = mg_eval_table_expand_expr__numeric(expr->left, info);
      if(bool_value)
      {
    ...
    {
      i64 column = mg_eval_table_expand_expr__numeric(expr->left, info);
      i64 current_column = out->total_size;
