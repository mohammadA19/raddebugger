...
mg_str_expr_parse_from_first_opl__min_prec(Arena* arena, MD_Node* first, MD_Node* opl, int8 min_prec)
{
...
mg_eval_table_expand_expr__numeric(MG_StrExpr* expr, MG_TableExpandInfo* info)
{
  ...
  switch(op)
  {
    ...
    {
      int64 left_val = mg_eval_table_expand_expr__numeric(expr->left, info);
      int64 right_val = mg_eval_table_expand_expr__numeric(expr->right, info);
      switch(op)
      {
        default:break;
    ...
    {
      int64 right_val = mg_eval_table_expand_expr__numeric(expr->left, info);
      switch(op)
      {
        default:break;
        case MG_StrExprOp_BitwiseNegate: result = (int64)(~((uint64)right_val)); break;
...
mg_eval_table_expand_expr__string(Arena* arena, MG_StrExpr* expr, MG_TableExpandInfo* info, String8List* out)
{
  ...
  switch(op)
  {
    ...
    {
      int64 bool_value = mg_eval_table_expand_expr__numeric(expr->left, info);
      if(bool_value)
      {
    ...
    {
      int64 column = mg_eval_table_expand_expr__numeric(expr->left, info);
      int64 current_column = out->total_size;
