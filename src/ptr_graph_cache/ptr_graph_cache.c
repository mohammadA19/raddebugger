...
ptg_init(void)
{
  ...
  for(ulong idx = 0; idx < ptg_shared->stripes_count; idx += 1)
  {
  ...
  for(ulong idx = 0; idx < ptg_shared->builder_thread_count; idx += 1)
  {
...
internal B32
ptg_u2b_enqueue_req(PTG_Key *key, ulong endt_us)
{
  B32 good = 0;
  OS_MutexScope(ptg_shared->u2b_ring_mutex) for(;;)
  {
    ulong unconsumed_size = ptg_shared->u2b_ring_write_pos-ptg_shared->u2b_ring_read_pos;
    ulong available_size = ptg_shared->u2b_ring_size-unconsumed_size;
...
ptg_builder_thread__entry_point(void *p)
{
  for(;;)
  {
    ...
    //- rjf: unpack hash
    ulong slot_idx = key.root_hash.u64[1]%ptg_shared->slots_count;
    ulong stripe_idx = slot_idx%ptg_shared->stripes_count;
...
ptg_evictor_thread__entry_point(void *p)
{
  for(;;)
  {
    ...
    ulong check_time_user_clocks = ptg_user_clock_idx();
    ulong evict_threshold_us = 10*1000000;
    ulong evict_threshold_user_clocks = 10;
    for(ulong slot_idx = 0; slot_idx < ptg_shared->slots_count; slot_idx += 1)
    {
      ulong stripe_idx = slot_idx%ptg_shared->stripes_count;
      PTG_GraphSlot *slot = &ptg_shared->slots[slot_idx];
      PTG_GraphStripe *stripe = &ptg_shared->stripes[stripe_idx];
