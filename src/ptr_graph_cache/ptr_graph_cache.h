...
{
  U128 root_hash;
  ulong link_offsets[8];
  ulong link_offsets_count;
};
...
struct PTG_Shared
{
  ...
  ulong u2b_ring_size;
  byte *u2b_ring_base;
  ulong u2b_ring_write_pos;
  ulong u2b_ring_read_pos;
  OS_Handle u2b_ring_cv;
  OS_Handle u2b_ring_mutex;
  
  // rjf: builder threads
  ulong builder_thread_count;
...
internal ulong ptg_user_clock_idx(void);
...
internal B32 ptg_u2b_enqueue_req(PTG_Key *key, ulong endt_us);
