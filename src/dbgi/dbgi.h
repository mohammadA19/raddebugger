...
static U64 di_hash_from_seed_string(U64 seed, String8 string, StringMatchFlags match_flags);
static U64 di_hash_from_string(String8 string, StringMatchFlags match_flags);
static U64 di_hash_from_key(DI_Key *k);
static DI_Key di_key_zero(void);
static B32 di_key_match(DI_Key *a, DI_Key *b);
static DI_Key di_key_copy(Arena *arena, DI_Key *src);
static DI_Key di_normalized_key_from_key(Arena *arena, DI_Key *src);
static void di_key_list_push(Arena *arena, DI_KeyList *list, DI_Key *key);
static DI_KeyArray di_key_array_from_list(Arena *arena, DI_KeyList *list);
static DI_KeyArray di_key_array_copy(Arena *arena, DI_KeyArray *src);
static DI_SearchParams di_search_params_copy(Arena *arena, DI_SearchParams *src);
static U64 di_hash_from_search_params(DI_SearchParams *params);
static void di_search_item_chunk_list_concat_in_place(DI_SearchItemChunkList *dst, DI_SearchItemChunkList *to_push);
static U64 di_search_item_num_from_array_element_idx__linear_search(DI_SearchItemArray *array, U64 element_idx);
static String8 di_search_item_string_from_rdi_target_element_idx(RDI_Parsed *rdi, RDI_SectionKind target, U64 element_idx);
...
static DI_Scope *di_scope_open(void);
static void di_scope_close(DI_Scope *scope);
static void di_scope_touch_node__stripe_mutex_r_guarded(DI_Scope *scope, DI_Stripe *stripe, DI_Node *node);
static void di_scope_touch_search_node__stripe_mutex_r_guarded(DI_Scope *scope, DI_SearchStripe *stripe, DI_SearchNode *node);

////////////////////////////////
//~ rjf: Per-Slot Functions

static DI_Node *di_node_from_key_slot__stripe_mutex_r_guarded(DI_Slot *slot, DI_Key *key);

////////////////////////////////
//~ rjf: Per-Stripe Functions

static U64 di_string_bucket_idx_from_string_size(U64 size);
static String8 di_string_alloc__stripe_mutex_w_guarded(DI_Stripe *stripe, String8 string);
static void di_string_release__stripe_mutex_w_guarded(DI_Stripe *stripe, String8 string);

////////////////////////////////
//~ rjf: Key Opening/Closing

static void di_open(DI_Key *key);
static void di_close(DI_Key *key);

////////////////////////////////
//~ rjf: Debug Info Cache Lookups

static RDI_Parsed *di_rdi_from_key(DI_Scope *scope, DI_Key *key, B32 high_priority, U64 endt_us);

////////////////////////////////
//~ rjf: Search Cache Lookups

static DI_SearchItemArray di_search_items_from_key_params_query(DI_Scope *scope, U128 key, DI_SearchParams *params, String8 query, U64 endt_us, B32 *stale_out);

////////////////////////////////
//~ rjf: Asynchronous Parse Work

static B32 di_u2p_enqueue_key(DI_Key *key, U64 endt_us);
static void di_u2p_dequeue_key(Arena *arena, DI_Key *out_key);

static void di_p2u_push_event(DI_Event *event);
static DI_EventList di_p2u_pop_events(Arena *arena, U64 endt_us);
...
static B32 di_u2s_enqueue_req(U128 key, U64 endt_us);
static U128 di_u2s_dequeue_req(U64 thread_idx);

ASYNC_WORK_DEF(di_search_work);
static int di_qsort_compare_search_items(DI_SearchItem *a, DI_SearchItem *b);
static void di_search_thread__entry_point(void *p);

static void di_search_evictor_thread__entry_point(void *p);

////////////////////////////////
//~ rjf: Match Store

static DI_MatchStore *di_match_store_alloc(void);
static void di_match_store_begin(DI_MatchStore *store, DI_KeyArray keys);
static RDI_SectionKind di_match_store_section_kind_from_name(DI_MatchStore *store, String8 name, U64 endt_us);
