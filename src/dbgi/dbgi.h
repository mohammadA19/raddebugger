...
RDI_Parsed* di_rdi_from_key(DI_Scope* scope, DI_Key* key, uint64 endt_us);
...
B32 di_u2p_enqueue_key(DI_Key* key, uint64 endt_us);
void di_u2p_dequeue_key(Arena* arena, DI_Key* out_key);

void di_p2u_push_event(DI_Event* event);
DI_EventList di_p2u_pop_events(Arena* arena, uint64 endt_us);
...
B32 di_u2s_enqueue_req(U128 key, uint64 endt_us);
U128 di_u2s_dequeue_req(uint64 thread_idx);
