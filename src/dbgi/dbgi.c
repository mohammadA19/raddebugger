...
di_init(void)
{
  ...
  di_shared->search_slots = new DI_SearchSlot[di_shared->search_slots_count];
  di_shared->search_stripes_count = Min(di_shared->search_slots_count, os_get_system_info()->logical_processor_count);
  di_shared->search_stripes = new DI_SearchStripe[di_shared->search_stripes_count];
  ...
  di_shared->search_threads = new DI_SearchThread[di_shared->search_threads_count];
  for EachIndex(idx, di_shared->search_threads_count)
  {
    ...
    di_shared->search_threads[idx].ring_base  = /* no zero */ new U8[di_shared->search_threads[idx].ring_size];
...
di_string_alloc__stripe_mutex_w_guarded(DI_Stripe *stripe, String8 string)
{
  ...
  if(node == 0)
  {
    ...
    U8 *chunk_memory = new U8[chunk_size];
...
di_u2p_dequeue_key(Arena *arena, DI_Key *out_key)
{
  OS_MutexScope(di_shared->u2p_ring_mutex) for(;;)
  {
    ...
    if(unconsumed_size >= sizeof(out_key->path.size) + sizeof(out_key->min_timestamp))
    {
      ...
      out_key->path.str = new U8[out_key->path.size];
...
di_p2u_pop_events(Arena *arena, U64 endt_us)
{
  ...
  OS_MutexScope(di_shared->p2u_ring_mutex) for(;;)
  {
    ...
    {
      DI_EventNode *n = new DI_EventNode[1];
      ...
      n->v.string.str = /* no zero */ new U8[n->v.string.size];
...
ASYNC_WORK_DEF(di_search_work)
{
  ...
  //- rjf: setup output
  DI_SearchWorkOut *out = new DI_SearchWorkOut[1];
  ...
  for(U64 idx = in->element_range.min; (idx < in->element_range.max && idx < element_count); idx += 1)
  {
    ...
    if(matches.count == matches.needle_part_count)
    {
      ...
      {
        chunk = new DI_SearchItemChunk[1];
        chunk->cap = 1024;
        chunk->count = 0;
...
di_search_thread__entry_point(void *p)
{
  ...
  for(;;)
  {
    ...
    RDI_Parsed **rdis = new RDI_Parsed *[rdis_count];
    ...
    if(arena != 0)
    {
      ...
      work_thread_arenas = new Arena *[async_thread_count()];
      for EachIndex(idx, rdis_count)
      {
        ...
        {
          DI_SearchWorkIn *in = new DI_SearchWorkIn[1];
          in->key                      = key;
    ...
    if(arena != 0 && !cancelled)
    {
      ...
      items.v = new DI_SearchItem[items.count];
      U64 off = 0;
...
{
  Arena *arena = arena_alloc();
  DI_MatchStore *store = new DI_MatchStore[1];
  store->arena                  = arena;
  ...
  store->match_name_slots       = new DI_MatchNameSlot[store->match_name_slots_count];
...
di_match_from_name(DI_MatchStore *store, String8 name, U64 endt_us)
{
  ...
  if(name.size != 0)
  {
    ...
    if(node == 0)
    {
      ...
      {
        node = /* no zero */ new DI_MatchNameNode[1];
      }
      MemoryZeroStruct(node);
      node->hash = hash;
      ...
      for EachIndex(dbgi_idx, params_keys.count)
      {
        ...
        for EachElement(name_map_kind_idx, name_map_kinds)
        {
          ...
          if(num != 0)
          {
            ...
            {
              DI_MatchNode *m = new DI_MatchNode[1];
