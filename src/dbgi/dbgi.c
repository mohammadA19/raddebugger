...
internal U64
di_hash_from_seed_string(U64 seed, string string, StringMatchFlags match_flags)
{
  U64 result = seed;
...
internal U64
di_hash_from_string(string string, StringMatchFlags match_flags)
{
...
internal string
di_string_alloc__stripe_mutex_w_guarded(DI_Stripe *stripe, string string)
{
  ...
  string allocated_string = str8((U8 *)node, string.size);
  MemoryCopy((U8 *)node, string.str, string.size);
  return allocated_string;
}

internal void
di_string_release__stripe_mutex_w_guarded(DI_Stripe *stripe, string string)
{
  ...
  if(key->path.size != 0)
  {
    ...
    OS_MutexScopeW(stripe->rw_mutex)
    {
      ...
      if(node == 0)
      {
        ...
        string path_stored = di_string_alloc__stripe_mutex_w_guarded(stripe, key->path);
...
di_search_items_from_key_params_query(DI_Scope *scope, U128 key, DI_SearchParams *params, string query, U64 endt_us, B32 *stale_out)
{
...
ASYNC_WORK_DEF(di_parse_work)
{
  ...
  ProfScope("analyze %.*s", str8_varg(og_path))
  {
    ...
    string data = str8((U8 *)base, props.size);
    if(!og_format_is_known)
    {
      string msf20_magic = ("Microsoft C/C++ program database 2.00\r\n\x1aJG\0\0");
      string msf70_magic = ("Microsoft C/C++ MSF 7.00\r\n\032DS\0\0");
      string msfxx_magic = ("Microsoft C/C++");
...
internal DI_Match
di_match_from_name(DI_MatchStore *store, string name, U64 endt_us)
{
  DI_Match result = {0};
  if(name.size != 0)
  {
