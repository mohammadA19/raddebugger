...
global uint64     g_linker_time_out;
...
internal int
t_invoke_linker_with_time_out(uint64 time_out, String8 cmdline)
{
...
{
  return t_invoke_linker_with_time_out(max_uint64, cmdline);
}
...
internal int
t_invoke_linker_with_time_outf(uint64 time_out, char *fmt, ...)
{
  Temp scratch = scratch_begin(0,0);
  va_list args;
  va_start(args, fmt);
...
t_coff_section_header_from_name(String8 string_table, COFF_SectionHeader *section_table, uint64 section_count, String8 name)
{
...
t_coff_section_header_array_from_name(Arena *arena, String8 string_table, COFF_SectionHeader *section_table, uint64 section_count, String8 name)
{
  ...
  for (uint64 sect_idx = 0, match_idx = 0; sect_idx < section_count; sect_idx += 1) {
    COFF_SectionHeader *section_header = &section_table[sect_idx];
    String8             section_name   = coff_name_from_section_header(string_table, section_header);
    if (str8_match(section_name, name, 0)) {
...
t_simple_link_test(void)
{
  ...
  for (uint64 sect_idx = 0; sect_idx < pe.section_count; sect_idx += 1) {
...
t_abs_vs_weak(void)
{
  ...
  uint32 abs_value = 0x123;
  uint8 text_code[] = { 0x48, 0xb8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xC3 };
...
t_abs_vs_regular(void)
{
  ...
  uint8 regular_payload[] = { 0xC0, 0xFF, 0xEE };
...
t_undef_weak(void)
{
  ...
  uint8 weak_payload[] = { 0xDE, 0xAD, 0xBE, 0xEF };
  ...
  uint8 undef_obj_payload[] = { 0x00, 0x00, 0x00, 0x00 };
...
t_weak_cycle(void)
{
  ...
  uint64 time_out = os_now_microseconds() + 3 * 1000 * 1000; // give a generous 3 seconds
...
t_weak_tag(void)
{
  ...
  uint32 weak_tag_expected_value = 0x12345678;
...
t_sect_symbol(void)
{
  ...
  String8 addr_data = str8_substr(sect_data, rng_1u64(0, sizeof(uint64)));
  if (addr_data.size != sizeof(uint64)) {
    goto exit;
  }
  uint64 addr = *(uint64 *)addr_data.str;
...
t_common_block(void)
{
  ...
  uint32                *a_addr    = (uint32 *)data.str;
  uint64                *b_addr    = (uint64 *)(data.str + sizeof(a_data));
...
t_base_relocs(void)
{
  ...
  // main.obj
  String8 entry_name = str8_lit("my_entry");
  uint64 mov_func_name64 = 2;
  uint64 mov_func_name32 = 16;
  uint8 main_text[] = {
...
t_simple_lib_test(void)
{
  ...
  uint32 *data_addr32nb = (uint32 *)(text_data.str+3);
...
t_import_export(void)
{
...

  {
    ...
    for (uint64 i = 0; i < ArrayCount(import_symbols); ++i) {
...

  {
    ...
    COFF_ObjSection *text_sect = coff_obj_writer_push_section(obj_writer, str8_lit(".text"), PE_TEXT_SECTION_FLAGS, str8_array_fixed((uint8[]){ 0xC3 }));
  ...
  if (0) {
...

    {
      ...
      uint64 match_count = 0;
      for (uint64 i = 0; i < export_table.export_count; i += 1) {
        for (uint64 k = 0; k < ArrayCount(expected_symbols); k += 1) {
    ...
    if (0) {
      ...
      uint64 match_count = 0;
      for (uint64 i = 0; i < export_table.export_count; i += 1) {
        for (uint64 k = 0; k < ArrayCount(expected_forwarders); k += 1) {
  ...
  for (uint64 i = 0; i < ArrayCount(export_dll_procs); ++i) {
  ...
  uint16 export_dll_ordinals[] = { 5 };
  for (uint64 i = 0; i < ArrayCount(export_dll_ordinals); ++i) {
...
t_communal_var_vs_regular(void)
{
  ...
  for (uint64 i = 0; i < ArrayCount(exes); i += 1) {
...
t_import_kernel32(void)
{
...
#if OS_WINDOWS
  {
    ...
    int exit_code = -1;
    os_process_join_exit_code(handle, max_uint64, &exit_code);
...
t_delay_import_user32(void)
{
...

  {
    ...
    uint64 msg_off = data_sect->data.total_size;
    str8_list_pushf(obj_writer->arena, &data_sect->data, "test\0");
    uint64 caption_off = data_sect->data.total_size;
...
entry_point(CmdLine *cmdline)
{
  ...
  //
  {
    if (cmd_line_has_flag(cmdline, str8_lit("list"))) {
      ...
      for (uint64 i = 0; i < ArrayCount(target_array); i += 1) {
  ...
  //
  {
    uint64 max_label_size = 0;
    for (uint64 i = 0; i < ArrayCount(target_array); i += 1) { max_label_size = Max(max_label_size, cstring8_length(target_array[i].label)); }

    uint64 dots_min = 10;
    uint64 dots_size = max_label_size+dots_min;
    uint8 *dots      = push_array(scratch.arena, uint8, dots_size);
    MemorySet(dots, '.', dots_size);

    uint64  target_indices_count;
    uint64 *target_indices;
    if (target.node_count == 0) {
      target_indices_count = ArrayCount(target_array);
      target_indices       = push_array(scratch.arena, uint64, ArrayCount(target_array));
      for (uint64 i = 0; i < target_indices_count; i += 1) { target_indices[i] = i; }
    } else {
      target_indices_count = 0;
      target_indices       = push_array(scratch.arena, uint64, target.node_count);
    ...
    uint64 pass_count  = 0;
    uint64 fail_count  = 0;
    uint64 crash_count = 0;
    for (uint64 i = 0; i < target_indices_count; i += 1) {
      uint64 target_idx = target_indices[i];
      ...
      uint64 dots_count = (max_label_size - cstring8_length(target_array[target_idx].label)) + dots_min;
