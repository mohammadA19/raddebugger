...
{
  FP_Handle result = {(uint64)font.face};
  return result;
}
...
fp_metrics_from_font(FP_Handle handle)
{
  ...
  if(font.face != 0)
  {
    result.design_units_per_em = (float)(font.face->units_per_EM * 72.f/96.f);
    result.ascent              = (float)font.face->ascender;
    result.descent             = (float)font.face->descender;
    result.line_gap            = (float)(font.face->height - font.face->ascender + font.face->descender);
    result.capital_height      = (float)(font.face->ascender);
  }
  return result;
}

fp_hook FP_RasterResult
fp_raster(Arena *arena, FP_Handle handle, float size, FP_RasterFlags flags, String8 string)
{
  ProfBeginFunction();
  FP_FT_Font font = fp_ft_font_from_handle(handle);
  ...
  if(font.face != 0)
  {
    ...
    uint64 ascent  = face->size->metrics.ascender >> 6;
    uint64 descent = abs_s64(face->size->metrics.descender >> 6);
    uint64 height  = face->size->metrics.height >> 6;
    ...
    Vec2uint16 dim = {(uint16)total_width+1, height+1};
    uint64 atlas_size = dim.x * dim.y * 4;
    uint8 *atlas = push_array(arena, uint8, atlas_size);
    uint32 baseline = ascent;
    uint32 atlas_write_x = 0;
    for EachIndex(idx, string32.size)
    {
      FT_Load_Char(face, string32.str[idx], FT_LOAD_RENDER);
      ...
      for(uint32 row = 0; row < (uint32)bmp->rows; row += 1)
      {
        uint32 y = baseline - top + row;
        for(uint32 col = 0; col < (uint32)bmp->width; col += 1)
        {
          uint32 x = atlas_write_x + left + col;
          uint64 off = (y*dim.x + x)*4;
          if(off+4 <= atlas_size)
          {
            atlas[off+0] = 255;
            atlas[off+1] = 255;
    ...
    result.advance   = (float)total_width;
    result.atlas     = atlas;
    scratch_end(scratch);
  }
  ProfEnd();
