  ...
  for (PathTask *t = first_task; t != 0 && font.file == 0; t = t.next)
  {
    ...
    if (t == first_task && font.file == 0 && t.path.size != 0)
    {
      // rjf: generate task for user-installed fonts
      {
        ...
        string user_fonts_path = str8_cstring(data);
...
fp_font_open_from_static_data_string(string* data_ptr)
{
  ...
  error = IDWriteFactory_CreateCustomFontFileReference(fp_dwrite_state.factory, &data_ptr, sizeof(string *), (IDWriteFontFileLoader *)&fp_dwrite_static_data_font_file_loader, &font.file);
