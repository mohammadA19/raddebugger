...
fp_dwrite_font_file_stream_node_alloc(String8 *data_ptr)
{
  ...
  if(node == 0)
  {
    ...
    {
      node = /* no zero */ new FP_DWrite_FontFileStreamNode[1];
    }
...
fp_init(void)
{
  ...
  //- rjf: initialize main state
  {
    ...
    fp_dwrite_state = new FP_DWrite_State[1];
...
fp_raster(Arena *arena, FP_Handle font_handle, F32 size, FP_RasterFlags flags, String8 string)
{
  ...
  U16 *glyph_indices = /* no zero */ new U16[string32.size];
  if(font.face != 0)
  {
  ...
  DWRITE_GLYPH_METRICS *glyphs_metrics = /* no zero */ new DWRITE_GLYPH_METRICS[glyphs_count];
  if(font.face != 0)
  {
  ...
  if(font.face != 0)
  {
    ...
    result.atlas        = /* no zero */ new U8[atlas_dim.x*atlas_dim.y*4];
