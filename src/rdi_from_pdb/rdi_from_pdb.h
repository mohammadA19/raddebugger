...
ulong p2r_end_of_cplusplus_container_name(String8 str);
ulong p2r_hash_from_voff(ulong voff);
...
RDIM_Location *p2r_location_from_addr_reg_off(Arena *arena, RDI_Arch arch, RDI_RegCode reg_code, uint reg_byte_size, uint reg_byte_pos, long offset, B32 extra_indirection);
RDI_RegCode p2r_reg_code_from_arch_encoded_fp_reg(RDI_Arch arch, CV_EncodedFramePtrReg encoded_reg);
void p2r_location_over_lvar_addr_range(Arena *arena, RDIM_ScopeChunkList *scopes, RDIM_LocationSet *locset, RDIM_Location *location, CV_LvarAddrRange *range, COFF_SectionHeader *section, CV_LvarAddrGap *gaps, ulong gap_count);
