...
internal ulong
p2r_hash_from_voff(ulong voff)
{
  ulong hash = (voff >> 3) ^ ((7 & voff) << 6);
  return hash;
}
...
p2r_location_from_addr_reg_off(Arena *arena, RDI_Arch arch, RDI_RegCode reg_code, uint reg_byte_size, uint reg_byte_pos, long offset, B32 extra_indirection)
{
  ...
  else
  {
    ...
    uint regread_param = RDI_EncodeRegReadParam(reg_code, reg_byte_size, reg_byte_pos);
...
internal void
p2r_location_over_lvar_addr_range(Arena *arena, RDIM_ScopeChunkList *scopes, RDIM_LocationSet *locset, RDIM_Location *location, CV_LvarAddrRange *range, COFF_SectionHeader *section, CV_LvarAddrGap *gaps, ulong gap_count)
{
  ...
  ulong voff_cursor = voff_first;
  for(ulong i = 0; i < gap_count; i += 1, gap_ptr += 1)
  {
    ulong voff_gap_first = voff_first + gap_ptr->off;
    ulong voff_gap_opl   = voff_gap_first + gap_ptr->len;
...
ASYNC_WORK_DEF(p2r_exe_hash_work)
{
  ...
  ulong *out = push_array(arena, ulong, 1);
...
ASYNC_WORK_DEF(p2r_units_convert_work)
{
  ...
  ProfScope("build units, initial src file map, & collect unit source files")
    if(in->comp_units != 0)
  {
    ulong units_chunk_cap = in->comp_units->count;
    ...
    ProfScope("pass 1: build per-unit info & per-unit line tables")
      for(ulong comp_unit_idx = 0; comp_unit_idx < in->comp_units->count; comp_unit_idx += 1)
    {
          ...
          node = node->next)
      {
        if(node->kind == CV_C13SubSectionKind_Lines)
        {
              ...
              lines_n = lines_n->next)
          {
            ...
            for(ulong idx = 0; idx < file_path_normalized.size; idx += 1)
            {
            ...
            ulong file_path_normalized_hash = rdi_hash(file_path_normalized.str, file_path_normalized.size);
            ulong src_file_slot = file_path_normalized_hash%src_file_map.slots_count;
      ...
      for(ulong comp_unit_idx = 0; comp_unit_idx < in->comp_units->count; comp_unit_idx += 1)
    {
          ...
          rec_range += 1)
      {
        ...
        ulong sym_off_first = rec_range->off + 2;
        ulong sym_off_opl   = rec_range->off + rec_range->hdr.size;
        ...
        ulong sym_header_struct_size = cv_header_struct_size_from_sym_kind(kind);
        ...
        switch(kind)
        {
          ...
          case CV_SymKind_INLINESITE:
          {
            ...
            {
              ulong hash = cv_hash_from_item_id(sym->inlinee);
              ulong slot_idx = hash%unit_c13->inlinee_lines_parsed_slots_count;
            ...
            if(inlinee_lines_parsed != 0)
            {
              ...
              struct LineChunk
              {
                ...
                ulong        cap;
                ulong        count;
                ulong       *voffs;     // [line_count + 1] (sorted)
                uint       *line_nums; // [line_count]
                ushort       *col_nums;  // [2*line_count]
              };
              LineChunk       *first_line_chunk            = 0;
              LineChunk       *last_line_chunk             = 0;
              ulong              total_line_chunk_line_count = 0;
              uint              last_file_off               = max_U32;
              uint              curr_file_off               = max_U32;
              RDIM_LineTable*  line_table                  = 0;
              ...
              for(;;)
              {
                ...
                if((last_file_off != max_U32 && last_file_off != curr_file_off))
                {
                  ...
                  if(last_file_off + sizeof(CV_C13Checksum) <= file_chksms->size)
                  {
                    ...
                    uint             name_off = checksum->name_off;
                  ...
                  RDI_U64  line_count = total_line_chunk_line_count;
                  {
                    ...
                    {
                      MemoryCopy(voffs+dst_idx, chunk->voffs, sizeof(ulong)*(chunk->count+1));
                      MemoryCopy(line_nums+dst_idx, chunk->line_nums, sizeof(uint)*chunk->count);
                      dst_idx += chunk->count;
                    }
                  }
                ...
                if(step.flags & CV_C13InlineSiteDecoderStepFlag_EmitLine)
                {
                  ...
                  if(chunk == 0 || chunk->count+1 >= chunk->cap)
                  {
                    ...
                    chunk->cap       = 256;
                    chunk->voffs     = push_array_no_zero(scratch.arena, ulong, chunk->cap);
                    chunk->line_nums = push_array_no_zero(scratch.arena, uint, chunk->cap);
                  }
...
ASYNC_WORK_DEF(p2r_link_name_map_build_work)
{
      ...
      rec_range += 1)
  {
    ...
    CV_SymKind kind = rec_range->hdr.kind;
    ulong header_struct_size = cv_header_struct_size_from_sym_kind(kind);
    ...
    switch(kind)
    {
      ...
      case CV_SymKind_PUB32:
      {
        ...
        ulong hash = p2r_hash_from_voff(voff);
        ulong bucket_idx = hash%in->link_name_map->buckets_count;
...
ASYNC_WORK_DEF(p2r_itype_fwd_map_fill_work)
{
  ...
  ProfScope("fill itype fwd map") for(CV_TypeId itype = in->itype_first; itype < in->itype_opl; itype += 1)
  {
    ...
    ulong header_struct_size = cv_header_struct_size_from_leaf_kind(kind);
           ...
           range->hdr.size >= 2)
        {
          ...
          switch(kind)
          {
            ...
            case CV_LeafKind_PROCEDURE:
            {
              ...
              uint arglist_itypes_count = arglist->count;
              
              // rjf: push arg types to chain
              for(uint idx = 0; idx < arglist_itypes_count; idx += 1)
              {
                P2R_TypeIdChain *c = push_array(arena, P2R_TypeIdChain, 1);
          ...
          for(;fl_todo_stack != 0;)
          {
            ...
            //- rjf: loop over all fields
            {
                  ...
                  read_ptr = next_read_ptr)
              {
                ...
                ulong field_leaf_header_size = cv_header_struct_size_from_leaf_kind(field_kind);
                ...
                switch(field_kind)
                {
                  ...
                  case CV_LeafKind_METHOD:
                  {
                        ...
                        method_read_ptr = next_method_read_ptr)
                    {
                      ...
                      if(prop == CV_MethodProp_Intro || prop == CV_MethodProp_PureIntro)
                      {
                        ...
                        {
                          vbaseoff = *(uint *)next_method_read_ptr;
                        }
                  ...
                  case CV_LeafKind_ONEMETHOD:
                  {
                    ...
                    if(prop == CV_MethodProp_Intro || prop == CV_MethodProp_PureIntro)
                    {
                      vbaseoff = *(uint *)(vbaseoff_ptr);
                      vbaseoff_opl_ptr += sizeof(uint);
                    }
                ...
                next_read_ptr = (byte *)AlignPow2((ulong)next_read_ptr, 4);
              }
            }
          }
                  ...
                  read_ptr = next_read_ptr)
              {
                ...
                switch(field_kind)
                {
                  ...
                  case CV_LeafKind_ENUMERATE:
                  {
                    ...
                    ulong val64 = cv_u64_from_numeric(&val);
...
#define p2r_type_ptr_from_itype(itype) ((in->itype_type_ptrs && (itype) < in->tpi_leaf->itype_opl) ? (in->itype_type_ptrs[(in->itype_fwd_map[(itype)] ? in->itype_fwd_map[(itype)] : (itype))]) : 0)
  ...
  //
  ulong sym_procedures_chunk_cap = 1024;
  ulong sym_global_variables_chunk_cap = 1024;
  ulong sym_thread_variables_chunk_cap = 1024;
  ulong sym_scopes_chunk_cap = 1024;
  ulong sym_inline_sites_chunk_cap = 1024;
  ...
  //
  ulong procedure_frameprocs_count = 0;
  ulong procedure_frameprocs_cap   = (in->sym_ranges_opl - in->sym_ranges_first);
  ...
  ProfScope("symbols pass 1: produce procedure frame info map (procedure -> frame info)")
  {
    ...
    ulong scratch_overkill = sizeof(procedure_frameprocs[0])*(procedure_frameprocs_cap-procedure_frameprocs_count);
      ...
      switch(kind)
      {
        ...
        case CV_SymKind_BLOCK32:
        {
          ...
          RDIM_Scope *scope = rdim_scope_chunk_list_push(arena, &sym_scopes, sym_scopes_chunk_cap);
          {
            ...
            if(section != 0)
            {
              ulong voff_first = section->voff + block32->off;
              ulong voff_last = voff_first + block32->len;
        ...
        case CV_SymKind_GDATA32:
        {
          ...
          ulong voff = (section ? section->voff : 0) + data32->off;
          ...
          if(!is_duplicate)
          {
            ...
            ulong container_name_opl = p2r_end_of_cplusplus_container_name(name);
        ...
        case CV_SymKind_GPROC32:
        {
          ...
          RDIM_Scope *procedure_root_scope = rdim_scope_chunk_list_push(arena, &sym_scopes, sym_scopes_chunk_cap);
          {
            ...
            if(section != 0)
            {
              ulong voff_first = section->voff + proc32->off;
              ulong voff_last = voff_first + proc32->len;
              RDIM_Rng1U64 voff_range = {voff_first, voff_last};
          ...
          {
            ulong voff = procedure_root_scope->voff_ranges.min;
            ulong hash = p2r_hash_from_voff(voff);
            ulong bucket_idx = hash%in->link_name_map->buckets_count;
        ...
        case CV_SymKind_DEFRANGE_REGISTER:
        {
          ...
          ulong gap_count = ((byte*)sym_data_opl - (byte*)gaps) / sizeof(*gaps);
          RDI_RegCode reg_code = p2r_rdi_reg_code_from_cv_reg_code(in->arch, cv_reg);
        ...
        case CV_SymKind_DEFRANGE_FRAMEPOINTER_REL:
        {
          ...
          uint byte_size = rdi_addr_size_from_arch(in->arch);
          uint byte_pos = 0;
        ...
        case CV_SymKind_INLINESITE:
        {
          ...
          if(inline_site_line_table != 0 && inline_site_line_table->chunk != 0)
          {
            ...
            ulong current_idx = (ulong)(inline_site_line_table - chunk->v);
...
p2r_convert(Arena *arena, P2R_User2Convert *in)
{
  ...
  ulong comp_unit_count = 0;
  PDB_CompUnitContributionArray *comp_unit_contributions = 0;
  ulong comp_unit_contribution_count = 0;
  {
  ...
  if(comp_units != 0) ProfScope("parse syms & line info for each compilation unit")
  {
    ...
    for(ulong idx = 0; idx < comp_unit_count; idx += 1)
    {
  ...
  ulong exe_voff_max = 0;
  {
    ...
    {
      ulong sec_voff_max = coff_sec_ptr->voff + coff_sec_ptr->vsize;
  ...
  ulong arch_addr_size = 0;
  {
    ...
    for(ulong comp_unit_idx = 0; comp_unit_idx < comp_unit_count; comp_unit_idx += 1)
    {
  ...
  //- rjf: join EXE hash
  //
  ulong exe_hash = *async_task_join_struct(exe_hash_task, ulong);
  ...
  //
  ulong symbol_count_prediction = 0;
  ProfScope("predict symbol count")
  {
    ulong rec_range_count = 0;
    if(sym != 0)
    {
  ...
  if(tpi_leaf != 0 && in->flags & P2R_ConvertFlag_Types) ProfScope("types pass 1: produce type forward resolution map")
  {
    ...
    itype_fwd_map = push_array(arena, CV_TypeId, (ulong)itype_opl);
    
    //- rjf: kick off tasks to fill forward resolution map
    ulong task_size_itypes = 1024;
    ulong tasks_count = ((ulong)itype_opl+(task_size_itypes-1))/task_size_itypes;
    ...
    for(ulong idx = 0; idx < tasks_count; idx += 1)
    {
  ...
  if(tpi_leaf != 0 && in->flags & P2R_ConvertFlag_Types) ProfScope("types pass 2: produce per-itype itype chain (for producing dependent types first)")
  {
    ...
    itype_chains = push_array(arena, P2R_TypeIdChain *, (ulong)itype_opl);
...
#define p2r_type_ptr_from_itype(itype) ((itype_type_ptrs && (itype) < itype_opl) ? (itype_type_ptrs[(itype_fwd_map[(itype)] ? itype_fwd_map[(itype)] : (itype))]) : 0)
  ...
  {
    itype_type_ptrs = push_array(arena, RDIM_Type *, (ulong)(itype_opl));
          ...
          itype_chain = itype_chain->next)
      {
        ...
        if(itype_is_basic)
        {
          ...
          if(basic_type == 0)
          {
            ...
            uint byte_size = rdi_size_from_basic_type_kind(type_kind);
            basic_type = dst_type = rdim_type_chunk_list_push(arena, &all_types, (ulong)itype_opl);
          ...
          {
            dst_type = rdim_type_chunk_list_push(arena, &all_types, (ulong)itype_opl);
             ...
             range->hdr.size >= 2)
          {
            ...
            switch(kind)
            {
              ...
              case CV_LeafKind_POINTER:
              {
                ...
                uint            ptr_size = CV_PointerAttribs_Extract_Size(lf->attribs);
                ...
                if(modifier_flags != 0)
                {
                  RDIM_Type *pointer_type = rdim_type_chunk_list_push(arena, &all_types, (ulong)itype_opl);
                  dst_type = rdim_type_chunk_list_push(arena, &all_types, (ulong)itype_opl);
                  dst_type->kind             = RDI_TypeKind_Modifier;
              ...
              case CV_LeafKind_ARRAY:
              {
                ...
                ulong full_size = cv_u64_from_numeric(&array_count);
              ...
              case CV_LeafKind_STRUCTURE:
              {
                ...
                byte *numeric_ptr = (byte*)(lf + 1);
                CV_NumericParsed size = cv_numeric_from_data_range(numeric_ptr, itype_leaf_opl);
                ulong size_u64 = cv_u64_from_numeric(&size);
                byte *name_ptr = numeric_ptr + size.encoded_size;
                String8 name = str8_cstring_capped(name_ptr, itype_leaf_opl);
                ...
                else
                {
                  ...
                  dst_type->byte_size = (uint)size_u64;
                  dst_type->name      = name;
                }
              }break;
  ...
  //
  ulong udt_task_size_itypes = 4096;
  ulong udt_tasks_count = ((ulong)itype_opl+(udt_task_size_itypes-1))/udt_task_size_itypes;
  ...
  {
    for(ulong idx = 0; idx < udt_tasks_count; idx += 1)
    {
  ...
  ProfScope("produce symbols from all streams")
  {
    ...
    //
    ulong global_stream_subdivision_tasks_count = sym ? (sym->sym_ranges.count+16383)/16384 : 0;
    ulong global_stream_syms_per_task = sym ? sym->sym_ranges.count/global_stream_subdivision_tasks_count : 0;
    ulong tasks_count = comp_unit_count + global_stream_subdivision_tasks_count;
...
ASYNC_WORK_DEF(p2r_bake_string_map_join_work)
{
  ...
  {
    for(ulong src_map_idx = 0; src_map_idx < in->src_maps_count; src_map_idx += 1)
    {
      for(ulong slot_idx = in->slot_idx_range.min; slot_idx < in->slot_idx_range.max; slot_idx += 1)
      {
...
ASYNC_WORK_DEF(p2r_bake_string_map_sort_work)
{
  ...
  {
    for(ulong slot_idx = in->slot_idx;
...
p2r_bake(Arena *arena, P2R_Convert2Bake *in)
{
  ...
  ASYNC_TaskList bake_string_map_build_tasks = {0};
  {
    ...
    {
      ulong items_per_task = 4096;
      ulong num_tasks = (in_params->types.total_count+items_per_task-1)/items_per_task;
      RDIM_TypeChunkNode *chunk = in_params->types.first;
      ulong chunk_off = 0;
      for(ulong task_idx = 0; task_idx < num_tasks; task_idx += 1)
      {
        P2R_BakeTypesStringsIn *in = push_array(scratch.arena, P2R_BakeTypesStringsIn, 1);
        in->top = &bake_string_map_topology;
        in->maps = bake_string_maps__in_progress;
        ulong items_left = items_per_task;
        for(;chunk != 0 && items_left > 0;)
        {
          ulong items_in_this_chunk = Min(items_per_task, chunk->count-chunk_off);
    ...
    {
      ulong items_per_task = 4096;
      ulong num_tasks = (in_params->udts.total_count+items_per_task-1)/items_per_task;
    ...
    ProfScope("kick off symbols string map build tasks")
    {
      ...
      for(ulong list_idx = 0; list_idx < ArrayCount(symbol_lists); list_idx += 1)
      {
        ulong items_per_task = 4096;
        ulong num_tasks = (symbol_lists[list_idx]->total_count+items_per_task-1)/items_per_task;
    ...
    {
      ulong items_per_task = 4096;
      ulong num_tasks = CeilIntegerDiv(in_params->inline_sites.total_count, items_per_task);
    ...
    {
      ulong items_per_task = 4096;
      ulong num_tasks = (in_params->scopes.total_count+items_per_task-1)/items_per_task;
  ...
  {
    ulong slots_per_task = 16384;
    ulong num_tasks = (bake_string_map_topology.slots_count+slots_per_task-1)/slots_per_task;
...
p2r_has_symbol_ref(String8 msf_data, String8List symbol_list, MSF_RawStreamTable *st)
{
  ...
  if(dbi)
  {
    ...
    if(gsi_parsed)
    {
      ...
      {
        ulong symbol_off = pdb_gsi_symbol_from_string(gsi_parsed, symbol_data, symbol_n->string);
  ...
  if(info)
  {
    ...
    if(named_streams)
    {
      ...
      if(strtbl)
      {
        ...
        {
          uint off = pdb_strtbl_off_from_string(strtbl, file_n->string);
          if(off != max_U32)
          {
            has_ref = 1;
            break;
