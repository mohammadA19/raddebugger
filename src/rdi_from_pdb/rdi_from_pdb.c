...
ASYNC_WORK_DEF(p2r_gather_unit_src_file_work)
{
    ...
    String8List src_file_paths = {0};
    {
        ...
        StringView obj_folder_path = lower_from_str8(scratch.arena, str8_chop_last_slash(obj_name));
                    ...
                    node = node.next)
        {
            if (node.kind == CV_C13SubSectionKind_Lines)
            {
                        ...
                        lines_n = lines_n.next)
                {
                    ...
                    StringView file_path = lines_n.v.file_name;
                    StringView file_path_sanitized = str8_copy(scratch.arena, str8_skip_chop_whitespace(file_path));
                    {
                    ...
                    rec_range += 1)
            {
                ...
                switch (kind)
                {
                    ...
                    case CV_SymKind_INLINESITE:
                    {
                        ...
                        StringView           binary_annots = str8((U8 *)(sym+1), rec_range.hdr.size - sizeof(rec_range.hdr.kind) - sizeof(*sym));
            ...
            switch (kind)
            {
                ...
                case CV_SymKind_GDATA32:
                {
                    ...
                    if (!is_duplicate)
                    {
                        ...
                        {
                            StringView container_name = str8(name.str, container_name_opl - 2);
                ...
                if (in.parsing_global_stream && top_scope_node == 0)
                {
                    ...
                    StringView name = str8_cstring_capped(udt+1, sym_data_opl);
                ...
                case CV_SymKind_CONSTANT:
                {
                    ...
                    U64 val64 = cv_u64_from_numeric(&val);
                    U8 *name_ptr = val_ptr + val.encoded_size;
                    StringView name = str8_cstring_capped(name_ptr, sym_data_opl);
                    StringView val_data = str8_struct(&val64);
                    U64 container_name_opl = 0;
                    if (type != 0)
                    {
                        container_name_opl = p2r_end_of_cplusplus_container_name(type.name);
                    }
                    StringView name_qualified = name;
...
p2r_convert(Arena *arena, ASYNC_Root *async_root, P2R_ConvertParams *in)
{
    ...
    //
    PDB_Strtbl *strtbl = 0;
    StringView raw_strtbl = str8_zero();
    ...
    ProfScope("gather all source file paths; build nodes")
    {
        ...
        for EachIndex(idx, tasks_count)
        {
            ...
            {
                StringView file_path_sanitized = tasks_outputs[idx].src_file_paths.v[path_idx];
                U64 file_path_sanitized_hash = rdi_hash(file_path_sanitized.str, file_path_sanitized.size);
