...
pe_make_indirect_jump_thunk_x64(COFF_ObjWriter *obj_writer, COFF_ObjSection *code_sect, COFF_ObjSymbol *iat_symbol, string thunk_name)
{
  ...
  // emit chunk
  string jmp_data        = push_str8_copy(obj_writer->arena, str8_array_fixed(thunk));
...
pe_make_load_thunk_x64(COFF_ObjWriter *obj_writer, COFF_ObjSection *code_sect, COFF_ObjSymbol *imp_addr_ptr, COFF_ObjSymbol *tail_merge, string func_name)
{
  ...
  string load_thunk_data        = push_str8_copy(obj_writer->arena, str8_array_fixed(load_thunk));
  ...
  // emit symbol
  string         thunk_name        = push_str8f(obj_writer->arena, "__imp_load_%S", func_name);
...
pe_make_tail_merge_thunk_x64(COFF_ObjWriter *obj_writer, COFF_ObjSection *code_sect, string dll_name, string delay_load_helper_name, COFF_ObjSymbol *dll_import_descriptor)
{
  ...
  string tail_merge_data = push_str8_copy(obj_writer->arena, str8_array_fixed(tail_merge));
  ...
  // emit symbol
  string tail_merge_name = push_str8f(obj_writer->arena, "__tailMerge_%S", dll_name);
...
pe_make_import_entry_obj_delayed(Arena *arena, string dll_name, COFF_TimeStamp time_stamp, COFF_MachineType machine, string debug_symbols)
{
  ...
  string dll_name_cstr = push_cstr(arena, dll_name);
...
pe_make_null_thunk_data_obj_delayed(Arena *arena, string dll_name, COFF_TimeStamp time_stamp, COFF_MachineType machine, string debug_symbols)
{
  ...
  COFF_ObjWriter *obj_writer = coff_obj_writer_alloc(time_stamp, machine);
  string dll_name_cstr = push_cstr(obj_writer->arena, dll_name);
  U64 import_size = coff_word_size_from_machine(machine);
  string null_thunk = str8(push_array(obj_writer->arena, U8, import_size), import_size);
  string null_lookup = str8(push_array(obj_writer->arena, U8, import_size), import_size);
  COFF_ObjSection *debugs = coff_obj_writer_push_section(obj_writer, (".debug$S"), PE_DEBUG_SECTION_FLAGS|COFF_SectionFlag_Align1Bytes, debug_symbols);
...
pe_make_import_dll_obj_static(Arena *arena, COFF_TimeStamp time_stamp, COFF_MachineType machine, string dll_name, string debug_symbols, String8List import_headers)
{
  ...
  for (String8Node *import_header_n = import_headers.first; import_header_n != 0; import_header_n = import_header_n->next) {
    ...
    case COFF_ImportBy_Ordinal: {
      string ordinal_data = coff_ordinal_data_from_hint(obj_writer->arena, import_header.machine, import_header.hint_or_ordinal);
      string iat_symbol_name = push_str8f(obj_writer->arena, "__imp_%S", import_header.func_name);
    ...
    case COFF_ImportBy_Name: {
      ...
      string int_data = coff_make_import_lookup(obj_writer->arena, import_header.hint_or_ordinal, import_header.func_name);
  ...
  string dll_obj = coff_obj_writer_serialize(arena, obj_writer);
  coff_obj_writer_release(&obj_writer);
  return dll_obj;
}
...
pe_make_import_dll_obj_delayed(Arena *arena, COFF_TimeStamp time_stamp, COFF_MachineType machine, string dll_name, string delay_load_helper_name, string debug_symbols, String8List import_headers, B32 emit_biat, B32 emit_uiat)
{
  ...
  for (String8Node *import_header_n = import_headers.first; import_header_n != 0; import_header_n = import_header_n->next) {
    ...
    case COFF_ImportBy_Name: {
      ...
      string int_symbol_name = push_str8f(obj_writer->arena, "%S.%S.name.delayed", dll_name, import_header.func_name);
