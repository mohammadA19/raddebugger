...
pe_bin_info_from_data(Arena *arena, String8 data)
{
  ...
  Rng1uint64              data_dir_range     = {0};
  Rng1uint64             *data_dir_franges   = 0;
  Rng1uint64             *data_dir_vranges   = 0;
  if(valid && optional_size > 0)
  {
    ...
    data_dir_franges = push_array(arena, Rng1uint64, Max(data_dir_count, PE_DataDirectoryIndex_COUNT));
    ...
    data_dir_vranges = push_array(arena, Rng1uint64, data_dir_count);
    for(uint32 dir_idx = 0; dir_idx < data_dir_count; dir_idx += 1)
    {
      uint64              dir_offset = optional_range.min + reported_data_dir_offset + sizeof(PE_DataDirectory)*dir_idx;
      PE_DataDirectory dir        = {0};
  ...
  {
    Rng1uint64 tls_header_frng = data_dir_franges[PE_DataDirectoryIndex_TLS];
...
internal uint64
pe_pdata_off_from_voff__binary_search_x8664(String8 raw_pdata, uint64 voff)
{
  uint64 result = 0;
...
internal uint64
pe_foff_from_voff(String8 data, PE_BinInfo *bin, uint64 voff)
{
  uint64                 foff              = 0;
  ...
  for(uint64 sect_idx = 0; sect_idx < bin->section_count; sect_idx += 1)
  {
...
internal Rng1uint64
pe_tls_rng_from_bin_base_vaddr(String8 data, PE_BinInfo *bin, uint64 base_vaddr)
{
        ...
        if(apply_to == bin->tls_header.index_address)
        {
          ...
          switch(kind)
          {
            ...
            case PE_BaseRelocKind_HIGH:
            {
              ...
              uint64 high_bits = (apply_to & max_uint16) << 16;
              result_addr = (high_bits + ((base_diff & max_uint32) >> 16)) & max_uint16;
            }break;
            case PE_BaseRelocKind_LOW:
            {
              // rjf: relocate low 16-bits.
              uint64 low_bits = apply_to & max_uint16;
              result_addr = (low_bits + (base_diff & max_uint32)) & max_uint16;
            }break;
            case PE_BaseRelocKind_HIGHLOW:
            {
              // rjf: relocate 32-bits.
              result_addr = (apply_to & max_uint32) + (base_diff & max_uint32);
            }break;
            case PE_BaseRelocKind_HIGHADJ:
            {
              ...
              result_addr  = (apply_to & max_uint16) << 16;
              result_addr += adj_offset;
              result_addr += (base_diff & max_uint32);
              result_addr += 0x8000;
              result_addr  = (result_addr >> 16) & max_uint16;
            }break;
  ...
  Rng1uint64 result = r1u64(result_addr, result_addr+result_size);
  return result;
}
                            ...
                            uint64                *import_count_out)
{
  ...
  uint64 name_table_foff = coff_foff_from_voff(sections, section_count, name_table_voff);
  String8 entries = str8_substr(raw_data, rng_1u64(name_table_foff, raw_data.size));
  if (is_pe32) {
    import_count = index_of_zero_u32((uint32 *)entries.str, entries.size/sizeof(uint32));
    if (import_count == max_uint64) { import_count = 0; }
    imports = push_array(arena, PE_ParsedImport, import_count);

    for (uint64 imp_idx = 0; imp_idx < import_count; imp_idx += 1) {
      uint32 raw_entry = 0;
    ...
    import_count = index_of_zero_u64((uint64 *)entries.str, entries.size/sizeof(uint64));
    if (import_count == max_uint64) { import_count = 0; }
    imports = push_array(arena, PE_ParsedImport, import_count);
...
internal uint64 *
pe_array_from_null_term_addr(Arena *arena, B32 is_pe32, String8 raw_data, Rng1uint64 range, uint64 *count_out)
{
  uint64 *result = 0;
  *count_out = 0;
                           ...
                           Rng1uint64             dir_file_range)
{
  ...
  for (uint64 dll_idx = 0; dll_idx < dll_count; ++dll_idx) {
    ...
    // parse bound table
    Rng1uint64 bound_table_range = {0};
    ...
    // parse unload table
    Rng1uint64 unload_table_range = {0};
...
pe_exports_from_data(Arena *arena, uint64 section_count, COFF_SectionHeader *sections, String8 raw_data, Rng1uint64 dir_file_range, Rng1uint64 dir_virt_range)
{
  Temp scratch = scratch_begin(&arena, 1);
    ...
    if (name_table && export_table && ordinal_table) {
      ...
      for (uint64 i = 0; i < header->name_pointer_table_count; ++i) {
        ...
        String8 forwarder = str8_zero();
        {
          ...
          if (is_forwarder) {
            uint64 fwd_name_off = coff_foff_from_voff(sections, section_count, export_voff);
...
internal PE_ResourceNode *
pe_resource_dir_push_entry_node(Arena *arena, PE_ResourceDir *dir, COFF_ResourceID id, COFF_ResourceID type, uint32 data_version, uint32 version, COFF_ResourceMemoryFlags memory_flags, String8 data)
{
...
pe_resource_dir_push_entry(Arena *arena, PE_ResourceDir *dir, COFF_ResourceID id, COFF_ResourceID type, uint32 data_version, uint32 version, COFF_ResourceMemoryFlags memory_flags, String8 data)
{
...
internal String8
pe_make_manifest_resource(Arena *arena, uint32 resource_id, String8 manifest_data)
{
...
internal String8
pe_make_debug_header_pdb70(Arena *arena, Guid guid, uint32 age, String8 pdb_path)
{
  Temp scratch = scratch_begin(&arena, 1);
...
pe_pdata_sort(COFF_MachineType machine, String8 raw_pdata)
{
  ...
  case COFF_MachineType_X86:
  case COFF_MachineType_X64: {
    uint64 count = raw_pdata.size / sizeof(PE_IntelPdata);
