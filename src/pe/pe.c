...
pe_string_from_fpo_flags(Arena* arena, PE_FPOFlags flags)
{
  ...
  StringView result = str8_list_join(arena, &l, &(StringJoin){.sep=(" ")});
  scratch_end(scratch);
  return result;
}
...
PE_BinInfo
pe_bin_info_from_data(Arena* arena, StringView data)
{
  PE_BinInfo info = {0};
  B32 valid = 1;
  ...
  uint32             file_header_off = dos_header.coff_file_offset + sizeof(pe_magic);
  ...
  uint32     optional_size             = file_header.optional_header_size;
  uint64     after_file_header_off     = file_header_off + sizeof(COFF_FileHeader);
  uint64     after_optional_header_off = after_file_header_off + optional_size;
  ...
  // rjf: get sections
  uint64                 sec_array_off     = optional_range.max;
  ...
  if(valid && optional_size > 0)
  {
    ...
    uint32 data_dir_max = (optional_size - reported_data_dir_offset) / sizeof(PE_DataDirectory);
    ...
    for(uint32 dir_idx = 0; dir_idx < data_dir_count; dir_idx += 1)
    {
      uint64              dir_offset = optional_range.min + reported_data_dir_offset + sizeof(PE_DataDirectory)*dir_idx;
      PE_DataDirectory dir        = {0};
      if(str8_deserial_read_struct(data, dir_offset, &dir) == sizeof(dir))
      {
        uint64 file_off = coff_foff_from_voff(sections, clamped_sec_count, dir.virt_off);
...
PE_DebugInfoList
pe_parse_debug_directory(Arena* arena, StringView raw_image, StringView raw_debug_dir)
{
      ...
      case PE_DebugDirectoryType_CODEVIEW: {
        ...
        switch (cv_magic) {
          case PE_CODEVIEW_PDB20_MAGIC: {
            ...
            uint64 cv_read_size = str8_deserial_read_struct(raw_image, entry.foff, &cv);
...
uint64
pe_pdata_off_from_voff__binary_search_x8664(StringView raw_pdata, uint64 voff)
{
  uint64 result = 0;
  ...
  if(raw_pdata.size >= sizeof(PE_IntelPdata))
  {
    ...
    if(voff >= pdata_array[0].voff_first)
    {
      ...
      uint64 index = pdata_count;
      uint64 min   = 0;
      uint64 opl   = pdata_count;
      for(;;)
      {
...
void *
pe_ptr_from_voff(StringView data, PE_BinInfo* bin, uint64 voff)
{
...
void *
pe_ptr_from_section_num(StringView data, PE_BinInfo* bin, uint64 n)
{
  void* result = 0;
  uint64 sec_count = bin.section_count;
  if(1 <= n && n <= sec_count)
...
pe_base_reloc_block_list_from_data(Arena* arena, StringView raw_base_relocs)
{
  PE_BaseRelocBlockList list = {0};
  for(uint64 off = 0; off < raw_base_relocs.size;)
  {
    // rjf: read next entry
    uint32 page_virt_off = 0;
    uint32 block_size = 0;
    ...
    uint64 entries_size = block_size - (sizeof(block_size) + sizeof(page_virt_off));
    
    // rjf: fill block
    PE_BaseRelocBlock* block = &node.v;
    block.page_virt_off     = page_virt_off;
    block.entry_count       = entries_size / sizeof(uint16);
    block.entries           = push_array(arena, uint16, block.entry_count);
    uint64 entry_read_size = str8_deserial_read_array(raw_base_relocs, off, &block.entries[0], block.entry_count);
...
Rng1U64
pe_tls_rng_from_bin_base_vaddr(StringView data, PE_BinInfo* bin, uint64 base_vaddr)
{
  uint64 result_addr = (bin.tls_header.index_address - bin.image_base);
  uint64 result_size = sizeof(uint32);
  if(bin.arch != Arch_Null)
  {
    uint64 addr_size = bit_size_from_arch(bin.arch)/8;
    Temp scratch = scratch_begin(0, 0);
    StringView raw_relocs = str8_substr(data, bin.data_dir_franges[PE_DataDirectoryIndex_BASE_RELOC]);
    ...
    for(PE_BaseRelocBlockNode* n = relocs.first; n != 0; n = n.next)
    {
      ...
      for(uint64 ientry = 0; ientry < block.entry_count;)
      {
        uint32 reloc = block.entries[ientry];
        uint16 kind = PE_BaseRelocKindFromEntry(reloc);
        uint16 offset = PE_BaseRelocOffsetFromEntry(reloc);
        uint64 apply_to_voff = block.page_virt_off + offset;
        uint64 apply_to_foff = pe_foff_from_voff(data, bin, apply_to_voff);
        uint64 apply_to      = 0;
        str8_deserial_read(data, apply_to_foff, &apply_to, addr_size, 1);
        if(apply_to == bin.tls_header.index_address)
        {
          uint64 base_diff = base_vaddr-bin.image_base;
          switch(kind)
          {
            default:
            {
            ...
            case PE_BaseRelocKind_HIGHADJ:
            {
              ...
              uint16 adj_offset = PE_BaseRelocOffsetFromEntry(block.entries[ientry + 1]);
        ...
        uint32 advance = (kind == PE_BaseRelocKind_HIGHADJ) ? 2 : 1;
        ientry += advance;
      }
    }
    dbl_break:;
                         ...
                         PE_ImageFileCharacteristics file_characteristics)
{
  ...
  if (file_characteristics & PE_ImageFileCharacteristic_FILE_DLL) {
    if (machine == COFF_Machine_X86) {
      read_only static StringView dll_entry_point_arr[] = {
    ...
    case PE_WindowsSubsystem_WINDOWS_GUI: {
      read_only static StringView gui_entry_point_arr[] = {
        ("WinMain"),
        ("wWinMain"),
        ("WinMainCRTStartup"),
        ("wWinMainCRTStartup"),
    ...
    case PE_WindowsSubsystem_WINDOWS_CUI: {
      read_only static StringView cui_entry_point_arr[] = {
        ("main"),
        ("wmain"),
        ("mainCRTStartup"),
        ("wmainCRTStartup"),
...
pe_parsed_imports_from_data(Arena*              arena,
                            ...
                            uint64*                import_count_out)
{
  PE_ParsedImport* imports      = 0;
  uint64              import_count = 0;

  uint64 name_table_off = coff_foff_from_voff(sections, section_count, name_table_voff);
  ...
  if (is_pe32) {
    ...
    for (uint64 imp_idx = 0; imp_idx < import_count; ++imp_idx) {
      uint32 raw_entry = 0;
      ...
      if (is_ordinal) {
        ...
        // map voff . foff
        uint64 off = coff_foff_from_voff(sections, section_count, raw_entry);
...
uint64 *
pe_array_from_null_term_addr(Arena* arena, B32 is_pe32, StringView raw_data, Rng1U64 range, uint64* count_out)
{
  uint64* result = 0;
  *count_out = 0;
  ...
  if (is_pe32) {
    ...
    // push output array
    *count_out = (uint64)(ptr - src);
    result     = push_array(arena, uint64, *count_out);

    // convert & copy
    for (uint64 i = 0; i < *count_out; ++i) {
...
{
  // count imports
  uint64 max_dll_count = dim_1u64(dir_file_range) / sizeof (PE_ImportEntry);
  uint64 dll_count     = max_dll_count;
  for (uint64 i = 0; i < max_dll_count; ++i) {
  ...
  for (uint64 dll_idx = 0; dll_idx < dll_count; ++dll_idx) {
    PE_ImportEntry* raw_dll = str8_deserial_get_raw_ptr(raw_data, dir_file_range.min+(dll_idx*sizeof(*raw_dll)), sizeof(*raw_dll));

    // get name
    uint64     name_off = coff_foff_from_voff(sections, section_count, raw_dll.name_voff);
    StringView name     = StringView();
...
{
  // count imports
  uint64 max_dll_count = dim_1u64(dir_file_range) / sizeof(PE_DelayedImportEntry);
  uint64 dll_count     = 0;
  ...
  for (uint64 dll_idx = 0; dll_idx < dll_count; ++dll_idx) {
    ...
    // parse bound table
    uint64     bound_table_foff  = coff_foff_from_voff(sections, section_count, raw_dll.bound_table_voff);
    Rng1U64 bound_table_range = rng_1u64(bound_table_foff, raw_data.size);
    uint64     bound_table_count;
    uint64 *   bound_table       = pe_array_from_null_term_addr(arena, is_pe32, raw_data, bound_table_range, &bound_table_count);

    // parse unload table
    uint64     unload_table_foff  = coff_foff_from_voff(sections, section_count, raw_dll.unload_table_voff);
    Rng1U64 unload_table_range = rng_1u64(unload_table_foff, raw_data.size);
    uint64     unload_table_count;
    uint64 *   unload_table       = pe_array_from_null_term_addr(arena, is_pe32, raw_data, unload_table_range, &unload_table_count);
...
pe_exports_from_data(Arena* arena, uint64 section_count, COFF_SectionHeader* sections, StringView raw_data, Rng1U64 dir_file_range, Rng1U64 dir_virt_range)
{
  Temp scratch = scratch_begin(&arena, 1);

  PE_ParsedExportTable exptab = {0};

  StringView               raw_dir = str8_substr(raw_data, dir_file_range);
  PE_ExportTableHeader* header  = str8_deserial_get_raw_ptr(raw_dir, 0, sizeof(*header));
  if (header) {
    uint64 name_table_off    = coff_foff_from_voff(sections, section_count, header.name_pointer_table_voff);
    uint64 export_table_off  = coff_foff_from_voff(sections, section_count, header.export_address_table_voff);
    uint64 ordinal_table_off = coff_foff_from_voff(sections, section_count, header.ordinal_table_voff);

    uint32* name_table    = str8_deserial_get_raw_ptr(raw_data, name_table_off,    sizeof(*name_table   )*header.name_pointer_table_count);
    uint32* export_table  = str8_deserial_get_raw_ptr(raw_data, export_table_off,  sizeof(*export_table )*header.export_address_table_count);
    uint16* ordinal_table = str8_deserial_get_raw_ptr(raw_data, ordinal_table_off, sizeof(*ordinal_table)*header.name_pointer_table_count);
    ...
    if (name_table && export_table && ordinal_table) {
      ...
      uint64 ordinal_count = 0;
      for (uint64 voff_idx = 0; voff_idx < header.export_address_table_count; ++voff_idx) {
        if (export_table[voff_idx] != 0) {
          ++ordinal_count;
        }
      }

      uint64  ordinal_max     = header.export_address_table_count;
      ...
      for (uint64 i = 0; i < header.name_pointer_table_count; ++i) {
        // get name
        ...
        // get voff
        uint32 export_voff = 0;
        if (ordinal_nb < header.export_address_table_count) {
          export_voff = export_table[ordinal_nb];
        }

        // make ordinal
        uint16 ordinal = header.ordinal_base + ordinal_nb;

        StringView forwarder = StringView();
        {
          B32 is_forwarder = dir_virt_range.min <= export_voff && export_voff < dir_virt_range.max;
          if (is_forwarder) {
            uint64 fwd_name_off = coff_foff_from_voff(sections, section_count, name_voff);
      ...
      for (uint64 ordinal_nb = 0; ordinal_nb < header.export_address_table_count; ++ordinal_nb) {
        uint32 voff            = export_table[ordinal_nb];
        B32 is_voff_taken   = (voff != 0);
        B32 is_ordinal_free = !is_ordinal_used[ordinal_nb];
        if (is_voff_taken && is_ordinal_free) {
          curr_exp.name      = StringView();
          curr_exp.voff      = voff;
...
pe_tls_from_data(Arena*              arena,
                 ...
                 Rng1U64             tls_frange)
{
  StringView raw_tls = str8_substr(raw_data, tls_frange);

  PE_TLSHeader64  header64       = {0};
  uint64             callback_count = 0;
  uint64*            callback_addrs = 0;
  ...
  switch (machine) {
    ...
    case COFF_Machine_X86: {
      ...
      callback_count = (uint64)(ptr-src);
      callback_addrs = push_array(arena, uint64, callback_count);
      for (uint64 i = 0; i < callback_count; ++i) {
        callback_addrs[i] = (uint64)src[i];
      }
    } break;
    case COFF_Machine_X64: {
...
B32
pe_is_res(StringView data)
{
  uint8 magic[sizeof(PE_RES_MAGIC)]; MemoryZeroStruct(&magic);
...
PE_ResourceNode *
pe_resource_dir_push_dir_node(Arena* arena, PE_ResourceDir* dir, COFF_ResourceID id, uint32 characteristics, COFF_TimeStamp time_stamp, uint16 major_version, uint16 minor_version)
{
...
PE_ResourceNode *
pe_resource_dir_push_entry_node(Arena* arena, PE_ResourceDir* dir, COFF_ResourceID id, COFF_ResourceID type, uint32 data_version, uint32 version, COFF_ResourceMemoryFlags memory_flags, StringView data)
{
...
PE_Resource *
pe_resource_dir_push_entry(Arena* arena, PE_ResourceDir* dir, COFF_ResourceID id, COFF_ResourceID type, uint32 data_version, uint32 version, COFF_ResourceMemoryFlags memory_flags, StringView data)
{
...
PE_Resource *
pe_resource_dir_push_dir(Arena* arena, PE_ResourceDir* dir, COFF_ResourceID id, uint32 characteristics, COFF_TimeStamp time_stamp, uint16 major_version, uint16 minor_version)
{
...
void
pe_resource_dir_push_res_file(Arena* arena, PE_ResourceDir* root_dir, StringView res_file)
{
  // parse file into resource list
  StringView res_data = str8_substr(res_file, rng_1u64(sizeof(PE_RES_MAGIC), res_file.size));
...
PE_ResourceDir *
pe_resource_table_from_directory_data(Arena* arena, StringView data)
{
  struct stack_s {
    struct stack_s* next;
    uint64 table_offset;
    uint64 name_base_offset;
    uint64 id_base_offset;
    PE_ResourceDir* table;
    PE_ResourceDir** directory_ptr;
    uint64 name_ientry;
    uint64 id_ientry;
    uint64 name_entry_count;
    uint64 id_entry_count;
  };
  ...
  while (stack) {
    ...
    while (stack.name_ientry < stack.name_entry_count) {
      uint64 entry_offset = stack.name_base_offset + stack.name_ientry * sizeof(COFF_ResourceDirEntry);
      ++stack.name_ientry;
      ...
      uint32 name_offset = coff_entry.name.offset & ~COFF_Resource_SubDirFlag;
      uint16 name_size = 0;
    ...
    while (stack.id_ientry < stack.id_entry_count) {
      uint64 entry_offset = stack.id_base_offset + stack.id_ientry * sizeof(COFF_ResourceDirEntry);
      ++stack.id_ientry;
...
StringView
pe_make_manifest_resource(Arena* arena, uint32 resource_id, StringView manifest_data)
{
  ...
  StringView res = coff_write_resource(arena, type, id, 1, 0, 1033, 0, 0, manifest_data);
  return res;
}
...
StringView
pe_make_debug_header_pdb70(Arena* arena, Guid guid, uint32 age, StringView pdb_path)
{
  Temp scratch = scratch_begin(&arena, 1);
  ...
  StringView cv_data = str8_serial_end(arena, &cv_list);
...
StringView
pe_make_debug_header_rdi(Arena* arena, Guid guid, StringView rdi_path)
{
  Temp scratch = scratch_begin(&arena,1);
  ...
  StringView cv_data = str8_serial_end(arena, &list);
...
uint32 
pe_compute_checksum(uint8* buffer, uint64 buffer_size)
{
  // https://bytepointer.com/resources/microsoft_pe_checksum_algo_distilled.htm
  uint32 hash = 0;
  for (uint16* ptr16 = (uint16*)buffer, *opl16 = (uint16*)(buffer + buffer_size);
       ptr16 < opl16;
       ptr16 += 1) {
    hash += *ptr16;
    hash = (hash >> 16) + (hash & 0xffff);
  }
  hash = (uint16)(((hash >> 16) + hash) & 0xffff);
  hash += buffer_size;
  return hash;
}
