...
public static D3D11_INPUT_ELEMENT_DESC[?] r_d3d11_g_rect_ilay_elements =
.(
...
public static D3D11_INPUT_ELEMENT_DESC[?] r_d3d11_g_mesh_ilay_elements =
.(
...
public static R_D3D11_Window* r_d3d11_window_from_handle(R_Handle handle)
{
    R_D3D11_Window* window = (R_D3D11_Window*)handle.uint64[0];
    if (window == null)
    {
...
public static R_Handle r_d3d11_handle_from_window(R_D3D11_Window* window)
{
    R_Handle handle = default;
    handle.uint64[0] = (uint64)window;
    return handle;
}

public static R_D3D11_Tex2D* r_d3d11_tex2d_from_handle(R_Handle handle)
{
    R_D3D11_Tex2D* texture = (R_D3D11_Tex2D*)handle.uint64[0];
    if (texture == null)
    {
...
public static R_Handle r_d3d11_handle_from_tex2d(R_D3D11_Tex2D* texture)
{
    R_Handle handle = default;
    handle.uint64[0] = (uint64)texture;
    return handle;
}

public static R_D3D11_Buffer* r_d3d11_buffer_from_handle(R_Handle handle)
{
    R_D3D11_Buffer* buffer = (R_D3D11_Buffer*)handle.uint64[0];
    if (buffer == null)
    {
...
public static R_Handle r_d3d11_handle_from_buffer(R_D3D11_Buffer* buffer)
{
    R_Handle handle = default;
    handle.uint64[0] = (uint64)buffer;
    return handle;
}

public static ID3D11Buffer* r_d3d11_instance_buffer_from_size(uint64 size)
{
    ...
    if (size > KB(64))
    {
        ...
        // rjf: build buffer
        {
            ...
            HRESULT error = r_d3d11_state.device.CreateBuffer(r_d3d11_state.device, &desc, 0, &buffer);
        }
...
public static void r_usage_access_flags_from_resource_kind(R_ResourceKind kind, D3D11_USAGE* out_d3d11_usage, UINT* out_cpu_access_flags)
{
    switch (kind)
    {
...
public static void r_init(CmdLine* cmdln)
{
    ...
    if (cmd_line_has_flag(cmdln, "d3d11_debug")) ProfScope("enable break-on-error")
    {
        ...
        error = r_d3d11_state.base_device.QueryInterface(r_d3d11_state.base_device, &IID_ID3D11InfoQueue, (void**)(&info));
        if (SUCCEEDED(error))
        {
            error = info.SetBreakOnSeverity(info, D3D11_MESSAGE_SEVERITY_CORRUPTION, TRUE);
            error = info.SetBreakOnSeverity(info, D3D11_MESSAGE_SEVERITY_ERROR, TRUE);
            info.Release(info);
        }
    }
#endif
    ...
    error = r_d3d11_state.base_device.QueryInterface(r_d3d11_state.base_device, &IID_ID3D11Device1, (void**)(&r_d3d11_state.device));
    error = r_d3d11_state.base_device_ctx.QueryInterface(r_d3d11_state.base_device_ctx, &IID_ID3D11DeviceContext1, (void**)(&r_d3d11_state.device_ctx));
    ProfEnd();
    
    //- rjf: get dxgi device/adapter/factory
    ProfBegin("get dxgi device/adapter/factory");
    error = r_d3d11_state.device.QueryInterface(r_d3d11_state.device, &IID_IDXGIDevice1, (void**)(&r_d3d11_state.dxgi_device));
    error = r_d3d11_state.dxgi_device.GetAdapter(r_d3d11_state.dxgi_device, &r_d3d11_state.dxgi_adapter);
    error = r_d3d11_state.dxgi_adapter.GetParent(r_d3d11_state.dxgi_adapter, &IID_IDXGIFactory2, (void**)(&r_d3d11_state.dxgi_factory));
    error = r_d3d11_state.dxgi_device.SetMaximumFrameLatency(r_d3d11_state.dxgi_device, 1);
    ProfEnd();
    ...
    ProfScope("create main rasterizer")
    {
        ...
        error = r_d3d11_state.device.CreateRasterizerState1(r_d3d11_state.device, &desc, &r_d3d11_state.main_rasterizer);
    }
    ...
    ProfScope("create main blend state")
    {
        ...
        error = r_d3d11_state.device.CreateBlendState(r_d3d11_state.device, &desc, &r_d3d11_state.main_blend_state);
    }
    ...
    ProfScope("create empty blend state")
    {
        ...
        error = r_d3d11_state.device.CreateBlendState(r_d3d11_state.device, &desc, &r_d3d11_state.no_blend_state);
    }
    
    //- rjf: create nearest-neighbor sampler
    ProfScope("create nearest-neighbor sampler")
    {
        D3D11_SAMPLER_DESC desc = default;
        {
        ...
        error = r_d3d11_state.device.CreateSamplerState(r_d3d11_state.device, &desc, &r_d3d11_state.samplers[R_Tex2DSampleKind_Nearest]);
    }
    ...
    ProfScope("create bilinear sampler")
    {
        ...
        error = r_d3d11_state.device.CreateSamplerState(r_d3d11_state.device, &desc, &r_d3d11_state.samplers[R_Tex2DSampleKind_Linear]);
    }
    ...
    ProfScope("create noop depth/stencil state")
    {
        ...
        error = r_d3d11_state.device.CreateDepthStencilState(r_d3d11_state.device, &desc, &r_d3d11_state.noop_depth_stencil);
    }
    ...
    ProfScope("create plain depth/stencil state")
    {
        ...
        error = r_d3d11_state.device.CreateDepthStencilState(r_d3d11_state.device, &desc, &r_d3d11_state.plain_depth_stencil);
    }
    ...
    ProfScope("create buffers")
    {
        ...
        error = r_d3d11_state.device.CreateBuffer(r_d3d11_state.device, &desc, 0, &r_d3d11_state.instance_scratch_buffer_64kb);
    }
                ...
                kind = (R_D3D11_VShadKind)(kind+1))
    {
        ...
        ID3DBlob* vshad_source_blob = null;
        ID3DBlob* vshad_source_errors = null;
        ID3D11VertexShader* vshad = null;
        ProfScope("compile vertex shader")
        {
            error = D3DCompile(source.str,
                            ...
                            &vshad_source_blob,
                            &vshad_source_errors);
            String8 errors = default;
            if (FAILED(error))
            {
                errors = str8((uint8*)vshad_source_errors.GetBufferPointer(vshad_source_errors),
                            (uint64)vshad_source_errors.GetBufferSize(vshad_source_errors));
                os_graphical_message(1, "Vertex Shader Compilation Failure", errors);
            }
            else
            {
                error = r_d3d11_state.device.CreateVertexShader(r_d3d11_state.device, vshad_source_blob.GetBufferPointer(vshad_source_blob), vshad_source_blob.GetBufferSize(vshad_source_blob), 0, &vshad);
            }
        }
        
        // rjf: make input layout
        ID3D11InputLayout* ilay = null;
        if (ilay_elements != 0)
        {
            error = r_d3d11_state.device.CreateInputLayout(r_d3d11_state.device, ilay_elements, ilay_elements_count,
                                                                vshad_source_blob.GetBufferPointer(vshad_source_blob),
                                                                vshad_source_blob.GetBufferSize(vshad_source_blob),
                                                                &ilay);
        }
        
        vshad_source_blob.Release(vshad_source_blob);
            ...
            kind = (R_D3D11_PShadKind)(kind+1))
    {
        ...
        ID3DBlob* pshad_source_blob = null;
        ID3DBlob* pshad_source_errors = null;
        ID3D11PixelShader* pshad = null;
        ProfScope("compile pixel shader")
        {
            ...
            if (FAILED(error))
            {
                errors = str8((uint8*)pshad_source_errors.GetBufferPointer(pshad_source_errors),
                                            (uint64)pshad_source_errors.GetBufferSize(pshad_source_errors));
                os_graphical_message(1, "Pixel Shader Compilation Failure", errors);
            }
            else
            {
                error = r_d3d11_state.device.CreatePixelShader(r_d3d11_state.device, pshad_source_blob.GetBufferPointer(pshad_source_blob), pshad_source_blob.GetBufferSize(pshad_source_blob), 0, &pshad);
            }
        }
        
        pshad_source_blob.Release(pshad_source_blob);
            ...
            kind = (R_D3D11_UniformTypeKind)(kind+1))
    {
        ID3D11Buffer* buffer = null;
        {
            ...
            r_d3d11_state.device.CreateBuffer(r_d3d11_state.device, &desc, 0, &buffer);
        }
...
[CLink, CallingConvention(.Cdecl)]
public static R_Handle r_window_equip(OS_Handle handle)
{
    ProfBeginFunction();
    R_Handle result = default;
    OS_MutexScopeW(r_d3d11_state.device_rw_mutex)
    {
        ...
        DXGI_SWAP_CHAIN_DESC1 swapchain_desc = default;
        {
        ...
        HRESULT error = r_d3d11_state.dxgi_factory.CreateSwapChainForHwnd(r_d3d11_state.dxgi_factory, (IUnknown*)r_d3d11_state.device, hwnd, &swapchain_desc, 0, 0, &window.swapchain);
        if (FAILED(error))
        {
            char[256] buffer = default;
        ...
        r_d3d11_state.dxgi_factory.MakeWindowAssociation(r_d3d11_state.dxgi_factory, hwnd, DXGI_MWA_NO_ALT_ENTER);
        
        //- rjf: create framebuffer & view
        D3D11_RENDER_TARGET_VIEW_DESC framebuffer_rtv_desc = default;
        ...
        window.swapchain.GetBuffer(window.swapchain, 0, &IID_ID3D11Texture2D, (void**)(&window.framebuffer));
        r_d3d11_state.device.CreateRenderTargetView(r_d3d11_state.device, (ID3D11Resource*)window.framebuffer, &framebuffer_rtv_desc, &window.framebuffer_rtv);
        
        result = r_d3d11_handle_from_window(window);
        r_d3d11_state.window_count += 1;
        r_d3d11_state.dxgi_device.SetMaximumFrameLatency(r_d3d11_state.dxgi_device, Clamp(1, r_d3d11_state.window_count, 16));
    }
    ProfEnd();
    return result;
}
...
[CLink, CallingConvention(.Cdecl)]
public static void r_window_unequip(OS_Handle handle, R_Handle equip_handle)
{
    ProfBeginFunction();
    OS_MutexScopeW(r_d3d11_state.device_rw_mutex)
    {
        R_D3D11_Window* window = r_d3d11_window_from_handle(equip_handle);
        window.stage_color_srv.Release(window.stage_color_srv);
        window.stage_color_rtv.Release(window.stage_color_rtv);
        window.stage_color.Release(window.stage_color);
        window.stage_scratch_color_srv.Release(window.stage_scratch_color_srv);
        window.stage_scratch_color_rtv.Release(window.stage_scratch_color_rtv);
        window.stage_scratch_color.Release(window.stage_scratch_color);
        window.framebuffer_rtv.Release(window.framebuffer_rtv);
        window.framebuffer.Release(window.framebuffer);
        window.swapchain.Release(window.swapchain);
        window.generation += 1;
...
public static R_Handle r_tex2d_alloc(R_ResourceKind kind, Vec2S32 size, R_Tex2DFormat format, void* data)
{
    ...
    DXGI_FORMAT dxgi_format = DXGI_FORMAT_R8G8B8A8_UNORM;
    {
        switch (format)
        {
            case R_Tex2DFormat_R8:     dxgi_format = DXGI_FORMAT_R8_UNORM;
            case R_Tex2DFormat_RG8:    dxgi_format = DXGI_FORMAT_R8G8_UNORM;
            case R_Tex2DFormat_RGBA8:  dxgi_format = DXGI_FORMAT_R8G8B8A8_UNORM;
            case R_Tex2DFormat_BGRA8:  dxgi_format = DXGI_FORMAT_B8G8R8A8_UNORM;
            case R_Tex2DFormat_R16:    dxgi_format = DXGI_FORMAT_R16_UNORM;
            case R_Tex2DFormat_RGBA16: dxgi_format = DXGI_FORMAT_R16G16B16A16_UNORM;
            case R_Tex2DFormat_R32:    dxgi_format = DXGI_FORMAT_R32_FLOAT;
            case R_Tex2DFormat_RG32:   dxgi_format = DXGI_FORMAT_R32G32_FLOAT;
            case R_Tex2DFormat_RGBA32: dxgi_format = DXGI_FORMAT_R32G32B32A32_FLOAT;
            default: {}
        }
    }
    
    //- rjf: prep initial data, if passed
    D3D11_SUBRESOURCE_DATA initial_data_ = default;
    ...
    //- rjf: create texture
    D3D11_TEXTURE2D_DESC texture_desc = default;
    {
    ...
    r_d3d11_state.device.CreateTexture2D(r_d3d11_state.device, &texture_desc, initial_data, &texture.texture);
    
    //- rjf: create texture srv
    r_d3d11_state.device.CreateShaderResourceView(r_d3d11_state.device, (ID3D11Resource*)texture.texture, 0, &texture.view);
...
[CLink, CallingConvention(.Cdecl)]
public static void r_tex2d_release(R_Handle handle)
{
    ProfBeginFunction();
    OS_MutexScopeW(r_d3d11_state.device_rw_mutex)
    {
...
[CLink, CallingConvention(.Cdecl)]
public static R_ResourceKind r_kind_from_tex2d(R_Handle handle)
{
    R_D3D11_Tex2D* texture = r_d3d11_tex2d_from_handle(handle);
    return texture.kind;
}
...
[CLink, CallingConvention(.Cdecl)]
public static Vec2S32 r_size_from_tex2d(R_Handle handle)
{
    R_D3D11_Tex2D* texture = r_d3d11_tex2d_from_handle(handle);
    return texture.size;
}
...
[CLink, CallingConvention(.Cdecl)]
public static R_Tex2DFormat r_format_from_tex2d(R_Handle handle)
{
    R_D3D11_Tex2D* texture = r_d3d11_tex2d_from_handle(handle);
...
[CLink, CallingConvention(.Cdecl)]
public static void r_fill_tex2d_region(R_Handle handle, Rng2S32 subrect, void* data)
{
    ProfBeginFunction();
    OS_MutexScopeW(r_d3d11_state.device_rw_mutex)
    {
        ...
        if (texture != &r_d3d11_tex2d_nil)
        {
            ...
            r_d3d11_state.device_ctx.UpdateSubresource(r_d3d11_state.device_ctx, (ID3D11Resource *)texture.texture, 0, &dst_box, data, dim.x*bytes_per_pixel, 0);
        }
    }
    ProfEnd();
}
...
public static R_Handle r_buffer_alloc(R_ResourceKind kind, uint64 size, void* data)
{
    ...
    r_d3d11_state.device.CreateBuffer(r_d3d11_state.device, &desc, initial_data, &buffer.buffer);
...
[CLink, CallingConvention(.Cdecl)]
public static void r_buffer_release(R_Handle handle)
{
    ProfBeginFunction();
    OS_MutexScopeW(r_d3d11_state.device_rw_mutex)
    {
...
public static void r_end_frame()
{
    OS_MutexScopeW(r_d3d11_state.device_rw_mutex)
    {
        ...
        {
            buffer.buffer.Release(buffer.buffer);
        }
        for (R_D3D11_Tex2D* tex = r_d3d11_state.first_to_free_tex2d, next = null;
                tex != null;
                tex = next)
        {
            next = tex.next;
            tex.view.Release(tex.view);
            tex.texture.Release(tex.texture);
            tex.view = 0;
            tex.texture = 0;
            tex.generation += 1;
            SLLStackPush(r_d3d11_state.first_free_tex2d, tex);
        }
        for (R_D3D11_Buffer* buf = r_d3d11_state.first_to_free_buffer, next = null;
                buf != null;
                buf = next)
        {
            next = buf.next;
            buf.buffer.Release(buf.buffer);
            buf.generation += 1;
...
public static void r_window_begin_frame(OS_Handle window, R_Handle window_equip)
{
    ...
    OS_MutexScopeW(r_d3d11_state.device_rw_mutex)
    {
              ...
              wnd.last_resolution.y != resolution.y)
        {
            ...
            if (wnd.stage_scratch_color_srv){wnd.stage_scratch_color_srv.Release(wnd.stage_scratch_color_srv);}
            if (wnd.stage_scratch_color_rtv){wnd.stage_scratch_color_rtv.Release(wnd.stage_scratch_color_rtv);}
            if (wnd.stage_scratch_color)    {wnd.stage_scratch_color.Release(wnd.stage_scratch_color);}
            if (wnd.stage_color_srv)        {wnd.stage_color_srv.Release(wnd.stage_color_srv);}
            if (wnd.stage_color_rtv)        {wnd.stage_color_rtv.Release(wnd.stage_color_rtv);}
            if (wnd.stage_color)            {wnd.stage_color.Release(wnd.stage_color);}
            if (wnd.geo3d_color_srv)        {wnd.geo3d_color_srv.Release(wnd.geo3d_color_srv);}
            if (wnd.geo3d_color_rtv)        {wnd.geo3d_color_rtv.Release(wnd.geo3d_color_rtv);}
            if (wnd.geo3d_color)            {wnd.geo3d_color.Release(wnd.geo3d_color);}
            if (wnd.geo3d_depth_srv)        {wnd.geo3d_depth_srv.Release(wnd.geo3d_depth_srv);}
            if (wnd.geo3d_depth_dsv)        {wnd.geo3d_depth_dsv.Release(wnd.geo3d_depth_dsv);}
            if (wnd.geo3d_depth)            {wnd.geo3d_depth.Release(wnd.geo3d_depth);}
            
            // rjf: resize swapchain & main framebuffer
            wnd.framebuffer_rtv.Release(wnd.framebuffer_rtv);
            wnd.framebuffer.Release(wnd.framebuffer);
            wnd.swapchain.ResizeBuffers(wnd.swapchain, 0, 0, 0, DXGI_FORMAT_UNKNOWN, 0);
            wnd.swapchain.GetBuffer(wnd.swapchain, 0, &IID_ID3D11Texture2D, (void**)(&wnd.framebuffer));
            D3D11_RENDER_TARGET_VIEW_DESC framebuffer_rtv_desc = default;
            framebuffer_rtv_desc.Format        = DXGI_FORMAT_B8G8R8A8_UNORM_SRGB;
            framebuffer_rtv_desc.ViewDimension = D3D11_RTV_DIMENSION_TEXTURE2D;
            r_d3d11_state.device.CreateRenderTargetView(r_d3d11_state.device, (ID3D11Resource*)wnd.framebuffer, &framebuffer_rtv_desc, &wnd.framebuffer_rtv);
            
            // rjf: create stage color targets
            {
                D3D11_TEXTURE2D_DESC color_desc = default;
                {
                    wnd.framebuffer.GetDesc(wnd.framebuffer, &color_desc);
                    color_desc.Format = DXGI_FORMAT_R16G16B16A16_FLOAT;
                    color_desc.BindFlags = D3D11_BIND_RENDER_TARGET|D3D11_BIND_SHADER_RESOURCE;
                }
                D3D11_RENDER_TARGET_VIEW_DESC rtv_desc = default;
                {
                    rtv_desc.Format         = color_desc.Format;
                    rtv_desc.ViewDimension  = D3D11_RTV_DIMENSION_TEXTURE2D;
                }
                D3D11_SHADER_RESOURCE_VIEW_DESC srv_desc = default;
                {
                    srv_desc.Format                    = DXGI_FORMAT_R16G16B16A16_FLOAT;
                    srv_desc.ViewDimension             = D3D11_SRV_DIMENSION_TEXTURE2D;
                    srv_desc.Texture2D.MipLevels       = -1;
                }
                r_d3d11_state.device.CreateTexture2D(r_d3d11_state.device, &color_desc, 0, &wnd.stage_color);
                r_d3d11_state.device.CreateRenderTargetView(r_d3d11_state.device, (ID3D11Resource*)wnd.stage_color, &rtv_desc, &wnd.stage_color_rtv);
                r_d3d11_state.device.CreateShaderResourceView(r_d3d11_state.device, (ID3D11Resource*)wnd.stage_color, &srv_desc, &wnd.stage_color_srv);
                r_d3d11_state.device.CreateTexture2D(r_d3d11_state.device, &color_desc, 0, &wnd.stage_scratch_color);
                r_d3d11_state.device.CreateRenderTargetView(r_d3d11_state.device, (ID3D11Resource*)wnd.stage_scratch_color, &rtv_desc, &wnd.stage_scratch_color_rtv);
                r_d3d11_state.device.CreateShaderResourceView(r_d3d11_state.device, (ID3D11Resource*)wnd.stage_scratch_color, &srv_desc, &wnd.stage_scratch_color_srv);
            }
            ...
            // rjf: create geo3d targets
            {
                ...
                D3D11_RENDER_TARGET_VIEW_DESC color_rtv_desc = default;
                {
                    color_rtv_desc.Format         = color_desc.Format;
                    color_rtv_desc.ViewDimension  = D3D11_RTV_DIMENSION_TEXTURE2D;
                }
                D3D11_SHADER_RESOURCE_VIEW_DESC color_srv_desc = default;
                {
                ...
                D3D11_TEXTURE2D_DESC depth_desc = default;
                {
                    wnd.framebuffer.GetDesc(wnd.framebuffer, &depth_desc);
                    depth_desc.Format = DXGI_FORMAT_R24G8_TYPELESS;
                    depth_desc.BindFlags = D3D11_BIND_DEPTH_STENCIL|D3D11_BIND_SHADER_RESOURCE;
                }
                D3D11_DEPTH_STENCIL_VIEW_DESC depth_dsv_desc = default;
                {
                ...
                D3D11_SHADER_RESOURCE_VIEW_DESC depth_srv_desc = default;
                {
                ...
                r_d3d11_state.device.CreateTexture2D(r_d3d11_state.device, &color_desc, 0, &wnd.geo3d_color);
                r_d3d11_state.device.CreateRenderTargetView(r_d3d11_state.device, (ID3D11Resource*)wnd.geo3d_color, &color_rtv_desc, &wnd.geo3d_color_rtv);
                r_d3d11_state.device.CreateShaderResourceView(r_d3d11_state.device, (ID3D11Resource*)wnd.geo3d_color, &color_srv_desc, &wnd.geo3d_color_srv);
                r_d3d11_state.device.CreateTexture2D(r_d3d11_state.device, &depth_desc, 0, &wnd.geo3d_depth);
                r_d3d11_state.device.CreateDepthStencilView(r_d3d11_state.device, (ID3D11Resource*)wnd.geo3d_depth, &depth_dsv_desc, &wnd.geo3d_depth_dsv);
                r_d3d11_state.device.CreateShaderResourceView(r_d3d11_state.device, (ID3D11Resource*)wnd.geo3d_depth, &depth_srv_desc, &wnd.geo3d_depth_srv);
            }
        }
        
        //- rjf: clear framebuffers
        Vec4F32 clear_color = {0, 0, 0, 0};
        d_ctx.ClearRenderTargetView(d_ctx, wnd.framebuffer_rtv, clear_color.v);
        d_ctx.ClearRenderTargetView(d_ctx, wnd.stage_color_rtv, clear_color.v);
        if (resize_done)
        {
            d_ctx.Flush(d_ctx);
        }
...
public static void r_window_end_frame(OS_Handle window, R_Handle window_equip)
{
    ...
    OS_MutexScopeW(r_d3d11_state.device_rw_mutex)
    {
        ...
        //
        {
            ...
            d_ctx.OMSetRenderTargets(d_ctx, 1, &wnd.framebuffer_rtv, 0);
            d_ctx.OMSetDepthStencilState(d_ctx, r_d3d11_state.noop_depth_stencil, 0);
            d_ctx.OMSetBlendState(d_ctx, r_d3d11_state.main_blend_state, 0, 0xffffffff);
            ...
            d_ctx.RSSetViewports(d_ctx, 1, &viewport);
            d_ctx.RSSetState(d_ctx, (ID3D11RasterizerState *)r_d3d11_state.main_rasterizer);
            
            // rjf: setup input assembly
            d_ctx.IASetPrimitiveTopology(d_ctx, D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP);
            d_ctx.IASetInputLayout(d_ctx, 0);
            
            // rjf: setup shaders
            d_ctx.VSSetShader(d_ctx, vshad, 0, 0);
            d_ctx.PSSetShader(d_ctx, pshad, 0, 0);
            d_ctx.PSSetShaderResources(d_ctx, 0, 1, &wnd.stage_color_srv);
            d_ctx.PSSetSamplers(d_ctx, 0, 1, &sampler);
            
            // rjf: setup scissor rect
            {
                D3D11_RECT rect = default;
                rect.left = 0;
                rect.right = (LONG)wnd.last_resolution.x;
                rect.top = 0;
                rect.bottom = (LONG)wnd.last_resolution.y;
                d_ctx.RSSetScissorRects(d_ctx, 1, &rect);
            }
            
            // rjf: draw
            d_ctx.Draw(d_ctx, 4, 0);
        }
        
        ////////////////////////////
        //- rjf: present
        //
        HRESULT error = wnd.swapchain.Present(wnd.swapchain, 1, 0);
        if (FAILED(error))
        {
            char8[256] buffer = default;
...
public static void r_window_submit(OS_Handle window, R_Handle window_equip, R_PassList* passes)
{
    ...
    OS_MutexScopeW(r_d3d11_state.device_rw_mutex)
    {
        ...
        //- rjf: do passes
        //
        for (R_PassNode* pass_n = passes.first; pass_n != null; pass_n = pass_n.next)
        {
            ...
            switch (pass.kind)
            {
                ...
                case R_PassKind_UI:
                {
                    ...
                    for (R_BatchGroup2DNode* group_n = rect_batch_groups.first; group_n != null; group_n = group_n.next)
                    {
                        ...
                        {
                            D3D11_MAPPED_SUBRESOURCE sub_rsrc = default;
                            d_ctx.Map(d_ctx, (ID3D11Resource*)buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &sub_rsrc);
                            uint8* dst_ptr = (uint8*)sub_rsrc.pData;
                            uint64 off = 0;
                            for (R_BatchNode* batch_n = batches.first; batch_n != null; batch_n = batch_n.next)
                            {
                                MemoryCopy(dst_ptr+off, batch_n.v.v, batch_n.v.byte_count);
                                off += batch_n.v.byte_count;
                            }
                            d_ctx.Unmap(d_ctx, (ID3D11Resource*)buffer, 0);
                        }
                        ...
                        // rjf: upload uniforms
                        R_D3D11_Uniforms_Rect uniforms = default;
                        {
                        ...
                        {
                            D3D11_MAPPED_SUBRESOURCE sub_rsrc = default;
                            d_ctx.Map(d_ctx, (ID3D11Resource *)uniforms_buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &sub_rsrc);
                            MemoryCopy((uint8 *)sub_rsrc.pData, &uniforms, sizeof(uniforms));
                            d_ctx.Unmap(d_ctx, (ID3D11Resource *)uniforms_buffer, 0);
                        }
                        
                        // rjf: setup output merger
                        d_ctx.OMSetRenderTargets(d_ctx, 1, &wnd.stage_color_rtv, 0);
                        d_ctx.OMSetDepthStencilState(d_ctx, r_d3d11_state.noop_depth_stencil, 0);
                        d_ctx.OMSetBlendState(d_ctx, r_d3d11_state.main_blend_state, 0, 0xffffffff);
                        ...
                        uint32 offset = 0;
                        d_ctx.IASetPrimitiveTopology(d_ctx, D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP);
                        d_ctx.IASetInputLayout(d_ctx, ilay);
                        d_ctx.IASetVertexBuffers(d_ctx, 0, 1, &buffer, &stride, &offset);
                        
                        // rjf: setup shaders
                        d_ctx.VSSetShader(d_ctx, vshad, 0, 0);
                        d_ctx.VSSetConstantBuffers(d_ctx, 0, 1, &uniforms_buffer);
                        d_ctx.PSSetShader(d_ctx, pshad, 0, 0);
                        d_ctx.PSSetConstantBuffers(d_ctx, 0, 1, &uniforms_buffer);
                        d_ctx.PSSetShaderResources(d_ctx, 0, 1, &texture.view);
                        d_ctx.PSSetSamplers(d_ctx, 0, 1, &sampler);
                        ...
                        // rjf: draw
                        d_ctx.DrawInstanced(d_ctx, 4, batches.byte_count / batches.bytes_per_inst, 0, 0);
                    }
                }
                ...
                case R_PassKind_Blur:
                {
                    ...
                    // rjf: setup output merger
                    d_ctx.OMSetDepthStencilState(d_ctx, r_d3d11_state.noop_depth_stencil, 0);
                    d_ctx.OMSetBlendState(d_ctx, r_d3d11_state.no_blend_state, 0, 0xffffffff);
                    ...
                    {
                        float[ArrayCount(uniforms.kernel)*2] weights = default;
                    ...
                    uint32[Axis2_COUNT][2] uniform_offset =
                    .(
                    ...
                    uint32[Axis2_COUNT][2] uniform_count =
                    .(
                    ...
                    // horizontal pass
                    d_ctx.OMSetRenderTargets(d_ctx, 1, &wnd.stage_scratch_color_rtv, 0);
                    d_ctx.PSSetConstantBuffers1(d_ctx, 0, ArrayCount(uniforms_buffers), uniforms_buffers, uniform_offset[Axis2_X], uniform_count[Axis2_X]);
                    d_ctx.PSSetShaderResources(d_ctx, 0, 1, &wnd.stage_color_srv);
                    d_ctx.Draw(d_ctx, 4, 0);
                    d_ctx.PSSetShaderResources(d_ctx, 0, 1, &srv);
                    
                    // vertical pass
                    d_ctx.OMSetRenderTargets(d_ctx, 1, &wnd.stage_color_rtv, 0);
                    d_ctx.PSSetConstantBuffers1(d_ctx, 0, ArrayCount(uniforms_buffers), uniforms_buffers, uniform_offset[Axis2_Y], uniform_count[Axis2_Y]);
                    d_ctx.PSSetShaderResources(d_ctx, 0, 1, &wnd.stage_scratch_color_srv);
                    d_ctx.Draw(d_ctx, 4, 0);
                    d_ctx.PSSetShaderResources(d_ctx, 0, 1, &srv);
                }
                ...
                case R_PassKind_Geo3D:
                {
                    ...
                    //- rjf: clear render targets
                    {
                        ...
                        d_ctx.ClearRenderTargetView(d_ctx, wnd.geo3d_color_rtv, bg_color.v);
                        d_ctx.ClearDepthStencilView(d_ctx, wnd.geo3d_depth_dsv, D3D11_CLEAR_DEPTH, 1.f, 0);
                    }
                    ...
                    //- rjf: draw mesh batches
                    {
                        ...
                        // rjf: setup output merger
                        d_ctx.OMSetRenderTargets(d_ctx, 1, &wnd.geo3d_color_rtv, wnd.geo3d_depth_dsv);
                        d_ctx.OMSetDepthStencilState(d_ctx, r_d3d11_state.plain_depth_stencil, 0);
                        d_ctx.OMSetBlendState(d_ctx, r_d3d11_state.main_blend_state, 0, 0xffffffff);
                        
                        // rjf: draw all batches
                        for (uint64 slot_idx = 0; slot_idx < mesh_group_map.slots_count; slot_idx += 1)
                        {
                            for (R_BatchGroup3DMapNode* n = mesh_group_map.slots[slot_idx]; n != null; n = n.next)
                            {
                                ...
                                d_ctx.IASetInputLayout(d_ctx, ilay);
                                d_ctx.IASetVertexBuffers(d_ctx, 0, 1, &mesh_vertices.buffer, &stride, &offset);
                                d_ctx.IASetIndexBuffer(d_ctx, mesh_indices.buffer, DXGI_FORMAT_R32_UINT, 0);
                                
                                // rjf: setup uniforms buffer
                                R_D3D11_Uniforms_Mesh uniforms = default;
                                {
                                ...
                                // rjf: draw
                                d_ctx.DrawIndexed(d_ctx, mesh_indices.size/sizeof(uint32), 0, 0);
                            }
                        }
                    }
                    ...
                    //- rjf: composite to main staging buffer
                    {
                        ...
                        d_ctx.VSSetShader(d_ctx, vshad, 0, 0);
                        d_ctx.PSSetShader(d_ctx, pshad, 0, 0);
                        d_ctx.PSSetShaderResources(d_ctx, 0, 1, &wnd.geo3d_color_srv);
                        d_ctx.PSSetSamplers(d_ctx, 0, 1, &sampler);
