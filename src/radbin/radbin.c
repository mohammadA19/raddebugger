  ...
  ProfScope("analyze & load command line input files")
  {
    ...
    for(String8Node *n = input_file_path_tasks.first; n != 0; n = n->next)
    {
      ...
      ProfScope("do thin analysis of file")
      {
        ...
        if(file_format == RB_FileFormat_Null)
        {
          uint8 coff_archive_sig_maybe[sizeof(g_coff_archive_sig)] = {0};
        ...
        if(file_format == RB_FileFormat_Null)
        {
          uint8 coff_thin_archive_sig_maybe[sizeof(g_coff_thin_archive_sig)] = {0};
        ...
        if(file_format == RB_FileFormat_Null)
        {
          ...
          os_file_read_struct(file, 0, &header_maybe);
          uint64 section_count = header_maybe.section_count;
          uint64 section_hdr_opl_off = sizeof(header_maybe) + section_count*sizeof(COFF_SectionHeader);
          ...
          {
            uint64 symbol_table_off = header_maybe.symbol_table_foff;
            uint64 symbol_table_size = sizeof(COFF_Symbol16)*header_maybe.symbol_count;
        ...
        if(file_format == RB_FileFormat_Null)
        {
          uint8 identifier_maybe[ELF_Identifier_Max] = {0};
          ...
          if(is_elf_magic)
          {
            file_format = ELF_HdrIs64Bit(identifier_maybe) ? RB_FileFormat_ELdouble : RB_FileFormat_ELfloat;
          }
        }
      ...
      //
      if(file_format == RB_FileFormat_ELfloat ||
         file_format == RB_FileFormat_ELdouble)
      {
...
#undef X
      ...
      RDIM_BakeParams bake_params = {0};
      {
        ...
        if(!convert_done &&
           ((input_files_from_format_table[RB_FileFormat_PE].count != 0 &&
            ...
            (input_files_from_format_table[RB_FileFormat_ELfloat].count != 0 ||
             input_files_from_format_table[RB_FileFormat_ELdouble].count != 0)))
        {
          convert_done = 1;
          ...
          D2R_ConvertParams convert_params = {0};
          {
            ...
            if(!got_exe)
            {
              for(RB_FileNode *n = input_files_from_format_table[RB_FileFormat_ELfloat].first; n != 0; n = n->next)
              {
                got_exe = 1;
                convert_params.exe_name = n->v->path;
                convert_params.exe_data = n->v->data;
            ...
            if(!got_exe)
            {
              for(RB_FileNode *n = input_files_from_format_table[RB_FileFormat_ELdouble].first; n != 0; n = n->next)
              {
                got_exe = 1;
                convert_params.exe_name = n->v->path;
                convert_params.exe_data = n->v->data;
      ...
      if(convert_done) switch(output_kind)
      {
        ...
        case OutputKind_Breakpad:
        {
          ...
          uint64 unit_count = bake_params.units.total_count;
          uint32 *unit_line_table_idxs = push_array(arena, uint32, unit_count+1);
          {
            uint64 dst_idx = 1;
            for(RDIM_UnitChunkNode *n = bake_params.units.first; n != 0; n = n->next)
            {
              for(uint64 n_idx = 0; n_idx < n->count; n_idx += 1, dst_idx += 1)
              {
          ...
          uint32 unit_vmap_count = bake_unit_vmap_out->vmap.count;
          ProfEnd();
          ...
          {
            for(uint64 idx = 0; idx < bake_params.procedures.chunk_count; idx += 1)
            {
...
#undef X
      ...
      for(RB_FileNode *n = input_files.first; n != 0; n = n->next)
      {
        ...
        DW_Input dw = {0};
        {
          ...
          if(f->format == RB_FileFormat_ELfloat ||
             f->format == RB_FileFormat_ELdouble)
          {
          ...
          if(f->format_flags & RB_FileFormatFlag_HasDWARF)
          {
            ...
            else if(f->format == RB_FileFormat_ELfloat ||
                    f->format == RB_FileFormat_ELdouble)
            {
        ...
        switch(f->format)
        {
          ...
          //- rjf: ELF
          case RB_FileFormat_ELfloat:
          case RB_FileFormat_ELdouble:
          {
            // TODO(rjf)
          }break;
  ...
  else ProfScope("write outputs [stdout]")
  {
    ...
    {
      for(uint64 off = 0; off < n->string.size;)
      {
        uint64 size_to_write = Min(n->string.size - off, GB(2));
