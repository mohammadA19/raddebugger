...
internal void md_token_chunk_list_push(Arena *arena, MD_TokenChunkList *list, ulong cap, MD_Token token);
...
internal MD_NodeRec md_node_rec_depth_first(MD_Node *node, MD_Node *subtree_root, ulong child_off, ulong sib_off);
...
//- rjf: tree building
internal MD_Node *md_push_node(Arena *arena, MD_NodeKind kind, MD_NodeFlags flags, String8 string, String8 raw_string, ulong src_offset);
...
internal MD_Node *  md_node_from_chain_index(MD_Node *first, MD_Node *opl, ulong index);
internal MD_Node *  md_node_from_chain_flags(MD_Node *first, MD_Node *opl, MD_NodeFlags flags);
internal ulong        md_index_from_node(MD_Node *node);
...
internal MD_Node *  md_child_from_index(MD_Node *node, ulong index);
internal MD_Node *  md_tag_from_index(MD_Node *node, ulong index);
internal MD_Node *  md_tag_arg_from_index(MD_Node *node, String8 tag_string, StringMatchFlags flags, ulong index);
...
internal ulong        md_child_count_from_node(MD_Node *node);
internal ulong        md_tag_count_from_node(MD_Node *node);
