...
static void md_msg_list_push(Arena *arena, MD_MsgList *msgs, MD_Node *node, MD_MsgKind kind, String8 string);
static void md_msg_list_pushf(Arena *arena, MD_MsgList *msgs, MD_Node *node, MD_MsgKind kind, char *fmt, ...);
static void md_msg_list_concat_in_place(MD_MsgList *dst, MD_MsgList *to_push);

////////////////////////////////
//~ rjf: Token Type Functions

static MD_Token md_token_make(Rng1U64 range, MD_TokenFlags flags);
static B32 md_token_match(MD_Token a, MD_Token b);
static String8List md_string_list_from_token_flags(Arena *arena, MD_TokenFlags flags);
static void md_token_chunk_list_push(Arena *arena, MD_TokenChunkList *list, U64 cap, MD_Token token);
static MD_TokenArray md_token_array_from_chunk_list(Arena *arena, MD_TokenChunkList *chunks);
static String8 md_content_string_from_token_flags_str8(MD_TokenFlags flags, String8 string);

////////////////////////////////
//~ rjf: Node Type Functions

//- rjf: flag conversions
static MD_NodeFlags md_node_flags_from_token_flags(MD_TokenFlags flags);

//- rjf: nil
static B32 md_node_is_nil(MD_Node *node);

//- rjf: iteration
#define MD_EachNode(it, first) (MD_Node *it = first; !md_node_is_nil(it); it = it->next)
static MD_NodeRec md_node_rec_depth_first(MD_Node *node, MD_Node *subtree_root, U64 child_off, U64 sib_off);
...
//- rjf: tree building
static MD_Node *md_push_node(Arena *arena, MD_NodeKind kind, MD_NodeFlags flags, String8 string, String8 raw_string, U64 src_offset);
static void md_node_insert_child(MD_Node *parent, MD_Node *prev_child, MD_Node *node);
static void md_node_insert_tag(MD_Node *parent, MD_Node *prev_child, MD_Node *node);
static void md_node_push_child(MD_Node *parent, MD_Node *node);
static void md_node_push_tag(MD_Node *parent, MD_Node *node);
static void md_unhook(MD_Node *node);

//- rjf: tree introspection
static MD_Node *  md_node_from_chain_string(MD_Node *first, MD_Node *opl, String8 string, StringMatchFlags flags);
static MD_Node *  md_node_from_chain_index(MD_Node *first, MD_Node *opl, U64 index);
static MD_Node *  md_node_from_chain_flags(MD_Node *first, MD_Node *opl, MD_NodeFlags flags);
static U64        md_index_from_node(MD_Node *node);
static MD_Node *  md_root_from_node(MD_Node *node);
static MD_Node *  md_child_from_string(MD_Node *node, String8 child_string, StringMatchFlags flags);
static MD_Node *  md_tag_from_string(MD_Node *node, String8 tag_string, StringMatchFlags flags);
static MD_Node *  md_child_from_index(MD_Node *node, U64 index);
static MD_Node *  md_tag_from_index(MD_Node *node, U64 index);
static MD_Node *  md_tag_arg_from_index(MD_Node *node, String8 tag_string, StringMatchFlags flags, U64 index);
static MD_Node *  md_tag_arg_from_string(MD_Node *node, String8 tag_string, StringMatchFlags tag_str_flags, String8 arg_string, StringMatchFlags arg_str_flags);
static B32        md_node_has_child(MD_Node *node, String8 string, StringMatchFlags flags);
static B32        md_node_has_tag(MD_Node *node, String8 string, StringMatchFlags flags);
static U64        md_child_count_from_node(MD_Node *node);
static U64        md_tag_count_from_node(MD_Node *node);
static String8    md_string_from_children(Arena *arena, MD_Node *root);

//- rjf: tree comparison
static B32 md_tree_match(MD_Node *a, MD_Node *b, StringMatchFlags flags);
static B32 md_node_match(MD_Node *a, MD_Node *b, StringMatchFlags flags);

//- rjf: tree duplication
static MD_Node *md_tree_copy(Arena *arena, MD_Node *src_root);

////////////////////////////////
//~ rjf: Text -> Tokens Functions

static MD_TokenizeResult md_tokenize_from_text(Arena *arena, String8 text);

////////////////////////////////
//~ rjf: Tokens -> Tree Functions

static MD_ParseResult md_parse_from_text_tokens(Arena *arena, String8 filename, String8 text, MD_TokenArray tokens);

////////////////////////////////
//~ rjf: Bundled Text -> Tree Functions

static MD_ParseResult md_parse_from_text(Arena *arena, String8 filename, String8 text);
#define md_tree_from_string(arena, string) (md_parse_from_text((arena), str8_zero(), (string)).root)

////////////////////////////////
//~ rjf: Tree -> Text Functions

static String8List md_debug_string_list_from_tree(Arena *arena, MD_Node *root);

////////////////////////////////
//~ rjf: Node Pointer List Functions

static void md_node_ptr_list_push(Arena *arena, MD_NodePtrList *list, MD_Node *node);
static void md_node_ptr_list_push_front(Arena *arena, MD_NodePtrList *list, MD_Node *node);
