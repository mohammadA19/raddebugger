...
DASM_Inst
dasm_inst_from_code(Arena *arena, Arch arch, ulong vaddr, String8 code, DASM_Syntax syntax)
{
  DASM_Inst inst = {0};
  switch(arch)
  {
...
DASM_CtrlFlowInfo
dasm_ctrl_flow_info_from_arch_vaddr_code(Arena *arena, DASM_InstFlags exit_points_mask, Arch arch, ulong vaddr, String8 code)
{
  Temp scratch = scratch_begin(&arena, 1);
  DASM_CtrlFlowInfo info = {0};
  for(ulong offset = 0; offset < code.size;)
  {
    DASM_Inst inst = dasm_inst_from_code(scratch.arena, arch, vaddr+offset, str8_skip(code, offset), DASM_Syntax_Intel);
    ulong inst_vaddr = vaddr+offset;
    offset += inst.size;
...
void
dasm_line_chunk_list_push(Arena *arena, DASM_LineChunkList *list, ulong cap, DASM_Line *inst)
{
...
ulong
dasm_line_array_idx_from_code_off__linear_scan(DASM_LineArray *array, ulong off)
{
  ulong result = 0;
  for(ulong idx = 0; idx < array->count; idx += 1)
  {
    ulong next_off = (idx+1 < array->count ? array->v[idx+1].code_off : max_U64);
...
ulong
dasm_line_array_code_off_from_idx(DASM_LineArray *array, ulong idx)
{
  ulong off = 0;
  if(idx < array->count)
  {
...
dasm_init()
{
  ...
  for(ulong idx = 0; idx < dasm_shared->stripes_count; idx += 1)
  {
...
dasm_scope_open()
{
  ...
  ulong base_pos = arena_pos(dasm_tctx->arena);
...
dasm_scope_close(DASM_Scope *scope)
{
  ...
  {
    next = t->next;
    ulong slot_idx = t->hash.u64[1]%dasm_shared->slots_count;
    ulong stripe_idx = slot_idx%dasm_shared->stripes_count;
...
dasm_info_from_hash_params(DASM_Scope *scope, U128 hash, DASM_Params *params)
{
  ...
  {
    ulong slot_idx = hash.u64[1]%dasm_shared->slots_count;
    ulong stripe_idx = slot_idx%dasm_shared->stripes_count;
    DASM_Slot *slot = &dasm_shared->slots[slot_idx];
    DASM_Stripe *stripe = &dasm_shared->stripes[stripe_idx];
    B32 found = 0;
...
B32
dasm_u2p_enqueue_req(U128 hash, DASM_Params *params, ulong endt_us)
{
  B32 good = 0;
  OS_MutexScope(dasm_shared->u2p_ring_mutex) for(;;)
  {
    ulong unconsumed_size = dasm_shared->u2p_ring_write_pos - dasm_shared->u2p_ring_read_pos;
    ulong available_size = dasm_shared->u2p_ring_size - unconsumed_size;
    if(available_size >= sizeof(hash)+sizeof(ulong)+sizeof(Arch)+sizeof(DASM_StyleFlags)+sizeof(DASM_Syntax)+sizeof(ulong)+sizeof(ulong)+params->dbgi_key.path.size+sizeof(ulong))
    {
      good = 1;
...
dasm_u2p_dequeue_req(Arena *arena, U128 *hash_out, DASM_Params *params_out)
{
  ...
  {
    ulong unconsumed_size = dasm_shared->u2p_ring_write_pos - dasm_shared->u2p_ring_read_pos;
    if(unconsumed_size >= sizeof(*hash_out)+sizeof(ulong)+sizeof(Arch)+sizeof(DASM_StyleFlags)+sizeof(DASM_Syntax)+sizeof(ulong)+sizeof(ulong)+sizeof(ulong))
    {
...
ASYNC_WORK_DEF(dasm_parse_work)
{
  ...
  ulong change_gen = fs_change_gen();
      ...
      case Arch_x86:
      {
        ...
        for(ulong off = 0; off < data.size;)
        {
          ...
          if(params.style_flags & (DASM_StyleFlag_SourceFilesNames|DASM_StyleFlag_SourceLines))
          {
            ...
            {
              ulong voff = (params.vaddr+off) - params.base_vaddr;
              uint unit_idx = rdi_vmap_idx_from_section_kind_voff(rdi, RDI_SectionKind_UnitVMap, voff);
              ...
              ulong line_info_idx = rdi_line_info_idx_from_voff(&unit_line_info, voff);
          ...
          if(params.style_flags & DASM_StyleFlag_CodeBytes)
          {
            ...
            for(ulong byte_idx = 0; byte_idx < inst.size || byte_idx < 16; byte_idx += 1)
            {
              if(byte_idx < inst.size)
              {
                str8_list_pushf(scratch.arena, &code_bytes_strings, "%02x%s ", (uint)data.str[off+byte_idx], byte_idx == inst.size-1 ? "}" : "");
              }
              else if(byte_idx < 8)
              {
                str8_list_push(scratch.arena, &code_bytes_strings, str8_lit("   "));
...
dasm_evictor_detector_thread__entry_point(void *p)
{
  ...
  for(;;)
  {
    ...
    ulong retry_threshold_us =  1*1000000;
    ulong evict_threshold_user_clocks = 10;
    ulong retry_threshold_user_clocks = 10;
    for(ulong slot_idx = 0; slot_idx < dasm_shared->slots_count; slot_idx += 1)
    {
      ulong stripe_idx = slot_idx%dasm_shared->stripes_count;
      DASM_Slot *slot = &dasm_shared->slots[slot_idx];
      DASM_Stripe *stripe = &dasm_shared->stripes[stripe_idx];
