...
{
  string delay_load_helper_name = str8_zero();
  switch (machine) {
                                   ...
                                   Rng1U64             except_frange)
{
  ...
  string        raw_pdata   = str8_substr(raw_data, except_frange);
...
mscrt_string_from_eh_adjectives(Arena* arena, MSCRT_EhHandlerTypeFlags adjectives)
{
  ...
  string result = str8_list_join(arena, &adj_list, &(StringJoin){.sep=(", ")});
  scratch_end(scratch);
  return result;
}
