...
internal uint64
mscrt_v4_parse_s32(String8 raw_data, uint64 offset, uint32 *int_out)
{
                            ...
                            MSCRT_IP2State32V4 *ip2state_map_out)
{
  ...
  uint32 *states = push_array(arena, uint32, count);
  ...
  for (uint32 i = 0; i < count; ++i) {
    ...
    states[i] = (uint32)encoded_state - 1;
  }
