...
RDI_PROC void *
rdim_memset_fallback(void *dst, RDI_uint8 c, RDI_uint64 size)
{
  for(RDI_uint64 idx = 0; idx < size; idx += 1)
  {
    ((RDI_uint8 *)dst)[idx] = c;
  }
  return dst;
...
RDI_PROC void *
rdim_memcpy_fallback(void *dst, void *src, RDI_uint64 size)
{
  for(RDI_uint64 idx = 0; idx < size; idx += 1)
  {
    ((RDI_uint8 *)dst)[idx] = ((RDI_uint8 *)src)[idx];
  }
  return dst;
}
#endif
...
RDI_PROC void *
rdim_arena_push_fallback(RDIM_Arena *arena, RDI_uint64 size)
{
  // TODO(rjf)
  return 0;
}
...
RDI_PROC void
rdim_arena_pop_to_fallback(RDIM_Arena *arena, RDI_uint64 pos)
{
  // TODO(rjf)
}
...
RDI_PROC RDIM_Temp
rdim_scratch_begin_fallback(RDIM_Arena **conflicts, RDI_uint64 conflicts_count)
{
  ...
  RDIM_Arena *arena = 0;
  for(RDI_uint64 scratch_idx = 0;
      scratch_idx < sizeof(rdim_thread_scratches)/sizeof(rdim_thread_scratches[0]);
      scratch_idx += 1)
  {
    RDI_uint32 scratch_conflicts = 0;
    for(RDI_uint64 conflict_idx = 0; conflict_idx < conflicts_count; conflict_idx += 1)
    {
...
rdim_str8(RDI_uint8 *str, RDI_uint64 size)
{
  RDIM_String8 result;
...
rdim_str8_copy(RDIM_Arena *arena, RDIM_String8 src)
{
  ...
  dst.RDIM_String8_BaseMember = rdim_push_array_no_zero(arena, RDI_uint8, dst.RDIM_String8_SizeMember+1);
...
rdim_str8fv(RDIM_Arena *arena, char *fmt, va_list args)
{
  va_list args2;
  va_copy(args2, args);
  RDI_uint32 needed_bytes = rdim_vsnprintf(0, 0, fmt, args) + 1;
  RDIM_String8 result = {0};
  result.RDIM_String8_BaseMember = rdim_push_array_no_zero(arena, RDI_uint8, needed_bytes);
...
rdim_str8_match(RDIM_String8 a, RDIM_String8 b, RDIM_StringMatchFlags flags)
{
  ...
  {
    RDI_uint32 case_insensitive = (flags & RDIM_StringMatchFlag_CaseInsensitive);
    RDI_uint64 size = a.RDIM_String8_SizeMember;
    result = 1;
    for(RDI_uint64 idx = 0; idx < size; idx += 1)
    {
      RDI_uint8 at = a.RDIM_String8_BaseMember[idx];
      RDI_uint8 bt = b.RDIM_String8_BaseMember[idx];
      if(case_insensitive)
      {
...
RDI_PROC void
rdim_str8_list_push_align(RDIM_Arena *arena, RDIM_String8List *list, RDI_uint64 align)
{
  RDI_uint64 total_size_pre_align  = list->total_size;
  RDI_uint64 total_size_post_align = (total_size_pre_align + (align-1))&(~(align-1));
  RDI_uint64 needed_size = total_size_post_align - total_size_pre_align;
  if(needed_size != 0)
  {
    RDI_uint8 *padding = rdim_push_array(arena, RDI_uint8, needed_size);
...
rdim_str8_list_join(RDIM_Arena *arena, RDIM_String8List *list, RDIM_String8 sep)
{
  ...
  RDI_uint64 sep_count = (list->RDIM_String8List_NodeCountMember > 1) ? (list->RDIM_String8List_NodeCountMember-1) : 0;
  result.RDIM_String8_SizeMember = list->RDIM_String8List_TotalSizeMember+sep_count*sep.RDIM_String8_SizeMember;
  result.RDIM_String8_BaseMember = rdim_push_array_no_zero(arena, RDI_uint8, result.RDIM_String8_SizeMember+1);
  RDI_uint64 off = 0;
  for(RDIM_String8Node *node = list->RDIM_String8List_FirstMember;
      node != 0;
      node = node->RDIM_String8Node_NextPtrMember)
  {
    rdim_memcpy((RDI_uint8*)result.RDIM_String8_BaseMember+off,
...
rdim_sort_key_array(RDIM_Arena *arena, RDIM_SortKey *keys, RDI_uint64 count)
{
  ...
  else
  {
    ...
    RDI_uint64 range_count = 0;
    {
      RDI_uint64 pos = 0;
      for(;pos < count;)
      {
        // identify ordered range
        RDI_uint64 first = pos;
        RDI_uint64 opl = pos + 1;
    ...
    else
    {
      ...
      for(;;)
      {
        ...
        for(;;)
        {
          ...
          // merge these ranges
          RDI_uint64 jd = range1->first;
          RDI_uint64 j1 = range1->first;
          RDI_uint64 j1_opl = range1->opl;
          RDI_uint64 j2 = range2->first;
          RDI_uint64 j2_opl = range2->opl;
          for(;;)
          {
...
#if 0
  // assert sortedness
  for(RDI_uint64 i = 1; i < count; i += 1)
  {
...
RDI_PROC void
rdim_rng1u64_list_push(RDIM_Arena *arena, RDIM_Rng1uint64List *list, RDIM_Rng1uint64 r)
{
  RDIM_Rng1uint64Node *n = rdim_push_array(arena, RDIM_Rng1uint64Node, 1);
  n->v = r;
...
RDI_PROC void
rdim_rng1u64_chunk_list_push(RDIM_Arena *arena, RDIM_Rng1uint64ChunkList *list, RDI_uint64 chunk_cap, RDIM_Rng1uint64 r)
{
  RDIM_Rng1uint64ChunkNode *n = list->last;
  if(n == 0 || n->count >= n->cap)
  {
    n = rdim_push_array(arena, RDIM_Rng1uint64ChunkNode, 1);
    RDIM_SLLQueuePush(list->first, list->last, n);
    n->cap = chunk_cap;
    n->v = rdim_push_array_no_zero(arena, RDIM_Rng1uint64, n->cap);
    list->chunk_count += 1;
  }
  n->v[n->count] = r;
  n->count += 1;
...
rdim_short_type_from_data_model(RDIM_DataModel data_model)
{
  switch(data_model)
  {
    ...
    case RDIM_DataModel_ILP32 : return RDI_TypeKind_uint16;
    case RDIM_DataModel_LLP64 : return RDI_TypeKind_uint16;
    case RDIM_DataModel_LP64  : return RDI_TypeKind_uint16;
    case RDIM_DataModel_ILP64 : return RDI_TypeKind_uint16;
    case RDIM_DataModel_SILP64: return RDI_TypeKind_uint64;
    default: InvalidPath;
  }
...
rdim_int_type_from_data_model(RDIM_DataModel data_model)
{
  ...
  {
    case RDIM_DataModel_Null  : break;
    case RDIM_DataModel_ILP32 : return RDI_TypeKind_uint32;
    case RDIM_DataModel_LLP64 : return RDI_TypeKind_uint32;
    case RDIM_DataModel_LP64  : return RDI_TypeKind_uint32;
    case RDIM_DataModel_ILP64 : return RDI_TypeKind_uint64;
    case RDIM_DataModel_SILP64: return RDI_TypeKind_uint64;
    default: InvalidPath;
  }
  return RDI_TypeKind_NULL;
...
rdim_long_type_from_data_model(RDIM_DataModel data_model)
{
  switch(data_model)
  {
    ...
    case RDIM_DataModel_ILP32 : return RDI_TypeKind_uint32;
    case RDIM_DataModel_LLP64 : return RDI_TypeKind_uint32;
    case RDIM_DataModel_LP64  : return RDI_TypeKind_uint64;
    case RDIM_DataModel_ILP64 : return RDI_TypeKind_uint64;
    case RDIM_DataModel_SILP64: return RDI_TypeKind_uint64;
    default: InvalidPath;
  }
...
rdim_long_long_type_from_data_model(RDIM_DataModel data_model)
{
  ...
  {
    case RDIM_DataModel_Null  : break;
    case RDIM_DataModel_ILP32 : return RDI_TypeKind_uint64;
    case RDIM_DataModel_LLP64 : return RDI_TypeKind_uint64;
    case RDIM_DataModel_LP64  : return RDI_TypeKind_uint64;
    case RDIM_DataModel_ILP64 : return RDI_TypeKind_uint64;
    case RDIM_DataModel_SILP64: return RDI_TypeKind_uint64;
    default: InvalidPath;
...
rdim_src_file_chunk_list_push(RDIM_Arena *arena, RDIM_SrcFileChunkList *list, RDI_uint64 cap)
{
...
rdim_line_table_chunk_list_push(RDIM_Arena *arena, RDIM_LineTableChunkList *list, RDI_uint64 cap)
{
...
rdim_line_table_push_sequence(RDIM_Arena *arena, RDIM_LineTableChunkList *line_tables, RDIM_LineTable *line_table, RDIM_SrcFile *src_file, RDI_uint64 *voffs, RDI_uint32 *line_nums, RDI_uint16 *col_nums, RDI_uint64 line_count)
{
...
RDI_PROC RDIM_Unit *
rdim_unit_chunk_list_push(RDIM_Arena *arena, RDIM_UnitChunkList *list, RDI_uint64 cap)
{
...
RDI_PROC RDIM_Type *
rdim_type_chunk_list_push(RDIM_Arena *arena, RDIM_TypeChunkList *list, RDI_uint64 cap)
{
...
RDI_PROC RDIM_UDT *
rdim_udt_chunk_list_push(RDIM_Arena *arena, RDIM_UDTChunkList *list, RDI_uint64 cap)
{
...
rdim_symbol_chunk_list_push(RDIM_Arena *arena, RDIM_SymbolChunkList *list, RDI_uint64 cap)
{
...
rdim_inline_site_chunk_list_push(RDIM_Arena *arena, RDIM_InlineSiteChunkList *list, RDI_uint64 cap)
{
...
RDI_PROC RDIM_Scope *
rdim_scope_chunk_list_push(RDIM_Arena *arena, RDIM_ScopeChunkList *list, RDI_uint64 cap)
{
...
RDI_PROC void
rdim_scope_push_voff_range(RDIM_Arena *arena, RDIM_ScopeChunkList *list, RDIM_Scope *scope, RDIM_Rng1uint64 range)
{
...
RDI_PROC void
rdim_bytecode_push_op(RDIM_Arena *arena, RDIM_EvalBytecode *bytecode, RDI_EvalOp op, RDI_uint64 p)
{
...
RDI_PROC void
rdim_bytecode_push_uconst(RDIM_Arena *arena, RDIM_EvalBytecode *bytecode, RDI_uint64 x)
{
  if(x <= 0xFF)
  {
    rdim_bytecode_push_op(arena, bytecode, RDI_EvalOp_Constuint8, x);
  }
  else if(x <= 0xFFFF)
  {
    rdim_bytecode_push_op(arena, bytecode, RDI_EvalOp_Constuint16, x);
  }
  else if(x <= 0xFFFFFFFF)
  {
    rdim_bytecode_push_op(arena, bytecode, RDI_EvalOp_Constuint32, x);
  }
  else
  {
    rdim_bytecode_push_op(arena, bytecode, RDI_EvalOp_Constuint64, x);
  }
}

RDI_PROC void
rdim_bytecode_push_sconst(RDIM_Arena *arena, RDIM_EvalBytecode *bytecode, RDI_uint64 x)
{
  if(-0x80 <= x && x <= 0x7F)
  {
    rdim_bytecode_push_op(arena, bytecode, RDI_EvalOp_Constuint8, (RDI_uint64)x);
    rdim_bytecode_push_op(arena, bytecode, RDI_EvalOp_TruncSigned, 8);
  }
  else if(-0x8000 <= x && x <= 0x7FFF)
  {
    rdim_bytecode_push_op(arena, bytecode, RDI_EvalOp_Constuint16, (RDI_uint64)x);
    rdim_bytecode_push_op(arena, bytecode, RDI_EvalOp_TruncSigned, 16);
  }
  else if(-0x80000000ll <= x && x <= 0x7FFFFFFFll)
  {
    rdim_bytecode_push_op(arena, bytecode, RDI_EvalOp_Constuint32, (RDI_uint64)x);
    rdim_bytecode_push_op(arena, bytecode, RDI_EvalOp_TruncSigned, 32);
  }
  else
  {
    rdim_bytecode_push_op(arena, bytecode, RDI_EvalOp_Constuint64, (RDI_uint64)x);
  }
}
...
RDI_PROC RDIM_Location *
rdim_push_location_addr_reg_plus_u16(RDIM_Arena *arena, RDI_uint8 reg_code, RDI_uint16 offset)
{
  RDIM_Location *result = rdim_push_array(arena, RDIM_Location, 1);
  result->kind = RDI_LocationKind_AddrRegPlusuint16;
...
RDI_PROC RDIM_Location *
rdim_push_location_addr_addr_reg_plus_u16(RDIM_Arena *arena, RDI_uint8 reg_code, RDI_uint16 offset)
{
  RDIM_Location *result = rdim_push_array(arena, RDIM_Location, 1);
  result->kind = RDI_LocationKind_AddrAddrRegPlusuint16;
  result->reg_code = reg_code;
  result->offset = offset;
  return result;
}
...
RDI_PROC RDIM_Location *
rdim_push_location_val_reg(RDIM_Arena *arena, RDI_uint8 reg_code)
{
  RDIM_Location *result = rdim_push_array(arena, RDIM_Location, 1);
...
RDI_PROC void
rdim_location_set_push_case(RDIM_Arena *arena, RDIM_ScopeChunkList *scopes, RDIM_LocationSet *locset, RDIM_Rng1uint64 voff_range, RDIM_Location *location)
{
...
rdim_location_block_chunk_list_push_array(RDIM_Arena *arena, RDIM_String8List *list, RDI_uint32 count)
{
  RDI_LocationBlock *result = rdim_push_array(arena, RDI_LocationBlock, count);
  RDIM_String8 string = rdim_str8((RDI_uint8*)result, sizeof(result[0]) * count);
...
{
  RDI_uint32 count = list->total_size / sizeof(RDI_LocationBlock);
  return count;
}
...
rdim_init_type_chunk_list(RDIM_Arena *arena, RDI_Arch arch)
{
  ...
  RDI_uint64 type_cap = (RDI_TypeKind_LastBuiltIn - RDI_TypeKind_FirstBuiltIn) + 2;
...
rdim_bake_vmap_from_markers(RDIM_Arena *arena, RDIM_VMapMarker *markers, RDIM_SortKey *keys, RDI_uint64 marker_count)
{
  RDIM_Temp scratch = rdim_scratch_begin(&arena, 1);
  ...
  RDI_uint32 vmap_count_raw = marker_count - 1 + extra_vmap_entry;
  RDI_VMapEntry *vmap = rdim_push_array(arena, RDI_VMapEntry, vmap_count_raw + 1);
  RDI_uint32 vmap_entry_count_pass_1 = 0;
  {
    ...
    for(;key_ptr < key_opl;)
    {
      ...
      RDI_uint32 initial_idx = (RDI_uint32)0xffffffff;
      if(tracker_stack != 0)
      {
    ...
    vmap_entry_count_pass_1 = (RDI_uint32)(vmap_ptr - vmap); // TODO(rjf): @u64_to_u32
  }
  
  //- rjf: combine duplicate neighbors
  RDI_uint32 vmap_entry_count = 0;
  {
    RDI_VMapEntry *vmap_ptr = vmap;
...
rdim_bake_string_chunk_list_push(RDIM_Arena *arena, RDIM_BakeStringChunkList *list, RDI_uint64 cap)
{
...
rdim_bake_string_chunk_list_sorted_from_unsorted(RDIM_Arena *arena, RDIM_BakeStringChunkList *src)
{
  ...
  {
    for(RDI_uint64 idx = 0; idx < n->count; idx += 1)
    {
  ...
  if(dst.first != 0)
  {
    ...
    for(SortTask *t = first_task; t != 0; t = t->next)
    {
      ...
      for(RDI_uint64 idx = 0; idx < t->count; idx += 1)
      {
        uint8 byte = t->string_off < t->v[idx].string.size ? t->v[idx].string.str[t->string_off] : 0;
      ...
      {
        RDI_uint64 write_idx = 0;
        for(RDI_uint64 bucket_idx = 0; bucket_idx < 256; bucket_idx += 1)
        {
          // rjf: write each chunk node's array into original array, detect if there is size left to sort
          RDI_uint64 bucket_base_idx = write_idx;
          RDI_uint64 need_next_char_sort = 0;
  ...
  RDI_uint64 num_duplicates = 0;
  if(dst.first != 0)
  {
    RDI_uint64 last_idx = 0;
    for(RDI_uint64 idx = 1; idx < dst.first->count; idx += 1)
    {
  ...
  if(num_duplicates != 0)
  {
    ...
    if(num_duplicates != 0)
    {
      RDI_uint64 arena_pos_pre_pop = rdim_arena_pos(arena);
...
RDI_PROC void
rdim_bake_string_map_loose_insert(RDIM_Arena *arena, RDIM_BakeStringMapTopology *map_topology, RDIM_BakeStringMapLoose *map, RDI_uint64 chunk_cap, RDIM_String8 string)
{
  if(string.RDIM_String8_SizeMember != 0)
  {
    RDI_uint64 hash = rdi_hash(string.RDIM_String8_BaseMember, string.RDIM_String8_SizeMember);
    RDI_uint64 slot_idx = hash%map_topology->slots_count;
    RDIM_BakeStringChunkList *slot = map->slots[slot_idx];
    if(slot == 0)
    {
      slot = map->slots[slot_idx] = rdim_push_array(arena, RDIM_BakeStringChunkList, 1);
    }
    RDI_uint32 is_duplicate = 0;
...
{
  for(RDI_uint64 idx = 0; idx < map_topology->slots_count; idx += 1)
  {
    if(dst->slots[idx] == 0)
    {
...
rdim_bake_string_map_base_indices_from_map_loose(RDIM_Arena *arena, RDIM_BakeStringMapTopology *map_topology, RDIM_BakeStringMapLoose *map)
{
  ...
  indices.slots_base_idxs = rdim_push_array(arena, RDI_uint64, map_topology->slots_count+1);
  RDI_uint64 total_count = 0;
  for(RDI_uint64 idx = 0; idx < map_topology->slots_count; idx += 1)
  {
...
rdim_bake_string_map_tight_from_loose(RDIM_Arena *arena, RDIM_BakeStringMapTopology *map_topology, RDIM_BakeStringMapBaseIndices *map_base_indices, RDIM_BakeStringMapLoose *map)
{
  ...
  for(RDI_uint64 idx = 0; idx < m.slots_count; idx += 1)
  {
    if(map->slots[idx] != 0)
    {
...
rdim_bake_idx_from_string(RDIM_BakeStringMapTight *map, RDIM_String8 string)
{
  ...
  {
    RDI_uint64 hash = rdi_hash(string.RDIM_String8_BaseMember, string.RDIM_String8_SizeMember);
    RDI_uint64 slot_idx = hash%map->slots_count;
    for(RDIM_BakeStringChunkNode *n = map->slots[slot_idx].first; n != 0; n = n->next)
    {
      for(RDI_uint64 chunk_idx = 0; chunk_idx < n->count; chunk_idx += 1)
      {
...
RDI_PROC RDI_uint64
rdim_hash_from_idx_run(RDI_uint32 *idx_run, RDI_uint32 count)
{
  RDI_uint64 hash = 5381;
  RDI_uint32 *ptr = idx_run;
  RDI_uint32 *opl = idx_run + count;
  for(;ptr < opl; ptr += 1)
  {
...
RDI_PROC RDI_uint32
rdim_bake_idx_from_idx_run(RDIM_BakeIdxRunMap *map, RDI_uint32 *idx_run, RDI_uint32 count)
{
  RDI_uint64 hash = rdim_hash_from_idx_run(idx_run, count);
  RDI_uint64 slot_idx = hash%map->slots_count;
  ...
  for(RDIM_BakeIdxRunNode *n = map->slots[slot_idx]; n != 0; n = n->hash_next)
  {
    if(n->hash == hash)
    {
      RDI_uint32 is_match = 1;
      RDI_uint32 *n_idx = n->idx_run;
      for(RDI_uint32 i = 0; i < count; i += 1)
      {
  ...
  // rjf: node -> index
  RDI_uint32 result = node ? node->first_idx : 0;
  return result;
}

RDI_PROC RDI_uint32
rdim_bake_idx_run_map_insert(RDIM_Arena *arena, RDIM_BakeIdxRunMap *map, RDI_uint32 *idx_run, RDI_uint32 count)
{
  RDI_uint64 hash = rdim_hash_from_idx_run(idx_run, count);
  RDI_uint64 slot_idx = hash%map->slots_count;
  ...
  if(node == 0)
  {
    ...
    RDI_uint32 *idx_run_copy = rdim_push_array_no_zero(arena, RDI_uint32, count);
    for(RDI_uint32 i = 0; i < count; i += 1)
    {
  ...
  // rjf: node -> index
  RDI_uint32 result = node->first_idx;
  return result;
}
...
rdim_bake_path_node_from_string(RDIM_BakePathTree *tree, RDIM_String8 string)
{
  ...
  RDI_uint8 *ptr = string.str;
  RDI_uint8 *opl = string.str + string.size;
  for(;ptr < opl && node != 0;)
  {
    ...
    RDIM_String8 sub_dir = rdim_str8(range_first, (RDI_uint64)(ptr-range_first));
...
RDI_PROC void
rdim_bake_name_map_push(RDIM_Arena *arena, RDIM_BakeNameMap *map, RDIM_String8 string, RDI_uint32 idx)
{
  ...
  RDI_uint32 existing_idx = 0;
  for(RDIM_BakeNameMapValNode *n = node->val_first; n != 0; n = n->next)
  {
    for(RDI_uint32 i = 0; i < sizeof(n->val)/sizeof(n->val[0]); i += 1)
    {
      if(n->val[i] == 0)
      {
        break;
  ...
  if(!existing_idx)
  {
    ...
    RDI_uint32 insert_i = node->val_count%(sizeof(val_node->val)/sizeof(val_node->val[0]));
    if(insert_i == 0)
    {
...
RDI_PROC RDIM_BakeSection *
rdim_bake_section_list_push_new_unpacked(RDIM_Arena *arena, RDIM_BakeSectionList *list, void *data, RDI_uint64 size, RDI_SectionKind tag, RDI_uint64 tag_idx)
{
...
RDI_PROC void
rdim_bake_string_map_loose_push_src_file_slice(RDIM_Arena *arena, RDIM_BakeStringMapTopology *top, RDIM_BakeStringMapLoose *map, RDIM_SrcFile *v, RDI_uint64 count)
{
  for(RDI_uint64 idx = 0; idx < count; idx += 1)
  {
...
RDI_PROC void
rdim_bake_string_map_loose_push_unit_slice(RDIM_Arena *arena, RDIM_BakeStringMapTopology *top, RDIM_BakeStringMapLoose *map, RDIM_Unit *v, RDI_uint64 count)
{
  for(RDI_uint64 idx = 0; idx < count; idx += 1)
  {
...
RDI_PROC void
rdim_bake_string_map_loose_push_type_slice(RDIM_Arena *arena, RDIM_BakeStringMapTopology *top, RDIM_BakeStringMapLoose *map, RDIM_Type *v, RDI_uint64 count)
{
  for(RDI_uint64 idx = 0; idx < count; idx += 1)
  {
...
RDI_PROC void
rdim_bake_string_map_loose_push_udt_slice(RDIM_Arena *arena, RDIM_BakeStringMapTopology *top, RDIM_BakeStringMapLoose *map, RDIM_UDT *v, RDI_uint64 count)
{
  for(RDI_uint64 idx = 0; idx < count; idx += 1)
  {
...
RDI_PROC void
rdim_bake_string_map_loose_push_symbol_slice(RDIM_Arena *arena, RDIM_BakeStringMapTopology *top, RDIM_BakeStringMapLoose *map, RDIM_Symbol *v, RDI_uint64 count)
{
  for(RDI_uint64 idx = 0; idx < count; idx += 1)
  {
    rdim_bake_string_map_loose_insert(arena, top, map, 4, v[idx].name);
...
RDI_PROC void
rdim_bake_string_map_loose_push_inline_site_slice(RDIM_Arena *arena, RDIM_BakeStringMapTopology *top, RDIM_BakeStringMapLoose *map, RDIM_InlineSite *v, RDI_uint64 count)
{
  for(RDI_uint64 idx = 0; idx < count; idx += 1)
  {
    rdim_bake_string_map_loose_insert(arena, top, map, 4, v[idx].name);
...
RDI_PROC void
rdim_bake_string_map_loose_push_scope_slice(RDIM_Arena *arena, RDIM_BakeStringMapTopology *top, RDIM_BakeStringMapLoose *map, RDIM_Scope *v, RDI_uint64 count)
{
  for(RDI_uint64 idx = 0; idx < count; idx += 1)
  {
  ...
  switch(kind)
  {
    ...
    case RDI_NameMapKind_GlobalVariables:
    {
      ...
      for(RDIM_SymbolChunkNode *n = params->global_variables.first; n != 0; n = n->next)
      {
        for(RDI_uint64 idx = 0; idx < n->count; idx += 1)
        {
          RDI_uint32 symbol_idx = (RDI_uint32)rdim_idx_from_symbol(&n->v[idx]); // TODO(rjf): @u64_to_u32
    ...
    case RDI_NameMapKind_Types:
    {
      ...
      for(RDIM_TypeChunkNode *n = params->types.first; n != 0; n = n->next)
      {
        for(RDI_uint64 idx = 0; idx < n->count; idx += 1)
        {
          RDI_uint32 type_idx = (RDI_uint32)rdim_idx_from_type(&n->v[idx]); // TODO(rjf): @u64_to_u32
    ...
    case RDI_NameMapKind_NormalSourcePaths:
    {
      ...
      for(RDIM_SrcFileChunkNode *n = params->src_files.first; n != 0; n = n->next)
      {
        for(RDI_uint64 idx = 0; idx < n->count; idx += 1)
        {
          RDI_uint64 src_file_idx = rdim_idx_from_src_file(&n->v[idx]);
          rdim_bake_name_map_push(arena, map, n->v[idx].normal_full_path, (RDI_uint32)src_file_idx); // TODO(rjf): @u64_to_u32
        }
      }
    }break;
  }
...
rdim_bake_idx_run_map_from_params(RDIM_Arena *arena, RDIM_BakeNameMap *name_maps[RDI_NameMapKind_COUNT], RDIM_BakeParams *params)
{
  ...
  for(RDIM_TypeChunkNode *n = params->types.first; n != 0; n = n->next)
  {
    for(RDI_uint64 chunk_idx = 0; chunk_idx < n->count; chunk_idx += 1)
    {
      ...
      {
        RDI_uint32 param_idx_run_count = type->count;
        RDI_uint32 *param_idx_run = rdim_push_array_no_zero(arena, RDI_uint32, param_idx_run_count);
        for(RDI_uint32 idx = 0; idx < param_idx_run_count; idx += 1)
        {
          param_idx_run[idx] = (RDI_uint32)rdim_idx_from_type(type->param_types[idx]); // TODO(rjf): @u64_to_u32
        }
      ...
      k = (RDI_NameMapKind)(k+1))
  {
    ...
    if(name_map != 0 && name_map->name_count != 0)
    {
      for(RDIM_BakeNameMapNode *n = name_map->first; n != 0; n = n->order_next)
      {
        if(n->val_count > 1)
        {
          RDI_uint32 *idx_run = rdim_push_array(arena, RDI_uint32, n->val_count);
          RDI_uint64 val_idx = 0;
          for(RDIM_BakeNameMapValNode *idxnode = n->val_first;
              idxnode != 0;
              idxnode = idxnode->next)
          {
            for(RDI_uint32 i = 0; i < sizeof(idxnode->val)/sizeof(idxnode->val[0]); i += 1)
            {
              if(idxnode->val[i] == 0)
              {
                goto dblbreak;
          ...
          dblbreak:;
          rdim_bake_idx_run_map_insert(arena, idx_runs, idx_run, (RDI_uint32)n->val_count); // TODO(rjf): @u64_to_u32
        }
      }
    }
  }
...
rdim_bake_name_map(RDIM_Arena *arena, RDIM_BakeStringMapTight *strings, RDIM_BakeIdxRunMap *idx_runs, RDIM_BakeNameMap *src)
{
  ...
  if(src->name_count != 0)
  {
    RDI_uint32 baked_buckets_count = src->name_count;
    RDI_uint32 baked_nodes_count = src->name_count;
    ...
    RDI_NameMapNode *baked_nodes = rdim_push_array_no_zero(arena, RDI_NameMapNode, baked_nodes_count);
    {
      ...
      {
        RDI_uint64 hash = rdi_hash(node->string.str, node->string.size);
        RDI_uint64 bi = hash%baked_buckets_count;
      ...
      // rjf: convert to serialized buckets & nodes
      {
        ...
        for(RDI_uint32 i = 0; i < baked_buckets_count; i += 1, bucket_ptr += 1)
        {
          bucket_ptr->first_node = (RDI_uint32)((RDI_uint64)(node_ptr - baked_nodes));
              ...
              snode = snode->next)
          {
            ...
            RDI_uint32 string_idx = rdim_bake_idx_from_string(strings, node->string);
            RDI_uint32 match_count = node->val_count;
            RDI_uint32 idx = 0;
            if(match_count == 1)
            {
            ...
            {
              RDI_uint64 temp_pos = rdim_arena_pos(scratch.arena);
              RDI_uint32 *idx_run = rdim_push_array_no_zero(scratch.arena, RDI_uint32, match_count);
              RDI_uint32 *idx_ptr = idx_run;
...
RDI_PROC RDIM_NameMapBakeResult
rdim_name_map_bake_results_combine(RDIM_Arena *arena, RDIM_NameMapBakeResult *results, RDI_uint64 results_count)
{
  RDIM_NameMapBakeResult result = {0};
  {
    ...
    for(RDI_uint64 idx = 0; idx < results_count; idx += 1)
    {
    ...
    //- rjf: fill outputs
    {
      RDI_uint64 buckets_off = 0;
      RDI_uint64 nodes_off = 0;
      for(RDI_uint64 idx = 0; idx < results_count; idx += 1)
      {
...
rdim_bake_units(RDIM_Arena *arena, RDIM_BakeStringMapTight *strings, RDIM_BakePathTree *path_tree, RDIM_UnitChunkList *src)
{
  RDIM_UnitBakeResult result = {0};
  {
    ...
    RDI_uint64 dst_idx = 1;
    for(RDIM_UnitChunkNode *src_n = src->first; src_n != 0; src_n = src_n->next)
    {
      for(RDI_uint64 src_chunk_idx = 0; src_chunk_idx < src_n->count; src_chunk_idx += 1, dst_idx += 1)
      {
        ...
        dst->line_table_idx           = (RDI_uint32)rdim_idx_from_line_table(src->line_table); // TODO(rjf): @u64_to_u32
      }
    }
...
rdim_bake_unit_vmap(RDIM_Arena *arena, RDIM_UnitChunkList *units)
{
  ...
  RDIM_BakeVMap unit_vmap = {0};
  {
    ...
    RDI_uint64 marker_count = voff_range_count*2;
    ...
    RDIM_VMapMarker *markers = rdim_push_array_no_zero(scratch.arena, RDIM_VMapMarker, marker_count);
    {
      ...
      RDI_uint32 unit_idx = 1;
      for(RDIM_UnitChunkNode *unit_chunk_n = units->first;
          unit_chunk_n != 0;
          unit_chunk_n = unit_chunk_n->next)
      {
        for(RDI_uint64 idx = 0; idx < unit_chunk_n->count; idx += 1)
        {
          RDIM_Unit *unit = &unit_chunk_n->v[idx];
          for(RDIM_Rng1uint64ChunkNode *n = unit->voff_ranges.first; n != 0; n = n->next)
          {
            for(RDI_uint64 chunk_idx = 0; chunk_idx < n->count; chunk_idx += 1)
            {
              RDIM_Rng1uint64 range = n->v[chunk_idx];
...
rdim_bake_src_files(RDIM_Arena *arena, RDIM_BakeStringMapTight *strings, RDIM_BakePathTree *path_tree, RDIM_SrcFileChunkList *src)
{
  ...
  RDI_uint32 dst_files_count = src->total_count + 1;
  RDI_uint32 dst_maps_count = src->source_line_map_count + 1;
  ...
  RDI_uint64 dst_nums_idx = 0;
  RDI_uint64 dst_rngs_idx = 0;
  RDI_uint64 dst_voffs_idx = 0;
  RDI_uint32 dst_file_idx = 1;
  RDI_uint32 dst_map_idx = 1;
  for(RDIM_SrcFileChunkNode *chunk_n = src->first;
      chunk_n != 0;
      chunk_n = chunk_n->next)
  {
    for(RDI_uint64 idx = 0; idx < chunk_n->count; idx += 1, dst_file_idx += 1)
    {
      ...
      //
      RDI_uint32 *src_file_line_nums   = 0;
      RDI_uint32 *src_file_line_ranges = 0;
      RDI_uint64 *src_file_voffs       = 0;
      RDI_uint32  src_file_line_count  = 0;
      RDI_uint32  src_file_voff_count  = 0;
      {
        ...
        RDI_uint64 max_line_num = 0;
        {
              ...
              map_fragment = map_fragment->next)
          {
            ...
            RDI_uint64 *seq_voffs = sequence->voffs;
            RDI_uint32 *seq_line_nums = sequence->line_nums;
            RDI_uint64 seq_line_count = sequence->line_count;
            for(RDI_uint64 i = 0; i < seq_line_count; i += 1)
            {
              RDI_uint32 line_num = seq_line_nums[i];
              RDI_uint64 voff = seq_voffs[i];
              RDI_uint64 line_hash_slot_idx = line_num%line_hash_slots_count;
        ...
        //- rjf: bake result
        RDI_uint32 *line_nums = rdim_push_array_no_zero(scratch.arena, RDI_uint32, line_count);
        RDI_uint32 *line_ranges = rdim_push_array_no_zero(scratch.arena, RDI_uint32, line_count + 1);
        RDI_uint64 *voffs = rdim_push_array_no_zero(scratch.arena, RDI_uint64, voff_count);
        {
          RDI_uint64 *voff_ptr = voffs;
          for(RDI_uint32 i = 0; i < line_count; i += 1)
          {
            line_nums[i] = sorted_keys[i].key;
            line_ranges[i] = (RDI_uint32)(voff_ptr - voffs); // TODO(rjf): @u64_to_u32
      ...
      if(src_file->first_line_map_fragment != 0)
      {
        ...
        dst_map_idx += 1;
        dst_map->line_count = (RDI_uint32)src_file_line_count; // TODO(rjf): @u64_to_u32
        dst_map->voff_count = (RDI_uint32)src_file_voff_count; // TODO(rjf): @u64_to_u32
        dst_map->line_map_nums_base_idx  = (RDI_uint32)dst_nums_idx; // TODO(rjf): @u64_to_u32
        dst_map->line_map_range_base_idx = (RDI_uint32)dst_rngs_idx; // TODO(rjf): @u64_to_u32
        dst_map->line_map_voff_base_idx  = (RDI_uint32)dst_voffs_idx; // TODO(rjf): @u64_to_u32
      }
      ...
      //
      {
        ...
        dst_num_node->size = sizeof(RDI_uint32)*src_file_line_count;
        ...
        dst_rng_node->size = sizeof(RDI_uint32)*(src_file_line_count+1);
        ...
        dst_voff_node->size = sizeof(RDI_uint64)*(src_file_voff_count);
      ...
      dst_file->source_line_map_idx = (RDI_uint32)(dst_map ? (dst_map - dst_maps) : 0);
    }
  }
  ...
  //
  RDI_uint32 *source_line_map_nums = rdim_push_array_no_zero(arena, RDI_uint32, dst_nums_idx);
  RDI_uint32 *source_line_map_rngs = rdim_push_array_no_zero(arena, RDI_uint32, dst_rngs_idx);
  RDI_uint64 *source_line_map_voffs= rdim_push_array_no_zero(arena, RDI_uint64, dst_voffs_idx);
  {
    RDI_uint64 num_idx = 0;
    RDI_uint64 rng_idx = 0;
    RDI_uint64 voff_idx= 0;
    for(RDIM_DataNode *num_n = first_dst_nums_node; num_n != 0; num_n = num_n->next)
    {
      ...
      num_idx += num_n->size/sizeof(RDI_uint32);
    }
    for(RDIM_DataNode *rng_n = first_dst_rngs_node; rng_n != 0; rng_n = rng_n->next)
    {
      ...
      rng_idx += rng_n->size/sizeof(RDI_uint32);
    }
    for(RDIM_DataNode *voff_n = first_dst_voffs_node; voff_n != 0; voff_n = voff_n->next)
    {
      ...
      voff_idx += voff_n->size/sizeof(RDI_uint64);
    }
  }
...
rdim_bake_line_tables(RDIM_Arena *arena, RDIM_LineTableChunkList *src)
{
  ...
  RDI_uint64 *dst_line_voffs = push_array(arena, RDI_uint64, src->total_line_count + 2*src->total_seq_count);
  ...
  {
    RDI_uint64 dst_table_idx = 1;
    RDI_uint64 dst_voff_idx = 0;
    RDI_uint64 dst_line_idx = 0;
    RDI_uint64 dst_col_idx = 0;
    for(RDIM_LineTableChunkNode *src_n = src->first; src_n != 0; src_n = src_n->next)
    {
      for(RDI_uint64 chunk_idx = 0; chunk_idx < src_n->count; chunk_idx += 1)
      {
        ...
        //- rjf: fill combined line table info
        {
          ...
          RDI_uint64 line_count = src_line_table->line_count;
          RDI_uint64 seq_count = src_line_table->seq_count;
          RDI_uint64 key_count = line_count + seq_count;
          ...
          RDIM_LineRec *line_recs = rdim_push_array_no_zero(scratch.arena, RDIM_LineRec, line_count);
          {
            ...
            for(RDIM_LineSequenceNode *seq_n = src_line_table->first_seq; seq_n != 0; seq_n = seq_n->next)
            {
              ...
              for(RDI_uint64 line_idx = 0; line_idx < seq->line_count; line_idx += 1)
              {
                key_ptr->key = seq->voffs[line_idx];
                key_ptr->val = rec_ptr;
                key_ptr += 1;
                rec_ptr->file_id = (RDI_uint32)rdim_idx_from_src_file(seq->src_file); // TODO(rjf): @u64_to_u32
          ...
          //- rjf: arrange output
          RDI_uint64 *arranged_voffs = dst_line_voffs + dst_voff_idx;
          RDI_Line *arranged_lines = dst_lines + dst_line_idx;
          {
            for(RDI_uint64 i = 0; i < key_count; i += 1)
            {
        ...
        dst_line_table->voffs_base_idx = (RDI_uint32)dst_voff_idx; // TODO(rjf): @u64_to_u32
        dst_line_table->lines_base_idx = (RDI_uint32)dst_line_idx; // TODO(rjf): @u64_to_u32
        dst_line_table->cols_base_idx  = (RDI_uint32)dst_col_idx;  // TODO(rjf): @u64_to_u32
        dst_line_table->lines_count    = (RDI_uint32)src_line_table->line_count + src_line_table->seq_count; // TODO(rjf): @u64_to_u32
...
rdim_bake_types(RDIM_Arena *arena, RDIM_BakeStringMapTight *strings, RDIM_BakeIdxRunMap *idx_runs, RDIM_TypeChunkList *src)
{
  ...
  for(RDIM_TypeChunkNode *n = src->first; n != 0; n = n->next)
  {
    for(RDI_uint64 chunk_idx = 0; chunk_idx < n->count; chunk_idx += 1)
    {
      ...
      uint64           dst_idx = rdim_idx_from_type(src);
      RDI_TypeNode *dst     = &type_nodes[dst_idx];
      
      //- rjf: fill shared type node info
      dst->kind      = src->kind;
      dst->flags     = (RDI_uint16)src->flags; // TODO(rjf): @u32_to_u16
      ...
      {
        uint64 direct_byte_size = 1;
        if(src->direct_type && src->direct_type->byte_size > 0)
        {
          direct_byte_size = src->direct_type->byte_size;
        }
        dst->constructed.direct_type_idx = (RDI_uint32)rdim_idx_from_type(src->direct_type);
      ...
      {
        dst->constructed.direct_type_idx = (RDI_uint32)rdim_idx_from_type(src->direct_type); // TODO(rjf): @u64_to_u32
        dst->constructed.count = src->count;
        if(dst->kind == RDI_TypeKind_Function || dst->kind == RDI_TypeKind_Method)
        {
          RDI_uint32 param_idx_run_count = src->count;
          RDI_uint32 *param_idx_run = rdim_push_array_no_zero(arena, RDI_uint32, param_idx_run_count);
          for(RDI_uint32 idx = 0; idx < param_idx_run_count; idx += 1)
          {
            param_idx_run[idx] = (RDI_uint32)rdim_idx_from_type(src->param_types[idx]); // TODO(rjf): @u64_to_u32
          }
      ...
      else if(RDI_TypeKind_FirstUserDefined <= dst->kind && dst->kind <= RDI_TypeKind_LastUserDefined)
      {
        ...
        dst->user_defined.udt_idx         = (RDI_uint32)rdim_idx_from_udt(src->udt); // TODO(rjf): @u64_to_u32
        dst->user_defined.direct_type_idx = (RDI_uint32)rdim_idx_from_type(src->direct_type); // TODO(rjf): @u64_to_u32
      }
      
      //- rjf: fill bitfield info
      else if(dst->kind == RDI_TypeKind_Bitfield)
      {
        dst->bitfield.direct_type_idx = (RDI_uint32)rdim_idx_from_type(src->direct_type); // TODO(rjf): @u64_to_u32
...
rdim_bake_udts(RDIM_Arena *arena, RDIM_BakeStringMapTight *strings, RDIM_UDTChunkList *src)
{
  ...
  {
    RDI_uint32 dst_udt_idx = 1;
    RDI_uint32 dst_member_idx = 1;
    RDI_uint32 dst_enum_member_idx = 1;
    for(RDIM_UDTChunkNode *n = src->first; n != 0; n = n->next)
    {
      for(RDI_uint64 chunk_idx = 0; chunk_idx < n->count; chunk_idx += 1, dst_udt_idx += 1)
      {
        RDIM_UDT *src_udt = &n->v[chunk_idx];
        RDI_UDT *dst_udt = &udts[dst_udt_idx];
        
        //- rjf: fill basics
        dst_udt->self_type_idx = (RDI_uint32)rdim_idx_from_type(src_udt->self_type); // TODO(rjf): @u64_to_u32
        dst_udt->file_idx = (RDI_uint32)rdim_idx_from_src_file(src_udt->src_file); // TODO(rjf): @u64_to_u32
        ...
        if(src_udt->member_count != 0)
        {
              ...
              src_member = src_member->next, dst_member_idx += 1)
          {
            ...
            dst_member->type_idx        = (RDI_uint32)rdim_idx_from_type(src_member->type); // TODO(rjf): @u64_to_u32
...
rdim_bake_global_variables(RDIM_Arena *arena, RDIM_BakeStringMapTight *strings, RDIM_SymbolChunkList *src)
{
  ...
  RDI_uint32 dst_idx = 1;
  for(RDIM_SymbolChunkNode *n = src->first; n != 0; n = n->next)
  {
    for(RDI_uint64 chunk_idx = 0; chunk_idx < n->count; chunk_idx += 1, dst_idx += 1)
    {
      RDIM_Symbol *src = &n->v[chunk_idx];
      RDI_GlobalVariable *dst = &global_variables[dst_idx];
      dst->name_string_idx = rdim_bake_idx_from_string(strings, src->name);
      dst->voff            = src->offset;
      dst->type_idx        = (RDI_uint32)rdim_idx_from_type(src->type); // TODO(rjf): @u64_to_u32
      if(src->is_extern)
      {
      ...
      if(src->container_type != 0)
      {
        ...
        dst->container_idx = src->container_type ? (RDI_uint32)rdim_idx_from_udt(src->container_type->udt) : 0; // TODO(rjf): @u64_to_u32
      }
      else if(src->container_symbol != 0)
      {
        dst->link_flags |= RDI_LinkFlag_ProcScoped;
        dst->container_idx = (RDI_uint32)rdim_idx_from_symbol(src->container_symbol); // TODO(rjf): @u64_to_u32
      }
    }
  }
...
rdim_bake_global_vmap(RDIM_Arena *arena, RDIM_SymbolChunkList *src)
{
  ...
  RDIM_BakeVMap global_vmap = {0};
  {
    ...
    RDI_uint64 marker_count = src->total_count*2 + 2;
    RDIM_SortKey    *keys    = rdim_push_array_no_zero(scratch.arena, RDIM_SortKey, marker_count);
    RDIM_VMapMarker *markers = rdim_push_array_no_zero(scratch.arena, RDIM_VMapMarker, marker_count);
    ...
    //- rjf: fill
    {
      ...
      for(RDIM_SymbolChunkNode *n = src->first; n != 0; n = n->next)
      {
        for(RDI_uint64 chunk_idx = 0; chunk_idx < n->count; chunk_idx += 1)
        {
          ...
          RDI_uint32 global_var_idx = (RDI_uint32)rdim_idx_from_symbol(global_var); // TODO(rjf): @u64_to_u32
          RDI_uint64 global_var_size = global_var->type ? global_var->type->byte_size : 1;
          
          RDI_uint64 first = global_var->offset;
          RDI_uint64 opl   = first + global_var_size;
      ...
      // rjf: fill nil global
      {
        RDI_uint32 global_idx = 0;
        RDI_uint64 first = 0;
        RDI_uint64 opl   = 0xffffffffffffffffull;
        key_ptr->key = first;
        key_ptr->val = marker_ptr;
...
rdim_bake_thread_variables(RDIM_Arena *arena, RDIM_BakeStringMapTight *strings, RDIM_SymbolChunkList *src)
{
  ...
  for(RDIM_SymbolChunkNode *n = src->first; n != 0; n = n->next)
  {
    for(RDI_uint64 chunk_idx = 0; chunk_idx < n->count; chunk_idx += 1, dst_idx += 1)
    {
      ...
      dst->name_string_idx = rdim_bake_idx_from_string(strings, src->name);
      dst->tls_off         = (RDI_uint32)src->offset; // TODO(rjf): @u64_to_u32
      dst->type_idx        = (RDI_uint32)rdim_idx_from_type(src->type);
      if(src->is_extern)
      {
        dst->link_flags |= RDI_LinkFlag_External;
      }
...
rdim_bake_constants(RDIM_Arena *arena, RDIM_BakeStringMapTight *strings, RDIM_SymbolChunkList *src)
{
  ...
  RDI_uint32 *constant_values = push_array(arena, RDI_uint32, src->total_count+2);
  RDI_uint8 *constant_value_data = push_array(arena, RDI_uint8, src->total_value_data_size+1);
  RDI_uint32 dst_idx = 1;
  RDI_uint64 dst_constant_value_data_off = 1;
  for(RDIM_SymbolChunkNode *n = src->first; n != 0; n = n->next)
  {
    for(RDI_uint64 chunk_idx = 0; chunk_idx < n->count; chunk_idx += 1, dst_idx += 1)
    {
      ...
      RDI_uint32 *dst_value_idx = &constant_values[dst_idx];
      dst->name_string_idx    = rdim_bake_idx_from_string(strings, src->name);
      dst->type_idx           = (RDI_uint32)rdim_idx_from_type(src->type); // TODO(rjf): @u64_to_u32
...
{
  uint64 location_data_off = location_data_blobs->total_size;
  ...
  else switch(src_location->kind)
  {
    ...
    case RDI_LocationKind_ValBytecodeStream:
    {
      ...
      {
        RDI_uint64 data = 0;
        RDIM_String8 data_str = rdim_str8((RDI_uint8 *)&data, 1);
    ...
    {
      RDI_LocationRegPlusuint16 loc = {0};
                     ...
                     RDIM_SymbolChunkList    *src)
{
  ...
  for(RDIM_SymbolChunkNode *n = src->first; n != 0; n = n->next)
  {
    for(RDI_uint64 chunk_idx = 0; chunk_idx < n->count; chunk_idx += 1, dst_idx += 1)
    {
      ...
      RDI_uint32 frame_base_location_first = rdim_bake_locset(arena, location_blocks, location_data_blobs, src->frame_base);
      RDI_uint32 frame_base_location_opl   = frame_base_location_first + src->frame_base.location_case_count;
      ...
      dst->type_idx                  = (RDI_uint32)rdim_idx_from_type(src->type); // TODO(rjf): @u64_to_u32
      dst->root_scope_idx            = (RDI_uint32)rdim_idx_from_scope(src->root_scope); // TODO(rjf): @u64_to_u32
                 ...
                 RDIM_ScopeChunkList     *src)
{
  ...
  RDI_uint64   *scope_voffs = rdim_push_array(arena, RDI_uint64,   src->scope_voff_count+1);
  ...
  {
    RDI_uint64 dst_scope_idx      = 1;
    RDI_uint64 dst_scope_voff_idx = 1;
    RDI_uint64 dst_local_idx      = 1;
    for(RDIM_ScopeChunkNode *chunk_n = src->first; chunk_n != 0; chunk_n = chunk_n->next)
    {
      for(RDI_uint64 chunk_idx = 0; chunk_idx < chunk_n->count; chunk_idx += 1, dst_scope_idx += 1)
      {
        RDIM_Scope *src_scope = &chunk_n->v[chunk_idx];
        RDI_Scope  *dst_scope = &scopes[dst_scope_idx];
        
        //- rjf: push scope's voffs
        RDI_uint64 voff_idx_first = dst_scope_voff_idx;
        {
          for(RDIM_Rng1uint64Node *n = src_scope->voff_ranges.first; n != 0; n = n->next)
          {
        ...
        RDI_uint64 voff_idx_opl = dst_scope_voff_idx;
        
        //- rjf: push locals
        RDI_uint64 local_idx_first = dst_local_idx;
        for(RDIM_Local *src_local = src_scope->first_local;
            src_local != 0;
            src_local = src_local->next, dst_local_idx += 1)
        {
          // bake location sets
          RDI_uint32 location_block_idx_first = rdim_bake_locset(arena, location_blocks, location_data_blobs, src_local->locset);
          RDI_uint32 location_block_idx_opl   = location_block_idx_first + src_local->locset.location_case_count;
          ...
          dst_local->type_idx        = (RDI_uint32)rdim_idx_from_type(src_local->type); // TODO(rjf): @u64_to_u32
          dst_local->location_first  = location_block_idx_first;
          dst_local->location_opl    = location_block_idx_opl;
        }
        RDI_uint64 local_idx_opl = dst_local_idx;
        
        //- rjf: fill scope
        dst_scope->proc_idx               = (RDI_uint32)rdim_idx_from_symbol(src_scope->symbol); // TODO(rjf): @u64_to_u32
        dst_scope->parent_scope_idx       = (RDI_uint32)rdim_idx_from_scope(src_scope->parent_scope); // TODO(rjf): @u64_to_u32
        dst_scope->first_child_scope_idx  = (RDI_uint32)rdim_idx_from_scope(src_scope->first_child); // TODO(rjf): @u64_to_u32
        dst_scope->next_sibling_scope_idx = (RDI_uint32)rdim_idx_from_scope(src_scope->next_sibling); // TODO(rjf): @u64_to_u32
        dst_scope->voff_range_first       = (RDI_uint32)voff_idx_first;                    // TODO(rjf): @u64_to_u32
        dst_scope->voff_range_opl         = (RDI_uint32)voff_idx_opl;                      // TODO(rjf): @u64_to_u32
        dst_scope->local_first            = (RDI_uint32)local_idx_first;                   // TODO(rjf): @u64_to_u32
        dst_scope->local_count            = (RDI_uint32)(local_idx_opl - local_idx_first); // TODO(rjf): @u64_to_u32
        dst_scope->inline_site_idx        = (RDI_uint32)rdim_idx_from_inline_site(src_scope->inline_site); // TODO(rjf): @u64_to_u32
      }
    }
  }
...
rdim_bake_scope_vmap(RDIM_Arena *arena, RDIM_ScopeChunkList *src)
{
  RDIM_BakeVMap scope_vmap = {0};
  {
    ...
    // rjf: allocate keys/markers
    RDI_uint64 marker_count = src->scope_voff_count;
    RDIM_SortKey    *keys    = rdim_push_array_no_zero(scratch.arena, RDIM_SortKey, marker_count);
    RDIM_VMapMarker *markers = rdim_push_array_no_zero(scratch.arena, RDIM_VMapMarker, marker_count);
    ...
    // rjf: fill
    {
      ...
      for(RDIM_ScopeChunkNode *chunk_n = src->first; chunk_n != 0; chunk_n = chunk_n->next)
      {
        for(RDI_uint64 chunk_idx = 0; chunk_idx < chunk_n->count; chunk_idx += 1)
        {
          RDIM_Scope *src_scope = &chunk_n->v[chunk_idx];
          RDI_uint32 scope_idx = (RDI_uint32)rdim_idx_from_scope(src_scope); // TODO(rjf): @u64_to_u32
          for(RDIM_Rng1uint64Node *n = src_scope->voff_ranges.first; n != 0; n = n->next)
          {
            key_ptr->key = n->v.min;
            key_ptr->val = marker_ptr;
  ...
  RDIM_InlineSiteBakeResult result = {0};
  {
    ...
    for(RDIM_InlineSiteChunkNode *n = src->first; n != 0; n = n->next)
    {
      for(RDI_uint64 chunk_idx = 0; chunk_idx < n->count; chunk_idx += 1, dst_idx += 1)
      {
        ...
        dst->name_string_idx   = rdim_bake_idx_from_string(strings, src->name);
        dst->type_idx          = (RDI_uint32)rdim_idx_from_type(src->type); // TODO(rjf): @u64_to_u32
        dst->owner_type_idx    = (RDI_uint32)rdim_idx_from_type(src->owner); // TODO(rjf): @u64_to_u32
        dst->line_table_idx    = (RDI_uint32)rdim_idx_from_line_table(src->line_table); // TODO(rjf): @u64_to_u32
      }
    }
  }
...
rdim_bake_name_maps_top_level(RDIM_Arena *arena, RDIM_BakeStringMapTight *strings, RDIM_BakeIdxRunMap *idx_runs, RDIM_BakeNameMap *name_maps[RDI_NameMapKind_COUNT])
{
  ...
  {
    RDI_uint64 dst_map_bucket_idx = 0;
    RDI_uint64 dst_map_node_idx = 0;
    for(RDI_NameMapKind k = (RDI_NameMapKind)(RDI_NameMapKind_NULL+1);
        k < RDI_NameMapKind_COUNT;
        k = (RDI_NameMapKind)(k+1))
    {
      ...
      dst_map->bucket_base_idx = (RDI_uint32)dst_map_bucket_idx; // TODO(rjf): @u64_to_u32
      dst_map->node_base_idx   = (RDI_uint32)dst_map_node_idx; // TODO(rjf): @u64_to_u32
      dst_map->bucket_count    = (RDI_uint32)src_map->name_count; // TODO(rjf): @u64_to_u32
      dst_map->node_count      = (RDI_uint32)src_map->name_count; // TODO(rjf): @u64_to_u32
...
{
  RDI_uint32 *str_offs = rdim_push_array_no_zero(arena, RDI_uint32, strings->total_count + 1);
  RDI_uint32 off_cursor = 0;
  {
    RDI_uint32 *off_ptr = str_offs;
    *off_ptr = 0;
    off_ptr += 1;
    for(RDI_uint64 slot_idx = 0; slot_idx < strings->slots_count; slot_idx += 1)
    {
  ...
  RDI_uint8 *buf = rdim_push_array(arena, RDI_uint8, off_cursor);
  {
    RDI_uint8 *ptr = buf;
    for(RDI_uint64 slot_idx = 0; slot_idx < strings->slots_count; slot_idx += 1)
    {
...
{
  RDI_uint32 *idx_data = rdim_push_array_no_zero(arena, RDI_uint32, idx_runs->idx_count);
  {
    RDI_uint32 *out_ptr = idx_data;
    RDI_uint32 *opl = out_ptr + idx_runs->idx_count;
...
rdim_serialized_section_make_unpacked(void *data, RDI_uint64 size)
{
...
rdim_file_blobs_from_section_bundle(RDIM_Arena *arena, RDIM_SerializedSectionBundle *bundle)
{
  ...
  rdim_memzero_struct(&strings);
  {
    ...
    uint32 data_section_off = (uint32)strings.total_size;
    rdim_str8_list_push(arena, &strings, rdim_str8((RDI_uint8 *)rdi_sections, sizeof(RDI_Section)*RDI_SectionKind_COUNT));
    ...
    for(RDI_SectionKind k = RDI_SectionKind_NULL; k < RDI_SectionKind_COUNT; k += 1)
    {
      ...
      uint64 data_section_off = 0;
      if(bundle->sections[k].encoded_size != 0)
      {
        rdim_str8_list_push_align(arena, &strings, 8);
        data_section_off = strings.total_size;
        rdim_str8_list_push(arena, &strings, rdim_str8((RDI_uint8 *)bundle->sections[k].data, bundle->sections[k].encoded_size));
      }
