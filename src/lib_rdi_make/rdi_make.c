...
rdim_str8_copy(RDIM_Arena *arena, RDIM_String8 src)
{
  ...
  dst.RDIM_String8_BaseMember = rdim_new RDI_U8[dst.RDIM_String8_SizeMember + 1] /* no zero */;
...
rdim_str8fv(RDIM_Arena *arena, char *fmt, va_list args)
{
  ...
  result.RDIM_String8_BaseMember = rdim_new RDI_U8[needed_bytes] /* no zero */;
...
rdim_str8_list_join(RDIM_Arena *arena, RDIM_String8List *list, RDIM_String8 sep)
{
  ...
  result.RDIM_String8_BaseMember = rdim_new RDI_U8[result.RDIM_String8_SizeMember + 1] /* no zero */;
  RDI_U64 off = 0;
...
rdim_sort_key_array(RDIM_Arena *arena, RDIM_SortKey *keys, RDI_U64 count)
{
  ...
  else
  {
    ...
    {
      RDIM_SortKey *keys_swap = rdim_new RDIM_SortKey[count] /* no zero */;
...
rdim_rng1u64_chunk_list_push(RDIM_Arena *arena, RDIM_Rng1U64ChunkList *list, RDI_U64 chunk_cap, RDIM_Rng1U64 r)
{
  ...
  if(n == 0 || n->count >= n->cap)
  {
    ...
    RDIM_SLLQueuePush(list->first, list->last, n);
    n->cap = chunk_cap;
    n->v = rdim_new RDIM_Rng1U64[n->cap] /* no zero */;
    list->chunk_count += 1;
  }
  n->v[n->count] = r;
  n->count += 1;
...
{
  RDIM_LineSequenceNode *n = push_array(RDIM_LineSequenceNode, 1);
...
{
  RDIM_Type **arr = push_array(RDIM_Type *, list.count);
  U64         i   = 0;
...
rdim_bake_idx_run_map_insert(RDIM_Arena *arena, RDIM_BakeIdxRunMap *map, RDI_U32 *idx_run, RDI_U32 count)
{
  ...
  if(node == 0)
  {
    node = rdim_new RDIM_BakeIdxRunNode[1] /* no zero */;
    RDI_U32 *idx_run_copy = rdim_new RDI_U32[count] /* no zero */;
    for(RDI_U32 i = 0; i < count; i += 1)
    {
  ...
  for(RDIM_TypeChunkNode *n = params->types.first; n != 0; n = n->next)
  {
    for(RDI_U64 chunk_idx = 0; chunk_idx < n->count; chunk_idx += 1)
    {
      ...
      if(type->kind == RDI_TypeKind_Function || type->kind == RDI_TypeKind_Method)
      {
        ...
        RDI_U32 *param_idx_run = rdim_new RDI_U32[param_idx_run_count] /* no zero */;
...
rdim_bake_name_map(RDIM_Arena *arena, RDIM_BakeStringMapTight *strings, RDIM_BakeIdxRunMap *idx_runs, RDIM_BakeNameMap *src)
{
  ...
  if(src->name_count != 0)
  {
    ...
    RDI_NameMapNode *baked_nodes = rdim_new RDI_NameMapNode[baked_nodes_count] /* no zero */;
    {
      RDIM_Temp scratch = rdim_scratch_begin(&arena, 1);
        ...
        for(RDI_U32 i = 0; i < baked_buckets_count; i += 1, bucket_ptr += 1)
        {
              ...
              snode = snode->next)
          {
            ...
            else
            {
              ...
              RDI_U32 *idx_run = rdim_/* no zero */ push_array(RDI_U32, match_count);
...
rdim_name_map_bake_results_combine(RDIM_Arena *arena, RDIM_NameMapBakeResult *results, RDI_U64 results_count)
{
  RDIM_NameMapBakeResult result = {0};
  {
    ...
    result.buckets       = rdim_new RDI_NameMapBucket[result.buckets_count] /* no zero */;
    result.nodes_count   = all_nodes_count;
    result.nodes         = rdim_new RDI_NameMapNode[result.nodes_count] /* no zero */;
...
rdim_bake_unit_vmap(RDIM_Arena *arena, RDIM_UnitChunkList *units)
{
  ...
  RDIM_BakeVMap unit_vmap = {0};
  {
    ...
    RDIM_SortKey    *keys = rdim_/* no zero */ push_array(RDIM_SortKey, marker_count);
    RDIM_VMapMarker *markers = rdim_/* no zero */ push_array(RDIM_VMapMarker, marker_count);
    {
...
rdim_bake_src_files(RDIM_Arena *arena, RDIM_BakeStringMapTight *strings, RDIM_BakePathTree *path_tree, RDIM_SrcFileChunkList *src)
{
      ...
      chunk_n = chunk_n->next)
  {
    for(RDI_U64 idx = 0; idx < chunk_n->count; idx += 1, dst_file_idx += 1)
    {
      ...
      RDI_U32  src_file_voff_count  = 0;
      {
        ...
        RDIM_SortKey *keys = rdim_/* no zero */ push_array(RDIM_SortKey, line_count);
        {
          RDIM_SortKey *key_ptr = keys;
        ...
        //- rjf: bake result
        RDI_U32 *line_nums = rdim_/* no zero */ push_array(RDI_U32, line_count);
        RDI_U32 *line_ranges = rdim_/* no zero */ push_array(RDI_U32, line_count + 1);
        RDI_U64 *voffs = rdim_/* no zero */ push_array(RDI_U64, voff_count);
        {
  ...
  //
  RDI_U32 *source_line_map_nums = rdim_new RDI_U32[dst_nums_idx] /* no zero */;
  RDI_U32 *source_line_map_rngs = rdim_new RDI_U32[dst_rngs_idx] /* no zero */;
  RDI_U64 *source_line_map_voffs= rdim_new RDI_U64[dst_voffs_idx] /* no zero */;
  {
    RDI_U64 num_idx = 0;
    RDI_U64 rng_idx = 0;
    RDI_U64 voff_idx= 0;
...
rdim_bake_line_tables(RDIM_Arena *arena, RDIM_LineTableChunkList *src)
{
  ...
  //
  RDI_LineTable *dst_line_tables = push_array(RDI_LineTable, src->total_count+1);
  RDI_U64 *dst_line_voffs = push_array(RDI_U64, src->total_line_count + 2*src->total_seq_count);
  RDI_Line *dst_lines = push_array(RDI_Line, src->total_line_count + src->total_seq_count);
  RDI_Column *dst_cols = push_array(RDI_Column, 1);
  {
    ...
    for(RDIM_LineTableChunkNode *src_n = src->first; src_n != 0; src_n = src_n->next)
    {
      for(RDI_U64 chunk_idx = 0; chunk_idx < src_n->count; chunk_idx += 1)
      {
        ...
        //- rjf: fill combined line table info
        {
          ...
          RDIM_SortKey *line_keys = rdim_/* no zero */ push_array(RDIM_SortKey, key_count);
          RDIM_LineRec *line_recs = rdim_/* no zero */ push_array(RDIM_LineRec, line_count);
          {
...
{
  RDI_TypeNode *type_nodes = push_array(RDI_TypeNode, src->total_count+1);
...
{
  //- rjf: build tables
  RDI_UDT *       udts         = push_array(RDI_UDT,        src->total_count+1);
  RDI_Member *    members      = push_array(RDI_Member,     src->total_member_count+1);
  RDI_EnumMember *enum_members = push_array(RDI_EnumMember, src->total_enum_val_count+1);
  {
...
{
  RDI_GlobalVariable *global_variables = push_array(RDI_GlobalVariable, src->total_count+1);
  RDI_U32 dst_idx = 1;
...
{
  RDI_ThreadVariable *thread_variables = push_array(RDI_ThreadVariable, src->total_count+1);
  RDI_U32 dst_idx = 1;
  for(RDIM_SymbolChunkNode *n = src->first; n != 0; n = n->next)
  {
    for(RDI_U64 chunk_idx = 0; chunk_idx < n->count; chunk_idx += 1, dst_idx += 1)
...
{
  RDI_Constant *constants = push_array(RDI_Constant, src->total_count+1);
  RDI_U32 *constant_values = push_array(RDI_U32, src->total_count+2);
  RDI_U8 *constant_value_data = push_array(RDI_U8, src->total_value_data_size+1);
  RDI_U32 dst_idx = 1;
...
{
  RDI_Procedure *procedures = push_array(RDI_Procedure, src->total_count+1);
  RDI_U32 dst_idx = 1;
  for(RDIM_SymbolChunkNode *n = src->first; n != 0; n = n->next)
  {
    for(RDI_U64 chunk_idx = 0; chunk_idx < n->count; chunk_idx += 1, dst_idx += 1)
...
{
  RDI_U32 *str_offs = rdim_/* no zero */ push_array(RDI_U32, strings->total_count + 1);
...
{
  RDI_U32 *idx_data = rdim_new RDI_U32[idx_runs->idx_count] /* no zero */;
  {
