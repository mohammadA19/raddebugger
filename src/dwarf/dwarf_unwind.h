...
typedef uint16 DW_CFAControlBits;
enum
{
...
internal DW_UnwindResult dw_unwind_x64__apply_frame_rules(String8 raw_eh_frame, DW_CFIRow *row, uint64 text_base_vaddr, DW_ReadMemorySig *read_memory, void *read_memory_ud, uint64 stack_pointer, DW_RegsX64 *regs);

////////////////////////////////
// x64 Unwind Helper Functions

internal void dw_unwind_init_x64(void);
internal uint64  dw_unwind_parse_pointer_x64(void *base, Rng1uint64 range, DW_EhPtrCtx *ptr_ctx, DW_EhPtrEnc ptr_enc, uint64 off, uint64 *ptr_out);

//- eh_frame parsing
internal void dw_unwind_parse_cie_x64(void *base,Rng1uint64 range,DW_EhPtrCtx *ptr_ctx, uint64 off, DW_CIEUnpacked *cie_out);
internal void dw_unwind_parse_fde_x64(void *base,Rng1uint64 range,DW_EhPtrCtx *ptr_ctx, DW_CIEUnpacked *parent_cie, uint64 off, DW_FDEUnpacked *fde_out);
internal DW_CFIRecords dw_unwind_eh_frame_cfi_from_ip_slow_x64(String8 raw_eh_frame, DW_EhPtrCtx *ptr_ctx, uint64 ip_voff);
internal DW_CFIRecords dw_unwind_eh_frame_hdr_from_ip_fast_x64(String8 raw_eh_frame, String8 raw_eh_frame_hdr, DW_EhPtrCtx *ptr_ctx, uint64 ip_voff);

//- cfi machine

internal DW_CFIMachine dw_unwind_make_machine_x64(uint64 cells_per_row, DW_CIEUnpacked *cie, DW_EhPtrCtx *ptr_ctx);
internal void          dw_unwind_machine_equip_initial_row_x64(DW_CFIMachine *machine, DW_CFIRow *initial_row);
internal void          dw_unwind_machine_equip_fde_ip_x64(DW_CFIMachine *machine, uint64 fde_ip);

internal DW_CFIRow* dw_unwind_row_alloc_x64(Arena *arena, uint64 cells_per_row);
internal void       dw_unwind_row_zero_x64(DW_CFIRow *row, uint64 cells_per_row);
internal void       dw_unwind_row_copy_x64(DW_CFIRow *dst, DW_CFIRow *src, uint64 cells_per_row);

internal B32 dw_unwind_machine_run_to_ip_x64(void *base, Rng1uint64 range, DW_CFIMachine *machine, uint64 target_ip, DW_CFIRow *row_out);
