...
internal B32
dw_is_dwarf_present_from_elf_bin(StringView data, ELF_Bin *bin)
{
    ...
    for EachIndex(idx, bin.shdrs.count)
    {
        ...
        StringView name = elf_name_from_shdr64(data, bin, shdr);
...
internal DW_Input
dw_input_from_elf_bin(Arena *arena, StringView data, ELF_Bin *bin)
{
    DW_Input result = {0};
    ...
    for (U64 section_idx = 1; section_idx < bin.shdrs.count; section_idx += 1)
    {
        ...
        //- rjf: unpack section
        StringView section_name = elf_name_from_shdr64(data, bin, shdr);
        DW_SectionKind section_kind = dw_section_kind_from_string(section_name);
        StringView section_data__maybe_compressed = str8_substr(data, r1u64(shdr.sh_offset, shdr.sh_offset + shdr.sh_size));
        B32 is_dwo = 0;
        ...
        StringView section_data__uncompressed = {0};
        if (section_kind != DW_Section_Null)
        {
            ...
            else
            {
                ...
                // rjf: decompress
                {
                    StringView section_data__compressed_contents = str8_skip(section_data__maybe_compressed, chdr_size);
