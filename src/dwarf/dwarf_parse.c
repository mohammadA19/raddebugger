...
internal u64
str8_deserial_read_sleb128_array(Arena *arena, String8 string, u64 off, u64 count, i64 **arr_out)
{
  Temp temp = temp_begin(arena);
  
  i64 *arr = push_array(arena, i64, count);
  u64 i, cursor;
  for (i = 0, cursor = off; i < count; ++i) {
...
dw_abbrev_offset_from_abbrev_id(DW_AbbrevTable table, u64 abbrev_id)
{
  ...
  if (table.count > 0) {
    for (i64 l = 0, r = (i64)table.count - 1; l <= r; ) {
      i64 m = l + (r - l) / 2;
...
{
  local_persist @(rodata) DW_Attrib null_attrib;
    ...
    switch (opcode) {
      default: {
        ...
        if (opcode >= vm_header.opcode_base) {
          ...
          i32 line_inc        = (i32)vm_header.line_base + ((i32)adjusted_opcode) % (i32)vm_header.line_range;
          ...
          vm_state.line            = (u32)((i32)vm_state.line + line_inc);
