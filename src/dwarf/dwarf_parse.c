...
internal U64
str8_deserial_read_dwarf_packed_size(StringView string, U64 off, U64 *size_out)
{
    U64 bytes_read = 0;
...
internal U64
str8_deserial_read_dwarf_uint(StringView string, U64 off, DW_Format format, U64 *uint_out)
{
    U64 bytes_read = 0;
    switch (format) {
...
internal U64
str8_deserial_read_uleb128(StringView string, U64 off, U64 *value_out)
{
    U64 value  = 0;
    U64 shift  = 0;
    U64 cursor = off;
...
internal U64
str8_deserial_read_sleb128(StringView string, U64 off, S64 *value_out)
{
    U64 value  = 0;
    U64 shift  = 0;
    U64 cursor = off;
...
internal U64
str8_deserial_read_uleb128_array(Arena *arena, StringView string, U64 off, U64 count, U64 **arr_out)
{
...
internal U64
str8_deserial_read_sleb128_array(Arena *arena, StringView string, U64 off, U64 count, S64 **arr_out)
{
    Temp temp = temp_begin(arena);
...
dw_section_kind_from_string(StringView string)
{
...
internal DW_SectionKind
dw_section_dwo_kind_from_string(StringView string)
{
    DW_SectionKind s = DW_Section_Null;
#define X(_K,_L,_M,_W)                                        \
...
dw_unit_ranges_from_data(Arena *arena, StringView data)
{
...
internal U64
dw_read_list_unit_header_addr(StringView unit_data, DW_ListUnit *lu_out)
{
    U64 header_size = 0;
...
internal U64
dw_read_list_unit_header_str_offsets(StringView unit_data, DW_ListUnit *lu_out)
{
    U64 header_size = 0;
...
internal U64
dw_read_list_unit_header_list(StringView unit_data, DW_ListUnit *lu_out)
{
    U64 header_size = 0;
...
dw_list_unit_input_from_input(Arena *arena, DW_Input *input)
{
    ...
    {
        StringView     data        = debug_addr.data;
        ...
        for (U64 unit_idx = 0; unit_idx < result.addr_ranges.count; ++unit_idx) {
            StringView unit_data = str8_substr(debug_addr.data, result.addr_ranges.v[unit_idx]);
    ...
    {
        StringView     data        = debug_str_offsets.data;
        Rng1U64List unit_ranges = dw_unit_ranges_from_data(scratch.arena, data);
        ...
        for (U64 unit_idx = 0; unit_idx < result.str_offset_ranges.count; ++unit_idx) {
            StringView unit_data = str8_substr(data, result.str_offset_ranges.v[unit_idx]);
    ...
    {
        StringView     data        = debug_rnglists.data;
        Rng1U64List unit_ranges = dw_unit_ranges_from_data(scratch.arena, data);
        ...
        for (U64 unit_idx = 0; unit_idx < result.rnglist_ranges.count; ++unit_idx) {
            StringView unit_data = str8_substr(data, result.rnglist_ranges.v[unit_idx]);
    ...
    {
        StringView     data        = debug_loclists.data;
        Rng1U64List unit_ranges = dw_unit_ranges_from_data(scratch.arena, data);
        ...
        for (U64 unit_idx = 0; unit_idx < result.loclist_ranges.count; ++unit_idx) {
            StringView unit_data = str8_substr(data, result.loclist_ranges.v[unit_idx]);
...
internal U64
dw_read_abbrev_tag(StringView data, U64 offset, DW_Abbrev *out_abbrev)
{
...
internal U64
dw_read_abbrev_attrib(StringView data, U64 offset, DW_Abbrev *out_abbrev)
{
    U64 total_bytes_read = 0;
...
dw_make_abbrev_table(Arena *arena, StringView abbrev_data, U64 abbrev_offset)
{
...
{
    StringView tag_data = str8_substr(input.sec[DW_Section_Info].data, cu.info_range);
...
internal B32
dw_try_u64_from_const_value(U64 type_byte_size, DW_ATE type_encoding, StringView const_value, U64 *value_out)
{
    B32 is_parsed = 0;
...
internal U64
dw_u64_from_const_value(StringView const_value)
{
    U64 result       = 0;
...
dw_interp_loclist(Arena *arena, DW_Input *input, DW_CompUnit *cu, DW_FormKind form_kind, DW_Form form)
{
    ...
    if (cu.version < DW_Version_5) {
        if (form_kind == DW_Form_SecOffset) {
            ...
            StringView sec       = str8_skip(input.sec[DW_Section_Loc].data, sec_offset);
...
dw_interp_rnglist(Arena *arena, DW_Input *input, DW_CompUnit *cu, DW_FormKind form_kind, DW_Form form)
{
    ...
    if (cu.version < DW_Version_5) {
        ...
        StringView sec       = str8_skip(input.sec[DW_Section_Ranges].data, sec_offset);
        U64     base_addr = cu.low_pc;
        U64     base_sel  = DW_SentinelFromSize(cu.address_size);
        for (U64 cursor = 0; cursor < sec.size; ) {
            U64 range_min      = 0;
...
dw_interp_secptr(DW_Input *input, DW_SectionKind section, DW_FormKind form_kind, DW_Form form)
{
    StringView secptr = {0};
    if (form_kind == DW_Form_SecOffset) {
        StringView sect  = input.sec[section].data;
...
dw_cu_from_info_off(Arena *arena, DW_Input *input, DW_ListUnitInput lu_input, U64 offset, B32 relaxed)
{
    ...
    StringView info_data = input.sec[DW_Section_Info].data;
    ...
    if (length_size) {
        ...
        StringView data   = str8_substr(info_data, range);
        ...
        if (version_size) {
            ...
            if (is_header_ok) {
                ...
                StringView        abbrev_data  = input.sec[DW_Section_Abbrev].data;
...
internal void
dw_tag_tree_from_data(Arena *arena, StringView info_data, StringView abbrev_data, DW_CompUnit *cu, DW_TagNode *parent, U64 *cursor, U64 *tag_count)
{
...
{
    StringView    abbrev_data = input.sec[DW_Section_Abbrev].data;
    StringView    info_data   = str8_substr(input.sec[DW_Section_Info].data, cu.info_range);
    DW_TagNode root        = {0};
...
internal U64
dw_read_line_file(StringView       data,
                                    U64           off,
                                    DW_Input     *input,
                                    DW_Version    version,
                                    DW_Format     format,
                                              ...
                                              DW_LineVMHeader *header_out)
{
    ...
    StringView   unit_data   = str8_substr(line_data, unit_range);
...
{
    Temp scratch = scratch_begin(&arena, 1);
    StringView      dir   = vm.dir_table.v[file.dir_idx].file_name;
...
dw_v4_pub_strings_table_from_section_kind(Arena *arena, DW_Input *input, DW_SectionKind section_kind)
{
    ...
    StringView section_data = input.sec[section_kind].data;
