...
internal uint64 dw_based_range_read(void *base, Rng1uint64 range, uint64 off, uint64 size, void *out) { return 0; }
internal uint64 dw_based_range_read_uleb128(void *base, Rng1uint64 range, uint64 off, uint64 *out)    { return 0; }
internal uint64 dw_based_range_read_sleb128(void *base, Rng1uint64 range, uint64 off, uint64 *out)    { return 0; }
internal uint64 dw_based_range_read_length(void *base, Rng1uint64 range, uint64 off, uint64 *out)     { return 0; }
...
dw_unwind_x64(String8           raw_text,
              ...
              Rng1uint64           eh_frame_hdr_vrange,
              ...
              void             *read_memory_ud)
{
  ...
  Rng1uint64  frame_range = rng_1u64(0, raw_eh_frame.size);
  
  //- section vaddrs
  uint64 text_base_vaddr = text_vrange.min + rebase_voff_to_vaddr;
  uint64 frame_base_voff = text_vrange.min;
  uint64 data_base_vaddr = eh_frame_hdr_vrange.min + rebase_voff_to_vaddr;
  ...
  if (cfi_recs.valid) {
    Rng1uint64    init_cfi_range = cfi_recs.cie.cfi_range;
    DW_CFIRow *row            = dw_unwind_row_alloc_x64(scratch.arena, machine.cells_per_row);
    if (dw_unwind_machine_run_to_ip_x64(frame_base, init_cfi_range, &machine, max_uint64, row)) {
      init_row = row;
    }
    if (init_row == 0) {
      result.is_invalid = 1;
  ...
  if (init_row != 0) {
    ...
    // decode main row
    Rng1uint64    main_cfi_range = cfi_recs.fde.cfi_range;
    DW_CFIRow *row            = dw_unwind_row_alloc_x64(scratch.arena, machine.cells_per_row);
                                 ...
                                 DW_RegsX64       *regs)
{
  ...
  switch (row->cfa_cell.rule) {
    ...
    case DW_CFI_CFA_Rule_Expr: {
      Rng1uint64     expr_range = row->cfa_cell.expr;
  ...
  // compute registers
  {
    ...
    for (uint64 i = 0; i < DW_UNWIND_X64__REG_SLOT_COUNT; ++i, ++cell) {
      ...
      switch (cell->rule) {
        ...
        {
          Rng1uint64     expr_range = cell->expr;
          uint64         addr       = 0;
          DW_Location location   = dw_expr__eval(0, raw_eh_frame.str, expr_range, &dwexpr_config);
          if (location.non_piece_loc.kind == DW_SimpleLocKind_Fail && location.non_piece_loc.fail_kind == DW_LocFailKind_MissingMemory) {
            missed_read_addr = location.non_piece_loc.fail_data;
...
internal uint64
dw_unwind_parse_pointer_x64(void *frame_base, Rng1uint64 frame_range, DW_EhPtrCtx *ptr_ctx, DW_EhPtrEnc encoding, uint64 off, uint64 *ptr_out)
{
  // aligned offset
  uint64 pointer_off = off;
  ...
  uint64 size_param        = 0;
  uint64 after_pointer_off = 0;
  uint64 raw_pointer       = 0;
  switch (encoding & DW_EhPtrEnc_TypeMask) {
    ...
    sfixed:
    {
      ...
      after_pointer_off = pointer_off + size_param;
      // sign extension
      uint64 sign_bit = size_param*8 - 1;
    ...
    {
      uint64 size = dw_based_range_read_uleb128(frame_base, frame_range, pointer_off, &raw_pointer);
      after_pointer_off = pointer_off + size;
    } break;
    
    case DW_EhPtrEnc_SLEB128:
    {
      uint64 size = dw_based_range_read_sleb128(frame_base, frame_range, pointer_off,
                                             (uint64*)&raw_pointer);
      after_pointer_off = pointer_off + size;
    } break;
  }
  ...
  // return
  *ptr_out = pointer;
  uint64 result = after_pointer_off - off;
  return(result);
}
...
internal void
dw_unwind_parse_cie_x64(void *base, Rng1uint64 range, DW_EhPtrCtx *ptr_ctx, uint64 off, DW_CIEUnpacked *cie_out)
{
  NotImplemented;
#if 0
  ...
  if (version == 1 || version == 3) {
    ...
    // read augmentation
    uint64     augmentation_off = version_off + 1;
    String8 augmentation     = dw_based_range_read_string(base, range, augmentation_off);
    
    // read code align
    uint64 code_align_factor_off  = augmentation_off + augmentation.size + 1;
    uint64 code_align_factor      = 0;
    uint64 code_align_factor_size = dw_based_range_read_uleb128(base, range, code_align_factor_off, &code_align_factor);
    
    // read data align
    uint64 data_align_factor_off  = code_align_factor_off + code_align_factor_size;
    uint64 data_align_factor      = 0;
    uint64 data_align_factor_size = dw_based_range_read_sleb128(base, range, data_align_factor_off, &data_align_factor);
    
    // return address register
    uint64 ret_addr_reg_off       = data_align_factor_off + data_align_factor_size;
    uint64 after_ret_addr_reg_off = 0;
    uint64 ret_addr_reg           = 0;
    if (version == 1) {
      dw_based_range_read(base, range, ret_addr_reg_off, 1, &ret_addr_reg);
      after_ret_addr_reg_off = ret_addr_reg_off + 1;
    } else {
      uint64 ret_addr_reg_size = dw_based_range_read_uleb128(base, range, ret_addr_reg_off, &ret_addr_reg);
    ...
    uint64 aug_size_off          = after_ret_addr_reg_off;
    ...
    uint64 aug_data_off       = after_aug_size_off;
    uint64 after_aug_data_off = after_aug_size_off;
      ...
      for (uint8 *ptr = augmentation.str + 1, *opl = augmentation.str + augmentation.size; ptr < opl; ++ptr) {
        switch (*ptr) {
          ...
          case 'P': {
            ...
            uint64 ptr_off  = aug_data_cursor + sizeof(handler_encoding);
            uint64 ptr_size = dw_unwind_parse_pointer_x64(base, range, ptr_ctx, handler_encoding, ptr_off, &handler_ip);
    ...
    // cfi range
    uint64 cfi_off  = range.min + after_aug_data_off;
    uint64 cfi_size = 0;
...
internal void
dw_unwind_parse_fde_x64(void *base, Rng1uint64 range, DW_EhPtrCtx *ptr_ctx, DW_CIEUnpacked *cie, uint64 off, DW_FDEUnpacked *fde_out)
{
  ...
  uint64 ip_first      = 0;
  uint64 ip_first_size = dw_unwind_parse_pointer_x64(base, range, ptr_ctx, ptr_enc, ip_first_off, &ip_first);
  
  // ip range size
  uint64 ip_range_size_off  = ip_first_off + ip_first_size;
  uint64 ip_range_size      = 0;
  uint64 ip_range_size_size = dw_unwind_parse_pointer_x64(base, range, ptr_ctx, ptr_enc & DW_EhPtrEnc_TypeMask, ip_range_size_off, &ip_range_size);
  
  // augmentation data
  uint64 aug_data_off       = ip_range_size_off + ip_range_size_size;
...
dw_unwind_eh_frame_cfi_from_ip_slow_x64(String8 raw_eh_frame, DW_EhPtrCtx *ptr_ctx, uint64 ip_voff)
{
  Temp scratch = scratch_begin(0, 0);
  ...
  for (;;) {
    ...
    // compute end offset
    uint64 rec_opl = rec_off + after_rec_size_off + rec_size;
    
    // sub-range the rest of the reads
    Rng1uint64 rec_range = rng_1u64(rec_off, rec_opl);
    String8 raw_rec   = str8_substr(raw_eh_frame, rec_range);
    
    
    // discriminator
    uint64 discrim_off = after_rec_size_off;
    uint32 discrim     = 0;
    str8_deserial_read(raw_rec, discrim_off, &discrim, 4, 1);
    
    uint64 after_discrim_off = discrim_off + 4;
    ...
    // FDE
      ...
      // compute cie offset
      uint64 cie_offset = rec_range.min + discrim_off - discrim;
...
internal uint64
dw_search_eh_frame_hdr_linear_x64(String8 raw_eh_frame_hdr, DW_EhPtrCtx *ptr_ctx, uint64 location)
{
  ...
  uint64 closest_location = max_uint64;
  uint64 closest_address  = max_uint64;
  ...
  if (version == 1) {
    ...
    for (uint64 fde_idx = 0; fde_idx < fde_count; ++fde_idx) {
      ...
      uint64 current_delta = (uint64)(location - init_location);
      uint64 closest_delta = (uint64)(location - closest_location);
...
internal DW_CFIRecords
dw_unwind_eh_frame_hdr_from_ip_fast_x64(String8 raw_eh_frame, String8 raw_eh_frame_hdr, DW_EhPtrCtx *ptr_ctx, uint64 ip_voff)
{
  DW_CFIRecords result = {0};
  ...
  B32 is_fde_offset_valid = (fde_offset != max_uint64);
  if (is_fde_offset_valid) {
    ...
    uint64 cie_read_offset = fde_read_offset - (fde_discrim + sizeof(fde_discrim));
    ...
    if (is_fde && is_cie) {
      Rng1uint64 cie_range = rng_1u64(0, cie_read_offset + (cie_size - sizeof(cie_discrim)));
      Rng1uint64 fde_range = rng_1u64(0, fde_read_offset + (fde_size - sizeof(fde_discrim)));
...
dw_unwind_make_machine_x64(uint64 cells_per_row, DW_CIEUnpacked *cie, DW_EhPtrCtx *ptr_ctx)
{
...
internal B32
dw_unwind_machine_run_to_ip_x64(void *base, Rng1uint64 range, DW_CFIMachine *machine, uint64 target_ip, DW_CFIRow *row)
{
  Temp scratch = scratch_begin(0, 0);
  ...
  for (;;) {
    ...
    uint64 decode_cursor = cfi_off + 1;
    {
      // setup loop ins/outs
      uint64 o[2];
      ...
      for (uint64 i = 0; i < 2; i += 1, out += 1) {
        ...
        switch (d) {
          ...
          case DW_CFADecode_SLEB128: {
            o_size = dw_based_range_read_sleb128(base, range, decode_cursor, (uint64*)out);
          } break;
        }
    ...
    uint64 after_decode_off = decode_cursor;
    ...
    uint64 new_table_ip = table_ip;
    
    // step
    uint64 step_cursor = after_decode_off;
    switch (opcode) {
      default: goto done;
      ...
      case DW_CFA_DefCfaSf: {
        row->cfa_cell.rule = DW_CFI_CFA_Rule_RegOff;
        row->cfa_cell.reg_idx = operand1;
        row->cfa_cell.offset = ((uint64)operand2)*cie->data_align_factor;
      } break;
      ...
      case DW_CFA_DefCfaOffsetSf: {
        ...
        // commit new cfa
        row->cfa_cell.offset = ((uint64)operand1)*cie->data_align_factor;
      } break;
      
      case DW_CFA_DefCfaExpr: {
        // setup expr range
        uint64 expr_first = range.min + after_decode_off;
        uint64 expr_size  = operand1;
      ...
      case DW_CFA_OffsetExtSf: {
        DW_CFICell *cell = &row->cells[operand1];
        cell->rule       = DW_CFIRegisterRule_Offset;
        cell->n          = ((uint64)operand2)*cie->data_align_factor;
      } break;
