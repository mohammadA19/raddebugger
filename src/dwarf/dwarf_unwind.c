...
uint64
dw_unwind_parse_pointer_x64(void* frame_base, Rng1U64 frame_range, DW_EhPtrCtx* ptr_ctx, DW_EhPtrEnc encoding, uint64 off, uint64* ptr_out)
{
  // aligned offset
...
void
dw_unwind_parse_cie_x64(void* base, Rng1U64 range, DW_EhPtrCtx* ptr_ctx, uint64 off, DW_CIEUnpacked* cie_out)
{
...
DW_CFIRecords
dw_unwind_eh_frame_cfi_from_ip_slow_x64(StringView raw_eh_frame, DW_EhPtrCtx* ptr_ctx, uint64 ip_voff)
{
  Temp scratch = scratch_begin(0, 0);
...
uint64
dw_search_eh_frame_hdr_linear_x64(StringView raw_eh_frame_hdr, DW_EhPtrCtx* ptr_ctx, uint64 location)
{
  ...
  uint64 closest_location = max_U64;
  uint64 closest_address  = max_U64;
...
DW_CFIRecords
dw_unwind_eh_frame_hdr_from_ip_fast_x64(StringView raw_eh_frame, StringView raw_eh_frame_hdr, DW_EhPtrCtx* ptr_ctx, uint64 ip_voff)
{
  DW_CFIRecords result = {0};
...
B32
dw_unwind_machine_run_to_ip_x64(void* base, Rng1U64 range, DW_CFIMachine* machine, uint64 target_ip, DW_CFIRow* row)
{
  Temp scratch = scratch_begin(0, 0);
      ...
      for (uint64 i = 0; i < 2; i += 1, out += 1) {
        ...
        switch (d) {
          ...
          case DW_CFADecode_SLEB128: {
            o_size = dw_based_range_read_sleb128(base, range, decode_cursor, (int64*)out);
          } break;
        }
