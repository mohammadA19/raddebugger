...
{
  switch (kind) {
    case DW_TagKind_Null: return ("Null");
#define X(_N,_ID) case DW_TagKind_##_N: return (Stringify(_N));
...
#undef X
  string result = push_str8f(arena, "%x", kind);
  return result;
}
...
{
  switch (kind) {
#define X(_N,_ID) case _ID: return (Stringify(_N));
    DW_Inl_XList(X)
#undef X
  }
  return push_str8f(arena, "%x", kind);
...
{
  string reg_str = str8_zero();
  switch (arch) {
    case Arch_Null: break;
    case Arch_x86: {
      switch (reg_id) {
#define X(_N, _ID, ...) case DW_RegX86_##_N: reg_str = (Stringify(_N)); break;
        DW_Regs_X86_XList(X)
#undef X
      }
    } break;
    case Arch_x64: {
      switch (reg_id) {
#define X(_N, _ID, ...) case DW_RegX64_##_N: reg_str = (Stringify(_N)); break;
        DW_Regs_X64_XList(X)
#undef X
      }
    } break;
