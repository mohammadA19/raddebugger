...
internal String8
dw_string_from_reg_off(Arena *arena, Arch arch, uint64 reg_idx, uint64 reg_off)
{
...
dw_string_list_from_expression(Arena *arena, String8 raw_data, uint64 cu_base, uint64 address_size, Arch arch, DW_Version ver, DW_Ext ext, DW_Format format)
{
  Temp scratch = scratch_begin(&arena, 1);
  ...
  for (uint64 cursor = 0; cursor < raw_data.size; ) {
    ...
    switch (op) {
      ...
      case DW_ExprOp_ImplicitValue: {
        ...
        Rng1uint64 value_range = rng_1u64(cursor, cursor + value_size);
      ...
      case DW_ExprOp_GNU_ConstType: {
        uint64 type_cu_off      = 0;
        uint8  const_value_size = 0;
        cursor += str8_deserial_read_uleb128(raw_data, cursor, &type_cu_off);
        cursor += str8_deserial_read_struct(raw_data, cursor, &const_value_size);
        Rng1uint64 const_value_range = rng_1u64(cursor, cursor + const_value_size);
      ...
      case DW_ExprOp_GNU_RegvalType: {
        uint64 reg_idx = 0, type_cu_off = 0;
        cursor += str8_deserial_read_uleb128(raw_data, cursor, &reg_idx);
        cursor += str8_deserial_read_uleb128(raw_data, cursor, &type_cu_off);
      ...
      case DW_ExprOp_GNU_EntryValue: {
        ...
        Rng1uint64     block_range = rng_1u64(cursor, cursor + block_size);
...
internal String8
dw_single_line_string_from_expression(Arena *arena, String8 raw_data, uint64 cu_base, uint64 address_size, Arch arch, DW_Version ver, DW_Ext ext, DW_Format format)
{
  Temp        scratch    = scratch_begin(&arena, 1);
  ...
  for (uint64 cursor = 0; cursor < raw_data.size; /* empty */) {
    ...
    switch (opcode) {
      ...
      case DW_CFA_Offset: {
        uint64 offset = 0;
        cursor += str8_deserial_read_uleb128(raw_data, cursor, &offset);
        uint64 v = (uint64)offset * cie->data_align_factor;
...
dw_print_eh_frame(Arena *arena, String8List *out, String8 indent, String8 raw_eh_frame, Arch arch, DW_Version ver, DW_Ext ext, DW_EhPtrCtx *ptr_ctx)
{
  ...
  for (uint64 cursor = 0; cursor < raw_eh_frame.size; ) {
    ...
    Rng1uint64 cfi_range = rng_1u64(0,0);
...
dw_print_debug_loc(Arena *arena, String8List *out, String8 indent, DW_Input *input, Arch arch, ExecutableImageKind image_type, B32 relaxed)
{
  ...
  Rng1uint64List cu_range_list = dw_comp_unit_ranges_from_info(scratch.arena, info);
  
  // parse debug_info for attributes with LOCLIST and store .debug_loc offsets 
  uint64List    *loc_lists     = push_array(scratch.arena, uint64List,    cu_range_list.count);
  uint64        *address_sizes = push_array(scratch.arena, uint64,        cu_range_list.count);
  uint64        *address_bases = push_array(scratch.arena, uint64,        cu_range_list.count);
  uint64        *cu_bases      = push_array(scratch.arena, uint64,        cu_range_list.count);
  ...
  uint64 comp_idx = 0;
  for (Rng1uint64Node *cu_range_n = cu_range_list.first; cu_range_n != 0; cu_range_n = cu_range_n->next, ++comp_idx) {
    ...
    // parse tags
    for (uint64 info_off = cu.tags_range.min; info_off < cu.tags_range.max; /* empty */) {
  ...
  Rng1uint64  range = dw_range_from_sec(input, DW_Section_Loc);
    ...
    for (uint64 loc_idx = 0; loc_idx < locs_set.count; ++loc_idx) {
      ...
      for (uint64 cursor = locs_set.v[loc_idx]; cursor < dim_1u64(range); /* empty */) {
        ...
        if (is_list_end) {
          ...
          Rng1uint64 expr_range = rng_1u64(range.min+cursor, range.min+cursor+expr_size);
          cursor += expr_size;
...
dw_print_debug_ranges(Arena *arena, String8List *out, String8 indent, DW_Input *input, Arch arch, ExecutableImageKind image_type, B32 relaxed)
{
  ...
  Rng1uint64     range  = dw_range_from_sec(input, DW_Section_Ranges);
  ...
  Rng1uint64List  cu_range_list = dw_comp_unit_ranges_from_info(scratch.arena, sections->v[DW_Section_Info]);
...
dw_print_debug_aranges(Arena *arena, String8List *out, String8 indent, DW_Input *input)
{
  ...
  Rng1uint64  range = dw_range_from_sec(sections, DW_Section_ARanges);
  ...
  for (uint64 cursor = 0; cursor < dim_1u64(range); ) {
    ...
    B32 is_dwarf64 = unit_length >= max_uint32;
...
dw_print_debug_addr(Arena *arena, String8List *out, String8 indent, DW_Input *input)
{
  ...
  Rng1uint64  range = dw_range_from_sec(sections, DW_Section_Addr);
...
internal uint64
dw_based_range_read_address(void *base, Rng1uint64 range, uint64 offset, Rng1uint64Array segment_ranges, uint8 segment_selector_size, uint8 address_size, uint64 *address_out)
{
...
internal void
dw_print_debug_loclists(Arena *arena, String8List *out, String8 indent, DW_Input *input, Rng1uint64Array segment_virtual_ranges, Arch arch)
{
  NotImplemented;
#if 0
  void    *base  = dw_base_from_sec(sections, DW_Section_LocLists);
  Rng1uint64  range = dw_range_from_sec(sections, DW_Section_LocLists);
...
internal void
dw_print_debug_rnglists(Arena *arena, String8List *out, String8 indent, DW_Input *input, Rng1uint64Array segment_ranges)
{
  NotImplemented;
#if 0
  void    *base  = dw_base_from_sec(sections, DW_Section_RngLists);
  Rng1uint64  range = dw_range_from_sec(sections, DW_Section_RngLists);
...
dw_format_string_table(Arena *arena, String8List *out, String8 indent, DW_Input *input, DW_SectionKind sec)
{
  ...
  Rng1uint64  range = dw_range_from_sec(sections, sec);
...
dw_print_debug_line_str(Arena *arena, String8List *out, String8 indent, DW_Input *input)
{
  ...
  Rng1uint64  range = dw_range_from_sec(sections, DW_Section_LineStr);
...
dw_print_debug_str_offsets(Arena *arena, String8List *out, String8 indent, DW_Input *input)
{
  ...
  Rng1uint64  range = dw_range_from_sec(sections, DW_Section_StrOffsets);
  
  void    *debug_str_base  = dw_base_from_sec(sections, DW_Section_Str);
  Rng1uint64  debug_str_range = dw_range_from_sec(sections, DW_Section_Str);
...
#define DumpSubset(name) if(subset_flags & DW_DumpSubsetFlag_##name) DeferLoop(dumpf("// %S\n\n", dw_name_title_from_dump_subset_table[DW_DumpSubset_##name]), dump(str8_lit("\n")))
  Temp scratch = scratch_begin(&arena, 1);
  Rng1uint64Array segment_vranges = {0};
  DW_ListUnitInput lu_input = dw_list_unit_input_from_input(scratch.arena, input);
  Rng1uint64List unit_ranges_list = dw_unit_ranges_from_data(scratch.arena, input->sec[DW_Section_Info].data);
  Rng1uint64Array unit_ranges = rng1u64_array_from_list(scratch.arena, &unit_ranges_list);
  B32 relaxed  = 1;
  ...
  DumpSubset(DebugInfo)
  {
    for EachIndex(unit_idx, unit_ranges.count)
    {
      ...
      //- rjf: unpack unit
      Rng1uint64 unit_range = unit_ranges.v[unit_idx];
      ...
      uint64 tag_depth = 0;
      uint64 tag_idx = 0;
      for(uint64 info_off = unit.first_tag_info_off; info_off < unit.info_range.max; tag_idx += 1)
      {
        Temp tag_temp = temp_begin(scratch.arena);
            ...
            attrib_n = attrib_n->next)
        {
          ...
          switch(value_class)
          {
            ...
            case DW_AttribClass_Reference:
            {
              ...
              {
                uint64 info_off = unit.info_range.min + attrib->form.ref;
          ...
          switch(attrib->attrib_kind)
          {
            ...
            {
              uint64          file_idx = dw_const_u64_from_attrib(input, &unit, attrib);
  ...
  DumpSubset(DebugAbbrev)
  {
    ...
    uint64 depth = 0;
    uint64 idx = 0;
    for(uint64 cursor = 0; cursor < abbrev.data.size; idx += 1)
    {
      // rjf: read id & advance
      uint64 id_off = cursor;
      uint64 id = 0;
  ...
  DumpSubset(DebugLine)
  {
    for EachIndex(unit_idx, unit_ranges.count)
    {
      ...
      DW_LineVMHeader line_vm        = {0};
      uint64             line_vm_size   = dw_read_line_vm_header(unit_temp.arena, unit_data, 0, input, cu_dir, cu_name, line_vm.address_size, &cu_str_offsets, &line_vm);
      if(line_vm_size == 0)
      {
        continue;
      }
      ...
      DeferLoop(dumpf("  opcodes:\n  {\n"), dumpf("  }\n\n"))
      {
        ...
        for(uint64 cursor = 0; cursor < opcodes.size;)
        {
          ...
          switch(opcode)
          {
            default:
            {
              if(opcode >= line_vm.opcode_base)
              {
                ...
                uint32 op_advance      = 0;
                uint32 line_advance    = 0;
                uint64 addr_advance    = 0;
                if(line_vm.line_range > 0 && line_vm.max_ops_for_inst > 0)
                {
                  adjusted_opcode = (uint32)(opcode - line_vm.opcode_base);
                  op_advance      = adjusted_opcode / line_vm.line_range;
                  line_advance    = (uint32)line_vm.line_base + ((uint32)adjusted_opcode) % (uint32)line_vm.line_range;
                ...
                vm_state.line            = (uint32)((uint32)vm_state.line + line_advance);
              ...
              else
              {
                if(opcode > 0 && opcode <= line_vm.num_opcode_lens)
                {
                  ...
                  uint64 num_operands = line_vm.opcode_lens[opcode - 1];
            ...
            {
              uint64 advance = (0xffu - line_vm.opcode_base)/line_vm.line_range;
              dw_line_vm_advance(&vm_state, advance, line_vm.min_inst_len, line_vm.max_ops_for_inst);
