...
dw_expr__analyze_fast(void *base, Rng1uint64 range, uint64 text_section_base)
{
  DW_SimpleLoc result = {DW_SimpleLocKind_Empty};
  
  String8 expr_data = str8((uint8*)data+range.min, (uint8*)data+range.max);
  ...
  if (str8_deserial_read_struct(expr_data, 0, &op)) {
    ...
    switch (op) {
      ...
      {
        uint64 x = 0;
        step_cursor += dw_based_range_read_sleb128(base, range, step_cursor, (uint64*)&x);
        result.kind = DW_SimpleLocKind_Address;
        result.addr = x;
      } break;
...
dw_expr__analyze_details(void *in_base, Rng1uint64 in_range, DW_ExprMachineCallConfig *call_config)
{
  ...
  for (;;) {
    ...
    Rng1uint64  task_range = rng_1u64(0, task_data.size);
    ...
    for (uint64 cursor = 0;;) {
      ...
      if (dw_based_range_read(task_base, task_range, op_offset, 1, &op)) {
        ...
        switch (op) {
          ...
          {
            uint64 x = 0;
            step_cursor += dw_based_range_read_sleb128(task_base, task_range, step_cursor, (uint64*)&x);
          } break;
          ...
          {
            uint64 reg_idx = 0; uint64 offset = 0;
...
dw_expr__eval(Arena *arena_optional, void *expr_base, Rng1uint64 expr_range, DW_ExprMachineConfig *config)
{
#if 0
  ...
  for (;;) {
    ...
    void    *base   = call->ptr;
    Rng1uint64  range  = rng_1u64(0, call->size);
    ...
    if (dw_based_range_read(base, range, op_offset, 1, &op)) {
      ...
      switch (op) {
        ...
        case DW_ExprOp_FormTlsAddress:
        {
          uint64 s = (uint64)dw_expr__stack_pop(&stack);
        ...
        case DW_ExprOp_Div:
        {
          uint64 d = (uint64)dw_expr__stack_pop(&stack);
          uint64 n = (uint64)dw_expr__stack_pop(&stack);
          uint64 x = (d == 0)?0:n/d;
          dw_expr__stack_push(scratch.arena, &stack, x);
        } break;
        ...
        case DW_ExprOp_Le:
        {
          uint64 b = (uint64)dw_expr__stack_pop(&stack);
          uint64 a = (uint64)dw_expr__stack_pop(&stack);
          uint64 x = (a <= b);
          dw_expr__stack_push(scratch.arena, &stack, x);
        } break;
