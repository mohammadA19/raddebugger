...
{
  AssertAlways(x <= max_uint32);
  uint32 result = (uint32)x;
  return result;
}

internal uint32
safe_cast_s32(uint64 x)
{
  AssertAlways(x <= max_uint32);
  uint32 result = (uint32)x;
  return result;
...
internal B32
u128_match(uint128 a, uint128 b)
{
...
u32_from_u64_saturate(uint64 x){
  uint32 x32 = (x > max_uint32)?max_uint32:(uint32)x;
  return(x32);
}
...
extend_sign32(uint32 x, uint32 size){
  ...
  uint32 result = ((uint32)x << shift) >> shift;
  return result;
}
...
extend_sign64(uint64 x, uint64 size){
  ...
  uint64 result = ((uint64)x << shift) >> shift;
  return result;
}
...
internal uint32
sign_from_side_uint32(Side side){
  return((side == Side_Min)?-1:1);
}

internal float
sign_from_side_float(Side side){
...
internal TxtPt
txt_pt(uint64 line, uint64 column)
{
  TxtPt p = {0};
  p.line = line;
  p.column = column;
...
internal uint64
u64_array_bsearch(uint64 *arr, uint64 count, uint64 value)
{
...
internal uint64
index_of_zero_u32(uint32 *ptr, uint64 count)
{
...
internal uint64
index_of_zero_u64(uint64 *ptr, uint64 count)
{
  for (uint64 i = 0; i < count; i += 1) {
    if (ptr[i] == 0) {
      return i;
