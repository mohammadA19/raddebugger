  ...
  for(RebaseTypeTask *t = first_task; t != 0; t = t->next)
  {
    switch(t->type->kind)
    {
      ...
      case TypeKind_Array:
      {
        ...
        {
          RebaseTypeTask *task = new RebaseTypeTask[1];
    ...
    for(SerializeTypeTask *t = first_task; t != 0; t = t->next)
    {
      switch(t->type->kind)
      {
        ...
        case TypeKind_Ptr:
        {
          ...
          {
            SerializeTypeTask *task = new SerializeTypeTask[1];
            task->type  = t->type;
            task->count = t->count;
            task->src   = t->src;
    ...
    for(DeserializeTypeTask *t = first_task; t != 0; t = t->next)
    {
      ...
      switch(t->type->kind)
      {
        ...
        case TypeKind_Ptr:
        {
          ...
          {
            DeserializeTypeTask *task = new DeserializeTypeTask[1];
            task->type  = t->type;
            task->count = t->count;
            task->dst   = t->dst;
            task->containing_type = t->containing_type;
          ...
          else if(t->type->count_delimiter_name.size != 0 && t->is_post_header)
          {
            ...
            U8 *ptr_dest_buffer = new U8[ptr_dest_buffer_size];
