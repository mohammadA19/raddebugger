...
Member *
member_from_name(Type* type, StringView name)
{
...
void
typed_data_rebase_ptrs(Type* type, StringView data, void* base_ptr)
{
  ...
  for(RebaseTypeTask* t = first_task; t != 0; t = t.next)
  {
    switch(t.type.kind)
    {
      ...
      {
        *(uint64 *)t.ptr = ((uint64)(*(uint8 **)t.ptr - (uint8 *)base_ptr));
      }break;
      case TypeKind_Array:
      {
        for(uint64 idx = 0; idx < t.type.count; idx += 1)
        {
...
StringView
serialized_from_typed_data(Arena* arena, Type* type, StringView data, TypeSerializeParams* params)
{
  ...
  str8_serial_begin(scratch.arena, &strings);
  {
    ...
    for(SerializeTypeTask* t = first_task; t != 0; t = t.next)
    {
      switch(t.type.kind)
      {
        ...
        {
          str8_serial_push_string(scratch.arena, &strings, StringView(t.src, t.type.size*t.count));
        }break;
        ...
        case TypeKind_Ptr:
        {
          ...
          for(uint64 idx = 0; idx < params.ptr_ref_infos_count; idx += 1)
          {
          ...
          if(ptr_ref_info != 0 && ptr_ref_info.indexify_base != 0)
          {
            ...
            uint64 ptr_write_value = ((uint64)((uint8 *)ptr_value - (uint8 *)ptr_ref_info.indexify_base)/t.type.direct.size);
          ...
          else if(ptr_ref_info != 0 && ptr_ref_info.offsetify_base != 0)
          {
            uint64 ptr_value = 0;
            MemoryCopy(&ptr_value, t.src, sizeof(ptr_value));
            uint64 ptr_write_value = (uint64)((uint8 *)ptr_value - (uint8 *)ptr_ref_info.offsetify_base);
            str8_serial_push_struct(scratch.arena, &strings, &ptr_write_value);
          }
        ...
        {
          uint64 off = 0;
          for(uint64 idx = 0; idx < t.count; idx += 1)
          {
            for(uint64 member_idx = 0; member_idx < t.type.count; member_idx += 1)
            {
  ...
  StringView result = str8_serial_end(arena, &strings);
  scratch_end(scratch);
  return result;
}

StringView
deserialized_from_typed_data(Arena* arena, Type* type, StringView data, TypeSerializeParams* params)
{
  StringView result = {0};
      ...
      switch(t.type.kind)
      {
        ...
        case TypeKind_Ptr:
        {
          ...
          if(ptr_ref_info != 0 && ptr_ref_info.indexify_base != 0)
          {
            ...
            uint64 ptr_write_value = (ptr_value + (uint64)ptr_ref_info.indexify_base) * t.type.direct.size;
          ...
          else if(ptr_ref_info != 0 && ptr_ref_info.offsetify_base != 0)
          {
            uint64 ptr_value = 0;
            MemoryCopy(&ptr_value, t_src, sizeof(ptr_value));
            uint64 ptr_write_value = ptr_value + (uint64)ptr_ref_info.offsetify_base;
            MemoryCopy(t.dst, &ptr_write_value, sizeof(ptr_write_value));
            read_off += sizeof(ptr_value);
          }
          ...
          else if(t.type.count_delimiter_name.size != 0 && t.is_post_header)
          {
            ...
            uint64 ptr_dest_buffer_size = (count+1)*t.type.direct.size;
            uint8* ptr_dest_buffer = push_array(arena, uint8, ptr_dest_buffer_size);
...
StringView
deep_copy_from_typed_data(Arena* arena, Type* type, StringView data, TypeSerializeParams* params)
{
  Temp scratch = scratch_begin(&arena, 1);
  StringView data_srlz = serialized_from_typed_data(scratch.arena, type, data, params);
  StringView data_copy = deserialized_from_typed_data(arena, type, data_srlz, params);
  scratch_end(scratch);
  return data_copy;
}
