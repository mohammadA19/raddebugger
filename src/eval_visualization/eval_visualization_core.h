...
static EV_Key ev_key_make(U64 parent_hash, U64 child_id);
static EV_Key ev_key_zero(void);
static EV_Key ev_key_root(void);
static B32 ev_key_match(EV_Key a, EV_Key b);
static U64 ev_hash_from_seed_string(U64 seed, String8 string);
static U64 ev_hash_from_key(EV_Key key);

////////////////////////////////
//~ rjf: Type Info Helpers

//- rjf: type info -> expandability/editablity
static E_TypeKey ev_expansion_type_from_key(E_TypeKey type_key);
static B32 ev_type_key_and_mode_is_expandable(E_TypeKey type_key, E_Mode mode);
static B32 ev_type_key_is_editable(E_TypeKey type_key);

////////////////////////////////
//~ rjf: View Functions

//- rjf: creation / deletion
static EV_View *ev_view_alloc(void);
static void ev_view_release(EV_View *view);

//- rjf: lookups / mutations
static EV_ExpandNode *ev_expand_node_from_key(EV_View *view, EV_Key key);
static B32 ev_expansion_from_key(EV_View *view, EV_Key key);
static String8 ev_view_rule_from_key(EV_View *view, EV_Key key);
static void ev_key_set_expansion(EV_View *view, EV_Key parent_key, EV_Key key, B32 expanded);
static void ev_key_set_view_rule(EV_View *view, EV_Key key, String8 view_rule_string);

////////////////////////////////
//~ rjf: View Rule Info Table Building / Selection / Lookups

static void ev_expand_rule_table_push(Arena *arena, EV_ExpandRuleTable *table, EV_ExpandRule *info);
#define ev_expand_rule_table_push_new(arena, table, ...) ev_expand_rule_table_push((arena), (table), &(EV_ExpandRule){__VA_ARGS__})
static void ev_select_expand_rule_table(EV_ExpandRuleTable *table);
static EV_ExpandRule *ev_expand_rule_from_string(String8 string);
static EV_ExpandRule *ev_expand_rule_from_type_key(E_TypeKey type_key);

////////////////////////////////
//~ rjf: Block Building

static EV_BlockTree ev_block_tree_from_eval(Arena *arena, EV_View *view, String8 filter, E_Eval root_eval);
static U64 ev_depth_from_block(EV_Block *block);

////////////////////////////////
//~ rjf: Block Coordinate Spaces

static U64 ev_block_id_from_num(EV_Block *block, U64 num);
static U64 ev_block_num_from_id(EV_Block *block, U64 id);
static EV_BlockRangeList ev_block_range_list_from_tree(Arena *arena, EV_BlockTree *block_tree);
static EV_BlockRange ev_block_range_from_num(EV_BlockRangeList *block_ranges, U64 num);
static EV_Key ev_key_from_num(EV_BlockRangeList *block_ranges, U64 num);
static U64    ev_num_from_key(EV_BlockRangeList *block_ranges, EV_Key key);
static U64    ev_vnum_from_num(EV_BlockRangeList *block_ranges, U64 num);
static U64    ev_num_from_vnum(EV_BlockRangeList *block_ranges, U64 vidx);

////////////////////////////////
//~ rjf: Row Building

static EV_WindowedRowList ev_windowed_row_list_from_block_range_list(Arena *arena, EV_View *view, EV_BlockRangeList *block_ranges, Rng1U64 vnum_range);
static EV_Row *ev_row_from_num(Arena *arena, EV_View *view, EV_BlockRangeList *block_ranges, U64 num);
static EV_WindowedRowList ev_rows_from_num_range(Arena *arena, EV_View *view, EV_BlockRangeList *block_ranges, Rng1U64 num_range);
static B32 ev_eval_is_expandable(E_Eval eval);
static B32 ev_row_is_expandable(EV_Row *row);
static B32 ev_row_is_editable(EV_Row *row);

////////////////////////////////
//~ rjf: Stringification

//- rjf: leaf stringification
static String8 ev_string_from_ascii_value(Arena *arena, U8 val);
static String8 ev_string_from_hresult_facility_code(U32 code);
static String8 ev_string_from_hresult_code(U32 code);
static String8 ev_string_from_simple_typed_eval(Arena *arena, EV_StringParams *params, E_Eval eval);
static String8 ev_escaped_from_raw_string(Arena *arena, String8 raw);

//- rjf: tree stringification iterator
static EV_StringIter *ev_string_iter_begin(Arena *arena, E_Eval eval, EV_StringParams *params);
static B32 ev_string_iter_next(Arena *arena, EV_StringIter *it, String8 *out_string);
