...
ev_view_rule_list_from_expr_fastpaths(Arena *arena, String8 string)
{
  ...
  String8List fastpath_view_rules = {0};
  {
    ...
    U64 comma_pos = str8_find_needle(string, parse_opl, (","), 0);
    U64 passthrough_pos = str8_find_needle(string, 0, ("--"), 0);
    if(comma_pos < string.size && comma_pos < passthrough_pos)
    {
      ...
      if(str8_match(comma_extension, ("x"), StringMatchFlags.CaseInsensitive))
      {
        str8_list_pushf(scratch.arena, &fastpath_view_rules, "hex");
      }
      else if(str8_match(comma_extension, ("b"), StringMatchFlags.CaseInsensitive))
      {
        str8_list_pushf(scratch.arena, &fastpath_view_rules, "bin");
      }
      else if(str8_match(comma_extension, ("o"), StringMatchFlags.CaseInsensitive))
      {
