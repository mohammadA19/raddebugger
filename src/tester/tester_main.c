...
entry_point(CmdLine *cmdline)
{
  ...
  //
  string artifacts_path = path_normalized_from_string(arena, ("./tester_artifacts"));
...
for (Test *test = test_##name_identifier; test != 0; test = 0)
  ...
  Test(pdb2rdi_determinism)
  {
    ...
    for EachElement(pdb_idx, pdb_paths)
    {
      ...
      String8List dump_paths = {0};
      {
        ...
        {
          string rdi_path = push_str8f(arena, "%S/repeat_%I64u.rdi", repeat_folder, repeat_idx);
      ...
      string *rdi_paths_array = push_array(arena, string, rdi_hashes_count);
  ...
  Test(eval_compiler_basics)
  {
    ...
    {
      string log = e_debug_log_from_expr_string(arena, exprs[idx]);
