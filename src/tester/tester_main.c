...
for (Test *test = test_##name_identifier; test != 0; test = 0)
    ...
    Test(pdb2rdi_determinism)
    {
        ...
        for EachElement(pdb_idx, pdb_paths)
        {
            ...
            StringView repeat_folder = push_str8f(arena, "%S/%S", artifacts_path, test.name);
    ...
    Test(eval_compiler_basics)
    {
        ...
        {
            StringView log = e_debug_log_from_expr_string(arena, exprs[idx]);
            str8_list_push(arena, &logs, log);
        }
        StringView log = str8_list_join(arena, &logs, 0);
        StringView test_artifacts_path = push_str8f(arena, "%S/%S", artifacts_path, test.name);
        os_make_directory(test_artifacts_path);
        StringView current_file_path = push_str8f(arena, "%S/current.txt", test_artifacts_path);
        StringView correct_file_path = push_str8f(arena, "%S/%S/correct.txt", test_data_folder_path, test.name);
        os_write_data_to_file_path(current_file_path, log);
        StringView current_file_data = log;
        StringView correct_file_data = os_data_from_file_path(arena, correct_file_path);
