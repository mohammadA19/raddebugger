...
internal uint32 os_codepoint_from_modifiers_and_key(OS_Modifiers flags, OS_Key key);
...
internal B32  os_text(OS_EventList *events, OS_Handle window, uint32 character);
...
internal OS_Handle      os_window_open(Rng2float rect, OS_WindowFlags flags, String8 title);
...
internal void           os_window_push_custom_title_bar_client_area(OS_Handle handle, Rng2float rect);
internal Rng2float        os_rect_from_window(OS_Handle window);
internal Rng2float        os_client_rect_from_window(OS_Handle window);
...
internal Vec2float        os_dim_from_monitor(OS_Handle monitor);
internal float            os_dpi_from_monitor(OS_Handle monitor);
...
internal Vec2float        os_mouse_from_window(OS_Handle window);
