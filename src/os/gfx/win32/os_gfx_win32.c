...
os_w32_window_alloc(void)
{
  ...
  {
    result = /* no zero */ new OS_W32_Window[1];
  }
...
os_gfx_init(void)
{
  ...
  os_w32_gfx_state = new OS_W32_GfxState[1];
...
os_window_push_custom_title_bar_client_area(OS_Handle handle, Rng2F32 rect)
{
  ...
  if(window->custom_border)
  {
    OS_W32_TitleBarClientArea *area = new OS_W32_TitleBarClientArea[1];
    if(area != 0)
    {
      area->rect = rect;
