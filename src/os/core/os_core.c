// Copyright (c) 2024 Epic Games Tools
// Licensed under the MIT license (https://opensource.org/license/mit/)

////////////////////////////////
//~ rjf: Handle Type Functions (Helpers, Implemented Once)

public static OS_Handle os_handle_zero()
{
  OS_Handle handle = default;
  return handle;
}

public static B32 os_handle_match(OS_Handle a, OS_Handle b)
{
  return a.u64[0] == b.u64[0];
}

public static void os_handle_list_push(Arena* arena, OS_HandleList* handles, OS_Handle handle)
{
  OS_HandleNode* n = push_array(arena, OS_HandleNode, 1);
  n.v = handle;
  SLLQueuePush(handles.first, handles.last, n);
  handles.count += 1;
}

public static OS_HandleArray os_handle_array_from_list(Arena* arena, OS_HandleList* list)
{
  OS_HandleArray result = default;
  result.count = list.count;
  result.v = arena.PushArrayNoZero<OS_Handle>(result.count);
  uint64 idx = 0;
  for(OS_HandleNode* n = list.first; n != 0; n = n.next, idx += 1)
  {
    result.v[idx] = n.v;
  }
  return result;
}

////////////////////////////////
//~ rjf: Command Line Argc/Argv Helper (Helper, Implemented Once)

public static String8List os_string_list_from_argcv(Arena* arena, int argc, char* *argv)
{
  String8List result = default;
  for(int i = 0; i < argc; i += 1)
  {
    String8 str = str8_cstring(argv[i]);
    str8_list_push(arena, &result, str);
  }
  return result;
}

////////////////////////////////
//~ rjf: Filesystem Helpers (Helpers, Implemented Once)

public static String8 os_data_from_file_path(Arena* arena, String8 path)
{
  OS_Handle file = os_file_open(OS_AccessFlags.Read|OS_AccessFlags.ShareRead, path);
  FileProperties props = os_properties_from_file(file);
  String8 data = os_string_from_file_range(arena, file, r1u64(0, props.size));
  os_file_close(file);
  return data;
}

public static B32 os_write_data_to_file_path(String8 path, String8 data)
{
  B32 good = 0;
  OS_Handle file = os_file_open(OS_AccessFlags.Write, path);
  if(!os_handle_match(file, os_handle_zero()))
  {
    good = 1;
    os_file_write(file, r1u64(0, data.size), data.str);
    os_file_close(file);
  }
  return good;
}

public static B32 os_write_data_list_to_file_path(String8 path, String8List list)
{
  B32 good = 0;
  OS_Handle file = os_file_open(OS_AccessFlags.Write, path);
  if(!os_handle_match(file, os_handle_zero()))
  {
    good = 1;
    uint64 off = 0;
    for(String8Node* n = list.first; n != 0; n = n.next)
    {
      os_file_write(file, r1u64(off, off+n.string.size), n.string.str);
      off += n.string.size;
    }
    os_file_close(file);
  }
  return good;
}

public static B32 os_append_data_to_file_path(String8 path, String8 data)
{
  B32 good = 0;
  if(data.size != 0)
  {
    OS_Handle file = os_file_open(OS_AccessFlags.Write|OS_AccessFlags.Append, path);
    if(!os_handle_match(file, os_handle_zero()))
    {
      good = 1;
      uint64 pos = os_properties_from_file(file).size;
      os_file_write(file, r1u64(pos, pos+data.size), data.str);
      os_file_close(file);
    }
  }
  return good;
}

public static OS_FileID os_id_from_file_path(String8 path)
{
  OS_Handle file = os_file_open(OS_AccessFlags.Read|OS_AccessFlags.ShareRead, path);
  OS_FileID id = os_id_from_file(file);
  os_file_close(file);
  return id;
}

public static S64 os_file_id_compare(OS_FileID a, OS_FileID b)
{
  S64 cmp = MemoryCompare((void*)&a.v[0], (void*)&b.v[0], sizeof(a.v));
  return cmp;
}

public static String8 os_string_from_file_range(Arena* arena, OS_Handle file, Rng1uint64 range)
{
  uint64 pre_pos = arena_pos(arena);
  String8 result;
  result.size = dim_1u64(range);
  result.str = arena.PushArrayNoZero<uint8>(result.size);
  uint64 actual_read_size = os_file_read(file, range, result.str);
  if(actual_read_size < result.size)
  {
    arena_pop_to(arena, pre_pos + actual_read_size);
    result.size = actual_read_size;
  }
  return result;
}

////////////////////////////////
//~ rjf: Process Launcher Helpers

public static OS_Handle os_cmd_line_launch(String8 string)
{
  Temp scratch = scratch_begin(0, 0);
  uint8 split_chars[] = {' '};
  String8List parts = str8_split(scratch.arena, string, split_chars, ArrayCount(split_chars), 0);
  OS_Handle handle = default;
  if(parts.node_count != 0)
  {
    // rjf: unpack exe part
    String8 exe = parts.first.string;
    String8 exe_folder = str8_chop_last_slash(exe);
    if(exe_folder.size == 0)
    {
      exe_folder = os_get_current_path(scratch.arena);
    }
    
    // rjf: find stdout delimiter
    String8Node* stdout_delimiter_n = 0;
    for(String8Node* n = parts.first; n != 0; n = n.next)
    {
      if(str8_match(n.string, (">"), 0))
      {
        stdout_delimiter_n = n;
        break;
      }
    }
    
    // rjf: read stdout path
    String8 stdout_path = default;
    if(stdout_delimiter_n && stdout_delimiter_n.next)
    {
      stdout_path = stdout_delimiter_n.next.string;
    }
    
    // rjf: open stdout handle
    OS_Handle stdout_handle = default;
    if(stdout_path.size != 0)
    {
      OS_Handle file = os_file_open(OS_AccessFlags.Write|OS_AccessFlags.Read, stdout_path);
      os_file_close(file);
      stdout_handle = os_file_open(OS_AccessFlags.Write|OS_AccessFlags.Append|OS_AccessFlags.ShareRead|OS_AccessFlags.ShareWrite|OS_AccessFlags.Inherited, stdout_path);
    }
    
    // rjf: form command line
    String8List cmdline = default;
    for(String8Node* n = parts.first; n != stdout_delimiter_n && n != 0; n = n.next)
    {
      str8_list_push(scratch.arena, &cmdline, n.string);
    }
    
    // rjf: launch
    OS_ProcessLaunchParams params = default;
    params.cmd_line = cmdline;
    params.path = exe_folder;
    params.inherit_env = 1;
    params.stdout_file = stdout_handle;
    handle = os_process_launch(&params);
    
    // rjf: close stdout handle
    {
      if(stdout_path.size != 0)
      {
        os_file_close(stdout_handle);
      }
    }
  }
  scratch_end(scratch);
  return handle;
}

public static OS_Handle os_cmd_line_launchf(char* fmt, ...)
{
  Temp scratch = scratch_begin(0, 0);
  va_list args;
  va_start(args, fmt);
  String8 string = push_str8fv(scratch.arena, fmt, args);
  OS_Handle result = os_cmd_line_launch(string);
  va_end(args);
  scratch_end(scratch);
  return result;
}

