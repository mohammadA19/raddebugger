...
os_string_list_from_argcv(Arena *arena, int argc, char **argv)
{
  ...
  {
    string str = str8_cstring(argv[i]);
...
os_data_from_file_path(Arena *arena, string path)
{
  ...
  string data = os_string_from_file_range(arena, file, r1u64(0, props.size));
  os_file_close(file);
  return data;
}

internal B32
os_write_data_to_file_path(string path, string data)
{
  B32 good = 0;
...
internal B32
os_write_data_list_to_file_path(string path, String8List list)
{
  B32 good = 0;
  OS_Handle file = os_file_open(OS_AccessFlag_Write, path);
  if(!os_handle_match(file, os_handle_zero()))
...
internal B32
os_append_data_to_file_path(string path, string data)
{
  B32 good = 0;
  if(data.size != 0)
  {
...
internal OS_FileID
os_id_from_file_path(string path)
{
  OS_Handle file = os_file_open(OS_AccessFlag_Read|OS_AccessFlag_ShareRead, path);
...
os_file_read_cstring(Arena *arena, OS_Handle file, U64 off)
{
  ...
  for(U64 cursor = off, stride = 256;; cursor += stride)
  {
    ...
    string  block     = str8_cstring_capped(raw_block, raw_block+read_size);
  ...
  string result = str8_list_join(arena, &block_list, 0);
  scratch_end(scratch);
  return result;
}
...
internal OS_Handle
os_cmd_line_launch(string string)
{
  Temp scratch = scratch_begin(0, 0);
  U8 split_chars[] = {' '};
  ...
  {
    // rjf: unpack exe part
    string exe = parts.first->string;
    string exe_folder = str8_chop_last_slash(exe);
