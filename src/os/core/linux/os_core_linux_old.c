...
void os_set_large_pages_enabled(B32 flag)
{
  NotImplemented;
}

B32 os_large_pages_enabled(void)
{
  NotImplemented;
  return 0;
}

U64 os_large_page_size(void)
{
  NotImplemented;
  return 0;
}
...
U64 os_allocation_granularity(void)
{
  // On linux there is no equivalent of "dwAllocationGranularity"
  os_page_size();
}

U64 os_logical_core_count(void)
{
  // TODO(rjf): check this
  return get_nprocs();
}
...
String8List os_get_command_line_arguments(void)
{
...
String8List os_environment(void)
{
  NotImplemented;
...
U64 os_string_list_from_system_path(Arena *arena, OS_SystemPath path, String8List *out){
  U64 result = 0;
...
B32 os_launch_process(OS_LaunchOptions *options){
...
void os_release_thread_handle(OS_Handle thread){
...
void os_mutex_take_(OS_Handle mutex){
  LNX_Entity *entity = (LNX_Entity*)PtrFromInt(mutex.id);
  pthread_mutex_lock(&entity->mutex);
}

void os_mutex_drop_(OS_Handle mutex){
  LNX_Entity *entity = (LNX_Entity*)PtrFromInt(mutex.id);
...
void os_rw_mutex_take_r_(OS_Handle mutex)
{
  NotImplemented;
}

void os_rw_mutex_drop_r_(OS_Handle mutex)
{
  NotImplemented;
}

void os_rw_mutex_take_w_(OS_Handle mutex)
{
  NotImplemented;
}

void os_rw_mutex_drop_w_(OS_Handle mutex)
{
  NotImplemented;
}
...
B32 os_condition_variable_wait_(OS_Handle cv, OS_Handle mutex, U64 endt_us){
  B32 result = false;
...
B32 os_condition_variable_wait_rw_r_(OS_Handle cv, OS_Handle mutex_rw, U64 endt_us)
{
  NotImplemented;
  return 0;
}

B32 os_condition_variable_wait_rw_w_(OS_Handle cv, OS_Handle mutex_rw, U64 endt_us)
{
  NotImplemented;
  return 0;
}

void os_condition_variable_signal_(OS_Handle cv){
  LNX_Entity *entity = (LNX_Entity*)PtrFromInt(cv.id);
  pthread_cond_signal(&entity->cond);
}

void os_condition_variable_broadcast_(OS_Handle cv){
  LNX_Entity *entity = (LNX_Entity*)PtrFromInt(cv.id);
  DontCompile;
}
