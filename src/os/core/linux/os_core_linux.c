...
os_get_process_start_time_unix(void)
{
  ...
  U64 start_time = 0;
  pid_t pid = getpid();
  string path = push_str8f(scratch.arena, "/proc/%u", pid);
  struct stat st;
...
os_full_path_from_path(Arena* arena, string path)
{
  ...
  string result = push_str8_copy(arena, str8_cstring(buffer));
  scratch_end(scratch);
  return result;
}
...
os_file_iter_next(Arena* arena, OS_FileIter* iter, OS_FileInfo* info_out)
{
  ...
  for (;lnx_iter.dir != 0;)
  {
    ...
    {
      Temp scratch = scratch_begin(&arena, 1);
      string full_path = push_str8f(scratch.arena, "%S/%s", lnx_iter.path, lnx_iter.dp.d_name);
    ...
    //- rjf: grab dynamically allocated process info
    {
      ...
      // rjf: grab binary path
      {
        ...
        if (got_final_result && size > 0)
        {
          ...
          string name_chopped = str8_chop_last_slash(full_name);
