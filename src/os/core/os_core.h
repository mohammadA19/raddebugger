...
static void os_handle_list_push(Arena *arena, OS_HandleList *handles, OS_Handle handle);
static OS_HandleArray os_handle_array_from_list(Arena *arena, OS_HandleList *list);

////////////////////////////////
//~ rjf: Command Line Argc/Argv Helper (Helper, Implemented Once)

static String8List os_string_list_from_argcv(Arena *arena, int argc, char **argv);
...
static String8        os_file_read_cstring(Arena *arena, OS_Handle file, U64 off);
...
static void os_set_thread_name(String8 string);
...
static void *    os_file_map_view_open(OS_Handle map, OS_AccessFlags flags, Rng1U64 range);
static void      os_file_map_view_close(OS_Handle map, void *ptr, Rng1U64 range);

//- rjf: directory iteration
static OS_FileIter *os_file_iter_begin(Arena *arena, String8 path, OS_FileIterFlags flags);
static B32          os_file_iter_next(Arena *arena, OS_FileIter *iter, OS_FileInfo *info_out);
...
static void      os_shared_memory_view_close(OS_Handle handle, void *ptr, Rng1U64 range);
...
static B32       os_process_join_exit_code(OS_Handle handle, U64 endt_us, int *exit_code_out);
static void      os_process_detach(OS_Handle handle);
static B32       os_process_kill(OS_Handle handle);

////////////////////////////////
//~ rjf: @os_hooks Threads (Implemented Per-OS)

static OS_Handle os_thread_launch(OS_ThreadFunctionType *func, void *ptr, void *params);
...
static void      os_rw_mutex_take_r(OS_Handle mutex);
static void      os_rw_mutex_drop_r(OS_Handle mutex);
static void      os_rw_mutex_take_w(OS_Handle mutex);
static void      os_rw_mutex_drop_w(OS_Handle mutex);
...
static B32       os_condition_variable_wait(OS_Handle cv, OS_Handle mutex, U64 endt_us);
static B32       os_condition_variable_wait_rw_r(OS_Handle cv, OS_Handle mutex_rw, U64 endt_us);
static B32       os_condition_variable_wait_rw_w(OS_Handle cv, OS_Handle mutex_rw, U64 endt_us);
static void      os_condition_variable_signal(OS_Handle cv);
static void      os_condition_variable_broadcast(OS_Handle cv);

//- rjf: cross-process semaphores
static OS_Handle os_semaphore_alloc(U32 initial_count, U32 max_count, String8 name);
...
static void os_safe_call(OS_ThreadFunctionType *func, OS_ThreadFunctionType *fail_handler, void *ptr);
