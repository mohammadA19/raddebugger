...
internal string        os_data_from_file_path(Arena *arena, string path);
internal B32            os_write_data_to_file_path(string path, string data);
internal B32            os_write_data_list_to_file_path(string path, String8List list);
internal B32            os_append_data_to_file_path(string path, string data);
internal OS_FileID      os_id_from_file_path(string path);
internal S64            os_file_id_compare(OS_FileID a, OS_FileID b);
internal string        os_string_from_file_range(Arena *arena, OS_Handle file, Rng1U64 range);
internal string        os_file_read_cstring(Arena *arena, OS_Handle file, U64 off);

////////////////////////////////
//~ rjf: Process Launcher Helpers

internal OS_Handle os_cmd_line_launch(string string);
...
internal string         os_get_current_path(Arena *arena);
...
internal void os_set_thread_name(string string);
...
//- rjf: files
internal OS_Handle      os_file_open(OS_AccessFlags flags, string path);
...
internal B32            os_delete_file_at_path(string path);
internal B32            os_copy_file_path(string dst, string src);
internal B32            os_move_file_path(string dst, string src);
internal string        os_full_path_from_path(Arena *arena, string path);
internal B32            os_file_path_exists(string path);
internal B32            os_folder_path_exists(string path);
internal FileProperties os_properties_from_file_path(string path);
...
internal OS_FileIter *os_file_iter_begin(Arena *arena, string path, OS_FileIterFlags flags);
...
internal OS_Handle os_semaphore_alloc(U32 initial_count, U32 max_count, string name);
...
internal OS_Handle os_library_open(string path);
internal void      os_library_close(OS_Handle lib);
internal VoidProc *os_library_load_proc(OS_Handle lib, string name);
