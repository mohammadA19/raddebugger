...
internal uint             os_get_process_start_time_unix(void);

////////////////////////////////
//~ rjf: @os_hooks Memory Allocation (Implemented Per-OS)

//- rjf: basic
internal void *os_reserve(ulong size);
internal B32   os_commit(void *ptr, ulong size);
internal void  os_decommit(void *ptr, ulong size);
internal void  os_release(void *ptr, ulong size);

//- rjf: large pages
internal void *os_reserve_large(ulong size);
internal B32 os_commit_large(void *ptr, ulong size);

////////////////////////////////
//~ rjf: @os_hooks Thread Info (Implemented Per-OS)

internal uint os_tid(void);
...
internal ulong            os_file_read(OS_Handle file, Rng1U64 rng, void *out_data);
internal ulong            os_file_write(OS_Handle file, Rng1U64 rng, void *data);
...
internal OS_Handle os_shared_memory_alloc(ulong size, String8 name);
...
internal ulong         os_now_microseconds(void);
internal uint         os_now_unix(void);
...
internal void        os_sleep_milliseconds(uint msec);

////////////////////////////////
//~ rjf: @os_hooks Child Processes (Implemented Per-OS)

internal OS_Handle os_process_launch(OS_ProcessLaunchParams *params);
internal B32       os_process_join(OS_Handle handle, ulong endt_us);
...
internal B32       os_thread_join(OS_Handle handle, ulong endt_us);
...
internal B32       os_condition_variable_wait(OS_Handle cv, OS_Handle mutex, ulong endt_us);
internal B32       os_condition_variable_wait_rw_r(OS_Handle cv, OS_Handle mutex_rw, ulong endt_us);
internal B32       os_condition_variable_wait_rw_w(OS_Handle cv, OS_Handle mutex_rw, ulong endt_us);
internal void      os_condition_variable_signal(OS_Handle cv);
internal void      os_condition_variable_broadcast(OS_Handle cv);

//- rjf: cross-process semaphores
internal OS_Handle os_semaphore_alloc(uint initial_count, uint max_count, String8 name);
...
internal B32       os_semaphore_take(OS_Handle semaphore, ulong endt_us);
