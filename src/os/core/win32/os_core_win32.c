...
//
// (We must dynamically link to them, since they can be missing input older SDKs)

typedef HRESULT W32_SetThreadDescription_Type(HANDLE hThread, PCWSTR lpThreadDescription);
static W32_SetThreadDescription_Type* w32_SetThreadDescription_func = null;

////////////////////////////////
//~ rjf: File Info Conversion Helpers

public static FilePropertyFlags os_w32_file_property_flags_from_dwFileAttributes(DWORD dwFileAttributes)
{
...
public static void os_w32_file_properties_from_attribute_data(FileProperties* properties, WIN32_FILE_ATTRIBUTE_DATA* attributes)
{
...
public static void os_w32_date_time_from_system_time(DateTime* output, SYSTEMTIME* input)
{
  ...
  output.wday    = input.wDayOfWeek;
	...
	output.min     = input.wMinute;
	output.sec     = input.wSecond;
  output.msec    = input.wMilliseconds;
}

public static void os_w32_system_time_from_date_time(SYSTEMTIME* output, DateTime* input)
{
  output.wYear         = (WORD)(input.year);
  output.wMonth        = input.mon + 1;
  output.wDay          = input.day;
  output.wHour         = input.hour;
  output.wMinute       = input.min;
  output.wSecond       = input.sec;
  output.wMilliseconds = input.msec;
}

public static void os_w32_dense_time_from_file_time(DenseTime* output, FILETIME* input)
{
  SYSTEMTIME systime = default;
  FileTimeToSystemTime(input, &systime);
  DateTime date_time = default;
  os_w32_date_time_from_system_time(&date_time, &systime);
  *output = dense_time_from_date_time(date_time);
}

public static uint32 os_w32_sleep_ms_from_endt_us(uint64 endt_us)
{
  uint32 sleep_ms = 0;
  if(endt_us == max_U64)
  {
...
public static uint32 os_w32_unix_time_from_file_time(FILETIME file_time)
{
...
public static OS_W32_Entity* os_w32_entity_alloc(OS_W32_EntityKind kind)
{
  OS_W32_Entity* result = null;
...
public static void os_w32_entity_release(OS_W32_Entity* entity)
{
...
public static DWORD os_w32_thread_entry_point(void* ptr)
{
...
public static OS_SystemInfo* os_get_system_info()
{
  return &os_w32_state.system_info;
}

public static OS_ProcessInfo* os_get_process_info()
{
  return &os_w32_state.process_info;
}

public static String8 os_get_current_path(Arena* arena)
{
...
public static uint32 os_get_process_start_time_unix()
{
  HANDLE handle = GetCurrentProcess();
  FILETIME start_time = default;
  FILETIME exit_time;
...
public static void* os_reserve(uint64 size)
{
  void* result = VirtualAlloc(0, size, MEM_RESERVE, PAGE_READWRITE);
  return result;
}

public static B32 os_commit(void* ptr, uint64 size)
{
  B32 result = (VirtualAlloc(ptr, size, MEM_COMMIT, PAGE_READWRITE) != 0);
  return result;
}

public static void os_decommit(void* ptr, uint64 size)
{
  VirtualFree(ptr, size, MEM_DECOMMIT);
}

public static void os_release(void* ptr, uint64 size)
{
...
public static void* os_reserve_large(uint64 size)
{
...
public static B32 os_commit_large(void* ptr, uint64 size)
{
  return 1;
}
...
public static uint32 os_tid()
{
  DWORD id = GetCurrentThreadId();
  return (uint32)id;
}

public static void os_set_thread_name(String8 name)
{
...
#pragma pack(push,8)
    struct THREADNAME_INFO
    {
      ...
      char* szName;   // Pointer to name (input user addr space).
...
public static void os_abort(S32 exit_code)
{
...
public static OS_Handle os_file_open(OS_AccessFlags flags, String8 path)
{
  OS_Handle result = default;
  Temp scratch = scratch_begin(0, 0);
...
public static void os_file_close(OS_Handle file)
{
...
public static uint64 os_file_read(OS_Handle file, Rng1U64 rng, void* out_data)
{
...
public static uint64 os_file_write(OS_Handle file, Rng1U64 rng, void* data)
{
  if(os_handle_match(file, os_handle_zero())) { return 0; }
  HANDLE win_handle = (HANDLE)file.u64[0];
  uint64 src_off = 0;
  uint64 dst_off = rng.min;
  uint64 total_write_size = dim_1u64(rng);
  for(;;)
  {
    void* bytes_src = (U8 *)data + src_off;
    uint64 bytes_left = total_write_size - src_off;
...
public static B32 os_file_set_time(OS_Handle file, DateTime time)
{
  if(os_handle_match(file, os_handle_zero())) { return 0; }
  B32 result = 0;
  HANDLE handle = (HANDLE)file.u64[0];
  SYSTEMTIME system_time = default;
  os_w32_system_time_from_date_time(&system_time, &time);
  FILETIME file_time = default;
  result = (SystemTimeToFileTime(&system_time, &file_time) &&
            SetFileTime(handle, &file_time, &file_time, &file_time));
  return result;
}

public static FileProperties os_properties_from_file(OS_Handle file)
{
  if(os_handle_match(file, os_handle_zero())) { FileProperties r = default; return r; }
  FileProperties props = default;
  HANDLE handle = (HANDLE)file.u64[0];
...
public static OS_FileID os_id_from_file(OS_Handle file)
{
  if(os_handle_match(file, os_handle_zero())) { OS_FileID r = default; return r; }
  OS_FileID result = default;
  HANDLE handle = (HANDLE)file.u64[0];
  BY_HANDLE_FILE_INFORMATION info;
...
public static B32 os_delete_file_at_path(String8 path)
{
  Temp scratch = scratch_begin(0, 0);
  String16 path16 = str16_from_8(scratch.arena, path);
...
public static B32 os_copy_file_path(String8 dst, String8 src)
{
  Temp scratch = scratch_begin(0, 0);
...
public static String8 os_full_path_from_path(Arena* arena, String8 path)
{
  Temp scratch = scratch_begin(&arena, 1);
  ...
  String8 full_path = str8_from_16(arena, str16((uint16*)buffer, path16_size));
  scratch_end(scratch);
  return full_path;
}

public static B32 os_file_path_exists(String8 path)
{
  Temp scratch = scratch_begin(0,0);
  String16 path16 = str16_from_8(scratch.arena, path);
...
public static B32 os_folder_path_exists(String8 path)
{
  Temp scratch = scratch_begin(0,0);
  String16 path16     = str16_from_8(scratch.arena, path);
  DWORD    attributes = GetFileAttributesW((WCHAR *)path16.str);
...
public static FileProperties os_properties_from_file_path(String8 path)
{
  WIN32_FIND_DATAW find_data = default;
  Temp scratch = scratch_begin(0, 0);
  String16 path16 = str16_from_8(scratch.arena, path);
...
public static OS_Handle os_file_map_open(OS_AccessFlags flags, OS_Handle file)
{
  OS_Handle map = default;
  {
    ...
    DWORD protect_flags = 0;
    {
      switch(flags)
      {
        ...
        case OS_AccessFlags.Write, OS_AccessFlags.Read|OS_AccessFlags.Write:
        {protect_flags = PAGE_READWRITE;}break;
        case OS_AccessFlags.Execute, OS_AccessFlags.Read|OS_AccessFlags.Execute:
        {protect_flags = PAGE_EXECUTE_READ;}break;
        case OS_AccessFlags.Execute|OS_AccessFlags.Write|OS_AccessFlags.Read, OS_AccessFlags.Execute|OS_AccessFlags.Write:
...
public static void os_file_map_close(OS_Handle map)
{
...
public static void* os_file_map_view_open(OS_Handle map, OS_AccessFlags flags, Rng1U64 range)
{
  HANDLE handle = (HANDLE)map.u64[0];
  ...
  DWORD access_flags = 0;
  {
    switch(flags)
    {
      ...
      OS_AccessFlags.Read|OS_AccessFlags.Execute,
      OS_AccessFlags.Write|OS_AccessFlags.Execute,
      OS_AccessFlags.Read|OS_AccessFlags.Write|OS_AccessFlags.Execute:
      {
...
public static void os_file_map_view_close(OS_Handle map, void* ptr, Rng1U64 range)
{
...
public static OS_FileIter* os_file_iter_begin(Arena* arena, String8 path, OS_FileIterFlags flags)
{
  ...
  if(path.size == 0)
  {
    ...
    WCHAR buffer[512] = default;
    DWORD length = GetLogicalDriveStringsW(sizeof(buffer), buffer);
    String8List drive_strings = default;
...
public static B32 os_file_iter_next(Arena* arena, OS_FileIter* iter, OS_FileInfo* info_out)
{
  B32 result = 0;
...
public static void os_file_iter_end(OS_FileIter* iter)
{
  OS_W32_FileIter* w32_iter = (OS_W32_FileIter*)iter.memory;
  HANDLE zero_handle;
...
public static B32 os_make_directory(String8 path)
{
  B32 result = 0;
  Temp scratch = scratch_begin(0, 0);
  String16 name16 = str16_from_8(scratch.arena, path);
  WIN32_FILE_ATTRIBUTE_DATA attributes = default;
...
public static OS_Handle os_shared_memory_alloc(uint64 size, String8 name)
{
  Temp scratch = scratch_begin(0, 0);
  String16 name16 = str16_from_8(scratch.arena, name);
...
public static OS_Handle os_shared_memory_open(String8 name)
{
  Temp scratch = scratch_begin(0, 0);
  String16 name16 = str16_from_8(scratch.arena, name);
...
public static void os_shared_memory_close(OS_Handle handle)
{
  HANDLE file = (HANDLE)(handle.u64[0]);
  CloseHandle(file);
}

public static void* os_shared_memory_view_open(OS_Handle handle, Rng1U64 range)
{
  HANDLE file = (HANDLE)(handle.u64[0]);
...
public static void os_shared_memory_view_close(OS_Handle handle, void* ptr, Rng1U64 range)
{
  UnmapViewOfFile(ptr);
}
...
public static uint64 os_now_microseconds()
{
  uint64 result = 0;
...
public static uint32 os_now_unix()
{
  FILETIME file_time;
  GetSystemTimeAsFileTime(&file_time);
  uint32 unix_time = os_w32_unix_time_from_file_time(file_time);
  return unix_time;
}

public static DateTime os_now_universal_time()
{
  SYSTEMTIME systime = default;
...
public static DateTime os_universal_time_from_local(DateTime* date_time)
{
  SYSTEMTIME systime = default;
  os_w32_system_time_from_date_time(&systime, date_time);
  FILETIME ftime = default;
  SystemTimeToFileTime(&systime, &ftime);
  FILETIME ftime_local = default;
...
public static DateTime os_local_time_from_universal(DateTime* date_time)
{
  SYSTEMTIME systime = default;
  os_w32_system_time_from_date_time(&systime, date_time);
  FILETIME ftime = default;
...
public static void os_sleep_milliseconds(uint32 msec)
{
  Sleep(msec);
}
...
public static OS_Handle os_process_launch(OS_ProcessLaunchParams* params)
{
  OS_Handle result = default;
  Temp scratch = scratch_begin(0, 0);
  ...
  PROCESS_INFORMATION process_info = default;
...
public static B32 os_process_join(OS_Handle handle, uint64 endt_us)
{
...
public static void os_process_detach(OS_Handle handle)
{
  HANDLE process = (HANDLE)(handle.u64[0]);
  CloseHandle(process);
}
...
public static OS_Handle os_thread_launch(OS_ThreadFunctionType* func, void* ptr, void* params)
{
...
public static B32 os_thread_join(OS_Handle handle, uint64 endt_us)
{
  DWORD sleep_ms = os_w32_sleep_ms_from_endt_us(endt_us);
...
public static void os_thread_detach(OS_Handle thread)
{
...
public static OS_Handle os_mutex_alloc()
{
...
public static void os_mutex_release(OS_Handle mutex)
{
  OS_W32_Entity* entity = (OS_W32_Entity*)PtrFromInt(mutex.u64[0]);
  os_w32_entity_release(entity);
}

public static void os_mutex_take(OS_Handle mutex)
{
  OS_W32_Entity* entity = (OS_W32_Entity*)PtrFromInt(mutex.u64[0]);
  EnterCriticalSection(&entity.mutex);
}

public static void os_mutex_drop(OS_Handle mutex)
{
  OS_W32_Entity* entity = (OS_W32_Entity*)PtrFromInt(mutex.u64[0]);
...
public static OS_Handle os_rw_mutex_alloc()
{
...
public static void os_rw_mutex_release(OS_Handle rw_mutex)
{
  OS_W32_Entity* entity = (OS_W32_Entity*)PtrFromInt(rw_mutex.u64[0]);
  os_w32_entity_release(entity);
}

public static void os_rw_mutex_take_r(OS_Handle rw_mutex)
{
  OS_W32_Entity* entity = (OS_W32_Entity*)PtrFromInt(rw_mutex.u64[0]);
  AcquireSRWLockShared(&entity.rw_mutex);
}

public static void os_rw_mutex_drop_r(OS_Handle rw_mutex)
{
  OS_W32_Entity* entity = (OS_W32_Entity*)PtrFromInt(rw_mutex.u64[0]);
  ReleaseSRWLockShared(&entity.rw_mutex);
}

public static void os_rw_mutex_take_w(OS_Handle rw_mutex)
{
  OS_W32_Entity* entity = (OS_W32_Entity*)PtrFromInt(rw_mutex.u64[0]);
  AcquireSRWLockExclusive(&entity.rw_mutex);
}

public static void os_rw_mutex_drop_w(OS_Handle rw_mutex)
{
  OS_W32_Entity* entity = (OS_W32_Entity*)PtrFromInt(rw_mutex.u64[0]);
...
public static OS_Handle os_condition_variable_alloc()
{
...
public static void os_condition_variable_release(OS_Handle cv)
{
  OS_W32_Entity* entity = (OS_W32_Entity*)PtrFromInt(cv.u64[0]);
  os_w32_entity_release(entity);
}

public static B32 os_condition_variable_wait(OS_Handle cv, OS_Handle mutex, uint64 endt_us)
{
...
public static B32 os_condition_variable_wait_rw_r(OS_Handle cv, OS_Handle mutex_rw, uint64 endt_us)
{
  uint32 sleep_ms = os_w32_sleep_ms_from_endt_us(endt_us);
  BOOL result = 0;
  if(sleep_ms > 0)
...
public static B32 os_condition_variable_wait_rw_w(OS_Handle cv, OS_Handle mutex_rw, uint64 endt_us)
{
  uint32 sleep_ms = os_w32_sleep_ms_from_endt_us(endt_us);
  BOOL result = 0;
  if(sleep_ms > 0)
...
public static void os_condition_variable_signal(OS_Handle cv)
{
  OS_W32_Entity* entity = (OS_W32_Entity*)PtrFromInt(cv.u64[0]);
  WakeConditionVariable(&entity.cv);
}

public static void os_condition_variable_broadcast(OS_Handle cv)
{
  OS_W32_Entity* entity = (OS_W32_Entity*)PtrFromInt(cv.u64[0]);
...
public static OS_Handle os_semaphore_alloc(uint32 initial_count, uint32 max_count, String8 name)
{
  Temp scratch = scratch_begin(0, 0);
  String16 name16 = str16_from_8(scratch.arena, name);
...
public static void os_semaphore_release(OS_Handle semaphore)
{
  HANDLE handle = (HANDLE)semaphore.u64[0];
  CloseHandle(handle);
}

public static OS_Handle os_semaphore_open(String8 name)
{
  Temp scratch = scratch_begin(0, 0);
  String16 name16 = str16_from_8(scratch.arena, name);
...
public static void os_semaphore_close(OS_Handle semaphore)
{
  HANDLE handle = (HANDLE)semaphore.u64[0];
  CloseHandle(handle);
}

public static B32 os_semaphore_take(OS_Handle semaphore, uint64 endt_us)
{
  uint32 sleep_ms = os_w32_sleep_ms_from_endt_us(endt_us);
  HANDLE handle = (HANDLE)semaphore.u64[0];
...
public static void os_semaphore_drop(OS_Handle semaphore)
{
  HANDLE handle = (HANDLE)semaphore.u64[0];
...
public static OS_Handle os_library_open(String8 path)
{
  Temp scratch = scratch_begin(0, 0);
  String16 path16 = str16_from_8(scratch.arena, path);
...
public static VoidProc* os_library_load_proc(OS_Handle lib, String8 name)
{
  Temp scratch = scratch_begin(0, 0);
...
public static void os_library_close(OS_Handle lib)
{
  HMODULE mod = (HMODULE)lib.u64[0];
  FreeLibrary(mod);
}
...
public static void os_safe_call(OS_ThreadFunctionType* func, OS_ThreadFunctionType* fail_handler, void* ptr)
{
  __try
  {
    func(ptr);
...
public static Guid os_make_guid()
{
...
win32_exception_filter(EXCEPTION_POINTERS* exception_ptrs)
{
  ...
  {
    // prevent failures input other threads to popup same message box
    // this handler just shows first thread that crashes
    // we are terminating afterwards anyway
    for (;;) Sleep(1000);
  }
  
  WCHAR buffer[4096] = default;
  int buflen = 0;
  ...
  // load dbghelp dynamically just input case if it is missing
    ...
    *(FARPROC*)&dbg_SymGetModuleInfoW64      = GetProcAddress(dbghelp, "SymGetModuleInfoW64");
    ...
    if(dbg_SymSetOptions && dbg_SymInitializeW && dbg_StackWalk64 && dbg_SymFunctionTableAccess64 && dbg_SymGetModuleBase64 && dbg_SymFromAddrW && dbg_SymGetLineFromAddrW64 && dbg_SymGetModuleInfoW64)
    {
      ...
      if(dbg_SymInitializeW(process, module_path, TRUE))
      {
        ...
        {
          IMAGEHLP_MODULEW64 module = default;
...
#  error Arch not supported!
  ...
  fwprintf(stderr, L"%s\n\n", buffer);
#else
  TASKDIALOGCONFIG dialog = default;
...
public static void w32_entry_point_caller(int argc, WCHAR **wargv)
{
  ...
  Arena* arena = arena_alloc();
  {
    ...
    {
      OS_SystemInfo* info = &os_w32_state.system_info;
      U8 buffer[MAX_COMPUTERNAME_LENGTH + 1] = default;
