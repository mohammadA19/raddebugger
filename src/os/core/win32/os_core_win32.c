...
os_file_read(OS_Handle file, Rng1uint64 rng, void *out_data)
{
  ...
  Rng1uint64 rng_clamped  = r1u64(ClampTop(rng.min, size), ClampTop(rng.max, size));
...
internal B32
os_file_reserve_size(OS_Handle file, uint64 size)
{
  HANDLE handle = (HANDLE)file.u64[0];
  
  FILE_ALLOCATION_INFO alloc_info    = {0};
  alloc_info.AllocationSize.LowPart  = size & max_uint32;
  alloc_info.AllocationSize.HighPart = (size >> 32) & max_uint32;
