...
internal B32
os_move_file_path(StringView dst, StringView src)
{
    Temp scratch = scratch_begin(0, 0);
    String16 dst16 = str16_from_8(scratch.arena, dst);
    String16 src16 = str16_from_8(scratch.arena, src);
...
os_full_path_from_path(Arena *arena, StringView path)
{
    ...
    StringView full_path = str8_from_16(arena, str16((U16*)buffer, path16_size));
    scratch_end(scratch);
    return full_path;
}
...
os_properties_from_file_path(StringView path)
{
    ...
    else
    {
        ...
        StringView path_trimmed = str8_prefix(path, last_slash_pos);
        for (U64 off = 0; off < (U64)length;)
        {
            String16 next_drive_string_16 = str16_cstring((U16 *)buffer+off);
            off += next_drive_string_16.size+1;
            StringView next_drive_string = str8_from_16(scratch.arena, next_drive_string_16);
