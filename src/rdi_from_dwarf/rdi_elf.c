...
elf_parsed_from_data(Arena *arena, String8 elf_data){
  ...
  ulong section_count_raw = e_shnum;
  uint section_header_string_table_index = e_shstrndx;
  if (header_is_good){
  ...
  ulong section_foff = 0;
  ulong section_size = 0;
  ulong section_count = 0;
  
  ulong segment_foff = 0;
  ulong segment_size = 0;
  ulong segment_count = 0;
  
  if (header_is_good){
    if (e_shentsize > 0){
      ulong section_opl_raw = e_shoff + e_shentsize*section_count_raw;
      ulong section_opl = ClampTop(section_opl_raw, elf_data.size);
    ...
    if (e_phentsize > 0){
      ulong segment_opl_raw = e_phoff + e_phentsize*e_phnum;
      ulong segment_opl = ClampTop(segment_opl_raw, elf_data.size);
  ...
  if (header_is_good){
    ...
    ulong load_segment_off = 0;
    {
      ulong segment_cursor = segment_foff;
      ulong segment_opl = segment_foff + segment_size*segment_count;
      for (;segment_cursor < segment_opl; segment_cursor += segment_size){
        uint p_type = *(uint*)(elf_data.str + segment_cursor);
  ...
  ulong section_name_table_foff = 0;
  ulong section_name_table_opl = 0;
  if (header_is_good){
    if (section_header_string_table_index < section_count){
      ulong sec_foff = section_foff + section_header_string_table_index*section_size;
      switch (elf_class){
        case ELF_Class_32:
        {
    ...
    switch (elf_class){
      case ELF_Class_32:
      {
        sections = push_array(arena, ELF_Shdr64, section_count);
        {
          ...
          for (ulong i = 0; i < section_count; i += 1, shdr32 += 1, shdr64 += 1){
  ...
  if (sections != 0 && section_count > 0){
    ...
    for (ulong i = 0;
      ...
      ulong name_foff = section_name_table_foff + sec->sh_name;
    ...
    switch (elf_class){
      case ELF_Class_32:
      {
        segments = push_array(arena, ELF_Phdr64, segment_count);
        {
          ...
          for (ulong i = 0; i < segment_count; i += 1, phdr32 += 1, phdr64 += 1){
  ...
  ulong dynsym_idx = 0;
  if (section_names != 0){
    for (ulong i = 0; i < section_count; i += 1){
...
static String8
elf_section_name_from_name_offset(ELF_Parsed *elf, ulong offset){
  String8 result = {0};
  if (elf != 0){
    if (offset > 0){
      ulong foff = elf->section_name_table_foff + offset;
...
static String8
elf_section_name_from_idx(ELF_Parsed *elf, uint idx){
  String8 result = {0};
  if (elf != 0){
...
elf_section_idx_from_name(ELF_Parsed *elf, String8 name){
  ...
  if (elf != 0){
    ...
    ulong count = elf->section_count;
    for (ulong i = 0; i < count; i += 1, sec_name += 1){
...
static String8
elf_section_data_from_idx(ELF_Parsed *elf, uint idx){
  String8 result = {0};
  if (elf != 0){
    if (idx < elf->section_count){
      ELF_Shdr64 *shdr = elf->sections + idx;
      ulong off_raw = shdr->sh_offset;
      ulong size = shdr->sh_size;
      if (shdr->sh_flags & ELF_SectionType_NOBITS){
        size = 0;
      }
      ulong opl_raw = off_raw + size;
      ulong opl = ClampTop(opl_raw, elf->data.size);
      ulong off = ClampTop(off_raw, opl);
  ...
  switch (elf_class){
    ...
    case ELF_Class_32:
    {
      ...
      symbols = push_array(arena, ELF_Sym64, count);
      {
        ...
        for (ulong i = 0; i < count; i += 1, sym32 += 1, sym64 += 1){
