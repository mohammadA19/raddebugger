...
static uint64
dwarf_leb128_decode_U64(uint8* ptr, uint8* opl){
  uint64 r = 0;
  switch (opl - ptr){
    case 10: r |= ((uint64)(ptr[9]&0x7F) << 63);
    case  9: r |= ((uint64)(ptr[8]&0x7F) << 56);
    case  8: r |= ((uint64)(ptr[7]&0x7F) << 49);
    case  7: r |= ((uint64)(ptr[6]&0x7F) << 42);
    case  6: r |= ((uint64)(ptr[5]&0x7F) << 35);
    case  5: r |= ((uint64)(ptr[4]&0x7F) << 28);
    case  4: r |= ((uint64)(ptr[3]&0x7F) << 21);
    case  3: r |= ((uint64)(ptr[2]&0x7F) << 14);
    case  2: r |= ((uint64)(ptr[1]&0x7F) <<  7);
    case  1: r |= ((uint64)(ptr[0]&0x7F)      );
...
static int64
dwarf_leb128_decode_S64(uint8* ptr, uint8* opl){
  uint64 u = dwarf_leb128_decode_U32(ptr, opl);
  uint64 s = (uint64)(opl - ptr)*7;
  B32 neg = ((u & (1llu << s)) != 0);
  if (neg){
    switch (opl - ptr){
...
static uint32
dwarf_leb128_decode_U32(uint8* ptr, uint8* opl){
  uint32 r = 0;
  switch (opl - ptr){
    case 5: r |= ((uint32)(ptr[4]&0x7F) << 28);
    case 4: r |= ((uint32)(ptr[3]&0x7F) << 21);
    case 3: r |= ((uint32)(ptr[2]&0x7F) << 14);
    case 2: r |= ((uint32)(ptr[1]&0x7F) <<  7);
    case 1: r |= ((uint32)(ptr[0]&0x7F)      );
    case 0: default: break;
  }
  return(r);
}
...
dwarf_parsed_from_elf(Arena* arena, ELF_Parsed* elf){
  ...
  if (elf != 0){
    ...
    for (uint64 i = 1; i < DWARF_SectionCode_COUNT; i += 1){
      ...
      uint32 idx = 0;
      for (uint32 j = 0; idx == 0 && j < DWARF_SECTION_NAME_VARIANT_COUNT; j += 1){
...
dwarf_info_from_data(Arena* arena, StringView data){
  ...
  for (;ptr < opl; unit_idx += 1){
    ...
    // version
    uint8 version = MemoryConsume(uint16, ptr, unit_opl);
    
    // rest of header depends on version
    uint64 abbrev_off = 0;
    uint8  address_size = 0;
    uint8  unit_type = 0;
    uint64 unit_dwo_id = 0;
    uint64 unit_type_signature = 0;
    uint64 unit_type_offset = 0;
    switch (version){
      case 4:
      {
        // abbrev_off
        if (is_64bit){
          abbrev_off = MemoryConsume(uint64, ptr, unit_opl);
        }
        else{
          abbrev_off = MemoryConsume(uint32, ptr, unit_opl);
        }
        
        // address_size
        address_size = MemoryConsume(uint8, ptr, unit_opl);
      }break;
      
      case 5:
      {
        // unit_type
        unit_type = (DWARF_UnitType)MemoryConsume(uint8, ptr, unit_opl);
        ...
        switch (unit_type){
          ...
          {
            unit_dwo_id = MemoryConsume(uint64, ptr, unit_opl);
          }break;
          case DWARF_UnitType_type: case DWARF_UnitType_split_type:
          {
            unit_type_signature = MemoryConsume(uint64, ptr, unit_opl);
            if (is_64bit){
              unit_type_offset = MemoryConsume(uint64, ptr, unit_opl);
            }
            else{
              unit_type_offset = MemoryConsume(uint32, ptr, unit_opl);
            }
          }break;
        }
      }break;
...
dwarf_pubnames_from_data(Arena* arena, StringView data){
  ...
  for (;ptr < opl; unit_idx += 1){
    ...
    uint64 info_off = 0;
    if (is_64bit){
      info_off = MemoryConsume(uint64, ptr, unit_opl);
    }
    else{
      info_off = MemoryConsume(uint32, ptr, unit_opl);
    }
    
    // info_length
    uint64 info_length = 0;
    if (is_64bit){
      info_length = MemoryConsume(uint64, ptr, unit_opl);
    }
    else{
      info_length = MemoryConsume(uint32, ptr, unit_opl);
    }
...
dwarf_names_from_data(Arena* arena, StringView data){
  ...
  for (;ptr < opl; unit_idx += 1){
    ...
    // *padding*
    MemoryConsume(uint16, ptr, unit_opl);
    
    // comp_unit_count
    uint32 comp_unit_count = MemoryConsume(uint32, ptr, unit_opl);
    
    // local_type_unit_count
    uint32 local_type_unit_count = MemoryConsume(uint32, ptr, unit_opl);
    
    // foreign_type_unit_count
    uint32 foreign_type_unit_count = MemoryConsume(uint32, ptr, unit_opl);
    
    // bucket_count
    uint32 bucket_count = MemoryConsume(uint32, ptr, unit_opl);
    
    // name_count
    uint32 name_count = MemoryConsume(uint32, ptr, unit_opl);
    
    // abbrev_table_size
    uint32 abbrev_table_size = MemoryConsume(uint32, ptr, unit_opl);
    
    // augmentation_string_size
    uint32 augmentation_string_size = MemoryConsume(uint32, ptr, unit_opl);
    
    // augmentation_string
    uint8* augmentation_string = ptr;
    {
      uint8* ptr_raw = ptr + augmentation_string_size;
...
dwarf_aranges_from_data(Arena* arena, StringView data){
  ...
  for (;ptr < opl; unit_idx += 1){
    ...
    // address_size
    uint8 address_size = MemoryConsume(uint8, ptr, unit_opl);
    
    // segment_selector_size
    uint8 segment_selector_size = MemoryConsume(uint8, ptr, unit_opl);
...
dwarf_line_from_data(Arena* arena, StringView data){
  ...
  for (;ptr < opl; unit_idx += 1){
    ...
    // rest of header depends on version
    uint8  minimum_instruction_length = 0;
    uint8  maximum_operations_per_instruction = 0;
    uint8  default_is_stmt = 0;
    int8  line_base = 0;
    uint8  line_range = 0;
    uint8  opcode_base = 0;
    uint8* standard_opcode_lengths = 0;
    // v4
    ...
    switch (version){
      case 4:
      {
        ...
        if (is_64bit){
          header_length = MemoryConsume(uint64, ptr, unit_opl);
        }
        else{
          header_length = MemoryConsume(uint32, ptr, unit_opl);
        }
        
        // header opl
        uint8* header_opl_raw = ptr + header_length;
        uint8* header_opl = ClampTop(header_opl_raw, unit_opl);
        
        // minimum_instruction_length
        minimum_instruction_length = MemoryConsume(uint8, ptr, header_opl);
        
        // maximum_operations_per_instruction
        maximum_operations_per_instruction = MemoryConsume(uint8, ptr, header_opl);
        
        // default_is_stmt
        default_is_stmt = MemoryConsume(uint8, ptr, header_opl);
        
        // line_base
        line_base = MemoryConsume(int8, ptr, header_opl);
        
        // line_range
        line_range = MemoryConsume(uint8, ptr, header_opl);
        
        // opcode_base
        opcode_base = MemoryConsume(uint8, ptr, header_opl);
        ...
        for (;ptr < header_opl;){
          ...
          uint8* file_name_first = ptr;
          for (;ptr < header_opl && *ptr != 0;) ptr += 1;
          uint8* file_name_opl = ptr;
          if (ptr < header_opl){
            ptr += 1;
          }
          
          // extract include directory index
          uint64 include_directory_idx = 0;
          DWARF_LEB128_DECODE_ADV(uint64, include_directory_idx, ptr, header_opl);
          
          // extract last modified time
          uint64 last_modified_time = 0;
          DWARF_LEB128_DECODE_ADV(uint64, last_modified_time, ptr, header_opl);
          
          // extract file size
          uint64 file_size = 0;
          DWARF_LEB128_DECODE_ADV(uint64, file_size, ptr, header_opl);
      ...
      case 5:
      {
        ...
        // segment_selector_size
        segment_selector_size = MemoryConsume(uint8, ptr, unit_opl);
        ...
        directory_entry_format_count = MemoryConsume(uint8, ptr, header_opl);
        ...
        // directory_entry_format
        {
          ...
          for (;entry < entry_opl && ptr < header_opl; entry += 1){
            DWARF_LEB128_DECODE_ADV(uint64, entry.content_type, ptr, header_opl);
            DWARF_LEB128_DECODE_ADV(uint64, entry.form, ptr, header_opl);
          }
        }
        
        // directories_count
        DWARF_LEB128_DECODE_ADV(uint64, directories_count, ptr, header_opl);
        ...
        file_name_entry_format_count = MemoryConsume(uint8, ptr, header_opl);
        ...
        // file_names_count
        DWARF_LEB128_DECODE_ADV(uint64, file_names_count, ptr, header_opl);
...
dwarf__initial_length(StringView data, uint8** ptr_inout, uint8** unit_opl_out, B32* is_64bit_out){
  ...
  {
    uint64 length = 0;
    uint32 m = MemoryConsume(uint32, ptr, opl);
    if (m == 0xFFFFFFFF){
      is_64bit = 1;
      length = MemoryConsume(uint64, ptr, opl);
    }
    else{
...
static void
dwarf__line_v5_directories(uint64 address_size, uint64 offset_size,
                           DWARF_V5LinePathEntryFormat* format, uint64 format_count,
                           DWARF_V5Directory* directories_out, uint64 dir_count,
                           uint8** ptr_io, uint8* opl){
  
  uint8* ptr = *ptr_io;
  
  DWARF_V5Directory* directory_ptr = directories_out;
  for (uint32 i = 0; i < dir_count; i += 1, directory_ptr += 1){
    DWARF_V5LinePathEntryFormat* fmt = format;
    for (uint32 j = 0; j < format_count; j += 1){
      ...
      switch (fmt.content_type){
        ...
        {
          if (decoded.dataptr != 0){
            uint64 size = ClampTop(decoded.val, 8);
        ...
        {
          if (decoded.dataptr != 0){
            uint64 size = ClampTop(decoded.val, 16);
...
dwarf_abbrev_unit_from_offset(DWARF_AbbrevParsed* abbrev, uint64 offset){
...
dwarf_abbrev_decl_from_code(DWARF_AbbrevUnit* unit, uint32 abbrev_code){
...
dwarf_form_decode_rule(DWARF_AttributeForm form, uint64 address_size, uint64 offset_size){
  ...
  if (rules.size > 0){
    ...
    if (success){
      if (rules.uleb128){
        ...
        val = (uint64)dwarf_leb128_decode_S64(val_ptr, ptr);
      }
    }
  }
    ...
    if (abbrev_decl != 0){
      if (abbrev_decl.implicit_const != 0){
        val = (uint64)abbrev_decl.implicit_const[attrib_i];
      }
    }
    else{
      success = 0;
...
dwarf_info_from_data(Arena* arena, StringView data, DWARF_InfoParams* params,
  ...
  uint64 unit_idx_min = params.unit_idx_min;
  uint64 unit_idx_max = params.unit_idx_max;
  ...
  for (;ptr < opl; unit_idx += 1){
    ...
    if (full_parse){
      ...
      version = MemoryConsume(uint16, ptr, unit_opl);
      ...
      switch (version){
        case 4:
        {
          ...
          if (is_64bit){
            abbrev_offset = MemoryConsume(uint64, ptr, unit_opl);
          }
          else{
            abbrev_offset = MemoryConsume(uint32, ptr, unit_opl);
          }
      ...
      if (abbrev_unit != 0){
        for (;ptr < unit_opl;){
          ...
          uint8* entry_start_ptr = ptr;
          
          // extract abbrev code
          uint8* abbrev_code_ptr = ptr;
          ...
          uint32 abbrev_code = dwarf_leb128_decode_U32(abbrev_code_ptr, ptr);
          ...
          for (uint32 i = 0; i < attrib_count; i += 1, attrib_spec += 1, attrib_val += 1){
            ...
            uint8* dataptr = 0;
            {
              if (size > 0){
                ...
                if (abbrev_decl.implicit_const != 0){
                  val = (uint64)abbrev_decl.implicit_const[i];
                }
              }
              else if (auto_1){
                val = 1;
      ...
      uint64 language = 0;
      uint64 str_offsets_base = 0;
      uint64 line_info_offset = 0;
      uint64 vbase = 0;
      uint64 addr_base = 0;
      uint64 rnglists_base = 0;
      uint64 loclists_base = 0;
      if (entry_root != 0){
        ...
        uint32 attrib_count = root_abbrev_decl.attrib_count;
  ...
  */
  ...
  for (;ptr < opl; unit_idx += 1){
    ...
    for (;ptr < opl;){
      ...
      // count attributes
      uint8* attrib_start_ptr = ptr;
      uint32 attrib_count = 0;
      B32 has_implicit_const = 0;
      if (success){
        for (;;){
          // decode normal attribute layout
          uint8* attrib_name = ptr;
          DWARF_LEB128_ADV(ptr, opl, success);
          uint8* attrib_form = ptr;
          DWARF_LEB128_ADV(ptr, opl, success);
          
          // handle special case implicit_const
          if (success && *attrib_form == (uint8)DWARF_AttributeForm_implicit_const){
            DWARF_LEB128_ADV(ptr, opl, success);
      ...
      if (full_parse && success){
        ...
        if (has_implicit_const){
          implicit_const = push_array(arena, uint64, attrib_count);
        }
        
        // extract abbrev fields
        uint32 abbrev_code = dwarf_leb128_decode_U32(abbrev_code_ptr, tag_ptr);
        uint32 tag = dwarf_leb128_decode_U32(tag_ptr, end_tag_ptr);
        
        uint8* attrib_ptr = attrib_start_ptr;
        DWARF_AbbrevAttribSpec* attrib = attribs;
        for (uint32 i = 0; i < attrib_count; i += 1, attrib += 1){
          // mark attribute fields
          uint8* attrib_name = attrib_ptr;
          DWARF_LEB128_ADV_NOCAP(attrib_ptr);
          uint8* attrib_form = attrib_ptr;
          DWARF_LEB128_ADV_NOCAP(attrib_ptr);
          
          // extract attribute fields
          uint32 name = dwarf_leb128_decode_U32(attrib_name, attrib_form);
          uint32 form = dwarf_leb128_decode_U32(attrib_form, attrib_ptr);
          ...
          if (form == DWARF_AttributeForm_implicit_const){
            uint8* attrib_value = attrib_ptr;
            DWARF_LEB128_ADV_NOCAP(attrib_ptr);
            int64 value = dwarf_leb128_decode_S64(attrib_form, attrib_ptr);
