                       ...
                       DW_AttribKind        kind)
{
  ...
  if (attrib_class == DW_AttribClass_LocList || attrib_class == DW_AttribClass_LocListPtr) {
    ...
    string expr = dw_exprloc_from_attrib(input, cu, attrib);
...
d2r_cu_contrib_map_from_aranges(Arena *arena, DW_Input *input, U64 image_base)
{
  ...
  string     aranges_data    = input->sec[DW_Section_ARanges].data;
  ...
  for (Rng1U64Node *range_n = unit_range_list.first; range_n != 0; range_n = range_n->next) {
    string unit_data = str8_substr(aranges_data, range_n->v);
    U64     unit_cursor    = 0;
...
d2r_convert(Arena *arena, ASYNC_Root *async_root, D2R_ConvertParams *params)
{
  ...
  switch (params->exe_kind)
  {
    ...
    case ExecutableImageKind_CoffPe:
    {
      ...
      string raw_sections  = str8_substr(params->exe_data, pe.section_table_range);
      ...
      string string_table = str8_substr(params->exe_data, pe.string_table_range);
  ...
  for (U64 cu_idx = 0; cu_idx < cu_ranges.count; ++cu_idx) {
    ...
    string      cu_stmt_list = dw_line_ptr_from_tag_attrib_kind(&input, cu, cu->tag, DW_AttribKind_StmtList);
    string      cu_dir       = dw_string_from_tag_attrib_kind(&input, cu, cu->tag, DW_AttribKind_CompDir);
    string      cu_name      = dw_string_from_tag_attrib_kind(&input, cu, cu->tag, DW_AttribKind_Name);
  ...
  for (U64 cu_idx = 0; cu_idx < cu_ranges.count; ++cu_idx) {
    ...
    for (U64 file_idx = 0; file_idx < file_table->count; ++file_idx) {
      ...
      string       file_path            = dw_path_from_file_idx(scratch.arena, &line_table->vm_header, file_idx);
      ...
      string       file_path_resolved   = str8_path_list_join_by_style(scratch.arena, &file_path_split, PathStyle_WindowsAbsolute);
  ...
  for (U64 cu_idx = 0; cu_idx < cu_ranges.count; ++cu_idx) {
    ...
    string dwo_name     = dw_string_from_tag_attrib_kind(&input, cu, cu->tag, DW_AttribKind_DwoName);
    string gnu_dwo_name = dw_string_from_tag_attrib_kind(&input, cu, cu->tag, DW_AttribKind_GNU_DwoName);
    ...
    string     cu_name      = dw_string_from_tag_attrib_kind(&input, cu, cu->tag, DW_AttribKind_Name);
    string     cu_dir       = dw_string_from_tag_attrib_kind(&input, cu, cu->tag, DW_AttribKind_CompDir);
    string     cu_prod      = dw_string_from_tag_attrib_kind(&input, cu, cu->tag, DW_AttribKind_Producer);
      ...
      while (tag_stack->cur_node) {
        ...
        switch (tag.kind) {
          ...
          case DW_TagKind_SubProgram: {
            ...
            switch (inl) {
              case DW_Inl_NotInlined: {
                ...
                string frame_base_expr = dw_exprloc_from_tag_attrib_kind(&input, cu, tag, DW_AttribKind_FrameBase);
          ...
          case DW_TagKind_Variable: {
            string    name = dw_string_from_tag_attrib_kind(&input, cu, tag, DW_AttribKind_Name);
