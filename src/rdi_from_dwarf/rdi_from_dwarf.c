...
struct Frame *f = new struct Frame[1]; \
                        ...
                        DW_Tag               tag)
{
  ...
  if (has_const_value) {
    ...
    // fill out location
    RDIM_Location *loc = new RDIM_Location[1];
...
d2r_cu_contrib_map_from_aranges(Arena *arena, DW_Input *input, U64 image_base)
{
  ...
  cm.count                  = 0;
  cm.info_off_arr           = new U64[unit_range_list.count];
  cm.voff_range_arr         = new RDIM_Rng1U64ChunkList[unit_range_list.count];
...
d2r_convert(Arena *arena, ASYNC_Root *async_root, D2R_ConvertParams *params)
{
  ...
  DW_CompUnit *cu_arr = new DW_CompUnit[cu_ranges.count];
  ...
  DW_LineTableParseResult *cu_line_tables = new DW_LineTableParseResult[cu_ranges.count];
  for (U64 cu_idx = 0; cu_idx < cu_ranges.count; ++cu_idx) {
  ...
  RDIM_LineTable **cu_line_tables_rdi = new RDIM_LineTable *[cu_ranges.count];
  ...
  for (U64 cu_idx = 0; cu_idx < cu_ranges.count; ++cu_idx) {
    ...
    RDIM_SrcFile           **src_file_map = new RDIM_SrcFile *[file_table->count];
    ...
    for (DW_LineSeqNode *line_seq = line_table->first_seq; line_seq != 0; line_seq = line_seq->next) {
      ...
      U64 *voffs     = new U64[line_seq->count];
      U32 *line_nums = new U32[line_seq->count];
      U16 *col_nums  = 0;
      U64  line_idx  = 0;
  ...
  for (U64 cu_idx = 0; cu_idx < cu_ranges.count; ++cu_idx) {
    ...
    D2R_TypeTable *type_table   = new D2R_TypeTable[1];
    ...
    D2R_TagNode *free_tags = new D2R_TagNode[1];
    D2R_TagNode *tag_stack = new D2R_TagNode[1];
