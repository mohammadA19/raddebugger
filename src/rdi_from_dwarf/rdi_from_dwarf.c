...
dump_symtab(Arena* arena, String8List* out, ELF_SymArray* symbols, StringView strtab,
  ...
  for (uint64 i = 0; i < count; i += 1, symbol += 1){
...
dump_entry_tree(Arena* arena, String8List* out,
                ...
                DWARF_InfoEntry* entry, uint32 indent){
  static char spaces[] = "                                ";
  ...
  for (uint32 i = 0; i < attrib_count; i += 1, attrib_spec += 1, attrib_val += 1){
    ...
    switch (attrib_spec.form){
      ...
      {
        uint64 rnglist_off = unit.rnglists_base + attrib_val.val;
        int x = 0;
      }break;
  ...
  if (params.dump) ProfScope("dump"){
    ...
    if (params.dump_segments){
      if (elf != 0){
        ...
        uint64 count = segment_array.count;
