...
d2r_rdi_reg_from_dw_reg_code_x64(uint64 reg_code)
{
  switch (reg_code) {
...
internal RDI_RegCode
d2r_rdi_reg_from_dw_reg_code_x86(uint64 reg_code)
{
  switch (reg_code) {
...
internal RDI_RegCode
d2r_rdi_reg_from_dw_reg_code(Arch arch, uint64 reg_code)
{
  switch (arch) {
...
internal RDIM_Type *
d2r_find_or_create_type_from_offset(Arena *arena, D2R_TypeTable *type_table, uint64 info_off)
{
  RDIM_Type *type = 0;
...
d2r_range_list_from_tag(Arena *arena, DW_Input *input, DW_CompUnit *cu, uint64 image_base, DW_Tag tag)
{
  // collect non-contiguous range
  Rng1uint64List ranges = dw_rnglist_from_tag_attrib_kind(arena, input, cu, tag, DW_AttribKind_Ranges);
  
  // debase ranges
  for (Rng1uint64Node *range_n = ranges.first; range_n != 0; range_n = range_n->next) {
    // TODO: error handling
  ...
  if (lo_pc_attrib->attrib_kind != DW_AttribKind_Null && hi_pc_attrib->attrib_kind != DW_AttribKind_Null) {
    uint64 lo_pc = dw_address_from_attrib(input, cu, lo_pc_attrib);
    ...
    uint64 lo_voff = lo_pc - image_base;
    uint64 hi_voff = hi_pc - image_base;
...
internal RDIM_Type **
d2r_collect_proc_params(Arena *arena, D2R_TypeTable *type_table, DW_Input *input, DW_CompUnit *cu, DW_TagNode *cur_node, uint64 *param_count_out)
{
...
d2r_unsigned_type_kind_from_size(uint64 byte_size)
{
  RDI_TypeKind result = RDI_TypeKind_NULL;
  switch (byte_size) {
    case 1: result = RDI_TypeKind_uint8;  break;
    case 2: result = RDI_TypeKind_uint16; break;
    case 4: result = RDI_TypeKind_uint32; break;
    case 8: result = RDI_TypeKind_uint64; break;
  }
  return result;
}

internal RDI_TypeKind
d2r_signed_type_kind_from_size(uint64 byte_size)
{
  RDI_TypeKind result = RDI_TypeKind_NULL;
  switch (byte_size) {
    case 1: result = RDI_TypeKind_uint8;  break;
...
d2r_type_group_from_type_kind(RDI_TypeKind x)
{
  switch (x) {
    ...
    return RDI_EvalTypeGroup_float;
    case RDI_TypeKind_double:
    return RDI_EvalTypeGroup_double;
    default: InvalidPath;
  }
...
#define peek_type() stack->value_type
  ...
  if (address_size == 4) {
    addr_type_kind = RDI_TypeKind_uint32;
  } else if (address_size == 8) {
    addr_type_kind = RDI_TypeKind_uint64;
  }
  
  
  for (uint64 cursor = 0; cursor < expr.size; ) {
    uint8 op = 0;
    ...
    switch (op) {
      ...
      case DW_ExprOp_Lit30: case DW_ExprOp_Lit31: {
        uint64 lit = op - DW_ExprOp_Lit0;
        
        rdim_bytecode_push_uconst(arena, &bc, lit);
        push_of_type(RDI_TypeKind_uint64);
      } break;
      ...
      case DW_ExprOp_Const1U: {
        ...
        push_of_type(RDI_TypeKind_uint8);
      } break;
      case DW_ExprOp_Const2U: {
        ...
        rdim_bytecode_push_uconst(arena, &bc, val);
        push_of_type(RDI_TypeKind_uint16);
      } break;
      case DW_ExprOp_Const4U: {
        ...
        rdim_bytecode_push_uconst(arena, &bc, val);
        push_of_type(RDI_TypeKind_uint32);
      } break;
      ...
      case DW_ExprOp_Reg30: case DW_ExprOp_Reg31: {
        uint64 reg_code_dw  = op - DW_ExprOp_Reg0;
        uint64 reg_size     = dw_reg_size_from_code(arch, reg_code_dw);
        uint64 reg_pos      = dw_reg_pos_from_code(arch, reg_code_dw);
        
        RDI_RegCode reg_code_rdi  = d2r_rdi_reg_from_dw_reg_code(arch, reg_code_dw);
        uint32         regread_param = RDI_EncodeRegReadParam(reg_code_rdi, reg_size, reg_pos);
        rdim_bytecode_push_op(arena, &bc, RDI_EvalOp_RegRead, regread_param);
        push_of_type(d2r_unsigned_type_kind_from_size(reg_size));
      } break;
      
      case DW_ExprOp_RegX: {
        uint64 reg_code_dw = 0;
      ...
      case DW_ExprOp_Piece: {
        uint64 piece_byte_size = 0;
        cursor += str8_deserial_read_uleb128(expr, cursor, &piece_byte_size);
        
        uint64 partial_value_size32 = safe_cast_u32(piece_byte_size);
        rdim_bytecode_push_op(arena, &bc, RDI_EvalOp_PartialValue, partial_value_size32);
      } break;
      
      case DW_ExprOp_BitPiece: {
        uint64 piece_bit_size = 0;
        uint64 piece_bit_off  = 0;
        cursor += str8_deserial_read_uleb128(expr, cursor, &piece_bit_size);
        cursor += str8_deserial_read_uleb128(expr, cursor, &piece_bit_off);
        
        uint32 piece_bit_size32 = safe_cast_u32(piece_bit_size);
        uint32 piece_bit_off32  = safe_cast_u32(piece_bit_off);
        
        uint64 partial_value = ((uint64)piece_bit_size32 << 32) | (uint64)piece_bit_off32;
      ...
      case DW_ExprOp_BReg30: case DW_ExprOp_BReg31: {
        uint64 reg_code_dw = op - DW_ExprOp_BReg0;
        uint64 reg_off     = 0;
      ...
      case DW_ExprOp_GNU_Convert: {
        ...
        if (type_info_off == 0) {
          ...
          if (tag.kind == DW_TagKind_BaseType) {
            ...
            switch (encoding) {
              ...
              case DW_ATE_Float: {
                uint64 byte_size = dw_const_u64_from_tag_attrib_kind(input, cu, tag, DW_AttribKind_ByteSize);
                switch (byte_size) {
                  case 4: out = RDI_EvalTypeGroup_float; break;
                  case 8: out = RDI_EvalTypeGroup_double; break;
                  default: InvalidPath;
                }
              } break;
              default: InvalidPath;
        ...
        uint16 operand = (uint16)in | ((uint16)out << 8);
      ...
      case DW_ExprOp_GNU_EntryValue: {
        uint64     entry_value_expr_size = 0;
        ...
        uint32 encoded_size32 = safe_cast_u32(call_site_bc.encoded_size);
      ...
      case DW_ExprOp_Addrx: {
        ...
        uint64 addr = dw_addr_from_list_unit(addr_lu, addr_idx);
        if (addr != max_uint64) {
          if (addr >= image_base) {
            uint64 voff = addr - image_base;
            rdim_bytecode_push_op(arena, &bc, RDI_EvalOp_ModuleOff, voff);
...
d2r_transpile_expression(Arena *arena, DW_Input *input, uint64 image_base, uint64 address_size, Arch arch, DW_ListUnit *addr_lu, DW_CompUnit *cu, String8 expr)
{
  RDIM_Location *loc = 0;
  if (expr.size) {
    B32               is_addr  = 0;
...
internal RDIM_Location *
d2r_location_from_attrib(Arena *arena, DW_Input *input, DW_CompUnit *cu, uint64 image_base, Arch arch, DW_Tag tag, DW_AttribKind kind)
{
                       ...
                       DW_AttribKind        kind)
{
  ...
  if (attrib_class == DW_AttribClass_LocList || attrib_class == DW_AttribClass_LocListPtr) {
    ...
    for (DW_LocNode *loc_n = loclist.first; loc_n != 0; loc_n = loc_n->next) {
      ...
      RDIM_Rng1uint64   voff_range = { .min = loc_n->v.range.min -  image_base, .max = loc_n->v.range.max - image_base };
    ...
    RDIM_Location *location = d2r_transpile_expression(arena, input, image_base, cu->address_size, arch, cu->addr_lu, cu, expr);
    for (RDIM_Rng1uint64Node *range_n = curr_scope->voff_ranges.first; range_n != 0; range_n = range_n->next) {
                        ...
                        DW_Tag               tag)
{
  ...
  if (has_const_value) {
    // extract const value
    uint64 const_value = dw_u64_from_attrib(input, cu, tag, DW_AttribKind_ConstValue);
...
d2r_cu_contrib_map_from_aranges(Arena *arena, DW_Input *input, uint64 image_base)
{
  Temp scratch = scratch_begin(&arena, 1);
  
  String8     aranges_data    = input->sec[DW_Section_ARanges].data;
  Rng1uint64List unit_range_list = dw_unit_ranges_from_data(scratch.arena, aranges_data);
  
  D2R_CompUnitContribMap cm = {0};
  cm.count                  = 0;
  cm.info_off_arr           = push_array(arena, uint64,                   unit_range_list.count);
  cm.voff_range_arr         = push_array(arena, RDIM_Rng1uint64ChunkList, unit_range_list.count);
  
  for (Rng1uint64Node *range_n = unit_range_list.first; range_n != 0; range_n = range_n->next) {
    String8 unit_data = str8_substr(aranges_data, range_n->v);
    uint64     unit_cursor    = 0;
    
    uint64 unit_length = 0;
    uint64 unit_length_size = str8_deserial_read_dwarf_packed_size(unit_data, unit_cursor, &unit_length);
    ...
    uint64       cu_info_off      = 0;
    uint64       cu_info_off_size = str8_deserial_read_dwarf_uint(unit_data, unit_cursor, unit_format, &cu_info_off);
    ...
    uint64 segment_selector_size_size = str8_deserial_read_struct(unit_data, unit_cursor, &segment_selector_size);
    ...
    uint64 bytes_too_far_past_boundary = unit_cursor % tuple_size;
    if (bytes_too_far_past_boundary > 0) {
      unit_cursor += tuple_size - bytes_too_far_past_boundary;
    }
    
    RDIM_Rng1uint64ChunkList voff_ranges = {0};
    if (segment_selector_size == 0) {
      while (unit_cursor + address_size * 2 <= unit_data.size) {
        ...
        uint64 min = address - image_base;
        uint64 max = min + length;
        rdim_rng1u64_chunk_list_push(arena, &voff_ranges, 256, (RDIM_Rng1uint64){.min = min, .max = max});
      }
    } else {
...
internal RDIM_Rng1uint64ChunkList
d2r_voff_ranges_from_cu_info_off(D2R_CompUnitContribMap map, uint64 info_off)
{
  RDIM_Rng1uint64ChunkList voff_ranges   = {0};
  uint64              voff_list_idx = u64_array_bsearch(map.info_off_arr, map.count, info_off);
...
internal RDIM_Scope *
d2r_push_scope(Arena *arena, RDIM_ScopeChunkList *scopes, uint64 scope_chunk_cap, D2R_TagNode *tag_stack, Rng1uint64List ranges)
{
  // fill out scope
  RDIM_Scope *scope = rdim_scope_chunk_list_push(arena, scopes, scope_chunk_cap);
  
  // push ranges
  for (Rng1uint64Node *i = ranges.first; i != 0; i = i->next) {
    rdim_scope_push_voff_range(arena, scopes, scope, (RDIM_Rng1uint64){.min = i->v.min, i->v.max});
  }
...
d2r_convert(Arena *arena, ASYNC_Root *async_root, D2R_ConvertParams *params)
{
  ...
  uint64 exe_hash = rdi_hash(params->exe_data.str, params->exe_data.size);
  ProfEnd();
  ...
  switch(params->exe_kind)
  {
    ...
    case ExecutableImageKind_CoffPe:
    {
      ...
      uint64                 section_count = raw_sections.size / sizeof(COFF_SectionHeader);
  ...
  RDIM_TopLevelInfo top_level_info = rdim_make_top_level_info(params->exe_name, arch, exe_hash, binary_sections);
  
  ////////////////////////////////
  
  uint64 arch_addr_size = rdi_addr_size_from_arch(arch);
  
  ////////////////////////////////
  
  static const uint64 UNIT_CHUNK_CAP        = 256;
  static const uint64 UDT_CHUNK_CAP         = 256;
  static const uint64 TYPE_CHUNK_CAP        = 256;
  static const uint64 GVAR_CHUNK_CAP        = 256;
  static const uint64 TVAR_CHUNK_CAP        = 256;
  static const uint64 PROC_CHUNK_CAP        = 256;
  static const uint64 SCOPE_CHUNK_CAP       = 256;
  static const uint64 INLINE_SITE_CHUNK_CAP = 256;
  static const uint64 SRC_FILE_CAP          = 256;
  static const uint64 LINE_TABLE_CAP        = 256;
  ...
  Rng1uint64List      cu_range_list = dw_unit_ranges_from_data(scratch.arena, input.sec[DW_Section_Info].data);
  Rng1uint64Array     cu_ranges     = rng1u64_array_from_list(scratch.arena, &cu_range_list);
  ProfEnd();
  ...
  for (uint64 cu_idx = 0; cu_idx < cu_ranges.count; ++cu_idx) {
  ...
  for (uint64 cu_idx = 0; cu_idx < cu_ranges.count; ++cu_idx) {
    ...
    for (uint64 file_idx = 0; file_idx < file_table->count; ++file_idx) {
    ...
    for (DW_LineSeqNode *line_seq = line_table->first_seq; line_seq != 0; line_seq = line_seq->next) {
      ...
      uint64 *voffs     = push_array(arena, uint64, line_seq->count);
      uint32 *line_nums = push_array(arena, uint32, line_seq->count);
      uint16 *col_nums  = 0;
      uint64  line_idx  = 0;
      
      DW_LineNode *file_line_n     = line_seq->first;
      uint64          file_line_count = 0;
      ...
      for (DW_LineNode *line_n = file_line_n; line_n != 0; line_n = line_n->next) {
        if (file_line_n->v.file_index != line_n->v.file_index || line_n->next == 0) {
          uint64  file_index     = file_line_n->v.file_index;
          uint64 *file_voffs     = &voffs[line_idx];
          uint32 *file_line_nums = &line_nums[line_idx];
          uint16 *file_col_nums  = 0;
          
          uint64          lines_written = 0;
          uint64          prev_ln       = max_uint64;
  ...
  for (uint64 cu_idx = 0; cu_idx < cu_ranges.count; ++cu_idx) {
    ...
    RDIM_Rng1uint64ChunkList cu_voff_ranges = d2r_voff_ranges_from_cu_info_off(cu_contrib_map, cu_ranges.v[cu_idx].min);
      ...
      while (tag_stack->cur_node) {
        ...
        switch (tag.kind) {
          ...
          case DW_TagKind_SubroutineType: {
            ...
            for (DW_TagNode *n = cur_node->first_child; n != 0; n = n->sibling) {
              if (n->tag.kind == DW_TagKind_FormalParameter) {
                RDIM_Type *param_type = d2r_type_from_attrib(arena, type_table, &input, cu, n->tag, DW_AttribKind_Type);
            ...
            // init proceudre type
            RDIM_Type *ret_type = d2r_type_from_attrib(arena, type_table, &input, cu, tag, DW_AttribKind_Type);
            RDIM_Type *type     = d2r_find_or_create_type_from_offset(arena, type_table, tag.info_off);
          ...
          case DW_TagKind_BaseType: {
            DW_ATE encoding  = dw_const_u64_from_tag_attrib_kind(&input, cu, tag, DW_AttribKind_Encoding);
            uint64    byte_size = dw_byte_size_from_tag(&input, cu, tag);
            ...
            switch (encoding) {
              ...
              case DW_ATE_ComplexFloat: {
                switch (byte_size) {
                  case 4:  kind = RDI_TypeKind_Complexfloat;  break;
                  case 8:  kind = RDI_TypeKind_Complexdouble;  break;
              ...
              case DW_ATE_Float: {
                switch (byte_size) {
                  ...
                  case 4:  kind = RDI_TypeKind_float;  break;
                  case 6:  kind = RDI_TypeKind_F48;  break;
                  case 8:  kind = RDI_TypeKind_double;  break;
              ...
              case DW_ATE_Signed: {
                switch (byte_size) {
                  case 1:  kind = RDI_TypeKind_uint8;   break;
                  case 2:  kind = RDI_TypeKind_uint16;  break;
                  case 4:  kind = RDI_TypeKind_uint32;  break;
                  case 8:  kind = RDI_TypeKind_uint64;  break;
                  case 16: kind = RDI_TypeKind_uint128; break;
          ...
          case DW_TagKind_PointerType: {
            ...
            uint64 byte_size = arch_addr_size;
          ...
          case DW_TagKind_ArrayType: {
            ...
            //                          \
            //                           -> (B0) DW_TAG_PointerType -> (A1) DW_TAG_ArrayType -> DW_TAG_Subrange [3] -> DW_TagKind_Subrange [4]
            //                                                      \
            ...
            for (DW_TagNode *n = cur_node->first_child; n != 0; n = n->sibling) {
              ...
              if (dw_tag_has_attrib(&input, cu, n->tag, DW_AttribKind_Count)) {
                uint64 count = dw_u64_from_attrib(&input, cu, n->tag, DW_AttribKind_Count);
            ...
            switch (inl) {
              case DW_Inl_NotInlined: {
                ...
                if (dw_tag_has_attrib(&input, cu, tag, DW_AttribKind_ContainingType)) {
                  container_type = d2r_type_from_attrib(arena, type_table, &input, cu, tag, DW_AttribKind_ContainingType);
                }
                ...
                // make scope
                Rng1uint64List  ranges     = d2r_range_list_from_tag(comp_temp.arena, &input, cu, image_base, tag);
          ...
          case DW_TagKind_InlinedSubroutine: {
            ...
            RDIM_Type *owner = 0;
            if (dw_tag_has_attrib(&input, cu, tag, DW_AttribKind_ContainingType)) {
              owner = d2r_type_from_attrib(arena, type_table, &input, cu, tag, DW_AttribKind_ContainingType);
            }
          ...
          case DW_TagKind_Variable: {
            ...
            DW_TagKind parent_tag_kind = tag_stack->next->cur_node->tag.kind;
            if (parent_tag_kind == DW_TagKind_SubProgram ||
                parent_tag_kind == DW_TagKind_InlinedSubroutine ||
          ...
          case DW_TagKind_LexicalBlock: {
            if (tag_stack->next->cur_node->tag.kind == DW_TagKind_SubProgram ||
                tag_stack->next->cur_node->tag.kind == DW_TagKind_InlinedSubroutine ||
                tag_stack->next->cur_node->tag.kind == DW_TagKind_LexicalBlock) {
              Rng1uint64List ranges = d2r_range_list_from_tag(comp_temp.arena, &input, cu, image_base, tag);
          ...
          break;
          case DW_TagKind_Namespace: break;
          case DW_TagKind_ImportedDeclaration: break;
          case DW_TagKind_PtrToMemberType: break;
          case DW_TagKind_TemplateTypeParameter: break;
          case DW_TagKind_ReferenceType: break;
  ...
  
  {
    for (RDIM_TypeChunkNode *chunk_n = types.first; chunk_n != 0; chunk_n = chunk_n->next) {
      for (uint64 i = 0; i < chunk_n->count; ++i) {
    ...
    for (RDIM_TypeChunkNode *chunk_n = types.first; chunk_n != 0; chunk_n = chunk_n->next) {
      for (uint64 i = 0; i < chunk_n->count; ++i) {
        ...
        if (type->kind == RDI_TypeKind_Array) {
          ...
          uint64 base_type_size = 0;
          if (t) {
            base_type_size = t->byte_size;
          }
          
          uint64 array_size = base_type_size;
          while (type_stack) {
...
internal B32
rdi_reg_from_dw_reg_x64(DW_RegX64 v, RDI_RegCodeX64 *code_out, uint64 *off_out, uint64 *size_out)
{
...
internal B32
rdi_reg_from_dw_reg(Arch arch, DW_Reg v, RDI_RegCode *code_out, uint64 *off_out, uint64 *size_out)
{
