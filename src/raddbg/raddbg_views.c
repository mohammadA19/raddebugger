...
rd_code_view_build(Arena* arena, RD_CodeViewState* cv, RD_CodeViewBuildFlags flags, Rng2F32 rect, StringView text_data, TXT_TextInfo* text_info, DASM_LineArray* dasm_lines, Rng1U64 dasm_vaddr_range, DI_Key dasm_dbgi_key)
{
  ProfBeginFunction();
  ...
  int64 num_possible_visible_lines = (int64)(code_area_dim.y/code_line_height)+1;
  ...
  for(RD_Cmd* cmd = 0; rd_next_cmd(&cmd);)
  {
    ...
    switch(kind)
    {
      ...
      case RD_CmdKind_FindTextForward:
      {
        ...
        cv.find_text_fwd = push_str8_copy(cv.find_text_arena, cmd.regs.str);
      }break;
      case RD_CmdKind_FindTextBackward:
      {
        arena_clear(cv.find_text_arena);
        cv.find_text_bwd = push_str8_copy(cv.find_text_arena, cmd.regs.str);
      }break;
  ...
  //
  Rng1S64 visible_line_num_range = r1s64(scroll_pos.y.idx + (int64)(scroll_pos.y.off) + 1 - !!(scroll_pos.y.off < 0),
                                         scroll_pos.y.idx + (int64)(scroll_pos.y.off) + 1 + num_possible_visible_lines);
  Rng1S64 target_visible_line_num_range = r1s64(scroll_pos.y.idx + 1,
                                                scroll_pos.y.idx + 1 + num_possible_visible_lines);
  uint64 visible_line_count = 0;
  {
    visible_line_num_range.min = Clamp(1, visible_line_num_range.min, (int64)text_info.lines_count);
    visible_line_num_range.max = Clamp(1, visible_line_num_range.max, (int64)text_info.lines_count);
    visible_line_num_range.min = Max(1, visible_line_num_range.min);
    visible_line_num_range.max = Max(1, visible_line_num_range.max);
    target_visible_line_num_range.min = Clamp(1, target_visible_line_num_range.min, (int64)text_info.lines_count);
    target_visible_line_num_range.max = Clamp(1, target_visible_line_num_range.max, (int64)text_info.lines_count);
    ...
    visible_line_count = (uint64)dim_1s64(visible_line_num_range)+1;
  }
  ...
  Rng1S64 scroll_idx_rng[Axis2_COUNT] = {0};
  {
    ...
    line_size_x = ClampBot(line_size_x, (int64)big_glyph_advance*120);
    line_size_x = ClampBot(line_size_x, (int64)code_area_dim.x);
    scroll_idx_rng[Axis2_X] = r1s64(0, line_size_x-(int64)code_area_dim.x);
    scroll_idx_rng[Axis2_Y] = r1s64(0, (int64)text_info.lines_count-1);
  }
  ...
  B32 search_query_is_active = 0;
  {
    ...
    {
      search_query = StringView(window.query_view_stack_top.query_buffer, window.query_view_stack_top.query_string_size);
  ...
  RD_CodeSliceParams code_slice_params = {0};
  {
    ...
    code_slice_params.line_text                 = push_array(scratch.arena, StringView, visible_line_count);
    ...
    code_slice_params.line_vaddrs               = push_array(scratch.arena, uint64, visible_line_count);
    ...
    // rjf: fill text info
    {
      int64 line_num = visible_line_num_range.min;
      uint64 line_idx = visible_line_num_range.min-1;
      for(uint64 visible_line_idx = 0;
    ...
    if(!dasm_lines) ProfScope("find visible breakpoints for source code")
    {
      ...
      for(RD_EntityNode* n = bps.first; n != 0; n = n.next)
      {
        ...
        if(visible_line_num_range.min <= loc.text_point.line && loc.text_point.line <= visible_line_num_range.max)
        {
          ...
          {
            if(path_match_normalized(loc.str, override_n.str))
            {
              uint64 slice_line_idx = (loc.text_point.line-visible_line_num_range.min);
    ...
    if(!dasm_lines) ProfScope("find live threads mapping to this file")
    {
      ...
      for(CTRL_EntityNode* thread_n = threads.first; thread_n != 0; thread_n = thread_n.next)
      {
        ...
        uint64 inline_depth = (thread == selected_thread) ? rd_regs().inline_depth : 0;
        uint64 rip_vaddr = d_query_cached_rip_from_thread_unwind(thread, unwind_count);
        uint64 last_inst_on_unwound_rip_vaddr = rip_vaddr - !!unwind_count;
        CTRL_Entity* module = ctrl_module_from_process_vaddr(process, last_inst_on_unwound_rip_vaddr);
        uint64 rip_voff = ctrl_voff_from_vaddr(module, last_inst_on_unwound_rip_vaddr);
        ...
        for(D_LineNode* n = lines.first; n != 0; n = n.next)
        {
          if(visible_line_num_range.min <= n.v.pt.line && n.v.pt.line <= visible_line_num_range.max)
          {
            for(String8Node* override_n = file_path_possible_overrides.first;
                ...
                override_n = override_n.next)
            {
              if(path_match_normalized(n.v.file_path, override_n.str))
              {
                uint64 slice_line_idx = n.v.pt.line-visible_line_num_range.min;
    ...
    if(dasm_lines) ProfScope("find live threads mapping to this disassembly")
    {
      ...
      for(CTRL_EntityNode* thread_n = threads.first; thread_n != 0; thread_n = thread_n.next)
      {
        ...
        {
          uint64 rip_off = rip_vaddr - dasm_vaddr_range.min;
          int64 line_num = dasm_line_array_idx_from_code_off__linear_scan(dasm_lines, rip_off)+1;
          if(contains_1s64(visible_line_num_range, line_num))
          {
            uint64 slice_line_idx = (line_num-visible_line_num_range.min);
            ctrl_entity_list_push(scratch.arena, &code_slice_params.line_ips[slice_line_idx], thread);
          }
        }
      }
    ...
    if(dasm_lines) ProfScope("find breakpoints mapping to this disassembly")
    {
      ...
      for(RD_EntityNode* n = bps.first; n != 0; n = n.next)
      {
        ...
        {
          uint64 off = loc.vaddr-dasm_vaddr_range.min;
          uint64 idx = dasm_line_array_idx_from_code_off__linear_scan(dasm_lines, off);
          int64 line_num = (int64)(idx+1);
          if(contains_1s64(visible_line_num_range, line_num))
          {
            uint64 slice_line_idx = (line_num-visible_line_num_range.min);
            rd_entity_list_push(scratch.arena, &code_slice_params.line_bps[slice_line_idx], bp);
    ...
    if(dasm_lines)
    {
      ...
      DI_Key dbgi_key = ctrl_dbgi_key_from_module(module);
      for(int64 line_num = visible_line_num_range.min; line_num < visible_line_num_range.max; line_num += 1)
      {
        uint64 vaddr = dasm_vaddr_range.min + dasm_line_array_code_off_from_idx(dasm_lines, line_num-1);
        uint64 voff = ctrl_voff_from_vaddr(module, vaddr);
        uint64 slice_idx = line_num-visible_line_num_range.min;
  ...
  //
  {
    ...
    if(cv.find_text_fwd.size != 0)
    {
      B32 found = 0;
      B32 first = 1;
      int64 line_num_start = rd_regs().cursor.line;
      int64 line_num_last = (int64)text_info.lines_count;
      for(int64 line_num = line_num_start; 1 <= line_num && line_num <= line_num_last; first = 0)
      {
        // rjf: gather line info
        StringView line_string = str8_substr(text_data, text_info.lines_ranges[line_num-1]);
        ...
        // rjf: search str
        uint64 needle_pos = str8_find_needle(line_string, search_start, cv.find_text_fwd, StringMatchFlag_CaseInsensitive);
    ...
    if(cv.find_text_bwd.size != 0)
    {
      ...
      for(int64 line_num = line_num_start; 1 <= line_num && line_num <= line_num_last; first = 0)
      {
        ...
        // rjf: search str
        uint64 next_needle_pos = line_string.size;
  ...
  {
    int64 line_num = cv.goto_line_num;
    cv.goto_line_num = 0;
  ...
  UI_Parent(container_box)
  {
    ...
    {
      ui_kill_action();
      rd_cmd(RD_CmdKind_GoToName, .str = txt_string_from_info_data_txt_rng(text_info, text_data, sig.mouse_expr_rng));
    }
    ...
    if(cv.watch_expr_at_mouse)
    {
      ...
      rd_cmd(RD_CmdKind_ToggleWatchExpression, .str = txt_string_from_info_data_txt_rng(text_info, text_data, sig.mouse_expr_rng));
    }
    ...
    {
      StringView text = txt_string_from_info_data_txt_rng(text_info, text_data, txt_rng(rd_regs().cursor, rd_regs().mark));
  ...
  if(text_info.lines_count != 0)
  {
    ...
    if(cv.center_cursor && text_info.lines_count != 0)
    {
      ...
      if(cursor_in_range)
      {
        StringView cursor_line = str8_substr(text_data, text_info.lines_ranges[cursor.line-1]);
        ...
        // rjf: scroll x
        {
          int64 new_idx = (int64)(cursor_advance - code_area_dim.x/2);
        ...
        // rjf: scroll y
        {
          int64 new_idx = (cursor.line-1) - num_possible_visible_lines/2 + 2;
    ...
    {
      StringView cursor_line = str8_substr(text_data, text_info.lines_ranges[cursor.line-1]);
      int64 cursor_off = (int64)(fnt_dim_from_tag_size_string(code_font, code_font_size, 0, code_tab_size, str8_prefix(cursor_line, cursor.column-1)).x + priority_margin_width_px + catchall_margin_width_px + line_num_width_px);
      Rng1S64 visible_pixel_range =
      {
        scroll_pos.x.idx,
        scroll_pos.x.idx + (int64)code_area_dim.x,
      };
      Rng1S64 cursor_pixel_range =
      {
        cursor_off - (int64)(big_glyph_advance*4) - (int64)(priority_margin_width_px + catchall_margin_width_px + line_num_width_px),
        cursor_off + (int64)(big_glyph_advance*4),
      };
      int64 min_delta = Min(0, cursor_pixel_range.min - visible_pixel_range.min);
      int64 max_delta = Max(0, cursor_pixel_range.max - visible_pixel_range.max);
      int64 new_idx = scroll_pos.x.idx+min_delta+max_delta;
      new_idx = Clamp(scroll_idx_rng[Axis2_X].min, new_idx, scroll_idx_rng[Axis2_X].max);
      ui_scroll_pt_target_idx(&scroll_pos.x, new_idx);
    }
    ...
    if(snap[Axis2_Y])
    {
      ...
      int64 min_delta = Min(0, cursor_visibility_range.min-(target_visible_line_num_range.min));
      int64 max_delta = Max(0, cursor_visibility_range.max-(target_visible_line_num_range.min+num_possible_visible_lines));
      int64 new_idx = scroll_pos.y.idx+min_delta+max_delta;
      new_idx = Clamp(0, new_idx, (int64)text_info.lines_count-1);
      ui_scroll_pt_target_idx(&scroll_pos.y, new_idx);
    }
  }
  ...
  if(text_info.lines_count != 0)
  {
    ...
    if(sig.scroll.x != 0)
    {
      int64 new_idx = scroll_pos.x.idx+sig.scroll.x*big_glyph_advance;
      new_idx = clamp_1s64(scroll_idx_rng[Axis2_X], new_idx);
      ui_scroll_pt_target_idx(&scroll_pos.x, new_idx);
    }
    if(sig.scroll.y != 0)
    {
      int64 new_idx = scroll_pos.y.idx + sig.scroll.y;
...
RD_WatchViewColumn *
rd_watch_view_column_from_x(RD_WatchViewState* wv, int64 index)
{
...
rd_watch_view_point_from_tbl(EV_BlockRangeList* block_ranges, Vec2S64 tbl)
{
  ...
  pt.x           = tbl.x;
  pt.key         = ev_key_from_num(block_ranges, (uint64)tbl.y);
  pt.parent_key  = ev_block_range_from_num(block_ranges, (uint64)tbl.y).block.key;
  return pt;
}
...
rd_tbl_from_watch_view_point(EV_BlockRangeList* block_ranges, RD_WatchViewPoint pt)
{
  Vec2S64 tbl = {0};
  tbl.x = pt.x;
  tbl.y = (int64)ev_num_from_key(block_ranges, pt.key);
  return tbl;
}
...
rd_watch_view_row_info_from_row(EV_Row* row)
{
  RD_WatchViewRowInfo info = {0};
  {
    ...
    {
      uint64 block_relative_num = block.expand_view_rule_info.expr_expand_num_from_id(key.child_id, block.expand_view_rule_info_user_data);
    ...
    uint64 unwind_count = 0;
    uint64 inline_depth = 0;
    if(thread != &ctrl_entity_nil)
    {
      ...
      uint64 frame_num = 1;
      for(uint64 base_frame_idx = 0; base_frame_idx < rich_unwind.concrete_frame_count; base_frame_idx += 1, frame_num += 1)
      {
...
{
  Temp scratch = scratch_begin(0, 0);
  RD_ViewRuleInfo* ui_view_rule_info = rd_view_rule_info_from_string(row.block.expand_view_rule_info.str);
...
rd_expr_from_watch_view_row_column(Arena* arena, EV_View* ev_view, EV_Row* row, RD_WatchViewColumn* col)
{
  ...
  switch(col.kind)
  {
    ...
    {
      Temp scratch = scratch_begin(&arena, 1);
      StringView access_string = StringView(col.string_buffer, col.string_size);
      String8List accesses = str8_split(scratch.arena, access_string, (uint8 *)".", 1, 0);
      for(String8Node* n = accesses.first; n != 0; n = n.next)
      {
        expr = e_expr_ref_member_access(arena, expr, n.str);
      }
      scratch_end(scratch);
    }break;
  }
  ...
  {
    EV_ViewRuleList* view_rules = ev_view_rule_list_from_string(arena, StringView(col.view_rule_buffer, col.view_rule_size));
...
StringView
rd_string_from_eval_viz_row_column(Arena* arena, EV_View* ev, EV_Row* row, RD_WatchViewColumn* col, EV_StringFlags string_flags, uint32 default_radix, FNT_Tag font, float font_size, float max_size_px)
{
  ProfBeginFunction();
  StringView result = {0};
  ...
  switch(col.kind)
  {
    ...
    ProfScope("module cell str")
    {
      ...
      result = push_str8_copy(arena, str8_skip_last_slash(module.str));
    }break;
    case RD_WatchViewColumnKind_CallStackFrame:
    ProfScope("call stack frame cell str")
    {
      Temp scratch = scratch_begin(&arena, 1);
      DI_Scope* di_scope = di_scope_open();
      E_Eval eval = e_eval_from_expr(arena, row_col_expr);
      ...
      uint64 vaddr = vaddr_value_eval.value.u64;
      uint64 depth = depth_value_eval.value.u64;
      CTRL_Entity* entity = rd_ctrl_entity_from_eval_space(eval.space);
      CTRL_Entity* process = ctrl_process_from_entity(entity);
...
rd_watch_view_text_edit_state_from_pt(RD_WatchViewState* wv, RD_WatchViewPoint pt)
{
  ...
  if(wv.text_edit_state_slots_count != 0 && wv.text_editing != 0)
  {
    ...
    uint64 slot_idx = hash%wv.text_edit_state_slots_count;
...
rd_watch_view_column_alloc_(RD_WatchViewState* wv, RD_WatchViewColumnKind kind, float pct, RD_WatchViewColumnParams* params)
{
  ...
  col.kind = kind;
  col.pct = pct;
  col.string_size = Min(sizeof(col.string_buffer), params.str.Length);
  MemoryCopy(col.string_buffer, params.str.Ptr, col.string_size);
  col.display_string_size = Min(sizeof(col.display_string_buffer), params.display_string.size);
  MemoryCopy(col.display_string_buffer, params.display_string.Ptr, col.display_string_size);
  col.view_rule_size = Min(sizeof(col.view_rule_buffer), params.view_rule.size);
  MemoryCopy(col.view_rule_buffer, params.view_rule.Ptr, col.view_rule_size);
...
void
rd_watch_view_build(RD_WatchViewState* ewv, RD_WatchViewFlags flags, StringView root_expr, StringView root_view_rule, B32 modifiable, uint32 default_radix, Rng2F32 rect)
{
  ProfBeginFunction();
  DI_Scope* di_scope = di_scope_open();
  Temp scratch = scratch_begin(0, 0);
  ...
  StringView eval_view_key_string = push_str8f(scratch.arena, "eval_view_watch_%p", ewv);
  EV_View* eval_view = rd_ev_view_from_key(d_hash_from_string(eval_view_key_string));
  StringView filter = rd_view_filter();
  float row_height_px = floor_f32(ui_top_font_size()*2.5f);
  int64 num_possible_visible_rows = (int64)(dim_2f32(rect).y/row_height_px);
  ...
  uint64 row_ctrls_count = ArrayCount(row_ctrls_);
  ...
  B32 is_top_level_hook = 0;
  {
    ...
    {
      RD_ViewRuleInfo* rule_info = rd_view_rule_info_from_string(n.v.root.str);
  ...
  if(!is_top_level_hook) ProfScope("consume events & perform navigations/edits - calculate state") UI_Focus(UI_FocusKind_On)
  {
    ...
    for(UI_Event* event = 0;;)
    {
      ...
      if(snap_to_cursor)
      {
        ...
        int64 cursor_item_idx = cursor_tbl.y;
        if(item_range.min <= cursor_item_idx && cursor_item_idx <= item_range.max)
        {
          ...
          {
            cursor_visibility_row_range.min = (int64)ui_scroll_list_row_from_item(&row_blocks, (uint64)cursor_item_idx) - 1;
            cursor_visibility_row_range.max = cursor_visibility_row_range.min + 3;
          }
          
          //- rjf: compute deltas & apply
          int64 min_delta = Min(0, cursor_visibility_row_range.min-visible_row_range.min);
          int64 max_delta = Max(0, cursor_visibility_row_range.max-visible_row_range.max);
          int64 new_idx = scroll_pt.idx+min_delta+max_delta;
         ...
         (selection_tbl.min.y != 0 || selection_tbl.min.y != 0))
      {
        ...
        for(int64 y = selection_tbl.min.y; y <= selection_tbl.max.y; y += 1, row = row.next)
        {
          ...
          {
            ewv.text_editing = 1;
            for(int64 x = selection_tbl.min.x; x <= selection_tbl.max.x; x += 1)
            {
              RD_WatchViewColumn* col = rd_watch_view_column_from_x(ewv, x);
              StringView str = rd_string_from_eval_viz_row_column(scratch.arena, eval_view, row, col, string_flags, default_radix, ui_top_font(), ui_top_font_size(), row_string_max_size_px);
              str.Length = Min(str.Length, sizeof(ewv.dummy_text_edit_state.input_buffer));
              ...
              edit_state.pt = pt;
              edit_state.cursor = txt_pt(1, str.Length+1);
              edit_state.mark = txt_pt(1, 1);
              edit_state.input_size = str.Length;
              MemoryCopy(edit_state.input_buffer, str.Ptr, str.Length);
              edit_state.initial_size = str.Length;
              MemoryCopy(edit_state.initial_buffer, str.Ptr, str.Length);
            }
          }
        }
      }
      ...
      if(!ewv.text_editing && evt.slot == UI_EventActionSlot_Accept)
      {
        ...
        EV_WindowedRowList rows = ev_windowed_row_list_from_block_range_list(scratch.arena, eval_view, filter, &block_ranges, r1u64(ui_scroll_list_row_from_item(&row_blocks, selection_tbl.min.y),
                                                                                                                                    ui_scroll_list_row_from_item(&row_blocks, selection_tbl.max.y)+1));
        EV_Row* row = rows.first;
        for(int64 y = selection_tbl.min.y; y <= selection_tbl.max.y && row != 0; y += 1, row = row.next)
        {
          // rjf: unpack row info
          RD_WatchViewRowInfo row_info = rd_watch_view_row_info_from_row(row);
          RD_WatchViewRowKind row_kind = rd_watch_view_row_kind_from_flags_row_info(flags, row, &row_info);
            ...
            evt.delta_2s32.y == 0))
        {
          ...
          for(int64 y = selection_tbl.min.y; y <= selection_tbl.max.y; y += 1, row = row.next)
          {
            ...
            for(int64 x = selection_tbl.min.x; x <= selection_tbl.max.x; x += 1)
            {
              ...
              StringView str = StringView(edit_state.input_buffer, edit_state.input_size);
              UI_TxtOp op = ui_single_line_txt_op_from_event(scratch.arena, evt, str, edit_state.cursor, edit_state.mark);
              ...
              {
                take_autocomplete = 1;
                StringView word_query = rd_autocomp_query_word_from_input_string_off(str, edit_state.cursor.column-1);
                uint64 word_off = (uint64)(word_query.Ptr - str.Ptr);
                StringView new_string = ui_push_string_replace_range(scratch.arena, str, r1s64(word_off+1, word_off+1+word_query.size), autocomplete_hint_string);
                new_string.size = Min(sizeof(edit_state.input_buffer), new_string.size);
                MemoryCopy(edit_state.input_buffer, new_string.Ptr, new_string.size);
                edit_state.input_size = new_string.size;
                edit_state.cursor = edit_state.mark = txt_pt(1, word_off+1+autocomplete_hint_string.size);
                str = StringView(edit_state.input_buffer, edit_state.input_size);
                op = ui_single_line_txt_op_from_event(scratch.arena, evt, str, edit_state.cursor, edit_state.mark);
              }
              
              // rjf: cancel? . revert to initial str
              if(editing_complete && evt.slot == UI_EventActionSlot_Cancel)
              {
                str = StringView(edit_state.initial_buffer, edit_state.initial_size);
              }
              
              // rjf: obtain edited str
              StringView new_string = str;
              if(!txt_pt_match(op.range.min, op.range.max) || op.replace.size != 0)
              {
                new_string = ui_push_string_replace_range(scratch.arena, str, r1s64(op.range.min.column, op.range.max.column), op.replace);
              }
      ...
      if(!ewv.text_editing && evt.flags & UI_EventFlag_Copy)
      {
        ...
        for(int64 y = selection_tbl.min.y; y <= selection_tbl.max.y && row != 0; y += 1, row = row.next)
        {
          ...
          {
            RD_WatchViewColumn* col = rd_watch_view_column_from_x(ewv, x);
            StringView cell_string = rd_string_from_eval_viz_row_column(scratch.arena, eval_view, row, col, string_flags|EV_StringFlag_ReadOnlyDisplayRules, default_radix, ui_top_font(), ui_top_font_size(), row_string_max_size_px);
        ...
        StringView str = str8_list_join(scratch.arena, &strs, 0);
          ...
          for(int64 x = selection_tbl.min.x; x <= selection_tbl.max.x; x += 1)
          {
               ...
               row_info.collection_entity_kind != RD_EntityKind_Nil)
            {
              ...
              if(!rd_entity_is_nil(entity))
              {
                ...
                uint64 deleted_id = row.key.child_id;
                uint64 deleted_num = row.block.expand_view_rule_info.expr_expand_num_from_id(deleted_id, row.block.expand_view_rule_info_user_data);
                if(deleted_num != 0)
                {
                  uint64 fallback_id_next = row.block.expand_view_rule_info.expr_expand_id_from_num(deleted_num+1, row.block.expand_view_rule_info_user_data);
                  uint64 fallback_id_prev = row.block.expand_view_rule_info.expr_expand_id_from_num(deleted_num-1, row.block.expand_view_rule_info_user_data);
            ...
            else if(tbl.y != 0 && col.kind == RD_WatchViewColumnKind_ViewRule && row_kind == RD_WatchViewRowKind_Normal)
            {
              ...
              ev_key_set_view_rule(eval_view, row.key, StringView());
            }
            else if(tbl.y != 0 && (col.kind == RD_WatchViewColumnKind_Value || col.kind == RD_WatchViewColumnKind_Member) && row_kind == RD_WatchViewRowKind_Normal)
            {
              E_Expr* expr = rd_expr_from_watch_view_row_column(scratch.arena, eval_view, row, col);
              E_Eval dst_eval = e_eval_from_expr(scratch.arena, expr);
              rd_commit_eval_value_string(dst_eval, StringView(), 0);
            }
          }
        }
        ...
        for(EV_Row* row = rows.first; row != 0; row = row.next, global_row_idx += 1)
        {
          ...
          RD_RegsScope(.module = row_module.handle) UI_Parent(row_box) switch(row_kind)
          {
            ...
            ProfScope("header row")
            {
              UI_FlagsAdd(UI_BoxFlag_DrawTextWeak)
              {
                for(RD_WatchViewColumn* col = ewv.first_column; col != 0; col = col.next)
                  UI_TableCell
                {
                  StringView name = StringView(col.display_string_buffer, col.display_string_size);
                  if(name.size == 0)
                  {
                    switch(col.kind)
                    {
                      ...
                      case RD_WatchViewColumnKind_Member:
                      {
                        name = StringView(col.string_buffer, col.string_size);
                      }break;
                    }
                  }
                  switch(col.kind)
            ...
            ProfScope("canvas row") UI_FocusHot(row_selected ? UI_FocusKind_On : UI_FocusKind_Off)
            {
              ...
              StringView canvas_view_expr = e_string_from_expr(scratch.arena, row.expr);
              B32 need_new_spec = (!str8_match(StringView(canvas_view.query_buffer, canvas_view.query_string_size), canvas_view_expr, 0) ||
              ...
              UI_Flags(UI_BoxFlag_DrawDropShadow) UI_CornerRadius(ui_top_font_size()*0.5f)
              {
                ...
                if(ui_clicked(sig))
                {
                  rd_cmd(RD_CmdKind_OpenTab,
                         .str      = e_string_from_expr(scratch.arena, row.expr),
              ...
              UI_Parent(canvas_box) UI_WidthFill UI_HeightFill
              {
                ...
                rd_push_regs();
                {
                  ...
                  rd_regs().file_path = rd_file_path_from_eval_string(rd_frame_arena(), StringView(canvas_view.query_buffer, canvas_view.query_string_size));
                }
                
                //- rjf: build
                UI_PermissionFlags(UI_PermissionFlag_Clicks|UI_PermissionFlag_ScrollX)
                {
                  ui_view_rule_info.ui(StringView(canvas_view.query_buffer, canvas_view.query_string_size), canvas_view.params_roots[canvas_view.params_read_gen%ArrayCount(canvas_view.params_roots)], canvas_rect);
                }
            ...
            ProfScope("pretty entity controls row")
            {
              ...
              for(uint64 idx = 0; idx < row_depth; idx += 1)
              {
              ...
              {
                ui_set_next_focus_hot(row_selected ? UI_FocusKind_On : UI_FocusKind_Off);
                if(ui_clicked(rd_cmd_spec_button(rd_cmd_kind_info_table[RD_CmdKind_AddTarget].str)))
                {
                  rd_cmd(RD_CmdKind_RunCommand, .cmd_name = rd_cmd_kind_info_table[RD_CmdKind_AddTarget].str);
                }
              }
                ...
                UI_Palette(palette)
              {
                ...
                if(ui_clicked(rd_cmd_spec_button(rd_cmd_kind_info_table[RD_CmdKind_AddAddressBreakpoint].str)))
                {
                  rd_cmd(RD_CmdKind_RunCommand, .cmd_name = rd_cmd_kind_info_table[RD_CmdKind_AddAddressBreakpoint].str);
                }
                ui_set_next_focus_hot(row_selected && selection_tbl.min.x == 2 ? UI_FocusKind_On : UI_FocusKind_Off);
                if(ui_clicked(rd_cmd_spec_button(rd_cmd_kind_info_table[RD_CmdKind_AddFunctionBreakpoint].str)))
                {
                  rd_cmd(RD_CmdKind_RunCommand, .cmd_name = rd_cmd_kind_info_table[RD_CmdKind_AddFunctionBreakpoint].str);
                }
              }
              ...
              {
                ui_set_next_focus_hot(row_selected && selection_tbl.min.x == 1 ? UI_FocusKind_On : UI_FocusKind_Off);
                if(ui_clicked(rd_cmd_spec_button(rd_cmd_kind_info_table[RD_CmdKind_AddWatchPin].str)))
                {
                  rd_cmd(RD_CmdKind_RunCommand, .cmd_name = rd_cmd_kind_info_table[RD_CmdKind_AddWatchPin].str);
                }
              }
              ...
              if(!rd_entity_is_nil(entity) || ctrl_entity != &ctrl_entity_nil)
              {
                ...
                StringView fstrs_string = dr_string_from_fancy_string_list(scratch.arena, &fstrs);
                    ...
                    if(ui_double_clicked(sig))
                    {
                         ...
                         entity.kind == RD_EntityKind_WatchPin)
                      {
                        ...
                        rd_cmd(RD_CmdKind_FindCodeLocation,
                               .file_path = (loc.flags & RD_EntityFlag_HasTextPoint) ? loc.str : StringView(),
            ...
            ProfScope("normal row") UI_HeightFill
            {
              ...
              {
                uint64 row_offset = row_eval.value.u64;
              ...
              if(!(flags & RD_WatchViewFlag_DisableCacheLines))
              {
                if((row_eval.mode == E_Mode_Offset || row_eval.mode == E_Mode_Null) &&
                   ...
                   !row_expanded)
                {
                  uint64 next_off = (row_eval.value.u64 + e_type_byte_size_from_key(row_eval.type_key));
              ...
              ProfScope("build all columns")
              {
                ...
                for(RD_WatchViewColumn* col = ewv.first_column; col != 0; col = col.next, x += 1)
                {
                  ...
                  StringView cell_pre_edit_string = rd_string_from_eval_viz_row_column(scratch.arena, eval_view, row, col, string_flags|EV_StringFlag_ReadOnlyDisplayRules, default_radix, ui_top_font(), ui_top_font_size(), row_string_max_size_px);
                  ...
                  StringView cell_inheritance_string = {0};
                  StringView cell_error_string = {0};
                  StringView cell_error_tooltip_string = {0};
                  ...
                  switch(col.kind)
                  {
                    ...
                    case RD_WatchViewColumnKind_Expr:
                    {
                      ...
                      if(row.member != 0 && row.member.inheritance_key_chain.first != 0)
                      {
                        ...
                        {
                          StringView inherited_type_name = e_type_string_from_key(scratch.arena, n.v);
                        ...
                        if(inheritance_chain_type_names.node_count != 0)
                        {
                          StringJoin join = {0};
                          join.sep = ("::");
                          StringView inheritance_type = str8_list_join(scratch.arena, &inheritance_chain_type_names, &join);
                    ...
                    value_cell:;
                    {
                      ...
                      if(row_depth == 0 && row.str.Length != 0)
                      {
                        E_TokenArray tokens = e_token_array_from_text(scratch.arena, row.str);
                        E_Parse parse = e_parse_expr_from_text_tokens(scratch.arena, row.str, &tokens);
                      ...
                      {
                        EV_ViewRule* vr = &n.v;
                        RD_ViewRuleInfo* info = rd_view_rule_info_from_string(vr.root.str);
                    ...
                    case RD_WatchViewColumnKind_ViewRule:
                    {
                      ...
                      if(cell_pre_edit_string.size == 0)
                      {
                        ...
                        {
                          str8_list_push(scratch.arena, &strings, n.v.root.str);
                        }
                  ...
                  if(col.view_rule_size != 0)
                  {
                    StringView col_view_rule = StringView(col.view_rule_buffer, col.view_rule_size);
                    ...
                    UI_FlagsAdd(row_depth > 0 ? UI_BoxFlag_DrawTextWeak : 0)
                  {
                    ...
                    else if(cell_ui_hook != 0)
                    {
                      ...
                      UI_Parent(box)
                      {
                        StringView row_expr = e_string_from_expr(scratch.arena, row.expr);
                    ...
                    else
                    {
                      ...
                      {
                        StringView input = StringView(cell_edit_state.input_buffer, cell_edit_state.input_size);
                  ...
                  //- rjf: handle interactions
                  {
                    ...
                    if(ui_double_clicked(sig) || sig.f & UI_SignalFlag_KeyboardPressed)
                    {
                              ...
                              cell_eval.space.kind == RD_EvalSpaceKind_CtrlEntity)
                      {
                        ...
                        {
                          uint64 vaddr = cell_eval.value.u64;
                          CTRL_Entity* module = ctrl_module_from_process_vaddr(process, vaddr);
                          DI_Key dbgi_key = ctrl_dbgi_key_from_module(module);
                          uint64 voff = ctrl_voff_from_vaddr(module, vaddr);
                    ...
                    // rjf: hovering with inheritance str . show tooltip
                  ...
                  {
                    static char* spaces = "                                                                        ";
                    StringView         str      = ev_expr_string_from_row(scratch.arena, row, 0);
                    E_TokenArray    tokens      = e_token_array_from_text(scratch.arena, str);
                    E_Parse         parse       = e_parse_expr_from_text_tokens(scratch.arena, str, &tokens);
                    ...
                    StringView         bytecode    = e_bytecode_from_oplist(scratch.arena, &oplist);
                    UI_Flags(UI_BoxFlag_DrawTextWeak) ui_labelf("Text:");
                    ui_label(str);
                    ui_spacer(ui_em(2.f, 1.f));
                    UI_Flags(UI_BoxFlag_DrawTextWeak) ui_labelf("Tokens:");
                    for(uint64 idx = 0; idx < tokens.count; idx += 1)
                    {
                      ui_labelf("%S: '%S'", e_token_kind_strings[tokens.v[idx].kind], str8_substr(str, tokens.v[idx].range));
                    }
                    ui_spacer(ui_em(2.f, 1.f));
                    UI_Flags(UI_BoxFlag_DrawTextWeak) ui_labelf("Expression:");
                    {
                      ...
                      for(Task* t = first_task; t != 0; t = t.next)
                      {
                        ...
                        switch(t.expr.kind)
                        {
                          default:
                          {
                            if(t.expr.str.Length != 0)
                            {
                              ext = push_str8f(scratch.arena, "'%S'", t.expr.str);
                            }
...
#undef X
                    ...
                    {
                      for(uint64 idx = 0; idx < bytecode.size; idx += 1)
                      {
                        ui_labelf("  0x%x ('%c')", (uint32)bytecode[idx], (char)bytecode[idx]);
                      }
                    }
                  }
                }
...
RD_VIEW_RULE_UI_FUNCTION_DEF(getting_started)
{
    ...
    UI_Padding(ui_pct(1, 0)) UI_Focus(UI_FocusKind_Null)
  {
    ...
    if(processes.count == 0)
    {
      ...
      switch(targets.count)
      {
        ...
        case 1:
        {
          ...
          StringView target_full_path = target.str;
    ...
    {
      ui_labelf("use");
      UI_TextAlignment(UI_TextAlign_Center) rd_cmd_binding_buttons(rd_cmd_kind_info_table[RD_CmdKind_RunCommand].str);
...
rd_cmd_lister_item_list_from_needle(Arena* arena, StringView needle)
{
  Temp scratch = scratch_begin(&arena, 1);
  ...
  for EachNonZeroEnumVal(RD_CmdKind, k)
  {
    ...
    {
      StringView cmd_display_name = info.display_name;
      StringView cmd_desc = info.description;
      StringView cmd_tags = info.search_tags;
         ...
         name_matches.needle_part_count == 0)
      {
        ...
        node.item.cmd_name = info.str;
        node.item.registrar_idx = (uint64)k;
        node.item.ordering_idx = (uint64)k;
...
RD_VIEW_RULE_UI_FUNCTION_DEF(commands)
{
  ...
  {
    uint64 selected_cmd_hash;
  };
  UI_ScrollPt2 scroll_pos = rd_view_scroll_pos();
  RD_CmdsViewState* cv = rd_view_state(RD_CmdsViewState);
  
  //- rjf: build filtered array of commands
  RD_CmdListerItemList cmd_list = rd_cmd_lister_item_list_from_needle(scratch.arena, str);
  ...
  {
    rd_cmd(RD_CmdKind_CompleteQuery, .cmd_name = (cmd_array.count > 0 ? cmd_array.v[0].cmd_name : StringView()));
  }
  
  //- rjf: selected kind . cursor
  Vec2S64 cursor = {0};
  {
    for(uint64 idx = 0; idx < cmd_array.count; idx += 1)
    {
      if(d_hash_from_string(cmd_array.v[idx].cmd_name) == cv.selected_cmd_hash)
      {
        cursor.y = (int64)idx+1;
        break;
      }
    }
  }
  ...
  {
    //- rjf: build buttons
    for(int64 row_idx = visible_row_range.min;
        ...
        row_idx += 1)
    {
      ...
      UI_Parent(box) UI_PrefHeight(ui_em(1.65f, 1.f))
      {
        ...
        UI_Column UI_Padding(ui_pct(1, 0))
        {
          ...
          StringView cmd_display_name = info.display_name;
          StringView cmd_desc = info.description;
          UI_Box* name_box = ui_build_box_from_stringf(UI_BoxFlag_DrawText, "%S##name_%S", cmd_display_name, info.str);
          UI_Box* desc_box = &ui_nil_box;
          UI_PrefHeight(ui_em(1.8f, 1.f)) UI_FlagsAdd(UI_BoxFlag_DrawTextWeak)
          {
            desc_box = ui_build_box_from_stringf(UI_BoxFlag_DrawText, "%S##desc_%S", cmd_desc, info.str);
          }
...
RD_PathQuery
rd_path_query_from_string(StringView str)
{
  StringView dir_str_in_input = {0};
  for(uint64 i = 0; i < str.Length; i += 1)
  {
    StringView substr1 = str8_substr(str, r1u64(i, i+1));
    StringView substr2 = str8_substr(str, r1u64(i, i+2));
    StringView substr3 = str8_substr(str, r1u64(i, i+3));
    if(str8_match(substr1, ("/"), StringMatchFlag_SlashInsensitive))
    {
      dir_str_in_input = str8_substr(str, r1u64(i, str.Length));
    }
    else if(i != 0 && str8_match(substr2, (":/"), StringMatchFlag_SlashInsensitive))
    {
      dir_str_in_input = str8_substr(str, r1u64(i-1, str.Length));
    }
  ...
  if(dir_str_in_input.size != 0)
  {
    ...
    path_query.prefix = str8_substr(str, r1u64(0, path_query.path.Ptr - str.Ptr));
  }
  return path_query;
}
...
{
  return strncmp((char *)a.filename.Ptr, (char *)b.filename.Ptr, Min(a.filename.size, b.filename.size));
}
...
RD_VIEW_RULE_UI_FUNCTION_DEF(file_system)
{
  ProfBeginFunction();
  Temp scratch = scratch_begin(0, 0);
  StringView query = str;
  StringView query_normalized = path_normalized_from_string(scratch.arena, query);
  B32 query_has_slash = (query.size != 0 && char_to_correct_slash(query[query.size-1]) == '/');
  StringView query_normalized_with_opt_slash = push_str8f(scratch.arena, "%S%s", query_normalized, query_has_slash ? "/" : "");
  ...
  {
    StringView key = query_normalized;
    uint64 hash = d_hash_from_string(key);
    uint64 slot = hash % fs.path_state_table_size;
  ...
  uint64 file_count = fs.cached_file_count;
     ...
     fs.cached_files_sort_side != fs.sort_side)
  {
    ...
    if(fs.sort_kind != RD_FileSortKind_Null && fs.sort_side == Side_Max)
    {
      ...
      {
        uint64 rev_idx = new_file_count - idx - 1;
  ...
  if(ps.cursor.y == 0 && ui_slot_press(UI_EventActionSlot_Accept))
  {
    ...
    else if(query_normalized_with_opt_slash_props.created != 0 && path_query.search.size != 0)
    {
      ...
      {
        StringView new_path = push_str8f(scratch.arena, "%S%S/", path_query.path, path_query.search);
    ...
    {
      StringView filename = files[0].filename;
      if(files[0].props.flags & FilePropertyFlag_IsFolder)
      {
        StringView existing_path = str8_chop_last_slash(path_query.path);
        StringView new_path = push_str8f(scratch.arena, "%S/%S/", existing_path, files[0].filename);
        rd_store_view_filter(new_path);
      }
      else
      {
        StringView file_path = push_str8f(scratch.arena, "%S%S", path_query.path, filename);
  ...
  for(uint64 idx = 0; idx < ArrayCount(fs.col_pcts); idx += 1)
  {
    ...
    UI_Focus(UI_FocusKind_Null)
  {
    ...
    if(visible_row_range.min == 0)
    {
      ...
      if(ui_clicked(sig))
      {
        StringView new_path = str8_chop_last_slash(str8_chop_last_slash(path_query.path));
        new_path = path_normalized_from_string(scratch.arena, new_path);
        StringView new_cmd = push_str8f(scratch.arena, "%S%s", new_path, new_path.size != 0 ? "/" : "");
        rd_store_view_filter(new_cmd);
      }
    }
    
    // rjf: file buttons
    for(uint64 row_idx = Max(visible_row_range.min, 1);
        row_idx <= visible_row_range.max && row_idx <= file_count;
        row_idx += 1, row_num += 1)
    {
      uint64 file_idx = row_idx-1;
      ...
      UI_Parent(file_sig.box)
      {
          ...
          UI_FlagsAdd(UI_BoxFlag_DrawTextWeak)
        {
          ...
          StringView str = push_date_time_string(scratch.arena, &time_local);
          ui_label(str);
        }
      ...
      {
        StringView existing_path = str8_chop_last_slash(path_query.path);
        StringView new_path = push_str8f(scratch.arena, "%S%s%S/", existing_path, existing_path.size != 0 ? "/" : "", file.filename);
        new_path = path_normalized_from_string(scratch.arena, new_path);
        if(file.props.flags & FilePropertyFlag_IsFolder)
        {
          StringView new_cmd = push_str8f(scratch.arena, "%S%s", new_path, new_path.size != 0 ? "/" : "");
...
RD_ProcessInfoList
rd_process_info_list_from_query(Arena* arena, StringView query)
{
  Temp scratch = scratch_begin(&arena, 1);
  
  //- rjf: gather PIDs that we're currently attached to
  uint64 attached_process_count = 0;
  uint32* attached_process_pids = 0;
  {
    CTRL_EntityList processes = ctrl_entity_list_from_kind(d_state.ctrl_entity_store, CTRL_EntityKind_Process);
    attached_process_count = processes.count;
    attached_process_pids = push_array(scratch.arena, uint32, attached_process_count);
    uint64 idx = 0;
  ...
  RD_ProcessInfoList list = {0};
  {
    ...
    for(DMN_ProcessInfo info = {0}; dmn_process_iter_next(scratch.arena, &iter, &info);)
    {
      ...
      B32 is_attached = 0;
      for(uint64 attached_idx = 0; attached_idx < attached_process_count; attached_idx += 1)
      {
...
RD_VIEW_RULE_UI_FUNCTION_DEF(system_processes)
{
  ...
  Vec2S64 cursor = {0};
  {
    for(uint64 idx = 0; idx < process_info_array.count; idx += 1)
    {
  ...
  {
    //- rjf: build rows
    for(uint64 idx = visible_row_range.min;
...
rd_entity_lister_item_list_from_needle(Arena* arena, RD_EntityKind kind, RD_EntityFlags omit_flags, StringView needle)
{
  Temp scratch = scratch_begin(&arena, 1);
  ...
  for(RD_EntityNode* n = ent_list.first; n != 0; n = n.next)
  {
    ...
    if(!(entity.flags & omit_flags))
    {
      ...
      StringView title_string = dr_string_from_fancy_string_list(scratch.arena, &title_fstrs);
...
RD_VIEW_RULE_UI_FUNCTION_DEF(entity_lister)
{
  ...
  RD_EntityListerItemList ent_list = rd_entity_lister_item_list_from_needle(scratch.arena, entity_kind, 0, str);
  ...
  Vec2S64 cursor = {0};
  {
    for(uint64 idx = 0; idx < ent_arr.count; idx += 1)
    {
  ...
  UI_Focus(UI_FocusKind_On)
    ...
    UI_Focus(UI_FocusKind_Null)
  {
    for(int64 idx = visible_row_range.min;
...
rd_ctrl_entity_lister_item_list_from_needle(Arena* arena, CTRL_EntityKind kind, StringView needle)
{
  Temp scratch = scratch_begin(&arena, 1);
  ...
  for(CTRL_EntityNode* n = ent_list.first; n != 0; n = n.next)
  {
    ...
    StringView title_fstrs_text = dr_string_from_fancy_string_list(scratch.arena, &title_fstrs);
...
RD_VIEW_RULE_UI_FUNCTION_DEF(ctrl_entity_lister)
{
  ...
  //- rjf: build filtered array of entities
  RD_CtrlEntityListerItemList ent_list = rd_ctrl_entity_lister_item_list_from_needle(scratch.arena, entity_kind, str);
...
RD_VIEW_RULE_UI_FUNCTION_DEF(symbol_lister)
{
  ...
  B32 items_stale = 0;
  DI_SearchItemArray items = di_search_items_from_key_params_query(di_scope, search_key, &search_params, str, endt_us, &items_stale);
  if(items_stale)
  {
    rd_request_frame();
  }
  ...
  if(slv.cursor.y == 0 && items.count != 0 && ui_slot_press(UI_EventActionSlot_Accept))
  {
    ...
    if(item.dbgi_idx < rdis_count)
    {
      ...
      if(item.idx < rdi_procedures_count)
      {
        ...
        uint64 name_size = 0;
        uint8* name_base = rdi_string_from_idx(rdi, procedure.name_string_idx, &name_size);
        StringView name = StringView(name_base, name_size);
        if(name.size != 0)
        {
          rd_cmd(RD_CmdKind_CompleteQuery, .str = name);
        }
      }
    }
  }
    ...
    UI_TextRasterFlags(rd_raster_flags_from_slot(RD_FontSlot_Code))
  {
      ...
      UI_Focus((slv.cursor.y == idx+1) ? UI_FocusKind_On : UI_FocusKind_Off)
    {
      ...
      UI_Parent(box) UI_PrefWidth(ui_text_dim(10, 1)) RD_Font(RD_FontSlot_Code)
      {
        ...
        {
          StringView type_string = e_type_string_from_key(scratch.arena, type_key);
      ...
      if(ui_hovering(sig)) UI_Tooltip
      {
        ...
        uint64 binary_voff = d_voff_from_dbgi_key_symbol_name(&dbgi_key, name);
        ...
        {
          StringView file_path = lines.first.v.file_path;
          int64 line_num = lines.first.v.pt.line;
          RD_Font(RD_FontSlot_Main) UI_FlagsAdd(UI_BoxFlag_DrawTextWeak)
...
RD_VIEW_RULE_UI_FUNCTION_DEF(call_stack)
{
  ...
  if(!wv.initialized)
  {
    ...
    rd_watch_view_column_alloc(wv, RD_WatchViewColumnKind_Member,                  0.20f, .str = ("vaddr"), .display_string = ("Address"), .view_rule = ("cast:uint64"));
    rd_watch_view_column_alloc(wv, RD_WatchViewColumnKind_Module,                  0.25f, .str = ("module.Ptr"), .display_string = ("Module"), .dequote_string = 1, .is_non_code = 1);
  }
...
RD_VIEW_RULE_UI_FUNCTION_DEF(pending_file)
{
  ...
  StringView expr_string = rd_view_expr_string();
  StringView file_path = rd_file_path_from_eval_string(scratch.arena, expr_string);
  ...
  RD_ViewRuleKind viewer_kind = RD_ViewRuleKind_Text;
  {
    ...
    StringView data = hs_data_from_hash(hs_scope, file_hash);
    if(!u128_match(file_hash, u128_zero()))
    {
      uint64 num_utf8_bytes = 0;
      uint64 num_unknown_bytes = 0;
      for(uint64 idx = 0; idx < data.size && idx < file_range.max;)
      {
        UnicodeDecode decode = utf8_decode(data.Ptr+idx, data.size-idx);
  ...
  if(file_is_ready && viewer_kind != RD_ViewRuleKind_Null)
  {
    ...
    StringView query = rd_eval_string_from_file_path(scratch.arena, file_path);
    RD_View* view = rd_view_from_handle(rd_regs().view);
...
RD_VIEW_RULE_UI_FUNCTION_DEF(text)
{
  ...
  for(RD_Cmd* cmd = 0; rd_next_cmd(&cmd);)
  {
    ...
    switch(kind)
    {
      ...
      {
        StringView src = rd_file_path_from_eval_string(scratch.arena, rd_view_expr_string());
        StringView dst = cmd.regs.file_path;
        if(src.size != 0 && dst.size != 0)
        {
          // rjf: record src . dst mapping
          rd_cmd(RD_CmdKind_SetFileReplacementPath, .str = src, .file_path = dst);
  ...
  //
  StringView path = rd_file_path_from_eval_string(rd_frame_arena(), str);
  ...
  if(file_is_missing && !u128_match(hash, u128_zero()))
  {
    UI_WidthFill UI_HeightFill UI_Column UI_Padding(ui_pct(1, 0))
    {
      ...
      {
        rd_cmd(RD_CmdKind_RunCommand, .cmd_name = rd_cmd_kind_info_table[RD_CmdKind_PickFile].str);
      }
      scratch_end(scratch);
    }
  }
  ...
  StringView out_of_date_dbgi_name = {0};
  {
    uint64 file_timestamp = fs_timestamp_from_path(path);
    if(file_timestamp != 0)
    {
...
struct RD_DisasmViewState
{
  ...
  uint64 temp_look_vaddr;
  uint64 temp_look_run_gen;
  uint64 goto_vaddr;
  RD_CodeViewState cv;
};
...
RD_VIEW_RULE_UI_FUNCTION_DEF(disasm)
{
  ...
  if(str.Length == 0)
  {
    if(dv.temp_look_vaddr != 0 && dv.temp_look_run_gen == ctrl_run_gen())
    {
      ...
      str = push_str8f(scratch.arena, "(0x%I64x & (~(0x4000 - 1)))", dv.temp_look_vaddr);
    }
    else
    {
      auto_selected = 1;
  ...
  StringView dasm_text_data = hs_data_from_hash(hs_scope, dasm_text_hash);
  ...
  {
    uint64 vaddr = dv.goto_vaddr;
    uint64 line_idx = dasm_line_array_idx_from_code_off__linear_scan(&dasm_info.lines, vaddr-range.min);
    if(line_idx < dasm_info.lines.count)
    {
      int64 line_num = (int64)(line_idx+1);
      dv.goto_vaddr = 0;
  ...
  if(!is_loading && has_disasm)
  {
    uint64 off = dasm_line_array_code_off_from_idx(&dasm_info.lines, rd_regs().cursor.line-1);
  ...
  if(!is_loading && has_disasm)
  {
    ...
    UI_Row
      ...
      RD_Font(RD_FontSlot_Code)
    {
      uint64 cursor_vaddr = (1 <= rd_regs().cursor.line && rd_regs().cursor.line <= dasm_info.lines.count) ? (range.min+dasm_info.lines.v[rd_regs().cursor.line-1].code_off) : 0;
      if(dasm_module != &ctrl_entity_nil)
      {
        ui_labelf("%S", path_normalized_from_string(scratch.arena, dasm_module.str));
        ui_spacer(ui_em(1.5f, 1));
      }
...
RD_VIEW_RULE_UI_FUNCTION_DEF(memory)
{
  ...
  int64 num_possible_visible_rows = 0;
  {
    num_possible_visible_rows = dim_2f32(content_rect).y/row_height_px;
    viz_range_rows.min = scroll_pos.y.idx + (int64)scroll_pos.y.off - !!(scroll_pos.y.off<0);
    viz_range_rows.max = scroll_pos.y.idx + (int64)scroll_pos.y.off + num_possible_visible_rows,
    ...
    {
      Swap(uint64, viz_range_bytes.min, viz_range_bytes.max);
    }
  ...
  {
    uint64 next_cursor = cursor;
    uint64 next_mark = mark;
    for(UI_Event* evt = 0; ui_next_event(&evt);)
    {
      ...
      switch(evt.delta_unit)
      {
        ...
        case UI_EventDeltaUnit_Char:
        {
          cell_delta.x = (int64)evt.delta_2s32.x;
          cell_delta.y = (int64)evt.delta_2s32.y;
        }break;
        case UI_EventDeltaUnit_Word:
        case UI_EventDeltaUnit_Page:
        {
          ...
          {
            cell_delta.x = -(int64)(cursor%num_columns);
          }
          else if(evt.delta_2s32.x > 0)
          {
            cell_delta.x = (num_columns-1) - (int64)(cursor%num_columns);
          }
      ...
      if(good_action)
      {
        cell_delta.x = ClampBot(cell_delta.x, (int64)-next_cursor);
        cell_delta.y = ClampBot(cell_delta.y, (int64)-(next_cursor/num_columns));
  ...
  {
    mv.center_cursor = 0;
    int64 cursor_row_idx = cursor/num_columns;
    int64 new_idx = (cursor_row_idx-num_possible_visible_rows/2+1);
  ...
  if(mv.contain_cursor)
  {
    ...
    int64 min_delta = Min(0, cursor_viz_range.min-viz_range_rows.min);
    int64 max_delta = Max(0, cursor_viz_range.max-viz_range_rows.max);
    int64 new_idx = scroll_pos.y.idx+min_delta+max_delta;
    new_idx = clamp_1s64(scroll_idx_rng, new_idx);
    ui_scroll_pt_target_idx(&scroll_pos.y, new_idx);
  }
  ...
  //////////////////////////////
  //- rjf: produce fancy str runs for all possible byte values in all cells
  //
  DR_FancyStringList byte_fancy_strings[256] = {0};
  {
    ...
    for(uint64 idx = 0; idx < ArrayCount(byte_fancy_strings); idx += 1)
    {
      uint8 byte = (uint8)idx;
  ...
  //
  uint64 visible_memory_size = dim_1u64(viz_range_bytes);
  uint8* visible_memory = push_array(scratch.arena, uint8, visible_memory_size);
  {
  ...
  AnnotationList* visible_memory_annotations = push_array(scratch.arena, AnnotationList, visible_memory_size);
  {
    ...
    {
      uint64 last_stack_top = regs_rsp_from_arch_block(thread.arch, unwind.frames.v[0].regs);
      for(uint64 idx = 1; idx < unwind.frames.count; idx += 1)
      {
        CTRL_UnwindFrame* f = &unwind.frames.v[idx];
        uint64 f_stack_top = regs_rsp_from_arch_block(thread.arch, f.regs);
        ...
        {
          uint64 f_rip = regs_rip_from_arch_block(thread.arch, f.regs);
          CTRL_Entity* module = ctrl_module_from_process_vaddr(process, f_rip);
          DI_Key dbgi_key = ctrl_dbgi_key_from_module(module);
          uint64 rip_voff = ctrl_voff_from_vaddr(module, f_rip);
          StringView symbol_name = d_symbol_name_from_dbgi_key_voff(scratch.arena, &dbgi_key, rip_voff, 1);
          ...
          {
            uint64 visible_byte_idx = vaddr - viz_range_bytes.min;
    ...
    if(unwind.frames.count > 0)
    {
      uint64 stack_base_vaddr = thread.stack_base;
      uint64 stack_top_vaddr = regs_rsp_from_arch_block(thread.arch, unwind.frames.v[0].regs);
      ...
      {
        Annotation* annotation = push_array(scratch.arena, Annotation, 1);
        annotation.name_string = thread.str.Length ? thread.str : push_str8f(scratch.arena, "TID: %I64u", thread.id);
    ...
    //- rjf: fill local variable annotations
    {
      ...
      uint64 thread_rip_vaddr = d_query_cached_rip_from_thread_unwind(thread, rd_regs().unwind_count);
      for(E_String2NumMapNode* n = e_parse_ctx.locals_map.first; n != 0; n = n.order_next)
      {
        StringView local_name = n.str;
        ...
        if(local_eval.mode == E_Mode_Offset)
        {
          ...
          uint64 local_eval_type_size = e_type_byte_size_from_key(local_eval.type_key);
          ...
          if(vaddr_rng_in_visible.max != vaddr_rng_in_visible.min)
          {
            ...
            for(uint64 vaddr = vaddr_rng_in_visible.min; vaddr < vaddr_rng_in_visible.max; vaddr += 1)
            {
  ...
  UI_Parent(container_box)
  {
      ...
      UI_FlagsAdd(UI_BoxFlag_DrawTextWeak)
    {
        ...
        UI_TextAlignment(UI_TextAlign_Center)
      {
        ...
        for(uint64 row_off = 0; row_off < num_columns*bytes_per_cell; row_off += bytes_per_cell)
        {
  ...
  uint64 mouse_hover_byte_num = 0;
  {
    ...
    if(ui_hovering(sig) || ui_dragging(sig))
    {
      ...
      uint64 row_idx = ClampBot(0, mouse_rel.y) / row_height_px;
      
      // rjf: try from cells
      if(mouse_hover_byte_num == 0)
      {
        uint64 col_idx = ClampBot(mouse_rel.x-big_glyph_advance*20.f, 0)/cell_width_px;
      ...
      if(mouse_hover_byte_num == 0)
      {
        uint64 col_idx = ClampBot(mouse_rel.x - (big_glyph_advance*20.f + cell_width_px*num_columns + big_glyph_advance*1.5f), 0)/big_glyph_advance;
  ...
  UI_Parent(row_container_box) RD_Font(RD_FontSlot_Code) UI_FontSize(font_size)
  {
    ...
    uint8* row_ascii_buffer = push_array(scratch.arena, uint8, num_columns);
    UI_WidthFill UI_PrefHeight(ui_px(row_height_px, 1.f))
      for(int64 row_idx = viz_range_rows.min; row_idx <= viz_range_rows.max; row_idx += 1)
    {
      ...
      UI_Parent(row)
      {
        ...
        UI_PrefWidth(ui_px(cell_width_px, 1.f))
          ...
          UI_CornerRadius(0)
        {
          for(uint64 col_idx = 0; col_idx < num_columns; col_idx += 1)
          {
            // rjf: unpack information about this slot
            uint64 visible_byte_idx = (row_idx-viz_range_rows.min)*num_columns + col_idx;
            uint64 global_byte_idx = viz_range_bytes.min+visible_byte_idx;
            uint64 global_byte_num = global_byte_idx+1;
            ...
            else
            {
              ...
              uint8 byte_value = visible_memory[visible_byte_idx];
        ...
        UI_WidthFill
        {
          ...
          StringView ascii_text = StringView(row_ascii_buffer, num_bytes_this_row);
    ...
    UI_Parent(footer_box) RD_Font(RD_FontSlot_Code) UI_FontSize(font_size)
    {
        ...
        UI_PrefHeight(ui_px(row_height_px, 0.f))
      {
        ...
        if(cursor_in_range)
        {
          ...
          uint64 as_u32 = 0;
          uint64 as_u64 = 0;
          uint64 cursor_off = cursor-viz_range_bytes.min;
          as_u8  = (uint64)*(uint8 *)(visible_memory + cursor_off);
          as_u16 = (uint64)*(uint16*)(visible_memory + cursor_off);
          as_u32 = (uint64)*(uint32*)(visible_memory + cursor_off);
          as_u64 = (uint64)*(uint64*)(visible_memory + cursor_off);
...
RD_VIEW_RULE_UI_FUNCTION_DEF(bitmap)
{
  ...
  uint64 base_offset = rd_base_offset_from_eval(eval);
  uint64 expected_size = dim.x*dim.y*r_tex2d_format_bytes_per_pixel_table[fmt];
  ...
  StringView data = hs_data_from_hash(hs_scope, data_hash);
  ...
  if(ui_hovering(canvas_sig) && !ui_dragging(canvas_sig))
  {
    ...
    {
      mouse_bmp = v2s32((int32)(mouse_cvs.x-img_rect_cvs.x0), (int32)(mouse_cvs.y-img_rect_cvs.y0));
      int64 off_px = mouse_bmp.y*topology.dim.x + mouse_bmp.x;
      int64 off_bytes = off_px*r_tex2d_format_bytes_per_pixel_table[topology.fmt];
         ...
         r_tex2d_format_bytes_per_pixel_table[topology.fmt] != 0)
      {
        ...
        switch(topology.fmt)
        {
          ...
          case R_Tex2DFormat_R8:     {color = v4f32(((uint8 *)(data.Ptr+off_bytes))[0]/255.f, 0, 0, 1);}break;
          case R_Tex2DFormat_RG8:    {color = v4f32(((uint8 *)(data.Ptr+off_bytes))[0]/255.f, ((uint8 *)(data.Ptr+off_bytes))[1]/255.f, 0, 1);}break;
          case R_Tex2DFormat_RGBA8:  {color = v4f32(((uint8 *)(data.Ptr+off_bytes))[0]/255.f, ((uint8 *)(data.Ptr+off_bytes))[1]/255.f, ((uint8 *)(data.Ptr+off_bytes))[2]/255.f, ((uint8 *)(data.Ptr+off_bytes))[3]/255.f);}break;
          case R_Tex2DFormat_BGRA8:  {color = v4f32(((uint8 *)(data.Ptr+off_bytes))[3]/255.f, ((uint8 *)(data.Ptr+off_bytes))[2]/255.f, ((uint8 *)(data.Ptr+off_bytes))[1]/255.f, ((uint8 *)(data.Ptr+off_bytes))[0]/255.f);}break;
          case R_Tex2DFormat_R16:    {color = v4f32(((uint16 *)(data.Ptr+off_bytes))[0]/(float)max_U16, 0, 0, 1);}break;
          case R_Tex2DFormat_RGBA16: {color = v4f32(((uint16 *)(data.Ptr+off_bytes))[0]/(float)max_U16, ((uint16 *)(data.Ptr+off_bytes))[1]/(float)max_U16, ((uint16 *)(data.Ptr+off_bytes))[2]/(float)max_U16, ((uint16 *)(data.Ptr+off_bytes))[3]/(float)max_U16);}break;
          case R_Tex2DFormat_R32:    {color = v4f32(((float *)(data.Ptr+off_bytes))[0], 0, 0, 1);}break;
          case R_Tex2DFormat_RG32:   {color = v4f32(((float *)(data.Ptr+off_bytes))[0], ((float *)(data.Ptr+off_bytes))[1], 0, 1);}break;
          case R_Tex2DFormat_RGBA32: {color = v4f32(((float *)(data.Ptr+off_bytes))[0], ((float *)(data.Ptr+off_bytes))[1], ((float *)(data.Ptr+off_bytes))[2], ((float *)(data.Ptr+off_bytes))[3]);}break;
        }
        if(color_is_good)
        {
...
rd_rgba_from_eval_params(E_Eval eval, MD_Node* params)
{
  Vec4F32 rgba = {0};
  {
    ...
    uint64 type_size = e_type_byte_size_from_key(type_key);
    if(16 <= type_size)
    {
    ...
    {
      uint32 hex_val = value_eval.value.u32;
...
RD_VIEW_RULE_UI_FUNCTION_DEF(color_rgba)
{
  ...
  else
  {
    UI_WidthFill UI_HeightFill UI_Column UI_Padding(ui_px(padding, 1.f)) UI_Row UI_Padding(ui_pct(1.f, 0.f)) UI_HeightFill
    {
      ...
      {
        StringView hex_string = hex_string_from_rgba_4f32(scratch.arena, rgba);
        ui_label(hex_string);
        ui_labelf("%.2f", rgba.x);
        ui_labelf("%.2f", rgba.y);
        ui_labelf("%.2f", rgba.z);
...
RD_VIEW_RULE_UI_FUNCTION_DEF(geo3d)
{
  ...
  E_Eval eval = e_eval_from_string(scratch.arena, str);
  uint64 base_offset = rd_base_offset_from_eval(eval);
  Rng1U64 idxs_range = r1u64(base_offset, base_offset+count*sizeof(uint32));
...
RD_VIEW_RULE_UI_FUNCTION_DEF(exception_filters)
{
      ...
      k = (CTRL_ExceptionCodeKind)(k+1))
  {
    ...
    StringView name = push_str8f(scratch.arena, "0x%x %S", ctrl_exception_code_kind_code_table[k], ctrl_exception_code_kind_display_string_table[k]);
  ...
  UI_Focus(UI_FocusKind_On)
    ...
    UI_Focus(UI_FocusKind_Null)
  {
    for(int64 row = visible_row_range.min; row <= visible_row_range.max && row < opts.count; row += 1)
...
RD_VIEW_RULE_UI_FUNCTION_DEF(settings)
{
  ...
  RD_SettingsItemArray items = {0};
  {
    ...
    if(query.size == 0)
    {
      ...
      n.v.str = ("Global Interface Settings");
    ...
    if(sv.category_opened[RD_SettingsItemKind_GlobalSetting] || query.size != 0)
    {
      for EachEnumVal(RD_SettingCode, code)
      {
        ...
        StringView kind_string = ("Global Interface Setting");
        StringView str = rd_setting_code_display_string_table[code];
        FuzzyMatchRangeList kind_string_matches = fuzzy_match_find(scratch.arena, query, kind_string);
        FuzzyMatchRangeList string_matches = fuzzy_match_find(scratch.arena, query, str);
    ...
    if(query.size == 0)
    {
      ...
      items_list.count += 1;
      n.v.kind = RD_SettingsItemKind_CategoryHeader;
      n.v.str = ("Window Interface Settings");
    ...
    if(sv.category_opened[RD_SettingsItemKind_WindowSetting] || query.size != 0)
    {
      for EachEnumVal(RD_SettingCode, code)
      {
        ...
        StringView kind_string = ("Window Interface Setting");
        StringView str = rd_setting_code_display_string_table[code];
        FuzzyMatchRangeList kind_string_matches = fuzzy_match_find(scratch.arena, query, kind_string);
        FuzzyMatchRangeList string_matches = fuzzy_match_find(scratch.arena, query, str);
        if(string_matches.count == string_matches.needle_part_count ||
    ...
    if(sv.category_opened[RD_SettingsItemKind_ThemePreset] || query.size != 0)
    {
      ...
      {
        StringView kind_string = ("Theme Preset");
        StringView str = rd_theme_preset_display_string_table[preset];
        FuzzyMatchRangeList kind_string_matches = fuzzy_match_find(scratch.arena, query, kind_string);
        FuzzyMatchRangeList string_matches = fuzzy_match_find(scratch.arena, query, str);
        if(string_matches.count == string_matches.needle_part_count ||
           kind_string_matches.count == kind_string_matches.needle_part_count)
    ...
    if(sv.category_opened[RD_SettingsItemKind_ThemeColor] || query.size != 0)
    {
      ...
      {
        StringView kind_string = ("Theme Color");
        StringView str = rd_theme_color_display_string_table[color];
        FuzzyMatchRangeList kind_string_matches = fuzzy_match_find(scratch.arena, query, kind_string);
        FuzzyMatchRangeList string_matches = fuzzy_match_find(scratch.arena, query, str);
        if(string_matches.count == string_matches.needle_part_count ||
           kind_string_matches.count == kind_string_matches.needle_part_count)
  ...
  UI_Key* color_ctx_menu_keys = push_array(scratch.arena, UI_Key, RD_ThemeColor_COUNT);
  {
    ...
    {
      color_ctx_menu_keys[color] = ui_key_from_stringf(ui_key_zero(), "###settings_color_ctx_menu_%I64x", (uint64)color);
    }
  }
      ...
      color = (RD_ThemeColor)(color+1))
  {
      ...
      UI_PrefWidth(ui_em(28.5f, 1)) UI_PrefHeight(ui_children_sum(1.f))
    {
        ...
        UI_PrefHeight(ui_em(2.25f, 1.f))
      {
        ...
        StringView r_string = push_str8f(scratch.arena, "%.2f", rgba.x);
        StringView g_string = push_str8f(scratch.arena, "%.2f", rgba.y);
        StringView b_string = push_str8f(scratch.arena, "%.2f", rgba.z);
        StringView h_string = push_str8f(scratch.arena, "%.2f", hsva.x);
        StringView s_string = push_str8f(scratch.arena, "%.2f", hsva.y);
        StringView v_string = push_str8f(scratch.arena, "%.2f", hsva.z);
        StringView a_string = push_str8f(scratch.arena, "%.2f", rgba.w);
        UI_Row RD_Font(RD_FontSlot_Code)
        {
          ...
          if(ui_committed(sig))
          {
            StringView str = StringView(sv.txt_buffer, sv.txt_size);
            Vec4F32 new_rgba = rgba_from_hex_string_4f32(str);
        ...
        UI_Row
        {
          ...
          {
            StringView str = StringView(sv.txt_buffer, sv.txt_size);
            Vec4F32 new_rgba = v4f32((float)f64_from_str8(str), rgba.y, rgba.z, rgba.w);
            Vec4F32 new_hsva = hsva_from_rgba(new_rgba);
            sv.color_ctx_menu_color_hsva = new_hsva;
          }
        }
        UI_Row
        {
          ...
          {
            StringView str = StringView(sv.txt_buffer, sv.txt_size);
            Vec4F32 new_rgba = v4f32(rgba.x, (float)f64_from_str8(str), rgba.z, rgba.w);
            Vec4F32 new_hsva = hsva_from_rgba(new_rgba);
            sv.color_ctx_menu_color_hsva = new_hsva;
          }
        }
        UI_Row
        {
          ...
          {
            StringView str = StringView(sv.txt_buffer, sv.txt_size);
            Vec4F32 new_rgba = v4f32(rgba.x, rgba.y, (float)f64_from_str8(str), rgba.w);
            Vec4F32 new_hsva = hsva_from_rgba(new_rgba);
            sv.color_ctx_menu_color_hsva = new_hsva;
          }
        }
        ...
        UI_Row
        {
          ...
          {
            StringView str = StringView(sv.txt_buffer, sv.txt_size);
            Vec4F32 new_hsva = v4f32((float)f64_from_str8(str), hsva.y, hsva.z, hsva.w);
            sv.color_ctx_menu_color_hsva = new_hsva;
          }
        }
        UI_Row
        {
          ...
          {
            StringView str = StringView(sv.txt_buffer, sv.txt_size);
            Vec4F32 new_hsva = v4f32(hsva.x, (float)f64_from_str8(str), hsva.z, hsva.w);
            sv.color_ctx_menu_color_hsva = new_hsva;
          }
        }
        UI_Row
        {
          ...
          {
            StringView str = StringView(sv.txt_buffer, sv.txt_size);
            Vec4F32 new_hsva = v4f32(hsva.x, hsva.y, (float)f64_from_str8(str), hsva.w);
            sv.color_ctx_menu_color_hsva = new_hsva;
          }
        }
        ui_spacer(ui_em(0.75f, 1.f));
        UI_Row
        {
          ...
          {
            StringView str = StringView(sv.txt_buffer, sv.txt_size);
            Vec4F32 new_hsva = v4f32(hsva.x, hsva.y, hsva.z, (float)f64_from_str8(str));
            sv.color_ctx_menu_color_hsva = new_hsva;
          }
        }
      }
  ...
  UI_Focus(UI_FocusKind_On)
    ...
    UI_Focus(UI_FocusKind_Null)
  {
    for(int64 row_num = visible_row_range.min; row_num <= visible_row_range.max && row_num < items.count; row_num += 1)
    {
      //- rjf: unpack item
      ...
      UI_Row
      {
        ...
        UI_Focus(row_num+1 == sv.cursor.y ? UI_FocusKind_On : UI_FocusKind_Off) UI_Palette(palette)
        {
          ...
          item_box = ui_build_box_from_stringf(UI_BoxFlag_Clickable|flags, "###option_%S_%S", item.kind_string, item.str);
          UI_Parent(item_box)
          {
            ...
            UI_PrefWidth(ui_text_dim(10, 1))
            {
              UI_Box* box = ui_build_box_from_stringf(UI_BoxFlag_DrawText, "%S", item.str);
         ...
         is_slider && ui_dragging(sig))
      {
        ...
        int32 pre_drag_val = *ui_get_drag_struct(int32);
        Vec2F32 delta = ui_drag_delta();
        int32 pst_drag_val = pre_drag_val + (int32)(delta.x/(ui_top_font_size()*2.f));
