...
E_TYPE_EXPAND_INFO_FUNCTION_DEF(commands)
{
  E_TypeExpandInfo result = {0};
  {
    ...
    String8Array *accel = new String8Array[1];
...
{
  RD_SchemaIRExt *ext = new RD_SchemaIRExt[1];
  {
    Temp scratch = scratch_begin(&arena, 1);
...
E_TYPE_EXPAND_INFO_FUNCTION_DEF(schema)
{
  E_TypeExpandInfo result = {0};
  {
        ...
        if(!md_node_has_tag(child, str8_lit("no_expand"), 0))
        {
          ...
          if(expand_this_child)
          {
            ...
            if(name_matches.count == name_matches.needle_part_count ||
               ...
               desc_matches.count == desc_matches.needle_part_count)
            {
              ExpandChildNode *n = new ExpandChildNode[1];
              n->n = child;
    ...
    MD_Node **children = new MD_Node *[child_count];
    {
      U64 idx = 0;
    ...
    RD_SchemaExpandAccel *accel = new RD_SchemaExpandAccel[1];
...
{
  RD_CfgsIRExt *ext = new RD_CfgsIRExt[1];
  {
    Temp scratch = scratch_begin(&arena, 1);
...
{
  RD_CfgsExpandAccel *accel = new RD_CfgsExpandAccel[1];
...
{
  RD_CallStackAccel *accel = new RD_CallStackAccel[1];
  {
    Temp scratch = scratch_begin(&arena, 1);
    E_OpList oplist = e_oplist_from_irtree(scratch.arena, irtree->root);
    String8 bytecode = e_bytecode_from_oplist(scratch.arena, &oplist);
...
{
  RD_EnvironmentAccel *accel = new RD_EnvironmentAccel[1];
  {
    Temp scratch = scratch_begin(&arena, 1);
    E_OpList oplist = e_oplist_from_irtree(scratch.arena, irtree->root);
    String8 bytecode = e_bytecode_from_oplist(scratch.arena, &oplist);
...
{
  RD_WatchesAccel *accel = new RD_WatchesAccel[1];
  {
    Temp scratch = scratch_begin(&arena, 1);
    E_OpList oplist = e_oplist_from_irtree(scratch.arena, irtree->root);
    String8 bytecode = e_bytecode_from_oplist(scratch.arena, &oplist);
...
E_TYPE_EXPAND_INFO_FUNCTION_DEF(unattached_processes)
{
  E_TypeExpandInfo info = {0};
  {
    ...
    DMN_ProcessInfo *infos = new DMN_ProcessInfo[infos_count];
    CTRL_Entity **infos_machines = new CTRL_Entity *[infos_count];
    {
      U64 idx = 0;
    ...
    RD_UnattachedProcessesAccel *accel = new RD_UnattachedProcessesAccel[1];
    accel->infos       = infos;
...
E_TYPE_EXPAND_INFO_FUNCTION_DEF(ctrl_entities)
{
  ...
  Temp scratch = scratch_begin(&arena, 1);
  {
    ...
    //- rjf: list -> array & fill
    CTRL_EntityArray *accel = new CTRL_EntityArray[1];
...
E_TYPE_EXPAND_INFO_FUNCTION_DEF(debug_info_table)
{
  ...
  RD_DebugInfoTableLookupAccel *accel = new RD_DebugInfoTableLookupAccel[1];
