  ...
  for(RD_BindingNode *n = bindings.first; n != 0; n = n->next)
  {
    ...
    if(has_conflicts || rebinding_active_for_this_binding)
    {
      ...
      if(has_conflicts)
      {
        ...
        palette->colors[UI_ColorCode.TEXT_WEAK] = rd_rgba_from_theme_color(RD_ThemeColor_TextNegative);
      }
...
rd_code_slice(RD_CodeSliceParams *params, TxtPt *cursor, TxtPt *mark, S64 *preferred_column, String8 string)
{
  ...
  if(params->flags & RD_CodeSliceFlag_CatchallMargin) UI_Focus(UI_FocusKind.OFF) UI_Palette(margin_palette) UI_Parent(top_container_box) ProfScope("build catchall margins")
  {
    ...
    ui_set_next_pref_height(ui_px(params->line_height_px*(dim_1s64(params->line_num_range)+1), 1.f));
    ui_set_next_child_layout_axis(Axis2_Y);
    catchall_margin_container_box = ui_build_box_from_string(UI_BoxFlags.DRAWSIDELEFT|UI_BoxFlags.CLICKABLE*!!(params->flags & RD_CodeSliceFlag_Clickable), str8_lit("catchall_margin_container"));
  ...
  Vec4F32 line_drag_drop_color = rd_rgba_from_theme_color(RD_ThemeColor_DropSiteOverlay);
  {
    ...
    if(text_container_sig.f & UI_SignalFlags.LEFT_TRIPLE_DRAGGING)
    {
      mouse_drag_rng = mouse_line_rng;
    }
    else if(text_container_sig.f & UI_SignalFlags.LEFT_DOUBLE_DRAGGING)
    {
...
rd_line_edit(RD_LineEditFlags flags, S32 depth, FuzzyMatchRangeList *matches, TxtPt *cursor, TxtPt *mark, U8 *edit_buffer, U64 edit_buffer_size, U64 *edit_string_size_out, B32 *expanded_out, String8 pre_edit_value, String8 string)
{
  ...
  if(!is_focus_active)
  {
    B32 start_editing_via_sig = (ui_double_clicked(sig) || sig.f&UI_SignalFlags.KEYBOARD_PRESSED);
  ...
  else if(is_focus_active && sig.f&UI_SignalFlags.KEYBOARD_PRESSED)
  {
