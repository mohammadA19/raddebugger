...
{
  return (uint64)(entity - rd_state.entities_base);
}
...
RD_EntityRec
rd_entity_rec_depth_first(RD_Entity* entity, RD_Entity* subtree_root, uint64 sib_off, uint64 child_off)
{
...
rd_ev_key_from_entity(RD_Entity* entity)
{
  ...
  EV_Key key = ev_key_make(ev_hash_from_key(parent_key), (uint64)entity);
  return key;
}
...
{
  EV_Key parent_key = ev_key_make(5381, (uint64)entity);
  return parent_key;
}
...
rd_view_is_project_filtered(RD_View* view)
{
  ...
  if(view_project.size != 0)
  {
    ...
    {
      StringView current_project = rd_cfg_path_from_src(RD_CfgSrc_Project);
...
RD_PanelRec
rd_panel_rec_depth_first(RD_Panel* panel, uint64 sib_off, uint64 child_off)
{
  RD_PanelRec rec = {0};
...
rd_target_rect_from_panel(Rng2F32 root_rect, RD_Panel* root, RD_Panel* panel)
{
  ...
  for(int64 ancestor_idx = (int64)ancestor_count-1;
...
rd_title_fstrs_from_view(Arena* arena, RD_View* view, Vec4F32 primary_color, Vec4F32 secondary_color, float size)
{
  ...
  StringView query = StringView(view.query_buffer, view.query_string_size);
  StringView file_path = rd_file_path_from_eval_string(scratch.arena, query);
...
uint64
rd_name_bucket_idx_from_string_size(uint64 size)
{
  uint64 size_rounded = u64_up_to_pow2(size+1);
...
rd_cfg_tree_list_from_string(Arena* arena, StringView string)
{
  ...
  for MD_EachNode(tln, root.first)
  {
    ...
    for(MD_Node* src_n = tln; !md_node_is_nil(src_n); src_n = rec.next)
    {
      ...
      else for(int32 pop_idx = 0; pop_idx < rec.pop_count; pop_idx += 1)
      {
...
rd_string_from_cfg_tree(Arena* arena, RD_Cfg* cfg)
{
  ...
  String8List strings = {0};
  {
    ...
    for(RD_Cfg* c = cfg; c != &rd_nil_cfg; c = rec.next)
    {
      ...
      {
        for(int32 pop_idx = 0; pop_idx < rec.pop_count; pop_idx += 1, SLLStackPop(top_nest_task))
        {
...
rd_entity_alloc(RD_Entity* parent, RD_EntityKind kind)
{
  ...
  uint64 free_list_idx = !!user_defined_lifetime;
...
rd_entity_release(RD_Entity* entity)
{
  ...
  // rjf: unpack
  uint64 free_list_idx = !!(rd_entity_kind_flags_table[entity.kind] & RD_EntityKindFlag_UserDefinedLifetime);
...
void
rd_entity_equip_u64(RD_Entity* entity, uint64 u64)
{
  rd_require_entity_nonnil(entity, return);
  entity.u64 = u64;
...
void
rd_entity_equip_timestamp(RD_Entity* entity, uint64 timestamp)
{
  rd_require_entity_nonnil(entity, return);
...
void
rd_entity_equip_vaddr(RD_Entity* entity, uint64 vaddr)
{
  rd_require_entity_nonnil(entity, return);
  entity.vaddr = vaddr;
...
rd_mapped_from_file_path(Arena* arena, StringView file_path)
{
  ...
  if(file_path.size != 0)
  {
    StringView file_path__normalized = path_normalized_from_string(scratch.arena, file_path);
    ...
    uint64 best_map_match_length = max_U64;
    ...
    {
      StringView best_map_dst__normalized = path_normalized_from_string(scratch.arena, best_map_dst);
...
rd_title_fstrs_from_entity(Arena* arena, RD_Entity* entity, Vec4F32 secondary_color, float size)
{
  ...
  {
    dr_fancy_string_list_push_new(arena, &result, rd_font_from_slot(RD_FontSlot_Code), size, color_extrafied, (" if "));
  ...
  {
    dr_fancy_string_list_push_new(arena, &result, rd_font_from_slot(RD_FontSlot_Code), size, v4f32(0, 0, 0, 0), (" "));
    dr_fancy_string_list_push_new(arena, &result, rd_font_from_slot(RD_FontSlot_Main), size*0.95f, secondary_color, ("(Disabled)"));
  }
...
rd_title_fstrs_from_ctrl_entity(Arena* arena, CTRL_Entity* entity, Vec4F32 secondary_color, float size, B32 include_extras)
{
  ...
  if(entity.kind == CTRL_EntityKind_Thread && include_extras)
  {
    ...
    for(uint64 idx = 0, limit = 6; idx < unwind.frames.count && idx < limit; idx += 1)
    {
      ...
      if(rdi != &di_rdi_parsed_nil)
      {
        ...
        if(name.size != 0)
        {
          ...
          {
            dr_fancy_string_list_push_new(arena, &result, rd_font_from_slot(RD_FontSlot_Code), extras_size, secondary_color, (" > "));
            if(idx+1 == limit)
            {
              dr_fancy_string_list_push_new(arena, &result, rd_font_from_slot(RD_FontSlot_Code), extras_size, secondary_color, ("..."));
            }
  ...
  if(entity.kind == CTRL_EntityKind_Module && include_extras)
  {
    ...
    {
      dr_fancy_string_list_push_new(arena, &result, rd_font_from_slot(RD_FontSlot_Code), size, secondary_color, (" "));
      dr_fancy_string_list_push_new(arena, &result, rd_font_from_slot(RD_FontSlot_Main), extras_size, secondary_color, ("(Symbols not found)"));
    }
    di_scope_close(di_scope);
  }
...
rd_ctrl_meta_eval_from_ctrl_entity(Arena* arena, CTRL_Entity* entity)
{
  ...
  if(entity.kind == CTRL_EntityKind_Thread)
  {
    ...
    uint64 idx = 0;
    for(uint64 base_idx = 0; base_idx < rich_unwind.concrete_frame_count; base_idx += 1)
    {
      uint64 inline_idx = 0;
...
rd_eval_space_read(void* u, E_Space space, void* out, Rng1U64 range)
{
  ...
  switch(space.kind)
  {
    ...
    case RD_EvalSpaceKind_CtrlEntity:
    {
      ...
      switch(entity.kind)
      {
        ...
        case CTRL_EntityKind_Thread:
        {
          Temp scratch = scratch_begin(0, 0);
          CTRL_Unwind unwind = d_query_cached_unwind_from_thread(entity);
          uint64 frame_idx = e_interpret_ctx.reg_unwind_count;
    ...
    {
      CTRL_Entity* entity = rd_ctrl_entity_from_eval_space(space);
      uint64 hash = ctrl_hash_from_handle(entity.handle);
      uint64 slot_idx = hash%rd_state.ctrl_entity_meval_cache_slots_count;
      ...
      if(!node)
      {
        ...
        StringView meval_srlzed = serialized_from_struct(scratch.arena, CTRL_MetaEval, meval);
        uint64 pos_min = arena_pos(rd_frame_arena());
        ...
        uint64 pos_opl = arena_pos(scratch.arena);
    ...
    case RD_EvalSpaceKind_MetaEntity:
    {
      ...
      StringView meval_srlzed = serialized_from_struct(scratch.arena, CTRL_MetaEval, meval);
      uint64 pos_min = arena_pos(scratch.arena);
    ...
    meta_eval:;
    {
      ...
      {
        result = 1;
        uint64 range_dim = dim_1u64(range);
        uint64 bytes_to_read = Min(range_dim, (meval_legal_range.max - range.min));
        MemoryCopy(out, ((uint8 *)meval_read) + range.min, bytes_to_read);
        if(bytes_to_read < range_dim)
        {
          MemoryZero((uint8 *)out + bytes_to_read, range_dim - bytes_to_read);
        }
      }
    }break;
  }
...
#define StringMemberCase(name) else if(range.min == (uint64)meval_read.name.str)
      ...
      StringMemberCase(exe)               {result = 1; rd_entity_equip_name(rd_entity_child_from_kind_or_alloc(entity, RD_EntityKind_Executable), str8_cstring_capped(in, (uint8 *)in + 4096));}
      StringMemberCase(args)              {result = 1; rd_entity_equip_name(rd_entity_child_from_kind_or_alloc(entity, RD_EntityKind_Arguments), str8_cstring_capped(in, (uint8 *)in + 4096));}
      StringMemberCase(working_directory) {result = 1; rd_entity_equip_name(rd_entity_child_from_kind_or_alloc(entity, RD_EntityKind_WorkingDirectory), str8_cstring_capped(in, (uint8 *)in + 4096));}
      StringMemberCase(entry_point)       {result = 1; rd_entity_equip_name(rd_entity_child_from_kind_or_alloc(entity, RD_EntityKind_EntryPoint), str8_cstring_capped(in, (uint8 *)in + 4096));}
      StringMemberCase(stdout_path)       {result = 1; rd_entity_equip_name(rd_entity_child_from_kind_or_alloc(entity, RD_EntityKind_StdoutPath), str8_cstring_capped(in, (uint8 *)in + 4096));}
      StringMemberCase(stderr_path)       {result = 1; rd_entity_equip_name(rd_entity_child_from_kind_or_alloc(entity, RD_EntityKind_StderrPath), str8_cstring_capped(in, (uint8 *)in + 4096));}
      StringMemberCase(stdin_path)        {result = 1; rd_entity_equip_name(rd_entity_child_from_kind_or_alloc(entity, RD_EntityKind_StdinPath), str8_cstring_capped(in, (uint8 *)in + 4096));}
      StringMemberCase(source_path)       {result = 1; rd_entity_equip_name(rd_entity_child_from_kind_or_alloc(entity, RD_EntityKind_Source), str8_cstring_capped(in, (uint8 *)in + 4096));}
      StringMemberCase(destination_path)  {result = 1; rd_entity_equip_name(rd_entity_child_from_kind_or_alloc(entity, RD_EntityKind_Dest), str8_cstring_capped(in, (uint8 *)in + 4096));}
      StringMemberCase(type)              {result = 1; rd_entity_equip_name(rd_entity_child_from_kind_or_alloc(entity, RD_EntityKind_Source), str8_cstring_capped(in, (uint8 *)in + 4096));}
      StringMemberCase(view_rule)         {result = 1; rd_entity_equip_name(rd_entity_child_from_kind_or_alloc(entity, RD_EntityKind_Dest), str8_cstring_capped(in, (uint8 *)in + 4096));}
      StringMemberCase(condition)         {result = 1; rd_entity_equip_name(rd_entity_child_from_kind_or_alloc(entity, RD_EntityKind_Condition), str8_cstring_capped(in, (uint8 *)in + 4096));}
      ...
      {
        uint64 vaddr = 0;
        if(try_u64_from_str8_c_rules(str8_cstring_capped(in, (uint8 *)in + 4096), &vaddr))
        {
          RD_Entity* loc = rd_entity_child_from_kind_or_alloc(entity, RD_EntityKind_Location);
          rd_entity_equip_vaddr(loc, vaddr);
          rd_entity_equip_name(loc, StringView());
      ...
      StringMemberCase(function_location)
      {
        ...
        RD_Entity* loc = rd_entity_child_from_kind_or_alloc(entity, RD_EntityKind_Location);
        loc.flags &= ~RD_EntityFlag_HasTextPoint;
        rd_entity_equip_name(loc, str8_cstring_capped(in, (uint8 *)in + 4096));
      }
...
E_Value
rd_value_from_params_key(MD_Node* params, StringView key)
{
  Temp scratch = scratch_begin(0, 0);
  MD_Node* key_node = md_child_from_string(params, key, 0);
  StringView expr = md_string_from_children(scratch.arena, key_node);
...
{
  Temp scratch = scratch_begin(0, 0);
  uint64 size = rd_value_from_params_key(params, ("size")).u64;
...
rd_dim2s32_from_eval_params(E_Eval eval, MD_Node* params)
{
  ...
  {
    dim.x = rd_value_from_params_key(params, ("w")).s32;
    dim.y = rd_value_from_params_key(params, ("h")).s32;
  }
  return dim;
}
...
rd_view_alloc()
{
  ...
  RD_View* view = rd_state.free_view;
  {
    if(!rd_view_is_nil(view))
    {
      ...
      uint64 generation = view.generation;
  ...
  view.project_path = StringView();
  for(uint64 idx = 0; idx < ArrayCount(view.params_arenas); idx += 1)
  {
...
void
rd_view_equip_spec(RD_View* view, RD_ViewRuleInfo* spec, StringView query, MD_Node* params)
{
...
void
rd_view_equip_loading_info(RD_View* view, B32 is_loading, uint64 progress_v, uint64 progress_target)
{
...
void *
rd_view_get_or_push_user_state(RD_View* view, uint64 size)
{
  void* result = view.user_data;
  if(result == 0)
  {
    view.user_data = result = push_array(view.arena, uint8, size);
  }
  return result;
}
...
void
rd_view_store_param(RD_View* view, StringView key, StringView value)
{
  B32 new_copy = 0;
  ...
  {
    new_params_root = view.params_roots[view.params_write_gen%ArrayCount(view.params_arenas)] = md_push_node(new_params_arena, MD_NodeKind_Main, 0, StringView(), StringView(), 0);
  }
  MD_Node* key_node = md_child_from_string(new_params_root, key, 0);
  if(md_node_is_nil(key_node))
  {
    StringView key_copy = push_str8_copy(new_params_arena, key);
  ...
  StringView value_copy = push_str8_copy(new_params_arena, value);
  MD_TokenizeResult value_tokenize = md_tokenize_from_text(new_params_arena, value_copy);
  MD_ParseResult value_parse = md_parse_from_text_tokens(new_params_arena, StringView(), value_copy, value_tokenize.tokens);
...
void
rd_view_store_paramf(RD_View* view, StringView key, char* fmt, ...)
{
  Temp scratch = scratch_begin(0, 0);
  va_list args;
  va_start(args, fmt);
...
rd_transient_view_node_from_ev_key(RD_View* owner_view, EV_Key key)
{
  ...
  uint64 slot_idx = hash%owner_view.transient_view_slots_count;
...
rd_panel_alloc(RD_Window* ws)
{
  ...
  if(!rd_panel_is_nil(panel))
  {
    ...
    uint64 generation = panel.generation;
...
rd_window_open(Vec2F32 size, OS_Handle preferred_monitor, RD_CfgSrc cfg_src)
{
  ...
  {
    StringView title = (BUILD_TITLE_STRING_LITERAL);
  ...
  window.ctx_menu_input_buffer = push_array(window.arena, uint8, window.ctx_menu_input_buffer_size);
       ...
       (rd_state.drag_drop_regs_slot != RD_RegSlot_Null && rd_drag_is_active()))
    {
      ...
      RD_Palette(RD_PaletteCode_Floating) switch(slot)
      {
        ...
        UI_Tooltip
        {
          ...
          if(entity.kind == RD_EntityKind_Target && entity.cfg_src == RD_CfgSrc_CommandLine)
          {
            UI_Flags(UI_BoxFlag_DrawTextWeak) ui_label(("Specified on the command line; will not be saved."));
          }
        }break;
        ...
        UI_Tooltip
        {
          ...
          UI_PrefWidth(ui_children_sum(1)) UI_Row UI_PrefWidth(ui_text_dim(5, 1))
          {
              ...
              RD_Palette(RD_PaletteCode_NeutralPopButton)
            {
              ...
              {
                UI_FlagsAdd(UI_BoxFlag_DrawTextWeak|UI_BoxFlag_DrawBorder) ui_labelf("ID: %i", (uint32)ctrl_entity.id);
              }
            }
          }
          ...
          if(ctrl_entity.kind == CTRL_EntityKind_Thread)
          {
            ...
            for(uint64 idx = 0; idx < rich_unwind.concrete_frame_count; idx += 1)
            {
            ...
            UI_TextAlignment(main_text_align)
          {
            ...
            UI_Parent(container)
            {
              ...
              UI_Parent(view_preview_container) UI_Focus(UI_FocusKind_Off) UI_WidthFill
              {
                ...
                view_ui(StringView(view.query_buffer, view.query_string_size), view.params_roots[view.params_read_gen%ArrayCount(view.params_roots)], view_preview_container.rect);
              }
            }
          }
        }
...
#undef Handle
        ...
        for(RD_Window* window = rd_state.first_window; window != 0; window = window.next)
        {
          ...
          double avg_ui_hash_chain_length = 0;
          {
            ...
            for(uint64 idx = 0; idx < ws.ui.box_table_size; idx += 1)
            {
        ...
        switch(slot)
        {
          ...
          case RD_RegSlot_Cursor:
          {
            ...
            if(!txt_pt_match(range.min, range.max) && ui_clicked(rd_cmd_spec_button(rd_cmd_kind_info_table[RD_CmdKind_Copy].string)))
            {
              ...
              StringView copy_data = txt_string_from_info_data_txt_rng(&info, data, range);
            ...
            if(range.min.line == range.max.line && ui_clicked(rd_icon_buttonf(RD_IconKind_RightArrow, 0, "Set Next Statement")))
            {
              ...
              uint64 new_rip_vaddr = regs.vaddr;
          ...
          case RD_RegSlot_View:
          {
            ...
            DR_FancyStringList fstrs = rd_title_fstrs_from_view(scratch.arena, view, ui_top_palette().text, ui_top_palette().text_weak, ui_top_font_size());
            StringView file_path = rd_file_path_from_eval_string(scratch.arena, StringView(view.query_buffer, view.query_string_size));
            ...
            if(file_path.size != 0)
            {
              ...
              StringView full_path = path_normalized_from_string(scratch.arena, file_path);
            ...
            {
              Temp scratch = scratch_begin(0, 0);
              StringView schema_string = view.spec.params_schema;
              MD_TokenizeResult schema_tokenize = md_tokenize_from_text(scratch.arena, schema_string);
              MD_ParseResult schema_parse = md_parse_from_text_tokens(scratch.arena, StringView(), schema_string, schema_tokenize.tokens);
          ...
          ctrl_entity_title:;
          {
              ...
              UI_TextPadding(ui_top_font_size()*1.5f)
            {
              ...
              if(ctrl_entity.kind == CTRL_EntityKind_Thread)
              {
                  ...
                  UI_TextPadding(ui_top_font_size()*0.5f)
                {
                  ...
                  ui_spacer(ui_em(0.5f, 1.f));
                  UI_FlagsAdd(UI_BoxFlag_DrawTextWeak|UI_BoxFlag_DrawBorder) ui_labelf("TID: %i", (uint32)ctrl_entity.id);
                }
              }
            }
            ...
            if(ctrl_entity.kind == CTRL_EntityKind_Thread)
            {
              if(ui_clicked(rd_icon_buttonf(RD_IconKind_Clipboard, 0, "Copy Call Stack")))
              {
                ...
                CTRL_CallStack rich_unwind = ctrl_call_stack_from_unwind(scratch.arena, di_scope, process, &base_unwind);
                String8List lines = {0};
                for(uint64 frame_idx = 0; frame_idx < rich_unwind.concrete_frame_count; frame_idx += 1)
                {
                  CTRL_CallStackFrame* concrete_frame = &rich_unwind.frames[frame_idx];
                  uint64 rip_vaddr = regs_rip_from_arch_block(ctrl_entity.arch, concrete_frame.regs);
                  CTRL_Entity* module = ctrl_module_from_process_vaddr(process, rip_vaddr);
                ...
                StringJoin join = {0};
                join.sep = join.post = ("\n");
                StringView text = str8_list_join(scratch.arena, &lines, &join);
                ...
                UI_Row UI_Padding(ui_pct(1, 0))
                {
                  UI_PrefWidth(ui_em(1.5f, 1.f)) UI_PrefHeight(ui_em(9.f, 1.f)) UI_Column UI_PrefHeight(ui_em(1.5f, 0.f))
                  {
                    ...
                    UI_CornerRadius(ui_em(0.3f, 1.f).value)
                      for(uint64 preset_idx = 0; preset_idx < ArrayCount(presets); preset_idx += 1)
                    {
    ...
    {
      StringView input = StringView(ws.autocomp_lister_input_buffer, ws.autocomp_lister_input_size);
      StringView query_word = rd_autocomp_query_word_from_input_string_off(input, ws.autocomp_cursor_off);
      StringView query_path = rd_autocomp_query_path_from_input_string_off(input, ws.autocomp_cursor_off);
      ...
      if(!ui_box_is_nil(autocomp_root_box))
      {
        ...
        uint64 thread_rip_vaddr = d_query_cached_rip_from_thread_unwind(thread, rd_base_regs().unwind_count);
        CTRL_Entity* process = ctrl_entity_ancestor_from_kind(thread, CTRL_EntityKind_Process);
        ...
        RD_AutoCompListerItemChunkList item_list = {0};
        {
          ...
          if(ws.autocomp_lister_params.flags & RD_AutoCompListerFlag_Registers)
          {
            ...
            uint64 reg_names_count = regs_reg_code_count_from_arch(arch);
            uint64 alias_names_count = regs_alias_code_count_from_arch(arch);
            StringView* reg_names = regs_reg_code_string_table_from_arch(arch);
            StringView* alias_names = regs_alias_code_string_table_from_arch(arch);
            for(uint64 idx = 0; idx < reg_names_count; idx += 1)
            {
            ...
            for(uint64 idx = 0; idx < alias_names_count; idx += 1)
            {
          ...
          {
            for(uint64 slot_idx = 0; slot_idx < d_state.view_rule_spec_table_size; slot_idx += 1)
            {
          ...
          {
            U128 search_key = {d_hash_from_string(("autocomp_globals_search_key")), d_hash_from_string(("autocomp_globals_search_key"))};
            ...
            for(uint64 idx = 0; idx < 20 && idx < items.count; idx += 1)
            {
              ...
              StringView name = di_search_item_string_from_rdi_target_element_idx(rdi, search_params.target, items.v[idx].idx);
          ...
          {
            U128 search_key = {d_hash_from_string(("autocomp_tvars_dis_key")), d_hash_from_string(("autocomp_tvars_dis_key"))};
            DI_SearchParams search_params =
            {
          ...
          {
            U128 search_key = {d_hash_from_string(("autocomp_procedures_search_key")), d_hash_from_string(("autocomp_procedures_search_key"))};
            DI_SearchParams search_params =
            {
          ...
          {
            U128 search_key = {d_hash_from_string(("autocomp_types_search_key")), d_hash_from_string(("autocomp_types_search_key"))};
            DI_SearchParams search_params =
            {
          ...
          if(ws.autocomp_lister_params.flags & RD_AutoCompListerFlag_Files)
          {
            ...
            StringView dir_str_in_input = {0};
            for(uint64 i = 0; i < query_path.size; i += 1)
            {
              StringView substr1 = str8_substr(query_path, r1u64(i, i+1));
              StringView substr2 = str8_substr(query_path, r1u64(i, i+2));
              StringView substr3 = str8_substr(query_path, r1u64(i, i+3));
            ...
            if(dir_str_in_input.size != 0)
            {
              StringView dir = dir_str_in_input;
        ...
        if(item_array.count != 0)
        {
          ...
          {
            for(uint64 idx = 0; idx < item_array.count; idx += 1)
            {
              ...
              if(ui_clicked(item_sig))
              {
                ...
                move_back_evt.delta_2s32.x = -(int32)query_word.size;
        ...
        UI_Focus(UI_FocusKind_Null)
      {
        ...
        UI_WidthFill UI_NamedRow(("###menu_bar"))
        {
          ...
          UI_PrefWidth(ui_children_sum(1)) UI_Row UI_PrefWidth(ui_text_dim(20, 1)) UI_GroupKey(menu_bar_group_key)
          {
            ...
            // rjf: view menu
            UI_Key view_menu_key = ui_key_from_string(ui_key_zero(), ("_view_menu_key_"));
            RD_Palette(RD_PaletteCode_Floating)
        ...
        UI_WidthFill UI_Row
        {
          ...
          if(do_user_prof) RD_Palette(RD_PaletteCode_NeutralPopButton)
          {
            ...
            {
              StringView user_path = rd_cfg_path_from_src(RD_CfgSrc_User);
          ...
          if(do_user_prof) RD_Palette(RD_PaletteCode_NeutralPopButton)
          {
            ...
            {
              StringView prof_path = rd_cfg_path_from_src(RD_CfgSrc_Project);
    ...
    ProfScope("build bottom bar")
    {
        ...
        UI_Flags(0)
      {
        ...
        else if(ws.error_t >= 0.01f)
        {
          ...
          rd_request_frame();
          StringView error_string = StringView(ws.error_buffer, ws.error_string_size);
    ...
    if(ws.query_cmd_name.size != 0)
    {
      ...
      StringView query_view_name = cmd_kind_info.query.view_name;
      ...
      RD_Palette(RD_PaletteCode_Floating)
    {
      ...
      StringView query_cmd_name = ws.query_cmd_name;
        ...
        UI_Row
        {
            ...
            UI_TextPadding(ui_top_font_size()*0.5f)
          {
            UI_Signal sig = rd_line_edit(RD_LineEditFlag_Border|
                                         ...
                                         0,
                                         StringView(view.query_buffer, view.query_string_size),
                                         ("###query_text_input"));
            if(ui_pressed(sig))
            {
              ws.query_view_selected = 1;
      ...
      {
        RD_ViewRuleUIFunctionType* view_ui = view.spec.ui;
        view_ui(StringView(view.query_buffer, view.query_string_size), view.params_roots[view.params_read_gen%ArrayCount(view.params_roots)], query_container_content_rect);
      }
      ...
      if((ui_is_focus_active() && ui_slot_press(UI_EventActionSlot_Accept)) || query_completed)
      {
        ...
        RD_RegsScope()
        {
          rd_regs_fill_slot_from_string(query.slot, StringView(view.query_buffer, view.query_string_size));
    ...
    ProfScope("build hover eval")
    {
        ...
        RD_Palette(RD_PaletteCode_Floating)
      {
        ...
        StringView expr = ws.hover_eval_string;
        E_Eval eval = e_eval_from_string(scratch.arena, expr);
          ...
          UI_Focus((hover_eval_is_open && !ui_any_ctx_menu_is_open() && ws.hover_eval_focused && (!query_is_open || !ws.query_view_selected)) ? UI_FocusKind_Null : UI_FocusKind_Off)
        {
          ...
          uint64 expr_hash = d_hash_from_string(expr);
          StringView ev_view_key_string = push_str8f(scratch.arena, "eval_hover_%I64x", expr_hash);
          ...
          EV_BlockTree block_tree = ev_block_tree_from_string(scratch.arena, ev_view, StringView(), expr, &top_level_view_rules);
          EV_BlockRangeList block_ranges = ev_block_range_list_from_tree(scratch.arena, &block_tree);
          // EV_BlockList viz_blocks = ev_block_list_from_view_expr_keys(scratch.arena, ev_view, StringView(), &top_level_view_rules, expr, parent_key, key, 0);
          CTRL_Entity* entity = rd_ctrl_entity_from_eval_space(eval.space);
          uint32 default_radix = (entity.kind == CTRL_EntityKind_Thread ? 16 : 10);
          EV_WindowedRowList rows = ev_windowed_row_list_from_block_range_list(scratch.arena, ev_view, StringView(), &block_ranges, r1u64(0, 50));
          ...
          if(rows.first != 0)
          {
            ...
            StringView row_expr_string = ev_expr_string_from_row(scratch.arena, row, 0);
            StringView row_display_value = rd_value_string_from_eval(scratch.arena, EV_StringFlag_ReadOnlyDisplayRules, default_radix, ui_top_font(), ui_top_font_size(), 500.f, row_eval, row.member, row.view_rules);
          ...
          UI_Parent(hover_eval_box) UI_PrefHeight(ui_px(row_height, 1.f))
          {
            ...
            for(EV_Row* row = rows.first; row != 0; row = row.next)
            {
              ...
              E_Eval row_eval = e_eval_from_expr(scratch.arena, row.expr);
              StringView row_expr_string = ev_expr_string_from_row(scratch.arena, row, 0);
              StringView row_edit_value = rd_value_string_from_eval(scratch.arena, 0, default_radix, ui_top_font(), ui_top_font_size(), 500.f, row_eval, row.member, row.view_rules);
              StringView row_display_value = rd_value_string_from_eval(scratch.arena, EV_StringFlag_ReadOnlyDisplayRules, default_radix, ui_top_font(), ui_top_font_size(), 500.f, row_eval, row.member, row.view_rules);
              ...
              switch(row_eval.mode)
              {
                ...
                case E_Mode_Offset:
                {
                  ...
                  {
                    uint64 size = e_type_byte_size_from_key(row_eval.type_key);
                    size = Min(size, 64);
              ...
              UI_WidthFill UI_Row
              {
                ...
                if(row_depth > 0)
                {
                  for(uint64 indent = 0; indent < row_depth; indent += 1)
                  {
                    ui_spacer(ui_em(0.5f, 1.f));
                ...
                UI_WidthFill UI_TextRasterFlags(rd_raster_flags_from_slot(RD_FontSlot_Code))
                {
                  ...
                  if(row_is_editable)
                  {
                    ...
                    if(ui_committed(sig))
                    {
                      StringView commit_string = StringView(ws.hover_eval_txt_buffer, ws.hover_eval_txt_size);
          ...
          panel = rd_panel_rec_depth_first_pre(panel).next)
    {
      ...
      UI_Focus(panel_is_focused ? UI_FocusKind_Null : UI_FocusKind_Off)
      {
          ...
          UI_WidthFill
        {
          ...
          {
            RD_View* view = rd_selected_tab_from_panel(panel);
            StringView view_file_path = rd_file_path_from_eval_string(rd_frame_arena(), StringView(view.query_buffer, view.query_string_size));
          ...
          {
            RD_VIEW_RULE_UI_FUNCTION_NAME(empty)(StringView(), &md_nil_node, content_rect);
          }
          ...
          UI_Parent(view_container_box) if(!rd_view_is_nil(rd_selected_tab_from_panel(panel)))
          {
            RD_View* view = rd_selected_tab_from_panel(panel);
            RD_ViewRuleUIFunctionType* view_ui = view.spec.ui;
            view_ui(StringView(view.query_buffer, view.query_string_size), view.params_roots[view.params_read_gen%ArrayCount(view.params_roots)], content_rect);
          }
        ...
        UI_Focus(UI_FocusKind_Off)
        {
          ...
          uint64 drop_site_count = panel.tab_view_count+1;
          ...
          // rjf: more precise drop-sites on tab bar
          {
            ...
            if(rd_drag_is_active() && rd_state.drag_drop_regs_slot == RD_RegSlot_View && window_is_focused && contains_2f32(panel_rect, mouse) && !rd_view_is_nil(view))
            {
              ...
              {
                for(uint64 drop_site_idx = 0; drop_site_idx < drop_site_count; drop_site_idx += 1)
                {
    ...
    ProfScope("draw UI")
  {
    ...
    if(DEV_draw_ui_box_heatmap)
    {
      ...
      Vec2S32 buckets_dim = {(int32)(size.x/heatmap_bucket_size), (int32)(size.y/heatmap_bucket_size)};
    ...
    for(UI_Box* box = ui_root_from_state(ws.ui); !ui_box_is_nil(box);)
    {
      ...
      if(box.flags & UI_BoxFlag_DrawText)
      {
        ...
        if(!(box.flags & UI_BoxFlag_DisableTextTrunc))
        {
          ...
          ellipses_run = fnt_push_run_from_string(scratch.arena, box.font, box.font_size, 0, box.tab_size, 0, ("..."));
        }
    ...
    if(DEV_scratch_mouse_draw)
    {
      ...
      FNT_Run trailer_run = fnt_push_run_from_string(scratch.arena, rd_font_from_slot(RD_FontSlot_Main), 16.f, 0, 0, 0, ("..."));
      DR_FancyStringList strs = {0};
      DR_FancyString str = {rd_font_from_slot(RD_FontSlot_Main), ("Shift + F5"), v4f32(1, 1, 1, 1), 72.f, 0.f};
      ...
      DR_FancyStringList strs = {0};
      DR_FancyString str1 = {rd_font_from_slot(RD_FontSlot_Main), ("T"), v4f32(1, 1, 1, 1), 16.f, 4.f};
      dr_fancy_string_list_push(scratch.arena, &strs, &str1);
      DR_FancyString str2 = {rd_font_from_slot(RD_FontSlot_Main), ("his is a test of some "), v4f32(1, 0.5f, 0.5f, 1), 14.f, 0.f};
      dr_fancy_string_list_push(scratch.arena, &strs, &str2);
      DR_FancyString str3 = {rd_font_from_slot(RD_FontSlot_Code), ("very fancy text!"), v4f32(1, 0.8f, 0.4f, 1), 18.f, 4.f, 4.f};
...
EV_VIEW_RULE_EXPR_EXPAND_INFO_FUNCTION_DEF(machines)          { return rd_ev_view_rule_expr_expand_info__meta_ctrl_entities(arena, view, StringView(), expr, params, CTRL_EntityKind_Machine); }
EV_VIEW_RULE_EXPR_EXPAND_RANGE_INFO_FUNCTION_DEF(machines)    { return rd_ev_view_rule_expr_expand_range_info__meta_ctrl_entities(arena, view, StringView(), expr, params, idx_range, user_data, CTRL_EntityKind_Machine); }
...
EV_VIEW_RULE_EXPR_EXPAND_RANGE_INFO_FUNCTION_DEF(scheduler_machine)
{
  EV_ExpandRangeInfo info = {0};
  {
    ...
    if(processes != 0)
    {
      ...
      info.row_strings     = push_array(arena, StringView,    info.row_exprs_count);
      info.row_view_rules  = push_array(arena, StringView,    info.row_exprs_count);
      ...
      uint64 row_expr_idx = 0;
      for(uint64 idx = idx_range.min; idx < idx_range.max; idx += 1, row_expr_idx += 1)
      {
      ...
      if(child.kind == CTRL_EntityKind_Thread)
      {
        ...
        if(filter.size != 0)
        {
          ...
          else
          {
            ...
            for(uint64 idx = 0; idx < call_stack.concrete_frame_count && idx < 5; idx += 1)
            {
...
EV_VIEW_RULE_EXPR_EXPAND_RANGE_INFO_FUNCTION_DEF(locals)
{
  ...
  EV_ExpandRangeInfo result = {0};
  {
    ...
    result.row_strings     = push_array(arena, StringView, result.row_exprs_count);
    result.row_view_rules  = push_array(arena, StringView, result.row_exprs_count);
...
EV_VIEW_RULE_EXPR_EXPAND_INFO_FUNCTION_DEF(registers)
{
  ...
  StringView* reg_strings   = regs_reg_code_string_table_from_arch(arch);
  StringView* alias_strings = regs_alias_code_string_table_from_arch(arch);
...
EV_ExpandInfo
rd_ev_view_rule_expr_expand_info__meta_entities(Arena* arena, EV_View* view, StringView filter, E_Expr* expr, MD_Node* params, RD_EntityKind kind)
{
...
EV_ExpandRangeInfo
rd_ev_view_rule_expr_expand_range_info__meta_entities(Arena* arena, EV_View* view, StringView filter, E_Expr* expr, MD_Node* params, Rng1U64 idx_range, void* user_data, RD_EntityKind kind, B32 add_new_at_top)
{
  RD_EntityExpandAccel* accel = (RD_EntityExpandAccel *)user_data;
  EV_ExpandRangeInfo result = {0};
  {
    uint64 entities_base_idx = (uint64)!!add_new_at_top;
    uint64 needed_row_count = dim_1u64(idx_range);
    ...
    for EachIndex(row_expr_idx, result.row_exprs_count)
    {
      uint64 child_idx = idx_range.min + row_expr_idx;
      RD_Entity* entity = &rd_nil_entity;
...
uint64
rd_ev_view_rule_expr_id_from_num__meta_entities(uint64 num, void* user_data, RD_EntityKind kind, B32 add_new_at_top)
{
  uint64 id = 0;
  RD_EntityExpandAccel* accel = (RD_EntityExpandAccel *)user_data;
  uint64 entities_base_idx = (uint64)!!add_new_at_top;
...
uint64
rd_ev_view_rule_expr_num_from_id__meta_entities(uint64 id, void* user_data, RD_EntityKind kind, B32 add_new_at_top)
{
  RD_EntityExpandAccel* accel = (RD_EntityExpandAccel *)user_data;
  uint64 num = 0;
  uint64 entities_base_idx = (uint64)!!add_new_at_top;
  ...
  else for(uint64 idx = 0; idx < accel.entities.count; idx += 1)
  {
...
EV_ExpandInfo
rd_ev_view_rule_expr_expand_info__meta_ctrl_entities(Arena* arena, EV_View* view, StringView filter, E_Expr* expr, MD_Node* params, CTRL_EntityKind kind)
{
...
EV_ExpandRangeInfo
rd_ev_view_rule_expr_expand_range_info__meta_ctrl_entities(Arena* arena, EV_View* view, StringView filter, E_Expr* expr, MD_Node* params, Rng1U64 idx_range, void* user_data, CTRL_EntityKind kind)
{
  ...
  EV_ExpandRangeInfo result = {0};
  {
    ...
    for EachIndex(row_expr_idx, result.row_exprs_count)
    {
      ...
      StringView entity_expr_string = push_str8f(arena, "ctrl_entity:$_%I64x_%I64x", entity.handle.machine_id, entity.handle.dmn_handle.u64[0]);
      result.row_exprs[row_expr_idx] = e_parse_expr_from_text(arena, entity_expr_string);
      result.row_members[row_expr_idx] = &e_member_nil;
    }
  }
...
uint64
rd_ev_view_rule_expr_id_from_num__meta_ctrl_entities(uint64 num, void* user_data, CTRL_EntityKind kind)
{
  RD_CtrlEntityExpandAccel* accel = (RD_CtrlEntityExpandAccel *)user_data;
  uint64 id = 0;
...
uint64
rd_ev_view_rule_expr_num_from_id__meta_ctrl_entities(uint64 id, void* user_data, CTRL_EntityKind kind)
{
  RD_CtrlEntityExpandAccel* accel = (RD_CtrlEntityExpandAccel *)user_data;
  uint64 num = 0;
  if(id != 0)
  {
    ...
    {
      uint64 idx_id = d_hash_from_string(str8_struct(&accel.entities.v[idx].handle));
      if(idx_id == id)
      {
        num = idx+1;
        break;
...
EV_ExpandInfo
rd_ev_view_rule_expr_expand_info__debug_info_tables(Arena* arena, EV_View* view, StringView filter, E_Expr* expr, MD_Node* params, RDI_SectionKind section)
{
  RD_DebugInfoTableExpandAccel* accel = push_array(arena, RD_DebugInfoTableExpandAccel, 1);
  if(section != RDI_SectionKind_NULL)
  {
    Temp scratch = scratch_begin(&arena, 1);
    uint64 endt_us = os_now_microseconds()+200;
    ...
    U128 fuzzy_search_key = {(uint64)view, (uint64)section};
    B32 items_stale = 0;
...
EV_ExpandRangeInfo
rd_ev_view_rule_expr_expand_range_info__debug_info_tables(Arena* arena, EV_View* view, StringView filter, E_Expr* expr, MD_Node* params, Rng1U64 idx_range, void* user_data, RDI_SectionKind section)
{
      ...
      E_Expr* item_expr = &e_expr_nil;
      {
        ...
        switch(section)
        {
          ...
          case RDI_SectionKind_Procedures:
          {
            ...
            E_TypeKey type_key = e_type_key_ext(e_type_kind_from_rdi(type_node.kind), type_idx, (uint32)(module - e_parse_ctx.modules));
          ...
          case RDI_SectionKind_GlobalVariables:
          {
            ...
            uint64 voff = gvar.voff;
            E_OpList oplist = {0};
            e_oplist_push_op(arena, &oplist, RDI_EvalOp_ConstU64, e_value_u64(module.vaddr_range.min + voff));
            StringView bytecode = e_bytecode_from_oplist(arena, &oplist);
            uint32 type_idx = gvar.type_idx;
            RDI_TypeNode* type_node = rdi_element_from_name_idx(module.rdi, TypeNodes, type_idx);
            E_TypeKey type_key = e_type_key_ext(e_type_kind_from_rdi(type_node.kind), type_idx, (uint32)(module - e_parse_ctx.modules));
            item_expr = e_push_expr(arena, E_ExprKind_LeafBytecode, 0);
          ...
          case RDI_SectionKind_UDTs:
          {
            ...
            E_TypeKey type_key = e_type_key_ext(e_type_kind_from_rdi(type_node.kind), udt.self_type_idx, (uint32)(module - e_parse_ctx.modules));
...
uint64
rd_ev_view_rule_expr_id_from_num__debug_info_tables(uint64 num, void* user_data, RDI_SectionKind section)
{
  RD_DebugInfoTableExpandAccel* accel = (RD_DebugInfoTableExpandAccel *)user_data;
  uint64 id = 0;
...
uint64
rd_ev_view_rule_expr_num_from_id__debug_info_tables(uint64 id, void* user_data, RDI_SectionKind section)
{
  RD_DebugInfoTableExpandAccel* accel = (RD_DebugInfoTableExpandAccel *)user_data;
...
EV_View *
rd_ev_view_from_key(uint64 key)
{
  uint64 slot_idx = key % rd_state.eval_viz_view_cache_slots_count;
...
float
rd_append_value_strings_from_eval(Arena* arena, EV_StringFlags flags, uint32 default_radix, FNT_Tag font, float font_size, float max_size, int32 depth, E_Eval eval, E_Member* member, EV_ViewRuleList* view_rules, String8List* out)
{
  ProfBeginFunction();
  Temp scratch = scratch_begin(&arena, 1);
  float space_taken = 0;
  ...
  for(EV_ViewRuleNode* n = view_rules.first; n != 0; n = n.next)
  {
    ...
    else if(str8_match(n.v.root.string, ("digits"), 0))
    {
      ...
      E_Eval eval = e_eval_from_string(scratch.arena, expr);
      E_Eval value_eval = e_value_eval_from_eval(eval);
      min_digits = (uint32)value_eval.value.u64;
    }
  }
  ...
  {
    uint64 member_byte_size = e_type_byte_size_from_key(eval.type_key);
    StringView offset_string = str8_from_u64(arena, member.off, radix, 0, 0);
    StringView size_string = str8_from_u64(arena, member_byte_size, radix, 0, 0);
  ...
  {
    StringView basic_type_kind_string = e_kind_basic_string_table[e_type_kind_from_key(eval.type_key)];
    uint64 byte_size = e_type_byte_size_from_key(eval.type_key);
    StringView size_string = str8_from_u64(arena, byte_size, radix, 0, 0);
  ...
  else if(max_size > 0) switch(e_type_kind_from_key(e_type_unwrap(eval.type_key)))
  {
    ...
    case E_TypeKind_RRef:
    {
      ...
      if(!did_content && ptee_has_string && !no_string)
      {
        did_content = 1;
        did_string = 1;
        uint64 string_memory_addr = value_eval.value.u64;
        uint64 element_size = e_type_byte_size_from_key(direct_type_key);
        uint64 string_buffer_size = 256;
        uint8* string_buffer = push_array(arena, uint8, string_buffer_size);
        for(uint64 try_size = string_buffer_size; try_size >= 16; try_size /= 2)
        {
    ...
    case E_TypeKind_Array:
    {
      ...
      E_TypeKind direct_type_kind = e_type_kind_from_key(direct_type_key);
      uint64 array_count = eval_type.count;
      ...
      if(!did_content && array_is_string && !no_string && (member == 0 || member.kind != E_MemberKind_Padding))
      {
        uint64 element_size = e_type_byte_size_from_key(direct_type_key);
        did_content = 1;
        uint64 string_buffer_size = Clamp(1, array_count, 1024);
        uint8* string_buffer = push_array(arena, uint8, string_buffer_size);
        switch(eval.mode)
        {
          default:{}break;
          case E_Mode_Offset:
          {
            uint64 string_memory_addr = eval.value.u64;
      ...
      if(!did_content && (flags & EV_StringFlag_ReadOnlyDisplayRules))
      {
        ...
        if(depth < 4)
        {
          for(uint64 idx = 0; idx < array_count && max_size > space_taken; idx += 1)
          {
    ...
    case E_TypeKind_IncompleteClass:
    {
      ...
      if(depth < 4)
      {
        ...
        for(uint64 member_idx = 0; member_idx < data_members.count && max_size > space_taken; member_idx += 1)
        {
...
StringView
rd_value_string_from_eval(Arena* arena, EV_StringFlags flags, uint32 default_radix, FNT_Tag font, float font_size, float max_size, E_Eval eval, E_Member* member, EV_ViewRuleList* view_rules)
{
  Temp scratch = scratch_begin(&arena, 1);
  String8List strs = {0};
...
void
rd_autocomp_lister_item_chunk_list_push(Arena* arena, RD_AutoCompListerItemChunkList* list, uint64 cap, RD_AutoCompListerItem* item)
{
...
StringView
rd_autocomp_query_word_from_input_string_off(StringView input, uint64 cursor_off)
{
  ...
  StringView query = str8_skip(str8_prefix(input, cursor_off), word_start_off);
  return query;
}

StringView
rd_autocomp_query_path_from_input_string_off(StringView input, uint64 cursor_off)
{
  // rjf: find start of path
  uint64 path_start_off = 0;
  {
    B32 single_quoted = 0;
    B32 double_quoted = 0;
    for(uint64 off = 0; off < input.size && off < cursor_off; off += 1)
...
rd_view_rule_autocomp_lister_params_from_input_cursor(Arena* arena, StringView string, uint64 cursor_off)
{
  RD_AutoCompListerParams params = {0};
  {
    ...
    int32 paren_nest = 0;
    int32 colon_nest = 0;
    StringView last_step_string = {0};
    for(uint64 idx = 0; idx < input_tokenize.tokens.count; idx += 1)
    {
      ...
      if(str8_match(token_string, ("("), 0) || str8_match(token_string, ("["), 0) || str8_match(token_string, ("{"), 0))
      {
        paren_nest += 1;
      }
      if(str8_match(token_string, (")"), 0) || str8_match(token_string, ("]"), 0) || str8_match(token_string, ("}"), 0))
      ...
      if(str8_match(token_string, (";"), 0) || str8_match(token_string, (","), 0))
      {
        for(;colon_nest > paren_nest; colon_nest -= 1)
        {
          if(last_step != 0)
    ...
    MD_ParseResult schema_parse = md_parse_from_text_tokens(scratch.arena, StringView(), spec.info.schema, schema_tokenize.tokens);
    MD_Node* schema_rule_root = md_child_from_string(schema_parse.root, ("x"), 0);
...
void
rd_set_autocomp_lister_query(UI_Key root_key, RD_AutoCompListerParams* params, StringView input, uint64 cursor_off)
{
  RD_Window* window = rd_window_from_handle(rd_regs().window);
...
{
  StringView result = push_str8_copy(arena, rd_state.string_search_string);
  return result;
}
...
RD_BindingList
rd_bindings_from_name(Arena* arena, StringView name)
{
  RD_BindingList result = {0};
  uint64 hash = d_hash_from_string(name);
  uint64 slot = hash%rd_state.key_map_table_size;
...
void
rd_bind_name(StringView name, RD_Binding binding)
{
...
void
rd_unbind_name(StringView name, RD_Binding binding)
{
  uint64 hash = d_hash_from_string(name);
  uint64 slot = hash%rd_state.key_map_table_size;
...
{
  String8List result = {0};
  for(uint64 idx = 0; idx < rd_state.key_map_table_size; idx += 1)
  {
...
rd_font_size_from_slot(RD_FontSlot slot)
{
  ...
  if(dpi != ws.last_dpi)
  {
    ...
    for(uint64 idx = 0; idx < ArrayCount(pt_sizes); idx += 1)
    {
      float ratio = pt_sizes[idx][0] / old_dpi;
      float new_pt_size = ratio*new_dpi;
      pt_sizes[idx][0] = (int32)new_pt_size;
    }
  }
  switch(slot)
  {
...
String8List
rd_cfg_strings_from_gfx(Arena* arena, StringView root_path, RD_CfgSrc source)
{
  ProfBeginFunction();
  static char* spaces = "                                                                                ";
  static char* slashes= "////////////////////////////////////////////////////////////////////////////////";
  ...
  //- rjf: write all entities
  {
    for EachEnumVal(RD_EntityKind, k)
    {
      ...
      for(RD_EntityNode* n = entities.first; n != 0; n = n.next)
      {
        ...
        {
          first = 0;
          StringView title_name = d_entity_kind_name_lower_plural_table[k];
        ...
        for(RD_Entity* e = entity; !rd_entity_is_nil(e); e = rec.next)
        {
          ...
          //- rjf: unpack entity info
          typedef uint32 EntityInfoFlags;
          enum
          {
          ...
          switch(info_flags)
          {
            ...
            default:
            {
              ...
              if(e.flags & RD_EntityFlag_HasColor)
              {
                ...
                uint32 rgba_hex = u32_from_rgba(rgba);
          ...
          if(rec.push_count == 0)
          {
            for(int64 pop_idx = 0; pop_idx < rec.pop_count + opened_brace; pop_idx += 1)
            {
              if(depth > 0)
              {
                depth -= 1;
  ...
  {
    str8_list_push(arena, &strs, ("/// exception code filters ////////////////////////////////////////////////////\n"));
    str8_list_push(arena, &strs, ("\n"));
    str8_list_push(arena, &strs, ("exception_code_filters:\n"));
    str8_list_push(arena, &strs, ("{\n"));
    ...
    for(RD_Window* window = rd_state.first_window; window != 0; window = window.next)
    {
      ...
      }
      {
        ...
        for(RD_Panel* p = root_panel; !rd_panel_is_nil(p); p = rec.next)
        {
          ...
          struct { StringView key; B32 value; } options[] =
          {
            {("tabs_on_bottom"),   p.tab_side == Side_Max},
          };
          B32 has_options = 0;
          ...
          for(RD_View* view = p.first_tab_view; !rd_view_is_nil(view); view = view.order_next)
          {
            ...
            // rjf: serialize views
            {
              ...
              {
                str8_list_push(arena, &strs, ("selected "));
              }
              {
                if(view.project_path.size != 0)
                {
                  Temp scratch = scratch_begin(&arena, 1);
                  StringView project_path_absolute = path_normalized_from_string(scratch.arena, view.project_path);
                  StringView project_path_relative = path_relative_dst_from_absolute_dst_src(scratch.arena, project_path_absolute, root_path);
              ...
              {
                Temp scratch = scratch_begin(&arena, 1);
                StringView query_raw = StringView(view.query_buffer, view.query_string_size);
                {
                  StringView query_file_path = rd_file_path_from_eval_string(scratch.arena, query_raw);
                ...
                StringView query_sanitized = escaped_from_raw_str8(scratch.arena, query_raw);
                str8_list_pushf(arena, &strs, "query:{\"%S\"} ", query_sanitized);
                scratch_end(scratch);
              }
              {
                StringView reserved_keys[] =
                {
                  ("project"),
                  ("query"),
                  ("selected"),
                    ...
                    n = rec.next)
                {
                  ...
                  if(!is_reserved_key && n != params_root)
                  {
                    ...
                    for(int32 pop_idx = 0; pop_idx < rec.pop_count; pop_idx += 1)
                    {
  ...
  if(source == RD_CfgSrc_User)
  {
    Temp scratch = scratch_begin(&arena, 1);
    StringView indent_str = ("                                                                                                             ");
    uint64 string_binding_pair_count = 0;
    ...
    {
      str8_list_push(arena, &strs, ("/// keybindings ///////////////////////////////////////////////////////////////\n"));
      str8_list_push(arena, &strs, ("\n"));
      str8_list_push(arena, &strs, ("keybindings:\n"));
      str8_list_push(arena, &strs, ("{\n"));
      for(uint64 idx = 0; idx < string_binding_pair_count; idx += 1)
      {
        ...
        StringJoin join = {(""), (" "), ("")};
        StringView event_flags_string = str8_list_join(scratch.arena, &modifiers_strings, &join);
  ...
  if(source == RD_CfgSrc_User)
  {
    ...
    StringView indent_str = ("                                                                                                             ");
    str8_list_push(arena, &strs, ("/// colors ////////////////////////////////////////////////////////////////////\n"));
    str8_list_push(arena, &strs, ("\n"));
    ...
    if(!is_preset)
    {
      ...
      {
        StringView color_name = rd_theme_color_cfg_string_table[color];
  ...
  if(source == RD_CfgSrc_User)
  {
    StringView code_font_path_escaped = escaped_from_raw_str8(arena, rd_state.cfg_code_font_path);
    StringView main_font_path_escaped = escaped_from_raw_str8(arena, rd_state.cfg_main_font_path);
    str8_list_push(arena, &strs, ("/// fonts /////////////////////////////////////////////////////////////////////\n"));
    str8_list_push(arena, &strs, ("\n"));
  ...
  //- rjf: serialize static settings
  {
    ...
    for EachEnumVal(RD_SettingCode, code)
    {
      ...
      if(current.set)
      {
        ...
        {
          first = 0;
          str8_list_push(arena, &strs, ("/// static settings ///////////////////////////////////////////////////////////\n"));
          str8_list_push(arena, &strs, ("\n"));
        }
        str8_list_pushf(arena, &strs, "%S: %i\n", rd_setting_code_lower_string_table[code], current.s32);
      }
  ...
  switch(event.cause)
  {
    ...
    case CTRL_EventCause_Finished:
    {
      if(thread != &ctrl_entity_nil)
      {
        ...
        dr_fancy_string_list_push_new(arena, &fstrs, ui_top_font(), ui_top_font_size(), ui_top_palette().text, (" completed step"));
      }
      else
      {
        dr_fancy_string_list_push_new(arena, &fstrs, ui_top_font(), ui_top_font_size(), ui_top_palette().text, ("Stopped"));
      }
    }break;
    ...
    case CTRL_EventCause_EntryPoint:
    {
      ...
      {
        dr_fancy_string_list_concat_in_place(&fstrs, &thread_fstrs);
        dr_fancy_string_list_push_new(arena, &fstrs, ui_top_font(), ui_top_font_size(), ui_top_palette().text, (" stopped at entry point"));
      }
      else
      {
        dr_fancy_string_list_push_new(arena, &fstrs, ui_top_font(), ui_top_font_size(), ui_top_palette().text, ("Stopped at entry point"));
    ...
    case CTRL_EventCause_UserBreakpoint:
    {
      if(thread != &ctrl_entity_nil)
      {
        ...
        dr_fancy_string_list_push_new(arena, &fstrs, ui_top_font(), ui_top_font_size(), ui_top_palette().text, ("  "));
        dr_fancy_string_list_concat_in_place(&fstrs, &thread_fstrs);
        dr_fancy_string_list_push_new(arena, &fstrs, ui_top_font(), ui_top_font_size(), ui_top_palette().text, (" hit a breakpoint"));
      }
    }break;
      ...
      if(thread != &ctrl_entity_nil)
      {
        ...
        switch(event.exception_kind)
        {
          default:
          {
            dr_fancy_string_list_push_new(arena, &fstrs, ui_top_font(), ui_top_font_size(), ui_top_palette().text, ("  "));
            dr_fancy_string_list_concat_in_place(&fstrs, &thread_fstrs);
            dr_fancy_string_list_push_new(arena, &fstrs, ui_top_font(), ui_top_font_size(), ui_top_palette().text, (" hit an exception - "));
          ...
          case CTRL_ExceptionKind_CppThrow:
          {
            dr_fancy_string_list_push_new(arena, &fstrs, ui_top_font(), ui_top_font_size(), ui_top_palette().text, ("  "));
            dr_fancy_string_list_concat_in_place(&fstrs, &thread_fstrs);
            dr_fancy_string_list_push_new(arena, &fstrs, ui_top_font(), ui_top_font_size(), ui_top_palette().text, (" hit a C++ exception - "));
            StringView exception_code_string = str8_from_u64(arena, event.exception_code, 16, 0, 0);
          ...
          case CTRL_ExceptionKind_MemoryRead:
          {
            ...
            dr_fancy_string_list_push_new(arena, &fstrs, ui_top_font(), ui_top_font_size(), ui_top_palette().text, (" hit an exception - "));
            StringView exception_code_string = str8_from_u64(arena, event.exception_code, 16, 0, 0);
            StringView exception_info_string = push_str8f(arena, "%S (Access violation reading 0x%I64x)", exception_code_string, event.vaddr_rng.min);
            dr_fancy_string_list_push_new(arena, &fstrs, ui_top_font(), ui_top_font_size(), ui_top_palette().text, exception_info_string);
          }break;
          case CTRL_ExceptionKind_MemoryWrite:
          {
            ...
            dr_fancy_string_list_push_new(arena, &fstrs, ui_top_font(), ui_top_font_size(), ui_top_palette().text, (" hit an exception - "));
            StringView exception_code_string = str8_from_u64(arena, event.exception_code, 16, 0, 0);
            StringView exception_info_string = push_str8f(arena, "%S (Access violation writing 0x%I64x)", exception_code_string, event.vaddr_rng.min);
            dr_fancy_string_list_push_new(arena, &fstrs, ui_top_font(), ui_top_font_size(), ui_top_palette().text, exception_info_string);
          }break;
          case CTRL_ExceptionKind_MemoryExecute:
          {
            ...
            dr_fancy_string_list_push_new(arena, &fstrs, ui_top_font(), ui_top_font_size(), ui_top_palette().text, (" hit an exception - "));
            StringView exception_code_string = str8_from_u64(arena, event.exception_code, 16, 0, 0);
            StringView exception_info_string = push_str8f(arena, "%S (Access violation executing 0x%I64x)", exception_code_string, event.vaddr_rng.min);
            dr_fancy_string_list_push_new(arena, &fstrs, ui_top_font(), ui_top_font_size(), ui_top_palette().text, exception_info_string);
          }break;
        }
      }
    ...
    case CTRL_EventCause_InterruptedByTrap:
    {
      ...
      dr_fancy_string_list_push_new(arena, &fstrs, ui_top_font(), ui_top_font_size(), ui_top_palette().text, ("  "));
      dr_fancy_string_list_concat_in_place(&fstrs, &thread_fstrs);
      dr_fancy_string_list_push_new(arena, &fstrs, ui_top_font(), ui_top_font_size(), ui_top_palette().text, (" hit a trap"));
    }break;
    ...
    case CTRL_EventCause_InterruptedByHalt:
    {
      ...
      dr_fancy_string_list_push_new(arena, &fstrs, ui_top_font(), ui_top_font_size(), ui_top_palette().text, ("Halted"));
    }break;
  }
  return fstrs;
}
...
rd_regs_fill_slot_from_string(RD_RegSlot slot, StringView string)
{
  ...
  switch(slot)
  {
    ...
    case RD_RegSlot_Cursor:
    {
      ...
      {
        log_user_error(("Couldn't interpret as a line number."));
      }
    }break;
...
rd_init(CmdLine* cmdln)
{
  ...
  log_select(rd_state.log);
  {
    ...
    os_write_data_to_file_path(rd_state.log_path, StringView());
    scratch_end(scratch);
  }
  ...
  // rjf: set up user / project paths
  {
    ...
    StringView user_cfg_path = cmd_line_string(cmdln, ("user"));
    StringView project_cfg_path = cmd_line_string(cmdln, ("project"));
    if(project_cfg_path.size == 0)
    {
      project_cfg_path = cmd_line_string(cmdln, ("profile"));
    }
    {
      StringView user_program_data_path = os_get_process_info().user_program_data_path;
    ...
    StringView cfg_src_paths[RD_CfgSrc_COUNT] = {user_cfg_path, project_cfg_path};
...
#pragma pack(pop)
  ...
  // rjf: set up initial browse path
  {
    ...
    StringView current_path_with_slash = push_str8f(scratch.arena, "%S/", current_path);
...
rd_frame()
{
  ...
  if(rd_state.frame_index > 32)
  {
    ...
    uint64 frame_time_history_avg_us = frame_time_history_sum_us/num_frames_in_history;
    ...
    int64 best_target_hz_frame_time_us_diff = max_S64;
    for(uint64 idx = 0; idx < ArrayCount(possible_alternate_hz_targets); idx += 1)
    {
      ...
      if(candidate <= target_hz)
      {
        ...
        int64 frame_time_us_diff = (int64)frame_time_history_avg_us - (int64)candidate_frame_time_us;
  ...
  ProfScope("loop - consume events in core, tick engine, and repeat") for(uint64 cmd_process_loop_idx = 0; cmd_process_loop_idx < 3; cmd_process_loop_idx += 1)
  {
    ...
    CTRL_Entity* module = ctrl_module_from_process_vaddr(process, rip_vaddr);
    uint64 rip_voff = ctrl_voff_from_vaddr(module, rip_vaddr);
    uint64 tls_root_vaddr = ctrl_query_cached_tls_root_vaddr_from_thread(d_state.ctrl_entity_store, thread.handle);
      ...
      //- rjf: add macros for all evallable control entities
      {
        ...
        for EachElement(idx, evallable_kinds)
        {
          ...
          for(CTRL_EntityNode* n = list.first; n != 0; n = n.next)
          {
            ...
            {
              e_string2expr_map_insert(scratch.arena, ctx.macro_map, ("current_thread"), expr);
            }
            if(kind == CTRL_EntityKind_Process && ctrl_handle_match(rd_base_regs().process, entity.handle))
            {
              e_string2expr_map_insert(scratch.arena, ctx.macro_map, ("current_process"), expr);
            }
            if(kind == CTRL_EntityKind_Module && ctrl_handle_match(rd_base_regs().module, entity.handle))
            {
              e_string2expr_map_insert(scratch.arena, ctx.macro_map, ("current_module"), expr);
            }
          }
        }
      }
    ...
    {
      // ev_auto_view_rule_table_push_new(scratch.arena, auto_view_rule_table, e_type_key_cons_base(type(CTRL_CheckB32)), ("checkbox"), 1);
      ev_auto_view_rule_table_push_new(scratch.arena, auto_view_rule_table, e_type_key_cons_base(type(CTRL_MetaEvalFrameArray)), ("slice"), 1);
      ev_auto_view_rule_table_push_new(scratch.arena, auto_view_rule_table, e_type_key_cons_base(type(CTRL_MachineMetaEval)),    ("scheduler_machine"), 1);
      ev_auto_view_rule_table_push_new(scratch.arena, auto_view_rule_table, e_type_key_cons_base(type(CTRL_ProcessMetaEval)),    ("scheduler_process"), 1);
      for EachElement(idx, rd_collection_name_table)
      {
      ...
      for(;rd_next_cmd(&cmd);) RD_RegsScope()
      {
        ...
        switch(kind)
        {
          ...
          case RD_CmdKind_RunCommand:
          {
            ...
            {
              RD_RegsScope(.cmd_name = StringView()) rd_push_cmd(cmd.regs.cmd_name, rd_regs());
            }
          ...
          case RD_CmdKind_OpenProject:
          {
...
#if 0
            {
              StringView file_root_key = (kind == RD_CmdKind_OpenUser ? ("user") : ("project"));
            ...
            //- rjf: normalize path
            StringView new_path = path_normalized_from_string(scratch.arena, rd_regs().file_path);
            ...
            B32 file_is_okay = 1;
            if(props.modified != 0 && data.size != 0 && !str8_match(str8_prefix(data, 9), ("// raddbg"), 0) && rd_state.cfg_cached_timestamp[cfg_src] != 0)
            {
              file_is_okay = 0;
            }
            ...
            //- rjf: load files
            StringView cfg_data[RD_CfgSrc_COUNT] = {0};
            uint64 cfg_timestamps[RD_CfgSrc_COUNT] = {0};
            if(file_is_okay)
            {
              for(RD_CfgSrc src = (RD_CfgSrc)0; src < RD_CfgSrc_COUNT; src = (RD_CfgSrc)(src+1))
              {
                ...
                StringView data = os_string_from_file_range(scratch.arena, file, r1u64(0, props.size));
                if(props.modified != 0)
                {
                  cfg_data[src] = data;
          ...
          case RD_CmdKind_ApplyProjectData:
          {
            ...
            //- rjf: get paths
            StringView cfg_path   = rd_cfg_path_from_src(src);
            StringView cfg_folder = str8_chop_last_slash(cfg_path);
            ...
            RD_CfgVal* filter_tables = rd_cfg_val_from_string(table, ("exception_code_filters"));
            ...
            //- rjf: apply fonts
            {
              ...
              {
                RD_CfgVal* code_font_val = rd_cfg_val_from_string(table, ("code_font"));
                ...
                StringView code_font_path = path_absolute_dst_from_relative_dst_src(scratch.arena, code_font_relative_path, cfg_folder);
                StringView main_font_path = path_absolute_dst_from_relative_dst_src(scratch.arena, main_font_relative_path, cfg_folder);
            ...
            RD_CfgVal* windows = rd_cfg_val_from_string(table, ("window"));
                ...
                window_tree = window_tree.next)
            {
              ...
              RD_SettingVal setting_vals[RD_SettingCode_COUNT] = {0};
              {
                ...
                MD_Node* size_node = md_child_from_string(window_tree.root, ("size"), 0);
                {
                  ...
                  {
                    x_u64 = (uint64)(preferred_monitor_size.x*2/3);
                  }
                  if(!try_u64_from_str8_c_rules(y_string, &y_u64))
                  {
                    y_u64 = (uint64)(preferred_monitor_size.y*2/3);
                  }
                  size.x = (float)x_u64;
                  size.y = (float)y_u64;
                }
                MD_Node* dpi_node = md_child_from_string(window_tree.root, ("dpi"), 0);
                StringView dpi_string = md_string_from_children(scratch.arena, dpi_node);
              ...
              MD_Node* panel_tree = md_child_from_string(window_tree.root, ("panels"), 0);
                ...
                if(!rd_panel_is_nil(panel))
                {
                  ...
                  for MD_EachNode(op, n.first)
                  {
                    ...
                    RD_ViewRuleInfoFlags view_rule_info_flags = view_rule_info.flags;
                    {
                      ...
                      view_is_selected = !md_node_is_nil(md_child_from_string(op, ("selected"), 0));
                      ...
                      {
                        StringView query_file_path = rd_file_path_from_eval_string(scratch.arena, view_query);
                        if(query_file_path.size != 0)
                        {
            ...
            RD_CfgVal* keybindings = rd_cfg_val_from_string(table, ("keybindings"));
            ...
            RD_CfgVal* color_preset = rd_cfg_val_from_string(table, ("color_preset"));
            ...
            RD_CfgVal* colors = rd_cfg_val_from_string(table, ("colors"));
            ...
            for EachEnumVal(RD_SettingCode, code)
            {
              StringView name = rd_setting_code_lower_string_table[code];
          ...
          case RD_CmdKind_WriteProjectData:
          {
            ...
            StringView data = str8_list_join(scratch.arena, &strs, 0);
            StringView data_indented = indented_from_string(scratch.arena, data);
          ...
          case RD_CmdKind_SetFileReplacementPath:
          {
            ...
            {
              RD_Cfg* user = rd_cfg_child_from_string(rd_state.root_cfg, ("user"));
              RD_Cfg* map = rd_cfg_new(user, ("file_path_map"));
              RD_Cfg* src = rd_cfg_new(map, ("source"));
              RD_Cfg* dst = rd_cfg_new(map, ("dest"));
          ...
          case RD_CmdKind_SwitchToPartnerFile:
          {
            ...
            {
              StringView file_path      = rd_file_path_from_eval_string(scratch.arena, StringView(view.query_buffer, view.query_string_size));
              StringView file_full_path = path_normalized_from_string(scratch.arena, file_path);
              StringView file_folder    = str8_chop_last_slash(file_full_path);
              StringView file_name      = str8_skip_last_slash(str8_chop_last_dot(file_full_path));
              StringView file_ext       = str8_skip_last_dot(file_full_path);
          ...
          {
            StringView path = path_normalized_from_string(scratch.arena, rd_regs().file_path);
            
            //- TODO(rjf): @cfg record file in project
            {
              RD_Cfg* project = rd_cfg_child_from_string(rd_state.root_cfg, ("project"));
              RD_CfgList recent_files = rd_cfg_child_list_from_string(scratch.arena, project, ("recent_files"));
              ...
              recent_files = rd_cfg_child_list_from_string(scratch.arena, project, ("recent_files"));
          ...
          case RD_CmdKind_ResetToCompactPanels:
          {
            ...
            {
              watch = rd_view_alloc();
              rd_view_equip_spec(watch, rd_view_rule_info_from_kind(RD_ViewRuleKind_Watch), StringView(), &md_nil_node);
            }
            if(layout == Layout_Default && rd_view_is_nil(locals))
            {
              locals = rd_view_alloc();
              rd_view_equip_spec(locals, rd_view_rule_info_from_kind(RD_ViewRuleKind_Locals), StringView(), &md_nil_node);
            }
            if(layout == Layout_Default && rd_view_is_nil(regs))
            {
              regs = rd_view_alloc();
              rd_view_equip_spec(regs, rd_view_rule_info_from_kind(RD_ViewRuleKind_Registers), StringView(), &md_nil_node);
            }
            if(layout == Layout_Default && rd_view_is_nil(globals))
            {
              globals = rd_view_alloc();
              rd_view_equip_spec(globals, rd_view_rule_info_from_kind(RD_ViewRuleKind_Globals), StringView(), &md_nil_node);
            }
            if(layout == Layout_Default && rd_view_is_nil(tlocals))
            {
              tlocals = rd_view_alloc();
              rd_view_equip_spec(tlocals, rd_view_rule_info_from_kind(RD_ViewRuleKind_ThreadLocals), StringView(), &md_nil_node);
            }
            if(rd_view_is_nil(types))
            {
              types = rd_view_alloc();
              rd_view_equip_spec(types, rd_view_rule_info_from_kind(RD_ViewRuleKind_Types), StringView(), &md_nil_node);
            }
            if(layout == Layout_Default && rd_view_is_nil(procs))
            {
              procs = rd_view_alloc();
              rd_view_equip_spec(procs, rd_view_rule_info_from_kind(RD_ViewRuleKind_Procedures), StringView(), &md_nil_node);
            }
            if(rd_view_is_nil(callstack))
            {
              callstack = rd_view_alloc();
              rd_view_equip_spec(callstack, rd_view_rule_info_from_kind(RD_ViewRuleKind_CallStack), StringView(), &md_nil_node);
            }
            if(rd_view_is_nil(breakpoints))
            {
              breakpoints = rd_view_alloc();
              rd_view_equip_spec(breakpoints, rd_view_rule_info_from_kind(RD_ViewRuleKind_Breakpoints), StringView(), &md_nil_node);
            }
            if(layout == Layout_Default && rd_view_is_nil(watch_pins))
            {
              watch_pins = rd_view_alloc();
              rd_view_equip_spec(watch_pins, rd_view_rule_info_from_kind(RD_ViewRuleKind_WatchPins), StringView(), &md_nil_node);
            }
            if(rd_view_is_nil(output))
            {
              output = rd_view_alloc();
              rd_view_equip_spec(output, rd_view_rule_info_from_kind(RD_ViewRuleKind_Output), StringView(), &md_nil_node);
            }
            if(rd_view_is_nil(targets))
            {
              targets = rd_view_alloc();
              rd_view_equip_spec(targets, rd_view_rule_info_from_kind(RD_ViewRuleKind_Targets), StringView(), &md_nil_node);
            }
            if(rd_view_is_nil(scheduler))
            {
              scheduler = rd_view_alloc();
              rd_view_equip_spec(scheduler, rd_view_rule_info_from_kind(RD_ViewRuleKind_Scheduler), StringView(), &md_nil_node);
            }
            if(rd_view_is_nil(modules))
            {
              modules = rd_view_alloc();
              rd_view_equip_spec(modules, rd_view_rule_info_from_kind(RD_ViewRuleKind_Modules), StringView(), &md_nil_node);
            }
            if(rd_view_is_nil(disasm))
            {
              disasm = rd_view_alloc();
              rd_view_equip_spec(disasm, rd_view_rule_info_from_kind(RD_ViewRuleKind_Disasm), StringView(), &md_nil_node);
            }
            if(layout == Layout_Default && rd_view_is_nil(memory))
            {
              memory = rd_view_alloc();
              rd_view_equip_spec(memory, rd_view_rule_info_from_kind(RD_ViewRuleKind_Memory), StringView(), &md_nil_node);
            }
            if(code_views.count == 0 && rd_view_is_nil(getting_started))
            {
              getting_started = rd_view_alloc();
              rd_view_equip_spec(getting_started, rd_view_rule_info_from_kind(RD_ViewRuleKind_GettingStarted), StringView(), &md_nil_node);
            }
            ...
            if(name.size != 0)
            {
              ...
              if(!name_resolved)
              {
                ...
                if(quote_pos < name.size)
                {
                  ...
                  uint64 ender_quote_pos = str8_find_needle(file_part_of_name, 0, ("\""), 0);
                ...
                // rjf: get source path
                RD_View* src_view = rd_view_from_handle(rd_regs().view);
                StringView src_file_path = rd_file_path_from_eval_string(scratch.arena, StringView(src_view.query_buffer, src_view.query_string_size));
          ...
          case RD_CmdKind_FindCodeLocation:
          {
            ...
            if(file_path.size != 0 && src_code_dst_panel != &rd_nil_panel)
            {
              ...
              {
                RD_View* view = rd_view_alloc();
                StringView file_path_query = rd_eval_string_from_file_path(scratch.arena, file_path);
            ...
            if(process != &ctrl_entity_nil && vaddr != 0 && disasm_dst_panel != &rd_nil_panel)
            {
              ...
              {
                RD_View* view = rd_view_alloc();
                rd_view_equip_spec(view, rd_view_rule_info_from_kind(RD_ViewRuleKind_Disasm), StringView(), &md_nil_node);
                rd_panel_insert_tab_view(dst_panel, dst_panel.last_tab_view, view);
                dst_view = view;
              }
          ...
          case RD_CmdKind_Filter:
          {
            ...
            {
              view.is_filtering ^= 1;
              view.query_cursor = txt_pt(1, 1+(int64)view.query_string_size);
              view.query_mark = txt_pt(1, 1);
            }
          }break;
          ...
          case RD_CmdKind_CompleteQuery:
          {
            ...
            if(slot != RD_RegSlot_Null && !(ws.query_cmd_regs_mask[slot/64] & (1ull<<(slot%64))))
            {
              ...
              MemoryCopy((uint8 *)(ws.query_cmd_regs) + offset_range_in_regs.min,
                         (uint8 *)(regs_copy) + offset_range_in_regs.min,
          ...
          case RD_CmdKind_RelocateEntity:
          {
            ...
            if(rd_regs().vaddr != 0)
            {
              ...
              rd_entity_equip_name(location, StringView());
            }
            if(rd_regs().file_path.size != 0)
            {
            ...
            if(file_path.size != 0 || string.size != 0 || vaddr != 0)
            {
              //- TODO(rjf): @cfg add/toggle breakpoint
              {
                ...
                if(!removed_already_existing)
                {
                  ...
                  RD_Cfg* loc = rd_cfg_new(bp, ("location"));
                  if(vaddr != 0)
                  {
          ...
          case RD_CmdKind_ToggleWatchPin:
          {
            ...
            //- TODO(rjf): @cfg add/toggle watch pin
            {
              ...
              if(kind == RD_CmdKind_ToggleWatchPin)
              {
                ...
                {
                  RD_Cfg* wp = n.v;
                  RD_Cfg* name = rd_cfg_child_from_string(wp, ("name"));
          ...
          case RD_CmdKind_AddTarget:
          {
            ...
            StringView working_dir = str8_chop_last_slash(rd_regs().file_path);
            if(working_dir.size != 0)
            {
              StringView working_dir_path = push_str8f(scratch.arena, "%S/", working_dir);
          ...
          case RD_CmdKind_SelectUnwind:
          {
            ...
            if(rd_regs().unwind_count < rich_unwind.concrete_frame_count)
            {
              ...
              uint64 rip_vaddr = regs_rip_from_arch_block(thread.arch, frame.regs);
              CTRL_Entity* module = ctrl_module_from_process_vaddr(process, rip_vaddr);
              rd_state.base_regs.v.module = module.handle;
          ...
          case RD_CmdKind_DownOneFrame:
          {
            ...
            CTRL_Unwind base_unwind = d_query_cached_unwind_from_thread(thread);
            CTRL_CallStack rich_unwind = ctrl_call_stack_from_unwind(scratch.arena, di_scope, process, &base_unwind);
            uint64 crnt_unwind_idx = rd_state.base_regs.v.unwind_count;
            uint64 crnt_inline_dpt = rd_state.base_regs.v.inline_depth;
            uint64 next_unwind_idx = crnt_unwind_idx;
            uint64 next_inline_dpt = crnt_inline_dpt;
    ...
    for(D_EventNode* n = engine_events.first; n != 0; n = n.next)
    {
      ...
      switch(evt.kind)
      {
        ...
        case D_EventKind_Stop:
        {
          ...
          uint64 test_cached_vaddr = ctrl_query_cached_rip_from_thread(d_state.ctrl_entity_store, thread.handle);
    ...
    for(RD_Window* w = rd_state.first_window; w != 0; w = w.next)
    {
      for(RD_Panel* p = w.root_panel; !rd_panel_is_nil(p); p = rd_panel_rec_depth_first_pre(p).next)
      {
        for(RD_View* v = p.first_tab_view; !rd_view_is_nil(v); v = v.order_next)
        {
          ...
          StringView eval_string = StringView(v.query_buffer, v.query_string_size);
