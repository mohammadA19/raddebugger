...
global byte *ipc_shared_memory_base = 0;
global byte  ipc_s2m_ring_buffer[MB(4)] = {0};
global ulong ipc_s2m_ring_write_pos = 0;
global ulong ipc_s2m_ring_read_pos = 0;
  ...
  for(;;)
  {
    if(os_semaphore_take(ipc_signal_semaphore, max_U64))
    {
      if(os_semaphore_take(ipc_lock_semaphore, max_U64))
      {
        ...
        String8 msg = str8((byte *)(ipc_info+1), ipc_info->msg_size);
        msg.size = Min(msg.size, IPC_SHARED_MEMORY_BUFFER_SIZE - sizeof(IPCInfo));
        OS_MutexScope(ipc_s2m_ring_mutex) for(;;)
        {
          ulong unconsumed_size = ipc_s2m_ring_write_pos - ipc_s2m_ring_read_pos;
          ulong available_size = (sizeof(ipc_s2m_ring_buffer) - unconsumed_size);
          if(available_size >= sizeof(ulong)+sizeof(msg.size))
          {
...
entry_point(CmdLine *cmd_line)
{
  ...
  for(ulong idx = 0; idx < ArrayCount(output_handles); idx += 1)
  {
    B32 duplicate = 0;
    for(ulong idx2 = 0; idx2 < idx; idx2 += 1)
    {
  ...
  switch(exec_mode)
  {
    ...
    case ExecMode_Normal:
    {
      ...
      {
        Temp scratch = scratch_begin(0, 0);
        uint instance_pid = os_get_process_info()->pid;
        ...
        ipc_shared_memory_base = (byte *)os_shared_memory_view_open(ipc_shared_memory, r1u64(0, IPC_SHARED_MEMORY_BUFFER_SIZE));
          ...
          //- rjf: consume IPC messages, dispatch UI commands
          {
            ...
            OS_MutexScope(ipc_s2m_ring_mutex)
            {
              ...
              if(unconsumed_size >= sizeof(ulong))
              {
                ...
                msg.str = push_array(scratch.arena, byte, msg.size);
            ...
            if(msg.size != 0)
            {
              ...
              if(dst_window != 0)
              {
                ...
                ulong first_space_pos = str8_find_needle(msg, 0, str8_lit(" "), 0);
    ...
    case ExecMode_IPCSender:
    {
      ...
      if(dst_pid == 0)
      {
        uint this_pid = os_get_process_info()->pid;
      ...
      {
        IPCInfo *ipc_info = (IPCInfo *)ipc_shared_memory_base;
        byte *buffer = (byte *)(ipc_info+1);
        ulong buffer_max = IPC_SHARED_MEMORY_BUFFER_SIZE - sizeof(IPCInfo);
        StringJoin join = {str8_lit(""), str8_lit(" "), str8_lit("")};
