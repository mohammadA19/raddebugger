...
global uint8 *ipc_sender2main_shared_memory_base = 0;
global OS_Handle ipc_main2sender_signal_semaphore = {0};
global OS_Handle ipc_main2sender_lock_semaphore = {0};
global uint8 *ipc_main2sender_shared_memory_base = 0;
global uint8  ipc_s2m_ring_buffer[MB(4)] = {0};
global uint64 ipc_s2m_ring_write_pos = 0;
global uint64 ipc_s2m_ring_read_pos = 0;
...
ipc_signaler_thread__entry_point(void *p)
{
  ...
  for(;;)
  {
    if(os_semaphore_take(ipc_sender2main_signal_semaphore, max_uint64))
    {
      if(os_semaphore_take(ipc_sender2main_lock_semaphore, max_uint64))
      {
        ...
        OS_MutexScope(ipc_s2m_ring_mutex) for(;;)
        {
          ...
          os_condition_variable_wait(ipc_s2m_ring_cv, ipc_s2m_ring_mutex, max_uint64);
        }
  ...
  switch(exec_mode)
  {
    ...
    case ExecMode_Normal:
    {
      ...
      //- rjf: setup initial target from command line args
      {
        ...
        String8List target_args = {0};
        {
          ...
          for(uint64 idx = 1; idx < cmd_line->argc; idx += 1)
          {
      ...
      //- rjf: set up shared resources for ipc to this instance; launch IPC signaler thread
      {
        ...
        ipc_sender2main_shared_memory_base = (uint8 *)os_shared_memory_view_open(ipc_sender2main_shared_memory, r1u64(0, IPC_SHARED_MEMORY_BUFFER_SIZE));
        ...
        ipc_main2sender_shared_memory_base = (uint8 *)os_shared_memory_view_open(ipc_main2sender_shared_memory, r1u64(0, IPC_SHARED_MEMORY_BUFFER_SIZE));
    ...
    case ExecMode_IPCSender:
    {
      ...
      if(dst_pid != 0 &&
         ...
         os_semaphore_take(ipc_sender2main_lock_semaphore, max_uint64))
      {
        wrote_message = 1;
        IPCInfo *ipc_info = (IPCInfo *)ipc_sender2main_shared_memory_base;
        uint8 *buffer = (uint8 *)(ipc_info+1);
      ...
      {
        if(os_semaphore_take(ipc_main2sender_lock_semaphore, max_uint64))
        {
          IPCInfo *ipc_info = (IPCInfo *)ipc_main2sender_shared_memory_base;
          String8 msg = str8((uint8 *)(ipc_info+1), ipc_info->msg_size);
          msg.size = Min(msg.size, IPC_SHARED_MEMORY_BUFFER_SIZE - sizeof(IPCInfo));
