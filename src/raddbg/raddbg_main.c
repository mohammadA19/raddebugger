  ...
  for (;;)
  {
    if (os_semaphore_take(ipc_sender2main_signal_semaphore, max_U64))
    {
      if (os_semaphore_take(ipc_sender2main_lock_semaphore, max_U64))
      {
        ...
        string msg = str8((U8 *)(ipc_info+1), ipc_info.msg_size);
...
entry_point(CmdLine *cmd_line)
{
  ...
  U64 jit_addr = 0;
  {
    ...
    string jit_pid_string = cmd_line_string(cmd_line, ("jit_pid"));
    string jit_code_string = cmd_line_string(cmd_line, ("jit_code"));
    string jit_addr_string = cmd_line_string(cmd_line, ("jit_addr"));
  ...
  switch (exec_mode)
  {
    ...
    case ExecMode_Normal:
    {
      ...
      //- rjf: set up shared resources for ipc to this instance; launch IPC signaler thread
      {
        ...
        string ipc_sender2main_shared_memory_name = push_str8f(scratch.arena, "_raddbg_ipc_sender2main_shared_memory_%i_", instance_pid);
        string ipc_sender2main_signal_semaphore_name = push_str8f(scratch.arena, "_raddbg_ipc_sender2main_signal_semaphore_%i_", instance_pid);
        string ipc_sender2main_lock_semaphore_name = push_str8f(scratch.arena, "_raddbg_ipc_sender2main_lock_semaphore_%i_", instance_pid);
        ...
        string ipc_main2sender_shared_memory_name = push_str8f(scratch.arena, "_raddbg_ipc_main2sender_shared_memory_%i_", instance_pid);
        string ipc_main2sender_signal_semaphore_name = push_str8f(scratch.arena, "_raddbg_ipc_main2sender_signal_semaphore_%i_", instance_pid);
        string ipc_main2sender_lock_semaphore_name = push_str8f(scratch.arena, "_raddbg_ipc_main2sender_lock_semaphore_%i_", instance_pid);
        ...
        for (B32 quit = 0; !quit;)
        {
          ...
          if (ipc_command_frame)
          {
               ...
               os_semaphore_take(ipc_main2sender_lock_semaphore, os_now_microseconds()+5000000))
            {
              ...
              StringJoin join = {(""), ("\0"), ("")};
              string msg = str8_list_join(scratch.arena, &rd_state.cmd_outputs, &join);
    ...
    case ExecMode_IPCSender:
    {
      ...
      {
        string dst_pid_string = cmd_line_string(cmd_line, ("pid"));
        U64 dst_pid_u64 = 0;
      ...
      string ipc_sender2main_shared_memory_name = push_str8f(scratch.arena, "_raddbg_ipc_sender2main_shared_memory_%i_", dst_pid);
      string ipc_sender2main_signal_semaphore_name = push_str8f(scratch.arena, "_raddbg_ipc_sender2main_signal_semaphore_%i_", dst_pid);
      string ipc_sender2main_lock_semaphore_name = push_str8f(scratch.arena, "_raddbg_ipc_sender2main_lock_semaphore_%i_", dst_pid);
      OS_Handle ipc_sender2main_shared_memory = os_shared_memory_alloc(IPC_SHARED_MEMORY_BUFFER_SIZE, ipc_sender2main_shared_memory_name);
      ipc_sender2main_shared_memory_base = (U8 *)os_shared_memory_view_open(ipc_sender2main_shared_memory, r1u64(0, IPC_SHARED_MEMORY_BUFFER_SIZE));
      ipc_sender2main_signal_semaphore = os_semaphore_alloc(0, 1, ipc_sender2main_signal_semaphore_name);
      ipc_sender2main_lock_semaphore = os_semaphore_alloc(1, 1, ipc_sender2main_lock_semaphore_name);
      string ipc_main2sender_shared_memory_name = push_str8f(scratch.arena, "_raddbg_ipc_main2sender_shared_memory_%i_", dst_pid);
      string ipc_main2sender_signal_semaphore_name = push_str8f(scratch.arena, "_raddbg_ipc_main2sender_signal_semaphore_%i_", dst_pid);
      string ipc_main2sender_lock_semaphore_name = push_str8f(scratch.arena, "_raddbg_ipc_main2sender_lock_semaphore_%i_", dst_pid);
      OS_Handle ipc_main2sender_shared_memory = os_shared_memory_alloc(IPC_SHARED_MEMORY_BUFFER_SIZE, ipc_main2sender_shared_memory_name);
      ipc_main2sender_shared_memory_base = (U8 *)os_shared_memory_view_open(ipc_main2sender_shared_memory, r1u64(0, IPC_SHARED_MEMORY_BUFFER_SIZE));
      ipc_main2sender_signal_semaphore = os_semaphore_alloc(0, 1, ipc_main2sender_signal_semaphore_name);
      ipc_main2sender_lock_semaphore = os_semaphore_alloc(1, 1, ipc_main2sender_lock_semaphore_name);
         ...
         os_semaphore_take(ipc_sender2main_lock_semaphore, max_U64))
      {
        ...
        StringJoin join = {(""), (" "), ("")};
        string msg = str8_list_join(scratch.arena, &parts, &join);
        ipc_info.msg_size = Min(buffer_max, msg.size);
        MemoryCopy(buffer, msg.str, ipc_info.msg_size);
