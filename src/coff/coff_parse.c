...
coff_section_table_from_data(Arena *arena, String8 data, Rng1U64 section_table_range)
{
  ...
  COFF_SectionHeader **section_table = /* no zero */ new COFF_SectionHeader *[section_count+1];
  section_table[0] = new COFF_SectionHeader[1];
...
{
  COFF_Symbol16Node *node = new COFF_Symbol16Node[1];
  node->next = 0;
  node->data = symbol;
...
{
  COFF_RelocNode *node = new COFF_RelocNode[1];
  node->data = reloc;
  SLLQueuePush(list->first, list->last, node);
  ++list->count;
  return node;
...
coff_resource_list_from_data(Arena *arena, String8 data)
{
  ...
  for (cursor = 0 ; cursor < data.size; ) {
    COFF_ParsedResourceNode *node = new COFF_ParsedResourceNode[1];
                    ...
                    String8         data)
{
  ...
  COFF_ResourceHeaderPrefix *prefix      = new COFF_ResourceHeaderPrefix[1];
