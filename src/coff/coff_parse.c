...
coff_section_table_from_data(Arena *arena, String8 data, Rng1uint64 section_table_range)
{
  uint64                  section_count = dim_1u64(section_table_range) / sizeof(COFF_SectionHeader);
...
internal COFF_ParsedSymbol
coff_parse_symbol(COFF_FileHeaderInfo header, String8 string_table, String8 symbol_table, uint32 symbol_idx)
{
...
coff_symbol_array_from_data_16(Arena *arena, String8 raw_coff, uint64 symbol_array_off, uint64 symbol_count)
{
  ...
  Rng1uint64        sym16_arr_range = rng_1u64(symbol_array_off, symbol_array_off + sizeof(COFF_Symbol16) * symbol_count);
...
internal COFF_Symbol32Array
coff_symbol_array_from_data_32(Arena *arena, String8 data, uint64 symbol_array_off, uint64 symbol_count)
{
...
internal COFF_Symbol32Array
coff_symbol_array_from_data(Arena *arena, String8 data, uint64 symbol_array_off, uint64 symbol_count, uint64 symbol_size)
{
  COFF_Symbol32Array result = {0};
  switch (symbol_size) {
...
internal void
coff_parse_secdef(COFF_ParsedSymbol symbol, B32 is_big_obj, COFF_ComdatSelectType *selection_out, uint32 *number_out, uint32 *length_out, uint32 *check_sum_out)
{
  ...
  if (is_big_obj) {
    ...
    if (number_out)    *number_out    = sd->number_lo | ((uint32)sd->number_hi << 16);
  } else {
...
coff_reloc_info_from_section_header(String8 data, COFF_SectionHeader *header)
{
  ...
  if (header->flags & COFF_SectionFlag_LnkNRelocOvfl && header->reloc_count == max_uint16) {
    COFF_Reloc counter;
...
{
  AssertAlways(string.size <= max_uint16);
...
internal uint64
coff_read_resource_id_utf16(String8 raw_res, uint64 off, COFF_ResourceID16 *id_out)
{
  uint64 cursor = off;
...
internal uint64
coff_read_resource(Arena *arena, String8 raw_res, uint64 off, COFF_ParsedResource *res_out)
{
...
internal uint64
coff_parse_archive_member_header(String8 raw_archive, uint64 offset, COFF_ParsedArchiveMemberHeader *header_out)
{
...
coff_parse_first_archive_member(COFF_ArchiveMember *member)
{
  ...
  Rng1uint64 member_offsets_range = rng_1u64(cursor, cursor + symbol_count * sizeof(uint32));
  cursor += dim_1u64(member_offsets_range);
  
  Rng1uint64 string_table_range = rng_1u64(cursor, member->data.size);
...
coff_parse_second_archive_member(COFF_ArchiveMember *member)
{
  ...
  if (str8_match_lit("/", member->header.name, 0)) {
    ...
    Rng1uint64 member_offsets_range = rng_1u64(cursor, cursor + member_count * sizeof(uint32));
    cursor += dim_1u64(member_offsets_range);
    
    uint32 symbol_count = 0;
    cursor += str8_deserial_read_struct(member->data, cursor, &symbol_count);
    
    Rng1uint64 symbol_indices_range = rng_1u64(cursor, cursor + symbol_count * sizeof(uint16));
...
internal uint64
coff_parse_import(String8 raw_archive_member, uint64 offset, COFF_ParsedArchiveImportHeader *header_out)
{
  COFF_ImportHeader *header = str8_deserial_get_raw_ptr(raw_archive_member, offset, sizeof(*header));
  if (header) {
    Rng1uint64 data_range  = rng_1u64(offset + sizeof(*header), offset + sizeof(*header) + header->data_size);
...
coff_archive_member_from_offset(String8 raw_archive, uint64 offset)
{
...
internal B32
coff_regular_archive_member_iter_next(String8 raw_archive, uint64 *offset, COFF_ArchiveMember *member_out)
{
  B32 is_parsed = 0;
...
internal B32
coff_thin_archive_member_iter_next(String8 raw_archive, uint64 *offset, COFF_ArchiveMember *member_out)
{
  B32 is_parsed = 0;
  ...
  if (member_out->header.is_end_correct) {
    Rng1uint64 data_in_archive_range = {0};
