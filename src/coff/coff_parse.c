...
B32
coff_is_big_obj(StringView raw_coff)
{
  B32 is_big_obj = 0;
  if (raw_coff.size >= sizeof(COFF_BigObjHeader)) {
    COFF_BigObjHeader* file_header32 = (COFF_BigObjHeader*)(raw_coff.Ptr);
...
B32
coff_is_obj(StringView raw_coff)
{
  B32 is_obj = 0;
  
  if (raw_coff.size >= sizeof(COFF_FileHeader)) {
    COFF_FileHeader* header = (COFF_FileHeader*)(raw_coff.Ptr);
    ...
    if (is_machine_type_valid) {
      ...
      uint64 section_count = header.section_count;
      uint64 section_hdr_opl_off = sizeof(*header) + section_count*sizeof(COFF_SectionHeader);
      if (raw_coff.size >= section_hdr_opl_off) {
        
        COFF_SectionHeader* section_hdrs = (COFF_SectionHeader*)(raw_coff.Ptr + sizeof(*header));
        ...
        for (COFF_SectionHeader* sec_hdr = section_hdrs;
          ...
          if (!(sec_hdr.flags & COFF_SectionFlag_CntUninitializedData)) {
            uint64 min = sec_hdr.foff;
            uint64 max = min + sec_hdr.fsize;
        ...
        if (is_sect_range_valid) {
          ...
          uint64 symbol_table_off = header.symbol_table_foff;
          uint64 symbol_table_size = sizeof(COFF_Symbol16)*header.symbol_count;
          uint64 symbol_table_opl_off = symbol_table_off + symbol_table_size;
...
COFF_FileHeaderInfo
coff_file_header_info_from_data(StringView raw_coff)
{
  COFF_FileHeaderInfo info = {0};
  if (coff_is_big_obj(raw_coff)) {
    COFF_BigObjHeader* header32 = (COFF_BigObjHeader*)raw_coff.Ptr;
    info.is_big_obj             = 1;
    ...
    COFF_FileHeader* header16  = (COFF_FileHeader*)raw_coff.Ptr;
    info.is_big_obj            = 0;
...
COFF_ParsedSymbol
coff_parse_symbol32(StringView raw_coff, uint64 string_table_off, COFF_Symbol32* sym32)
{
...
COFF_ParsedSymbol
coff_parse_symbol16(StringView raw_coff, uint64 string_table_off, COFF_Symbol16* sym16)
{
  COFF_ParsedSymbol result = {0};
  ...
  if (sym16.section_number == COFF_Symbol_DebugSection16) {
    ...
    result.section_number = (uint32)sym16.section_number;
  }
...
COFF_Symbol32Array
coff_symbol_array_from_data_16(Arena* arena, StringView raw_coff, uint64 symbol_array_off, uint64 symbol_count)
{
  ...
  StringView        raw_sym16_arr   = str8_substr(raw_coff, sym16_arr_range);
  COFF_Symbol16* sym16_arr       = (COFF_Symbol16 *)raw_sym16_arr.Ptr;
  
  for (uint64 isymbol = 0, count = raw_sym16_arr.size / sizeof(COFF_Symbol16); isymbol < count; isymbol += 1) {
    ...
    if (sym16.section_number == COFF_Symbol_DebugSection16) {
      ...
      sym32.section_number = (uint32)sym16.section_number;
    }
    ...
    // copy aux symbols
    for (uint64 iaux = isymbol+1, iaux_hi = Min(count, iaux+sym16.aux_symbol_count); iaux < iaux_hi; iaux += 1) {
      ...
      MemoryZero((uint8 *)aux32 + sizeof(COFF_Symbol16), sizeof(COFF_Symbol32)-sizeof(COFF_Symbol16));
    }
...
COFF_Symbol32Array
coff_symbol_array_from_data_32(Arena* arena, StringView data, uint64 symbol_array_off, uint64 symbol_count)
{
  COFF_Symbol32Array result;
  result.count = symbol_count;
  result.v     = (COFF_Symbol32 *)(data.Ptr + symbol_array_off);
  return result;
}

COFF_Symbol32Array
coff_symbol_array_from_data(Arena* arena, StringView data, uint64 symbol_array_off, uint64 symbol_count, uint64 symbol_size)
{
  COFF_Symbol32Array result = {0};
  switch (symbol_size) {
...
COFF_SymbolValueInterpType
coff_interp_symbol(uint32 section_number, uint32 value, COFF_SymStorageClass storage_class)
{
...
COFF_RelocInfo
coff_reloc_info_from_section_header(StringView data, COFF_SectionHeader* header)
{
  COFF_RelocInfo result = {0};
  if (header.flags & COFF_SectionFlag_LnkNRelocOvfl && header.reloc_count == max_U16) {
    COFF_Reloc counter;
    uint64 read_size = str8_deserial_read_struct(data, header.relocs_foff, &counter);
...
StringView
coff_resource_string_from_str16(Arena* arena, Span<char16> str)
{
  AssertAlways(str.Length <= max_U16);
  uint16 size16 = (uint16)str.Length;
  
  uint16* buffer = push_array_no_zero(arena, uint16, size16 + 1);
  MemoryCopy(buffer + 0, &size16,    sizeof(size16));
  MemoryCopy(buffer + 1, str.Ptr, size16 * sizeof(str[0]));
  
  return str8_array(buffer, size16 + 1);
}

StringView
coff_resource_string_from_str8(Arena* arena, StringView str)
{
  Temp scratch = scratch_begin(&arena, 1);
  Span<char16> string16 = str16_from_8(scratch.arena, str);
...
StringView
coff_resource_number_from_u16(Arena* arena, uint16 number)
{
  uint16* buffer = push_array_no_zero(arena, uint16, 2);
  buffer[0] = max_U16;
  buffer[1] = number;
...
coff_utf8_resource_id_from_utf16(Arena* arena, COFF_ResourceID16* id_16)
{
  ...
  switch (id_16.type) {
    ...
    case COFF_ResourceIDType_String: {
      id.u.str = str8_from_16(arena, id_16.u.str);
    } break;
    default: InvalidPath;
  }
  return id;
...
uint64
coff_read_resource_id_utf16(StringView raw_res, uint64 off, COFF_ResourceID16* id_out)
{
  uint64 cursor = off;
  ...
  if (flag == max_U16) {
    ...
    cursor += str8_deserial_read_windows_utf16_string16(raw_res, cursor, &id_out.u.str);
  }
...
uint64
coff_read_resource(Arena* arena, StringView raw_res, uint64 off, COFF_ParsedResource* res_out)
{
  StringView raw_header    = str8_skip(raw_res, off);
  uint64     header_cursor = 0;
  ...
  // read data
  uint64 data_read_size = str8_deserial_read_block(raw_res, off + prefix.header_size, prefix.data_size, &res_out.data);
  Assert(prefix.data_size == data_read_size);
  
  // compute read size
  uint64 read_size = Max(prefix.header_size, sizeof(prefix)) + AlignPow2(prefix.data_size, COFF_ResourceAlign);
  return read_size;
}

COFF_ParsedResourceList
coff_resource_list_from_data(Arena* arena, StringView data)
{
...
coff_write_resource_id(Arena* arena, COFF_ResourceID id)
{
  ...
  switch (id.type) {
    ...
    case COFF_ResourceIDType_String: {
      result = coff_resource_string_from_str8(arena, id.u.str);
    } break;
    default: InvalidPath;
  }
  return result;
                    ...
                    StringView         data)
{
  ...
  StringView                    packed_type = coff_write_resource_id(scratch.arena, type);
  StringView                    packed_name = coff_write_resource_id(scratch.arena, name);
  ...
  // align
  uint64 align_size = AlignPow2(list.total_size, COFF_ResourceAlign) - list.total_size;
  uint8* align      = push_array(scratch.arena, uint8, align_size);
  str8_list_push(scratch.arena, &list, StringView(align, align_size));
  
  // join
  StringView res = str8_list_join(arena, &list, 0);
...
coff_resource_id_compar(void* raw_a, void* raw_b)
{
  ...
  if (a.type == b.type) {
    switch (a.type) {
      ...
      case COFF_ResourceIDType_String: cmp = strncmp((char *)a.u.str.Ptr, (char *)b.u.str.Ptr, Min(a.u.str.Length, b.u.str.Length)); break;
...
B32
coff_is_import(StringView raw_archive_member)
{
  B32 is_import = 0;
  if (raw_archive_member.size >= sizeof(uint16)*2) {
    uint16* sig1 = (uint16*)raw_archive_member.Ptr;
    uint16* sig2 = sig1 + 1;
...
COFF_DataType
coff_data_type_from_data(StringView raw_archive_member)
{
...
B32
coff_is_regular_archive(StringView raw_archive)
{
  B32 is_archive = 0;
  uint8 sig[sizeof(g_coff_archive_sig)];
...
B32
coff_is_thin_archive(StringView raw_archive)
{
  B32 is_archive = 0;
  uint8 sig[sizeof(g_coff_thin_archive_sig)];
  if (str8_deserial_read_struct(raw_archive, 0, &sig) == sizeof(sig)) {
...
COFF_ArchiveType
coff_archive_type_from_data(StringView raw_archive)
{
...
uint64
coff_parse_archive_member_header(StringView raw_archive, uint64 offset, COFF_ParsedArchiveMemberHeader* header_out)
{
  COFF_ArchiveMemberHeader* header = str8_deserial_get_raw_ptr(raw_archive, offset, sizeof(*header));
  if (header) {
    StringView name     = str8_skip_chop_whitespace(str8_cstring_capped(header.name,     header.name     + sizeof(header.name)    ));
    StringView date     = str8_skip_chop_whitespace(str8_cstring_capped(header.date,     header.date     + sizeof(header.date)    ));
    StringView user_id  = str8_skip_chop_whitespace(str8_cstring_capped(header.user_id,  header.user_id  + sizeof(header.user_id) ));
    StringView group_id = str8_skip_chop_whitespace(str8_cstring_capped(header.group_id, header.group_id + sizeof(header.group_id)));
    StringView mode     = str8_skip_chop_whitespace(str8_cstring_capped(header.mode,     header.mode     + sizeof(header.mode)    ));
    StringView size     = str8_skip_chop_whitespace(str8_cstring_capped(header.size,     header.size     + sizeof(header.size)    ));
    StringView end      = str8_cstring_capped(header.end, header.end + sizeof(header.end));
    
    uint32 data_size = u32_from_str8(size, 10);
    uint64 data_off  = offset + sizeof(COFF_ArchiveMemberHeader);
...
coff_parse_first_archive_member(COFF_ArchiveMember* member)
{
  ...
  Rng1U64 member_offsets_range = rng_1u64(cursor, cursor + symbol_count * sizeof(uint32));
  ...
  StringView  raw_member_offsets  = str8_substr(member.data, member_offsets_range);
  uint32*     member_offsets      = (uint32 *)raw_member_offsets.Ptr;
  uint64      member_offset_count = raw_member_offsets.size / sizeof(member_offsets[0]);
...
coff_parse_second_archive_member(COFF_ArchiveMember* member)
{
  ...
  if (str8_match_lit("/", member.header.name, 0)) {
    ...
    Rng1U64 member_offsets_range = rng_1u64(cursor, cursor + member_count * sizeof(uint32));
    cursor += dim_1u64(member_offsets_range);
    
    uint32 symbol_count = 0;
    cursor += str8_deserial_read_struct(member.data, cursor, &symbol_count);
    
    Rng1U64 symbol_indices_range = rng_1u64(cursor, cursor + symbol_count * sizeof(uint16));
    cursor += dim_1u64(symbol_indices_range);
    
    Rng1U64 string_table_range = rng_1u64(cursor, member.data.size);
    
    StringView raw_member_offsets = str8_substr(member.data, member_offsets_range);
    StringView raw_indices        = str8_substr(member.data, symbol_indices_range);
    
    uint32* member_offsets      = (uint32 *)raw_member_offsets.Ptr;
    uint64  member_offset_count = raw_member_offsets.size / sizeof(member_offsets[0]);
    
    uint16* symbol_indices     = (uint16 *)raw_indices.Ptr;
    uint64  symbol_index_count = raw_indices.size / sizeof(symbol_indices[0]);
...
StringView
coff_parse_long_name(StringView long_names, StringView name)
{
  StringView result = name;
  if (name.size > 0 && name[0] == '/') {
    StringView offset_str = StringView(name.Ptr + 1, name.size - 1);
    uint64 offset = u64_from_str8(offset_str, 10);
    if (offset < long_names.size) {
      uint8* ptr = long_names.Ptr + offset;
      uint8* opl = long_names.Ptr + long_names.size;
      for (; ptr < opl; ++ptr) {
        if (*ptr == '\0' || *ptr == '\n') {
          break;
        }
      }
      result = str8_range(long_names.Ptr + offset, ptr);
    }
  }
  return result;
}
...
uint64
coff_parse_import(StringView raw_archive_member, uint64 offset, COFF_ParsedArchiveImportHeader* header_out)
{
  ...
  if (header) {
    ...
    StringView raw_data    = str8_substr(raw_archive_member, data_range);
    uint64     data_cursor = 0;
    ...
    uint64 read_size = sizeof(*header) + header.data_size;
    return read_size;
  }
  return 0;  
}
...
COFF_ArchiveMember
coff_archive_member_from_offset(StringView raw_archive, uint64 offset)
{
...
COFF_ArchiveMember
coff_archive_member_from_data(StringView raw_archive_member)
{
  return coff_archive_member_from_offset(raw_archive_member, 0);
}

COFF_ParsedArchiveImportHeader
coff_archive_import_from_data(StringView raw_archive_member)
{
...
uint64
coff_regular_archive_member_iter_init(StringView raw_archive)
{
  uint64 cursor = raw_archive.size;
  if (coff_is_regular_archive(raw_archive)) {
...
B32
coff_regular_archive_member_iter_next(StringView raw_archive, uint64* offset, COFF_ArchiveMember* member_out)
{
  B32 is_parsed = 0;
  
  member_out.header.is_end_correct = 0;
  uint64 header_size = coff_parse_archive_member_header(raw_archive, *offset, &member_out.header);
  ...
  if (member_out.header.is_end_correct) {
    ...
    uint64 read_size = AlignPow2(header_size + dim_1u64(member_out.header.data_range), COFF_Archive_MemberAlign);
    *offset += read_size;
...
uint64
coff_thin_archive_member_iter_init(StringView raw_archive)
{
  uint64 cursor = raw_archive.size;
...
B32
coff_thin_archive_member_iter_next(StringView raw_archive, uint64* offset, COFF_ArchiveMember* member_out)
{
  B32 is_parsed = 0;
  ...
  if (member_out.header.is_end_correct) {
    ...
    uint64 read_size = AlignPow2(header_size + dim_1u64(data_in_archive_range), COFF_Archive_MemberAlign);
    *offset += read_size;
...
COFF_ArchiveParse
coff_regular_archive_parse_from_data(StringView raw_archive)
{
  COFF_ArchiveMemberList list = {0};
  COFF_ArchiveMemberNode node_arr[3] = {0};
  uint64 cursor = coff_regular_archive_member_iter_init(raw_archive);
  for (uint64 i = 0; i < ArrayCount(node_arr); ++i) {
...
COFF_ArchiveParse
coff_thin_archive_parse_from_data(StringView raw_archive)
{
  COFF_ArchiveMemberList list = {0};
  COFF_ArchiveMemberNode node_arr[3] = {0};
  uint64 cursor = coff_thin_archive_member_iter_init(raw_archive);
  for (uint64 i = 0; i < ArrayCount(node_arr); i += 1) {
    COFF_ArchiveMemberNode* node = &node_arr[i];
...
COFF_ArchiveParse
coff_archive_parse_from_data(StringView raw_archive)
{
