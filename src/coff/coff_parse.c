...
coff_is_obj(String8 raw_coff)
{
  ...
  if (raw_coff.size >= sizeof(COFF_FileHeader)) {
    ...
    if (is_machine_type_valid) {
      ...
      ulong section_count = header->section_count;
      ulong section_hdr_opl_off = sizeof(*header) + section_count*sizeof(COFF_SectionHeader);
      if (raw_coff.size >= section_hdr_opl_off) {
        ...
        for (COFF_SectionHeader *sec_hdr = section_hdrs;
          ...
          if (!(sec_hdr->flags & COFF_SectionFlag_CntUninitializedData)) {
            ulong min = sec_hdr->foff;
            ulong max = min + sec_hdr->fsize;
        ...
        if (is_sect_range_valid) {
          ...
          ulong symbol_table_off = header->symbol_table_foff;
          ulong symbol_table_size = sizeof(COFF_Symbol16)*header->symbol_count;
          ulong symbol_table_opl_off = symbol_table_off + symbol_table_size;
...
COFF_ParsedSymbol
coff_parse_symbol32(String8 raw_coff, ulong string_table_off, COFF_Symbol32 *sym32)
{
...
COFF_ParsedSymbol
coff_parse_symbol16(String8 raw_coff, ulong string_table_off, COFF_Symbol16 *sym16)
{
  COFF_ParsedSymbol result = {0};
  ...
  if (sym16->section_number == COFF_Symbol_DebugSection16) {
    ...
    result.section_number = (uint)sym16->section_number;
  }
...
COFF_Symbol32Array
coff_symbol_array_from_data_16(Arena *arena, String8 raw_coff, ulong symbol_array_off, ulong symbol_count)
{
  ...
  for (ulong isymbol = 0, count = raw_sym16_arr.size / sizeof(COFF_Symbol16); isymbol < count; isymbol += 1) {
    ...
    if (sym16->section_number == COFF_Symbol_DebugSection16) {
      ...
      sym32->section_number = (uint)sym16->section_number;
    }
    ...
    // copy aux symbols
    for (ulong iaux = isymbol+1, iaux_hi = Min(count, iaux+sym16->aux_symbol_count); iaux < iaux_hi; iaux += 1) {
...
COFF_Symbol32Array
coff_symbol_array_from_data_32(Arena *arena, String8 data, ulong symbol_array_off, ulong symbol_count)
{
...
COFF_Symbol32Array
coff_symbol_array_from_data(Arena *arena, String8 data, ulong symbol_array_off, ulong symbol_count, ulong symbol_size)
{
  COFF_Symbol32Array result = {0};
  switch (symbol_size) {
...
coff_interp_symbol(uint section_number, uint value, COFF_SymStorageClass storage_class)
{
...
coff_reloc_info_from_section_header(String8 data, COFF_SectionHeader *header)
{
  ...
  if (header->flags & COFF_SectionFlag_LnkNRelocOvfl && header->reloc_count == max_U16) {
    COFF_Reloc counter;
    ulong read_size = str8_deserial_read_struct(data, header->relocs_foff, &counter);
...
ulong
coff_read_resource_id_utf16(String8 raw_res, ulong off, COFF_ResourceID16 *id_out)
{
  ulong cursor = off;
...
ulong
coff_read_resource(Arena *arena, String8 raw_res, ulong off, COFF_ParsedResource *res_out)
{
  String8 raw_header    = str8_skip(raw_res, off);
  ulong     header_cursor = 0;
  ...
  // read data
  ulong data_read_size = str8_deserial_read_block(raw_res, off + prefix.header_size, prefix.data_size, &res_out->data);
  Assert(prefix.data_size == data_read_size);
  
  // compute read size
  ulong read_size = Max(prefix.header_size, sizeof(prefix)) + AlignPow2(prefix.data_size, COFF_ResourceAlign);
  return read_size;
}
                    ...
                    String8         data)
{
  ...
  // align
  ulong align_size = AlignPow2(list.total_size, COFF_ResourceAlign) - list.total_size;
...
ulong
coff_parse_archive_member_header(String8 raw_archive, ulong offset, COFF_ParsedArchiveMemberHeader *header_out)
{
  ...
  if (header) {
    ...
    uint data_size = u32_from_str8(size, 10);
    ulong data_off  = offset + sizeof(COFF_ArchiveMemberHeader);
...
coff_parse_first_archive_member(COFF_ArchiveMember *member)
{
  ...
  Rng1U64 member_offsets_range = rng_1u64(cursor, cursor + symbol_count * sizeof(uint));
  ...
  uint     *member_offsets      = (uint *)raw_member_offsets.str;
  ulong      member_offset_count = raw_member_offsets.size / sizeof(member_offsets[0]);
...
coff_parse_second_archive_member(COFF_ArchiveMember *member)
{
  ...
  if (str8_match_lit("/", member->header.name, 0)) {
    ...
    Rng1U64 member_offsets_range = rng_1u64(cursor, cursor + member_count * sizeof(uint));
    cursor += dim_1u64(member_offsets_range);
    ...
    ulong  symbol_index_count = raw_indices.size / sizeof(symbol_indices[0]);
...
coff_parse_long_name(String8 long_names, String8 name)
{
  ...
  if (name.size > 0 && name.str[0] == '/') {
    ...
    ulong offset = u64_from_str8(offset_str, 10);
...
ulong
coff_parse_import(String8 raw_archive_member, ulong offset, COFF_ParsedArchiveImportHeader *header_out)
{
  ...
  if (header) {
    ...
    ulong read_size = sizeof(*header) + header->data_size;
    return read_size;
  }
  return 0;  
}
...
COFF_ArchiveMember
coff_archive_member_from_offset(String8 raw_archive, ulong offset)
{
...
B32
coff_regular_archive_member_iter_next(String8 raw_archive, ulong *offset, COFF_ArchiveMember *member_out)
{
  B32 is_parsed = 0;
  
  member_out->header.is_end_correct = 0;
  ulong header_size = coff_parse_archive_member_header(raw_archive, *offset, &member_out->header);
  ...
  if (member_out->header.is_end_correct) {
    ...
    ulong read_size = AlignPow2(header_size + dim_1u64(member_out->header.data_range), COFF_Archive_MemberAlign);
    *offset += read_size;
...
B32
coff_thin_archive_member_iter_next(String8 raw_archive, ulong *offset, COFF_ArchiveMember *member_out)
{
  B32 is_parsed = 0;
  ...
  if (member_out->header.is_end_correct) {
    ...
    ulong read_size = AlignPow2(header_size + dim_1u64(data_in_archive_range), COFF_Archive_MemberAlign);
    *offset += read_size;
...
coff_regular_archive_parse_from_data(String8 raw_archive)
{
  ...
  ulong cursor = coff_regular_archive_member_iter_init(raw_archive);
  for (ulong i = 0; i < ArrayCount(node_arr); ++i) {
...
coff_thin_archive_parse_from_data(String8 raw_archive)
{
  COFF_ArchiveMemberList list = {0};
  COFF_ArchiveMemberNode node_arr[3] = {0};
  ulong cursor = coff_thin_archive_member_iter_init(raw_archive);
  for (ulong i = 0; i < ArrayCount(node_arr); i += 1) {
    COFF_ArchiveMemberNode *node = &node_arr[i];
