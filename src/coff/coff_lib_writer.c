...
coff_lib_writer_alloc(void)
{
  ...
  COFF_LibWriter *writer = new COFF_LibWriter[1];
  writer->arena = arena;
  return writer;
}
...
coff_lib_writer_serialize(Arena *arena, COFF_LibWriter *lib_writer, COFF_TimeStamp time_stamp, U16 mode, B32 emit_second_member)
{
  ...
  // serialize members
  U64         *member_offsets   = /* no zero */ new U64[member_count];
  ...
  U8 *name_buffer = /* no zero */ new U8[name_buffer_size];
  {
    U64 name_cursor = 0;
    for (COFF_LibWriterSymbol *ptr = &symbols[0], *opl = ptr + symbols_count; ptr < opl; ptr += 1) {
  ...
  if (emit_second_member) {
    ...
    U32 *member_off32_arr = /* no zero */ new U32[member_count];
    U16 *member_idx16_arr = /* no zero */ new U16[symbols_count];
  ...
  // first linker member (obsolete, but kept for compatability reasons)
  {
    ...
    U32 *member_off32_arr = /* no zero */ new U32[symbols_count];
