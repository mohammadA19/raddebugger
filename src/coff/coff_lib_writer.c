...
internal void
coff_lib_writer_symbol_array_sort(COFF_LibWriterSymbol *arr, uint64 count)
{
  Assert(count >= 2);
...
coff_lib_writer_push_obj(COFF_LibWriter *writer, String8 obj_path, String8 obj_data)
{
  ...
  // push external symbols
  {
    ...
    for (uint64 symbol_idx = 0; symbol_idx < obj_header.symbol_count; symbol_idx += (1 + symbol.aux_symbol_count)) {
      void *symbol_ptr;
...
internal void
coff_lib_writer_push_import(COFF_LibWriter *lib_writer, COFF_MachineType machine, COFF_TimeStamp time_stamp, String8 dll_name, COFF_ImportByType import_by, String8 name, uint16 hint_or_ordinal, COFF_ImportType import_type)
{
  // push import member
  uint64 member_idx = lib_writer->member_list.count;
  COFF_LibWriterMember member = {0};
...
coff_lib_writer_serialize(Arena *arena, COFF_LibWriter *lib_writer, COFF_TimeStamp time_stamp, uint16 mode, B32 emit_second_member)
{
  ...
  {
    uint64                   symbols_count_with_null = lib_writer->symbol_list.count + 2;
  ...
  // serialize members
  uint64         *member_offsets   = push_array_no_zero(scratch.arena, uint64, member_count);
  String8List  long_names_list  = {0};
  String8List  member_data_list = {0};
  {
    HashTable *name_ht = hash_table_init(scratch.arena, 1024);
    for (uint64 member_idx = 0; member_idx < member_count; member_idx += 1) {
      ...
      {
        uint64 pad_size = AlignPadPow2(member_data_list.total_size, COFF_Archive_MemberAlign);
  ...
  if (long_names_list.total_size) {
    ...
    {
      uint64 pad_size = AlignPadPow2(member_offset, COFF_Archive_MemberAlign);
      uint8 *pad      = push_array(arena, uint8, pad_size);
  ...
  uint64 members_base_offset;
  {
    uint64 sizeof_first_header  = sizeof(COFF_ArchiveMemberHeader) + sizeof(uint32) + sizeof(uint32) * symbols_count + name_buffer_size;
    uint64 sizeof_second_header = sizeof(COFF_ArchiveMemberHeader) + sizeof(uint32) + sizeof(uint32) * member_count + sizeof(uint32) + sizeof(uint16) * symbols_count + name_buffer_size;
  ...
  if (emit_second_member) {
    uint32 member_count32 = safe_cast_u32(member_count);
    uint32 symbol_count32 = safe_cast_u32(symbols_count);

    uint32 *member_off32_arr = push_array_no_zero(scratch.arena, uint32, member_count);
    uint16 *member_idx16_arr = push_array_no_zero(scratch.arena, uint16, symbols_count);

    // write member offset array
    for (uint64 member_idx = 0; member_idx < member_count; member_idx += 1) {
      uint64 member_offset = members_base_offset + member_offsets[member_idx];
      uint32 member_off32 = safe_cast_u32(member_offset);
      member_off32_arr[member_idx] = member_off32; 
    }

    // write member offset indices for each symbol
    for (uint64 symbol_idx = 0; symbol_idx < symbols_count; symbol_idx += 1) {
      // member offset indices are 1-based
      uint64 member_idx = symbols[symbol_idx].member_idx + 1;
  ...
  {
    uint32  symbol_count_be  = from_be_u32(symbols_count);
    uint32 *member_off32_arr = push_array_no_zero(scratch.arena, uint32, symbols_count);

    for (uint64 symbol_idx = 0; symbol_idx < symbols_count; symbol_idx += 1) {
      COFF_LibWriterSymbol *symbol = &symbols[symbol_idx];

      // write big endian member offset
      uint64 member_offset = members_base_offset + member_offsets[symbol->member_idx];
      uint32 member_off32 = from_be_u32(safe_cast_u32(member_offset));
