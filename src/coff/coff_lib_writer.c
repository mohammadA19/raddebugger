...
coff_lib_writer_push_obj(COFF_LibWriter* writer, string obj_path, string obj_data)
{
  ...
  // push external symbols
  {
    ...
    string             string_table = str8_substr(obj_data, obj_header.string_table_range);
    string             symbol_table = str8_substr(obj_data, obj_header.symbol_table_range);
...
coff_lib_writer_serialize(Arena* arena, COFF_LibWriter* lib_writer, COFF_TimeStamp time_stamp, U16 mode, B32 emit_second_member)
{
  ...
  if (long_names_list.total_size) {
    string header        = coff_make_lib_member_header(arena, ("//"), time_stamp, 0, 0, mode, long_names_list.total_size);
    string data          = str8_list_join(arena, &long_names_list, 0);
  ...
  if (emit_second_member) {
    ...
    string member_data   = str8_list_join(arena, &second_member_data_list, 0);
    string member_header = coff_make_lib_member_header(arena, ("/"), time_stamp, 0, 0, mode, member_data.size);
  ...
  // first linker member (obsolete, but kept for compatability reasons)
  {
    ...
    string member_data   = str8_list_join(arena, &first_member_data_list, 0);
    string member_header = coff_make_lib_member_header(arena, ("/"), time_stamp, 0, 0, mode, member_data.size);
