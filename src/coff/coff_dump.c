                         ...
                         COFF_SectionHeader *section_table)
{
  ...
  for (uint64 i = 0; i < symbol_table.count; ++i) {
  ...
  for (uint64 sect_idx = 0; sect_idx < sect_count; ++sect_idx) {
    ...
    if (reloc_info.count) {
      ...
      for (uint64 reloc_idx = 0; reloc_idx < reloc_info.count; ++reloc_idx) {
        ...
        uint64 apply = extend_sign64(raw_apply, apply_size);
...
coff_print_big_obj(Arena *arena, String8List *out, String8 indent, String8 raw_data, RD_Option opts)
{
  ...
  if (opts & RD_Option_Sections) {
    Rng1uint64 sect_headers_range = rng_1u64(sizeof(*big_obj), sizeof(*big_obj) + sizeof(COFF_SectionHeader)*big_obj->section_count);
    Rng1uint64 symbols_range      = rng_1u64(big_obj->symbol_table_foff, big_obj->symbol_table_foff + sizeof(COFF_Symbol32)*big_obj->symbol_count);
...
coff_print_obj(Arena *arena, String8List *out, String8 indent, String8 raw_data, RD_Option opts)
{
  ...
  if (opts & RD_Option_Sections) {
    Rng1uint64 sect_headers_range = rng_1u64(sizeof(*header), sizeof(*header) + sizeof(COFF_SectionHeader)*header->section_count);
    Rng1uint64 symbols_range      = rng_1u64(header->symbol_table_foff, header->symbol_table_foff + sizeof(COFF_Symbol16)*header->symbol_count);
