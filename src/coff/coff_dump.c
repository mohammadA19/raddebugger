                         ...
                         COFF_SectionHeader* section_table)
{
  ...
  string *symlinks = push_array(scratch.arena, string, section_count);
  ...
  if (section_count) {
    ...
    for (U64 i = 0; i < section_count; ++i) {
      ...
      string full_name = coff_name_from_section_header(string_table, header);
      ...
      string flags;
      {
        ...
        string mem = str8_list_join(scratch.arena, &mem_flags, 0);
        string cnt = str8_list_join(scratch.arena, &cnt_flags, 0);
        string lnk = str8_list_join(scratch.arena, &lnk_flags, 0);
        string ext = str8_list_join(scratch.arena, &mem_extra_flags, 0);
        string oth = str8_list_join(scratch.arena, &other_flags, 0);
      ...
      string line = str8_list_join(scratch.arena, &l, &(StringJoin){ .sep = (" "), });
      rd_printf("%S", line);
  ...
  if (section_count) {
    for (U64 sect_idx = 0; sect_idx < section_count; ++sect_idx) {
      ...
      if (sect.flags & COFF_SectionFlag_CntCode) {
        ...
        string        raw_code    = str8_substr(raw_data, rng_1u64(sect.foff, sect.foff+sect_size));
                       ...
                       COFF_SectionHeader* section_table)
{
  if (section_count) {
    for (U64 sect_idx = 0; sect_idx < section_count; ++sect_idx) {
      ...
      if (sect.fsize > 0) {
        U64         sect_size = is_obj ? sect.fsize : sect.vsize;
        string     raw_sect  = str8_substr(raw_data, rng_1u64(sect.foff, sect.foff+sect_size));
  ...
  for (U64 sect_idx = 0; sect_idx < sect_count; ++sect_idx) {
    ...
    if (reloc_info.count) {
      ...
      for (U64 reloc_idx = 0; reloc_idx < reloc_info.count; ++reloc_idx) {
        ...
        string     type       = coff_string_from_reloc(machine, reloc.type);
        ...
        string        symbol_name = coff_read_symbol_name(string_table, &symbol.name);
        ...
        string l = str8_list_join(scratch.arena, &line, &(StringJoin){.sep=(" ")});
        rd_printf("%S", l);
      }
                        ...
                        COFF_Symbol32Array  symbols)
{
  ...
  if (symbols.count) {
    ...
    for (U64 i = 0; i < symbols.count; ++i) {
      ...
      string        msb           = coff_string_from_sym_dtype(symbol.type.u.msb);
      string        lsb           = coff_string_from_sym_type(symbol.type.u.lsb);
      string        storage_class = coff_string_from_sym_storage_class(symbol.storage_class);
...
coff_print_big_obj_header(Arena* arena, String8List* out, string indent, COFF_BigObjHeader* header)
{
  ...
  string time_stamp = coff_string_from_time_stamp(scratch.arena, header.time_stamp);
  string machine    = coff_string_from_machine_type(header.machine);
...
coff_print_file_header(Arena* arena, String8List* out, string indent, COFF_FileHeader* header)
{
  ...
  string time_stamp = coff_string_from_time_stamp(scratch.arena, header.time_stamp);
  string machine    = coff_string_from_machine_type(header.machine);
  string flags      = coff_string_from_flags(scratch.arena, header.flags);
...
coff_print_big_obj(Arena* arena, String8List* out, string indent, string raw_data, RD_Option opts)
{
  ...
  string raw_header        = str8_substr(raw_data, header_info.header_range);
  string raw_section_table = str8_substr(raw_data, header_info.section_table_range);
  string raw_string_table  = str8_substr(raw_data, header_info.string_table_range);
...
coff_print_archive(Arena* arena, String8List* out, string indent, string raw_archive, RD_Option opts)
{
  ...
  for (U64 i = 0; i < member_offset_count; ++i) {
    ...
    string            raw_member         = str8_substr(raw_archive, rng_1u64(member_offset, next_member_offset));
