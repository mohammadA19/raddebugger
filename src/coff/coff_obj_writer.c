...
coff_obj_writer_serialize(Arena *arena, COFF_ObjWriter *obj_writer)
{
  ...
  uint32 *string_table_size = push_array(scratch.arena, uint32, 1);
  *string_table_size = sizeof(*string_table_size);
  ...
  AssertAlways(obj_sections_count <= max_uint16);
  ...
  String8List symbol_table = {0};
  {
    ...
    for (COFF_ObjSymbolNode *symbol_n = obj_writer->symbol_first; symbol_n != 0; symbol_n = symbol_n->next) {
      ...
      if (s->name.size > sizeof(name.short_name)) {
        uint64 string_table_offset = string_table.total_size;
      ...
      // symbol header
      AssertAlways(s->aux_symbols.node_count <= max_uint8);
      d->name             = name;
      d->value            = s->value;
      switch (s->loc.type) {
      ...
      uint64 start_symbol_idx = symbol_idx;
      if (s->storage_class == COFF_SymStorageClass_WeakExternal) {
        ...
        if (s->aux_symbols.node_count > 0) {
          ...
          d_sd->number_of_relocations = (uint16)sect->reloc_count;
          d_sd->check_sum             = 0;
      ...
      uint8 processed_aux_symbol_count = (uint8)(symbol_idx - start_symbol_idx);

      for (uint64 aux_idx = processed_aux_symbol_count; aux_idx < s->aux_symbols.node_count; aux_idx += 1) {
        COFF_Symbol16 *a = push_array(scratch.arena, COFF_Symbol16, 1);
        str8_list_push(scratch.arena, &symbol_table, str8_struct(a));
      }

      d->aux_symbol_count = (uint8)s->aux_symbols.node_count;
    }
  }
  ...
  {
    for (uint64 sect_idx = 0; sect_idx < obj_sections_count; sect_idx += 1) {
      ...
      if (sect_name.size > sizeof(d->name)) {
        uint64 sect_name_off = string_table.total_size;
      ...
      for (String8Node *data_n = s->data.first; data_n != 0; data_n = data_n->next) {
        if (data_n->string.str == 0 && data_n->string.size > 0) {
          data_n->string = str8(push_array(scratch.arena, uint8, data_n->string.size), data_n->string.size);
        }
      }
      ...
      // section relocs
      uint64 relocs_foff = 0;
      if (s->reloc_count) {
        AssertAlways(s->reloc_count <= max_uint16);
...
coff_obj_writer_push_symbol(COFF_ObjWriter *obj_writer, String8 name, uint32 value, COFF_SymbolLocation loc, COFF_SymbolType type, COFF_SymStorageClass storage_class)
{
...
internal COFF_ObjSymbol *
coff_obj_writer_push_symbol_extern(COFF_ObjWriter *obj_writer, String8 name, uint32 value, COFF_ObjSection *section)
{
...
internal COFF_ObjSymbol *
coff_obj_writer_push_symbol_extern_func(COFF_ObjWriter *obj_writer, String8 name, uint32 value, COFF_ObjSection *section)
{
...
internal COFF_ObjSymbol *
coff_obj_writer_push_symbol_static(COFF_ObjWriter *obj_writer, String8 name, uint32 off, COFF_ObjSection *section)
{
  COFF_SymbolLocation loc = {0};
  loc.type                = COFF_SymbolLocation_Section;
  loc.u.section           = section;
...
internal COFF_ObjSymbol *
coff_obj_writer_push_symbol_abs(COFF_ObjWriter *obj_writer, String8 name, uint32 value, COFF_SymStorageClass storage_class)
{
  COFF_SymbolLocation loc = {0};
...
internal COFF_ObjSymbol *
coff_obj_writer_push_symbol_undef_sect(COFF_ObjWriter *obj_writer, String8 name, uint32 value)
{
  COFF_SymbolType type = {0};
  COFF_SymbolLocation loc = {0};
  loc.type = COFF_SymbolLocation_Undef;
...
internal COFF_ObjSymbol *
coff_obj_writer_push_symbol_common(COFF_ObjWriter *obj_writer, String8 name, uint32 size)
{
  COFF_SymbolType type = {0};
  COFF_SymbolLocation loc = {0};
...
coff_obj_writer_section_push_reloc(COFF_ObjWriter *obj_writer, COFF_ObjSection *sect, uint32 apply_off, COFF_ObjSymbol *symbol, COFF_RelocType type)
{
...
internal COFF_ObjReloc *
coff_obj_writer_section_push_reloc_addr(COFF_ObjWriter *obj_writer, COFF_ObjSection *sect, uint32 apply_off, COFF_ObjSymbol *symbol)
{
...
internal COFF_ObjReloc *
coff_obj_writer_section_push_reloc_voff(COFF_ObjWriter *obj_writer, COFF_ObjSection *sect, uint32 apply_off, COFF_ObjSymbol *symbol)
{
  COFF_RelocType reloc_type = 0;
  switch (obj_writer->machine) {
  case COFF_MachineType_Unknown: break;
...
coff_obj_writer_push_directive(COFF_ObjWriter *obj_writer, String8 directive)
{
  if (obj_writer->drectve_sect == 0) {
    local_persist const uint8 bom_sig[]  = { ' ', ' ', ' ' };
