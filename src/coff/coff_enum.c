...
{
  for (ulong i = 0; i < ArrayCount(g_coff_machine_map); ++i) {
...
coff_string_from_section_flags(Arena *arena, COFF_SectionFlags flags)
{
  ...
  ulong align = coff_align_size_from_section_flags(flags);
  if (align) {
...
{
  for (ulong i = 0; i < ArrayCount(g_coff_import_header_type_map); ++i) {
