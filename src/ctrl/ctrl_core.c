...
uint64
ctrl_hash_from_string(StringView str)
{
  uint64 result = 5381;
...
{
  uint64 buf[] = {handle.machine_id, handle.dmn_handle.u64[0]};
  uint64 hash = ctrl_hash_from_string(StringView((uint8 *)buf, sizeof(buf)));
  return hash;
}
...
ctrl_user_breakpoint_list_copy(Arena* arena, CTRL_UserBreakpointList* src)
{
  ...
  for(CTRL_UserBreakpointNode* src_n = src.first; src_n != 0; src_n = src_n.next)
  {
    ...
    dst_bp.str = push_str8_copy(arena, src_n.v.str);
...
ctrl_serialized_string_from_msg_list(Arena* arena, CTRL_MsgList* msgs)
{
  ...
  str8_serial_begin(scratch.arena, &msgs_srlzed);
  {
    ...
    for(CTRL_MsgNode* msg_n = msgs.first; msg_n != 0; msg_n = msg_n.next)
    {
      ...
      str8_serial_push_data(scratch.arena, &msgs_srlzed, msg.path.Ptr, msg.path.size);
      
      // rjf: write entry point str list
      ...
      {
        str8_serial_push_struct(scratch.arena, &msgs_srlzed, &n.str.Length);
        str8_serial_push_data(scratch.arena, &msgs_srlzed, n.str.Ptr, n.str.Length);
      }
      
      // rjf: write command line str list
      ...
      // rjf: write environment str list
      ...
      for(CTRL_UserBreakpointNode* n = msg.user_bps.first; n != 0; n = n.next)
      {
        ...
        str8_serial_push_struct(scratch.arena, &msgs_srlzed, &bp.str.Length);
        str8_serial_push_data(scratch.arena, &msgs_srlzed, bp.str.Ptr, bp.str.Length);
        ...
        str8_serial_push_data(scratch.arena, &msgs_srlzed, bp.condition.Ptr, bp.condition.size);
      }
      
      // rjf: write meta-eval-info array
      StringView meta_evals_srlzed = serialized_from_struct(scratch.arena, CTRL_MetaEvalArray, &msg.meta_evals);
      str8_serial_push_string(scratch.arena, &msgs_srlzed, meta_evals_srlzed);
    }
  }
  StringView str = str8_serial_end(arena, &msgs_srlzed);
  scratch_end(scratch);
  return str;
}

CTRL_MsgList
ctrl_msg_list_from_serialized_string(Arena* arena, StringView str)
{
  CTRL_MsgList msgs = {0};
  {
    uint64 read_off = 0;
    ...
    uint64 msg_count = 0;
    read_off += str8_deserial_read_struct(str, read_off, &msg_count);
    
    // rjf: read data for all messages
    for(uint64 msg_idx = 0; msg_idx < msg_count; msg_idx += 1)
    {
      ...
      // rjf: read flat data
      read_off += str8_deserial_read_struct(str, read_off, &msg.kind);
      read_off += str8_deserial_read_struct(str, read_off, &msg.run_flags);
      read_off += str8_deserial_read_struct(str, read_off, &msg.msg_id);
      read_off += str8_deserial_read_struct(str, read_off, &msg.entity);
      read_off += str8_deserial_read_struct(str, read_off, &msg.parent);
      read_off += str8_deserial_read_struct(str, read_off, &msg.entity_id);
      read_off += str8_deserial_read_struct(str, read_off, &msg.exit_code);
      read_off += str8_deserial_read_struct(str, read_off, &msg.env_inherit);
      read_off += str8_deserial_read_struct(str, read_off, &msg.debug_subprocesses);
      read_off += str8_deserial_read_array (str, read_off, &msg.exception_code_filters[0], ArrayCount(msg.exception_code_filters));
      
      // rjf: read path str
      read_off += str8_deserial_read_struct(str, read_off, &msg.path.size);
      msg.path.Ptr = push_array_no_zero(arena, uint8, msg.path.size);
      read_off += str8_deserial_read(str, read_off, msg.path.Ptr, msg.path.size, 1);
      
      // rjf: read entry point str list
      uint64 entry_point_list_string_count = 0;
      read_off += str8_deserial_read_struct(str, read_off, &entry_point_list_string_count);
      for(uint64 idx = 0; idx < entry_point_list_string_count; idx += 1)
      {
        StringView str = {0};
        read_off += str8_deserial_read_struct(str, read_off, &str.Length);
        str.Ptr = push_array_no_zero(arena, uint8, str.Length);
        read_off += str8_deserial_read(str, read_off, str.Ptr, str.Length, 1);
        str8_list_push(arena, &msg.entry_points, str);
      }
      
      // rjf: read command line str list
      uint64 cmd_line_string_count = 0;
      read_off += str8_deserial_read_struct(str, read_off, &cmd_line_string_count);
      for(uint64 idx = 0; idx < cmd_line_string_count; idx += 1)
      {
        StringView cmd_line_str = {0};
        read_off += str8_deserial_read_struct(str, read_off, &cmd_line_str.size);
        cmd_line_str.Ptr = push_array_no_zero(arena, uint8, cmd_line_str.size);
        read_off += str8_deserial_read(str, read_off, cmd_line_str.Ptr, cmd_line_str.size, 1);
        str8_list_push(arena, &msg.cmd_line_string_list, cmd_line_str);
      }
      
      // rjf: read environment str list
      uint64 env_string_count = 0;
      read_off += str8_deserial_read_struct(str, read_off, &env_string_count);
      for(uint64 idx = 0; idx < env_string_count; idx += 1)
      {
        StringView env_str = {0};
        read_off += str8_deserial_read_struct(str, read_off, &env_str.size);
        env_str.Ptr = push_array_no_zero(arena, uint8, env_str.size);
        read_off += str8_deserial_read(str, read_off, env_str.Ptr, env_str.size, 1);
        str8_list_push(arena, &msg.env_string_list, env_str);
      }
      
      // rjf: read stdout/stderr/stdin paths
      read_off += str8_deserial_read_struct(str, read_off, &msg.stdout_path.size);
      msg.stdout_path.Ptr = push_array(arena, uint8, msg.stdout_path.size);
      read_off += str8_deserial_read(str, read_off, msg.stdout_path.Ptr, msg.stdout_path.size, 1);
      read_off += str8_deserial_read_struct(str, read_off, &msg.stderr_path.size);
      msg.stderr_path.Ptr = push_array(arena, uint8, msg.stderr_path.size);
      read_off += str8_deserial_read(str, read_off, msg.stderr_path.Ptr, msg.stderr_path.size, 1);
      read_off += str8_deserial_read_struct(str, read_off, &msg.stdin_path.size);
      msg.stdin_path.Ptr = push_array(arena, uint8, msg.stdin_path.size);
      read_off += str8_deserial_read(str, read_off, msg.stdin_path.Ptr, msg.stdin_path.size, 1);
      
      // rjf: read trap list
      uint64 trap_count = 0;
      read_off += str8_deserial_read_struct(str, read_off, &trap_count);
      for(uint64 idx = 0; idx < trap_count; idx += 1)
      {
        CTRL_TrapNode* n = push_array(arena, CTRL_TrapNode, 1);
        SLLQueuePush(msg.traps.first, msg.traps.last, n);
        msg.traps.count += 1;
        CTRL_Trap* trap = &n.v;
        read_off += str8_deserial_read_struct(str, read_off, &trap.flags);
        read_off += str8_deserial_read_struct(str, read_off, &trap.vaddr);
      }
      
      // rjf: read user breakpoint list
      uint64 user_bp_count = 0;
      read_off += str8_deserial_read_struct(str, read_off, &user_bp_count);
      for(uint64 idx = 0; idx < user_bp_count; idx += 1)
      {
        CTRL_UserBreakpointNode* n = push_array(arena, CTRL_UserBreakpointNode, 1);
        SLLQueuePush(msg.user_bps.first, msg.user_bps.last, n);
        msg.user_bps.count += 1;
        CTRL_UserBreakpoint* bp = &n.v;
        read_off += str8_deserial_read_struct(str, read_off, &bp.kind);
        read_off += str8_deserial_read_struct(str, read_off, &bp.str.Length);
        bp.str.Ptr = push_array_no_zero(arena, uint8, bp.str.Length);
        read_off += str8_deserial_read(str, read_off, bp.str.Ptr, bp.str.Length, 1);
        read_off += str8_deserial_read_struct(str, read_off, &bp.pt);
        read_off += str8_deserial_read_struct(str, read_off, &bp.u64);
        read_off += str8_deserial_read_struct(str, read_off, &bp.condition.size);
        bp.condition.Ptr = push_array_no_zero(arena, uint8, bp.condition.size);
        read_off += str8_deserial_read(str, read_off, bp.condition.Ptr, bp.condition.size, 1);
      }
      
      // rjf: read meta-eval-info array
      StringView meta_evals_data = str8_skip(str, read_off);
      uint64 meta_evals_size = 0;
...
StringView
ctrl_serialized_string_from_event(Arena* arena, CTRL_Event* event, uint64 max)
{
  Temp scratch = scratch_begin(&arena, 1);
  String8List srl = {0};
  str8_serial_begin(scratch.arena, &srl);
  {
    ...
    StringView str = event.str;
    str.Length = Min(str.Length, max-srl.total_size);
    str8_serial_push_struct(scratch.arena, &srl, &str.Length);
    str8_serial_push_data(scratch.arena, &srl, str.Ptr, str.Length);
  }
  StringView str = str8_serial_end(arena, &srl);
  scratch_end(scratch);
  return str;
}

CTRL_Event
ctrl_event_from_serialized_string(Arena* arena, StringView str)
{
  CTRL_Event event = {};
  {
    uint64 read_off = 0;
    read_off += str8_deserial_read_struct(str, read_off, &event.kind);
    read_off += str8_deserial_read_struct(str, read_off, &event.cause);
    read_off += str8_deserial_read_struct(str, read_off, &event.exception_kind);
    read_off += str8_deserial_read_struct(str, read_off, &event.msg_id);
    read_off += str8_deserial_read_struct(str, read_off, &event.entity);
    read_off += str8_deserial_read_struct(str, read_off, &event.parent);
    read_off += str8_deserial_read_struct(str, read_off, &event.arch);
    read_off += str8_deserial_read_struct(str, read_off, &event.u64_code);
    read_off += str8_deserial_read_struct(str, read_off, &event.entity_id);
    read_off += str8_deserial_read_struct(str, read_off, &event.vaddr_rng);
    read_off += str8_deserial_read_struct(str, read_off, &event.rip_vaddr);
    read_off += str8_deserial_read_struct(str, read_off, &event.stack_base);
    read_off += str8_deserial_read_struct(str, read_off, &event.tls_root);
    read_off += str8_deserial_read_struct(str, read_off, &event.timestamp);
    read_off += str8_deserial_read_struct(str, read_off, &event.exception_code);
    read_off += str8_deserial_read_struct(str, read_off, &event.rgba);
    read_off += str8_deserial_read_struct(str, read_off, &event.str.Length);
    event.str.Ptr = push_array_no_zero(arena, uint8, event.str.Length);
    read_off += str8_deserial_read(str, read_off, event.str.Ptr, event.str.Length, 1);
  }
  return event;
}
...
ctrl_entity_store_alloc()
{
  ...
  StringView local_machine_name = push_str8f(scratch.arena, "This PC (%S)", os_get_system_info().machine_name);
...
uint64
ctrl_name_bucket_idx_from_string_size(uint64 size)
{
...
StringView
ctrl_entity_string_alloc(CTRL_EntityStore* store, StringView str)
{
  if(str.Length == 0) {return StringView();}
  uint64 bucket_idx = ctrl_name_bucket_idx_from_string_size(str.Length);
  ...
  if(node == 0)
  {
    ...
    uint8* chunk_memory = push_array(store.arena, uint8, chunk_size);
...
void
ctrl_entity_string_release(CTRL_EntityStore* store, StringView str)
{
  if(str.Length == 0) {return;}
  uint64 bucket_idx = ctrl_name_bucket_idx_from_string_size(str.Length);
  CTRL_EntityStringChunkNode* node = (CTRL_EntityStringChunkNode *)str.Ptr;
...
CTRL_Entity *
ctrl_entity_alloc(CTRL_EntityStore* store, CTRL_Entity* parent, CTRL_EntityKind kind, Arch arch, CTRL_Handle handle, uint64 id)
{
  CTRL_Entity* entity = &ctrl_entity_nil;
  {
    ...
    {
      uint64 hash = ctrl_hash_from_handle(handle);
      uint64 slot_idx = hash%store.hash_slots_count;
...
ctrl_entity_release(CTRL_EntityStore* store, CTRL_Entity* entity)
{
  ...
  if(entity != &ctrl_entity_nil)
  {
    ...
    for(Task* t = first_task; t != 0; t = t.next)
    {
      ...
      {
        uint64 hash = ctrl_hash_from_handle(t.e.handle);
        uint64 slot_idx = hash%store.hash_slots_count;
        CTRL_EntityHashSlot* slot = &store.hash_slots[slot_idx];
        CTRL_EntityHashNode* node = 0;
        for(CTRL_EntityHashNode* n = slot.first; n != 0; n = n.next)
...
void
ctrl_entity_equip_string(CTRL_EntityStore* store, CTRL_Entity* entity, StringView str)
{
  if(entity.str.Length != 0)
  {
    ctrl_entity_string_release(store, entity.str);
  }
  entity.str = ctrl_entity_string_alloc(store, str);
}
...
CTRL_Entity *
ctrl_module_from_process_vaddr(CTRL_Entity* process, uint64 vaddr)
{
  CTRL_Entity* result = &ctrl_entity_nil;
...
ctrl_dbgi_key_from_module(CTRL_Entity* module)
{
  ...
  DI_Key dbgi_key = {debug_info_path.str, debug_info_path.timestamp};
  return dbgi_key;
}
...
ctrl_module_from_thread_candidates(CTRL_EntityStore* store, CTRL_Entity* thread, CTRL_EntityList* candidates)
{
  ...
  uint64 thread_rip_vaddr = ctrl_query_cached_rip_from_thread(store, thread.handle);
...
uint64
ctrl_vaddr_from_voff(CTRL_Entity* module, uint64 voff)
{
  uint64 result = voff + module.vaddr_range.min;
  return result;
}

uint64
ctrl_voff_from_vaddr(CTRL_Entity* module, uint64 vaddr)
{
  uint64 result = vaddr - module.vaddr_range.min;
  return result;
}
...
{
  uint64 dim = dim_1u64(voff_range);
  uint64 min = ctrl_vaddr_from_voff(module, voff_range.min);
...
{
  uint64 dim = dim_1u64(vaddr_range);
  uint64 min = ctrl_voff_from_vaddr(module, vaddr_range.min);
  Rng1U64 result = {min, min+dim};
  return result;
}
...
CTRL_EntityRec
ctrl_entity_rec_depth_first(CTRL_Entity* entity, CTRL_Entity* subtree_root, uint64 sib_off, uint64 child_off)
{
  ...
  for(CTRL_EventNode* n = list.first; n != 0; n = n.next)
  {
    ...
    switch(event.kind)
    {
      ...
      case CTRL_EventKind_NewProc:
      {
        ...
        CTRL_Entity* process = ctrl_entity_alloc(store, machine, CTRL_EntityKind_Process, event.arch, event.entity, (uint64)event.entity_id);
      }break;
      ...
      case CTRL_EventKind_NewThread:
      {
        ...
        CTRL_Entity* thread = ctrl_entity_alloc(store, process, CTRL_EntityKind_Thread, event.arch, event.entity, (uint64)event.entity_id);
        ...
        for(CTRL_EntityNode* n = pending_thread_names.first; n != 0; n = n.next)
        {
          ...
          {
            ctrl_entity_equip_string(store, thread, n.v.str);
      ...
      case CTRL_EventKind_ThreadName:
      {
        ...
        {
          ctrl_entity_equip_string(store, thread, event.str);
        }
        else
        {
          ...
          ctrl_entity_equip_string(store, pending_name, event.str);
        }
      }break;
      ...
      case CTRL_EventKind_NewModule:
      {
        ...
        ctrl_entity_equip_string(store, module, event.str);
        ...
        {
          ctrl_entity_equip_string(store, process, str8_skip_last_slash(event.str));
        }
        scratch_end(scratch);
      }break;
      ...
      case CTRL_EventKind_ModuleDebugInfoPathChange:
      {
        ...
        ctrl_entity_equip_string(store, debug_info_path, event.str);
...
ctrl_init()
{
  ...
  for(uint64 idx = 0; idx < ctrl_state.process_memory_cache.stripes_count; idx += 1)
  {
  ...
  for(uint64 idx = 0; idx < ctrl_state.thread_reg_cache.stripes_count; idx += 1)
  {
  ...
  for(uint64 idx = 0; idx < ctrl_state.module_image_info_cache.stripes_count; idx += 1)
  {
  ...
  ctrl_state.u2c_ring_base = push_array_no_zero(arena, uint8, ctrl_state.u2c_ring_size);
  ...
  ctrl_state.c2u_ring_base = push_array_no_zero(arena, uint8, ctrl_state.c2u_ring_size);
  ...
  ctrl_state.c2u_ring_cv = os_condition_variable_alloc();
  {
    ...
    os_write_data_to_file_path(ctrl_state.ctrl_thread_log_path, StringView());
    scratch_end(scratch);
  }
  ...
  ctrl_state.u2ms_ring_base = push_array(arena, uint8, ctrl_state.u2ms_ring_size);
...
{
  uint64 key_hash_data[] =
  {
    (uint64)process.machine_id,
    (uint64)process.dmn_handle.u64[0],
    range.min,
    range.max,
    (uint64)zero_terminated,
  };
  U128 key = hs_hash_from_data(StringView((uint8*)key_hash_data, sizeof(key_hash_data)));
  return key;
}

U128
ctrl_stored_hash_from_process_vaddr_range(CTRL_Handle process, Rng1U64 range, B32 zero_terminated, B32* out_is_stale, uint64 endt_us)
{
  ProfBeginFunction();
  U128 result = {0};
  uint64 size = dim_1u64(range);
  uint64 pre_mem_gen = dmn_mem_gen();
  if(size != 0) for(;;)
  {
    CTRL_ProcessMemoryCache* cache = &ctrl_state.process_memory_cache;
    uint64 process_hash = ctrl_hash_from_string(str8_struct(&process));
    uint64 process_slot_idx = process_hash%cache.slots_count;
    uint64 process_stripe_idx = process_slot_idx%cache.stripes_count;
    ...
    uint64 range_hash = ctrl_hash_from_string(str8_struct(&range));
    ...
    OS_MutexScopeR(process_stripe.rw_mutex)
    {
      for(CTRL_ProcessMemoryCacheNode* n = process_slot.first; n != 0; n = n.next)
      {
        ...
        {
          uint64 range_slot_idx = range_hash%n.range_hash_slots_count;
    ...
    uint64 last_time_requested_us = 0;
    if(!is_good)
    {
      OS_MutexScopeW(process_stripe.rw_mutex)
      {
          ...
          if(ctrl_handle_match(n.handle, process))
          {
            ...
            if(!range_node_exists)
            {
              ...
              range_n.vaddr_range_clamped = range;
              {
                ...
                uint64 max_size_cap = Min(max_U64-range_n.vaddr_range_clamped.min, GB(1));
  ...
  uint64 post_mem_gen = dmn_mem_gen();
...
ctrl_query_cached_data_from_process_vaddr_range(Arena* arena, CTRL_Handle process, Rng1U64 range, uint64 endt_us)
{
  ProfBeginFunction();
     ...
     range.max <= 0x000FFFFFFFFFFFFFull)
  {
    ...
    {
      for(uint64 page_idx = 0; page_idx < page_count; page_idx += 1)
      {
        uint64 page_base_vaddr = page_range.min + page_idx*page_size;
    ...
    void* read_out = push_array(arena, uint8, dim_1u64(range));
    uint64* byte_bad_flags = push_array(arena, uint64, (dim_1u64(range)+63)/64);
    uint64* byte_changed_flags = push_array(arena, uint64, (dim_1u64(range)+63)/64);
    ...
    ProfScope("iterate pages, fill output")
    {
      ...
      for(uint64 page_idx = 0; page_idx < page_count; page_idx += 1)
      {
        ...
        StringView data = hs_data_from_hash(scope, page_hashes[page_idx]);
        ...
        MemoryCopy((uint8*)read_out+write_off, in_range_data.Ptr, in_range_data.size);
        ...
        if(data.size < page_size) ProfScope("mark missing bytes as bad")
        {
          ...
          for(uint64 invalid_vaddr = in_range_invalid_range.min;
              invalid_vaddr < in_range_invalid_range.max;
              invalid_vaddr += 1)
          {
            uint64 idx_in_range = invalid_vaddr - range.min;
        ...
        {
          StringView last_data = hs_data_from_hash(scope, page_last_hashes[page_idx]);
          StringView in_range_last_data = last_data;
          if(page_idx == page_count-1 && data_vaddr_range.max > range.max)
          {
          ...
          for(uint64 idx = 0; idx < in_range_data.size; idx += 1)
          {
            uint8 last_byte = idx < in_range_last_data.size ? in_range_last_data[idx] : 0;
            uint8 now_byte  = idx < in_range_data.size ? in_range_data[idx] : 0;
            if(last_byte != now_byte)
            {
              uint64 idx_in_read_out = write_off+idx;
        ...
        uint64 bytes_to_skip = page_size;
        if(page_idx == 0 && range.min > data_vaddr_range.min)
        {
...
CTRL_ProcessMemorySlice
ctrl_query_cached_zero_terminated_data_from_process_vaddr_limit(Arena* arena, CTRL_Handle process, uint64 vaddr, uint64 limit, uint64 element_size, uint64 endt_us)
{
  ...
  uint64 element_count = result.data.size/element_size;
  for(uint64 element_idx = 0; element_idx < element_count; element_idx += 1)
  {
    B32 element_is_zero = 1;
    for(uint64 element_byte_idx = 0; element_byte_idx < element_size; element_byte_idx += 1)
    {
      if(result.data[element_idx*element_size + element_byte_idx] != 0)
      {
        element_is_zero = 0;
        break;
      }
...
B32
ctrl_read_cached_process_memory(CTRL_Handle process, Rng1U64 range, B32* is_stale_out, void* out, uint64 endt_us)
{
  Temp scratch = scratch_begin(0, 0);
  uint64 needed_size = dim_1u64(range);
  ...
  if(good)
  {
    MemoryCopy(out, slice.data.Ptr, needed_size);
  }
...
ctrl_process_write(CTRL_Handle process, Rng1U64 range, void* src)
{
  ...
  {
    Temp scratch = scratch_begin(0, 0);
    uint64 endt_us = os_now_microseconds()+5000;
    ...
    Task* last_task = 0;
    CTRL_ProcessMemoryCache* cache = &ctrl_state.process_memory_cache;
    for(uint64 slot_idx = 0; slot_idx < cache.slots_count; slot_idx += 1)
    {
      uint64 stripe_idx = slot_idx%cache.stripes_count;
      ...
      OS_MutexScopeW(stripe.rw_mutex)
      {
        ...
        {
          for(uint64 range_hash_idx = 0; range_hash_idx < proc_n.range_hash_slots_count; range_hash_idx += 1)
          {
...
ctrl_query_cached_reg_block_from_thread(Arena* arena, CTRL_EntityStore* store, CTRL_Handle handle)
{
  ...
  uint64 reg_block_size = regs_block_size_from_arch(arch);
  uint64 hash = ctrl_hash_from_handle(handle);
  ...
  void* result = push_array(arena, uint8, reg_block_size);
  OS_MutexScopeW(stripe.rw_mutex)
  {
    ...
    if(!node)
    {
      ...
      node.block      = push_array(stripe.arena, uint8, reg_block_size);
    }
    
    // rjf: copy from node
    if(node)
    {
      uint64 current_reg_gen = dmn_reg_gen();
      B32 need_stale = 1;
...
{
  uint64 result = dmn_tls_root_vaddr_from_thread(handle.dmn_handle);
  return result;
}
...
ctrl_query_cached_rip_from_thread(CTRL_EntityStore* store, CTRL_Handle handle)
{
  ...
  uint64 result = regs_rip_from_arch_block(arch, block);
  scratch_end(scratch);
  return result;
}
...
ctrl_query_cached_rsp_from_thread(CTRL_EntityStore* store, CTRL_Handle handle)
{
  ...
  Arch arch = thread_entity.arch;
  void* block = ctrl_query_cached_reg_block_from_thread(scratch.arena, store, handle);
  uint64 result = regs_rsp_from_arch_block(arch, block);
  scratch_end(scratch);
  return result;
}
...
PE_IntelPdata *
ctrl_intel_pdata_from_module_voff(Arena* arena, CTRL_Handle module_handle, uint64 voff)
{
  PE_IntelPdata* first_pdata = 0;
  {
    uint64 hash = ctrl_hash_from_handle(module_handle);
    uint64 slot_idx = hash%ctrl_state.module_image_info_cache.slots_count;
    uint64 stripe_idx = slot_idx%ctrl_state.module_image_info_cache.stripes_count;
    ...
    OS_MutexScopeR(stripe.rw_mutex) for(CTRL_ModuleImageInfoCacheNode* n = slot.first; n != 0; n = n.next)
    {
      if(ctrl_handle_match(n.module, module_handle))
      {
        ...
        uint64 pdatas_count = n.pdatas_count;
        if(n.pdatas_count != 0 && voff >= n.pdatas[0].voff_first)
        {
          ...
          uint64 index = pdatas_count;
          uint64 min = 0;
          uint64 opl = pdatas_count;
          for(;;)
          {
            uint64 mid = (min + opl)/2;
...
ctrl_unwind_deep_copy(Arena* arena, Arch arch, CTRL_Unwind* src)
{
  CTRL_Unwind dst = {0};
  {
    ...
    uint64 block_size = regs_block_size_from_arch(arch);
    for(uint64 idx = 0; idx < dst.frames.count; idx += 1)
    {
      dst.frames.v[idx].regs = push_array_no_zero(arena, uint8, block_size);
...
ctrl_unwind_step__pe_x64(CTRL_EntityStore* store, CTRL_Handle process_handle, CTRL_Handle module_handle, REGS_RegBlockX64* regs, uint64 endt_us)
{
  B32 is_stale = 0;
  B32 is_good = 1;
  Temp scratch = scratch_begin(0, 0);
  ...
  uint64 rip_voff = regs.rip.u64 - module.vaddr_range.min;
  ...
  if(first_pdata) ProfScope("pdata . detect if in epilog")
  {
    ...
    B32 is_epilog = 0;
    B32 keep_parsing = 1;
    uint64 read_vaddr = regs.rip.u64;
    uint64 read_vaddr_opl = read_vaddr + 256;
    ...
    for(;keep_parsing;)
    {
      ...
      uint64 check_vaddr = read_vaddr;
      uint8 check_inst_byte = inst_byte;
      ...
      if(check_inst_byte_good)
      {
        switch(check_inst_byte)
        {
          ...
          case 0xE9:
          {
            uint64 imm_vaddr = check_vaddr + 1;
            int32 imm = 0;
            B32 imm_good = 0;
            ...
            if(imm_good)
            {
              uint64 next_vaddr = (uint64)(imm_vaddr + sizeof(imm) + imm);
              uint64 next_voff = next_vaddr - module.vaddr_range.min; // TODO(rjf): verify that this offset is from module base vaddr, not section
    ...
    for(B32 keep_parsing = 1;keep_parsing != 0;)
    {
      ...
      switch(inst_byte)
      {
        ...
        case 0x81:
        {
          ...
          regs.rsp.u64 = (uint64)(regs.rsp.u64 + imm);
        ...
        case 0x8D:
        {
          ...
          // rjf: update stack pointer
          regs.rsp.u64 = (uint64)(reg_value + imm);
  ...
  if(first_pdata && !has_pdata_and_in_epilog) ProfScope("pdata & not in epilog . xdata unwind")
  {
    ...
    uint64 frame_off = 0;
    {
      uint64 unwind_info_off = first_pdata.voff_unwind_info;
      ...
      uint32 frame_reg_id = PE_UNWIND_INFO_REG_FROM_FRAME(unwind_info.frame);
      uint64 frame_off_val = PE_UNWIND_INFO_OFF_FROM_FRAME(unwind_info.frame);
      if(frame_reg_id != 0)
      {
    ...
    if(!bad_frame_reg_info) for(B32 keep_parsing = 1; keep_parsing && pdata != last_pdata;)
    {
      ...
      uint64 unwind_info_off = pdata.voff_unwind_info;
      PE_UnwindInfo unwind_info = {0};
      ...
      uint64 frame_base = regs.rsp.u64;
      if(frame_reg != 0)
      {
        uint64 raw_frame_base = frame_reg.u64;
        uint64 adjusted_frame_base = raw_frame_base - frame_off*16;
      ...
      for(PE_UnwindCode* next_code_ptr = 0; code_ptr < code_opl; code_ptr = next_code_ptr)
      {
        ...
        uint32 op_code = PE_UNWIND_OPCODE_FROM_FLAGS(code_ptr.flags);
        uint32 op_info = PE_UNWIND_INFO_FROM_FLAGS(code_ptr.flags);
        uint32 slot_count = pe_slot_count_from_unwind_op_code(op_code);
        ...
        uint64 code_voff = pdata.voff_first + code_ptr.off_in_prolog;
        if(code_voff <= rip_voff)
        {
          switch(op_code)
          {
            ...
            case PE_UnwindOpCode_SAVE_NONVOL:
            {
              ...
              uint64 off = code_ptr[1].u16*8;
              uint64 addr = frame_base + off;
              uint64 value = 0;
            ...
            {
              // rjf: read value from frame base
              uint64 off = code_ptr[1].u16 + ((uint32)code_ptr[2].u16 << 16);
              uint64 addr = frame_base + off;
              uint64 value = 0;
              if(!ctrl_read_cached_process_memory_struct(process.handle, addr, &is_stale, &value, endt_us) ||
                 is_stale)
            ...
            case PE_UnwindOpCode_PUSH_MACHFRAME:
            {
              ...
              uint64 sp_after_ss = sp_after_ip + 8;
              uint64 rflags_value = 0;
              ...
              uint64 sp_after_rflags = sp_after_ss + 8;
              uint64 sp_value = 0;
      ...
      if(keep_parsing)
      {
        uint32 flags = PE_UNWIND_INFO_FLAGS_FROM_HDR(unwind_info.header);
        if(!(flags & PE_UnwindInfoFlag_CHAINED))
        {
          break;
        }
        uint64 code_count_rounded = AlignPow2(unwind_info.codes_num, sizeof(PE_UnwindCode));
        uint64 code_size = code_count_rounded*sizeof(PE_UnwindCode);
        uint64 chained_pdata_off = unwind_info_off + sizeof(PE_UnwindInfo) + code_size;
        last_pdata = pdata;
...
CTRL_UnwindStepResult
ctrl_unwind_step(CTRL_EntityStore* store, CTRL_Handle process, CTRL_Handle module, Arch arch, void* reg_block, uint64 endt_us)
{
  CTRL_UnwindStepResult result = {0};
  switch(arch)
  {
...
CTRL_Unwind
ctrl_unwind_from_thread(Arena* arena, CTRL_EntityStore* store, CTRL_Handle thread, uint64 endt_us)
{
  ProfBeginFunction();
  Temp scratch = scratch_begin(&arena, 1);
  ...
  Arch arch = thread_entity.arch;
  uint64 arch_reg_block_size = regs_block_size_from_arch(arch);
  ...
  if(regs_block_good)
  {
    ...
    {
      // rjf: regs . rip*module
      uint64 rip = regs_rip_from_arch_block(arch, regs_block);
      ...
      frame.regs = push_array_no_zero(arena, uint8, arch_reg_block_size);
...
ctrl_call_stack_from_unwind(Arena* arena, DI_Scope* di_scope, CTRL_Entity* process, CTRL_Unwind* base_unwind)
{
  ...
  for(uint64 idx = 0; idx < result.concrete_frame_count; idx += 1)
  {
    CTRL_UnwindFrame* src = &base_unwind.frames.v[idx];
    CTRL_CallStackFrame* dst = &result.frames[idx];
    uint64 rip_vaddr = regs_rip_from_arch_block(arch, src.regs);
...
B32
ctrl_u2c_push_msgs(CTRL_MsgList* msgs, uint64 endt_us)
{
  Temp scratch = scratch_begin(0, 0);
  StringView msgs_srlzed_baked = ctrl_serialized_string_from_msg_list(scratch.arena, msgs);
  B32 good = 0;
  OS_MutexScope(ctrl_state.u2c_ring_mutex) for(;;)
  {
    uint64 unconsumed_size = (ctrl_state.u2c_ring_write_pos-ctrl_state.u2c_ring_read_pos);
    uint64 available_size = ctrl_state.u2c_ring_size-unconsumed_size;
    uint64 needed_size = sizeof(msgs_srlzed_baked.size) + msgs_srlzed_baked.size;
    if(available_size >= needed_size)
    {
      ...
      ctrl_state.u2c_ring_write_pos += ring_write(ctrl_state.u2c_ring_base, ctrl_state.u2c_ring_size, ctrl_state.u2c_ring_write_pos, msgs_srlzed_baked.Ptr, msgs_srlzed_baked.size);
      good = 1;
      break;
    }
    if(os_now_microseconds() >= endt_us)
...
{
  Temp scratch = scratch_begin(&arena, 1);
  StringView msgs_srlzed_baked = {0};
  OS_MutexScope(ctrl_state.u2c_ring_mutex) for(;;)
  {
    uint64 unconsumed_size = (ctrl_state.u2c_ring_write_pos-ctrl_state.u2c_ring_read_pos);
    if(unconsumed_size >= sizeof(uint64))
    {
      uint64 size_to_decode = 0;
      ...
      msgs_srlzed_baked.Ptr = push_array_no_zero(scratch.arena, uint8, msgs_srlzed_baked.size);
      ctrl_state.u2c_ring_read_pos += ring_read(ctrl_state.u2c_ring_base, ctrl_state.u2c_ring_size, ctrl_state.u2c_ring_read_pos, msgs_srlzed_baked.Ptr, size_to_decode);
      break;
    }
...
ctrl_c2u_push_events(CTRL_EventList* events)
{
  if(events.count != 0) ProfScope("ctrl_c2u_push_events")
  {
    ...
    {
      Temp scratch = scratch_begin(0, 0);
      StringView event_srlzed = ctrl_serialized_string_from_event(scratch.arena, &n.v, ctrl_state.c2u_ring_size-sizeof(uint64));
      OS_MutexScope(ctrl_state.c2u_ring_mutex) for(;;)
      {
        uint64 unconsumed_size = (ctrl_state.c2u_ring_write_pos-ctrl_state.c2u_ring_read_pos);
        uint64 available_size = ctrl_state.c2u_ring_size-unconsumed_size;
        uint64 needed_size = sizeof(event_srlzed.size) + event_srlzed.size;
        if(available_size >= needed_size)
        {
          ...
          ctrl_state.c2u_ring_write_pos += ring_write(ctrl_state.c2u_ring_base, ctrl_state.c2u_ring_size, ctrl_state.c2u_ring_write_pos, event_srlzed.Ptr, event_srlzed.size);
          break;
        }
...
ctrl_c2u_pop_events(Arena* arena)
{
  ...
  OS_MutexScope(ctrl_state.c2u_ring_mutex) for(;;)
  {
    ...
    if(unconsumed_size >= sizeof(uint64))
    {
      ...
      StringView event_srlzed = {0};
      event_srlzed.size = size_to_decode;
      event_srlzed.Ptr = push_array_no_zero(scratch.arena, uint8, event_srlzed.size);
      ctrl_state.c2u_ring_read_pos += ring_read(ctrl_state.c2u_ring_base, ctrl_state.c2u_ring_size, ctrl_state.c2u_ring_read_pos, event_srlzed.Ptr, event_srlzed.size);
...
ctrl_thread__entry_point(void* p)
{
  ...
  for(;;)
  {
      ...
      for(CTRL_MsgNode* msg_n = msgs.first; msg_n != 0; msg_n = msg_n.next)
      {
        ...
        switch(msg.kind)
        {
          ...
          case CTRL_MsgKind_SetUserEntryPoints:
          {
            ...
            for(String8Node* n = msg.entry_points.first; n != 0; n = n.next)
            {
              str8_list_push(ctrl_state.user_entry_point_arena, &ctrl_state.user_entry_points, n.str);
            }
          }break;
          case CTRL_MsgKind_SetModuleDebugInfoPath:
          {
            StringView path = msg.path;
            CTRL_Entity* module = ctrl_entity_from_handle(ctrl_state.ctrl_thread_entity_store, msg.entity);
            CTRL_Entity* debug_info_path = ctrl_entity_child_from_kind(module, CTRL_EntityKind_DebugInfoPath);
            DI_Key old_dbgi_key = {debug_info_path.str, debug_info_path.timestamp};
            di_close(&old_dbgi_key);
            ctrl_entity_equip_string(ctrl_state.ctrl_thread_entity_store, debug_info_path, path);
            uint64 new_dbgi_timestamp = os_properties_from_file_path(path).modified;
            debug_info_path.timestamp = new_dbgi_timestamp;
            DI_Key new_dbgi_key = {debug_info_path.str, new_dbgi_timestamp};
    ...
    if(log.strings[LogMsgKind_UserError].size != 0)
    {
      ...
      evt.str     = log.strings[LogMsgKind_UserError];
      ctrl_c2u_push_events(&evts);
    }
  }
...
ctrl_thread__append_resolved_module_user_bp_traps(Arena* arena, CTRL_Handle process, CTRL_Handle module, CTRL_UserBreakpointList* user_bps, DMN_TrapChunkList* traps_out)
{
  ...
  DI_Key dbgi_key = {debug_info_path_entity.str, debug_info_path_entity.timestamp};
  RDI_Parsed* rdi = di_rdi_from_key(di_scope, &dbgi_key, max_U64);
  uint64 base_vaddr = module_entity.vaddr_range.min;
    ...
    switch(bp.kind)
    {
      ...
      case CTRL_UserBreakpointKind_FileNameAndLineColNumber:
      {
        ...
        // rjf: src_id * pt . push
        {
          ...
          uint32 voff_count = 0;
          uint64* voffs = rdi_line_voffs_from_num(&line_map, pt.line, &voff_count);
          for(uint32 i = 0; i < voff_count; i += 1)
          {
            uint64 vaddr = voffs[i] + base_vaddr;
            DMN_Trap trap = {process.dmn_handle, vaddr, (uint64)bp};
      ...
      case CTRL_UserBreakpointKind_SymbolNameAndOffset:
      {
        ...
        if(node != 0)
        {
          uint32 id_count = 0;
          uint32* ids = rdi_matches_from_map_node(rdi, node, &id_count);
          for(uint32 match_i = 0; match_i < id_count; match_i += 1)
          {
            RDI_Procedure* procedure = rdi_element_from_name_idx(rdi, Procedures, ids[match_i]);
            uint64 proc_voff = rdi_first_voff_from_procedure(rdi, procedure);
            uint64 proc_vaddr = proc_voff + base_vaddr;
            DMN_Trap trap = {process.dmn_handle, proc_vaddr + voff, (uint64)bp};
            dmn_trap_chunk_list_push(arena, traps_out, 256, &trap);
          }
        }
      }break;
...
ctrl_thread__append_resolved_process_user_bp_traps(Arena* arena, CTRL_Handle process, CTRL_UserBreakpointList* user_bps, DMN_TrapChunkList* traps_out)
{
  for(CTRL_UserBreakpointNode* n = user_bps.first; n != 0; n = n.next)
  {
    ...
    {
      DMN_Trap trap = {process.dmn_handle, bp.u64, (uint64)bp};
      dmn_trap_chunk_list_push(arena, traps_out, 256, &trap);
    }
  }
}
...
void
ctrl_thread__module_open(CTRL_Handle process, CTRL_Handle module, Rng1U64 vaddr_range, StringView path)
{
  //////////////////////////////
  ...
  uint64 pdatas_count = 0;
  uint64 entry_point_voff = 0;
  Rng1U64 tls_vaddr_range = {0};
  uint32 pdb_dbg_time = 0;
  uint32 pdb_dbg_age = 0;
  Guid pdb_dbg_guid = {0};
  StringView pdb_dbg_path = StringView();
  uint32 rdi_dbg_time = 0;
  Guid rdi_dbg_guid = {0};
  StringView rdi_dbg_path = StringView();
  ProfScope("unpack relevant PE info")
  {
    ...
    uint32 opt_ext_size = file_header.optional_header_size;
    ...
    uint16 optional_magic = 0;
    uint64 image_base = 0;
    uint64 entry_point = 0;
    uint32 data_dir_count = 0;
    uint64 virt_section_align = 0;
    uint64 file_section_align = 0;
    Rng1U64* data_dir_franges = 0;
    if(opt_ext_size > 0)
    {
      // rjf: read magic number
      uint16 opt_ext_magic = 0;
      dmn_process_read_struct(process.dmn_handle, vaddr_range.min + opt_ext_off_range.min, &opt_ext_magic);
      
      // rjf: read info
      uint32 reported_data_dir_offset = 0;
      uint32 reported_data_dir_count = 0;
      ...
      uint32 data_dir_max = (opt_ext_size - reported_data_dir_offset) / sizeof(PE_DataDirectory);
      ...
      if(data_dir_count > PE_DataDirectoryIndex_EXCEPTIONS)
      {
        ...
        Rng1U64 pdatas_voff_range = r1u64((uint64)dir.virt_off, (uint64)dir.virt_off + (uint64)dir.virt_size);
      ...
      if(data_dir_count > PE_DataDirectoryIndex_TLS)
      {
        ...
        Rng1U64 tls_voff_range = r1u64((uint64)dir.virt_off, (uint64)dir.virt_off + (uint64)dir.virt_size);
        switch(file_header.machine)
        {
          ...
          case COFF_Machine_X86:
          {
            ...
            tls_header.raw_data_start    = (uint64)tls_header32.raw_data_start;
            tls_header.raw_data_end      = (uint64)tls_header32.raw_data_end;
            tls_header.index_address     = (uint64)tls_header32.index_address;
            tls_header.callbacks_address = (uint64)tls_header32.callbacks_address;
            tls_header.zero_fill_size    = (uint64)tls_header32.zero_fill_size;
            tls_header.characteristics   = (uint64)tls_header32.characteristics;
          }break;
      ...
      tls_vaddr_range = r1u64(tls_header.index_address, tls_header.index_address+sizeof(uint32));
      ...
      if(data_dir_count > PE_DataDirectoryIndex_DEBUG)
      {
        ...
        uint64 dbg_dir_count = dir.virt_size / sizeof(PE_DebugDirectory);
        for(uint64 dbg_dir_idx = 0; dbg_dir_idx < dbg_dir_count; dbg_dir_idx += 1)
        {
          // rjf: read debug directory
          uint64 dir_addr = vaddr_range.min + dir.virt_off + dbg_dir_idx * sizeof(PE_DebugDirectory);
          ...
          if(dbg_data.type == PE_DebugDirectoryType_CODEVIEW)
          {
            ...
            switch(cv_magic)
            {
              ...
              {
                PE_CvHeaderPDB20 cv;
                uint64 read_size = dmn_process_read_struct(process.dmn_handle, vaddr_range.min+dbg_data.voff, &cv);
  ...
  //
  StringView initial_debug_info_path = StringView();
  {
    Temp scratch = scratch_begin(0, 0);
    StringView exe_folder = str8_chop_last_slash(path);
    ...
    {
      StringView candidate_path = n.str;
  ...
  //
  {
    uint64 hash = ctrl_hash_from_handle(module);
    uint64 slot_idx = hash%ctrl_state.module_image_info_cache.slots_count;
    uint64 stripe_idx = slot_idx%ctrl_state.module_image_info_cache.stripes_count;
    CTRL_ModuleImageInfoCacheSlot* slot = &ctrl_state.module_image_info_cache.slots[slot_idx];
    CTRL_ModuleImageInfoCacheStripe* stripe = &ctrl_state.module_image_info_cache.stripes[stripe_idx];
...
ctrl_thread__next_dmn_event(Arena* arena, DMN_CtrlCtx* ctrl_ctx, CTRL_Msg* msg, DMN_RunCtrls* run_ctrls, CTRL_Spoof* spoof)
{
  ...
  ProfScope("loop . try to get event, run, repeat") for(B32 got_event = 0; got_event == 0;)
  {
    ...
    ProfScope("get next event")
    {
          ...
          case DMN_EventKind_Exception:
          {
               ...
               (spoof == 0 || ev.instruction_pointer != spoof.new_ip_value))
            {
              ...
              if(module != &ctrl_entity_nil)
              {
                ...
                uint64 asan_shadow_base_vaddr = 0;
                ...
                DI_Key dbgi_key = {dbg_path.str, dbg_path.timestamp};
                RDI_Parsed* rdi = di_rdi_from_key(di_scope, &dbgi_key, max_U64);
                RDI_NameMap* unparsed_map = rdi_element_from_name_idx(rdi, NameMaps, RDI_NameMapKind_GlobalVariables);
                {
                  ...
                  StringView name = ("__asan_shadow_memory_dynamic_address");
                  RDI_NameMapNode* node = rdi_name_map_lookup(rdi, &map, name.Ptr, name.size);
                  if(node != 0)
                  {
                    uint32 id_count = 0;
                    uint32* ids = rdi_matches_from_map_node(rdi, node, &id_count);
                    if(id_count > 0)
                    {
                      ...
                      uint64 global_var_voff = global_var.voff;
                      uint64 global_var_vaddr = global_var.voff + module.vaddr_range.min;
                      Arch arch = process.arch;
                      uint64 addr_size = bit_size_from_arch(arch)/8;
                ...
                {
                  uint64 asan_shadow_space_size = TB(128)/8;
      ...
      if(next_event_node != 0 && !should_filter_event)
      {
        ...
        event.str = push_str8_copy(arena, event.str);
    ...
    if(got_event == 0) ProfScope("no event . dmn_ctrl_run for a new one")
    {
      ...
      ProfScope("run for new events")
      {
        LogInfoNamedBlockF("dmn_ctrl_run")
        {
          ...
          LogInfoNamedBlockF("run_entities") for(uint64 idx = 0; idx < run_ctrls.run_entity_count; idx += 1)
          {
        ...
        for(DMN_EventNode* src_n = events.first; src_n != 0; src_n = src_n.next)
        {
          ...
          dst_n.v.str = push_str8_copy(ctrl_state.dmn_event_arena, dst_n.v.str);
  ...
  if(spoof != 0)
  {
    ...
    void* regs_block = push_array(scratch.arena, uint8, regs_block_size_from_arch(arch));
    dmn_thread_read_reg_block(spoof.thread, regs_block);
    uint64 spoof_thread_rip = regs_rip_from_arch_block(arch, regs_block);
  ...
  ProfScope("push ctrl events associated with this demon event") switch(event.kind)
  {
    ...
    case DMN_EventKind_LoadModule:
    {
      ...
      StringView module_path = path_normalized_from_string(scratch.arena, event.str);
      uint64 exe_timestamp = os_properties_from_file_path(module_path).modified;
      ...
      out_evt1.str     = module_path;
      CTRL_Event* out_evt2 = ctrl_event_list_push(scratch.arena, &evts);
      StringView initial_debug_info_path = ctrl_initial_debug_info_path_from_module(scratch.arena, module_handle);
      uint64 debug_info_timestamp = os_properties_from_file_path(initial_debug_info_path).modified;
      ...
      out_evt2.str     = initial_debug_info_path;
    ...
    case DMN_EventKind_UnloadModule:
    {
      ...
      {
        DI_Key dbgi_key = {debug_info_path_ent.str, debug_info_path_ent.timestamp};
        di_close(&dbgi_key);
      }
    }break;
    case DMN_EventKind_DebugString:
    {
      uint64 num_strings = (event.str.Length + ctrl_state.c2u_ring_max_string_size-1) / ctrl_state.c2u_ring_max_string_size;
      for(uint64 string_idx = 0; string_idx < num_strings; string_idx += 1)
      {
        CTRL_Event* out_evt = ctrl_event_list_push(scratch.arena, &evts);
        ...
        out_evt.entity     = ctrl_handle_make(CTRL_MachineID_Local, event.thread);
        out_evt.parent     = ctrl_handle_make(CTRL_MachineID_Local, event.process);
        out_evt.str     = str8_substr(event.str, r1u64(string_idx*ctrl_state.c2u_ring_max_string_size, (string_idx+1)*ctrl_state.c2u_ring_max_string_size));
      }
    }break;
  ...
  {
    uint64 endt_us = os_now_microseconds() + 1000000;
    ...
    ProfScope("pre-emptively load adjacent debug info for %.*s", str8_varg(loaded_module.str))
    {
      //- rjf: calculate seed path
      DI_Key loaded_di_key = ctrl_dbgi_key_from_module(loaded_module);
      StringView loaded_di_name = str8_skip_last_slash(loaded_di_key.path);
      StringView debug_info_ext = str8_skip_last_dot(loaded_di_key.path);
      StringView seed_folder_path = str8_chop_last_slash(loaded_di_key.path);
      if(seed_folder_path.size == 0)
      {
        StringView module_path = loaded_module.str;
      ...
      for(CTRL_DbgDirNode* dir_node = parent_dir_node; dir_node != 0; dir_node = dir_node.parent)
      {
        if(dir_node.search_count == 0 && dir_node.module_direct_count >= 1)
        {
          ...
          StringView dir_node_path = str8_list_join(scratch.arena, &dir_node_path_parts, &(StringJoin){.sep = ("/")});
  ...
  switch(space.kind)
  {
    ...
    case CTRL_EvalSpaceKind_Entity:
    {
      ...
      switch(entity.kind)
      {
        ...
        {
          uint64 read_size = dmn_process_read(entity.handle.dmn_handle, range, out);
        ...
        case CTRL_EntityKind_Thread:
        {
          ...
          MemoryCopy(out, (uint8 *)regs + read_range.min, read_size);
          result = (read_size == dim_1u64(range));
          scratch_end(scratch);
        }break;
      }
    ...
    {
      Temp scratch = scratch_begin(0, 0);
      uint64 meta_eval_idx = space.u64s[0];
      if(meta_eval_idx < ctrl_state.user_meta_evals.count)
      {
        ...
        if(contains_1u64(legal_range, range.min))
        {
          ...
          uint64 bytes_to_read = Min(range_dim, (legal_range.max - range.min));
...
void
ctrl_thread__flush_info_log(StringView str)
{
  os_append_data_to_file_path(ctrl_state.ctrl_thread_log_path, str);
}
...
ctrl_thread__launch(DMN_CtrlCtx* ctrl_ctx, CTRL_Msg* msg)
{
  ...
  uint32 id = dmn_ctrl_launch(ctrl_ctx, &params);
  ...
  for(String8Node* n = msg.entry_points.first; n != 0; n = n.next)
  {
    ...
    CTRL_Entity* entry = ctrl_entity_alloc(ctrl_state.ctrl_thread_entity_store, ctrl_state.ctrl_thread_entity_store.root, CTRL_EntityKind_EntryPoint, Arch_Null, ctrl_handle_zero(), (uint64)id);
    ctrl_entity_equip_string(ctrl_state.ctrl_thread_entity_store, entry, str);
  }
}
...
ctrl_thread__kill(DMN_CtrlCtx* ctrl_ctx, CTRL_Msg* msg)
{
  ...
  uint32 exit_code = msg.exit_code;
...
ctrl_thread__run(DMN_CtrlCtx* ctrl_ctx, CTRL_Msg* msg)
{
  ...
  //
  uint64 sp_check_value = dmn_rsp_from_thread(target_thread.dmn_handle);
  ...
  if(stop_event == 0)
  {
        ...
        machine = machine.next)
    {
      ...
      for(CTRL_Entity* process = machine.first; process != &ctrl_entity_nil; process = process.next)
      {
        ...
        {
          uint64 rip = dmn_rip_from_thread(thread.handle.dmn_handle);
        ...
        node = node.next)
    {
      ...
      uint64 thread_pre_rip = dmn_rip_from_thread(thread);
      uint64 thread_post_rip = thread_pre_rip;
      for(B32 done = 0; !done;)
      {
  ...
  if(stop_event == 0)
  {
    ...
    for(;;)
    {
      ...
      if(msg.run_flags & CTRL_RunFlag_StopOnEntryPoint && !launch_done_first_module && event.kind == DMN_EventKind_HandshakeComplete)
      {
        ...
        uint64 module_base_vaddr = module.vaddr_range.min;
        CTRL_Entity* dbg_path = ctrl_entity_child_from_kind(module, CTRL_EntityKind_DebugInfoPath);
        DI_Key dbgi_key = {dbg_path.str, dbg_path.timestamp};
        RDI_Parsed* rdi = di_rdi_from_key(di_scope, &dbgi_key, max_U64);
        ...
        if(!entries_found)
        {
          for(String8Node* n = msg.entry_points.first; n != 0; n = n.next)
          {
            ...
            uint64 voff = rdi_first_voff_from_procedure(rdi, procedure);
            if(voff != 0)
            {
              entries_found = 1;
        ...
        if(!entries_found)
        {
          ...
          for(uint64 idx = 0; idx < ArrayCount(hi_entry_points); idx += 1)
          {
            uint32 procedure_id = 0;
            {
              StringView name = hi_entry_points[idx];
              RDI_NameMapNode* node = rdi_name_map_lookup(rdi, &map, name.Ptr, name.size);
              uint32 id_count = 0;
              uint32* ids = rdi_matches_from_map_node(rdi, node, &id_count);
              if(id_count > 0)
        ...
        if(!entries_found)
        {
          uint64 voff = ctrl_entry_point_voff_from_module(module.handle);
          if(voff != 0)
          {
            DMN_Trap trap = {process.handle.dmn_handle, module_base_vaddr + voff};
            dmn_trap_chunk_list_push(scratch.arena, &entry_traps, 256, &trap);
        ...
        if(!entries_found)
        {
          ...
          for(uint64 idx = 0; idx < ArrayCount(lo_entry_points); idx += 1)
          {
            uint32 procedure_id = 0;
            {
              StringView name = lo_entry_points[idx];
              RDI_NameMapNode* node = rdi_name_map_lookup(rdi, &map, name.Ptr, name.size);
              uint32 id_count = 0;
              uint32* ids = rdi_matches_from_map_node(rdi, node, &id_count);
              if(id_count > 0)
      ...
      CTRL_Entity* process = ctrl_entity_from_handle(ctrl_state.ctrl_thread_entity_store, ctrl_handle_make(CTRL_MachineID_Local, event.process));
      Arch arch = thread.arch;
      uint64 thread_rip_vaddr = dmn_rip_from_thread(event.thread);
      CTRL_Entity* module = &ctrl_entity_nil;
      {
      ...
      //
      uint64 thread_rip_voff = thread_rip_vaddr - module.vaddr_range.min;
        ...
        ProfScope("for breakpoint events, gather bp info")
      {
        ...
        if(conditions.node_count != 0) ProfScope("evaluate hit stop conditions")
        {
          ...
          uint64 eval_modules_count = Max(1, ctrl_state.ctrl_thread_entity_store.entity_kind_counts[CTRL_EntityKind_Module]);
                ...
                machine = machine.next)
            {
                  ...
                  process = process.next)
              {
                    ...
                    mod = mod.next)
                {
                  ...
                  eval_modules[eval_module_idx].space.u64_0 = (uint64)process;
                  if(mod == module)
                  {
          ...
          for(String8Node* condition_n = conditions.first; condition_n != 0; condition_n = condition_n.next)
          {
            ...
            ProfScope("build eval parse context")
            {
              ...
              ctx.ip_thread_space.u64_0 = (uint64)thread;
              ctx.modules           = eval_modules;
              ctx.modules_count     = eval_modules_count;
              ctx.primary_module    = eval_modules_primary;
            ...
            E_InterpretCtx interpret_ctx = {};
            {
              ...
              ctx.reg_space.u64_0 = (uint64)thread;
              ctx.module_base   = push_array(temp.arena, uint64, 1);
              ctx.module_base[0]= module.vaddr_range.min;
              ctx.tls_base      = push_array(temp.arena, uint64, 1);
            }
            ...
            {
              eval = e_eval_from_string(temp.arena, condition_n.str);
            }
      ...
      {
        uint64 sp = dmn_rsp_from_thread(target_thread.dmn_handle);
      ...
      if(!hard_stop && use_trap_net_logic)
      {
        ...
        {
          uint64 thread_pre_rip = dmn_rip_from_thread(target_thread.dmn_handle);
          uint64 thread_post_rip = thread_pre_rip;
      ...
      if(!hard_stop && use_trap_net_logic)
      {
        if(hit_trap_flags & CTRL_TrapFlag_BeginSpoofMode) LogInfoNamedBlockF("trap_net__begin_spoof_mode")
        {
          ...
          begin_spoof_mode = 1;
          uint64 spoof_sp = dmn_rsp_from_thread(target_thread.dmn_handle);
          spoof_mode = 1;
...
B32
ctrl_u2ms_enqueue_req(CTRL_Handle process, Rng1U64 vaddr_range, B32 zero_terminated, uint64 endt_us)
{
  B32 good = 0;
  OS_MutexScope(ctrl_state.u2ms_ring_mutex) for(;;)
  {
    uint64 unconsumed_size = ctrl_state.u2ms_ring_write_pos-ctrl_state.u2ms_ring_read_pos;
    uint64 available_size = ctrl_state.u2ms_ring_size-unconsumed_size;
...
ASYNC_WORK_DEF(ctrl_mem_stream_work)
{
  ...
  uint64 range_hash = ctrl_hash_from_string(str8_struct(&vaddr_range));
  
  //- rjf: take task
  B32 got_task = 0;
  uint64 preexisting_mem_gen = 0;
  ...
  Arena* range_arena = 0;
  void* range_base = 0;
  uint64 zero_terminated_size = 0;
  uint64 pre_read_mem_gen = dmn_mem_gen();
  uint64 post_read_mem_gen = 0;
  if(got_task && pre_read_mem_gen != preexisting_mem_gen)
  {
    range_size = dim_1u64(vaddr_range_clamped);
    uint64 page_size = os_get_system_info().page_size;
    uint64 arena_size = AlignPow2(range_size + ARENA_HEADER_SIZE, page_size);
    ...
    {
      range_base = push_array_no_zero(range_arena, uint8, range_size);
      uint64 bytes_read = 0;
      uint64 retry_count = 0;
      uint64 retry_limit = range_size > page_size ? 64 : 0;
          ...
          retry_count += 1)
      {
        ...
        {
          uint64 diff = (vaddr_range_clamped_retry.max-vaddr_range_clamped_retry.min)/2;
      ...
      {
        MemoryZero((uint8 *)range_base + bytes_read, range_size-bytes_read);
      }
      zero_terminated_size = range_size;
      if(zero_terminated)
      {
        for(uint64 idx = 0; idx < bytes_read; idx += 1)
        {
          if(((uint8 *)range_base)[idx] == 0)
          {
  ...
  {
    hash = hs_submit_data(key, &range_arena, StringView((uint8*)range_base, zero_terminated_size));
  }
