...
ptr_type(CTRL_MetaEvalFrame__vaddr_type, type(), .flags = TypeFlag_IsExternal, .size = sizeof(ulong));
...
ulong ctrl_hash_from_string(String8 string);
ulong ctrl_hash_from_handle(CTRL_Handle handle);
...
//- rjf: serialization
String8 ctrl_serialized_string_from_event(Arena *arena, CTRL_Event *event, ulong max);
...
ulong ctrl_name_bucket_idx_from_string_size(ulong size);
...
CTRL_Entity *ctrl_entity_alloc(CTRL_EntityStore *store, CTRL_Entity *parent, CTRL_EntityKind kind, Arch arch, CTRL_Handle handle, ulong id);
...
CTRL_Entity *ctrl_module_from_process_vaddr(CTRL_Entity *process, ulong vaddr);
...
ulong ctrl_vaddr_from_voff(CTRL_Entity *module, ulong voff);
ulong ctrl_voff_from_vaddr(CTRL_Entity *module, ulong vaddr);
...
CTRL_EntityRec ctrl_entity_rec_depth_first(CTRL_Entity *entity, CTRL_Entity *subtree_root, ulong sib_off, ulong child_off);
...
U128 ctrl_stored_hash_from_process_vaddr_range(CTRL_Handle process, Rng1U64 range, B32 zero_terminated, B32 *out_is_stale, ulong endt_us);
...
CTRL_ProcessMemorySlice ctrl_query_cached_data_from_process_vaddr_range(Arena *arena, CTRL_Handle process, Rng1U64 range, ulong endt_us);
CTRL_ProcessMemorySlice ctrl_query_cached_zero_terminated_data_from_process_vaddr_limit(Arena *arena, CTRL_Handle process, ulong vaddr, ulong limit, ulong element_size, ulong endt_us);
B32 ctrl_read_cached_process_memory(CTRL_Handle process, Rng1U64 range, B32 *is_stale_out, void *out, ulong endt_us);
...
ulong ctrl_query_cached_tls_root_vaddr_from_thread(CTRL_EntityStore *store, CTRL_Handle handle);
ulong ctrl_query_cached_rip_from_thread(CTRL_EntityStore *store, CTRL_Handle handle);
ulong ctrl_query_cached_rsp_from_thread(CTRL_EntityStore *store, CTRL_Handle handle);
...
//- rjf: cache lookups
PE_IntelPdata *ctrl_intel_pdata_from_module_voff(Arena *arena, CTRL_Handle module_handle, ulong voff);
ulong ctrl_entry_point_voff_from_module(CTRL_Handle module_handle);
...
CTRL_UnwindStepResult ctrl_unwind_step__pe_x64(CTRL_EntityStore *store, CTRL_Handle process_handle, CTRL_Handle module_handle, REGS_RegBlockX64 *regs, ulong endt_us);

//- rjf: abstracted unwind step
CTRL_UnwindStepResult ctrl_unwind_step(CTRL_EntityStore *store, CTRL_Handle process, CTRL_Handle module, Arch arch, void *reg_block, ulong endt_us);

//- rjf: abstracted full unwind
CTRL_Unwind ctrl_unwind_from_thread(Arena *arena, CTRL_EntityStore *store, CTRL_Handle thread, ulong endt_us);
...
B32 ctrl_u2c_push_msgs(CTRL_MsgList *msgs, ulong endt_us);
...
B32 ctrl_u2ms_enqueue_req(CTRL_Handle process, Rng1U64 vaddr_range, B32 zero_terminated, ulong endt_us);
