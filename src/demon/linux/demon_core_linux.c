...
dmn_lnx_exe_path_from_pid(Arena *arena, pid_t pid)
{
  ...
  string exe_link_path = str8f(scratch.arena, "/proc/%d/exe", pid);
...
dmn_lnx_arch_from_pid(pid_t pid)
{
  Arch result = Arch_Null;
  {
    ...
    string exe_path = dmn_lnx_exe_path_from_pid(scratch.arena, pid);
...
dmn_lnx_aux_from_pid(pid_t pid, Arch arch)
{
  ...
  // rjf: open aux data
  string auxv_path = push_str8f(scratch.arena, "/proc/%d/auxv", pid);
...
dmn_process_iter_next(Arena *arena, DMN_ProcessIter *iter, DMN_ProcessInfo *info_out)
{
  ...
  if (got_pid)
  {
    ...
    string name = dmn_lnx_exe_path_from_pid(arena, pid);
    if (name.size == 0)
    {
