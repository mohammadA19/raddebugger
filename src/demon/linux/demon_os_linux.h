...
static DEMON_LNX_ThreadExt*  demon_lnx_thread_ext(DEMON_Entity *entity);

static B32                   demon_lnx_attach_pid(Arena *arena, pid_t pid, DEMON_LNX_AttachNode **new_node);

static String8               demon_lnx_executable_path_from_pid(Arena *arena, pid_t pid);
static int                   demon_lnx_open_memory_fd_for_pid(pid_t pid);

static Arch          demon_lnx_arch_from_pid(pid_t pid);
static DEMON_LNX_ProcessAux  demon_lnx_aux_from_pid(pid_t pid, Arch arch);
static DEMON_LNX_PhdrInfo    demon_lnx_phdr_info_from_memory(int memory_fd, B32 is_32bit,
                                                               U64 phvaddr, U64 phstride, U64 phcount);
static DEMON_LNX_ModuleNode* demon_lnx_module_list_from_process(Arena *arena, DEMON_Entity *process);

static U64     demon_lnx_read_memory(int memory_fd, void *dst, U64 src, U64 size);
static B32     demon_lnx_write_memory(int memory_fd, U64 dst, void *src, U64 size);
static String8 demon_lnx_read_memory_str(Arena *arena, int memory_fd, U64 address);

static void demon_lnx_regs_x64_from_usr_regs_x64(SYMS_RegX64 *dst, DEMON_LNX_UserRegsX64 *src);
static void demon_lnx_usr_regs_x64_from_regs_x64(DEMON_LNX_UserRegsX64 *dst, SYMS_RegX64 *src);

static String8 demon_lnx_read_int_string(int fd);
static B32     demon_lnx_read_expect(int fd, char expect);
static int     demon_lnx_read_whitespace(int fd);
static String8 demon_lnx_read_string(Arena *arena, int fd);

static int demon_lnx_open_maps(pid_t pid);
static B32 demon_lnx_next_map(Arena *arena, int maps, DEMON_LNX_MapsEntry *entry_out);
