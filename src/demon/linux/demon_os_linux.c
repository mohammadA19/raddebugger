...
static ulong demon_lnx_halt_code = 0;
static ulong demon_lnx_halt_user_data = 0;
...
static uint demon_lnx_ptrace_options = (PTRACE_O_TRACEEXIT|
                                       PTRACE_O_EXITKILL|
                                       PTRACE_O_TRACEFORK|
                                       PTRACE_O_TRACEVFORK|
                                       PTRACE_O_TRACECLONE);
...
DEMON_LNX_PhdrInfo
demon_lnx_phdr_info_from_memory(int memory_fd, B32 is_32bit, ulong phvaddr, ulong phentsize, ulong phcount){
  DEMON_LNX_PhdrInfo result = {0};
  result.range.min = max_U64;
  
  // how much phdr will we read?
  ulong phdr_size_expected = (is_32bit?sizeof(SYMS_ElfPhdr32):sizeof(SYMS_ElfPhdr64));
  ulong phdr_stride = (phentsize?phentsize:phdr_size_expected);
  ulong phdr_read_size = ClampTop(phdr_stride, phdr_size_expected);
  
  // scan table
  ulong va = phvaddr;
  for (ulong i = 0; i < phcount; i += 1, va += phdr_stride){
...
demon_lnx_module_list_from_process(Arena *arena, DEMON_Entity *process){
  ...
  ulong first_linkmap_va = 0;
  if (phdr_info.dynamic != 0){
    ulong off = phdr_info.dynamic;
    for (;;){
      SYMS_ElfDyn64 dyn = {0};
      if (is_32bit){
  ...
  if (first_linkmap_va != 0){
    ulong linkmap_va = first_linkmap_va;
...
ulong
demon_lnx_read_memory(int memory_fd, void *dst, ulong src, ulong size){
  ulong bytes_read = 0;
  byte *ptr = (byte*)dst;
  byte *opl = ptr + size;
  ulong cursor = src;
...
B32
demon_lnx_write_memory(int memory_fd, ulong dst, void *src, ulong size){
  B32 result = true;
  byte *ptr = (byte*)src;
  byte *opl = ptr + size;
  ulong cursor = dst;
...
String8
demon_lnx_read_memory_str(Arena *arena, int memory_fd, ulong address){
...
demon_lnx_read_int_string(Arena *arena, int fd, int radix){
  ...
  if (lseek(fd, -to_seek, SEEK_CUR) != -1) {
    ...
    read(fd, buf, to_read);
    buf[to_read] = '\0';
    integer = str8((byte*)buf, (ulong)to_read);
  }
...
demon_lnx_read_u64(int fd, int radix){
  ...
  ulong result = u64_from_str8(integer, radix);
  scratch_end(scratch);
  return(result);
}
...
demon_lnx_next_map(Arena *arena, int maps, DEMON_LNX_MapsEntry *entry_out){
  ...
  do{
    ...
    entry_out->dev_major  = (uint)dev_major;
    entry_out->dev_minor  = (uint)dev_minor;
  ...
  if (demon_ent_root == 0){
    ...
    if (result.first == 0){
      ...
      
      {
        ...
        for (ulong i = 0; i < controls->trap_count; i += 1, trap += 1){
      ...
      for (DEMON_Entity *process = demon_ent_root->first;
        ...
        if (process->kind == DEMON_EntityKind_Process){
          ...
          if (controls->run_entities_are_processes){
            for (ulong i = 0; i < controls->run_entity_count; i += 1){
        ...
        for (DEMON_Entity *proc_node = demon_ent_root->first;
          ...
          for (DEMON_Entity *entity = proc_node->first;
            ...
            if (entity->kind == DEMON_EntityKind_Module){
              ulong base = entity->id;
              ulong name = entity->ext_u64;
...
void
demon_os_halt(ulong code, ulong user_data){
...
demon_os_launch_process(OS_LaunchOptions *options){
  ...
  if (binary != 0){
    ...
    if (pid == -1){
      ...
      uint launch_result = LaunchCode_Null;
      // NOTE(allen): if wait_id != pid we don't know what that means; study that case before
      // deciding how error handling around it works.
      if (wait_id == pid){
        if (WIFSTOPPED(status)){
...
B32
demon_os_attach_process(uint pid){
  B32 result = false;
...
B32
demon_os_kill_process(DEMON_Entity *process, uint exit_code){
  B32 result = false;
  if (process != 0){
...
ulong
demon_os_reserve_memory(DEMON_Entity *process, ulong size){
  ulong result = 0;
  NotImplemented;
  return(result);
}
...
void
demon_os_set_memory_protect_flags(DEMON_Entity *process, ulong page_vaddr, ulong size, DEMON_MemoryProtectFlags flags){
  NotImplemented;
}

void
demon_os_release_memory(DEMON_Entity *process, ulong vaddr, ulong size){
  NotImplemented;
}
...
ulong
demon_os_read_memory(DEMON_Entity *process, void *dst, ulong src_address, ulong size){
  int memory_fd = (int)process->ext_u64;
  ulong result = demon_lnx_read_memory(memory_fd, dst, src_address, size);
  return(result);
}

B32
demon_os_write_memory(DEMON_Entity *process, ulong dst_address, void *src, ulong size){
  int memory_fd = (int)process->ext_u64;
...
demon_os_read_regs_x64(DEMON_Entity *thread, SYMS_RegX64 *dst){
  ...
  if (got_fpr){
    ...
    for (uint i = 0; i < 8; i += 1, dr_d += 1){
      if (i != 4 && i != 5){
        ulong offset = OffsetOf(DEMON_LNX_UserX64, u_debugreg[i]);
        errno = 0;
        int peek_result = ptrace(PTRACE_PEEKUSER, tid, PtrFromInt(offset), 0);
        if (errno == 0){
          dr_d->u32 = (uint)peek_result;
        }
        else{
          got_debug = false;
        }
