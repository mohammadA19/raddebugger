...
global uint64 demon_lnx_halt_code = 0;
global uint64 demon_lnx_halt_user_data = 0;
...
global uint32 demon_lnx_ptrace_options = (PTRACE_O_TRACEEXIT|
                                       PTRACE_O_EXITKILL|
                                       PTRACE_O_TRACEFORK|
                                       PTRACE_O_TRACEVFORK|
                                       PTRACE_O_TRACECLONE);
  ...
  if (first_linkmap_va != 0){
    ...
    for (;;){
      ...
      if (linkmap.base != 0){
        ...
        SYMS_uint64 phvaddr = 0;
        SYMS_uint64 phentsize = 0;
        SYMS_uint64 phcount = 0;
...
demon_lnx_read_s64(int fd, int radix){
  Temp scratch = scratch_begin(0, 0);
  String8 integer = demon_lnx_read_int_string(scratch.arena, fd, radix);
  uint64 result = s64_from_str8(integer, radix);
  scratch_end(scratch);
  return(result);
}
