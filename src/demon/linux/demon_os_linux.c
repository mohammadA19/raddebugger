...
demon_lnx_executable_path_from_pid(Arena *arena, pid_t pid){
  ...
  string exe_symbol_path = push_str8f(scratch.arena, "/proc/%d/exe", pid);
...
demon_lnx_open_memory_fd_for_pid(pid_t pid){
  ...
  string memory_path = push_str8f(scratch.arena, "/proc/%i/mem", pid);
...
demon_lnx_arch_from_pid(pid_t pid){
  ...
  // exe path
  string exe_path = demon_lnx_executable_path_from_pid(scratch.arena, pid);
...
demon_lnx_aux_from_pid(pid_t pid, Arch arch){
  ...
  // open aux data
  Temp scratch = scratch_begin(0, 0);
  string auxv_symbol_path = push_str8f(scratch.arena, "/proc/%d/auxv", pid);
...
demon_lnx_read_u64(int fd, int radix){
  Temp scratch = scratch_begin(0, 0);
  string integer = demon_lnx_read_int_string(scratch.arena, fd, radix);
...
demon_lnx_open_maps(pid_t pid){
  Temp scratch = scratch_begin(0, 0);
  string path = push_str8f(scratch.arena, "/proc/%d/maps", pid);
...
demon_lnx_next_map(Arena *arena, int maps, DEMON_LNX_MapsEntry *entry_out){
  ...
  do{
    ...
    if (str8_match(pathname, ("/"), StringMatchFlag_RightSideSloppy)){
      ...
      entry_out->type = DEMON_LNX_MapsEntryType_Stack;
      string tid = str8_substr(pathname, r1u64(7, pathname.size - 8));
...
demon_os_launch_process(OS_LaunchOptions *options){
  ...
  if (options->cmd_line.node_count > 0){
    ...
    for (String8Node *node = options->cmd_line.first;
      ...
      string string = push_str8_copy(scratch.arena, node->string);
  ...
  char *path = 0;
  {
    string string = push_str8_copy(scratch.arena, options->path);
...
demon_os_attach_process(U32 pid){
  ...
  // open thread list
  if (attached_proc){
    string threads_path = push_str8f(scratch.arena, "/proc/%d/task", pid);
    ...
    if (proc_dir == 0){
      ...
      for (;;){
        ...
        string name = str8_cstring(entry->d_name);
...
demon_os_full_path_from_module(Arena *arena, DEMON_Entity *module){
  ...
  string result = demon_lnx_read_memory_str(arena, memory_fd, name_va);
  return(result);
}
...
demon_os_proc_iter_next(Arena *arena, DEMON_ProcessIter *iter, DEMON_ProcessInfo *info_out){
  ...
  if (dir != 0 && iter->v[1] == 0){
    for (;;){
      ...
      string file_name = str8_cstring((char*)d->d_name);
  ...
  if (got_pid){
    ...
    string name = demon_lnx_executable_path_from_pid(arena, pid);
    if (name.size == 0){
