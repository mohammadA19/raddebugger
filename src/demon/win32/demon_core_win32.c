...
dmn_ctrl_launch(DMN_CtrlCtx *ctx, OS_ProcessLaunchParams *params)
{
    ...
    DMN_AccessScope
    {
        ...
        else
        {
            ...
            StringView message8 = message ? str8_from_16(scratch.arena, str16_cstring(message)) : ("unknown error");
            LocalFree(message);
                ...
                if (evt_good) ProfScope("process the new event")
                {
                    switch (evt.dwDebugEventCode)
                    {
                                        ...
                                        String8List thread_name_strings = {0};
                                        {
                                            ...
                                            for (;total_string_size < KB(4);)
                                            {
                                                ...
                                                if (good_read)
                                                {
                                                    ...
                                                    StringView string_part = str8(buffer, size);
                        ...
                        case OUTPUT_DEBUG_STRING_EVENT:
                        {
                            ...
                            StringView debug_string = str8(buffer, string_size);
