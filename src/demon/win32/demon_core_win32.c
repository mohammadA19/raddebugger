...
dmn_w32_entity_alloc(DMN_W32_Entity *parent, DMN_W32_EntityKind kind, U64 id)
{
  ...
  DMN_W32_Entity *e = dmn_w32_shared->entities_first_free;
  {
    ...
    {
      e = /* no zero */ new DMN_W32_Entity[1];
  ...
  if(id != 0)
  {
    ...
    if(node == 0)
    {
      ...
      {
        node = new DMN_W32_EntityIDHashNode[1];
      }
...
dmn_w32_full_path_from_module(Arena *arena, DMN_W32_Entity *module)
{
  ...
  String8 path8 = {0};
  {
    ...
    if(module->handle != 0)
    {
      ...
      U16 *buffer16 = /* no zero */ new U16[cap16];
...
dmn_init(void)
{
  ...
  dmn_w32_shared = new DMN_W32_Shared[1];
  ...
  dmn_w32_shared->entities_id_hash_slots = new DMN_W32_EntityIDHashSlot[dmn_w32_shared->entities_id_hash_slots_count];
...
dmn_ctrl_run(Arena *arena, DMN_CtrlCtx *ctx, DMN_RunCtrls *ctrls)
{
  ...
  switch(event_gen_path)
  {
    ...
    case DMN_W32_EventGenPath_Run:
    {
      ...
      //
      U8 *trap_swap_bytes = /* no zero */ new U8[ctrls->traps.trap_count];
      ...
      for(DMN_TrapChunkNode *n = ctrls->traps.first; n != 0; n = n->next)
      {
        for(U64 n_idx = 0; n_idx < n->count; n_idx += 1)
        {
          ...
          if(trap->flags != 0)
          {
            ...
            if(task == 0)
            {
              task = new DMN_FlaggedTrapTask[1];
      ...
      ProfScope("produce list of threads which will run")
      {
            ...
            process = process->next)
        {
              ...
              thread = thread->next)
          {
            ...
            if(!is_frozen)
            {
              DMN_W32_EntityNode *n = new DMN_W32_EntityNode[1];
              n->v = thread;
        ...
        if(evt_good) ProfScope("process the new event")
        {
          switch(evt.dwDebugEventCode)
          {
              ...
              if(should_do_rollback) ProfScope("roll back thread's instruction pointer")
              {
                switch(thread->arch)
                {
                  ...
                  default:
                  {
                    ...
                    void *regs_block = new U8[regs_block_size];
              ...
              if(!skip_event)
              {
                ...
                switch(exception->ExceptionCode)
                {
                  ...
                  case DMN_W32_EXCEPTION_RADDBG_SET_VADDR_RANGE_NOTE:
                  {
                    ...
                    U8 *name_buffer = new U8[name_size];
            ...
            case OUTPUT_DEBUG_STRING_EVENT:
            {
              ...
              // rjf: read memory
              U8 *buffer = /* no zero */ new U8[string_size + 1];
