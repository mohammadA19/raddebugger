...
dmn_w32_image_info_from_process_base_vaddr(HANDLE process, uint64 base_vaddr)
{
  ...
  if(got_file_header)
  {
    ...
    if(arch != Arch_Null)
    {
      uint64 optional_off = file_header_off + sizeof(COFF_FileHeader);
      uint32 size = 0;
...
dmn_run_gen()
{
  uint64 result = ins_atomic_u64_eval(&dmn_w32_shared.run_gen);
  return result;
}
...
dmn_mem_gen()
{
  uint64 result = ins_atomic_u64_eval(&dmn_w32_shared.mem_gen);
  return result;
}
...
dmn_reg_gen()
{
  uint64 result = ins_atomic_u64_eval(&dmn_w32_shared.reg_gen);
  return result;
}
