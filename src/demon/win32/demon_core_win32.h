...
struct DMN_W32_Entity
{
  ...
  struct
  {
    struct
    {
      uint64 injection_address;
      B32 did_first_bp;
    }
    proc;
    struct
    {
      uint64 thread_local_base;
      uint64 last_name_hash;
      uint64 name_gather_time_us;
    }
    thread;
    struct
    {
...
struct DMN_W32_Shared
{
  ...
  uint64 entities_id_hash_slots_count;
...
uint64 dmn_w32_hash_from_string(String8 string);
...
String8 dmn_w32_read_memory_str(Arena* arena, HANDLE process_handle, uint64 address);
...
//- rjf: threads
uint16 dmn_w32_real_tag_word_from_xsave(XSAVE_FORMAT* fxsave);
uint16 dmn_w32_xsave_tag_word_from_real_tag_word(uint16 ftw);
...
DWORD dmn_w32_inject_thread(HANDLE process, uint64 start_address);
