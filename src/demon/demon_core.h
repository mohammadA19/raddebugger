...
void dmn_trap_chunk_list_push(Arena *arena, DMN_TrapChunkList *list, ulong cap, DMN_Trap *trap);
...
ulong dmn_rip_from_thread(DMN_Handle thread);
ulong dmn_rsp_from_thread(DMN_Handle thread);
...
uint dmn_ctrl_launch(DMN_CtrlCtx *ctx, OS_ProcessLaunchParams *params);
B32 dmn_ctrl_attach(DMN_CtrlCtx *ctx, uint pid);
B32 dmn_ctrl_kill(DMN_CtrlCtx *ctx, DMN_Handle process, uint exit_code);
...
void dmn_halt(ulong code, ulong user_data);
...
//- rjf: processes
ulong dmn_process_memory_reserve(DMN_Handle process, ulong vaddr, ulong size);
void dmn_process_memory_commit(DMN_Handle process, ulong vaddr, ulong size);
void dmn_process_memory_decommit(DMN_Handle process, ulong vaddr, ulong size);
void dmn_process_memory_release(DMN_Handle process, ulong vaddr, ulong size);
void dmn_process_memory_protect(DMN_Handle process, ulong vaddr, ulong size, OS_AccessFlags flags);
ulong dmn_process_read(DMN_Handle process, Rng1U64 range, void *dst);
...
String8 dmn_process_read_cstring(Arena *arena, DMN_Handle process, ulong addr);

//- rjf: threads
Arch dmn_arch_from_thread(DMN_Handle handle);
ulong dmn_stack_base_vaddr_from_thread(DMN_Handle handle);
ulong dmn_tls_root_vaddr_from_thread(DMN_Handle handle);
