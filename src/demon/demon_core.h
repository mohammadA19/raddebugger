...
//- rjf: handles
static DMN_Handle dmn_handle_zero(void);
static B32 dmn_handle_match(DMN_Handle a, DMN_Handle b);

//- rjf: trap chunk lists
static void dmn_trap_chunk_list_push(Arena *arena, DMN_TrapChunkList *list, U64 cap, DMN_Trap *trap);
static void dmn_trap_chunk_list_concat_in_place(DMN_TrapChunkList *dst, DMN_TrapChunkList *to_push);
static void dmn_trap_chunk_list_concat_shallow_copy(Arena *arena, DMN_TrapChunkList *dst, DMN_TrapChunkList *to_push);

//- rjf: handle lists
static void dmn_handle_list_push(Arena *arena, DMN_HandleList *list, DMN_Handle handle);
static DMN_HandleArray dmn_handle_array_from_list(Arena *arena, DMN_HandleList *list);
static DMN_HandleArray dmn_handle_array_copy(Arena *arena, DMN_HandleArray *src);

//- rjf: event list building
static DMN_Event *dmn_event_list_push(Arena *arena, DMN_EventList *list);

////////////////////////////////
//~ rjf: Thread Reading Helper Functions (Helpers, Implemented Once)

static U64 dmn_rip_from_thread(DMN_Handle thread);
static U64 dmn_rsp_from_thread(DMN_Handle thread);

////////////////////////////////
//~ rjf: Process Reading Helper Functions (Helpers, Implemented Once)

static String8 dmn_process_read_cstring(Arena *arena, DMN_Handle process, U64 addr);

////////////////////////////////
//~ rjf: @dmn_os_hooks Main Layer Initialization (Implemented Per-OS)

static void dmn_init(void);

////////////////////////////////
//~ rjf: @dmn_os_hooks Blocking Control Thread Operations (Implemented Per-OS)

static DMN_CtrlCtx *dmn_ctrl_begin(void);
static void dmn_ctrl_exclusive_access_begin(void);
static void dmn_ctrl_exclusive_access_end(void);
#define DMN_CtrlExclusiveAccessScope DeferLoop(dmn_ctrl_exclusive_access_begin(), dmn_ctrl_exclusive_access_end())
static U32 dmn_ctrl_launch(DMN_CtrlCtx *ctx, OS_ProcessLaunchParams *params);
static B32 dmn_ctrl_attach(DMN_CtrlCtx *ctx, U32 pid);
static B32 dmn_ctrl_kill(DMN_CtrlCtx *ctx, DMN_Handle process, U32 exit_code);
static B32 dmn_ctrl_detach(DMN_CtrlCtx *ctx, DMN_Handle process);
static DMN_EventList dmn_ctrl_run(Arena *arena, DMN_CtrlCtx *ctx, DMN_RunCtrls *ctrls);

////////////////////////////////
//~ rjf: @dmn_os_hooks Halting (Implemented Per-OS)

static void dmn_halt(U64 code, U64 user_data);

////////////////////////////////
//~ rjf: @dmn_os_hooks Introspection Functions (Implemented Per-OS)

//- rjf: run/memory/register counters
static U64 dmn_run_gen(void);
static U64 dmn_mem_gen(void);
static U64 dmn_reg_gen(void);

//- rjf: non-blocking-control-thread access barriers
static B32 dmn_access_open(void);
static void dmn_access_close(void);
#define DMN_AccessScope DeferLoopChecked(dmn_access_open(), dmn_access_close())

//- rjf: processes
static U64 dmn_process_memory_reserve(DMN_Handle process, U64 vaddr, U64 size);
static void dmn_process_memory_commit(DMN_Handle process, U64 vaddr, U64 size);
static void dmn_process_memory_decommit(DMN_Handle process, U64 vaddr, U64 size);
static void dmn_process_memory_release(DMN_Handle process, U64 vaddr, U64 size);
static void dmn_process_memory_protect(DMN_Handle process, U64 vaddr, U64 size, OS_AccessFlags flags);
static U64 dmn_process_read(DMN_Handle process, Rng1U64 range, void *dst);
static B32 dmn_process_write(DMN_Handle process, Rng1U64 range, void *src);
...
//- rjf: threads
static Arch dmn_arch_from_thread(DMN_Handle handle);
static U64 dmn_stack_base_vaddr_from_thread(DMN_Handle handle);
static U64 dmn_tls_root_vaddr_from_thread(DMN_Handle handle);
static B32 dmn_thread_read_reg_block(DMN_Handle handle, void *reg_block);
static B32 dmn_thread_write_reg_block(DMN_Handle handle, void *reg_block);

//- rjf: system process listing
static void dmn_process_iter_begin(DMN_ProcessIter *iter);
static B32  dmn_process_iter_next(Arena *arena, DMN_ProcessIter *iter, DMN_ProcessInfo *info_out);
static void dmn_process_iter_end(DMN_ProcessIter *iter);
