...
dmn_trap_chunk_list_concat_shallow_copy(Arena *arena, DMN_TrapChunkList *dst, DMN_TrapChunkList *to_push)
{
  ...
  {
    DMN_TrapChunkNode *dst_n = new DMN_TrapChunkNode[1];
    dst_n->v     = src_n->v;
    dst_n->cap   = src_n->cap;
...
{
  DMN_HandleNode *node = new DMN_HandleNode[1];
  SLLQueuePush(list->first, list->last, node);
  node->v = handle;
  list->count += 1;
}
...
{
  DMN_EventNode *n = new DMN_EventNode[1];
...
dmn_rip_from_thread(DMN_Handle thread)
{
  ...
  Temp scratch = scratch_begin(0, 0);
  {
    ...
    void *reg_block = new U8[reg_block_size];
