...
void
dmn_trap_chunk_list_push(Arena* arena, DMN_TrapChunkList* list, uint64 cap, DMN_Trap* trap)
{
...
dmn_rip_from_thread(DMN_Handle thread)
{
  ...
  Temp scratch = scratch_begin(0, 0);
  {
    ...
    void* reg_block = push_array(scratch.arena, uint8, reg_block_size);
...
dmn_process_read_cstring(Arena* arena, DMN_Handle process, uint64 addr)
{
  ...
  for(uint64 cursor = addr, stride = 256; ; cursor += stride)
  {
