...
sld_main(CmdLine* cmdl)
{
  ...
  string in_lib_path  = cmd_line_string(cmdl, ("in"));
  string out_lib_path = cmd_line_string(cmdl, ("out"));
  ...
  string in_lib = os_data_from_file_path(scratch.arena, in_lib_path);
  if (in_lib.size == 0) {
  ...
  // read & parse lib
  string           out_lib = push_str8_copy(scratch.arena, in_lib);
  ...
  for (U64 member_idx = 0; member_idx < member_offsets_count; member_idx += 1) {
    ...
    string       member_data = str8_substr(out_lib, member_header.data_range);
    ...
    if (member_type == COFF_DataType_BigObj || member_type == COFF_DataType_Obj) {
      ...
      string              string_table     = str8_substr(member_data, file_header_info.string_table_range);
      for (U64 sect_idx = 0; sect_idx < file_header_info.section_count_no_null; sect_idx += 1) {
        ...
        string             name        = coff_name_from_section_header(string_table, sect_header);
