...
cv_rec_range_stream_from_data(Arena *arena, String8 sym_data, U64 sym_align)
{
  ...
  CV_RecRangeStream *result = new CV_RecRangeStream[1];
...
cv_sym_from_data(Arena *arena, String8 sym_data, U64 sym_align)
{
  ...
  CV_SymParsed *result = new CV_SymParsed[1];
...
cv_leaf_from_data(Arena *arena, String8 leaf_data, CV_TypeId itype_first)
{
  ...
  // convert to result
  CV_LeafParsed *result = new CV_LeafParsed[1];
...
cv_c13_parsed_from_data(Arena *arena, String8 c13_data, String8 strtbl, COFF_SectionHeaderArray sections)
{
  ...
  U64 count = 0;
  {
    ...
    for(; cursor + sizeof(CV_C13SubSectionHeader) <= c13_data.size;)
    {
      ...
      {
        CV_C13SubSectionNode *node = new CV_C13SubSectionNode[1];
  ...
  CV_C13InlineeLinesParsedNode **inlinee_lines_parsed_slots = new CV_C13InlineeLinesParsedNode *[inlinee_lines_parsed_slots_count];
    ...
    switch(node->kind)
    {
      ...
      if(sizeof(CV_C13SubSecLinesHeader) <= cap)
      {
        ...
        for(;read_off+sizeof(CV_C13File) <= read_off_opl;)
        {
          ...
          // emit parsed lines
          CV_C13LinesParsedNode *lines_parsed_node = new CV_C13LinesParsedNode[1];
      ...
      if(sizeof(CV_C13InlineeLinesSig) <= cap)
      {
        ...
        for(;read_off + sizeof(CV_C13InlineeSourceLineHeader) <= read_off_opl;)
        {
          ...
          CV_C13InlineeLinesParsedNode *n = new CV_C13InlineeLinesParsedNode[1];
  ...
  //- rjf: fill output
  //
  CV_C13Parsed *result = new CV_C13Parsed[1];
