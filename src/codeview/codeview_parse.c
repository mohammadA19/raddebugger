...
internal U64
cv_read_numeric(StringView data, U64 offset, CV_NumericParsed *out)
{
...
internal U64
cv_decode_inline_annot_u32(StringView data, U64 offset, U32 *out_value)
{
    U64 cursor = offset;
...
internal U64
cv_decode_inline_annot_s32(StringView data, U64 offset, S32 *out_value)
{
    U32 value;
...
cv_rec_range_stream_from_data(Arena *arena, StringView sym_data, U64 sym_align)
{
...
cv_sym_from_data(Arena *arena, StringView sym_data, U64 sym_align)
{
    Assert(1 <= sym_align && IsPow2OrZero(sym_align));
    ProfBeginFunction();
