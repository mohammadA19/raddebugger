...
static U64 cv_hash_from_string(String8 string);
static U64 cv_hash_from_item_id(CV_ItemId item_id);

//- Numeric Decoder

static CV_NumericParsed cv_numeric_from_data_range(U8 *first, U8 *opl);
static U64              cv_read_numeric(String8 data, U64 offset, CV_NumericParsed *out);
...
static U64 cv_decode_inline_annot_u32(String8 data, U64 offset, U32 *out_value);
static U64 cv_decode_inline_annot_s32(String8 data, U64 offset, S32 *out_value);
static S32 cv_inline_annot_signed_from_unsigned_operand(U32 value);

static CV_C13InlineSiteDecoder      cv_c13_inline_site_decoder_init(U32 file_off, U32 first_source_ln, U32 parent_voff);
static CV_C13InlineSiteDecoderStep cv_c13_inline_site_decoder_step(CV_C13InlineSiteDecoder *decoder, String8 binary_annots);

//- Symbol/Leaf Helpers

static B32 cv_is_udt_name_anon(String8 name);

static B32 cv_is_global_symbol(CV_SymKind kind);
static B32 cv_is_typedef(CV_SymKind kind);
static B32 cv_is_scope_symbol(CV_SymKind kind);
static B32 cv_is_end_symbol(CV_SymKind kind);

static B32                cv_is_udt(CV_LeafKind kind);
static B32                cv_is_leaf_type_server(CV_LeafKind kind);
static B32                cv_is_leaf_pch(CV_LeafKind kind);
static CV_TypeIndexSource cv_type_index_source_from_leaf_kind(CV_LeafKind leaf_kind);

static CV_TypeIndexInfoList cv_get_symbol_type_index_offsets(Arena *arena, CV_SymKind kind, String8 data);
static CV_TypeIndexInfoList cv_get_leaf_type_index_offsets(Arena *arena, CV_LeafKind leaf_kind, String8 data);
static CV_TypeIndexInfoList cv_get_inlinee_type_index_offsets(Arena *arena, String8 raw_data);
static String8Array         cv_get_data_around_type_indices(Arena *arena, CV_TypeIndexInfoList ti_list, String8 data);

static U64     cv_name_offset_from_symbol(CV_SymKind kind, String8 data);
static String8 cv_name_from_symbol(CV_SymKind kind, String8 data);

static CV_UDTInfo cv_get_udt_info(CV_LeafKind kind, String8 data);
static String8    cv_name_from_udt_info(CV_UDTInfo udt_info);

//- rjf: record range stream parsing
static CV_RecRangeStream * cv_rec_range_stream_from_data(Arena *arena, String8 data, U64 align);
static CV_RecRangeArray    cv_rec_range_array_from_stream(Arena *arena, CV_RecRangeStream *stream);
...
static CV_C13Parsed  * cv_c13_parsed_from_data(Arena *arena, String8 c13_data, String8 strtbl, COFF_SectionHeaderArray sections);
