...
tex_topology_make(Vec2uint32 dim, R_Tex2DFormat fmt)
{
  TEX_Topology top = {0};
  top.dim.x = (uint16)Clamp(0, dim.x, max_uint32);
  top.dim.y = (uint16)Clamp(0, dim.y, max_uint32);
  top.fmt = fmt;
  return top;
}
...
tex_init(void)
{
  ...
  for(uint64 idx = 0; idx < tex_shared->stripes_count; idx += 1)
  {
  ...
  tex_shared->u2x_ring_base = push_array_no_zero(arena, uint8, tex_shared->u2x_ring_size);
...
tex_scope_close(TEX_Scope *scope)
{
  for(TEX_Touch *touch = scope->top_touch, *next = 0; touch != 0; touch = next)
  {
    ...
    next = touch->next;
    uint64 slot_idx = hash.u64[1]%tex_shared->slots_count;
    uint64 stripe_idx = slot_idx%tex_shared->stripes_count;
...
internal R_Handle
tex_texture_from_hash_topology(TEX_Scope *scope, uint128 hash, TEX_Topology topology)
{
  R_Handle handle = {0};
  {
    uint64 slot_idx = hash.u64[1]%tex_shared->slots_count;
    ...
    if(node_is_new)
    {
      tex_u2x_enqueue_req(hash, topology, max_uint64);
...
internal R_Handle
tex_texture_from_key_topology(TEX_Scope *scope, HS_Key key, TEX_Topology topology, uint128 *hash_out)
{
  R_Handle handle = {0};
...
internal B32
tex_u2x_enqueue_req(uint128 hash, TEX_Topology top, uint64 endt_us)
{
  B32 good = 0;
  OS_MutexScope(tex_shared->u2x_ring_mutex) for(;;)
  {
    uint64 unconsumed_size = tex_shared->u2x_ring_write_pos-tex_shared->u2x_ring_read_pos;
    uint64 available_size = tex_shared->u2x_ring_size-unconsumed_size;
...
internal void
tex_u2x_dequeue_req(uint128 *hash_out, TEX_Topology *top_out)
{
  OS_MutexScope(tex_shared->u2x_ring_mutex) for(;;)
  {
    uint64 unconsumed_size = tex_shared->u2x_ring_write_pos-tex_shared->u2x_ring_read_pos;
    ...
    os_condition_variable_wait(tex_shared->u2x_ring_cv, tex_shared->u2x_ring_mutex, max_uint64);
  }
  os_condition_variable_broadcast(tex_shared->u2x_ring_cv);
}
...
tex_evictor_thread__entry_point(void *p)
{
  ...
  for(;;)
  {
    ...
    for(uint64 slot_idx = 0; slot_idx < tex_shared->slots_count; slot_idx += 1)
    {
      uint64 stripe_idx = slot_idx%tex_shared->stripes_count;
      TEX_Slot *slot = &tex_shared->slots[slot_idx];
      TEX_Stripe *stripe = &tex_shared->stripes[stripe_idx];
