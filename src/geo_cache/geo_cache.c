...
geo_init(void)
{
  ...
  for(uint64 idx = 0; idx < geo_shared->stripes_count; idx += 1)
  {
  ...
  geo_shared->u2x_ring_base = push_array_no_zero(arena, uint8, geo_shared->u2x_ring_size);
...
geo_scope_close(GEO_Scope *scope)
{
  for(GEO_Touch *touch = scope->top_touch, *next = 0; touch != 0; touch = next)
  {
    ...
    next = touch->next;
    uint64 slot_idx = hash.u64[1]%geo_shared->slots_count;
    uint64 stripe_idx = slot_idx%geo_shared->stripes_count;
...
internal R_Handle
geo_buffer_from_hash(GEO_Scope *scope, uint128 hash)
{
  R_Handle handle = {0};
  if(!u128_match(hash, u128_zero()))
  {
    ...
    if(node_is_new)
    {
      geo_u2x_enqueue_req(hash, max_uint64);
...
internal B32
geo_u2x_enqueue_req(uint128 hash, uint64 endt_us)
{
  B32 good = 0;
  OS_MutexScope(geo_shared->u2x_ring_mutex) for(;;)
  {
    uint64 unconsumed_size = geo_shared->u2x_ring_write_pos-geo_shared->u2x_ring_read_pos;
    uint64 available_size = geo_shared->u2x_ring_size-unconsumed_size;
...
internal void
geo_u2x_dequeue_req(uint128 *hash_out)
{
  OS_MutexScope(geo_shared->u2x_ring_mutex) for(;;)
  {
    uint64 unconsumed_size = geo_shared->u2x_ring_write_pos-geo_shared->u2x_ring_read_pos;
    ...
    os_condition_variable_wait(geo_shared->u2x_ring_cv, geo_shared->u2x_ring_mutex, max_uint64);
  }
  os_condition_variable_broadcast(geo_shared->u2x_ring_cv);
}
...
geo_evictor_thread__entry_point(void *p)
{
  ...
  for(;;)
  {
    ...
    for(uint64 slot_idx = 0; slot_idx < geo_shared->slots_count; slot_idx += 1)
    {
      uint64 stripe_idx = slot_idx%geo_shared->stripes_count;
      GEO_Slot *slot = &geo_shared->slots[slot_idx];
      GEO_Stripe *stripe = &geo_shared->stripes[stripe_idx];
