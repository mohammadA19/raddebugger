...
internal U64
fs_little_hash_from_string(string string)
{
  U64 result = 5381;
...
internal U128
fs_big_hash_from_string_range(string string, Rng1U64 range)
{
...
internal HS_Key
fs_key_from_path_range(string path, Rng1U64 range, U64 endt_us)
{
  Temp scratch = scratch_begin(0, 0);
...
internal U128
fs_hash_from_path_range(string path, Rng1U64 range, U64 endt_us)
{
  U128 hash = {0};
  {
...
fs_properties_from_path(string path)
{
  Temp scratch = scratch_begin(0, 0);
...
internal B32
fs_u2s_enqueue_req(HS_Key key, Rng1U64 range, string path, U64 endt_us)
{
  B32 result = 0;
...
ASYNC_WORK_DEF(fs_stream_work)
{
  ...
  ProfEnd();
  ProfBegin("read");
  string data = os_string_from_file_range(data_arena, file, r1u64(range.min, range.min+read_size));
  ProfEnd();
  os_file_close(file);
