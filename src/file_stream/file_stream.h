...
internal uint64 fs_little_hash_from_string(String8 string);
internal uint128 fs_big_hash_from_string_range(String8 string, Rng1uint64 range);
...
internal uint64 fs_change_gen(void);

////////////////////////////////
//~ rjf: Cache Interaction

internal HS_Key fs_key_from_path_range(String8 path, Rng1uint64 range, uint64 endt_us);
internal uint128 fs_hash_from_path_range(String8 path, Rng1uint64 range, uint64 endt_us);
internal FileProperties fs_properties_from_path(String8 path);

////////////////////////////////
//~ rjf: Streaming Work

internal B32 fs_u2s_enqueue_req(HS_Key key, Rng1uint64 range, String8 path, uint64 endt_us);
internal void fs_u2s_dequeue_req(Arena *arena, HS_Key *key_out, Rng1uint64 *range_out, String8 *path_out);
