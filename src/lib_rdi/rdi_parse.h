...
// Procedure Name -> Line
{
  ...
  RDI_uint64 procedure_first_voff = rdi_first_voff_from_procedure(rdi, procedure);        // 2. procedure -> virtual offset
  ...
  RDI_uint64 file_path_size = 0;                                                          // 5. source file -> path
  RDI_uint8 *file_path = rdi_normal_path_from_source_file(rdi, file, &file_path_size);
...
// Line -> Procedure Name
{
  ...
  RDI_uint32 line_num = 2557;
  RDI_SourceFile *file = rdi_source_file_from_normal_path_cstr(rdi, path);      // 1. path -> source file
  RDI_uint64 voff = rdi_first_voff_from_source_file_line_num(rdi, file, line_num); // 2. (source file, line) -> virtual offset
  RDI_Procedure *procedure = rdi_procedure_from_voff(rdi, voff);                // 3. virtual offset -> procedure
  RDI_uint64 name_size = 0;                                                        // 4. procedure -> name
  RDI_uint8 *name = rdi_name_from_procedure(rdi, procedure, &name_size);
...
{
  RDI_uint8 *raw_data;
  RDI_uint64 raw_data_size;
  RDI_Section *sections;
  RDI_uint64 sections_count;
};
...
struct RDI_ParsedLineTable
{
  ...
  RDI_uint64*    voffs; // [count + 1] sorted
...
struct RDI_ParsedSourceLineMap
{
  ...
  RDI_uint32* nums;   // [count] sorted
  RDI_uint32* ranges; // [count + 1]
  RDI_uint64* voffs;  // [voff_count]
  RDI_uint64 count;
...
RDI_PROC RDI_ParseStatus rdi_parse(RDI_uint8 *data, RDI_uint64 size, RDI_Parsed *out);

////////////////////////////////
//~ Base Parsed Info Extraction Helpers

//- section table/element raw data extraction
RDI_PROC void *rdi_section_raw_data_from_kind(RDI_Parsed *rdi, RDI_SectionKind kind, RDI_SectionEncoding *encoding_out, RDI_uint64 *size_out);
RDI_PROC void *rdi_section_raw_table_from_kind(RDI_Parsed *rdi, RDI_SectionKind kind, RDI_uint64 *count_out);
RDI_PROC void *rdi_section_raw_element_from_kind_idx(RDI_Parsed *rdi, RDI_SectionKind kind, RDI_uint64 idx);
...
RDI_PROC RDI_uint64 rdi_decompressed_size_from_parsed(RDI_Parsed *rdi);

//- decompression
internal void rdi_decompress_parsed(uint8 *decompressed_data, uint64 decompressed_size, RDI_Parsed *og_rdi);

//- strings
RDI_PROC RDI_uint8 *rdi_string_from_idx(RDI_Parsed *rdi, RDI_uint32 idx, RDI_uint64 *len_out);

//- index runs
RDI_PROC RDI_uint32 *rdi_idx_run_from_first_count(RDI_Parsed *rdi, RDI_uint32 raw_first, RDI_uint32 raw_count, RDI_uint32 *n_out);

//- line info
RDI_PROC void rdi_parsed_from_line_table(RDI_Parsed *rdi, RDI_LineTable *line_table, RDI_ParsedLineTable *out);
RDI_PROC RDI_uint64 rdi_line_info_idx_range_from_voff(RDI_ParsedLineTable *line_info, RDI_uint64 voff, RDI_uint64 *n_out);
RDI_PROC RDI_uint64 rdi_line_info_idx_from_voff(RDI_ParsedLineTable *line_info, RDI_uint64 voff);
RDI_PROC void rdi_parsed_from_source_line_map(RDI_Parsed *rdi, RDI_SourceLineMap *map, RDI_ParsedSourceLineMap *out);
RDI_PROC RDI_uint64 *rdi_line_voffs_from_num(RDI_ParsedSourceLineMap *map, RDI_uint32 linenum, RDI_uint32 *n_out);

//- vmap lookups
RDI_PROC RDI_uint64 rdi_vmap_idx_from_voff(RDI_VMapEntry *vmap, RDI_uint64 vmap_count, RDI_uint64 voff);
...
RDI_PROC RDI_NameMapNode *rdi_name_map_lookup(RDI_Parsed *p, RDI_ParsedNameMap *map, RDI_uint8 *str, RDI_uint64 len);
RDI_PROC RDI_uint32 *rdi_matches_from_map_node(RDI_Parsed *p, RDI_NameMapNode *node, RDI_uint32 *n_out);

////////////////////////////////
//~ High-Level Composite Lookup Functions

//- procedures
RDI_PROC RDI_Procedure *rdi_procedure_from_name(RDI_Parsed *rdi, RDI_uint8 *name, RDI_uint64 name_size);
RDI_PROC RDI_Procedure *rdi_procedure_from_name_cstr(RDI_Parsed *rdi, char *cstr);
RDI_PROC RDI_uint8 *rdi_name_from_procedure(RDI_Parsed *rdi, RDI_Procedure *procedure, RDI_uint64 *len_out);
...
RDI_PROC RDI_uint64 rdi_first_voff_from_procedure(RDI_Parsed *rdi, RDI_Procedure *procedure);
RDI_PROC RDI_uint64 rdi_opl_voff_from_procedure(RDI_Parsed *rdi, RDI_Procedure *procedure);
RDI_PROC RDI_Procedure *rdi_procedure_from_voff(RDI_Parsed *rdi, RDI_uint64 voff);

//- scopes
RDI_PROC RDI_uint64 rdi_first_voff_from_scope(RDI_Parsed *rdi, RDI_Scope *scope);
RDI_PROC RDI_uint64 rdi_opl_voff_from_scope(RDI_Parsed *rdi, RDI_Scope *scope);
RDI_PROC RDI_Scope *rdi_scope_from_voff(RDI_Parsed *rdi, RDI_uint64 voff);
...
//- global variables
RDI_PROC RDI_GlobalVariable *rdi_global_variable_from_voff(RDI_Parsed *rdi, RDI_uint64 voff);

//- units
RDI_PROC RDI_Unit *rdi_unit_from_voff(RDI_Parsed *rdi, RDI_uint64 voff);
RDI_PROC RDI_LineTable *rdi_line_table_from_unit(RDI_Parsed *rdi, RDI_Unit *unit);

//- line tables
RDI_PROC RDI_Line rdi_line_from_voff(RDI_Parsed *rdi, RDI_uint64 voff);
RDI_PROC RDI_Line rdi_line_from_line_table_voff(RDI_Parsed *rdi, RDI_LineTable *line_table, RDI_uint64 voff);
RDI_PROC RDI_SourceFile *rdi_source_file_from_line(RDI_Parsed *rdi, RDI_Line *line);

//- source files
RDI_PROC RDI_SourceFile *rdi_source_file_from_normal_path(RDI_Parsed *rdi, RDI_uint8 *name, RDI_uint64 name_size);
RDI_PROC RDI_SourceFile *rdi_source_file_from_normal_path_cstr(RDI_Parsed *rdi, char *cstr);
RDI_PROC RDI_uint8 *rdi_normal_path_from_source_file(RDI_Parsed *rdi, RDI_SourceFile *src_file, RDI_uint64 *len_out);
...
RDI_PROC RDI_uint64 rdi_first_voff_from_source_file_line_num(RDI_Parsed *rdi, RDI_SourceFile *src_file, RDI_uint32 line_num);

//- source line maps
RDI_PROC RDI_uint64 rdi_first_voff_from_source_line_map_num(RDI_Parsed *rdi, RDI_SourceLineMap *map, RDI_uint32 line_num);

//- file path nodes
RDI_PROC RDI_FilePathNode *rdi_parent_from_file_path_node(RDI_Parsed *rdi, RDI_FilePathNode *node);
RDI_PROC RDI_uint8 *rdi_name_from_file_path_node(RDI_Parsed *rdi, RDI_FilePathNode *node, RDI_uint64 *len_out);

////////////////////////////////
//~ Parser Helpers

#define rdi_parse__min(a,b) (((a)<(b))?(a):(b))
RDI_PROC RDI_uint64 rdi_cstring_length(char *cstr);
RDI_PROC RDI_uint64 rdi_size_from_bytecode_stream(RDI_uint8 *ptr, RDI_uint8 *opl);
