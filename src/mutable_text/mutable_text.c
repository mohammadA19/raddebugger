...
mtx_mut_thread__entry_point(void* p)
{
  ...
  for (;;)
  {
    ...
    if (op.range.max != op.range.min || op.replace.size != 0)
    {
      ...
      string pre_replace_data = str8_substr(data, r1u64(0, op.range.min));
      string post_replace_data = str8_substr(data, r1u64(op.range.max, data.size));
      ...
      string new_data = str8(new_data_base, new_data_size);
