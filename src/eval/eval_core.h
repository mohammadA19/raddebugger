...
static U64 e_hash_from_string(U64 seed, String8 string);
#define e_value_u64(v) (E_Value){.u64 = (v)}

////////////////////////////////
//~ rjf: Expr Kind Enum Functions

static RDI_EvalOp e_opcode_from_expr_kind(E_ExprKind kind);
static B32        e_expr_kind_is_comparison(E_ExprKind kind);

////////////////////////////////
//~ rjf: Key Type Functions

static B32 e_key_match(E_Key a, E_Key b);
static E_Key e_key_zero(void);

////////////////////////////////
//~ rjf: Type Key Type Functions

static void e_type_key_list_push(Arena *arena, E_TypeKeyList *list, E_TypeKey key);
static void e_type_key_list_push_front(Arena *arena, E_TypeKeyList *list, E_TypeKey key);
static E_TypeKeyList e_type_key_list_copy(Arena *arena, E_TypeKeyList *src);

////////////////////////////////
//~ rjf: Message Functions

static void e_msg(Arena *arena, E_MsgList *msgs, E_MsgKind kind, Rng1U64 range, String8 text);
static void e_msgf(Arena *arena, E_MsgList *msgs, E_MsgKind kind, Rng1U64 range, char *fmt, ...);
static void e_msg_list_concat_in_place(E_MsgList *dst, E_MsgList *to_push);
static E_MsgList e_msg_list_copy(Arena *arena, E_MsgList *src);

////////////////////////////////
//~ rjf: Space Functions

static E_Space e_space_make(E_SpaceKind kind);
static B32 e_space_match(E_Space a, E_Space b);

////////////////////////////////
//~ rjf: Map Functions

//- rjf: string -> num
static E_String2NumMap e_string2num_map_make(Arena *arena, U64 slot_count);
static void e_string2num_map_insert(Arena *arena, E_String2NumMap *map, String8 string, U64 num);
static U64 e_num_from_string(E_String2NumMap *map, String8 string);
static E_String2NumMapNodeArray e_string2num_map_node_array_from_map(Arena *arena, E_String2NumMap *map);
static int e_string2num_map_node_qsort_compare__num_ascending(E_String2NumMapNode **a, E_String2NumMapNode **b);
static void e_string2num_map_node_array_sort__in_place(E_String2NumMapNodeArray *array);

//- rjf: string -> expr
static E_String2ExprMap e_string2expr_map_make(Arena *arena, U64 slot_count);
static void e_string2expr_map_insert(Arena *arena, E_String2ExprMap *map, String8 string, E_Expr *expr);
static void e_string2expr_map_inc_poison(E_String2ExprMap *map, String8 string);
static void e_string2expr_map_dec_poison(E_String2ExprMap *map, String8 string);
static E_Expr *e_string2expr_map_lookup(E_String2ExprMap *map, String8 string);

//- rjf: string -> type-key
static E_String2TypeKeyMap e_string2typekey_map_make(Arena *arena, U64 slots_count);
static void e_string2typekey_map_insert(Arena *arena, E_String2TypeKeyMap *map, String8 string, E_TypeKey key);
static E_TypeKey e_string2typekey_map_lookup(E_String2TypeKeyMap *map, String8 string);

//- rjf: auto hooks
static E_AutoHookMap e_auto_hook_map_make(Arena *arena, U64 slots_count);
static void e_auto_hook_map_insert_new_(Arena *arena, E_AutoHookMap *map, E_AutoHookParams *params);
...
static E_String2NumMap *e_push_locals_map_from_rdi_voff(Arena *arena, RDI_Parsed *rdi, U64 voff);
static E_String2NumMap *e_push_member_map_from_rdi_voff(Arena *arena, RDI_Parsed *rdi, U64 voff);

////////////////////////////////
//~ rjf: Cache Creation & Selection

static E_Cache *e_cache_alloc(void);
static void e_cache_release(E_Cache *cache);
static void e_select_cache(E_Cache *cache);

////////////////////////////////
//~ rjf: Evaluation Phase Markers

static void e_select_base_ctx(E_BaseCtx *ctx);
static void e_select_ir_ctx(E_IRCtx *ctx);
...
static E_Key e_parent_key_push(E_Key key);
static E_Key e_parent_key_pop(void);
#define E_ParentKey(key) DeferLoop(e_parent_key_push(key), e_parent_key_pop())

//- rjf: key construction
static E_Key e_key_from_string(String8 string);
static E_Key e_key_from_stringf(char *fmt, ...);
static E_Key e_key_from_expr(E_Expr *expr);

//- rjf: base key -> bundle helper
static E_CacheBundle *e_cache_bundle_from_key(E_Key key);

//- rjf: bundle -> pipeline stage outputs
static E_Parse e_parse_from_bundle(E_CacheBundle *bundle);
static E_IRTreeAndType e_irtree_from_bundle(E_CacheBundle *bundle);
static String8 e_bytecode_from_bundle(E_CacheBundle *bundle);
static E_Interpretation e_interpretation_from_bundle(E_CacheBundle *bundle);
...
static String8 e_full_expr_string_from_key(Arena *arena, E_Key key);

//- rjf: comprehensive bundle
static E_Eval e_eval_from_bundle(E_CacheBundle *bundle);
static E_Eval e_value_eval_from_eval(E_Eval eval);
...
static E_AutoHookMatchList e_push_auto_hook_matches_from_type_key(Arena *arena, E_TypeKey type_key);
static E_AutoHookMatchList e_auto_hook_matches_from_type_key(E_TypeKey type_key);

//- rjf: string IDs
static U64 e_id_from_string(String8 string);
static String8 e_string_from_id(U64 id);

////////////////////////////////
//~ rjf: Key Extension Functions

static E_Key e_key_wrap(E_Key key, String8 string);
static E_Key e_key_wrapf(E_Key key, char *fmt, ...);
...
static U64 e_base_offset_from_eval(E_Eval eval);
static U64 e_range_size_from_eval(E_Eval eval);

////////////////////////////////
//~ rjf: Debug Functions

static String8 e_debug_log_from_expr_string(Arena *arena, String8 string);
