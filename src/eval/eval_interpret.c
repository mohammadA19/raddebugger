...
internal void
e_select_interpret_ctx(E_InterpretCtx *ctx, RDI_Parsed *primary_rdi, uint64 ip_voff)
{
  e_interpret_ctx = ctx;
  ...
  if(primary_rdi != 0)
  {
    ...
    for(uint64 loc_block_idx = proc->frame_base_location_first; loc_block_idx < proc->frame_base_location_opl; loc_block_idx += 1)
    {
      ...
      if (block->scope_off_first <= ip_voff && ip_voff < block->scope_off_opl) {
        ...
        uint8  *all_location_data      = rdi_table_from_name(primary_rdi, LocationData, &all_location_data_size);
        if(block->location_data_off + sizeof(RDI_LocationKind) <= all_location_data_size)
        {
          ...
          {
            uint8      *bytecode_ptr  = all_location_data + block->location_data_off + sizeof(RDI_LocationKind);
            uint8      *bytecode_opl  = all_location_data + all_location_data_size;
            uint64      bytecode_size = rdi_size_from_bytecode_stream(bytecode_ptr, bytecode_opl);
...
internal B32
e_space_read(E_Space space, void *out, Rng1uint64 range)
{
  ProfBeginFunction();
  B32 result = 0;
...
internal B32
e_space_write(E_Space space, void *in, Rng1uint64 range)
{
  ProfBeginFunction();
  B32 result = 0;
...
e_interpret(String8 bytecode)
{
  ...
  for(;ptr < opl;)
  {
    ...
    E_Value imm = {0};
    {
      uint32 decode_size = RDI_DECODEN_FROM_CTRLBITS(ctrlbits);
      uint8 *next_ptr = ptr + decode_size;
      if(next_ptr > opl)
      {
        result.code = E_InterpretationCode_BadOp;
        goto done;
    ...
    E_Value *svals = 0;
    {
      uint32 pop_count = RDI_POPN_FROM_CTRLBITS(ctrlbits);
    ...
    switch(op)
    {
      ...
      case RDI_EvalOp_Abs:
      {
        if(imm.u64 == RDI_EvalTypeGroup_float)
        {
        ...
        else if(imm.u64 == RDI_EvalTypeGroup_double)
        {
      ...
      {
        uint32 in = imm.u64&0xFF;
        uint32 out = (imm.u64 >> 8)&0xFF;
        if(in != out)
        {
          switch(in + out*RDI_EvalTypeGroup_COUNT)
          {
            case RDI_EvalTypeGroup_float + RDI_EvalTypeGroup_U*RDI_EvalTypeGroup_COUNT:
            {
              nval.u64 = (uint64)svals[0].f32;
            }break;
            case RDI_EvalTypeGroup_double + RDI_EvalTypeGroup_U*RDI_EvalTypeGroup_COUNT:
            {
              nval.u64 = (uint64)svals[0].f64;
            }break;
            
            case RDI_EvalTypeGroup_float + RDI_EvalTypeGroup_S*RDI_EvalTypeGroup_COUNT:
            {
              nval.s64 = (uint64)svals[0].f32;
            }break;
            case RDI_EvalTypeGroup_double + RDI_EvalTypeGroup_S*RDI_EvalTypeGroup_COUNT:
            {
              nval.s64 = (uint64)svals[0].f64;
            }break;
            
            case RDI_EvalTypeGroup_U + RDI_EvalTypeGroup_float*RDI_EvalTypeGroup_COUNT:
            {
              nval.f32 = (float)svals[0].u64;
            }break;
            case RDI_EvalTypeGroup_S + RDI_EvalTypeGroup_float*RDI_EvalTypeGroup_COUNT:
            {
              nval.f32 = (float)svals[0].s64;
            }break;
            case RDI_EvalTypeGroup_double + RDI_EvalTypeGroup_float*RDI_EvalTypeGroup_COUNT:
            {
              nval.f32 = (float)svals[0].f64;
            }break;
            
            case RDI_EvalTypeGroup_U + RDI_EvalTypeGroup_double*RDI_EvalTypeGroup_COUNT:
            {
              nval.f64 = (double)svals[0].u64;
            }break;
            case RDI_EvalTypeGroup_S + RDI_EvalTypeGroup_double*RDI_EvalTypeGroup_COUNT:
            {
              nval.f64 = (double)svals[0].s64;
            }break;
            case RDI_EvalTypeGroup_float + RDI_EvalTypeGroup_double*RDI_EvalTypeGroup_COUNT:
            {
      ...
      {
        uint64 bytes_to_read = imm.u64;
        uint64 offset = svals[0].u64;
        if(offset + bytes_to_read <= sizeof(E_Value))
        {
          E_Value src_val = svals[1];
          MemoryCopy(&nval.u512.u64[0], (uint8 *)(&src_val.u512.u64[0]) + offset, bytes_to_read);
        }
      }break;
      
      case RDI_EvalOp_ByteSwap:
      {
        uint64 byte_size = imm.u64;
        switch(byte_size)
        {
          default:
          {
    ...
    // rjf: push
    {
      uint64 push_count = RDI_PUSHN_FROM_CTRLBITS(ctrlbits);
      if(push_count == 1)
      {
