...
internal void
e_oplist_push_uconst(Arena *arena, E_OpList *list, uint64 x)
{
  if(0){}
  else if(x <= 0xFF)       { e_oplist_push_op(arena, list, RDI_EvalOp_Constuint8,  e_value_u64(x)); }
  else if(x <= 0xFFFF)     { e_oplist_push_op(arena, list, RDI_EvalOp_Constuint16, e_value_u64(x)); }
  else if(x <= 0xFFFFFFFF) { e_oplist_push_op(arena, list, RDI_EvalOp_Constuint32, e_value_u64(x)); }
  else                     { e_oplist_push_op(arena, list, RDI_EvalOp_Constuint64, e_value_u64(x)); }
}

internal void
e_oplist_push_sconst(Arena *arena, E_OpList *list, uint64 x)
{
  if(-0x80 <= x && x <= 0x7F)
  {
    e_oplist_push_op(arena, list, RDI_EvalOp_Constuint8, e_value_u64((uint64)x));
    e_oplist_push_op(arena, list, RDI_EvalOp_TruncSigned, e_value_u64(8));
  }
  else if(-0x8000 <= x && x <= 0x7FFF)
  {
    e_oplist_push_op(arena, list, RDI_EvalOp_Constuint16, e_value_u64((uint64)x));
    e_oplist_push_op(arena, list, RDI_EvalOp_TruncSigned, e_value_u64(16));
  }
  else if(-0x80000000ll <= x && x <= 0x7FFFFFFFll)
  {
    e_oplist_push_op(arena, list, RDI_EvalOp_Constuint32, e_value_u64((uint64)x));
    e_oplist_push_op(arena, list, RDI_EvalOp_TruncSigned, e_value_u64(32));
  }
  else
  {
    e_oplist_push_op(arena, list, RDI_EvalOp_Constuint64, e_value_u64((uint64)x));
  }
}
...
internal E_IRNode *
e_irtree_const_u(Arena *arena, uint64 v)
{
  // rjf: choose op
  RDI_EvalOp op = RDI_EvalOp_Constuint64;
  if     (v < 0x100)       { op = RDI_EvalOp_Constuint8; }
  else if(v < 0x10000)     { op = RDI_EvalOp_Constuint16; }
  else if(v < 0x100000000) { op = RDI_EvalOp_Constuint32; }
...
internal E_IRNode *
e_irtree_leaf_u128(Arena *arena, uint128 u128)
{
  E_IRNode *n = e_push_irnode(arena, RDI_EvalOp_Constuint128);
  n->value.u128 = u128;
  return n;
}
...
e_irtree_mem_read_type(Arena *arena, E_IRNode *c, E_TypeKey type_key)
{
  ...
  uint64 byte_size = e_type_byte_size_from_key(type_key);
...
e_irtree_convert_hi(Arena *arena, E_IRNode *c, E_TypeKey out, E_TypeKey in)
{
  ...
  uint8 in_group  = e_type_group_from_kind(in_kind);
  uint8 out_group = e_type_group_from_kind(out_kind);
  uint32 conversion_rule = rdi_eval_conversion_kind_from_typegroups(in_group, out_group);
  if(conversion_rule == RDI_EvalConversionKind_Legal)
  {
    result = e_irtree_convert_lo(arena, result, out_group, in_group);
  }
  uint64 in_byte_size = e_type_byte_size_from_key(in);
  uint64 out_byte_size = e_type_byte_size_from_key(out);
...
e_irtree_resolve_to_value(Arena *arena, E_Mode from_mode, E_IRNode *tree, E_TypeKey type_key)
{
  ...
  if(e_type_kind_from_key(type_key) == E_TypeKind_Bitfield)
  {
    ...
    if(type->byte_size <= sizeof(uint64))
    {
...
e_expr_is_poisoned(E_Expr *expr)
{
  ...
  uint64 hash = e_hash_from_string(5381, str8_struct(&expr));
  uint64 slot_idx = hash%e_cache->used_expr_map->slots_count;
...
E_TYPE_ACCESS_FUNCTION_DEF(default)
{
  ...
  switch(expr->kind)
  {
    ...
    case E_ExprKind_ArrayIndex:
    {
      ...
      uint64 direct_type_size = e_type_byte_size_from_key(direct_type);
  ...
  for(Task *t = first_task; t != 0; t = t->next)
  {
    ...
    switch(kind)
    {
      ...
      case E_ExprKind_Deref:
      {
        ...
        uint64 r_type_direct_size = e_type_byte_size_from_key(r_type_direct);
      ...
      case E_ExprKind_Cast:
      {
        ...
        uint64 cast_type_byte_size = e_type_byte_size_from_key(cast_type);
        ...
        uint64 casted_type_byte_size = e_type_byte_size_from_key(casted_type);
        uint8 in_group  = e_type_group_from_kind(casted_type_kind);
        uint8 out_group = e_type_group_from_kind(cast_type_kind);
      ...
      case E_ExprKind_Sizeof:
      {
        ...
        // rjf: generate
        {
          uint64 r_type_byte_size = e_type_byte_size_from_key(r_type);
          result.root     = e_irtree_const_u(arena, r_type_byte_size);
          result.type_key = e_type_key_basic(E_TypeKind_uint64);
          result.mode     = E_Mode_Value;
        }
      }break;
      ...
      case E_ExprKind_ByteSwap:
      {
        ...
        E_TypeKey r_type = e_type_key_unwrap(r_tree.type_key, E_TypeUnwrapFlag_AllDecorative);
        E_TypeKind r_type_kind = e_type_kind_from_key(r_type);
        uint64 r_type_size = e_type_byte_size_from_key(r_type);
...
#define E_ArithPath_PtrArrayCompare 3
        ...
        uint32 arith_path = E_ArithPath_Normal;
        ...
        else if(kind == E_ExprKind_Sub)
        {
          ...
          if(l_is_pointer_like && r_is_pointer_like)
          {
            ...
            E_TypeKey r_type_direct = e_type_key_unwrap(r_type, E_TypeUnwrapFlag_All);
            uint64 l_type_direct_byte_size = e_type_byte_size_from_key(l_type_direct);
            uint64 r_type_direct_byte_size = e_type_byte_size_from_key(r_type_direct);
      ...
      {
        uint64 val = expr->value.u64;
        E_IRNode *new_tree = e_irtree_const_u(arena, val);
        E_TypeKey type_key = zero_struct;
        if(0){}
        else if(val <= max_uint32){type_key = e_type_key_basic(E_TypeKind_uint32);}
        else if(val <= max_uint64){type_key = e_type_key_basic(E_TypeKind_uint64);}
        else                   {type_key = e_type_key_basic(E_TypeKind_uint64);}
        result.root     = new_tree;
        result.type_key = type_key;
        result.mode     = E_Mode_Value;
      }break;
      ...
      case E_ExprKind_Leafdouble:
      {
        ...
        E_IRNode *new_tree = e_irtree_const_u(arena, val);
        result.root     = new_tree;
        result.type_key = e_type_key_basic(E_TypeKind_double);
        result.mode     = E_Mode_Value;
      }break;
      ...
      {
        uint32 val = expr->value.u32;
        E_IRNode *new_tree = e_irtree_const_u(arena, val);
        result.root     = new_tree;
        result.type_key = e_type_key_basic(E_TypeKind_float);
        result.mode     = E_Mode_Value;
      }break;
      ...
      case E_ExprKind_LeafIdentifier:
      {
        ...
        for(uint64 path_idx = 0; !generated && path_idx < identifier_resolution_rule->count; path_idx += 1)
        {
          ...
          ProfScope("identifier resolution %i", path) switch(path)
          {
            ...
            if(!string_mapped && (qualifier.size == 0 || str8_match(qualifier, str8_lit("member"), 0)))
            {
              ...
              uint32 module_idx = (uint32)(module - e_base_ctx->modules);
            ...
            if(!string_mapped && (qualifier.size == 0 || str8_match(qualifier, str8_lit("local"), 0)))
            {
              ...
              uint32 module_idx = (uint32)(module - e_base_ctx->modules);
              RDI_Parsed *rdi = module->rdi;
              uint64 local_num = e_num_from_string(e_ir_ctx->locals_map, string__redirected);
              if(local_num != 0)
              {
                ...
                uint64 ip_voff = e_base_ctx->thread_ip_voff;
                for(uint32 loc_block_idx = local->location_first;
                ...
                if(match.idx != 0 && match.dbgi_idx < e_base_ctx->modules_count)
                {
                  ...
                  switch(match.section)
                  {
                    ...
                    case RDI_SectionKind_GlobalVariables:
                    {
                      ...
                      E_OpList oplist = {0};
                      e_oplist_push_op(arena, &oplist, RDI_EvalOp_Constuint64, e_value_u64(module->vaddr_range.min + global_var->voff));
                      string_mapped = 1;
                      mapped_type_key = e_type_key_ext(e_type_kind_from_rdi(type_node->kind), type_idx, (uint32)match.dbgi_idx);
                      mapped_bytecode = e_bytecode_from_oplist(arena, &oplist);
                    ...
                    case RDI_SectionKind_Constants:
                    {
                      ...
                      uint32 type_idx = constant->type_idx;
                      RDI_TypeNode *type_node = rdi_element_from_name_idx(rdi, TypeNodes, type_idx);
                      RDI_uint32 constant_value_off = *rdi_element_from_name_idx(rdi, ConstantValueTable, constant->constant_value_idx);
                      RDI_uint32 constant_value_size = *rdi_element_from_name_idx(rdi, ConstantValueTable, constant->constant_value_idx+1) - constant_value_off;
                      if(constant_value_size <= 8)
                      {
                        RDI_uint64 constant_value_data_size = 0;
                        RDI_uint8 *constant_value_data = rdi_table_from_name(rdi, ConstantValueData, &constant_value_data_size);
                        if(0 <= constant_value_off && constant_value_off + constant_value_size <= constant_value_data_size)
                        {
                          ...
                          E_OpList oplist = {0};
                          e_oplist_push_op(arena, &oplist, RDI_EvalOp_Constuint64, e_value_u64(value));
                          string_mapped = 1;
                          mapped_type_key = e_type_key_ext(e_type_kind_from_rdi(type_node->kind), type_idx, (uint32)match.dbgi_idx);
                          mapped_bytecode = e_bytecode_from_oplist(arena, &oplist);
                          mapped_bytecode_mode = E_Mode_Value;
          ...
          if(!generated && mapped_location_block != 0)
          {
            ...
            if(block->location_data_off + sizeof(RDI_LocationKind) <= all_location_data_size)
            {
              ...
              switch(loc_kind)
              {
                ...
                if(block->location_data_off + sizeof(RDI_LocationRegPlusuint16) <= all_location_data_size)
                {
                  string_mapped = 1;
                  RDI_LocationRegPlusuint16 loc = *(RDI_LocationRegPlusuint16 *)(all_location_data + block->location_data_off);
                  E_OpList oplist = {0};
                  uint64 byte_size = bit_size_from_arch(arch)/8;
                  uint64 regread_param = RDI_EncodeRegReadParam(loc.reg_code, byte_size, 0);
                  e_oplist_push_op(arena, &oplist, RDI_EvalOp_RegRead, e_value_u64(regread_param));
                  e_oplist_push_op(arena, &oplist, RDI_EvalOp_Constuint16, e_value_u64(loc.offset));
                ...
                if(block->location_data_off + sizeof(RDI_LocationReg) <= all_location_data_size)
                {
                  ...
                  uint64 byte_pos = 0;
                  uint64 regread_param = RDI_EncodeRegReadParam(loc.reg_code, byte_size, byte_pos);
                  e_oplist_push_op(arena, &oplist, RDI_EvalOp_RegRead, e_value_u64(regread_param));
                  mapped_bytecode = e_bytecode_from_oplist(arena, &oplist);
                  mapped_bytecode_mode = E_Mode_Value;
                  mapped_bytecode_space = space;
      ...
      {
        E_IRNode *new_tree = e_push_irnode(arena, RDI_EvalOp_Constuint64);
      ...
      {
        E_IRNode *new_tree = e_push_irnode(arena, RDI_EvalOp_Constuint128);
        new_tree->value = expr->value;
        new_tree->space = expr->space;
        result.root     = new_tree;
        result.type_key = expr->type_key;
      ...
      case E_ExprKind_Unsigned:
      {
        ...
        if(e_type_kind_is_signed(direct_type_kind))
        {
          ...
          switch(direct_type_kind)
          {
            ...
            case E_TypeKind_uint8:{new_kind = E_TypeKind_uint8;}break;
            case E_TypeKind_uint16:{new_kind = E_TypeKind_uint16;}break;
            case E_TypeKind_uint32:{new_kind = E_TypeKind_uint32;}break;
            case E_TypeKind_uint64:{new_kind = E_TypeKind_uint64;}break;
            case E_TypeKind_uint128:{new_kind = E_TypeKind_uint128;}break;
      ...
      if(e_type_kind_is_pointer_or_ref(e_type_kind_from_key(type_key)))
      {
           ...
           ptee_kind == E_TypeKind_Class)
        {
          ...
          if(has_vtable)
          {
            ...
            uint64 class_base_vaddr = interpret.value.u64;
            uint64 vtable_vaddr = 0;
            uint64 addr_size = e_type_byte_size_from_key(type_key);
            if(e_space_read(interpret.space, &vtable_vaddr, r1u64(class_base_vaddr, class_base_vaddr+addr_size)))
            {
              ...
              for(uint64 idx = 0; idx < e_base_ctx->modules_count; idx += 1)
              {
  ...
  switch(op)
  {
    ...
    default:
    {
      ...
      else
      {
        ...
        uint16 ctrlbits = rdi_eval_op_ctrlbits_table[op];
        uint64 child_count = RDI_POPN_FROM_CTRLBITS(ctrlbits);
        uint64 idx = 0;
        for(E_IRNode *child = root->first;
...
{
  // rjf: allocate buffer
  uint64 size = oplist->encoded_size;
  uint8 *str = push_array_no_zero(arena, uint8, size);
      ...
      op = op->next)
  {
    ...
    switch(opcode)
    {
      default:
      {
        ...
        uint16 ctrlbits = rdi_eval_op_ctrlbits_table[opcode];
        uint64 extra_byte_count = RDI_DECODEN_FROM_CTRLBITS(ctrlbits);
        uint8 *next_ptr = ptr + 1 + extra_byte_count;
        Assert(next_ptr <= opl);
      ...
      {
        // rjf: compute bytecode advance
        uint8 *next_ptr = ptr + 2 + op->value.u64;
        Assert(next_ptr <= opl);
        
        // rjf: fill
        ptr[0] = opcode;
        ptr[1] = (uint8)op->value.u64;
      ...
      {
        // rjf: compute bytecode advance
        uint64 size = op->string.size;
        uint8 *next_ptr = ptr + size;
        Assert(next_ptr <= opl);
      ...
      {
        // rjf: compute bytecode advance
        uint64 extra_byte_count = sizeof(E_Space);
        uint8 *next_ptr = ptr + 1 + extra_byte_count;
        Assert(next_ptr <= opl);
