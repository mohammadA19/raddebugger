...
internal void e_token_chunk_list_push(Arena *arena, E_TokenChunkList *list, uint64 chunk_size, E_Token *token);
...
internal E_Expr *e_push_expr(Arena *arena, E_ExprKind kind, Rng1uint64 range);
...
internal E_Parse e_push_parse_from_string_tokens__prec(Arena *arena, String8 text, E_TokenArray tokens, uint64 max_precedence, uint64 max_chain_count);
