...
//- rjf: string -> num
E_String2NumMap e_string2num_map_make(Arena *arena, ulong slot_count);
void e_string2num_map_insert(Arena *arena, E_String2NumMap *map, String8 string, ulong num);
ulong e_num_from_string(E_String2NumMap *map, String8 string);
...
//- rjf: string -> expr
E_String2ExprMap e_string2expr_map_make(Arena *arena, ulong slot_count);
...
E_String2NumMap *e_push_locals_map_from_rdi_voff(Arena *arena, RDI_Parsed *rdi, ulong voff);
E_String2NumMap *e_push_member_map_from_rdi_voff(Arena *arena, RDI_Parsed *rdi, ulong voff);
...
void e_token_chunk_list_push(Arena *arena, E_TokenChunkList *list, ulong chunk_size, E_Token *token);
...
uint e_parse_ctx_module_idx_from_rdi(RDI_Parsed *rdi);
...
E_Expr *e_expr_ref_array_index(Arena *arena, E_Expr *lhs, ulong index);
...
E_Parse e_parse_expr_from_text_tokens__prec(Arena *arena, String8 text, E_TokenArray *tokens, long max_precedence);
