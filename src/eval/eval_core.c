...
{
  E_TypeKeyNode *n = new E_TypeKeyNode[1];
  n->v = key;
...
e_string2num_map_make(Arena *arena, U64 slot_count)
{
  ...
  map.slots = new E_String2NumMapSlot[map.slots_count];
  return map;
}
...
e_string2num_map_insert(Arena *arena, E_String2NumMap *map, String8 string, U64 num)
{
  ...
  if(existing_node == 0)
  {
    E_String2NumMapNode *node = new E_String2NumMapNode[1];
...
e_string2num_map_node_array_from_map(Arena *arena, E_String2NumMap *map)
{
  ...
  result.v = new E_String2NumMapNode *[result.count];
  U64 idx = 0;
...
e_string2expr_map_make(Arena *arena, U64 slot_count)
{
  ...
  map.slots_count = slot_count;
  map.slots = new E_String2ExprMapSlot[map.slots_count];
  return map;
}
...
e_string2expr_map_insert(Arena *arena, E_String2ExprMap *map, String8 string, E_Expr *expr)
{
  ...
  if(existing_node == 0)
  {
    E_String2ExprMapNode *node = new E_String2ExprMapNode[1];
    SLLQueuePush_N(map->slots[slot_idx].first, map->slots[slot_idx].last, node, hash_next);
    node->string = push_str8_copy(arena, string);
    existing_node = node;
...
e_string2typekey_map_make(Arena *arena, U64 slots_count)
{
  ...
  map.slots = new E_String2TypeKeySlot[map.slots_count];
  return map;
}
...
{
  E_String2TypeKeyNode *n = new E_String2TypeKeyNode[1];
  U64 hash = e_hash_from_string(5381, string);
  U64 slot_idx = hash%map->slots_count;
...
e_auto_hook_map_make(Arena *arena, U64 slots_count)
{
  E_AutoHookMap map = {0};
  map.slots_count = slots_count;
  map.slots = new E_AutoHookSlot[map.slots_count];
  return map;
}
...
e_auto_hook_map_insert_new_(Arena *arena, E_AutoHookMap *map, E_AutoHookParams *params)
{
  ...
  if(e_type_key_match(e_type_key_zero(), type_key))
  {
    ...
    for(U64 off = 0; off <= params->type_pattern.size; off += 1)
    {
      ...
      if(byte == 0 || byte == '?')
      {
        ...
        if(new_part.size != 0)
        {
          E_PatternPart *p = new E_PatternPart[1];
  ...
  if(!e_type_key_match(e_type_key_zero(), type_key) ||
     pattern.count != 0)
  {
    E_AutoHookNode *node = new E_AutoHookNode[1];
...
e_push_locals_map_from_rdi_voff(Arena *arena, RDI_Parsed *rdi, U64 voff)
{
  ...
  //- rjf: build blank map
  E_String2NumMap *map = new E_String2NumMap[1];
...
e_select_base_ctx(E_BaseCtx *ctx)
{
  ...
  e_cache->key_slots = new E_CacheSlot[e_cache->key_slots_count];
  e_cache->string_slots_count = 4096;
  e_cache->string_slots = new E_CacheSlot[e_cache->string_slots_count];
  ...
  e_cache->cons_content_slots = new E_ConsTypeSlot[e_cache->cons_content_slots_count];
  e_cache->cons_key_slots = new E_ConsTypeSlot[e_cache->cons_key_slots_count];
  e_cache->member_cache_slots_count = 256;
  e_cache->member_cache_slots = new E_MemberCacheSlot[e_cache->member_cache_slots_count];
  e_cache->enum_val_cache_slots_count = 256;
  e_cache->enum_val_cache_slots = new E_EnumValCacheSlot[e_cache->enum_val_cache_slots_count];
  e_cache->type_cache_slots_count = 1024;
  e_cache->type_cache_slots = new E_TypeCacheSlot[e_cache->type_cache_slots_count];
  ...
  e_cache->used_expr_map = new E_UsedExprMap[1];
  e_cache->used_expr_map->slots_count = 64;
  e_cache->used_expr_map->slots = new E_UsedExprSlot[e_cache->used_expr_map->slots_count];
  e_cache->type_auto_hook_cache_map = new E_TypeAutoHookCacheMap[1];
  e_cache->type_auto_hook_cache_map->slots_count = 256;
  e_cache->type_auto_hook_cache_map->slots = new E_TypeAutoHookCacheSlot[e_cache->type_auto_hook_cache_map->slots_count];
  e_cache->string_id_gen = 0;
  e_cache->string_id_map = new E_StringIDMap[1];
  e_cache->string_id_map->id_slots_count = 1024;
  e_cache->string_id_map->id_slots = new E_StringIDSlot[e_cache->string_id_map->id_slots_count];
  e_cache->string_id_map->hash_slots_count = 1024;
  e_cache->string_id_map->hash_slots = new E_StringIDSlot[e_cache->string_id_map->hash_slots_count];
}
...
e_select_ir_ctx(E_IRCtx *ctx)
{
  ...
  if(ctx->macro_map == 0)      { ctx->macro_map = new E_String2ExprMap[1); ctx->macro_map[0] = e_string2expr_map_make(e_cache->arena, 512]; }
  e_ir_ctx = ctx;
}
...
e_full_expr_string_from_key(Arena *arena, E_Key key)
{
  ...
  if(!e_key_match(bundle->parent_key, e_key_zero()))
  {
    ...
    for(ParentResolveTask *t = first_task, *next = 0; t != 0; (t = next, next = 0))
    {
      ...
      {
        ParentResolveTask *task = new ParentResolveTask[1];
  ...
  if(e_ir_ctx != 0)
  {
    ...
    if(map != 0 && map->slots_count != 0)
    {
      ...
      for(E_AutoHookNode *n = map->slots[slot_idx].first; n != 0; n = n->hash_next)
      {
        ...
        {
          E_AutoHookMatch *match = new E_AutoHookMatch[1];
            ...
            if(pattern_string.size == 0)
            {
              ...
              for(B32 done = 0; !done && scan_pos < type_string.size; scan_pos += 1)
              {
                   ...
                   brack_nest_depth == 0)
                {
                  ...
                  E_AutoHookWildcardInst *inst = new E_AutoHookWildcardInst[1];
...
e_auto_hook_matches_from_type_key(E_TypeKey type_key)
{
  E_AutoHookMatchList matches = {0};
  {
    ...
    if(node == 0)
    {
      node = new E_TypeAutoHookCacheNode[1];
