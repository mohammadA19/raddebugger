...
internal U64
e_hash_from_string(U64 seed, string string)
{
...
e_msgf(Arena *arena, E_MsgList *msgs, E_MsgKind kind, Rng1U64 range, char *fmt, ...)
{
  va_list args;
  va_start(args, fmt);
  string text = push_str8fv(arena, fmt, args);
  va_end(args);
...
e_auto_hook_map_insert_new_(Arena *arena, E_AutoHookMap *map, E_AutoHookParams *params)
{
  ...
  if (e_type_key_match(e_type_key_zero(), type_key))
  {
    ...
    for (U64 off = 0; off <= params->type_pattern.size; off += 1)
    {
      ...
      if (byte == 0 || byte == '?')
      {
        string new_part = str8_substr(params->type_pattern, r1u64(start_string_off, off));
        if (new_part.size != 0)
        {
      ...
      if (byte == '?')
      {
        ...
        {
          off += 2;
          string wildcard_inst_names_string = str8_skip(params->type_pattern, off);
  ...
  for (Task *task = first_task; task != 0; task = task->next)
  {
    ...
    if (scope != 0)
    {
      ...
      for (U32 local_idx = scope->local_first; local_idx < local_opl_idx; local_idx += 1)
      {
        ...
        string name = push_str8_copy(arena, str8(local_name_str, local_name_size));
...
internal E_Key
e_key_from_string(string string)
{
  E_Key parent_key = {0};
...
e_interpretation_from_bundle(E_CacheBundle *bundle)
{
  if (bundle != &e_cache_bundle_nil && !(bundle->flags & E_CacheBundleFlag_Interpret))
  {
    ...
    string bytecode = e_bytecode_from_bundle(bundle);
...
e_push_auto_hook_matches_from_type_key(Arena *arena, E_TypeKey type_key)
{
  ...
  if (e_ir_ctx != 0)
  {
    ...
    string type_string = str8_skip_chop_whitespace(e_type_string_from_key(scratch.arena, type_key));
    ...
    if (map != 0 && map->first_pattern != 0)
    {
          ...
          auto_hook_node = auto_hook_node->pattern_order_next)
      {
        ...
        B32 fits_this_type_string = 1;
        {
          ...
          {
            string pattern_string = part->string;
            ...
            {
              string terminator_pattern_string = part->next ? part->next->string : str8_zero();
              ...
              for (B32 done = 0; !done && scan_pos < type_string.size; scan_pos += 1)
              {
                ...
                if ((type_string.str[scan_pos] == ',' || done) &&
                   ...
                   brack_nest_depth == 0)
                {
                  string wildcard_inst_string = str8_skip_chop_whitespace(str8_substr(type_string, r1u64(start_inst_off, scan_pos)));
...
internal U64
e_id_from_string(string string)
{
  U64 hash = e_hash_from_string(5381, string);
...
internal E_Key
e_key_wrap(E_Key key, string string)
{
...
e_debug_log_from_expr_string(Arena *arena, string string)
{
  ...
  E_Parse parse = e_push_parse_from_string(scratch.arena, expr_text);
  {
    ...
    for EachIndex(idx, tokens.count)
    {
      ...
      string token_string = str8_substr(expr_text, token.range);
