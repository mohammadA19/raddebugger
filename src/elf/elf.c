...
elf_hdr64_from_hdr32(ELF_Hdr32 h32)
{
  ...
  h64.e_entry     = (uint64)h32.e_entry;
  h64.e_phoff     = (uint64)h32.e_phoff;
  h64.e_shoff     = (uint64)h32.e_shoff;
...
elf_shdr64_from_shdr32(ELF_Shdr32 h32)
{
  ...
  h64.sh_flags     = (uint64)h32.sh_flags;
  h64.sh_addr      = (uint64)h32.sh_addr;
  h64.sh_offset    = (uint64)h32.sh_offset;
  h64.sh_size      = (uint64)h32.sh_size;
  h64.sh_link      = h32.sh_link;
  h64.sh_info      = h32.sh_info;
  h64.sh_addralign = (uint64)h32.sh_addralign;
  h64.sh_entsize   = (uint64)h32.sh_entsize;
  return h64;
}
...
elf_phdr64_from_phdr32(ELF_Phdr32 h32)
{
  ...
  h64.p_offset   = (uint64)h32.p_offset;
  h64.p_vaddr    = (uint64)h32.p_vaddr;
  h64.p_paddr    = (uint64)h32.p_paddr;
  h64.p_filesz   = (uint64)h32.p_filesz;
  h64.p_memsz    = (uint64)h32.p_memsz;
  h64.p_align    = (uint64)h32.p_align;
  return h64;
}
...
{
  ELF_Dyn64 h64 = {0};
  h64.tag       = (uint64)h32.tag;
  h64.val       = (uint64)h32.val;
  return h64;
}
...
{
  uint32 sym  = ELfloat_R_SYM(rel32.r_info);
  uint32 type = ELfloat_R_TYPE(rel32.r_info);
  ELF_Rel64 rel64 = {0};
  rel64.r_info    = ELdouble_R_INFO(sym, type);
...
{
  uint32 sym  = ELfloat_R_SYM(rela32.r_info);
  uint32 type = ELfloat_R_TYPE(rela32.r_info);
  ELF_Rela64 rela64 = {0};
  rela64.r_offset   = rela32.r_info;
  rela64.r_info     = ELdouble_R_INFO(sym, type);
