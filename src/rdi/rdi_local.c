...
internal string
rdi_string_from_bytecode(Arena *arena, RDI_Arch arch, string bc)
{
  Temp scratch = scratch_begin(&arena, 1);
  String8List fmt = {0};
  for(U64 cursor = 0; cursor < bc.size; )
  {
    ...
    switch (op) {
      ...
      case RDI_EvalOp_RegRead: {
        ...
        string     reg_str   = rdi_string_from_reg_code(scratch.arena, arch, reg_code);
      ...
      case RDI_EvalOp_Convert: {
        ...
        string in_str  = rdi_string_from_eval_type_group(scratch.arena, in);
        string out_str = rdi_string_from_eval_type_group(scratch.arena, out);
      ...
      case RDI_EvalOp_CallSiteValue: {
        ...
        string call_site_bc      = {0};
        cursor += str8_deserial_read_block(bc, cursor, call_site_bc_size, &call_site_bc);
        
        string call_site_str = rdi_string_from_bytecode(scratch.arena, arch, call_site_bc);
    ...
    string op_str = rdi_string_from_eval_op(scratch.arena, op);
    if (imm_fmt.size) {
  ...
  string result = str8_list_join(arena, &fmt, &(StringJoin){.sep = (", ")});
      ...
      block_idx +=1)
  {
    ...
    else
    {
      ...
      switch(kind)
      {
        ...
        {
          string bc     = str8_range(loc_base_ptr + 1, loc_data_opl);
          string bc_str = rdi_string_from_bytecode(scratch.arena, arch, bc);
...
#define DumpSubset(name) if(flags & RDI_DumpSubsetFlag_##name) DeferLoop(dumpf("////////////////////////////////\n//~ %S\n\n%S:\n{", rdi_name_title_from_dump_subset_table[RDI_DumpSubset_##name], rdi_name_lowercase_from_dump_subset_table[RDI_DumpSubset_##name]), dump(("}\n\n")))
  ...
  DumpSubset(DataSections)
  {
    ...
    for EachIndex(idx, rdi->sections_count)
    {
      ...
      string          kind_str = rdi_string_from_data_section_kind(scratch.arena, kind);
  ...
  DumpSubset(BinarySections)
  {
    ...
    for EachIndex(idx, count)
    {
      ...
      string name = str8_from_rdi_string_idx(rdi, bin_section->name_string_idx);
      string flags = rdi_string_from_binary_section_flags(scratch.arena, bin_section->flags);
  ...
  DumpSubset(SourceLineMaps)
  {
    ...
    for EachIndex(idx, count)
    {
      ...
      for EachIndex(line_num_idx, line_map.count)
      {
        ...
        string voffs_string = str8_list_join(temp.arena, &list, &(StringJoin){.sep = (", ")});
  ...
  DumpSubset(TypeNodes)
  {
    ...
    for EachIndex(idx, count)
    {
      ...
      else if(type->kind == RDI_TypeKind_Function)
      {
        ...
        string param_idx_str = str8_list_join(scratch.arena, &param_idx_strings, &(StringJoin){.pre = ("["), .sep = (", "), .post = ("]")});
      ...
      else
      {
        ...
        if(member_lo < member_hi)
        {
          ...
          for(U32 member_idx = member_lo; member_idx < member_hi; member_idx += 1)
          {
            ...
            string kind_str = rdi_string_from_member_kind(scratch.arena, member->kind);
            string name_str = str8_from_rdi_string_idx(rdi, member->name_string_idx);
  ...
  DumpSubset(Scopes)
  {
    ...
    for EachIndex(idx, count)
    {
      ...
      for(RDI_Scope *scope = root, *rec_next = nil; scope != nil; scope = rec_next)
      {
        ...
        string voff_range_list_string = {0};
        {
        ...
        string procedure_name = str8_from_rdi_string_idx(rdi, rdi_element_from_name_idx(rdi, Procedures, scope->proc_idx)->name_string_idx);
        ...
        string inline_site_name = str8_from_rdi_string_idx(rdi, rdi_element_from_name_idx(rdi, InlineSites, scope->inline_site_idx)->name_string_idx);
  ...
  DumpSubset(InlineSites)
  {
    ...
    for EachIndex(idx, count)
    {
      ...
      Temp scratch = scratch_begin(&arena, 1);
      string inline_site_idx = push_str8f(scratch.arena, "inline_site[%u]",      idx);
      string type_idx        = push_str8f(scratch.arena, "type_idx: %u,",       inline_site->type_idx);
      string owner_type_idx  = push_str8f(scratch.arena, "owner_type_idx: %u,", inline_site->owner_type_idx);
      string line_table_idx  = push_str8f(scratch.arena, "line_table_idx: %u,", inline_site->line_table_idx);
      string name            = push_str8f(scratch.arena, "'%S'", str8_from_rdi_string_idx(rdi, inline_site->name_string_idx));
  ...
  DumpSubset(NameMaps)
  {
    ...
    for EachIndex(idx, count)
    {
      ...
      for EachIndex(bucket_idx, name_map.bucket_count)
      {
        ...
        {
          Temp temp = temp_begin(scratch.arena);
          string str = str8_from_rdi_string_idx(rdi, node_ptr->string_idx);
          string indices = {0};
