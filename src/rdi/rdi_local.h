...
read_only global string rdi_name_lowercase_from_dump_subset_table[] =
{
...
read_only global string rdi_name_title_from_dump_subset_table[] =
{
...
internal string str8_from_rdi_string_idx(RDI_Parsed *rdi, U32 idx);

////////////////////////////////
//~ rjf: String <=> Enum

internal string rdi_string_from_data_section_kind(Arena *arena, RDI_SectionKind v);
internal string rdi_string_from_arch             (Arena *arena, RDI_Arch        v);
internal string rdi_string_from_language         (Arena *arena, RDI_Language    v);
internal string rdi_string_from_local_kind       (Arena *arena, RDI_LocalKind   v);
#if 0 // TODO(rjf): conflicts with RDI...
internal string rdi_string_from_type_kind        (Arena *arena, RDI_TypeKind    v);
#endif
internal string rdi_string_from_member_kind      (Arena *arena, RDI_MemberKind  v);
internal string rdi_string_from_name_map_kind(RDI_NameMapKind kind);

internal string rdi_string_from_binary_section_flags(Arena *arena, RDI_BinarySectionFlags flags);
internal string rdi_string_from_type_modifier       (Arena *arena, RDI_TypeModifierFlags  flags);
internal string rdi_string_from_udt_flags           (Arena *arena, RDI_UDTFlags           flags);
internal string rdi_string_from_link_flags          (Arena *arena, RDI_LinkFlags          flags);
internal string rdi_string_from_bytecode(Arena *arena, RDI_Arch arch, string bc);
