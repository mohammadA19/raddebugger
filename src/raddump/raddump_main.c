...
entry_point(CmdLine *cmdline)
{
  ...
  RD_Option opts = 0;
  {
    for (CmdLineOpt *cmd = cmdline->options.first; cmd != 0; cmd = cmd->next) {
      ...
      for (U64 opt_idx = 0; opt_idx < ArrayCount(g_rd_dump_option_map); ++opt_idx) {
        string opt_name = str8_cstring(g_rd_dump_option_map[opt_idx].name);
  ...
  // read input
  string file_path = str8_list_first(&cmdline->inputs);
  string raw_data  = os_data_from_file_path(arena, file_path);
  ...
  string out_string = str8_list_join(arena, out, &(StringJoin){ .sep = ("\n"),});
