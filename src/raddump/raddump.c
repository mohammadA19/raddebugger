...
internal String8
rd_string_from_reg_off(Arena *arena, String8 reg_str, uint64 reg_off)
{
  String8 result;
  if (reg_off > 0) {
...
internal String8
rd_string_from_hex_u8(Arena *arena, uint8 *v, uint64 count)
{
  Temp scratch = scratch_begin(&arena, 1);
  String8List list = {0};
  for (uint64 i = 0; i < count; ++i) {
...
internal uint64
rd_range_bsearch(Rng1uint64 *ranges, uint64 count, uint64 x)
{
...
rd_section_markers_from_rdi(Arena *arena, RDI_Parsed *rdi)
{
  ...
  Rng1uint64 *sect_vranges = push_array(scratch.arena, Rng1uint64, sect_count);
  ...
  for (uint64 i = 0; i < proc_count; ++i) {
    ...
    if (sect_range_idx < sect_count) {
      Rng1uint64 sect_range = sect_vranges[sect_range_idx];
...
internal RD_MarkerArray *
rd_section_markers_from_coff_symbol_table(Arena *arena, String8 string_table, uint64 section_count, COFF_Symbol32Array symbols)
{
  Temp scratch = scratch_begin(&arena, 1);
...
internal String8
cv_string_from_reg_off(Arena *arena, CV_Arch arch, uint32 reg_idx, uint32 reg_off)
{
  Temp scratch = scratch_begin(&arena, 1);
...
internal void
cv_format_debug_sections(Arena *arena, String8List *out, String8 indent, String8 raw_image, String8 string_table, uint64 section_count, COFF_SectionHeader *sections)
{
  CV_Arch arch = ~0;
  {
    B32 keep_parsing = 1;
    for (uint64 i = 0; i < section_count && keep_parsing; ++i) {
      ...
      Rng1uint64             sect_frange = rng_1u64(header->foff, header->foff+header->fsize);
...
pe_print(Arena *arena, String8List *out, String8 indent, String8 raw_data, RD_Option opts, RDI_Parsed *rdi)
{
  ...
  Rng1uint64 *dirs_file_ranges = push_array(scratch.arena, Rng1uint64, dir_count);
  Rng1uint64 *dirs_virt_ranges = push_array(scratch.arena, Rng1uint64, dir_count);
...
elf_print_dwarf_expressions(Arena *arena, String8List *out, String8 indent, String8 raw_data)
{
  ...
  B32              relaxed     = 1;
  Rng1uint64List      cu_ranges   = dw_unit_ranges_from_data(scratch.arena, dwarf_input.sec[DW_Section_Info].data);
  ...
  if (bin.hdr.e_type == ELF_Type_Exec || bin.hdr.e_type == ELF_Type_Dyn) {
    uint64 cu_idx = 0;
    for (Rng1uint64Node *cu_range_n = cu_ranges.first; cu_range_n != 0; cu_range_n = cu_range_n->next, ++cu_idx) {
      Temp comp_temp = temp_begin(scratch.arena);
      
      uint64         cu_base   = cu_range_n->v.min;
      Rng1uint64     cu_range  = cu_range_n->v;
      ...
      uint32 lexical_block_depth = 0;
      
      for (uint64 info_off = cu.first_tag_info_off, tag_size = 0; info_off < cu.info_range.max; info_off += tag_size) {
        DW_Tag tag = {0};
        ...
        if (tag.kind == DW_TagKind_LexicalBlock || tag.kind == DW_TagKind_CompileUnit || tag.kind == DW_TagKind_InlinedSubroutine || tag.kind == DW_TagKind_SubProgram) {
          ...
          if (ranges_attrib->attrib_kind == DW_AttribKind_Ranges) {
            Rng1uint64List ranges = dw_rnglist_from_tag_attrib_kind(temp.arena, &dwarf_input, &cu, ranges_attrib);
          }
          temp_end(temp);
        }
#endif
