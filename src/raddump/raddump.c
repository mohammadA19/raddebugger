...
internal RDI_Parsed *
rd_rdi_from_pe(Arena *arena, StringView pe_path)
{
  ...
  // run converter
  StringView raw_rdi = rc_rdi_from_cmd_line(scratch.arena, &cmdl);
  ...
  if (raw_rdi.size) {
    ...
    StringView parse_status_string = str8_zero();
...
internal StringView
rd_string_from_reg_off(Arena *arena, StringView reg_str, S64 reg_off)
{
  StringView result;
  if (reg_off > 0) {
...
internal RD_MarkerArray *
rd_section_markers_from_coff_symbol_table(Arena *arena, StringView string_table, U64 section_count, COFF_Symbol32Array symbols)
{
  Temp scratch = scratch_begin(&arena, 1);
...
rd_disasm_next_instruction(Arena *arena, Arch arch, U64 addr, StringView raw_code)
{
...
rd_format_hex_array(Arena *arena, U8 *ptr, U64 size)
{
  ...
  StringView result = str8((U8*)buf, buf_size);
  return result;
}
...
{
  Temp scratch = scratch_begin(&arena, 1);
  StringView reg_str = cv_string_from_reg_id(scratch.arena, arch, reg_idx);
  StringView result  = rd_string_from_reg_off(arena, reg_str, reg_off);
  scratch_end(scratch);
  return result;
}
  ...
  switch (kind) {
    ...
    case CV_LeafKind_STRUCT2: {
      ...
      if (lf.props & CV_TypeProp_HasUniqueName) {
        StringView unique_name = str8_zero();
...
internal void
cv_format_debug_sections(Arena *arena, String8List *out, StringView indent, StringView raw_image, StringView string_table, U64 section_count, COFF_SectionHeader *sections)
{
  CV_Arch arch = ~0;
  {
    B32 keep_parsing = 1;
    for (U64 i = 0; i < section_count && keep_parsing; ++i) {
      ...
      StringView             sect_name   = coff_name_from_section_header(string_table, header);
...
internal void
pe_print_data_directory_ranges(Arena *arena, String8List *out, StringView indent, U64 count, PE_DataDirectory *dirs)
{
  Temp scratch = scratch_begin(&arena, 1);
...
internal void
pe_print_static_import_table(Arena *arena, String8List *out, StringView indent, U64 image_base, PE_ParsedStaticImportTable imptab)
{
  Temp scratch = scratch_begin(&arena, 1);
...
internal void
pe_print_delay_import_table(Arena *arena, String8List *out, StringView indent, U64 image_base, PE_ParsedDelayImportTable imptab)
{
  if (imptab.count) {
    Temp scratch = scratch_begin(&arena, 1);
                          ...
                          RDI_Parsed         *rdi)
{
  ...
  for (U64 i = 0; i < count; ++i) {
    ...
    StringView flags_str = str8_zero();
    {
      U64 f = flags;
...
#define ExceptionHandlerDataFlag_GS         (1 << 3u)
      ...
      if (handler_data_flags & ExceptionHandlerDataFlag_FuncInfo4) {
        ...
        StringView header_str = str8_zero();
        {
...
pe_print(Arena *arena, String8List *out, StringView indent, StringView raw_data, RD_Option opts, RDI_Parsed *rdi)
{
  ...
  StringView raw_string_table = str8_substr(raw_data, rng_1u64(string_table_off, raw_data.size));
...
#if 0
internal void
elf_print_dwarf_expressions(Arena *arena, String8List *out, StringView indent, StringView raw_data)
{
  Temp scratch = scratch_begin(&arena, 1);
      ...
      for (U64 info_off = cu.first_tag_info_off, tag_size = 0; info_off < cu.info_range.max; info_off += tag_size) {
        ...
        if (tag.kind == DW_TagKind_Null) {
          ...
          if (tag.kind == DW_TagKind_SubProgram) {
            StringView expr = dw_exprloc_from_tag_attrib_kind(&dwarf_input, &cu, tag, DW_AttribKind_FrameBase);
            if (expr.size > 0) {
              StringView expr_str = dw_single_line_string_from_expression(comp_temp.arena, expr, cu_base, cu.address_size, arch, cu.version, cu.ext, cu.format);
            }
          }
        } else if (tag.kind == DW_Tag_VariaKindble || tag.kind == DW_Tag_FormalParameter) {
#if 0
          local_persist B32 is_global_var = 0;
          StringView         name            = dw_string_from_tag_attrib_kind(&dwarf_input, &cu, tag, DW_AttribKind_Name);
          ...
          if (value_class != DW_AttribClass_Null) {
            ...
            if (value_class == DW_AttribClass_LocListPtr || value_class == DW_AttribClass_LocList) {
              ...
              for (DW_LocNode *loc_n = location.first; loc_n != 0; loc_n = loc_n.next) {
                StringView expr_str = dw_single_line_string_from_expression(comp_temp.arena, loc_n.v.expr, cu_base, cu.address_size, arch, cu.version, cu.ext, cu.format);
                rd_printf("[%llx-%llx] %S", loc_n.v.range.min, loc_n.v.range.max, expr_str);
              }
            } else if (value_class == DW_AttribClass_ExprLoc) {
              StringView expr = dw_exprloc_from_tag_attrib_kind(&dwarf_input, &cu, location_attrib);
              StringView expr_str = dw_single_line_string_from_expression(comp_temp.arena, expr, cu_base, cu.address_size, arch, cu.version, cu.ext, cu.format);
