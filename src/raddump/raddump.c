...
rd_is_rdi(StringView raw_data)
{
  B32 is_rdi = 0;
  if (raw_data.size > 8) {
    uint64* magic = (uint64 *)raw_data.str;
                   ...
                   StringView      raw_data)
{
  ...
  for (; to_format.size > 0; ) {
    ...
    // offset
    uint64 offset = (uint64)(raw_row.str-raw_data.str);
    ...
    for (uint64 info_off = comp_root.tags_info_range.min; info_off < comp_root.tags_info_range.max; /* empty */) {
      ...
      for (DW_AttribNode* attrib_node = tag.attribs.first; attrib_node != 0; attrib_node = attrib_node.next) {
        ...
        case DW_AttribClass_String: {
          StringView string = dw_string_from_attrib_value(sections, attrib_value);
...
dw_print_debug_line(Arena* arena, String8List* out, StringView indent, DW_SectionArray* sections, B32 relaxed)
{
  ...
  for (Rng1U64Node* comp_unit_range_n = comp_unit_range_list.first; comp_unit_range_n != 0; comp_unit_range_n = comp_unit_range_n.next) {
    ...
    for (; cursor < vm_header.unit_opl; ) {
      ...
      StringView string = str8_list_join(opcode_temp.arena, &list, &(StringJoin){.sep=(" ")});
  ...
  switch (type) {
    ...
    case CV_SymKind_RETURN: {
      ...
      if (sym.style == CV_GenericStyle_REG) {
        ...
        StringView data = rd_format_hex_array(scratch.arena, raw_symbol.str, raw_symbol.size);
...
cv_print_debug_t(Arena* arena, String8List* out, StringView indent, CV_DebugT debug_t)
{
  ...
  for (uint64 lf_idx = 0; lf_idx < debug_t.count; ++lf_idx) {
    ...
    uint64          offset = (uint64)(lf.data.str-debug_t.v[0]);
...
coff_print_archive(Arena* arena, String8List* out, StringView indent, StringView raw_archive, RD_Option opts)
{
  ...
  if (archive_parse.has_long_names && opts & RD_Option_LongNames) {
    ...
    for (String8Node* name_n = long_names.first; name_n != 0; name_n = name_n.next, ++name_idx) {
      uint64 offset = (uint64)(name_n.string.str - archive_parse.long_names.str);
