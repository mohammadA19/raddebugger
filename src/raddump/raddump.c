...
rd_section_markers_from_rdi(Arena *arena, RDI_Parsed *rdi)
{
  ...
  Rng1U64 *sect_vranges = new Rng1U64[sect_count];
  ...
  RD_MarkerList *markers = new RD_MarkerList[sect_count];
  for (U64 i = 0; i < proc_count; ++i) {
    ...
    if (sect_range_idx < sect_count) {
      ...
      RD_MarkerNode *n = new RD_MarkerNode[1];
  ...
  // lists -> arrays
  RD_MarkerArray *result = new RD_MarkerArray[sect_count];
  for (U64 i = 0; i < sect_count; ++i) {
    result[i].count = 0;
    result[i].v     = new RD_Marker[markers[i].count];
...
rd_section_markers_from_coff_symbol_table(Arena *arena, String8 string_table, U64 section_count, COFF_Symbol32Array symbols)
{
  ...
  RD_MarkerList *markers = new RD_MarkerList[section_count+1];
  ...
  // lists -> arrays
  RD_MarkerArray *result = new RD_MarkerArray[section_count];
                ...
                String8           raw_code)
{
  ...
  char *bytes_buffer     = new char[bytes_buffer_max];
...
cv_print_symbol(Arena *arena, String8List *out, String8 indent, CV_Arch arch, CV_TypeIndex min_itype, CV_SymKind type, String8 raw_symbol)
{
  ...
  switch (type) {
    ...
    case CV_SymKind_CALLEES: {
      ...
      cursor += str8_deserial_read_struct(raw_symbol, cursor, &sym);
      CV_TypeIndex *funcs = new CV_TypeIndex[sym.count];
      ...
      U32 *invocations      = new U32[invocation_count];
...
pe_print(Arena *arena, String8List *out, String8 indent, String8 raw_data, RD_Option opts, RDI_Parsed *rdi)
{
  ...
  U8 *raw_opt_header = new U8[file_header->optional_header_size];
  ...
  Rng1U64 *dirs_file_ranges = new Rng1U64[dir_count];
  Rng1U64 *dirs_virt_ranges = new Rng1U64[dir_count];
