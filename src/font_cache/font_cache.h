...
static U128 fnt_hash_from_string(String8 string);
static U64 fnt_little_hash_from_string(U64 seed, String8 string);
static Vec2S32 fnt_vertex_from_corner(Corner corner);

////////////////////////////////
//~ rjf: Font Tags

static FNT_Tag fnt_tag_zero(void);
static B32 fnt_tag_match(FNT_Tag a, FNT_Tag b);
static FP_Handle fnt_handle_from_tag(FNT_Tag tag);
static FP_Metrics fnt_fp_metrics_from_tag(FNT_Tag tag);
static FNT_Tag fnt_tag_from_path(String8 path);
static FNT_Tag fnt_tag_from_static_data_string(String8 *data_ptr);
static String8 fnt_path_from_tag(FNT_Tag tag);

////////////////////////////////
//~ rjf: Atlas

static Rng2S16 fnt_atlas_region_alloc(Arena *arena, FNT_Atlas *atlas, Vec2S16 needed_size);
static void fnt_atlas_region_release(FNT_Atlas *atlas, Rng2S16 region);

////////////////////////////////
//~ rjf: Piece Type Functions

static FNT_Piece *fnt_piece_chunk_list_push_new(Arena *arena, FNT_PieceChunkList *list, U64 cap);
static void fnt_piece_chunk_list_push(Arena *arena, FNT_PieceChunkList *list, U64 cap, FNT_Piece *piece);
static FNT_PieceArray fnt_piece_array_from_chunk_list(Arena *arena, FNT_PieceChunkList *list);
static FNT_PieceArray fnt_piece_array_copy(Arena *arena, FNT_PieceArray *src);

////////////////////////////////
//~ rjf: Cache Usage

//- rjf: base cache lookups
static FNT_Hash2StyleRasterCacheNode *fnt_hash2style_from_tag_size_flags(FNT_Tag tag, F32 size, FNT_RasterFlags flags);
static FNT_Run fnt_run_from_string(FNT_Tag tag, F32 size, F32 base_align_px, F32 tab_size_px, FNT_RasterFlags flags, String8 string);

//- rjf: helpers
static String8List fnt_wrapped_string_lines_from_font_size_string_max(Arena *arena, FNT_Tag font, F32 size, F32 base_align_px, F32 tab_size_px, String8 string, F32 max);
static Vec2F32 fnt_dim_from_tag_size_string(FNT_Tag tag, F32 size, F32 base_align_px, F32 tab_size_px, String8 string);
static Vec2F32 fnt_dim_from_tag_size_string_list(FNT_Tag tag, F32 size, F32 base_align_px, F32 tab_size_px, String8List list);
static F32 fnt_column_size_from_tag_size(FNT_Tag tag, F32 size);
static U64 fnt_char_pos_from_tag_size_string_p(FNT_Tag tag, F32 size, F32 base_align_px, F32 tab_size_px, String8 string, F32 p);

////////////////////////////////
//~ rjf: Metrics

static FNT_Metrics fnt_metrics_from_tag_size(FNT_Tag tag, F32 size);
static F32 fnt_line_height_from_metrics(FNT_Metrics *metrics);

////////////////////////////////
//~ rjf: Main Calls

static void fnt_init(void);
static void fnt_reset(void);
static void fnt_frame(void);
