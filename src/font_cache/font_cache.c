...
internal uint64
fnt_little_hash_from_string(uint64 seed, String8 string)
{
...
{
  ProfBeginFunction();
  uint64 slot_idx = tag.u64[1] % fnt_state->font_hash_table_size;
...
fnt_tag_from_path(String8 path)
{
  ...
  FNT_Tag result = {0};
  {
    uint128 hash = fnt_hash_from_string(path);
    MemoryCopy(&result, &hash, sizeof(result));
    result.u64[1] |= bit64;
  }
  
  //- rjf: tag -> slot index
  uint64 slot_idx = result.u64[1] % fnt_state->font_hash_table_size;
...
fnt_tag_from_static_data_string(String8 *data_ptr)
{
  ...
  FNT_Tag result = {0};
  {
    uint128 hash = fnt_hash_from_string(str8((uint8 *)&data_ptr, sizeof(String8 *)));
    MemoryCopy(&result, &hash, sizeof(result));
    result.u64[1] &= ~bit64;
  }
...
internal Rng2uint16
fnt_atlas_region_alloc(Arena *arena, FNT_Atlas *atlas, Vec2uint16 needed_size)
{
  ProfBeginFunction();
  
  //- rjf: find node with best-fit size
  Vec2uint16 region_p0 = {0};
  Vec2uint16 region_sz = {0};
  Corner node_corner = Corner_Invalid;
  FNT_AtlasRegionNode *node = 0;
  {
    Vec2uint16 n_supported_size = atlas->root_dim;
    for(FNT_AtlasRegionNode *n = atlas->root, *next = 0; n != 0; n = next, next = 0)
    {
      ...
      Vec2uint16 child_size = v2s16(n_supported_size.x/2, n_supported_size.y/2);
      ...
      if(child_size.x >= needed_size.x && child_size.y >= needed_size.y)
      {
        for(Corner corner = (Corner)0; corner < Corner_COUNT; corner = (Corner)(corner+1))
        {
             ...
             n->max_free_size[corner].y >= needed_size.y)
          {
            ...
            node_corner = corner;
            Vec2uint32 side_vertex = fnt_vertex_from_corner(corner);
...
internal void
fnt_atlas_region_release(FNT_Atlas *atlas, Rng2uint16 region)
{
  ProfBeginFunction();
  
  //- rjf: extract region size
  Vec2uint16 region_size = v2s16(region.x1 - region.x0, region.y1 - region.y0);
  
  //- rjf: map region to associated node
  Vec2uint16 calc_region_size = {0};
  FNT_AtlasRegionNode *node = 0;
  Corner node_corner = Corner_Invalid;
  {
    Vec2uint16 n_p0 = v2s16(0, 0);
    Vec2uint16 n_sz = atlas->root_dim;
    for(FNT_AtlasRegionNode *n = atlas->root, *next = 0; n != 0; n = next)
    {
         ...
         n_p0.y <= region.p0.y && region.p0.y < n_p0.y+n_sz.y)
      {
        ...
        {
          Vec2uint16 r_midpoint = v2s16(region.p0.x + region_size.x/2,
                                     region.p0.y + region_size.y/2);
          Vec2uint16 n_midpoint = v2s16(n_p0.x + n_sz.x/2,
                                     n_p0.y + n_sz.y/2);
          ...
          n_sz.x /= 2;
          n_sz.y /= 2;
          Vec2uint32 side_vertex = fnt_vertex_from_corner(node_corner);
...
internal FNT_Piece *
fnt_piece_chunk_list_push_new(Arena *arena, FNT_PieceChunkList *list, uint64 cap)
{
...
internal void
fnt_piece_chunk_list_push(Arena *arena, FNT_PieceChunkList *list, uint64 cap, FNT_Piece *piece)
{
...
fnt_hash2style_from_tag_size_flags(FNT_Tag tag, float size, FNT_RasterFlags flags)
{
  ...
  uint64 style_hash = {0};
  {
    ...
    style_hash = fnt_little_hash_from_string(5381, str8((uint8 *)buffer, sizeof(buffer)));
  }
  ...
  FNT_Hash2StyleRasterCacheNode *hash2style_node = 0;
  {
    ...
    uint64 slot_idx = style_hash%fnt_state->hash2style_slots_count;
...
internal FNT_Run
fnt_run_from_string(FNT_Tag tag, float size, float base_align_px, float tab_size_px, FNT_RasterFlags flags, String8 string)
{
  ProfBeginFunction();
  ...
  uint64 run_hash = fnt_little_hash_from_string(5381, string);
  uint64 run_slot_idx = run_hash%hash2style_node->run_slots_count;
    ...
    for(uint64 idx = 0; idx <= string.size;)
    {
      ...
      uint64 piece_hash = 0;
      {
        ...
        if(piece_substring.size > 1)
        {
          ...
          uint64 slot_idx = piece_hash%hash2style_node->hash2info_slots_count;
      ...
      if(info == 0)
      {
        ...
        if(font_handle_mapped_on_miss == 0)
        {
          ...
          uint64 font_slot_idx = tag.u64[1] % fnt_state->font_hash_table_size;
        ...
        uint16 chosen_atlas_num = 0;
        FNT_Atlas *chosen_atlas = 0;
        Rng2uint16 chosen_atlas_region = {0};
        if(raster.atlas_dim.x != 0 && raster.atlas_dim.y != 0)
        {
          uint64 num_atlases = 0;
          for(FNT_Atlas *atlas = fnt_state->first_atlas;; atlas = atlas->next, num_atlases += 1)
          {
            ...
            if(atlas == 0 && num_atlases < 64)
            {
              ...
              atlas->texture = r_tex2d_alloc(R_ResourceKind_Dynamic, v2s32((uint32)atlas->root_dim.x, (uint32)atlas->root_dim.y), R_Tex2DFormat_RGBA8, 0);
            }
            
            // rjf: allocate from atlas
            if(atlas != 0)
            {
              Vec2uint16 needed_dimensions = v2s16(raster.atlas_dim.x + 2, raster.atlas_dim.y + 2);
              ...
              {
                chosen_atlas = atlas;
                chosen_atlas_num = (uint32)num_atlases;
                break;
              }
            }
            else
...
fnt_dim_from_tag_size_string_list(FNT_Tag tag, float size, float base_align_px, float tab_size_px, String8List list)
{
  ...
  {
    Vec2float str_dim = fnt_dim_from_tag_size_string(tag, size, base_align_px, tab_size_px, n->string);
    sum.x += str_dim.x;
