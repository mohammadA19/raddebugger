...
internal void
dr_fstrs_push_new_(Arena *arena, DR_FStrList *list, DR_FStrParams *params, DR_FStrParams *overrides, StringView string)
{
...
dr_fuzzy_match_find_from_fstrs(Arena *arena, DR_FStrList *fstrs, StringView needle)
{
    Temp scratch = scratch_begin(&arena, 1);
    StringView fstrs_string = {0};
...
internal void
dr_text(FNT_Tag font, F32 size, F32 base_align_px, F32 tab_size_px, FNT_RasterFlags flags, Vec2F32 p, Vec4F32 color, StringView string)
{
    Temp scratch = scratch_begin(0, 0);
