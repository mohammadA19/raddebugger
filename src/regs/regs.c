...
regs_rip_from_arch_block(Arch arch, void* block)
{
  ...
  if(block != 0) switch(arch)
  {
    ...
    case Arch_x86:{result = (uint64)((REGS_RegBlockX86 *)block).eip.u32;}break;
  }
  return result;
}
...
regs_rsp_from_arch_block(Arch arch, void* block)
{
  ...
  if(block != 0) switch(arch)
  {
    ...
    case Arch_x86:{result = (uint64)((REGS_RegBlockX86 *)block).esp.u32;}break;
  }
  return result;
}

void
regs_arch_block_write_rip(Arch arch, void* block, uint64 rip)
{
  if(block != 0) switch(arch)
  {
    default:{}break;
    case Arch_x64:{((REGS_RegBlockX64 *)block).rip.u64 = rip;}break;
    case Arch_x86:{((REGS_RegBlockX86 *)block).eip.u32 = (uint32)rip;}break;
  }
}

void
regs_arch_block_write_rsp(Arch arch, void* block, uint64 rsp)
{
  if(block != 0) switch(arch)
  {
    default:{}break;
    case Arch_x64:{((REGS_RegBlockX64 *)block).rsp.u64 = rsp;}break;
    case Arch_x86:{((REGS_RegBlockX86 *)block).esp.u32 = (uint32)rsp;}break;
  }
}
