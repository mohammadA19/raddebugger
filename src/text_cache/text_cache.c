...
txt_pt_from_info_off__linear_scan(TXT_TextInfo *info, u64 off)
{
  TxtPt pt = {0};
  {
    for(u64 line_idx = 0; line_idx < info->lines_count; line_idx += 1)
    {
      ...
      {
        pt.line = (i64)line_idx + 1;
        pt.column = (i64)(off - info->lines_ranges[line_idx].min) + 1;
      }
    }
  }
  return pt;
...
internal TXT_TokenArray
txt_token_array_from_info_line_num__linear_scan(TXT_TextInfo *info, i64 line_num)
{
...
internal String8
txt_string_from_info_data_line_num(TXT_TextInfo *info, String8 data, i64 line_num)
{
  String8 result = {0};
  if(1 <= line_num && line_num <= info->lines_count)
  {
...
txt_line_tokens_slice_from_info_data_line_range(Arena *arena, TXT_TextInfo *info, String8 data, Rng1S64 line_range)
{
  ...
  {
    Rng1S64 line_range_clamped = r1s64(clamp(1, line_range.min, (i64)info->lines_count), clamp(1, line_range.max, (i64)info->lines_count));
