...
void
txt_token_chunk_list_push(Arena *arena, TXT_TokenChunkList *list, ulong cap, TXT_Token *token)
{
...
TXT_TokenArray
txt_token_array_from_string__c_cpp(Arena *arena, ulong *bytes_processed_counter, String8 string)
{
  ProfBeginFunction();
  Temp scratch = scratch_begin(&arena, 1);
  ulong chunk_size = Clamp(8, string.size/8, 4096);
  ...
  TXT_TokenChunkList tokens = {0};
  {
    ...
    B32 escaped = 0;
    B32 next_escaped = 0;
    ulong byte_process_start_idx = 0;
    for(ulong idx = 0; idx <= string.size;)
    {
      ...
      if(ender_found)
      {
        ...
        {
          read_only static String8 cpp_keywords[] =
          {
          ...
          for(ulong keyword_idx = 0; keyword_idx < ArrayCount(cpp_keywords); keyword_idx += 1)
          {
        ...
        {
          read_only static String8 c_cpp_multichar_symbol_strings[] =
          {
            str8_lit_comp("<<"),
            str8_lit_comp(">>"),
            str8_lit_comp("<="),
          ...
          String8 token_string = str8_substr(string, r1u64(active_token_start_idx, idx+ender_pad));
          for(ulong off = 0, next_off = token_string.size; off < token_string.size; off = next_off)
          {
            B32 found = 0;
            for(ulong idx = 0; idx < ArrayCount(c_cpp_multichar_symbol_strings); idx += 1)
            {
...
TXT_TokenArray
txt_token_array_from_string__odin(Arena *arena, ulong *bytes_processed_counter, String8 string)
{
  Temp scratch = scratch_begin(&arena, 1);
  ...
  TXT_TokenChunkList tokens = {0};
  {
    ...
    for(ulong idx = 0; idx <= string.size;)
    {
      ...
      if(ender_found)
      {
        ...
        if(token.kind == TXT_TokenKind_Identifier)
        {
          read_only static String8 odin_keywords[] =
          {
          ...
          String8 token_string = str8_substr(string, r1u64(active_token_start_idx, idx+ender_pad));
          for(ulong keyword_idx = 0; keyword_idx < ArrayCount(odin_keywords); keyword_idx += 1)
          {
        ...
        {
          read_only static String8 odin_multichar_symbol_strings[] =
          {
            str8_lit_comp("<<"),
            str8_lit_comp(">>"),
            str8_lit_comp("<="),
          ...
          {
            B32 found = 0;
            for(ulong idx = 0; idx < ArrayCount(odin_multichar_symbol_strings); idx += 1)
            {
...
TXT_TokenArray
txt_token_array_from_string__jai(Arena *arena, ulong *bytes_processed_counter, String8 string)
{
  Temp scratch = scratch_begin(&arena, 1);
  ...
  TXT_TokenChunkList tokens = {0};
  {
    ...
    for(ulong idx = 0; idx <= string.size;)
    {
      ...
      if(ender_found)
      {
        ...
        if(token.kind == TXT_TokenKind_Identifier)
        {
          read_only static String8 jai_keywords[] =
          {
            str8_lit_comp("bool"),
            str8_lit_comp("true"),
            str8_lit_comp("false"),
          ...
          String8 token_string = str8_substr(string, r1u64(active_token_start_idx, idx+ender_pad));
          for(ulong keyword_idx = 0; keyword_idx < ArrayCount(jai_keywords); keyword_idx += 1)
          {
        ...
        {
          read_only static String8 jai_multichar_symbol_strings[] =
          {
            str8_lit_comp("<<"),
            str8_lit_comp(">>"),
            str8_lit_comp("<="),
          ...
          {
            B32 found = 0;
            for(ulong idx = 0; idx < ArrayCount(jai_multichar_symbol_strings); idx += 1)
            {
...
TXT_TokenArray
txt_token_array_from_string__zig(Arena *arena, ulong *bytes_processed_counter, String8 string)
{
  Temp scratch = scratch_begin(&arena, 1);
  ...
  TXT_TokenChunkList tokens = {0};
  {
    ...
    for(ulong idx = 0; idx <= string.size;)
    {
      ...
      if(ender_found)
      {
        ...
        if(token.kind == TXT_TokenKind_Identifier)
        {
          read_only static String8 zig_keywords[] =
          {
          ...
          String8 token_string = str8_substr(string, r1u64(active_token_start_idx, idx+ender_pad));
          for(ulong keyword_idx = 0; keyword_idx < ArrayCount(zig_keywords); keyword_idx += 1)
          {
        ...
        {
          read_only static String8 zig_multichar_symbol_strings[] =
          {
            str8_lit_comp("<<"),
            str8_lit_comp(">>"),
            str8_lit_comp("<="),
          ...
          {
            B32 found = 0;
            for(ulong idx = 0; idx < ArrayCount(zig_multichar_symbol_strings); idx += 1)
            {
...
TXT_TokenArray
txt_token_array_from_string__disasm_x64_intel(Arena *arena, ulong *bytes_processed_counter, String8 string)
{
  Temp scratch = scratch_begin(&arena, 1);
  ...
  {
    TXT_TokenKind active_token_kind = TXT_TokenKind_Null;
    ulong active_token_start_off = 0;
    ulong off = 0;
    B32 escaped = 0;
    B32 string_is_char = 0;
    int brace_nest = 0;
    ...
    for(ulong advance = 0; off <= string.size; off += advance)
    {
...
txt_init()
{
  ...
  for(ulong idx = 0; idx < txt_shared->stripes_count; idx += 1)
  {
...
txt_scope_close(TXT_Scope *scope)
{
  for(TXT_Touch *touch = scope->top_touch, *next = 0; touch != 0; touch = next)
  {
    ...
    next = touch->next;
    ulong slot_idx = hash.u64[1]%txt_shared->slots_count;
    ulong stripe_idx = slot_idx%txt_shared->stripes_count;
...
TxtPt
txt_pt_from_info_off__linear_scan(TXT_TextInfo *info, ulong off)
{
  TxtPt pt = {0};
  {
    for(ulong line_idx = 0; line_idx < info->lines_count; line_idx += 1)
    {
...
txt_token_array_from_info_line_num__linear_scan(TXT_TextInfo *info, long line_num)
{
  ...
  if(1 <= line_num && line_num <= info->lines_count)
  {
    ...
    for(ulong token_idx = 0; token_idx < info->tokens.count; token_idx += 1)
    {
...
Rng1U64
txt_expr_off_range_from_line_off_range_string_tokens(ulong off, Rng1U64 line_range, String8 line_text, TXT_TokenArray *line_tokens)
{
  Rng1U64 result = {0};
...
txt_expr_off_range_from_info_data_pt(TXT_TextInfo *info, String8 data, TxtPt pt)
{
  ...
  if(1 <= pt.line && pt.line <= info->lines_count)
  {
    ...
    TXT_Token *line_tokens_first = line_tokens.v;
    TXT_Token *line_tokens_opl = line_tokens.v+line_tokens.count;
    ulong pt_off = line_range.min + (pt.column-1);
...
txt_line_tokens_slice_from_info_data_line_range(Arena *arena, TXT_TextInfo *info, String8 data, Rng1S64 line_range)
{
  ...
  if(info->lines_count != 0)
  {
    ...
    ulong line_count = (ulong)dim_1s64(line_range_clamped)+1;
    ...
    ProfScope("binary search to find first token")
    {
      ...
      ulong min_idx = 0;
      ulong opl_idx = info->tokens.count;
      for(;;)
      {
        ulong mid_idx = (opl_idx+min_idx)/2;
        if(mid_idx >= opl_idx)
        {
          break;
        }
    ...
    for(ulong line_slice_idx = 0; line_slice_idx < line_count; line_slice_idx += 1)
    {
...
B32
txt_u2p_enqueue_req(U128 hash, TXT_LangKind lang, ulong endt_us)
{
  B32 good = 0;
  OS_MutexScope(txt_shared->u2p_ring_mutex) for(;;)
  {
    ulong unconsumed_size = txt_shared->u2p_ring_write_pos - txt_shared->u2p_ring_read_pos;
    ulong available_size = txt_shared->u2p_ring_size - unconsumed_size;
...
ASYNC_WORK_DEF(txt_parse_work)
{
  ...
  if(got_task && !u128_match(hash, u128_zero()))
  {
    ...
    ulong *bytes_processed_ptr = 0;
    ulong *bytes_to_process_ptr = 0;
    OS_MutexScopeR(stripe->rw_mutex)
    {
      for(TXT_Node *n = slot->first; n != 0; n = n->next)
      {
    ...
    TXT_LineEndKind line_end_kind = TXT_LineEndKind_Null;
    {
      ulong lf_count = 0;
      ulong cr_count = 0;
      for(ulong idx = 0; idx < data.size && idx < 1024; idx += 1)
      {
        if(data.str[idx] == '\r')
        {
          cr_count += 1;
    ...
    ulong line_count = 1;
    ulong byte_process_start_idx = 0;
    for(ulong idx = 0; idx < data.size; idx += 1)
    {
      if(data.str[idx] == '\n' || data.str[idx] == '\r')
      {
        line_count += 1;
...
txt_evictor_thread__entry_point(void *p)
{
  ...
  for(;;)
  {
    ...
    for(ulong slot_idx = 0; slot_idx < txt_shared->slots_count; slot_idx += 1)
    {
      ulong stripe_idx = slot_idx%txt_shared->stripes_count;
      TXT_Slot *slot = &txt_shared->slots[slot_idx];
      TXT_Stripe *stripe = &txt_shared->stripes[stripe_idx];
