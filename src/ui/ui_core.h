...
enum UI_AnimationInfoFlags : uint
{
...
long ui_scanned_column_from_column(String8 string, long start_column, Side side);
...
UI_Size ui_size(UI_SizeKind kind, float value, float strictness);
...
UI_ScrollPt ui_scroll_pt(long idx, float off);
void ui_scroll_pt_target_idx(UI_ScrollPt *v, long idx);
...
UI_BoxRec ui_box_rec_df(UI_Box *box, UI_Box *root, ulong sib_member_off, ulong child_member_off);
...
String8           ui_get_drag_data(ulong min_required_size);
...
void ui_begin_build(OS_Handle window, UI_EventList *events, UI_IconInfo *icon_info, UI_WidgetPaletteInfo *widget_palette_info, UI_AnimationInfo *animation_info, float real_dt, float animation_dt);
void ui_end_build();
...
B32               ui_any_ctx_menu_is_open();

//- rjf: focus tree coloring
B32               ui_is_focus_hot();
B32               ui_is_focus_active();
...
ulong               ui_box_char_pos_from_xy(UI_Box *box, Vec2F32 xy);
...
void     ui_push_corner_radius(float v);
