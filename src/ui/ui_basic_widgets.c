...
UI_Signal
ui_label(StringView str)
{
  UI_Box* box = ui_build_box_from_string(UI_BoxFlag_DrawText, StringView());
...
ui_labelf(char* fmt, ...)
{
  ...
  va_end(args);
  UI_Signal result = ui_label(str);
  scratch_end(scratch);
  return result;
}

void
ui_label_multiline(float max, StringView str)
{
  Temp scratch = scratch_begin(0, 0);
  ui_set_next_child_layout_axis(Axis2_Y);
  ui_set_next_pref_height(ui_children_sum(1));
  UI_Box* box = ui_build_box_from_key(0, ui_key_zero());
  String8List lines = fnt_wrapped_string_lines_from_font_size_string_max(scratch.arena, ui_top_font(), ui_top_font_size(), 0, ui_top_tab_size(), str, max);
...
ui_label_multilinef(float max, char* fmt, ...)
{
  ...
  StringView str = push_str8fv(scratch.arena, fmt, args);
  va_end(args);
  ui_label_multiline(max, str);
  scratch_end(scratch);
}

UI_Signal
ui_button(StringView str)
{
...
ui_buttonf(char* fmt, ...)
{
  ...
  StringView str = push_str8fv(scratch.arena, fmt, args);
  va_end(args);
  UI_Signal result = ui_button(str);
  scratch_end(scratch);
  return result;
}

UI_Signal
ui_hover_label(StringView str)
{
  UI_Box* box = ui_build_box_from_string(UI_BoxFlag_Clickable|UI_BoxFlag_DrawText, str);
  UI_Signal interact = ui_signal_from_box(box);
...
ui_hover_labelf(char* fmt, ...)
{
  ...
  StringView str = push_str8fv(scratch.arena, fmt, args);
  va_end(args);
  UI_Signal sig = ui_hover_label(str);
  scratch_end(scratch);
  return sig;
}
...
UI_BOX_CUSTOM_DRAW(ui_line_edit_draw)
{
  ...
  float cursor_thickness = ClampBot(4.f, font_size/6.f);
  Rng2F32 cursor_rect =
  {
...
UI_Signal
ui_line_edit(TxtPt* cursor, TxtPt* mark, uint8* edit_buffer, uint64 edit_buffer_size, uint64* edit_string_size_out, StringView pre_edit_value, StringView str)
{
  //- rjf: make key
...
UI_Signal
ui_line_editf(TxtPt* cursor, TxtPt* mark, uint8* edit_buffer, uint64 edit_buffer_size, uint64* edit_string_size_out, StringView pre_edit_value, char* fmt, ...)
{
  Temp scratch = scratch_begin(0, 0);
  va_list args;
  va_start(args, fmt);
  StringView str = push_str8fv(scratch.arena, fmt, args);
  va_end(args);
  UI_Signal result = ui_line_edit(cursor, mark, edit_buffer, edit_buffer_size, edit_string_size_out, pre_edit_value, str);
  scratch_end(scratch);
  return result;
}
...
UI_Signal
ui_image(R_Handle texture, R_Tex2DSampleKind sample_kind, Rng2F32 region, Vec4F32 tint, float blur, StringView str)
{
  UI_Box* box = ui_build_box_from_string(0, str);
...
UI_Signal
ui_imagef(R_Handle texture, R_Tex2DSampleKind sample_kind, Rng2F32 region, Vec4F32 tint, float blur, char* fmt, ...)
{
  Temp scratch = scratch_begin(0, 0);
  va_list args;
  va_start(args, fmt);
  StringView str = push_str8fv(scratch.arena, fmt, args);
  va_end(args);
  UI_Signal result = ui_image(texture, sample_kind, region, tint, blur, str);
  scratch_end(scratch);
  return result;
}
...
UI_Signal
ui_expander(B32 is_expanded, StringView str)
{
  ui_set_next_hover_cursor(OS_Cursor_HandPoint);
...
ui_expanderf(B32 is_expanded, char* fmt, ...)
{
  ...
  StringView str = push_str8fv(scratch.arena, fmt, args);
  va_end(args);
  UI_Signal sig = ui_expander(is_expanded, str);
  scratch_end(scratch);
  return sig;
}

UI_Signal
ui_sort_header(B32 sorting, B32 ascending, StringView str)
{
  ui_set_next_child_layout_axis(Axis2_X);
  ui_set_next_hover_cursor(OS_Cursor_HandPoint);
  UI_Box* box = ui_build_box_from_string(UI_BoxFlag_Clickable|UI_BoxFlag_DrawBackground|UI_BoxFlag_DrawActiveEffects, str);
  ui_push_parent(box);
  ...
  if(sorting)
  {
    ...
    ui_set_next_text_alignment(UI_TextAlign_Center);
    ui_set_next_font(ui_icon_font());
    UI_Box* icon = ui_build_box_from_string(UI_BoxFlag_DrawText, (""));
...
ui_sort_headerf(B32 sorting, B32 ascending, char* fmt, ...)
{
  ...
  StringView str = push_str8fv(scratch.arena, fmt, args);
  va_end(args);
  UI_Signal sig = ui_sort_header(sorting, ascending, str);
  scratch_end(scratch);
  return sig;
}
...
UI_Signal
ui_sat_val_picker(float hue, float* out_sat, float* out_val, StringView str)
{
  // rjf: build & interact
  ui_set_next_hover_cursor(OS_Cursor_HandPoint);
  UI_Box* box = ui_build_box_from_string(UI_BoxFlag_Clickable, str);
...
ui_sat_val_pickerf(float hue, float* out_sat, float* out_val, char* fmt, ...)
{
  ...
  StringView str = push_str8fv(scratch.arena, fmt, args);
  va_end(args);
  UI_Signal sig = ui_sat_val_picker(hue, out_sat, out_val, str);
  scratch_end(scratch);
  return sig;
}
...
UI_Signal
ui_hue_picker(float* out_hue, float sat, float val, StringView str)
{
  // rjf: build & interact
  ui_set_next_hover_cursor(OS_Cursor_HandPoint);
  UI_Box* box = ui_build_box_from_string(UI_BoxFlag_Clickable, str);
...
ui_hue_pickerf(float* out_hue, float sat, float val, char* fmt, ...)
{
  ...
  StringView str = push_str8fv(scratch.arena, fmt, args);
  va_end(args);
  UI_Signal sig = ui_hue_picker(out_hue, sat, val, str);
  scratch_end(scratch);
  return sig;
}
...
UI_Signal
ui_alpha_picker(float* out_alpha, StringView str)
{
  // rjf: build & interact
  ui_set_next_hover_cursor(OS_Cursor_HandPoint);
  UI_Box* box = ui_build_box_from_string(UI_BoxFlag_Clickable, str);
...
ui_alpha_pickerf(float* out_alpha, char* fmt, ...)
{
  ...
  StringView str = push_str8fv(scratch.arena, fmt, args);
  va_end(args);
  UI_Signal sig = ui_alpha_picker(out_alpha, str);
  scratch_end(scratch);
  return sig;
}
...
UI_Box* ui_row_begin()    { return ui_named_row_begin(("")); }
UI_Signal ui_row_end()    { return ui_named_row_end(); }
UI_Box* ui_column_begin() { return ui_named_column_begin(("")); }
UI_Signal ui_column_end() { return ui_named_column_end(); }

UI_Box *
ui_named_row_begin(StringView str)
{
  ui_set_next_child_layout_axis(Axis2_X);
  UI_Box* box = ui_build_box_from_string(0, str);
  ui_push_parent(box);
...
UI_Box *
ui_named_column_begin(StringView str)
{
  ui_set_next_child_layout_axis(Axis2_Y);
  UI_Box* box = ui_build_box_from_string(0, str);
  ui_push_parent(box);
...
UI_Box *
ui_pane_begin(Rng2F32 rect, StringView str)
{
  ui_push_rect(rect);
  ui_set_next_child_layout_axis(Axis2_Y);
  UI_Box* box = ui_build_box_from_string(UI_BoxFlag_Clickable|UI_BoxFlag_Clip|UI_BoxFlag_DrawBorder|UI_BoxFlag_DrawBackground, str);
  ui_pop_rect();
  ui_push_parent(box);
...
ui_pane_beginf(Rng2F32 rect, char* fmt, ...)
{
  ...
  StringView str = push_str8fv(scratch.arena, fmt, args);
  va_end(args);
  UI_Box* box = ui_pane_begin(rect, str);
  scratch_end(scratch);
  return box;
}
...
void
ui_table_begin(uint64 column_pct_count, float** column_pcts, StringView str)
{
  ...
  ui_set_next_pref_height(ui_children_sum(1));
  ui_set_next_child_layout_axis(Axis2_Y);
  UI_Box* table = ui_build_box_from_string(0, str);
  ...
  for(uint64 column_idx = 1; column_idx < ui_ts_col_pct_count; column_idx += 1)
  {
    ...
    UI_Rect(rect)
    {
      ...
      UI_Box* box = ui_build_box_from_stringf(UI_BoxFlag_Clickable, "###%S_boundary_%I64u", table.str, column_idx);
      ...
      if(ui_dragging(interact))
      {
        ...
        // rjf: calculate diff
        float min_size = 30.f;
        float left_pct__before     = ui_get_drag_struct(Vec2F32).x;
        ...
        float right_pct__before    = ui_get_drag_struct(Vec2F32).y;
...
ui_table_beginf(uint64 column_pct_count, float** column_pcts, char* fmt, ...)
{
  ...
  StringView str = push_str8fv(scratch.arena, fmt, args);
  va_end(args);
  ui_table_begin(column_pct_count, column_pcts, str);
  scratch_end(scratch);
}
...
UI_Box *
ui_named_table_vector_begin(StringView str)
{
  ui_set_next_pref_width(ui_pct(1, 0));
  ui_set_next_child_layout_axis(Axis2_X);
  UI_Box* vector = ui_build_box_from_string(UI_BoxFlag_DrawSideBottom, str);
  ui_ts_vector_idx += 1;
  ui_ts_cell_idx = 0;
...
ui_named_table_vector_beginf(char* fmt, ...)
{
  ...
  StringView str = push_str8fv(scratch.arena, fmt, args);
  va_end(args);
  UI_Box* vector = ui_named_table_vector_begin(str);
  scratch_end(scratch);
  return vector;
}
...
UI_ScrollPt
ui_scroll_bar(Axis2 axis, UI_Size off_axis_size, UI_ScrollPt pt, Rng1S64 idx_range, int64 view_num_indices)
{
  ui_push_palette(ui_state.widget_palette_info.scrollbar_palette);
  
  //- rjf: unpack
  int64 idx_range_dim = Max(dim_1s64(idx_range), 1);
  ...
  UI_Parent(container_box)
  {
    ...
    UI_Parent(scroll_area_box)
    {
      ...
      // rjf: scroller
      UI_Flags(disabled_flags) UI_PrefSize(axis, ui_pct(Clamp(0.01f, (float)((double)Max(view_num_indices, 1)/(double)idx_range_dim), 1.f), 0.f))
      {
  ...
  UI_ScrollPt new_pt = pt;
  {
    ...
    if(ui_dragging(scroller_sig))
    {
      ...
      int64 new_idx = original_pt.idx + drag_pct*idx_range_dim;
...
ui_scroll_list_begin(UI_ScrollListParams* params, UI_ScrollPt* scroll_pt, Vec2S64* cursor_out, Vec2S64* mark_out, Rng1S64* visible_row_range_out, UI_ScrollListSignal* signal_out)
{
  ...
  int64 num_possible_visible_rows = (int64)(params.dim_px.y/params.row_height_px);
  ...
  {
    int64 cursor_item_idx = cursor_out.y-1;
    if(params.item_range.min <= cursor_item_idx && cursor_item_idx <= params.item_range.max)
    {
      ...
      {
        cursor_visibility_row_range.min = (int64)ui_scroll_list_row_from_item(&params.row_blocks, (uint64)cursor_item_idx);
  ...
  Rng1S64 visible_row_range = r1s64(scroll_pt.idx + (int64)(scroll_pt.off) + 0 - !!(scroll_pt.off < 0),
                                    scroll_pt.idx + (int64)(scroll_pt.off) + 0 + num_possible_visible_rows + 1);
...
ui_scroll_list_end()
{
  ...
  //- rjf: scroll
  {
    ...
    if(sig.scroll.y != 0)
    {
      int64 new_idx = ui_scroll_list_scroll_pt_ptr.idx + sig.scroll.y;
