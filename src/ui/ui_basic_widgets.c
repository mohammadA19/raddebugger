...
internal void
ui_label_multiline(F32 max, string string)
{
  Temp scratch = scratch_begin(0, 0);
...
internal UI_BOX_CUSTOM_DRAW(ui_line_edit_draw)
{
  ...
  Vec4F32 cursor_color = ui_color_from_tags_key_name(box->tags_key, ("cursor"));
  ...
  string edited_string = draw_data->edited_string;
...
ui_line_edit(TxtPt *cursor, TxtPt *mark, U8 *edit_buffer, U64 edit_buffer_size, U64 *edit_string_size_out, string pre_edit_value, string string)
{
  ...
  if(is_focus_active)
  {
    ...
    {
      string edit_string = str8(edit_buffer, edit_string_size_out[0]);
  ...
  {
    string edit_string = pre_edit_value;
...
internal UI_Signal
ui_image(R_Handle texture, R_Tex2DSampleKind sample_kind, Rng2F32 region, Vec4F32 tint, F32 blur, string string)
{
...
internal UI_Signal
ui_expander(B32 is_expanded, string string)
{
...
internal UI_Signal
ui_sort_header(B32 sorting, B32 ascending, string string)
{
...
internal UI_Box *
ui_pane_begin(Rng2F32 rect, string string)
{
  ui_push_rect(rect);
  ui_set_next_child_layout_axis(Axis2_Y);
...
internal UI_Box *
ui_named_table_vector_begin(string string)
{
...
ui_scroll_bar(Axis2 axis, UI_Size off_axis_size, UI_ScrollPt pt, Rng1S64 idx_range, S64 view_num_indices)
{
  ...
  {
    string arrow_string = ui_icon_string_from_kind(axis == Axis2_X ? UI_IconKind_LeftArrow : UI_IconKind_UpArrow);
  ...
  UI_Parent(container_box)
    ...
    UI_Font(ui_icon_font())
  {
    string arrow_string = ui_icon_string_from_kind(axis == Axis2_X ? UI_IconKind_RightArrow : UI_IconKind_DownArrow);
