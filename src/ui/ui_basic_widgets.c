...
fn void ui_divider(UI_Size size)
{
...
fn UI_Signal ui_labelf(char* fmt, ...)
{
...
fn void ui_label_multiline(F32 max, String8 string)
{
  Temp scratch = scratch_begin(0, 0);
...
fn void ui_label_multilinef(F32 max, char* fmt, ...)
{
  Temp scratch = scratch_begin(0, 0);
  va_list args;
  va_start(args, fmt);
...
fn UI_Signal ui_buttonf(char* fmt, ...)
{
  Temp scratch = scratch_begin(0, 0);
  va_list args;
  va_start(args, fmt);
...
fn UI_Signal ui_hover_labelf(char* fmt, ...)
{
  Temp scratch = scratch_begin(0, 0);
  va_list args;
  va_start(args, fmt);
...
fn UI_BOX_CUSTOM_DRAW(ui_line_edit_draw)
{
...
fn UI_Signal ui_line_edit(TxtPt* cursor, TxtPt* mark, U8* edit_buffer, ulong edit_buffer_size, ulong* edit_string_size_out, String8 pre_edit_value, String8 string)
{
  //- rjf: make key
...
fn UI_Signal ui_line_editf(TxtPt* cursor, TxtPt* mark, U8* edit_buffer, ulong edit_buffer_size, ulong* edit_string_size_out, String8 pre_edit_value, char* fmt, ...)
{
  Temp scratch = scratch_begin(0, 0);
  va_list args;
  va_start(args, fmt);
...
fn UI_BOX_CUSTOM_DRAW(ui_image_draw)
{
...
fn UI_Signal ui_image(R_Handle texture, R_Tex2DSampleKind sample_kind, Rng2F32 region, Vec4F32 tint, F32 blur, String8 string)
{
...
fn UI_Signal ui_imagef(R_Handle texture, R_Tex2DSampleKind sample_kind, Rng2F32 region, Vec4F32 tint, F32 blur, char* fmt, ...)
{
  Temp scratch = scratch_begin(0, 0);
  va_list args;
  va_start(args, fmt);
...
fn UI_Signal ui_expander(B32 is_expanded, String8 string)
{
  ui_set_next_hover_cursor(OS_Cursor_HandPoint);
...
fn UI_Signal ui_expanderf(B32 is_expanded, char* fmt, ...)
{
  Temp scratch = scratch_begin(0, 0);
  va_list args;
  va_start(args, fmt);
...
fn UI_Signal ui_sort_header(B32 sorting, B32 ascending, String8 string)
{
...
fn UI_Signal ui_sort_headerf(B32 sorting, B32 ascending, char* fmt, ...)
{
  Temp scratch = scratch_begin(0, 0);
  va_list args;
  va_start(args, fmt);
...
fn void ui_do_color_tooltip_hsv(Vec3F32 hsv)
{
...
fn void ui_do_color_tooltip_hsva(Vec4F32 hsva)
{
...
fn UI_BOX_CUSTOM_DRAW(ui_sat_val_picker_draw)
{
...
fn UI_Signal ui_sat_val_picker(F32 hue, F32* out_sat, F32* out_val, String8 string)
{
  // rjf: build & interact
  ui_set_next_hover_cursor(OS_Cursor_HandPoint);
...
fn UI_Signal ui_sat_val_pickerf(F32 hue, F32* out_sat, F32* out_val, char* fmt, ...)
{
  Temp scratch = scratch_begin(0, 0);
  va_list args;
  va_start(args, fmt);
...
fn UI_BOX_CUSTOM_DRAW(ui_hue_picker_draw)
{
...
fn UI_Signal ui_hue_picker(F32* out_hue, F32 sat, F32 val, String8 string)
{
  // rjf: build & interact
  ui_set_next_hover_cursor(OS_Cursor_HandPoint);
  UI_Box* box = ui_build_box_from_string(UI_BoxFlag_Clickable, string);
...
fn UI_Signal ui_hue_pickerf(F32* out_hue, F32 sat, F32 val, char* fmt, ...)
{
  Temp scratch = scratch_begin(0, 0);
  va_list args;
  va_start(args, fmt);
...
fn UI_BOX_CUSTOM_DRAW(ui_alpha_picker_draw)
{
...
fn UI_Signal ui_alpha_picker(F32* out_alpha, String8 string)
{
  // rjf: build & interact
  ui_set_next_hover_cursor(OS_Cursor_HandPoint);
  UI_Box* box = ui_build_box_from_string(UI_BoxFlag_Clickable, string);
...
fn UI_Signal ui_alpha_pickerf(F32* out_alpha, char* fmt, ...)
{
  Temp scratch = scratch_begin(0, 0);
  va_list args;
  va_start(args, fmt);
...
fn UI_Box* ui_row_begin()    { return ui_named_row_begin(str8_lit("")); }
fn UI_Signal ui_row_end()    { return ui_named_row_end(); }
fn UI_Box* ui_column_begin() { return ui_named_column_begin(str8_lit("")); }
fn UI_Signal ui_column_end() { return ui_named_column_end(); }
...
fn UI_Signal ui_named_column_end()
{
  UI_Box* box = ui_pop_parent();
  UI_Signal sig = ui_signal_from_box(box);
  return sig;
...
fn void ui_table_begin(ulong column_pct_count, F32 **column_pcts, String8 string)
{
...
fn void ui_table_beginf(ulong column_pct_count, F32 **column_pcts, char* fmt, ...)
{
  Temp scratch = scratch_begin(0, 0);
  va_list args;
  va_start(args, fmt);
...
fn UI_Signal ui_table_vector_end()
{
  UI_Box* box = ui_pop_parent();
...
fn UI_Signal ui_table_cell_end()
{
...
fn void ui_scroll_list_row_block_chunk_list_push(Arena* arena, UI_ScrollListRowBlockChunkList* list, ulong cap, UI_ScrollListRowBlock* block)
{
...
fn UI_ScrollListRowBlockArray ui_scroll_list_row_block_array_from_chunk_list(Arena* arena, UI_ScrollListRowBlockChunkList* list)
{
...
fn ulong ui_scroll_list_row_from_item(UI_ScrollListRowBlockArray* blocks, ulong item)
{
  ulong result = 0;
  {
    ulong row_idx = 0;
...
fn ulong ui_scroll_list_item_from_row(UI_ScrollListRowBlockArray* blocks, ulong row)
{
  ulong result = 0;
  {
    ulong row_idx = 0;
...
fn UI_ScrollPt ui_scroll_bar(Axis2 axis, UI_Size off_axis_size, UI_ScrollPt pt, Rng1S64 idx_range, S64 view_num_indices)
{
  ...
  {
    String8 arrow_string = ui_icon_string_from_kind(axis == Axis2_X ? UI_IconKind.LEFT_ARROW : UI_IconKind.UP_ARROW);
  ...
  UI_Parent(container_box)
    ...
    UI_Font(ui_icon_font())
  {
    String8 arrow_string = ui_icon_string_from_kind(axis == Axis2_X ? UI_IconKind.RIGHT_ARROW : UI_IconKind.DOWN_ARROW);
...
fn void ui_scroll_list_begin(UI_ScrollListParams* params, UI_ScrollPt* scroll_pt, Vec2S64* cursor_out, Vec2S64* mark_out, Rng1S64* visible_row_range_out, UI_ScrollListSignal* signal_out)
{
  ...
  if(params.flags & UI_ScrollListFlag_Snap && moved)
  {
    ...
    if(params.item_range.min <= cursor_item_idx && cursor_item_idx <= params.item_range.max)
    {
      ...
      {
        cursor_visibility_row_range.min = (S64)ui_scroll_list_row_from_item(&params.row_blocks, (ulong)cursor_item_idx);
