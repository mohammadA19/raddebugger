...
internal S64
ui_scanned_column_from_column(StringView string, S64 start_column, Side side)
{
...
ui_push_string_replace_range(Arena *arena, StringView string, Rng1S64 col_range, StringView replace)
{
  ...
  StringView result = str8(push_base, new_size);
  return result;
}
...
internal void
ui_set_autocomplete_string(StringView string)
{
...
{
  StringView result = ui_state.autocomplete_string;
...
ui_get_drag_data(U64 min_required_size)
{
  ...
  {
    Temp scratch = scratch_begin(0, 0);
    StringView str = {push_array(scratch.arena, U8, min_required_size), min_required_size};
...
internal Vec4F32
ui_color_from_name(StringView name)
{
    ...
    if (node == 0 || node.last_build_index_accessed < ui_state.build_index)
    {
      ...
      for (U64 idx = 0; idx < theme.patterns_count; idx += 1)
      {
        ...
        for EachIndex(p_tags_idx, p.tags.count)
        {
          ...
          {
            StringView key_string = key_tags_idx < tags.count ? tags.v[key_tags_idx] : extras.v[key_tags_idx - tags.count];
...
internal Vec4F32
ui_color_from_tags_key_name(UI_Key key, StringView name)
{
...
internal void
ui__push_tags_key_from_appended_string(StringView string)
{
...
internal StringView
ui_push_tag(StringView v)
{
  ui__push_tags_key_from_appended_string(v);
  UI_StackPushImpl(ui_state, Tag, tag, StringView, push_str8_copy(ui_build_arena(), v))
}
...
internal StringView
ui_set_next_tag(StringView v)
{
  ui__push_tags_key_from_appended_string(v);
  UI_StackSetNextImpl(ui_state, Tag, tag, StringView, push_str8_copy(ui_build_arena(), v))
}
