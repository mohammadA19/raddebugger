...
String8 ui_push_string_replace_range(Arena *arena, String8 string, Rng1S64 col_range, String8 replace)
{
...
void ui_set_autocomplete_string(String8 string)
{
  ui_state->autocomplete_string = push_str8_copy(ui_build_arena(), string);
}

String8 ui_autocomplete_string(void)
{
  return ui_state->autocomplete_string;
}

String8 ui_autocomplete(void)
{
...
U64 ui_string_hover_begin_time_us(void)
{
  return ui_state->string_hover_begin_us;
}

DR_FStrList ui_string_hover_fstrs(Arena *arena)
{
...
void ui_begin_build(OS_Handle window, UI_EventList *events, UI_IconInfo *icon_info, UI_Theme *theme, UI_AnimationInfo *animation_info, F32 real_dt, F32 animation_dt)
{
...
void ui_calc_sizes_standalone__in_place(UI_Box *root, Axis2 axis)
{
  ProfBeginFunction();
...
void ui_calc_sizes_upwards_dependent__in_place(UI_Box *root, Axis2 axis)
{
  ProfBeginFunction();
  for(UI_Box *b = root; !ui_box_is_nil(b); b = ui_box_rec_df_pre(b, root).next)
  {
...
void ui_calc_sizes_downwards_dependent__in_place(UI_Box *root, Axis2 axis)
{
  ProfBeginFunction();
  UI_BoxRec rec = {0};
...
void ui_layout_enforce_constraints__in_place(UI_Box *root, Axis2 axis)
{
  ProfBeginFunction();
  Temp scratch = scratch_begin(0, 0);
...
void ui_layout_position__in_place(UI_Box *root, Axis2 axis)
{
  ProfBeginFunction();
  for(UI_Box *box = root; !ui_box_is_nil(box); box = ui_box_rec_df_pre(box, root).next)
  {
...
Vec4F32 ui_color_from_name(String8 name)
{
  Vec4F32 result = ui_color_from_tags_key_name(ui_top_tags_key(), name);
  return result;
}

Vec4F32 ui_color_from_tags_key_extras(UI_Key key, String8Array extras)
{
  Vec4F32 result = {0};
...
Vec4F32 ui_color_from_tags_key_name(UI_Key key, String8 name)
{
...
void ui_box_equip_display_fstrs(UI_Box *box, DR_FStrList *strings)
{
  box->flags |= UI_BoxFlag_HasDisplayString;
...
void ui_box_equip_draw_bucket(UI_Box *box, DR_Bucket *bucket)
{
...
void ui_box_equip_custom_draw(UI_Box *box, UI_BoxCustomDrawFunctionType *custom_draw, void *user_data)
{
...
void ui__push_tags_key_from_appended_string(String8 string)
{
