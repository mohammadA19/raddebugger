...
UI_Signal ui_label(StringView str);
UI_Signal ui_labelf(char* fmt, ...);
void ui_label_multiline(float max, StringView str);
void ui_label_multilinef(float max, char* fmt, ...);
UI_Signal ui_button(StringView str);
UI_Signal ui_buttonf(char* fmt, ...);
UI_Signal ui_hover_label(StringView str);
UI_Signal ui_hover_labelf(char* fmt, ...);
UI_Signal ui_line_edit(TxtPt* cursor, TxtPt* mark, uint8* edit_buffer, uint64 edit_buffer_size, uint64* edit_string_size_out, StringView pre_edit_value, StringView str);
...
UI_Signal ui_image(R_Handle texture, R_Tex2DSampleKind sample_kind, Rng2F32 region, Vec4F32 tint, float blur, StringView str);
UI_Signal ui_imagef(R_Handle texture, R_Tex2DSampleKind sample_kind, Rng2F32 region, Vec4F32 tint, float blur, char* fmt, ...);

////////////////////////////////
//~ rjf: Special Buttons

UI_Signal ui_expander(B32 is_expanded, StringView str);
UI_Signal ui_expanderf(B32 is_expanded, char* fmt, ...);
UI_Signal ui_sort_header(B32 sorting, B32 ascending, StringView str);
...
//- rjf: tooltips
void ui_do_color_tooltip_hsv(Vec3F32 hsv);
void ui_do_color_tooltip_hsva(Vec4F32 hsva);

//- rjf: saturation/value picker
UI_Signal ui_sat_val_picker(float hue, float* out_sat, float* out_val, StringView str);
UI_Signal ui_sat_val_pickerf(float hue, float* out_sat, float* out_val, char* fmt, ...);

//- rjf: hue picker
UI_Signal ui_hue_picker(float* out_hue, float sat, float val, StringView str);
UI_Signal ui_hue_pickerf(float* out_hue, float sat, float val, char* fmt, ...);

//- rjf: alpha picker
UI_Signal ui_alpha_picker(float* out_alpha, StringView str);
...
UI_Box* ui_named_row_begin(StringView str);
UI_Signal ui_named_row_end();
UI_Box* ui_named_column_begin(StringView str);
UI_Signal ui_named_column_end();

////////////////////////////////
//~ rjf: Floating Panes

UI_Box* ui_pane_begin(Rng2F32 rect, StringView str);
...
void ui_table_begin(uint64 column_pct_count, float** column_pcts, StringView str);
void ui_table_beginf(uint64 column_pct_count, float** column_pcts, char* fmt, ...);
void ui_table_end();
UI_Box *  ui_named_table_vector_begin(StringView str);
