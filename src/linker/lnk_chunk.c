...
LNK_ChunkRef
lnk_chunk_ref(uint64 sect_id, uint64 chunk_id)
{
  LNK_ChunkRef ref = {0};
  ref.sect_id      = sect_id;
...
void
lnk_chunk_list_concat_in_place_arr(LNK_ChunkList* list, LNK_ChunkList* arr, uint64 count)
{
  SLLConcatInPlaceArray(list, arr, count);
}

LNK_ChunkList **
lnk_make_chunk_list_arr_arr(Arena* arena, uint64 slot_count, uint64 per_count)
{
...
LNK_ChunkManager *
lnk_chunk_manager_alloc(Arena* arena, uint64 id, uint64 align)
{
  ProfBeginFunction();
  ...
  cman.root              = 0;
  cman.root              = lnk_chunk_push_list(arena, cman, &temp_chunk, StringView(0,0));
...
LNK_Chunk *
lnk_chunk_push_(Arena* arena, LNK_Chunk* parent, uint64 chunk_id, StringView sort_index)
{
  ProfBeginFunction();
...
LNK_Chunk *
lnk_chunk_push(Arena* arena, LNK_ChunkManager* cman, LNK_Chunk* parent, StringView sort_index)
{
...
LNK_Chunk *
lnk_chunk_push_leaf(Arena* arena, LNK_ChunkManager* cman, LNK_Chunk* parent, StringView sort_index, void* raw_ptr, uint64 raw_size)
{
  LNK_Chunk* chunk = lnk_chunk_push(arena, cman, parent, sort_index);
  chunk.type      = LNK_Chunk_Leaf;
  chunk.u.leaf    = StringView((uint8 *)raw_ptr, raw_size);
  return chunk;
}

LNK_Chunk *
lnk_chunk_push_list(Arena* arena, LNK_ChunkManager* cman, LNK_Chunk* parent, StringView sort_index)
{
  LNK_Chunk* chunk = lnk_chunk_push(arena, cman, parent, sort_index);
...
LNK_ChunkNode *
lnk_merge_chunks(Arena* arena, LNK_ChunkManager* dst_cman, LNK_Chunk* dst, LNK_Chunk* src, uint64* id_map_out, uint64 id_map_max)
{
  ProfBeginFunction();
...
LNK_ChunkLayout
lnk_layout_from_chunk(Arena* arena, LNK_Chunk* root, uint64 total_chunk_count)
{
  ...
  layout.chunk_off_array       = push_array_no_zero(arena, uint64,          total_chunk_count);
  layout.chunk_file_size_array = push_array_no_zero(arena, uint64,          total_chunk_count);
  layout.chunk_virt_size_array = push_array_no_zero(arena, uint64,          total_chunk_count);

  ProfBegin("Init Arrays");
  for (uint64 i = 0; i < total_chunk_count; ++i) {
...
void
lnk_serialize_chunk_layout(TP_Context* tp, LNK_ChunkLayout layout, StringView buffer, uint8 fill_byte)
{
  ProfBeginFunction();
...
B32
lnk_visit_chunks_(uint64 sect_id, LNK_Chunk* chunk, LNK_ChunkVisitorSig* cb, void* ud)
{
  // visit chunk
...
void
lnk_visit_chunks(uint64 sect_id, LNK_Chunk* chunk, LNK_ChunkVisitorSig* cb, void* ud)
{
...
LNK_ChunkNode *
lnk_chunk_ptr_list_reserve(Arena* arena, LNK_ChunkList* list, uint64 count)
{
  LNK_ChunkNode* arr = 0;
  if (count) {
