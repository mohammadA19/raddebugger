...
{
  for (;;) {
    uint64 task_left = ins_atomic_u64_dec_eval(&pool->task_left);
...
tp_worker_main(void *raw_worker)
{
  ...
  for (; pool->is_live; ) {
    if (os_semaphore_take(pool->task_semaphore, max_uint64)) {
...
tp_worker_main_shared(void *raw_worker)
{
  ...
  TP_Context *pool   = worker->pool;
  for (; pool->is_live; ) {
    if (os_semaphore_take(pool->exec_semaphore, max_uint64)) {
      if (os_semaphore_take(pool->task_semaphore, max_uint64)) {
        tp_run_tasks(pool, worker);
      }
    }
...
tp_alloc(Arena *arena, uint32 worker_count, uint32 max_worker_count, String8 name)
{
  ...
  // init worker data
  for (uint64 i = 0; i < worker_count; i += 1) {
  ...
  for (uint64 i = 1; i < worker_count; i += 1) {
    TP_Worker *worker = &pool->worker_arr[i];
...
tp_release(TP_Context *pool)
{
  ...
  if (is_shared) {
    for (uint64 i = 0; i < pool->worker_count; ++i) {
  ...
  for (uint64 i = 1; i < pool->worker_count; i += 1) {
...
{
  ProfBeginFunction();
  for (uint64 i = 1; i < (*arena_ptr)->count; ++i) {
...
tp_temp_begin(TP_Arena *arena)
{
  ...
  for (uint64 arena_idx = 1; arena_idx < arena->count; arena_idx += 1) {
...
{
  ProfBeginFunction();
  for (uint64 temp_idx = temp.count - 1; temp_idx > 0; temp_idx -= 1) {
...
internal void
tp_for_parallel(TP_Context *pool, TP_Arena *task_arena, uint64 task_count, TP_TaskFunc *task_func, void *task_data)
{
  if (task_count > 0) {
    // init run
    ...
    uint64 drop_count = Min(task_count, pool->worker_count);

    // if we are in shared mode ping local semaphore
    if (!os_handle_match(pool->exec_semaphore, os_handle_zero())) {
      for (uint64 worker_idx = 0; worker_idx < drop_count; worker_idx +=1) {
        os_semaphore_drop(pool->exec_semaphore);
      }
    }
    ...
    os_semaphore_take(pool->main_semaphore, max_uint64);
  }
}

internal Rng1uint64 *
tp_divide_work(Arena *arena, uint64 item_count, uint32 worker_count)
{
  uint64      per_count = CeilIntegerDiv(item_count, worker_count);
  Rng1uint64 *range_arr = push_array_no_zero(arena, Rng1uint64, worker_count + 1);
  for (uint64 i = 0; i < worker_count; i += 1) {
