...
tp_arena_alloc(TP_Context *pool)
{
  ...
  Arena **arr = new Arena *[pool->worker_count];
  for (U64 i = 0; i < pool->worker_count; ++i) {
    arr[i] = arena_alloc();
  }
  Arena **dst = new Arena *[pool->worker_count];
  MemoryCopy(dst, arr, sizeof(Arena*) * pool->worker_count);
  TP_Arena *worker_arena_arr = new TP_Arena[1];
...
tp_temp_begin(TP_Arena *arena)
{
  ...
  temp.v     = /* no zero */ new Temp[arena->count];
