...
internal uint64
cv_compute_leaf_record_size(String8 data, uint64 align)
{
  uint64 size = 0;
...
internal uint64
cv_serialize_leaf_to_buffer(uint8 *buffer, uint64 buffer_cursor, uint64 buffer_size, CV_LeafKind kind, String8 data, uint64 align)
{
...
internal String8
cv_serialize_raw_leaf(Arena *arena, CV_LeafKind kind, String8 data, uint64 align)
{
...
internal uint64
cv_deserial_leaf(String8 raw_data, uint64 off, uint64 align, CV_Leaf *leaf_out)
{
...
internal String8
cv_make_obj_name(Arena *arena, String8 obj_path, uint32 sig)
{
  ProfBeginFunction();
  Temp scratch = scratch_begin(&arena, 1);
...
internal CV_Symbol
cv_make_proc_ref(Arena *arena, CV_ModIndex imod, uint32 stream_offset, String8 name, B32 is_local)
{
  ...
  symbol.offset = max_uint64;
  
  return symbol;
}

internal CV_Symbol
cv_make_pub32(Arena *arena, CV_Pub32Flags flags, uint32 off, uint16 isect, String8 name)
{
...
cv_parse_debug_s_c13_(Arena *arena, CV_DebugS *debug_s, String8 raw_debug_s)
{
  for (uint64 cursor = 0; cursor + sizeof(CV_C13SubSectionHeader) <= raw_debug_s.size; ) {
    ...
    if (~header.kind & CV_C13SubSectionKind_IgnoreFlag) {
      ...
      Rng1uint64 sub_sect_range = r1u64(cursor, cursor + header.size);
...
internal CV_DebugT
cv_debug_t_from_data(Arena *arena, String8 data, uint64 align)
{
  String8Array arr = {0};
  arr.count        = 1;
  arr.v            = &data;
...
cv_debug_t_get_leaf(CV_DebugT debug_t, uint64 leaf_idx)
{
  ...
  String8 data = str8(ptr, max_uint64);
...
internal void
cv_parse_symbol_sub_section_capped(Arena *arena, CV_SymbolList *list, uint64 offset_base, String8 data, uint64 align, uint64 cap)
{
  uint64 count = 0;
  for (uint64 cursor = 0, opl = data.size; cursor < opl && count < cap; count += 1) {
    // read symbol header
    ...
    // get symbol data
    Rng1uint64 symbol_data_range = r1u64(cursor, symbol_opl);
...
internal void
cv_parse_symbol_sub_section(Arena *arena, CV_SymbolList *list, uint64 offset_base, String8 data, uint64 align)
{
  cv_parse_symbol_sub_section_capped(arena, list, offset_base, data, align, max_uint64);
}
...
THREAD_POOL_TASK_FUNC(cv_symbol_list_syncer)
{
  ...
  // context shortcuts
  Rng1uint64 list_range  = task->list_range_arr[task_id];
...
cv_symbol_ptr_array_from_list(Arena *arena, TP_Context *tp, uint64 count, CV_SymbolList *list_arr)
{
  ...
  for (uint64 thread_idx = 0, symbol_base = 0; thread_idx < tp->worker_count; thread_idx += 1) {
    ...
    Rng1uint64 range = task.list_range_arr[thread_idx];
...
internal void
cv_c13_patch_string_offsets_in_checksum_list(CV_ChecksumList checksum_list, String8 string_data, uint64 string_data_base_offset, CV_StringHashTable string_ht)
{
...
cv_c13_lines_from_sub_sections(Arena *arena, String8 c13_data, Rng1uint64 ss_range)
{
  ProfBeginFunction();
...
cv_c13_line_array_from_data(Arena *arena, String8 c13_data, uint64 sec_base, CV_C13LinesHeader parsed_lines)
{
  CV_LineArray result;
                                  ...
                                  String8                   binary_annots)
{
  ...
  struct SourceFile {
    ...
    uint64                line_count;
    uint64                checksum_off;
    Rng1uint64            last_code_range;
  };

  Rng1uint64List        code_ranges = {0};
...
cv_make_defined_range_list_from_gaps(Arena *arena, Rng1uint64 defrange, CV_LvarAddrGap *gaps, uint64 gap_count)
{
  ...
  if (gap_count == 0) {
    ...
    for (uint64 gap_idx = 0; gap_idx < gap_count; ++gap_idx) {
      // make range
      Rng1uint64 range = rng_1u64(cursor, cursor + gaps[gap_idx].off);
    ...
    if (gap_count > 0) {
      ...
      if (last_range_byte_size) {
        Rng1uint64 last_range = rng_1u64(defrange.min + last_gap.off + last_gap.len, defrange.max);
