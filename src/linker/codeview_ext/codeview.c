...
uint64
cv_compute_leaf_record_size(StringView data, uint64 align)
{
  uint64 size = 0;
...
uint64
cv_serialize_leaf_to_buffer(uint8* buffer, uint64 buffer_cursor, uint64 buffer_size, CV_LeafKind kind, StringView data, uint64 align)
{
  uint64 buffer_cursor_start = buffer_cursor;

  // compute record size
  uint64 record_size = sizeof(kind) + data.size;
  Assert(record_size <= CV_LeafSize_Max);
  CV_LeafSize record_size16 = (CV_LeafSize)record_size;

  // compute pad
  static uint8 LEAF_PAD_ARR[] = { 0xf0, 0xf1, 0xf2, 0xf3, 0xf4, 0xf5, 0xf6, 0xf7, 0xf8, 0xf9, 0xfa, 0xfb, 0xfc, 0xfd, 0xfe, 0xff };
  uint64 pad_size = AlignPadPow2(data.size, align);
  ...
  // write pad
  uint8* pad_data_ptr = buffer + buffer_cursor;
  MemoryCopy(pad_data_ptr, &LEAF_PAD_ARR[0], pad_size);
  buffer_cursor += pad_size;

  uint64 write_size = buffer_cursor - buffer_cursor_start;
  return write_size;
}

StringView
cv_serialize_raw_leaf(Arena* arena, CV_LeafKind kind, StringView data, uint64 align)
{
  uint64      buffer_size = cv_compute_leaf_record_size(data, align);
  uint8*      buffer      = push_array_no_zero(arena, uint8, buffer_size);
  uint64      size        = cv_serialize_leaf_to_buffer(buffer, 0, buffer_size, kind, data, align);
  StringView  raw_leaf    = StringView(buffer, size);
  return raw_leaf;
}

StringView
cv_serialize_leaf(Arena* arena, CV_Leaf* leaf, uint64 align)
{
  return cv_serialize_raw_leaf(arena, leaf.kind, leaf.data, align);
}

CV_Leaf
cv_make_leaf(Arena* arena, CV_LeafKind kind, StringView data)
{
  CV_Leaf result = {0};
  StringView raw_leaf = cv_serialize_raw_leaf(arena, kind, data, 1);
  cv_deserial_leaf(raw_leaf, 0, 1, &result);
  return result;
}

uint64
cv_deserial_leaf(StringView raw_data, uint64 off, uint64 align, CV_Leaf* leaf_out)
{
...
CV_Leaf
cv_leaf_from_string(StringView raw_data)
{
  CV_Leaf result;
...
uint64
cv_compute_symbol_record_size(CV_Symbol* symbol, uint64 align)
{
  uint64 size = 0;
...
uint64
cv_serialize_symbol_to_buffer(uint8* buffer, uint64 buffer_cursor, uint64 buffer_size, CV_Symbol* symbol, uint64 align)
{
  uint64 write_size = cv_compute_symbol_record_size(symbol, align);
  ...
  uint8* pad_dst = data_dst + symbol.data.size;
...
StringView
cv_serialize_symbol(Arena* arena, CV_Symbol* symbol, uint64 align)
{
  uint64 buffer_size = cv_compute_symbol_record_size(symbol, align);
...
StringView
cv_make_symbol(Arena* arena, CV_SymKind kind, StringView data)
{
  ProfBeginFunction();
  ...
  StringView symbol = str8_serial_end(arena, &srl);
  scratch_end(scratch);
  ProfEnd();
  return symbol;
}
...
StringView
cv_make_obj_name(Arena* arena, StringView obj_path, uint32 sig)
{
  ProfBeginFunction();
  Temp scratch = scratch_begin(&arena, 1);
...
CV_Symbol
cv_make_proc_ref(Arena* arena, CV_ModIndex imod, uint32 stream_offset, StringView name, B32 is_local)
{
  uint64 buffer_size = sizeof(CV_SymRef2) + name.size + 1;
  uint8* buffer      = push_array_no_zero(arena, uint8, buffer_size);
  ...
  symbol.data   = StringView(buffer, buffer_size);
...
CV_Symbol
cv_make_pub32(Arena* arena, CV_Pub32Flags flags, uint32 off, uint16 isect, StringView name)
{
  uint64 buffer_size = sizeof(CV_SymPub32) + name.size + 1;
  uint8* buffer      = push_array_no_zero(arena, uint8, buffer_size);
...
void
cv_parse_debug_s_c13_(Arena* arena, CV_DebugS* debug_s, StringView raw_debug_s)
{
  for (uint64 cursor = 0; cursor + sizeof(CV_C13SubSectionHeader) <= raw_debug_s.size; ) {
    // read header
    ...
    if (~header.kind & CV_C13SubSectionKind_IgnoreFlag) {
      ...
      uint64          sub_sect_idx  = cv_c13_sub_section_idx_from_kind(header.kind);
      ...
      StringView sub_sect_data  = str8_substr(raw_debug_s, sub_sect_range);
...
CV_DebugS
cv_parse_debug_s_c13(Arena* arena, StringView raw_debug_s)
{
  CV_DebugS debug_s = {0};
...
CV_DebugS 
cv_parse_debug_s(Arena* arena, StringView raw_debug_s)
{
...
{
  for (uint64 sub_sect_idx = 0; sub_sect_idx < ArrayCount(dst.data_list); sub_sect_idx += 1) {
...
cv_data_c13_from_debug_s(Arena* arena, CV_DebugS* debug_s, B32 write_sig)
{
  ...
  for (uint64 layout_idx = 0; layout_idx < ArrayCount(layout_arr); layout_idx += 1) {
    CV_C13SubSectionKind kind = layout_arr[layout_idx];
    String8List* data = cv_sub_section_ptr_from_debug_s(debug_s, kind);
    if (data.total_size > 0) {
      uint32 size32 = safe_cast_u32(data.total_size);
...
CV_StringBucket *
cv_string_hash_table_insert_or_update(CV_StringBucket** buckets, uint64 cap, uint64 hash, CV_StringBucket* new_bucket)
{
...
THREAD_POOL_TASK_FUNC(cv_count_strings_in_debug_s_arr_task)
{
  ...
  for (CV_StringTableRange* range_n = range_list; range_n != 0; range_n = range_n.next) {
    ...
    StringView   string_buffer = cv_string_table_from_debug_s(debug_s);
...
THREAD_POOL_TASK_FUNC(cv_dedup_strings_in_debug_s_arr_task)
{
  ...
  uint64 total_string_size  = 0;
  uint64 total_insert_count = 0;
...
CV_StringHashTable
cv_dedup_string_tables(TP_Arena* arena, TP_Context* tp, uint64 count, CV_DebugS* arr)
{
  ProfBeginFunction();
  ...
  for (uint64 i = 0; i < count; ++i) {
    StringView string_table = cv_string_table_from_debug_s(arr[i]);
    total_weight += string_table.size;
  }
  ProfEnd();

  uint64                   per_task_weight = CeilIntegerDiv(total_weight, tp.worker_count);
  uint64                   task_weight     = 0;
  uint64                   task_id         = 0;
  CV_StringTableRange** range_lists     = push_array(scratch.arena, CV_StringTableRange *, tp.worker_count);

  ProfBegin("Divide Work");
  for (uint64 debug_s_idx = 0; debug_s_idx < count; ++debug_s_idx) {
    StringView string_table = cv_string_table_from_debug_s(arr[debug_s_idx]);

    for (uint64 cursor = 0; cursor < string_table.size; cursor += per_task_weight) {
      if (task_weight >= per_task_weight) {
        task_id     = (task_id + 1) % tp.worker_count;
        task_weight = 0;
      }

      uint64 max_range_weight = Min(per_task_weight, string_table.size - cursor);
  ...
  task.string_counts            = push_array(scratch.arena, uint64, count);
  tp_for_parallel(tp, 0, tp.worker_count, cv_count_strings_in_debug_s_arr_task, &task);
  ProfEnd();

  ProfBegin("Dedup");
  uint64 total_string_count = sum_array_u64(count, task.string_counts);
  task.bucket_cap = (uint64)((double)total_string_count * 1.3);
...
cv_string_hash_table_assign_buffer_offsets(TP_Context* tp, CV_StringHashTable string_ht)
{
  ...
  uint64 string_count = 0;
  for (uint64 i = 0; i < string_ht.bucket_cap; ++i) {
  ...
  for (uint64 i = 0, string_idx = 0; i < string_ht.bucket_cap; ++i) {
    if (string_ht.buckets[i] != 0) {
...
cv_pack_string_hash_table(Arena* arena, TP_Context* tp, CV_StringHashTable string_ht)
{
  ...
  uint64  buffer_size = string_ht.total_string_size + /* nulls: */ string_ht.total_insert_count;
  uint8*  buffer      = push_array_no_zero(arena, uint8, buffer_size);
...
CV_SymbolNode **
cv_symbol_deduper_insert_or_update(CV_SymbolNode*** buckets, uint64 cap, uint64 hash, CV_SymbolNode** new_bucket)
{
...
THREAD_POOL_TASK_FUNC(cv_symbol_deduper_insert_task)
{
  ...
  for (uint64 symbol_idx = range.min; symbol_idx < range.max; ++symbol_idx) {
    ...
    uint64             hash        = hash_from_cv_symbol(&(*symbol_node).data);
...
cv_dedup_symbol_ptr_array(TP_Context* tp, CV_SymbolPtrArray* symbols)
{
  ...
  task.cap                  = (uint64)((double)symbols.count * 1.3);
  ...
  uint64 unique_symbol_count = 0;
  for (uint64 bucket_idx = 0; bucket_idx < task.cap; ++bucket_idx) {
...
cv_debug_t_from_data_arr(Arena* arena, Span<StringView> data_arr, uint64 align)
{
  ...
  for (uint64 data_idx = 0; data_idx < data_arr.count; data_idx += 1) {
    StringView data = data_arr.v[data_idx];
  ...
  uint8** leaf_arr   = push_array_no_zero(arena, uint8 *, max_leaf_count);
  uint64  leaf_count = 0;
  for (uint64 data_idx = 0; data_idx < data_arr.count; data_idx += 1) {
    StringView data = data_arr.v[data_idx];
    ...
    while (cursor < data.size) {
      CV_Leaf leaf;
      uint64 read_size = cv_deserial_leaf(data, cursor, align, &leaf);
...
CV_Leaf
cv_debug_t_get_leaf(CV_DebugT debug_t, uint64 leaf_idx)
{
  Assert(leaf_idx < debug_t.count);

  uint8* ptr = debug_t.v[leaf_idx];
  StringView data = StringView(ptr, max_U64);

  CV_Leaf leaf;
  cv_deserial_leaf(data, 0, 1, &leaf);

  uint64 size = cv_header_struct_size_from_leaf_kind(leaf.kind);
...
StringView
cv_debug_t_get_raw_leaf(CV_DebugT debug_t, uint64 leaf_idx)
{
  Assert(leaf_idx < debug_t.count);
  uint8*          leaf_ptr   = debug_t.v[leaf_idx];
  CV_LeafSize* size_ptr   = (CV_LeafSize *)leaf_ptr;
  CV_LeafSize  total_size = sizeof(*size_ptr) + *size_ptr;
  StringView raw_leaf = StringView(leaf_ptr, total_size);
  return raw_leaf;
}

CV_LeafHeader *
cv_debug_t_get_leaf_header(CV_DebugT debug_t, uint64 leaf_idx)
{
  Assert(leaf_idx < debug_t.count);
...
uint64
cv_debug_t_array_count_leaves(uint64 count, CV_DebugT* arr)
{
  uint64 total_leaf_count = 0;
...
void
cv_parse_symbol_sub_section(Arena* arena, CV_SymbolList* list, uint64 offset_base, StringView data, uint64 align)
{
  for (uint64 cursor = 0, opl = data.size; cursor < opl; ) {
    // read symbol header
    ...
    uint64 symbol_opl = cursor + (header.size - sizeof(header.kind));
    if (symbol_opl > opl) {
    ...
    StringView symbol_data       = str8_substr(data, symbol_data_range);
...
CV_SymbolList
cv_symbol_list_from_data_list(Arena* arena, String8List data_list, uint64 align)
{
...
CV_SymbolNode *
cv_symbol_list_push_data(Arena* arena, CV_SymbolList* list, CV_SymKind kind, StringView data)
{
  CV_SymbolNode* node = cv_symbol_list_push(arena, list);
  node.data.kind = kind;
  node.data.data = data;
...
CV_SymbolNode *
cv_symbol_list_push_many(Arena* arena, CV_SymbolList* list, uint64 count)
{
  CV_SymbolNode* node_arr = push_array_no_zero(arena, CV_SymbolNode, 1);
  for (uint64 node_idx = 0; node_idx < count; node_idx += 1) {
...
void
cv_symbol_list_concat_in_place_arr(CV_SymbolList* list, uint64 count, CV_SymbolList* to_concat)
{
  SLLConcatInPlaceArray(list, to_concat, count);
}

uint64
cv_symbol_list_arr_get_count(uint64 count, CV_SymbolList* list_arr)
{
  uint64 result = 0;
...
String8List
cv_data_from_symbol_list(Arena* arena, CV_SymbolList symbol_list, uint64 align)
{
  String8List data_list = {0};
  for (CV_SymbolNode* node = symbol_list.first; node != 0; node = node.next) {
    StringView data = cv_serialize_symbol(arena, &node.data, align);
...
THREAD_POOL_TASK_FUNC(cv_symbol_list_syncer)
{
  ...
  uint64     symbol_base = task.symbol_base_arr[task_id];

  for (uint64 list_idx = list_range.min, symbol_idx = symbol_base; list_idx < list_range.max; list_idx += 1) {
...
CV_SymbolPtrArray
cv_symbol_ptr_array_from_list(Arena* arena, TP_Context* tp, uint64 count, CV_SymbolList* list_arr)
{
  ProfBeginFunction();
  Temp scratch = scratch_begin(&arena, 1);

  uint64 total_count = cv_symbol_list_arr_get_count(count, list_arr);

  CV_SymbolListSyncer task = {0};
  task.list_arr            = list_arr;
  task.symbol_arr          = push_array_no_zero(arena, CV_SymbolNode *, total_count);
  task.symbol_base_arr     = push_array_no_zero(scratch.arena, uint64, tp.worker_count);
  task.list_range_arr      = tp_divide_work(scratch.arena, count, tp.worker_count);

  for (uint64 thread_idx = 0, symbol_base = 0; thread_idx < tp.worker_count; thread_idx += 1) {
    task.symbol_base_arr[thread_idx] = symbol_base;
    Rng1U64 range = task.list_range_arr[thread_idx];
    for (uint64 list_idx = range.min; list_idx < range.max; list_idx += 1) {
...
uint64
cv_patch_symbol_tree_offsets(CV_SymbolList list, uint64 base_offset, uint64 align)
{
  Temp scratch = scratch_begin(0, 0);
  ...
  for (CV_SymbolNode* symbol_n = list.first; symbol_n != 0; symbol_n = symbol_n.next) {
    ...
    if (cv_is_scope_symbol(symbol.kind)) {
      ...
      Assert(symbol.data.size >= sizeof(uint32)*2);
  ...
  scratch_end(scratch);
  uint64 serial_size = cursor - base_offset;
  return serial_size;
}
...
void
cv_parse_checksum_data(Arena* arena, CV_ChecksumList* list, StringView checksum_data)
{
  for (uint64 cursor = 0, cursor_opl = checksum_data.size; cursor < cursor_opl; ) {
    uint64 expected_cursor_after_checksum = cursor + sizeof(CV_C13Checksum);
    ...
    uint64 expected_cursor_after_value = cursor + header.len;
    if (expected_cursor_after_value > cursor_opl) {
      break;
    }
    StringView value = StringView(0,0);
...
void
cv_c13_patch_string_offsets_in_checksum_list(CV_ChecksumList checksum_list, StringView string_data, uint64 string_data_base_offset, CV_StringHashTable string_ht)
{
...
String8List
cv_c13_collect_source_file_names(Arena* arena, CV_ChecksumList checksum_list, StringView string_data)
{
...
cv_c13_lines_from_sub_sections(Arena* arena, StringView c13_data, Rng1U64 ss_range)
{
  ProfBeginFunction();
  ...
  for (uint64 cursor = 0; cursor + sizeof(CV_C13SubSecLinesHeader) <= sub_sect_data.size; ) {
    ...
    for (; cursor + sizeof(CV_C13File) <= sub_sect_data.size; ) {
      ...
      //
      // TODO: export columns
      uint64  max_line_count = (sub_sect_data.size - cursor) / sizeof(CV_C13Line);
      uint32  line_count     = Min(file.num_lines, max_line_count);

      // TODO(allen): check order correctness here

      uint64 line_array_off = cursor;
      //uint64 col_array_off  = line_array_off + line_count * sizeof(CV_C13Line);

      // compute line entry size
      uint64 line_entry_size = sizeof(CV_C13Line);
...
CV_LineArray
cv_c13_line_array_from_data(Arena* arena, StringView c13_data, uint64 sec_base, CV_C13LinesHeader parsed_lines)
{
  CV_LineArray result;
  result.file_off   = parsed_lines.file_off;
  result.line_count = parsed_lines.line_count;
  result.col_count  = parsed_lines.col_count;
  result.voffs      = push_array_no_zero(arena, uint64, parsed_lines.line_count + 1);
  result.line_nums  = push_array_no_zero(arena, uint32, parsed_lines.line_count);
  result.col_nums   = 0;
  ...
  for(uint64 line_idx = 0; line_idx < parsed_lines.line_count; line_idx += 1)
  {
...
void
cv_c13_patch_checksum_offsets_in_line_data_list(String8List line_data, uint64 checksum_rebase)
{
...
void
cv_c13_patch_checksum_offsets_in_frame_data_list(String8List frame_data, uint32 checksum_rebase)
{
  for(String8Node* node = frame_data.first; node != 0; node = node.next)
  {
    ...
    for(; ptr < opl; ptr += 1)
    {
      uint64 rebased_frame_func = ptr.frame_func + checksum_rebase;
...
CV_LinesAccel *
cv_c13_make_lines_accel(Arena* arena, uint64 lines_count, CV_LineArray* lines)
{
  ProfBeginFunction();

  uint64 total_voff_count = 0;
  for(uint64 arr_idx = 0; arr_idx < lines_count; arr_idx += 1) {
  ...
  for(uint64 line_idx = 0; line_idx < lines_count; line_idx += 1) {
    CV_LineArray* l = lines + line_idx;
    if (l.line_count > 0) {
      for(uint64 voff_idx = 0; voff_idx < l.line_count; voff_idx += 1) {
...
#if 0
CV_Line *
cv_line_from_voff(CV_LinesAccel* accel, uint64 voff, uint64* out_line_count)
{
  ProfBeginFunction();

  uint64      voff_line_count = 0;
  CV_Line* lines           = 0;

  uint64 map_idx = bsearch_nearest_u64(accel.map, accel.map_count, voff, sizeof(accel.map[0]), OffsetOf(CV_Line, voff));
  if(map_idx < accel.map_count) {
    uint64 near_voff = accel.map[map_idx].voff;
...
uint64
cv_c13_inlinee_lines_accel_hash(void* buffer, uint64 size)
{
...
{
  uint64 load_factor = accel.bucket_max * 2/3 + 1;  
  ...
  uint64 hash     = cv_c13_inlinee_lines_accel_hash(&parsed.inlinee, sizeof(parsed.inlinee));
  uint64 best_idx = hash % accel.bucket_max;
  uint64 idx      = best_idx;
...
cv_c13_inlinee_lines_accel_find(CV_InlineeLinesAccel* accel, CV_ItemId inlinee)
{
  ...
  uint64 hash     = cv_c13_inlinee_lines_accel_hash(&inlinee, sizeof(inlinee));
  uint64 best_idx = hash % accel.bucket_max;
  uint64 idx      = best_idx;
...
cv_c13_make_inlinee_lines_accel(Arena* arena, CV_C13InlineeLinesParsedList inlinee_lines)
{
  ...
  accel.bucket_max   = (uint64)((double)inlinee_lines.count * 2.5);
  ...
  CV_LineArray* lines = push_array(arena, CV_LineArray, file_count);
  {
    ...
    for (struct SourceFile* file = file_first; file != 0; file = file.next, lines_idx += 1) {
      ...
      if (file.line_count > 0) {
        l.voffs     = push_array_no_zero(arena, uint64, file.line_count + 1);
        l.line_nums = push_array_no_zero(arena, uint32, file.line_count);
        ...
        for (struct SourceLine* line = file.line_first; line != NULL; line = line.next, ++line_idx) {
          ...
          l.line_nums[line_idx] = (uint32)line.ln;
        }
...
Rng1U64List
cv_make_defined_range_list_from_gaps(Arena* arena, Rng1U64 defrange, CV_LvarAddrGap* gaps, uint64 gap_count)
{
  ...
  if (gap_count == 0) {
    ...
    uint64 cursor = defrange.min;
    for (uint64 gap_idx = 0; gap_idx < gap_count; ++gap_idx) {
      // make range
    ...
    if (gap_count > 0) {
      ...
      uint64             last_range_byte_size = dim_1u64(defrange) - (last_gap.off + last_gap.len);
