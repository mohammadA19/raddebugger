...
internal LNK_LibNode *       lnk_lib_list_reserve(Arena *arena, LNK_LibList *list, ulong count);
...
internal void            lnk_lib_symbol_array_sort(LNK_LibSymbol *arr, ulong count);
...
internal void            lnk_lib_writer_push_export(LNK_LibWriter *writer, COFF_MachineType machine, ulong time_stamp, String8 dll_name, LNK_Export *exp);
internal LNK_LibBuild    lnk_lib_build_from_writer(Arena *arena, LNK_LibWriter *writer);
internal String8List     lnk_coff_archive_from_lib_build(Arena *arena, LNK_LibBuild *lib, B32 emit_second_member, COFF_TimeStamp time_stamp, uint mode);
...
internal String8      lnk_build_lib_member_header(Arena *arena, String8 name, COFF_TimeStamp time_stamp, ushort user_id, ushort group_id, ushort mode, uint size);
