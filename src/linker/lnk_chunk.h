...
LNK_ChunkRef lnk_chunk_ref(ulong sect_id, ulong chunk_id);
...
void             lnk_chunk_list_concat_in_place_arr(LNK_ChunkList *list, LNK_ChunkList *arr, ulong count);
LNK_ChunkList ** lnk_make_chunk_list_arr_arr(Arena *arena, ulong slot_count, ulong per_count);
void             lnk_chunk_array_sort(LNK_ChunkArray arr);

LNK_ChunkManager * lnk_chunk_manager_alloc(Arena *arena, ulong id, ulong align);
LNK_Chunk *        lnk_chunk_push(Arena *arena, LNK_ChunkManager *cman, LNK_Chunk *parent, String8 sort_index);
LNK_Chunk *        lnk_chunk_push_leaf(Arena *arena, LNK_ChunkManager *cman, LNK_Chunk *parent, String8 sort_index, void *raw_ptr, ulong raw_size);
...
LNK_ChunkNode *    lnk_merge_chunks(Arena *arena, LNK_ChunkManager *dst_cman, LNK_Chunk *dst, LNK_Chunk *src, ulong *id_map_out, ulong id_map_max);
void               lnk_chunk_associate(LNK_Chunk *head, LNK_Chunk *associate);
B32                lnk_chunk_is_discarded(LNK_Chunk *chunk);
ulong                lnk_chunk_get_size(LNK_Chunk *chunk);
ulong                lnk_chunk_list_get_node_count(LNK_Chunk *chunk);

void lnk_chunk_op_list_push_node(LNK_ChunkOpList *list, LNK_ChunkOp *op);

LNK_ChunkOp * lnk_push_chunk_op_begin(Arena *arena, ulong chunk_id);
LNK_ChunkOp * lnk_push_chunk_op_end_virt(Arena *arena);
LNK_ChunkOp * lnk_push_chunk_op_end_file(Arena *arena);
LNK_ChunkOp * lnk_push_chunk_op_align(Arena *arena, ulong align, ulong val);
LNK_ChunkOp * lnk_push_chunk_op_write(Arena *arena, String8 string);

LNK_ChunkLayout lnk_layout_from_chunk(Arena *arena, LNK_Chunk *root, ulong total_chunk_count);
...
void lnk_visit_chunks(ulong sect_id, LNK_Chunk *root, LNK_ChunkVisitorSig *cb, void *ud);

LNK_ChunkNode * lnk_chunk_ptr_list_reserve(Arena *arena, LNK_ChunkList *list, ulong count);
String8Array    lnk_data_arr_from_chunk_ptr_list(Arena *arena, LNK_ChunkList list);
String8Array *  lnk_data_arr_from_chunk_ptr_list_arr(Arena *arena, LNK_ChunkList *list_arr, ulong count);
