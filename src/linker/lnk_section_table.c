...
LNK_Reloc *
lnk_section_push_reloc(LNK_Section* sect, LNK_Chunk* chunk, LNK_RelocType type, uint64 apply_off, LNK_Symbol* symbol)
{
  Assert(symbol);
...
lnk_section_merge(LNK_Section* dst, LNK_Section* src)
{
  ...
  src.id_map        = push_array_no_zero(src.arena, uint64, src.cman.total_chunk_count);
...
LNK_SectionTable *
lnk_section_table_alloc(uint64 section_virt_off, uint64 sect_align, uint64 file_align)
{
  ProfBeginFunction();
...
LNK_Section *
lnk_section_table_search_id(LNK_SectionTable* st, uint64 id)
{
...
lnk_section_table_get_output_sections(Arena* arena, LNK_SectionTable* st)
{
  ...
  uint64 unused_entry_count = st.list.count - result.count;
...
THREAD_POOL_TASK_FUNC(lnk_section_data_builder)
{
  ...
  for (uint64 sect_idx = range.min; sect_idx < range.max; ++sect_idx) {
...
lnk_section_table_assign_virtual_offsets(LNK_SectionTable* st)
{
  ...
  uint64           cursor      = st.section_virt_off;
...
lnk_section_table_assign_file_offsets(LNK_SectionTable* st)
{
  ...
  for (LNK_SectionNode* sect_node = st.list.first; sect_node != NULL; sect_node = sect_node.next) {
    ...
    uint64 root_size = sect.layout.chunk_file_size_array[sect.root.ref.chunk_id];
    cursor += root_size;
  }
  ProfEnd();
}
...
lnk_sect_id_map_from_section_table(Arena* arena, LNK_SectionTable* st)
{
  ...
  for (uint64 list_idx = 0; list_idx < ArrayCount(list_arr); ++list_idx) {
  ...
  if (map[0] == NULL) {
    ...
    sect.layout.chunk_off_array = push_array(arena, uint64, 1);
    sect.layout.chunk_file_size_array = push_array(arena, uint64, 1);
    sect.layout.chunk_virt_size_array = push_array(arena, uint64, 1);
    map[0] = sect;
  }
  ProfEnd();
  return map;
...
lnk_off_from_chunk_ref(LNK_Section** sect_id_map, LNK_ChunkRef chunk_ref)
{
  LNK_ChunkRef final_chunk_ref = lnk_get_final_chunk_ref(sect_id_map, chunk_ref);
  LNK_Section* sect = sect_id_map[final_chunk_ref.sect_id];
  uint64 off = sect.layout.chunk_off_array[final_chunk_ref.chunk_id];
  return off;
}
...
lnk_virt_off_from_chunk_ref(LNK_Section** sect_id_map, LNK_ChunkRef chunk_ref)
{
  ...
  LNK_Section* sect = sect_id_map[final_chunk_ref.sect_id];
  uint64 off = sect.layout.chunk_off_array[final_chunk_ref.chunk_id];
  uint64 virt_off = off + sect.virt_off;
  return virt_off;
}
...
lnk_file_off_from_chunk_ref(LNK_Section** sect_id_map, LNK_ChunkRef chunk_ref)
{
  ...
  LNK_Section* sect = sect_id_map[final_chunk_ref.sect_id];
  uint64 off = sect.layout.chunk_off_array[final_chunk_ref.chunk_id];
  uint64 file_off = off + sect.file_off;
  return file_off;
}
...
lnk_virt_size_from_chunk_ref(LNK_Section** sect_id_map, LNK_ChunkRef chunk_ref)
{
  LNK_ChunkRef final_chunk_ref = lnk_get_final_chunk_ref(sect_id_map, chunk_ref);
  LNK_Section* sect = sect_id_map[final_chunk_ref.sect_id];
  uint64 virt_size = sect.layout.chunk_virt_size_array[final_chunk_ref.chunk_id];
  return virt_size;
}
...
lnk_file_size_from_chunk_ref(LNK_Section** sect_id_map, LNK_ChunkRef chunk_ref)
{
  LNK_ChunkRef final_chunk_ref = lnk_get_final_chunk_ref(sect_id_map, chunk_ref);
  LNK_Section* sect = sect_id_map[final_chunk_ref.sect_id];
  uint64 file_size = sect.layout.chunk_file_size_array[final_chunk_ref.chunk_id];
  return file_size;
}
...
{
  uint64 virt_off = lnk_virt_off_from_chunk_ref(sect_id_map, reloc.chunk.ref);
...
lnk_isect_from_symbol(LNK_Section** sect_id_map, LNK_Symbol* symbol)
{
  ...
  uint64 symbol_isect = lnk_isect_from_chunk_ref(sect_id_map, symbol_chunk_ref);
  return symbol_isect;
}
...
lnk_sect_off_from_symbol(LNK_Section** sect_id_map, LNK_Symbol* symbol)
{
  Assert(LNK_Symbol_IsDefined(symbol.type));
  LNK_ChunkRef symbol_chunk_ref = symbol.u.defined.u.chunk.ref;
  uint64 chunk_off = lnk_off_from_chunk_ref(sect_id_map, symbol_chunk_ref);
  uint64 symbol_off = chunk_off + symbol.u.defined.u.chunk_offset;
  return symbol_off;
}
...
lnk_virt_off_from_symbol(LNK_Section** sect_id_map, LNK_Symbol* symbol)
{
  Assert(LNK_Symbol_IsDefined(symbol.type));
  LNK_ChunkRef symbol_chunk_ref = symbol.u.defined.u.chunk.ref;
  uint64 chunk_voff = lnk_virt_off_from_chunk_ref(sect_id_map, symbol_chunk_ref);
  uint64 symbol_voff = chunk_voff + symbol.u.defined.u.chunk_offset;
  return symbol_voff;
}
...
lnk_file_off_from_symbol(LNK_Section** sect_id_map, LNK_Symbol* symbol)
{
  Assert(LNK_Symbol_IsDefined(symbol.type));
  LNK_ChunkRef symbol_chunk_ref = symbol.u.defined.u.chunk.ref;
  uint64 chunk_foff = lnk_file_off_from_chunk_ref(sect_id_map, symbol_chunk_ref);
  uint64 symbol_foff = chunk_foff + symbol.u.defined.u.chunk_offset;
  return symbol_foff;
}
...
{
  Assert(LNK_Symbol_IsDefined(symbol.type));
  uint64 symbol_chunk_virt_size = lnk_virt_size_from_chunk_ref(sect_id_map, symbol.u.defined.u.chunk.ref);
...
{
  Assert(LNK_Symbol_IsDefined(symbol.type));
  uint64 symbol_chunk_file_size = lnk_file_size_from_chunk_ref(sect_id_map, symbol.u.defined.u.chunk.ref);
