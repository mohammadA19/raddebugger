...
{
  LNK_SectionContribChunk *chunk = new LNK_SectionContribChunk[1];
  chunk->count    = 0;
  chunk->cap      = cap;
  chunk->v        = new LNK_SectionContrib *[cap];
  chunk->v2       = new LNK_SectionContrib[cap];
...
{
  LNK_SectionContribChunk **result = new LNK_SectionContribChunk *[list.chunk_count];
  U64 i = 0;
...
lnk_section_array_from_list(Arena *arena, LNK_SectionList list)
{
  ...
  result.count = 0;
  result.v = /* no zero */ new LNK_Section *[list.count];
...
lnk_section_table_alloc(void)
{
  ...
  LNK_SectionTable *sectab = new LNK_SectionTable[1];
  sectab->arena            = arena;
...
lnk_section_table_push(LNK_SectionTable *sectab, String8 name, COFF_SectionFlags flags)
{
  ...
  LNK_SectionNode *sect_node = new LNK_SectionNode[1];
...
lnk_section_table_search_many(Arena *arena, LNK_SectionTable *sectab, String8 full_or_partial_name)
{
  ...
  if (match_count > 0) {
    result.count = 0;
    result.v = new LNK_Section *[match_count];
