...
internal LNK_Symbol *
lnk_make_defined_symbol(Arena *arena, StringView name, struct LNK_Obj *obj, U32 symbol_idx)
{
...
internal LNK_Symbol *
lnk_make_lib_symbol(Arena *arena, StringView name, struct LNK_Lib *lib, U64 member_offset)
{
    LNK_Symbol *symbol = push_array(arena, LNK_Symbol, 1);
    symbol.name                = name;
...
internal LNK_Symbol *
lnk_make_undefined_symbol(Arena *arena, StringView name, struct LNK_Obj *obj)
{
    LNK_Symbol *symbol = push_array(arena, LNK_Symbol, 1);
    symbol.name        = name;
            ...
            // handle COMDATs
                ...
                if (src_select == dst_select) {
                    ...
                    case COFF_ComdatSelect_ExactMatch: {
                        ...
                        StringView             dst_data        = str8_substr(dst_obj.data, rng_1u64(dst_sect_header.foff, dst_sect_header.foff + dst_sect_header.fsize));
                        StringView             src_data        = str8_substr(src_obj.data, rng_1u64(src_sect_header.foff, src_sect_header.foff + src_sect_header.fsize));
...
internal LNK_SymbolHashTrie *
lnk_symbol_hash_trie_search(LNK_SymbolHashTrie *trie, U64 hash, StringView name)
{
...
internal LNK_SymbolHashTrie *
lnk_symbol_table_search_(LNK_SymbolTable *symtab, LNK_SymbolScope scope, StringView name)
{
...
internal LNK_Symbol *
lnk_symbol_table_search(LNK_SymbolTable *symtab, LNK_SymbolScope scope, StringView name)
{
        ...
        if (symbol_interp == COFF_SymbolValueInterp_Weak) {
            ...
            for (;;) {
                ...
                if (was_visited) {
                    ...
                    StringView           loc_string    = str8_list_join(temp.arena, &ref_list, &(StringJoin){ .sep = ("\n") });
