...
void
lnk_init_defined_symbol_chunk(LNK_Symbol *symbol, String8 name, LNK_DefinedSymbolVisibility visibility, LNK_DefinedSymbolFlags flags, LNK_Chunk *chunk, ulong offset, COFF_ComdatSelectType selection, uint check_sum)
{
...
void
lnk_init_defined_symbol_va(LNK_Symbol *symbol, String8 name, LNK_DefinedSymbolVisibility visibility, LNK_DefinedSymbolFlags flags, ulong va)
{
  lnk_init_defined_symbol(symbol, name, visibility, flags);
  LNK_DefinedSymbol *def = &symbol->u.defined;
...
void
lnk_init_lazy_symbol(LNK_Symbol *symbol, String8 name, LNK_Lib *lib, ulong member_offset)
{
...
LNK_Symbol * 
lnk_make_defined_symbol_chunk(Arena *arena, String8 name, LNK_DefinedSymbolVisibility visibility, LNK_DefinedSymbolFlags flags, LNK_Chunk *chunk, ulong offset, COFF_ComdatSelectType selection, uint check_sum)
{
  LNK_Symbol *symbol = push_array_no_zero(arena, LNK_Symbol, 1);
...
LNK_Symbol *
lnk_make_defined_symbol_va(Arena *arena, String8 name, LNK_DefinedSymbolVisibility visibility, LNK_DefinedSymbolFlags flags, ulong va)
{
  LNK_Symbol *symbol = push_array_no_zero(arena, LNK_Symbol, 1);
...
LNK_Symbol *
lnk_make_lazy_symbol(Arena *arena, String8 name, LNK_Lib *lib, ulong member_offset)
{
  LNK_Symbol *symbol = push_array_no_zero(arena, LNK_Symbol, 1);
...
LNK_SymbolHashTrie *
lnk_symbol_hash_trie_chunk_list_push(Arena *arena, LNK_SymbolHashTrieChunkList *list, ulong cap)
{
...
lnk_can_replace_symbol(const LNK_Symbol *dst, const LNK_Symbol *src)
{
  ...
  if (dst->type == LNK_Symbol_Lazy && src->type == LNK_Symbol_Lazy) {
    ...
    // strong definition found, replace weak symbol
    can_replace = 1;
  ...
  } else if (LNK_Symbol_IsDefined(dst->type) && LNK_Symbol_IsDefined(src->type)) {
    const LNK_DefinedSymbol *dst_defn = &dst->u.defined;
    const LNK_DefinedSymbol *src_defn = &src->u.defined;

    if (dst_defn->value_type == LNK_DefinedSymbolValue_Chunk &&
        src_defn->value_type == LNK_DefinedSymbolValue_Chunk) {
      ...
      if (src_select == dst_select) {
        ...
        case COFF_ComdatSelect_Any: {
          ...
          ulong dst_chunk_size = lnk_chunk_get_size(dst_chunk);
          ulong src_chunk_size = lnk_chunk_get_size(src_chunk);
        ...
        case COFF_ComdatSelect_SameSize: {
          ...
          ulong dst_chunk_size = lnk_chunk_get_size(dst_chunk);
          ulong src_chunk_size = lnk_chunk_get_size(src_chunk);
          B32 is_same_size = (dst_chunk_size == src_chunk_size);
          if (!is_same_size) {
            lnk_error_obj(LNK_Error_MultiplyDefinedSymbol, src->obj, "multiply defined symbol %S in %S.", dst->name, dst->obj->path);
          }
        } break;
    ...
    } else if (dst_defn->value_type == LNK_DefinedSymbolValue_VA && src_defn->value_type == LNK_DefinedSymbolValue_Chunk) {
      can_replace = 1;
    } else {
      InvalidPath;
    }
...
void
lnk_symbol_hash_trie_insert_or_replace(Arena *arena, LNK_SymbolHashTrieChunkList *chunk_list, LNK_SymbolHashTrie **trie, ulong hash, LNK_Symbol *new_symbol)
{
  LNK_SymbolHashTrie **curr_trie_ptr = trie;

  for (ulong h = hash; ; h <<= 2) {
    // load current pointer
    LNK_SymbolHashTrie *curr_trie = ins_atomic_ptr_eval(curr_trie_ptr);

    if (curr_trie == 0) {
      // init node
      LNK_SymbolHashTrie *new_trie = lnk_symbol_hash_trie_chunk_list_push(arena, chunk_list, 512);
      new_trie->name               = &new_symbol->name;
      new_trie->symbol             = new_symbol;
    ...
    if (curr_name) {
      if (str8_match(*curr_name, new_symbol->name, 0)) {
        for (LNK_Symbol *src = new_symbol, *dst;;) {
          ...
          // try insert back symbol
          dst = ins_atomic_ptr_eval_cond_assign(&curr_trie->symbol, src, 0);
...
LNK_SymbolHashTrie *
lnk_symbol_hash_trie_search(LNK_SymbolHashTrie *trie, ulong hash, String8 name)
{
...
lnk_symbol_table_init(TP_Arena *arena)
{
  ...
  symtab->arena           = arena;
  for (ulong i = 0; i < LNK_SymbolScopeIndex_Count; ++i) {
...
LNK_Symbol *
lnk_symbol_table_search_hash(LNK_SymbolTable *symtab, LNK_SymbolScopeFlags scope_flags, ulong hash, String8 name)
{
  LNK_Symbol *result = 0;
  while (scope_flags) {
...
{
  ulong hash = lnk_symbol_hash(name);
...
void
lnk_symbol_table_push_(LNK_SymbolTable *symtab, Arena *arena, LNK_SymbolHashTrieChunkList *chunk_list, LNK_SymbolScopeIndex scope_idx, ulong hash, LNK_Symbol *symbol)
{
  lnk_symbol_hash_trie_insert_or_replace(arena, chunk_list, &symtab->scopes[scope_idx], hash, symbol);
}

void
lnk_symbol_table_push_hash(LNK_SymbolTable *symtab, ulong hash, LNK_Symbol *symbol)
{
  switch (symbol->type) {
...
LNK_Symbol *
lnk_symbol_table_push_defined_chunk(LNK_SymbolTable *symtab, String8 name, LNK_DefinedSymbolVisibility visibility, LNK_DefinedSymbolFlags flags, LNK_Chunk *chunk, ulong offset, COFF_ComdatSelectType selection, uint check_sum)
{
...
LNK_Symbol *
lnk_symbol_table_push_defined_va(LNK_SymbolTable *symtab, String8 name, LNK_DefinedSymbolVisibility visibility, LNK_DefinedSymbolFlags flags, ulong va)
{
