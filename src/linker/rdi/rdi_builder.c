...
RDIB_LocationNode *
rdib_push_location_addr_reg_off(Arena* arena, RDIB_LocationList* list, RDI_Arch arch, RDI_RegCode reg_code, uint32 reg_byte_size, uint32 reg_byte_pos, int64 offset, B32 is_reference, Rng1U64List ranges)
{
  RDIB_Location loc;

  if (0 <= offset && offset <= (int64)max_U16) {
    if (is_reference) {
