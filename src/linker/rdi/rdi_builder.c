...
rdib_short_type_from_data_model(RDIB_DataModel data_model)
{
  ...
  case RDIB_DataModel_ILP32 : return RDI_TypeKind_uint16;
  case RDIB_DataModel_LLP64 : return RDI_TypeKind_uint16;
  case RDIB_DataModel_LP64  : return RDI_TypeKind_uint16;
  case RDIB_DataModel_ILP64 : return RDI_TypeKind_uint16;
  case RDIB_DataModel_SILP64: return RDI_TypeKind_uint64;
  default: InvalidPath;
  }
...
rdib_int_type_from_data_model(RDIB_DataModel data_model)
{
  switch (data_model) {
  case RDIB_DataModel_Null  : break;
  case RDIB_DataModel_ILP32 : return RDI_TypeKind_uint32;
  case RDIB_DataModel_LLP64 : return RDI_TypeKind_uint32;
  case RDIB_DataModel_LP64  : return RDI_TypeKind_uint32;
  case RDIB_DataModel_ILP64 : return RDI_TypeKind_uint64;
  case RDIB_DataModel_SILP64: return RDI_TypeKind_uint64;
  default: InvalidPath;
  }
  return RDI_TypeKind_NULL;
...
rdib_long_type_from_data_model(RDIB_DataModel data_model)
{
  ...
  case RDIB_DataModel_ILP32 : return RDI_TypeKind_uint32;
  case RDIB_DataModel_LLP64 : return RDI_TypeKind_uint32;
  case RDIB_DataModel_LP64  : return RDI_TypeKind_uint64;
  case RDIB_DataModel_ILP64 : return RDI_TypeKind_uint64;
  case RDIB_DataModel_SILP64: return RDI_TypeKind_uint64;
  default: InvalidPath;
  }
...
rdib_long_long_type_from_data_model(RDIB_DataModel data_model)
{
  switch (data_model) {
  case RDIB_DataModel_Null  : break;
  case RDIB_DataModel_ILP32 : return RDI_TypeKind_uint64;
  case RDIB_DataModel_LLP64 : return RDI_TypeKind_uint64;
  case RDIB_DataModel_LP64  : return RDI_TypeKind_uint64;
  case RDIB_DataModel_ILP64 : return RDI_TypeKind_uint64;
  case RDIB_DataModel_SILP64: return RDI_TypeKind_uint64;
  default: InvalidPath;
...
internal RDIB_LineTableFragment *
rdib_line_table_fragment_chunk_list_push(Arena *arena, RDIB_LineTableFragmentChunkList *list, uint64 cap)
{
...
internal RDIB_Unit *
rdib_unit_chunk_list_push(Arena *arena, RDIB_UnitChunkList *list, uint64 cap)
{
...
internal RDIB_Scope *
rdib_scope_chunk_list_push(Arena *arena, RDIB_ScopeChunkList *list, uint64 cap)
{
...
rdib_procedure_chunk_list_push(Arena *arena, RDIB_ProcedureChunkList *list, uint64 cap)
{
...
rdib_variable_chunk_list_push(Arena *arena, RDIB_VariableChunkList *list, uint64 cap)
{
...
rdib_line_table_chunk_list_push(Arena *arena, RDIB_LineTableChunkList *list, uint64 cap)
{
...
internal RDIB_Type *
rdib_type_chunk_list_push(Arena *arena, RDIB_TypeChunkList *list, uint64 cap)
{
...
rdib_udt_member_chunk_list_push(Arena *arena, RDIB_UDTMemberChunkList *list, uint64 cap)
{
...
rdib_source_file_chunk_list_push(Arena *arena, RDIB_SourceFileChunkList *list, uint64 cap)
{
...
rdib_inline_site_chunk_list_push(Arena *arena, RDIB_InlineSiteChunkList *list, uint64 cap)
{
...
internal RDIB_Unit *
rdib_unit_chunk_list_push_zero(Arena *arena, RDIB_UnitChunkList *list, uint64 cap)
{
...
internal RDIB_Scope *
rdib_scope_chunk_list_push_zero(Arena *arena, RDIB_ScopeChunkList *list, uint64 cap)
{
...
internal RDIB_Procedure *
rdib_procedure_chunk_list_push_zero(Arena *arena, RDIB_ProcedureChunkList *list, uint64 cap)
{
...
internal RDIB_Variable *
rdib_variable_chunk_list_push_zero(Arena *arena, RDIB_VariableChunkList *list, uint64 cap)
{
...
internal RDIB_LineTable *
rdib_line_table_chunk_list_push_zero(Arena *arena, RDIB_LineTableChunkList *list, uint64 cap)
{
...
internal RDIB_Type *
rdib_type_chunk_list_push_zero(Arena *arena, RDIB_TypeChunkList *list, uint64 cap)
{
...
internal RDIB_UDTMember *
rdib_udt_member_chunk_list_push_zero(Arena *arena, RDIB_UDTMemberChunkList *list, uint64 cap)
{
...
internal RDIB_SourceFile *
rdib_source_file_chunk_list_push_zero(Arena *arena, RDIB_SourceFileChunkList *list, uint64 cap)
{
...
internal RDIB_InlineSite *
rdib_inline_site_chunk_list_push_zero(Arena *arena, RDIB_InlineSiteChunkList *list, uint64 cap)
{
...
rdib_unit_chunk_list_reserve_ex(Arena *arena, RDIB_UnitChunkList *list, uint64 count_per_chunk, uint64 item_count)
{
  uint64             chunk_count = CeilIntegerDiv(item_count, count_per_chunk);
  RDIB_UnitChunk *chunks      = push_array(arena, RDIB_UnitChunk, chunk_count);
  uint64             base        = list->last ? list->last->base : 0;

  for (uint64 i = 0; i+1 < chunk_count; i += 1, item_count -= count_per_chunk, base += count_per_chunk) {
    chunks[i].base  = base;
    ...
    for (uint64 k = 0; k < count_per_chunk; ++k) {
...
internal void
rdib_unit_chunk_list_reserve(Arena *arena, RDIB_UnitChunkList *list, uint64 cap)
{
  // fill out node
...
internal void
rdib_type_chunk_list_reserve(Arena *arena, RDIB_TypeChunkList *list, uint64 cap)
{
  // fill out node
...
internal void
rdib_source_file_list_reserve(Arena *arena, RDIB_SourceFileChunkList *list, uint64 cap)
{
  // fill out node
...
internal void
rdib_line_table_chunk_list_concat_in_place_many(RDIB_LineTableChunkList *list, RDIB_LineTableChunkList *to_concat, uint64 count)
{
  SLLConcatInPlaceChunkListArray(list, to_concat, RDIB_LineTableChunk, count);
}

internal void
rdib_scope_chunk_list_concat_in_place_many(RDIB_ScopeChunkList *list, RDIB_ScopeChunkList *to_concat, uint64 count)
{
  SLLConcatInPlaceChunkListArray(list, to_concat, RDIB_ScopeChunk, count);
}

internal void
rdib_variable_chunk_list_concat_in_place_many(RDIB_VariableChunkList *list, RDIB_VariableChunkList *to_concat, uint64 count)
{
  SLLConcatInPlaceChunkListArray(list, to_concat, RDIB_VariableChunk, count);
}

internal void
rdib_procedure_chunk_list_concat_in_place_many(RDIB_ProcedureChunkList *list, RDIB_ProcedureChunkList *to_concat, uint64 count)
{
  SLLConcatInPlaceChunkListArray(list, to_concat, RDIB_ProcedureChunk, count);
}

internal void
rdib_inline_site_chunk_list_concat_in_place_many(RDIB_InlineSiteChunkList *list, RDIB_InlineSiteChunkList *to_concat, uint64 count)
{
  SLLConcatInPlaceChunkListArray(list, to_concat, RDIB_InlineSiteChunk, count);
}

internal void
rdib_type_chunk_list_concat_in_place_many(RDIB_TypeChunkList *list, RDIB_TypeChunkList *to_concat, uint64 count)
{
  SLLConcatInPlaceChunkListArray(list, to_concat, RDIB_TypeChunk, count);
}

internal void
rdib_udt_member_chunk_list_concat_in_place_many(RDIB_UDTMemberChunkList *list, RDIB_UDTMemberChunkList *to_concat, uint64 count)
{
...
rdib_bytecode_push_op(Arena *arena, RDIB_EvalBytecode *bytecode, RDI_EvalOp op, RDI_uint64 p)
{
...
internal void
rdib_bytecode_push_ucsont(Arena *arena, RDIB_EvalBytecode *bytecode, RDI_uint64 uconst)
{
  if (uconst <= max_uint8) {
    rdib_bytecode_push_op(arena, bytecode, RDI_EvalOp_Constuint8, uconst);
  } else if (uconst <= max_uint16) {
    rdib_bytecode_push_op(arena, bytecode, RDI_EvalOp_Constuint16, uconst);
  } else if (uconst <= max_uint32) {
    rdib_bytecode_push_op(arena, bytecode, RDI_EvalOp_Constuint32, uconst);
  } else {
    rdib_bytecode_push_op(arena, bytecode, RDI_EvalOp_Constuint64, uconst);
  }
}

internal void
rdib_bytecode_push_sconst(Arena *arena, RDIB_EvalBytecode *bytecode, RDI_uint64 sconst)
{
  if (min_uint8 <= sconst && sconst <= max_uint8) {
    rdib_bytecode_push_op(arena, bytecode, RDI_EvalOp_Constuint8, (RDI_uint64)sconst);
    rdib_bytecode_push_op(arena, bytecode, RDI_EvalOp_TruncSigned, 8);
  } else if (min_uint16 <= sconst && sconst <= max_uint16) {
    rdib_bytecode_push_op(arena, bytecode, RDI_EvalOp_Constuint16, (RDI_uint64)sconst);
    rdib_bytecode_push_op(arena, bytecode, RDI_EvalOp_TruncSigned, 16);
  } else if (min_uint32 <= sconst && sconst <= max_uint32) {
    rdib_bytecode_push_op(arena, bytecode, RDI_EvalOp_Constuint32, (RDI_uint64)sconst);
  } else {
    rdib_bytecode_push_op(arena, bytecode, RDI_EvalOp_Constuint64, (RDI_uint64)sconst);
  }
}
...
rdib_make_location_addr_byte_stream(Rng1uint64List ranges, RDIB_EvalBytecode bytecode)
{
  RDIB_Location loc = {0};
  loc.ranges        = ranges;
...
internal RDIB_Location
rdib_make_location_addr_bytecode_stream(Rng1uint64List ranges, RDIB_EvalBytecode bytecode)
{
  RDIB_Location loc = {0};
  loc.ranges        = ranges;
  loc.kind          = RDI_LocationKind_AddrBytecodeStream;
...
internal RDIB_Location
rdib_make_location_val_bytecode_stream(Rng1uint64List ranges, RDIB_EvalBytecode bytecode)
{
  RDIB_Location loc = {0};
  loc.ranges        = ranges;
...
internal RDIB_Location
rdib_make_location_addr_reg_plus_u16(Rng1uint64List ranges, RDI_RegCode reg_code, RDI_uint16 offset)
{
  RDIB_Location loc = {0};
  loc.ranges        = ranges;
  loc.kind          = RDI_LocationKind_AddrRegPlusuint16;
...
internal RDIB_Location
rdib_make_location_addr_addr_reg_plus_u16(Rng1uint64List ranges, RDI_RegCode reg_code, RDI_uint16 offset)
{
  RDIB_Location loc = {0};
  loc.kind          = RDI_LocationKind_AddrAddrRegPlusuint16;
  loc.ranges        = ranges;
  loc.reg_code      = reg_code;
  loc.offset        = offset;
  return loc;
...
internal RDIB_Location
rdib_make_location_val_reg(Rng1uint64List ranges, RDI_RegCode reg_code)
{
  RDIB_Location loc = {0};
...
internal RDIB_LocationNode *
rdib_push_location_addr_reg_off(Arena *arena, RDIB_LocationList *list, RDI_Arch arch, RDI_RegCode reg_code, uint32 reg_byte_size, uint32 reg_byte_pos, uint64 offset, B32 is_reference, Rng1uint64List ranges)
{
  RDIB_Location loc;

  if (0 <= offset && offset <= (uint64)max_uint16) {
    if (is_reference) {
      loc = rdib_make_location_addr_addr_reg_plus_u16(ranges, reg_code, (uint16)offset);
    } else {
      loc = rdib_make_location_addr_reg_plus_u16(ranges, reg_code, (uint16)offset);
    }
  }
  ...
  // long offset, emit byte code
    ...
    uint32 reg_read_param = RDI_EncodeRegReadParam(reg_code, reg_byte_size, reg_byte_pos);
    ...
    if (is_reference) {
      uint64 addr_size = rdi_addr_size_from_arch(arch);
...
rdib_size_from_type(RDIB_Type *type)
{
  if (type) {
    ...
    case RDI_TypeKind_uint8:
    case RDI_TypeKind_uint16:
    case RDI_TypeKind_uint32:
    case RDI_TypeKind_uint64:
    case RDI_TypeKind_uint128:
    ...
    case RDI_TypeKind_float:
    case RDI_TypeKind_floatPP:
    case RDI_TypeKind_F48:
    case RDI_TypeKind_double:
    case RDI_TypeKind_F80:
    case RDI_TypeKind_F128:
    case RDI_TypeKind_Complexfloat:
    case RDI_TypeKind_Complexdouble:
  ...
  for (RDIB_TypeChunk *chunk = list->first; chunk != 0; chunk = chunk->next) {
    for (uint64 i = 0; i < chunk->count; ++i) {
      ...
      if (type->kind == RDI_TypeKind_Struct || type->kind == RDI_TypeKind_Class ||
        ...
        for (uint64 i = 0; i < type->params.count; ++i) {
...
THREAD_POOL_TASK_FUNC(rdib_type_stats_task)
{
  ...
  for (uint64 itype = 0; itype < chunk->count; ++itype) {
...
THREAD_POOL_TASK_FUNC(rdib_concat_members_task)
{
  ...
  for (uint64 chunk_idx = task->ranges[task_id].min; chunk_idx < task->ranges[task_id].max; ++chunk_idx) {
...
THREAD_POOL_TASK_FUNC(rdib_assign_head_member_indices_task)
{
  ProfBeginFunction();
  RDIB_MembersTask *task = raw_task;
  uint64 cursor = task->offsets[task_id];
  for (uint64 chunk_idx = task->ranges[task_id].min; chunk_idx < task->ranges[task_id].max; ++chunk_idx) {
    RDIB_TypeChunk *chunk = task->type_chunks[chunk_idx];
    for (uint64 i = 0; i < chunk->count; ++i) {
      RDIB_Type *type = &chunk->v[i];
...
THREAD_POOL_TASK_FUNC(rdib_fill_udt_members_task)
{
  ...
  for (uint64 i = 0; i < chunk->count; ++i) {
    ...
    for (RDIB_UDTMember *src = type->members.list.first; src != 0; src = src->next, ++member_idx) {
      uint64        idx  = type->members.first_member_idx + member_idx;
...
THREAD_POOL_TASK_FUNC(rdib_fill_udts_task)
{
  ...
  uint64             ichunk     = task_id;
  RDIB_TypeChunk *chunk      = task->type_chunks[ichunk];
  uint64             udt_cursor = task->udt_base_idx[ichunk];
  uint64             udt_cap    = task->type_stats.udt_counts[ichunk];
...
THREAD_POOL_TASK_FUNC(rdib_type_nodes_task)
{
  ...
  for (uint64 itype = 0; itype < chunk->count; ++itype) {
    ...
    uint64 dst_idx = rdib_idx_from_type(src);
    ...
    if (src->kind == RDI_TypeKind_NULL) {
      ...
      uint64 entry_size = rdib_size_from_type(src->array.entry_type);
      uint64 array_size = src->array.size;
      uint64 array_count = entry_size > 0 ? array_size / entry_size : 0;
                              ...
                              RDIB_TypeStats         type_stats)
{
  ...
  RDI_Member *udt_members_rdi;
  {
    ...
    ProfBegin("Count");
    task.counts = push_array(scratch.arena, uint64, tp->worker_count);
  ...
  uint64 total_udt_count = sum_array_u64(type_chunk_count, type_stats.udt_counts);
  ProfEnd();
...
rdib_path_tree_init(Arena *arena, uint64 list_count)
{
...
rdib_path_tree_insert(Arena *arena, RDIB_PathTree *tree, String8 path, RDIB_SourceFile *src_file)
{
  ...
  for (String8Node *n = sub_paths.first; n != 0; n = n->next) {
    ...
    if (sub_child == 0) {
      ...
      uint64 list_idx = tree->next_list_idx % tree->list_count;
...
rdib_init_string_map(Arena *arena, uint64 cap)
{
  RDIB_StringMap *string_map = push_array(arena, RDIB_StringMap, 1);
  string_map->cap            = (uint64)((double)cap * 1.3);
...
rdib_string_map_insert_or_update(RDIB_StringMapBucket **buckets, uint64 cap, uint64 hash, RDIB_StringMapBucket *new_bucket, RDIB_StringMapUpdateFunc *update_func)
{
...
THREAD_POOL_TASK_FUNC(rdib_get_extant_buckets_string_map_task)
{
  ProfBeginFunction();
  RDIB_GetExtantBucketsStringMapTask *task = raw_task;
  for (uint64 bucket_idx = task->ranges[task_id].min, cursor = task->offsets[task_id]; bucket_idx < task->ranges[task_id].max; ++bucket_idx) {
...
internal RDIB_StringMapBucket **
rdib_extant_buckets_from_string_map(TP_Context *tp, Arena *arena, RDIB_StringMap *string_map, uint64 *bucket_count_out)
{
  ProfBeginFunction();
  Temp scratch = scratch_begin(&arena, 1);
...
THREAD_POOL_TASK_FUNC(rdib_string_map_radix_sort_chunk_idx_task)
{
  ...
  for (uint64 bucket_idx = task->ranges[task_id].min; bucket_idx < task->ranges[task_id].max; ++bucket_idx) {
    RDIB_StringMapBucket *bucket    = task->src[bucket_idx];
    uint32                   chunk_idx = bucket->sorter.hi;
    uint32                   dst_idx   = ins_atomic_u32_inc_eval(&task->chunk_offsets[chunk_idx]) - 1;
...
THREAD_POOL_TASK_FUNC(rdib_string_map_radix_sort_element_idx_task)
{
  ...
  for (uint64 chunk_idx = task->ranges[task_id].min; chunk_idx < task->ranges[task_id].max; ++chunk_idx) {
    uint64 range_lo = task->chunk_offsets[chunk_idx];
    uint64 range_hi = task->chunk_offsets[chunk_idx] + task->chunk_histo[chunk_idx];

    ProfBegin("Zero out Histogram");
    uint32 histo_bot[1 << 10]; MemoryZeroArray(histo_bot);
    uint32 histo_mid[1 << 11]; MemoryZeroArray(histo_mid);
    uint32 histo_top[1 << 11]; MemoryZeroArray(histo_top);
    ProfEnd();

    ProfBegin("Element Histogram");
    for (uint64 i = range_lo; i < range_hi; ++i) {
      RDIB_StringMapBucket *elem = task->dst[i];
      uint32 elem_idx  = elem->sorter.lo;
      uint32 digit_bot = (elem_idx >>  0) % ArrayCount(histo_bot);
      uint32 digit_mid = (elem_idx >> 10) % ArrayCount(histo_mid);
      uint32 digit_top = (elem_idx >> 21) % ArrayCount(histo_top);
    ...
    for (uint64 i = range_lo; i < range_hi; ++i) {
      RDIB_StringMapBucket *elem     = task->dst[i];
      uint32                   elem_idx = elem->sorter.lo;
      uint32                   digit    = (elem_idx >> 0) % ArrayCount(histo_bot);
      uint32                   src_idx  = range_lo + histo_bot[digit]++;
    ...
    for (uint64 i = range_lo; i < range_hi; ++i) {
      ...
      uint32                   elem_idx = elem->sorter.lo;
      uint32                   digit    = (elem_idx >> 10) % ArrayCount(histo_mid);
      uint32                   dst_idx  = range_lo + histo_mid[digit]++;
    ...
    for (uint64 i = range_lo; i < range_hi; ++i) {
      RDIB_StringMapBucket *elem     = task->dst[i];
      uint32                   elem_idx = elem->sorter.lo;
      uint32                   digit    = (elem_idx >> 21) % ArrayCount(histo_top);
      uint32                   src_idx  = range_lo + histo_top[digit]++;
      task->src[src_idx] = elem;
    }
    ProfEnd();
  }
...
internal void
rdib_string_map_sort_buckets(TP_Context *tp, RDIB_StringMapBucket **buckets, uint64 bucket_count, uint64 chunk_idx_opl)
{
  ProfBeginFunction();
  Temp scratch = scratch_begin(0,0);
  ...
  task.chunk_histo = push_array(scratch.arena, uint32, chunk_idx_opl);
...
#if 0
  for (uint64 i = 1; i < bucket_count; ++i) {
    RDIB_StringMapBucket *prev = buckets[i - 1];
    RDIB_StringMapBucket *curr = buckets[i + 0];
    uint32 prev_chunk_idx = prev->sorter.hi;
    uint32 curr_chunk_idx = curr->sorter.hi;
...
#if 0
  {
    for (uint64 i = 1; i < bucket_count; ++i) {
      ...
      if (prev_chunk_idx == curr_chunk_idx) {
        uint32 prev_elem_idx = prev->sorter.lo;
        uint32 curr_elem_idx = curr->sorter.lo;
...
internal void
rdib_string_map_assign_indices(RDIB_StringMapBucket **buckets, uint64 bucket_count)
{
  ProfBeginFunction();
  for (uint64 idx = 0; idx < bucket_count; ++idx) {
    buckets[idx]->idx = idx;
  }
  ProfEnd();
}
...
THREAD_POOL_TASK_FUNC(rdib_collect_strings_sects_task)
{
  ...
  for (uint64 sect_idx = task->ranges[task_id].min; sect_idx < task->ranges[task_id].max; ++sect_idx) {
...
THREAD_POOL_TASK_FUNC(rdib_name_map_var_task)
{
  ...
  for (uint64 chunk_idx = task->ranges[task_id].min; chunk_idx < task->ranges[task_id].max; ++chunk_idx) {
    ...
    for (uint64 var_idx = 0; var_idx < chunk->count; ++var_idx) {
...
THREAD_POOL_TASK_FUNC(rdib_name_map_procedure_task)
{
  ...
  for (uint64 chunk_idx = task->ranges[task_id].min; chunk_idx < task->ranges[task_id].max; ++chunk_idx) {
    RDIB_ProcedureChunk *chunk = task->procs[chunk_idx];
    VoidNode *nodes = push_array(arena, VoidNode, chunk->count);
    for (uint64 proc_idx = 0; proc_idx < chunk->count; ++proc_idx) {
...
THREAD_POOL_TASK_FUNC(rdib_name_map_types_task)
{
  ...
  for (uint64 chunk_idx = task->ranges[task_id].min; chunk_idx < task->ranges[task_id].max; ++chunk_idx) {
    ...
    for (uint64 type_idx = 0; type_idx < chunk->count; ++type_idx) {
...
THREAD_POOL_TASK_FUNC(rdib_name_map_normal_paths_task)
{
  ...
  for (uint64 chunk_idx = task->ranges[task_id].min; chunk_idx < task->ranges[task_id].max; ++chunk_idx) {
    ...
    VoidNode *nodes             = push_array(arena, VoidNode, chunk->count);
    VoidNode *node_cursor       = nodes;
    for (uint64 i = 0; i < chunk->count; ++i, ++node_cursor) {
...
internal uint64
rdib_index_run_hash(uint32 count, uint32 *idxs)
{
...
rdib_init_index_run_map(Arena *arena, uint64 cap)
{
  ProfBeginFunction();
...
rdib_index_run_map_insert_or_update(Arena *arena, RDIB_IndexRunBucket **buckets, uint64 cap, uint64 hash, RDIB_IndexRunBucket *new_bucket, uint64 *bucket_idx_out)
{
  B32 was_bucket_inserted_or_updated = 0;
...
internal uint32
rdib_idx_run_from_bucket_idx(RDIB_IndexRunMap *map, uint64 bucket_idx)
{
  RDIB_IndexRunBucket *bucket = map->buckets[bucket_idx];
  uint32 idx_run32 = safe_cast_u32(bucket->index_in_output_array);
  return idx_run32;
}
...
internal RDIB_IndexRunBucket **
rdib_extant_buckets_from_index_run_map(TP_Context *tp, Arena *arena, RDIB_IndexRunMap *idx_run_map, uint64 *bucket_count_out)
{
  ProfBeginFunction();
  Temp scratch = scratch_begin(&arena, 1);
...
internal void
rdib_index_run_map_sort_buckets(TP_Context *tp, RDIB_IndexRunBucket **buckets, uint64 bucket_count, uint64 chunk_idx_opl)
{
  ProfBeginFunction();
  Temp scratch = scratch_begin(0,0);
  ...
  for (uint64 i = 1; i < bucket_count; ++i) {
    RDIB_StringMapBucket *prev = buckets[i - 1];
    RDIB_StringMapBucket *curr = buckets[i + 0];
    uint32 prev_chunk_idx = RDIB_StringMap_ChunkIdx32FromSorter(prev->sorter);
    uint32 curr_chunk_idx = RDIB_StringMap_ChunkIdx32FromSorter(curr->sorter);
    AssertAlways(prev_chunk_idx <= curr_chunk_idx);
  }
#endif
...
internal void
rdib_index_run_map_assign_indices(RDIB_IndexRunBucket **buckets, uint64 bucket_count)
{
  ProfBeginFunction();
  for (uint64 bucket_idx = 0, cursor = 0; bucket_idx < bucket_count; ++bucket_idx) {
...
internal uint64
rdib_index_run_map_insert_item(Arena *arena, RDIB_BuildIndexRunsTask *task, uint64 worker_id, uint64 item_idx, uint64 count, uint32 *idxs)
{
  Assert(item_idx < max_uint32);
  ...
  bucket->sorter.v      = task->sorter_idx << 32 | (uint32)item_idx;

  // insert bucket
  uint64                  hash       = rdib_index_run_hash(count, idxs);
  uint64                  bucket_idx = max_uint64;
...
THREAD_POOL_TASK_FUNC(rdib_build_idx_runs_params_task)
{
  ...
  for (RDIB_Type *type = &chunk->v[0], *opl = chunk->v + chunk->count; type < opl; ++type) {
    if (type->kind == RDI_TypeKind_Function) {
      ...
      // pack params
      uint64  type_index_count = params->params.count;
      uint32 *type_indices     = push_array_no_zero(arena, uint32, type_index_count);
      for (uint64 param_idx = 0; param_idx < params->params.count; ++param_idx) {
        type_indices[param_idx] = rdib_idx_from_type(params->params.types[param_idx]);
      }

      // insert type indices
      uint32 func_type_idx = rdib_idx_from_type(type);
      ...
      uint64  type_index_count = params->params.count + 1;
      uint32 *type_indices     = push_array_no_zero(arena, uint32, type_index_count);
      uint64  type_idx_cursor  = 0;
      ...
      // insert type indices
      uint32 method_type_idx = rdib_idx_from_type(type);
      ...
      // insert type indices
      uint32 static_method_type_idx = rdib_idx_from_type(type);
...
rdib_idx_from_name_map_void_node(RDIB_BuildIndexRunsTask *task, VoidNode *node)
{
  ...
  uint32 idx32 = safe_cast_u32(idx);
  return idx32;
}
...
THREAD_POOL_TASK_FUNC(rdib_build_idx_runs_name_map_buckets_task)
{
  ...
  for (uint64 bucket_idx = task->ranges[task_id].min; bucket_idx < task->ranges[task_id].max; ++bucket_idx) {
    ...
    uint64                   count  = void_list_count_nodes(bucket->raw_values);

    if (count > 1) {
      // build array of indices that point to name map respective arrays
      uint32 *idxs = push_array_no_zero(arena, uint32, count);
      {
        uint64 curr_idx = 0;
      ...
      uint64 idx_run_bucket_idx = rdib_index_run_map_insert_item(arena, task, worker_id, bucket_idx, count, idxs); // TODO: fix `idx` leak when we insert same runs
      
      // fill out bucket
      bucket->count              = count;
      bucket->idx_run_bucket_idx = idx_run_bucket_idx;
    } if (count == 1) {
      uint32 match_idx = rdib_idx_from_name_map_void_node(task, bucket->raw_values->v);
...
rdib_idx_from_params(RDIB_IndexRunMap *map, RDIB_Type *params)
{
  ...
  uint32 idx = params->params.idx_run_bucket->index_in_output_array;
  return idx;
}
#endif
...
internal void
rdib_data_sections_from_binary_sections(Arena *arena, RDIB_DataSectionList *sect_list, RDIB_StringMap *string_map, RDIB_BinarySection *binary_sects, uint64 binary_sects_count)
{
  ProfBeginFunction();

  RDI_BinarySection *dst_arr = push_array(arena, RDI_BinarySection, binary_sects_count);

  for (uint64 sect_idx = 0; sect_idx < binary_sects_count; ++sect_idx) {
...
rdib_data_sections_from_units(Arena                *arena,
                              ...
                              uint64                   total_unit_count,
                              uint64                   unit_chunk_count,
                              RDIB_UnitChunk      **unit_chunks)
{
  ProfBeginFunction();

  RDI_Unit *dst_arr = push_array(arena, RDI_Unit, total_unit_count);
  for (uint64 chunk_idx = 0; chunk_idx < unit_chunk_count; chunk_idx += 1) {
    RDIB_UnitChunk *chunk = unit_chunks[chunk_idx];
    for (uint64 i = 0; i < chunk->count; i += 1) {
      RDIB_Unit *src     = &chunk->v[i];
      uint64       unit_idx = rdib_idx_from_unit(src);
...
THREAD_POOL_TASK_FUNC(rdib_fill_vmap_entries_unit_task)
{
  ProfBeginFunction();
  RDIB_VMapBuilderTask *task            = raw_task;
  uint64                  range_cursor     = task->offsets[task_id];
  uint64                  range_cursor_opl = task->offsets[task_id] + task->counts[task_id];

  for (uint64 chunk_idx = task->ranges[task_id].min; chunk_idx < task->ranges[task_id].max; ++chunk_idx) {
    RDIB_UnitChunk *chunk = task->unit_chunks[chunk_idx];
    for (uint64 i = 0; i < chunk->count; ++i) {
      RDIB_Unit *unit = &chunk->v[i];
      for (Rng1uint64 *range_ptr = unit->virt_ranges, *range_opl = unit->virt_ranges + unit->virt_range_count;
...
THREAD_POOL_TASK_FUNC(rdib_fill_vmap_entries_gvar_task)
{
  ...
  for (uint64 chunk_idx = task->ranges[task_id].min; chunk_idx < task->ranges[task_id].max; ++chunk_idx) {
    ...
    for (uint64 var_idx = 0; var_idx < chunk->count; ++var_idx) {
      ...
      for (RDIB_LocationNode *loc_n = var->locations.first; loc_n != 0; loc_n = loc_n->next) {
        for (Rng1uint64Node *range_n = loc_n->v.ranges.first; range_n != 0; range_n = range_n->next) {
          Assert(range_cursor < range_cursor_opl);
          Assert(range_n->v.min <= range_n->v.max);
          uint64 size = range_n->v.max - range_n->v.min;
...
THREAD_POOL_TASK_FUNC(rdib_fill_vmap_entries_scope_task)
{
  ...
  for (uint64 chunk_idx = task->ranges[task_id].min; chunk_idx < task->ranges[task_id].max; ++chunk_idx) {
    RDIB_ScopeChunk *chunk = task->scope_chunks[chunk_idx];
    for (uint64 scope_idx = 0; scope_idx < chunk->count; ++scope_idx) {
      RDIB_Scope *scope = &chunk->v[scope_idx];
      for (Rng1uint64Node *range_n = scope->ranges.first; range_n != 0; range_n = range_n->next) {
        Assert(range_cursor  < range_cursor_opl);
        Assert(range_n->v.min <= range_n->v.max);
...
internal void
rdib_sort_procs_radix_32(RDIB_Procedure **v, uint64 count)
{
  ProfBeginFunction();
  Temp scratch = scratch_begin(0,0);
  ...
  uint32 count_8lo[256];    MemoryZeroArray(count_8lo);
  uint32 count_8hi[256];    MemoryZeroArray(count_8hi);
  uint32 count_16[1 << 16]; MemoryZeroArray(count_16);
  ProfEnd();
  ...
  for (uint64 i = 0; i < count; i += 1) {
    ...
    uint64 digit_8lo  = (p->scope->ranges.first->v.min >> 0)  % ArrayCount(count_8lo);
    uint64 digit_8hi  = (p->scope->ranges.first->v.min >> 8)  % ArrayCount(count_8hi);
    uint64 digit_16   = (p->scope->ranges.first->v.min >> 16) % ArrayCount(count_16);
  ...
  for (uint64 i = 0; i < count; i += 1) {
    RDIB_Procedure *p = src[i];
    uint64 digit = (p->scope->ranges.first->v.min >> 0) % ArrayCount(count_8lo);
  ...
  for (uint64 i = 0; i < count; i += 1) {
    ...
    uint64 digit = (p->scope->ranges.first->v.min >> 8) % ArrayCount(count_8hi);
  ...
  for (uint64 i = 0; i < count; i += 1) {
    RDIB_Procedure *p = src[i];
    uint64 digit = (p->scope->ranges.first->v.min >> 16) % ArrayCount(count_16);
...
rdib_data_from_vmap(Arena *arena, uint64 range_count, RDIB_VMapRange *ranges)
{
  ProfBeginFunction();
  Temp scratch = scratch_begin(&arena, 1);

  const uint64 size_bit_count0 = 8;
  const uint64 size_bit_count1 = 8;
  const uint64 size_bit_count2 = 16;

  const uint64 voff_bit_count0 = 11;
  const uint64 voff_bit_count1 = 11;
  const uint64 voff_bit_count2 = 10;

  ProfBegin("Push shared buffer");
  uint64 radix_memory_size = sizeof(RDIB_VMapRange) * range_count +
                          sizeof(uint32) * ((1 << size_bit_count0) + (1 << size_bit_count1) + (1 << size_bit_count2)) +
                          sizeof(uint32) * ((1 << voff_bit_count0) + (1 << voff_bit_count1) + (1 << voff_bit_count2));
  uint8 *radix_memory      = push_array_no_zero(arena, uint8, radix_memory_size);
  ProfEnd();
  ...
  ProfBegin("Sort");
  {
    ...
    uint32 *size_count0 = (uint32 *)(dst + range_count);
    uint32 *size_count1 = size_count0 + (1 << size_bit_count0);
    uint32 *size_count2 = size_count1 + (1 << size_bit_count1);

    uint32 *voff_count0 = size_count2 + (1 << size_bit_count2);
    uint32 *voff_count1 = voff_count0 + (1 << voff_bit_count0);
    uint32 *voff_count2 = voff_count1 + (1 << voff_bit_count1);
    ...
    for (uint64 i = 0; i < range_count; ++i) {
      RDIB_VMapRange *r = src+i;

      uint32 size_digit0 = (-r->size >> 0)                                   % (1 << size_bit_count0);
      uint32 size_digit1 = (-r->size >> size_bit_count0)                     % (1 << size_bit_count1);
      uint32 size_digit2 = (-r->size >> (size_bit_count0 + size_bit_count1)) % (1 << size_bit_count2);

      uint64 voff_digit0 = (r->voff >> 0)                                   % (1 << voff_bit_count0);
      uint64 voff_digit1 = (r->voff >> voff_bit_count0)                     % (1 << voff_bit_count1);
      uint64 voff_digit2 = (r->voff >> (voff_bit_count0 + voff_bit_count1)) % (1 << voff_bit_count2);
    ...
    for (uint64 i = 0; i < range_count; ++i) {
      RDIB_VMapRange r = src[i];
      uint32 digit = (-r.size >> 0) % (1 << size_bit_count0);
      dst[size_count0[digit]++] = r;
    }

    for (uint64 i = 0; i < range_count; ++i) {
      RDIB_VMapRange r = dst[i];
      uint32 digit = (-r.size >> size_bit_count0) % (1 << size_bit_count1);
      src[size_count1[digit]++] = r;
    }

    for (uint64 i = 0; i < range_count; ++i) {
      RDIB_VMapRange r = src[i];
      uint32 digit = (-r.size >> (size_bit_count0 + size_bit_count1)) % (1 << size_bit_count2);
    ...
    for (uint64 i = 0; i < range_count; ++i) {
      RDIB_VMapRange r = dst[i];
      uint32 digit = (r.voff >> 0) % (1 << voff_bit_count0);
      src[voff_count0[digit]++] = r;
    }

    for (uint64 i = 0; i < range_count; ++i) {
      RDIB_VMapRange r = src[i];
      uint32 digit = (r.voff >> voff_bit_count0) % (1 << voff_bit_count1);
      dst[voff_count1[digit]++] = r;
    }

    for (uint64 i = 0; i < range_count; ++i) {
      RDIB_VMapRange r = dst[i];
      uint32 digit = (r.voff >> (voff_bit_count0 + voff_bit_count1)) % (1 << voff_bit_count2);
  ...
  {
    uint64            default_vme_cap = 4096;
    uint64            vme_block_cap   = radix_memory_size / sizeof(RDI_VMapEntry);
    uint64            vme_block_size  = 0;
...
#define push_vme() (vme_block_size < raw_vmap.last->string.size/sizeof(vme_block[0])) ? &vme_block[vme_block_size++] :    \
    ...
    for (uint64 range_idx = 1; range_idx < range_count; ++range_idx) {
...
rdib_data_sections_from_unit_gvar_scope_vmaps(TP_Context           *tp,
                                              TP_Arena             *arena,
                                              RDIB_DataSectionList *sect_list,
                                              uint64 unit_chunk_count,  RDIB_UnitChunk     **unit_chunks,
                                              uint64 gvar_chunk_count,  RDIB_VariableChunk **gvar_chunks,
                                              uint64 scope_chunk_count, RDIB_ScopeChunk    **scope_chunks)
{
  ProfBeginFunction();
...
internal void
rdib_data_sections_from_string_map(TP_Context *tp, Arena *arena, RDIB_DataSectionList *sect_list, RDIB_StringMapBucket **buckets, uint64 bucket_count)
{
  ProfBeginFunction();
  Temp scratch = scratch_begin(&arena, 1);
  ...
  task.string_data      = push_array_no_zero(arena, uint8, task.string_data_size);
...
internal void
rdib_data_sections_from_index_runs(TP_Context *tp, Arena *arena, RDIB_DataSectionList *sect_list, RDIB_IndexRunBucket **buckets, uint64 bucket_count)
{
  ProfBeginFunction();
  Temp scratch = scratch_begin(&arena, 1);

  ProfBegin("Count Indices");
  uint64 total_index_count = 0;
  for (uint64 bucket_idx = 0; bucket_idx < bucket_count; ++bucket_idx) {
    total_index_count += buckets[bucket_idx]->indices.count;
  }
  ProfEnd();

  uint32 *output_array = push_array_no_zero(arena, uint32, total_index_count);
                     ...
                     RDIB_SourceFileChunk **src_file_chunks)
{
  ...
  ProfBegin("Units");
  for (uint64 ichunk = 0; ichunk < unit_chunk_count; ++ichunk) {
    RDIB_UnitChunk *chunk = unit_chunks[ichunk];
    for (uint64 iunit = 0; iunit < chunk->count; ++iunit) {
  ...
  for (uint64 chunk_idx = 0; chunk_idx < src_file_chunk_count; ++chunk_idx) {
...
THREAD_POOL_TASK_FUNC(rdib_count_scopes_task)
{
  ...
  for (uint64 chunk_idx = task->ranges[task_id].min; chunk_idx < task->ranges[task_id].max; ++chunk_idx) {
    ...
    for (uint64 scope_i = 0; scope_i < chunk->count; ++scope_i) {
      ...
      for (RDIB_Variable *var = scope->local_first; var != 0; var = var->next) {
        for (RDIB_LocationNode *loc_n = var->locations.first; loc_n != 0; loc_n = loc_n->next) {
          ...
          case RDI_LocationKind_AddrRegPlusuint16:
          case RDI_LocationKind_AddrAddrRegPlusuint16: {
            task->loc_data_sizes[task_id] += sizeof(RDI_LocationRegPlusuint16);
          } break;
          default: InvalidPath;
          }
...
THREAD_POOL_TASK_FUNC(rdib_build_scopes_task)
{
  ...
  uint64  scope_voff_cursor = task->scope_voff_offsets[task_id];
  uint64  scope_voff_max    = task->scope_voff_offsets[task_id] + task->scope_voff_counts[task_id];
  uint64 *scope_voff_ptr    = task->scope_voffs_rdi;
 
  // local fill info
  uint64 local_cursor  = task->local_offsets[task_id];
  uint64 local_max     = task->local_offsets[task_id] + task->local_counts[task_id];
  RDI_Local *locals = task->locals_rdi;

  // location data fill info
  uint64  loc_data_max    = task->loc_data_offsets[task_id] + task->loc_data_sizes[task_id];
  uint64  loc_data_cursor = task->loc_data_offsets[task_id];
  uint8  *loc_data        = task->loc_data_rdi;

  // location block fill info
  uint64                loc_block_cursor = task->loc_block_offsets[task_id];
  uint64                loc_block_max    = task->loc_block_offsets[task_id] + task->loc_block_counts[task_id];
  RDI_LocationBlock *loc_blocks       = task->loc_blocks_rdi;

  for (uint64 ichunk = task->ranges[task_id].min; ichunk < task->ranges[task_id].max; ++ichunk) {
    RDIB_ScopeChunk *chunk = task->scopes_rdib[ichunk];
    for (uint64 iscope = 0; iscope < chunk->count; ++iscope) {
      RDIB_Scope *scope_src = &chunk->v[iscope];
      uint64         scope_idx = rdib_idx_from_scope(scope_src);
      ...
      for (Rng1uint64Node *range_n = scope_src->ranges.first; range_n != 0; range_n = range_n->next) {
      ...
      for (RDIB_Variable *local_src = scope_src->local_first; local_src != 0; local_src = local_src->next, ++local_cursor) {
        uint64 loc_block_first = loc_block_cursor;
        ...
        for (RDIB_LocationNode *loc_n = local_src->locations.first; loc_n != 0; loc_n = loc_n->next) {
          ...
          uint64 location_data_off = loc_data_cursor;
          switch (loc->kind) {
          case RDI_LocationKind_NULL: break;
          case RDI_LocationKind_AddrBytecodeStream:
          case RDI_LocationKind_ValBytecodeStream: {
          ...
          case RDI_LocationKind_AddrRegPlusuint16:
          case RDI_LocationKind_AddrAddrRegPlusuint16: {
            Assert(loc_data_cursor + sizeof(RDI_LocationRegPlusuint16) <= loc_data_max);
            RDI_LocationRegPlusuint16 *dst = (RDI_LocationRegPlusuint16 *) (loc_data + loc_data_cursor);
            dst->kind                   = loc->kind;
          ...
          uint64 align_size = AlignPadPow2(loc_data_cursor, 8);
          ...
          for (Rng1uint64Node *range_n = loc->ranges.first; range_n != 0; range_n = range_n->next, ++loc_block_cursor) {
                               ...
                               RDIB_ScopeChunk      **scopes)
{
  ...
  task.scope_voff_counts = push_array(scratch.arena, uint64, tp->worker_count);
  task.local_counts      = push_array(scratch.arena, uint64, tp->worker_count);
  task.loc_block_counts  = push_array(scratch.arena, uint64, tp->worker_count);
  task.loc_data_sizes    = push_array(scratch.arena, uint64, tp->worker_count);
  tp_for_parallel(tp, 0, tp->worker_count, rdib_count_scopes_task, &task);
  ProfEnd();

  uint64 total_scope_voff_count = sum_array_u64(tp->worker_count, task.scope_voff_counts);
  uint64 total_local_count      = sum_array_u64(tp->worker_count, task.local_counts     );
  uint64 total_loc_block_count  = sum_array_u64(tp->worker_count, task.loc_block_counts );
  uint64 total_loc_data_size    = sum_array_u64(tp->worker_count, task.loc_data_sizes   );
  ...
  ProfBegin("Push");
  task.scope_voffs_rdi = push_array_no_zero(arena->v[0], uint64,               total_scope_voff_count);
  ...
  task.loc_data_rdi    = push_array_no_zero(arena->v[0], uint8,                total_loc_data_size   );
  ProfEnd();
...
THREAD_POOL_TASK_FUNC(rdib_build_name_map_task)
{
  ...
  uint64 out_node_count   = task->in_bucket_counts[name_map_idx];
  uint64 load_factor      = 4;
  uint64 out_bucket_count = CeilIntegerDiv(out_node_count, load_factor);
  ...
  for (uint64 bucket_idx = 0, node_cursor = 0; bucket_idx < out_bucket_count; ++bucket_idx) {
...
rdib_data_sections_from_name_maps(TP_Context            *tp,
                                  ...
                                  uint64                    src_name_map_counts[RDI_NameMapKind_COUNT])
{
  ProfBeginFunction();
  Temp scratch = scratch_begin(arena->v, arena->count);
  ...
  task.out_bucket_counts = push_array(scratch.arena, uint64,                 RDI_NameMapKind_COUNT);
  task.out_node_counts   = push_array(scratch.arena, uint64,                 RDI_NameMapKind_COUNT);
  tp_for_parallel(tp, arena, RDI_NameMapKind_COUNT, rdib_build_name_map_task, &task);
  ProfEnd();

  uint64 *bucket_offsets = offsets_from_counts_array_u64(scratch.arena, task.out_bucket_counts, RDI_NameMapKind_COUNT);
  uint64 *node_offsets   = offsets_from_counts_array_u64(scratch.arena, task.out_node_counts,   RDI_NameMapKind_COUNT);

  String8List raw_name_maps = {0}, raw_name_map_buckets = {0}, raw_name_map_nodes = {0}; 
  for (uint64 i = 0; i < RDI_NameMapKind_COUNT; ++i) {
...
THREAD_POOL_TASK_FUNC(rdib_build_src_line_map_task)
{
  ...
  uint64              src_file_idx = rdib_idx_from_source_file(src_file);
  ...
  Pairuint32 *ln_voff_arr = push_array_no_zero(scratch.arena, Pairuint32, ln_voff_count);
  ProfEnd();
  ...
  ProfBegin("Fill out ln_voff_arr");
  {
    uint64 cursor = 0;
    for (RDIB_LineTableFragment *frag = src_file->line_table_frags; frag != 0; frag = frag->next_src_file) {
      for (uint64 line_idx = 0; line_idx < frag->line_count; ++line_idx) {
  ...
  uint32 *line_nums   = push_array_no_zero(arena, uint32, ln_voff_count);
  uint32 *line_ranges = push_array_no_zero(arena, uint32, ln_voff_count + 1);
  uint64 *voffs       = push_array_no_zero(arena, uint64, ln_voff_count);

  uint64 voff_cursor     = 0;
  uint64 line_num_cursor = 0;
  if (ln_voff_count > 0) {
    ...
    for (uint64 i = 1; i < ln_voff_count; ++i) {
      ...
      if (ln_voff_arr[i].v0 != line_nums[line_num_cursor-1]) {
        ...
        line_ranges[line_num_cursor] = (uint32)voff_cursor;
        ++line_num_cursor;
      }
                                         ...
                                         RDIB_SourceFileChunk **src_file_chunks)
{
  ...
  for (uint64 chunk_idx = 0, cursor = 0; chunk_idx < src_file_chunk_count; ++chunk_idx) {
    RDIB_SourceFileChunk *chunk = src_file_chunks[chunk_idx];  
    for (uint64 i = 0; i < chunk->count; ++i) {
  ...
  task.out_line_counts = push_array_no_zero(scratch.arena, uint32,   total_src_file_count);
  task.out_voff_counts = push_array_no_zero(scratch.arena, uint32,   total_src_file_count);
  task.out_line_nums   = push_array_no_zero(scratch.arena, uint32 *, total_src_file_count);
  task.out_line_ranges = push_array_no_zero(scratch.arena, uint32 *, total_src_file_count);
  task.out_voffs       = push_array_no_zero(scratch.arena, uint64 *, total_src_file_count);
  ProfEnd();
  ...
  uint64 src_line_map_cursor = 0;
  uint64 line_num_cursor     = 0;
  uint64 line_range_cursor   = 0;
  uint64 voff_cursor         = 0;
...
THREAD_POOL_TASK_FUNC(rdib_build_line_tables_task)
{
  ...
  Rng1uint64                   range   = task->ranges[task_id];
  ...
  for (uint64 chunk_idx = range.min; chunk_idx < range.max; ++chunk_idx) {
    ...
    for (uint64 i = 0; i < chunk->count; ++i) {
      ...
      uint64             line_table_idx = chunk->base + i;
      ...
      if (total_line_count > 0) {
        ...
        uint64           pair_cursor = 0;

        for (RDIB_LineTableFragment *frag = line_table->first; frag != 0; frag = frag->next_line_table) {
          for (uint64 line_idx = 0; line_idx < frag->line_count; ++line_idx, ++pair_cursor) {
        ...
        uint64       line_count = pair_cursor + 1;
        uint64      *voffs      = push_array_no_zero(arena, uint64,      line_count);
        RDI_Line *lines      = push_array_no_zero(arena, RDI_Line, line_count);

        uint64 line_cursor = 0;
        for (uint64 line_idx = 0; line_idx < pair_cursor; ++line_idx) {
                                    ...
                                    RDIB_LineTableChunk  **chunks)
{
  ...
  task.chunks                   = chunks;
  task.ranges                   = tp_divide_work(scratch.arena, chunk_count, tp->worker_count);
  task.out_line_table_counts    = push_array_no_zero(scratch.arena, uint64,        total_line_table_count);
  task.out_line_table_voffs     = push_array_no_zero(scratch.arena, uint64 *,      total_line_table_count);
  ...
  uint64 line_table_cursor      = 0;
  uint64 line_table_voff_cursor = 0;
  uint64 line_table_line_cursor = 0;
  ...
  for (uint64 chunk_idx = 0; chunk_idx < chunk_count; ++chunk_idx) {
    ...
    for (uint64 i = 0; i < chunk->count; ++i) {
      ...
      uint64             src_idx = rdib_idx_from_line_table(src);
...
THREAD_POOL_TASK_FUNC(rdib_fill_src_files_task)
{
  ...
  for (uint64 chunk_idx = task->ranges[task_id].min; chunk_idx < task->ranges[task_id].max; ++chunk_idx) {
    ...
    for (uint64 i = 0; i < chunk->count; ++i) {
      ...
      uint32              src_file_idx = rdib_idx_from_source_file(src);
...
rdib_data_sections_from_inline_sites(TP_Context *tp,
                                     ...
                                     RDIB_DataSectionList *sect_list,
                                     RDIB_StringMap *string_map,
                                     uint64 total_inline_site_count,
                                     uint64 inline_site_chunk_count,
                                     RDIB_InlineSiteChunk **inline_site_chunks)
{
  ...
  for (uint64 chunk_idx = 0; chunk_idx < inline_site_chunk_count; ++chunk_idx) {
    RDIB_InlineSiteChunk *chunk = inline_site_chunks[chunk_idx];
    for (uint64 i = 0; i < chunk->count; ++i) {
      RDIB_InlineSite *src = &chunk->v[i];
      uint64 idx = rdib_idx_from_inline_site(src);
...
rdib_init_input(Arena *arena)
{
  ...
  RDIB_InlineSite        *null_inline_site = rdib_inline_site_chunk_list_push_zero(arena, &input.inline_sites, 1);
  {
    ...
    null_frag->voffs    = push_array(arena, uint64, 1);
    ...
    null_unit->virt_ranges      = push_array(arena, Rng1uint64, 1);
    null_unit->virt_ranges[0]   = rng_1u64(0,0);
    null_unit->line_table       = null_line_table;

    // Scope
    rng1u64_list_push(arena, &null_scope->ranges, rng_1u64(0,max_uint32));
...
rdib_finish(TP_Context *tp, TP_Arena *arena, RDIB_Input *input)
{
  ...
  //uint64 type_chunk_count        = types.count;
  //uint64 struct_chunk_count      = struct_list.count;
  //uint64 union_chunk_count       = union_list.count;
  //uint64 enum_chunk_count        = enum_list.count;
  //uint64 total_struct_count      = rdib_type_chunk_list_total_count(struct_list);
  //uint64 total_union_count       = rdib_type_chunk_list_total_count(union_list);
  //uint64 total_enum_count        = rdib_type_chunk_list_total_count(enum_list);
  //uint64 extern_gvar_chunk_count = extern_gvars.count;
  //uint64 extern_tvar_chunk_count = extern_tvars.count;
  //uint64 extern_proc_chunk_count = extern_procs.count;
  //uint64 static_gvar_chunk_count = static_gvars.count;
  //uint64 static_tvar_chunk_count = static_tvars.count;
  //uint64 static_proc_chunk_count = static_procs.count;
  //uint64 total_extern_gvar_count = rdib_variable_chunk_list_total_count (extern_gvars);
  //uint64 total_extern_tvar_count = rdib_variable_chunk_list_total_count (extern_tvars);
  //uint64 total_extern_proc_count = rdib_procedure_chunk_list_total_count(extern_procs);
  ...
  uint64 total_unit_count             = rdib_unit_chunk_list_total_count       (all_units            );
  uint64 total_src_file_count         = rdib_source_file_chunk_list_total_count(all_src_files        );
  uint64 total_line_table_count       = rdib_line_table_chunk_list_total_count (all_line_tables      );
  uint64 total_scope_count            = rdib_scope_chunk_list_total_count      (all_scopes           );
  uint64 total_local_count            = rdib_variable_chunk_list_total_count   (all_locals           );
  uint64 total_inline_site_count      = rdib_inline_site_chunk_list_total_count(all_inline_sites     );
  uint64 total_udt_member_count       = rdib_udt_member_chunk_list_total_count (all_udt_members      );
  uint64 total_enum_member_count      = rdib_udt_member_chunk_list_total_count (all_enum_members     );
  uint64 total_type_count             = rdib_type_chunk_list_total_count       (all_types            );
  uint64 total_param_type_count       = rdib_type_chunk_list_total_count       (all_param_types      );
  //uint64 total_udt_member_type_count  = rdib_type_chunk_list_total_count       (all_udt_member_types );
  //uint64 total_enum_member_type_count = rdib_type_chunk_list_total_count       (all_enum_member_types); 
  uint64 total_tvar_count             = rdib_variable_chunk_list_total_count   (all_tvars            );
  uint64 total_gvar_count             = rdib_variable_chunk_list_total_count   (all_gvars            );
  uint64 total_proc_count             = rdib_procedure_chunk_list_total_count  (all_procs            );
  ProfEnd();
  ...
  //uint64                  udt_chunk_count     = struct_chunk_count + union_chunk_count + enum_chunk_count;

  ProfBegin("Assign Type Indices");
  uint64 total_type_node_count = 1;
  {
    struct TypeNode {
...
if (((RDIB_Type*)(t))->kind == RDI_TypeKindExt_VirtualTable) break; \
  ...
  SLLStackPush(stack, n);                                           \
    ...
    for (uint64 chunk_idx = 0; chunk_idx < all_types.count; ++chunk_idx) {
      ...
      for (uint64 i = 0; i < chunk->count; ++i) {
        ...
        for (struct TypeNode *cursor = stack; cursor != 0; cursor = cursor->next) {
          if (cursor->type->kind == RDI_TypeKind_NULL){
            ...
            for (uint64 i = 0; i < params->params.count; ++i) {
...
#undef push_node
  ...
  {
    type_stats.udt_counts = push_array(scratch.arena, uint64, all_types.count);
  ...
  RDIB_StringMap *string_map;
  {
    uint64 top_level_string_count   = 2;
    uint64 sect_string_count        = 1;
    uint64 src_file_string_count    = 1;
    uint64 unit_string_count        = 6;
    uint64 variable_string_count    = 2;
    uint64 procedure_string_count   = 2;
    uint64 scope_string_count       = 0;
    uint64 inline_site_string_count = 0;
    uint64 member_string_count      = 2;
    uint64 type_string_count        = 3;
    uint64 path_tree_node_count     = 1;

    uint64 total_string_count = 1 /* :string_map_null */                           +
    ...
    task.element_indices        = push_array(scratch.arena, uint64,                    tp->worker_count);
  ...
  uint64                    string_map_bucket_count;
  ...
  uint64                    name_map_bucket_counts[RDI_NameMapKind_COUNT];
  for (uint64 i = 0; i < ArrayCount(name_map_buckets); ++i) {
  ...
  uint64                   idx_run_bucket_count;
  {
    // TODO: we over-allocate for name map index runs since not every bucket has > 1 value
    uint64 total_name_map_value_count = 0;
    for (uint64 i = 0; i < ArrayCount(name_map_bucket_counts); ++i) {
      total_name_map_value_count += name_map_bucket_counts[i];
    }

    // rough bucket estimate
    uint64 idx_run_cap = (total_param_type_count + total_name_map_value_count) * 2;
    ...
    for (uint64 name_map_kind = 0; name_map_kind < ArrayCount(name_maps); ++name_map_kind) {
  ...
  String8List rdi_data = {0};
  {
    ...
    for (uint64 sect_idx = 0; sect_idx < RDI_SectionKind_COUNT; ++sect_idx) {
