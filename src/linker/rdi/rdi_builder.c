...
{
  RDIB_LineTableFragment *n = new RDIB_LineTableFragment[1];
...
rdib_unit_chunk_list_reserve_ex(Arena *arena, RDIB_UnitChunkList *list, U64 count_per_chunk, U64 item_count)
{
  ...
  RDIB_UnitChunk *chunks      = new RDIB_UnitChunk[chunk_count];
  ...
  for (U64 i = 0; i+1 < chunk_count; i += 1, item_count -= count_per_chunk, base += count_per_chunk) {
    ...
    chunks[i].v     = new RDIB_Unit[count_per_chunk];
  ...
  chunks[chunk_count-1].v     = new RDIB_Unit[item_count];
...
{
  // fill out node
  RDIB_UnitChunk *chunk = new RDIB_UnitChunk[1];
  chunk->cap            = cap;
  chunk->v              = new RDIB_Unit[cap];
...
{
  // fill out node
  RDIB_TypeChunk *chunk = new RDIB_TypeChunk[1];
  chunk->cap            = cap;
  chunk->v              = new RDIB_Type[cap];
...
{
  // fill out node
  RDIB_SourceFileChunk *chunk = new RDIB_SourceFileChunk[1];
  chunk->cap                  = cap;
  chunk->v                    = new RDIB_SourceFile[cap];
...
{
  ProfBeginFunction();
  RDIB_UnitChunk **result = /* no zero */ new RDIB_UnitChunk *[list.count];
  U64 chunk_idx = 0;
...
{
  ProfBeginFunction();
  RDIB_ScopeChunk **result = /* no zero */ new RDIB_ScopeChunk *[list.count];
  U64 chunk_idx = 0;
...
{
  ProfBeginFunction();
  RDIB_VariableChunk **result = /* no zero */ new RDIB_VariableChunk *[list.count];
  U64 chunk_idx = 0;
...
{
  ProfBeginFunction();
  RDIB_LineTableChunk **result = /* no zero */ new RDIB_LineTableChunk *[list.count];
  U64 chunk_idx = 0;
...
{
  ProfBeginFunction();
  RDIB_ProcedureChunk **result = /* no zero */ new RDIB_ProcedureChunk *[list.count];
  U64 chunk_idx = 0;
...
{
  ProfBeginFunction();
  RDIB_InlineSiteChunk **result = /* no zero */ new RDIB_InlineSiteChunk *[list.count];
  U64 chunk_idx = 0;
...
{
  ProfBeginFunction();
  RDIB_UDTMemberChunk **result = /* no zero */ new RDIB_UDTMemberChunk *[list.count];
  U64 chunk_idx = 0;
...
{
  ProfBeginFunction();
  RDIB_TypeChunk **result = /* no zero */ new RDIB_TypeChunk *[list.count];
  U64 chunk_idx = 0;
...
{
  ProfBeginFunction();
  RDIB_SourceFileChunk **result = /* no zero */ new RDIB_SourceFileChunk *[list.count];
  U64 chunk_idx = 0;
...
{
  RDIB_EvalBytecodeOp *node = new RDIB_EvalBytecodeOp[1];
  node->op                  = op;
...
{
  RDIB_LocationNode *node = new RDIB_LocationNode[1];
  node->v = v;
...
{
  RDIB_VariableNode *node = new RDIB_VariableNode[1];
                              ...
                              RDIB_TypeStats         type_stats)
{
  ...
  RDI_Member *udt_members_rdi;
  {
    ...
    ProfBegin("Count");
    task.counts = new U64[tp->worker_count];
    ...
    udt_members_rdi      = new RDI_Member[udt_member_count_rdi] /* no zero */;
  ...
  RDI_EnumMember *enum_members_rdi;
  {
    ...
    enum_members_rdi      = new RDI_EnumMember[enum_member_count_rdi] /* no zero */;
  ...
  RDI_UDT      *udts       = /* no zero */ new RDI_UDT[total_udt_count      ];
...
{
  RDIB_PathTree *tree = new RDIB_PathTree[1];
  tree->root          = new RDIB_PathTreeNode[1];
  tree->list_count    = list_count;
  tree->node_lists    = new RDIB_PathTreeNodeList[list_count];
  return tree;
}
...
rdib_path_tree_insert(Arena *arena, RDIB_PathTree *tree, String8 path, RDIB_SourceFile *src_file)
{
  ...
  for (String8Node *n = sub_paths.first; n != 0; n = n->next) {
    ...
    // new directory/file
    if (sub_child == 0) {
      sub_child           = new RDIB_PathTreeNode[1];
...
{
  RDIB_StringMap *string_map = new RDIB_StringMap[1];
  string_map->cap            = (U64)((F64)cap * 1.3);
  string_map->buckets        = new RDIB_StringMapBucket *[string_map->cap];
  return string_map;
}
...
rdib_string_map_insert_item(Arena *arena, RDIB_CollectStringsTask *task, U64 task_id, String8 string, void *value)
{
  ...
  if (*bucket == 0) {
    *bucket = new RDIB_StringMapBucket[1];
  }
...
rdib_extant_buckets_from_string_map(TP_Context *tp, Arena *arena, RDIB_StringMap *string_map, U64 *bucket_count_out)
{
  ...
  *bucket_count_out = sum_array_u64(tp->worker_count, task.counts);
  ...
  task.offsets = offsets_from_counts_array_u64(scratch.arena, task.counts, tp->worker_count);
  task.result  = new RDIB_StringMapBucket *[*bucket_count_out];
...
THREAD_POOL_TASK_FUNC(rdib_string_map_bucket_chunk_idx_histo_task)
{
  ...
  U32 *range_histo = new U32[task->chunk_idx_opl];
...
rdib_string_map_sort_buckets(TP_Context *tp, RDIB_StringMapBucket **buckets, U64 bucket_count, U64 chunk_idx_opl)
{
  ...
  task.src           = buckets;
  task.dst           = /* no zero */ new RDIB_StringMapBucket *[bucket_count];

  ProfBegin("Chunk Index Histogram");
  task.chunk_histo = new U32[chunk_idx_opl];
...
THREAD_POOL_TASK_FUNC(rdib_name_map_var_task)
{
  ...
  for (U64 chunk_idx = task->ranges[task_id].min; chunk_idx < task->ranges[task_id].max; ++chunk_idx) {
    RDIB_VariableChunk *chunk = task->vars[chunk_idx];
    VoidNode           *nodes = new VoidNode[chunk->count];
...
{
  ProfBeginFunction();
  RDIB_IndexRunMap *map = new RDIB_IndexRunMap[1];
  map->cap              = cap;
  map->buckets          = new RDIB_IndexRunBucket *[cap];
  ProfEnd();
  return map;
}
...
rdib_extant_buckets_from_index_run_map(TP_Context *tp, Arena *arena, RDIB_IndexRunMap *idx_run_map, U64 *bucket_count_out)
{
  ...
  *bucket_count_out = sum_array_u64(tp->worker_count, task.counts);
  ...
  ProfBegin("Copy Extant Buckets");
  task.offsets = offsets_from_counts_array_u64(scratch.arena, task.counts, tp->worker_count);
  task.result  = new RDIB_IndexRunBucket *[*bucket_count_out];
...
rdib_index_run_map_sort_buckets(TP_Context *tp, RDIB_IndexRunBucket **buckets, U64 bucket_count, U64 chunk_idx_opl)
{
  ...
  task.ranges        = tp_divide_work(scratch.arena, bucket_count, tp->worker_count);
  task.src           = buckets;
  task.dst           = /* no zero */ new RDIB_IndexRunBucket *[bucket_count];
...
rdib_index_run_map_insert_item(Arena *arena, RDIB_BuildIndexRunsTask *task, U64 worker_id, U64 item_idx, U64 count, U32 *idxs)
{
  ...
  if (bucket == 0) {
    bucket = new RDIB_IndexRunBucket[1];
  }
...
THREAD_POOL_TASK_FUNC(rdib_build_idx_runs_params_task)
{
  ...
  for (RDIB_Type *type = &chunk->v[0], *opl = chunk->v + chunk->count; type < opl; ++type) {
    if (type->kind == RDI_TypeKind_Function) {
      ...
      U32 *type_indices     = new U32[type_index_count] /* no zero */;
...
THREAD_POOL_TASK_FUNC(rdib_build_idx_runs_name_map_buckets_task)
{
  ...
  for (U64 bucket_idx = task->ranges[task_id].min; bucket_idx < task->ranges[task_id].max; ++bucket_idx) {
    ...
    if (count > 1) {
      ...
      U32 *idxs = new U32[count] /* no zero */;
      {
        U64 curr_idx = 0;
...
{
  RDIB_DataSectionNode *node = new RDIB_DataSectionNode[1];
  node->v                    = v;
...
rdib_data_sections_from_top_level_info(Arena *arena, RDIB_DataSectionList *sect_list, RDIB_StringMap *string_map, RDIB_TopLevelInfo *src)
{
  ...
  RDI_TopLevelInfo *dst         = new RDI_TopLevelInfo[1];
  dst->arch                     = src->arch;
...
rdib_data_sections_from_binary_sections(Arena *arena, RDIB_DataSectionList *sect_list, RDIB_StringMap *string_map, RDIB_BinarySection *binary_sects, U64 binary_sects_count)
{
  ...
  RDI_BinarySection *dst_arr = new RDI_BinarySection[binary_sects_count];
                              ...
                              RDIB_UnitChunk      **unit_chunks)
{
  ...
  RDI_Unit *dst_arr = new RDI_Unit[total_unit_count];
...
rdib_sort_procs_radix_32(RDIB_Procedure **v, U64 count)
{
  ...
  RDIB_Procedure **temp = /* no zero */ new RDIB_Procedure *[count];
...
rdib_data_from_vmap(Arena *arena, U64 range_count, RDIB_VMapRange *ranges)
{
  ...
  U8 *radix_memory      = new U8[radix_memory_size] /* no zero */;
  ProfEnd();
...
#define push_vme() (vme_block_size < raw_vmap.last->string.size/sizeof(vme_block[0])) ? &vme_block[vme_block_size++] :    \
                                                  (vme_block = new RDI_VMapEntry[vme_block_cap],           \
                                              ...
                                              U64 scope_chunk_count, RDIB_ScopeChunk    **scope_chunks)
{
  ...
  RDIB_VMapRange *unit_vmaps;
  {
    ...
    unit_vmaps      = /* no zero */ new RDIB_VMapRange[unit_vmap_count];
    ProfEnd();
  ...
  RDIB_VMapRange *gvar_vmaps;
  {
    ...
    gvar_vmaps      = /* no zero */ new RDIB_VMapRange[gvar_vmap_count];
    ProfEnd();
  ...
  RDIB_VMapRange *scope_vmaps;
  {
    ...
    scope_vmaps      = /* no zero */ new RDIB_VMapRange[scope_vmap_count];
    ProfEnd();
...
rdib_data_sections_from_string_map(TP_Context *tp, Arena *arena, RDIB_DataSectionList *sect_list, RDIB_StringMapBucket **buckets, U64 bucket_count)
{
  ...
  U64  cursor       = 0;
  U32 *string_table = new U32[bucket_count] /* no zero */;
  ...
  task.string_data      = new U8[task.string_data_size] /* no zero */;
...
rdib_data_sections_from_index_runs(TP_Context *tp, Arena *arena, RDIB_DataSectionList *sect_list, RDIB_IndexRunBucket **buckets, U64 bucket_count)
{
  ...
  U32 *output_array = new U32[total_index_count] /* no zero */;
...
{
  ProfBeginFunction();
  RDI_FilePathNode *nodes_dst = new RDI_FilePathNode[path_tree->node_count] /* no zero */;
...
THREAD_POOL_TASK_FUNC(rdib_build_var_section_task)
{
  ...
  for (U64 chunk_idx = task->ranges[task_id].min; chunk_idx < task->ranges[task_id].max; ++chunk_idx) {
    ...
    RDI_GlobalVariable *vars  = new RDI_GlobalVariable[chunk->count] /* no zero */;
                                         ...
                                         RDIB_VariableChunk   **chunks)
{
  ...
  task.gvars_out                   = new String8List[tp->worker_count];
...
THREAD_POOL_TASK_FUNC(rdib_build_tvar_section_task)
{
  ...
  for (U64 chunk_idx = task->ranges[task_id].min; chunk_idx < task->ranges[task_id].max; ++chunk_idx) {
    ...
    RDI_ThreadVariable *vars  = new RDI_ThreadVariable[chunk->count] /* no zero */;
                                         ...
                                         RDIB_VariableChunk   **chunks)
{
  ...
  task.tvars_out                   = new String8List[tp->worker_count];
...
THREAD_POOL_TASK_FUNC(rdib_build_procs_section_task)
{
  ...
  for (U64 chunk_idx = task->ranges[task_id].min; chunk_idx < task->ranges[task_id].max; ++chunk_idx) {
    ...
    RDI_Procedure       *procs = new RDI_Procedure[chunk->count] /* no zero */;
                                   ...
                                   RDIB_ProcedureChunk  **chunks)
{
  ...
  task.procs_out                   = new String8List[tp->worker_count];
                               ...
                               RDIB_ScopeChunk      **scopes)
{
  ...
  task.scope_voff_counts = new U64[tp->worker_count];
  task.local_counts      = new U64[tp->worker_count];
  task.loc_block_counts  = new U64[tp->worker_count];
  task.loc_data_sizes    = new U64[tp->worker_count];
  ...
  ProfBegin("Push");
  task.scope_voffs_rdi = /* no zero */ new U64[total_scope_voff_count];
  task.scopes_rdi      = /* no zero */ new RDI_Scope[total_scope_count     ];
  task.locals_rdi      = /* no zero */ new RDI_Local[total_local_count     ];
  task.loc_blocks_rdi  = /* no zero */ new RDI_LocationBlock[total_loc_block_count ];
  task.loc_data_rdi    = /* no zero */ new U8[total_loc_data_size   ];
  ProfEnd();
...
THREAD_POOL_TASK_FUNC(rdib_build_name_map_task)
{
  ...
  struct NodeList *temp_map   = new         struct NodeList[out_bucket_count];
  struct Node     *temp_nodes = /* no zero */ new struct Node[out_node_count];
  ...
  RDI_NameMapNode   *out_nodes   = /* no zero */ new RDI_NameMapNode[out_node_count];
  ProfEnd();
                                  ...
                                  U64                    src_name_map_counts[RDI_NameMapKind_COUNT])
{
  ...
  task.out_buckets       = new RDI_NameMapBucket *[RDI_NameMapKind_COUNT];
  task.out_nodes         = new RDI_NameMapNode *[RDI_NameMapKind_COUNT];
  task.out_bucket_counts = new U64[RDI_NameMapKind_COUNT];
  task.out_node_counts   = new U64[RDI_NameMapKind_COUNT];
  ...
  for (U64 i = 0; i < RDI_NameMapKind_COUNT; ++i) {
    RDI_NameMap *dst_name_map = new RDI_NameMap[1];
...
THREAD_POOL_TASK_FUNC(rdib_build_src_line_map_task)
{
  ...
  PairU32 *ln_voff_arr = /* no zero */ new PairU32[ln_voff_count];
  ProfEnd();
  ...
  U32 *line_nums   = new U32[ln_voff_count] /* no zero */;
  U32 *line_ranges = new U32[ln_voff_count + 1] /* no zero */;
  U64 *voffs       = new U64[ln_voff_count] /* no zero */;
                                         ...
                                         RDIB_SourceFileChunk **src_file_chunks)
{
  ...
  RDIB_SourceFile **src_file_arr = /* no zero */ new RDIB_SourceFile *[total_src_file_count];
  ...
  task.out_line_counts = /* no zero */ new U32[total_src_file_count];
  task.out_voff_counts = /* no zero */ new U32[total_src_file_count];
  task.out_line_nums   = /* no zero */ new U32 *[total_src_file_count];
  task.out_line_ranges = /* no zero */ new U32 *[total_src_file_count];
  task.out_voffs       = /* no zero */ new U64 *[total_src_file_count];
  ProfEnd();
  ...
  ProfBegin("Push");
  RDI_SourceLineMap *src_line_maps = /* no zero */ new RDI_SourceLineMap[total_src_file_count + 1];
  ProfEnd();
  ...
  for (U64 chunk_idx = range.min; chunk_idx < range.max; ++chunk_idx) {
    ...
    for (U64 i = 0; i < chunk->count; ++i) {
      ...
      if (total_line_count > 0) {
        ...
        KeyValuePair *pairs       = /* no zero */ new KeyValuePair[total_line_count];
        struct Value *values      = /* no zero */ new struct Value[total_line_count];
        U64           pair_cursor = 0;
        ...
        U64      *voffs      = /* no zero */ new U64[line_count];
                                    ...
                                    RDIB_LineTableChunk  **chunks)
{
  ...
  task.chunks                   = chunks;
  task.ranges                   = tp_divide_work(scratch.arena, chunk_count, tp->worker_count);
  task.out_line_table_counts    = /* no zero */ new U64[total_line_table_count];
  task.out_line_table_voffs     = /* no zero */ new U64 *[total_line_table_count];
  task.out_line_table_lines     = /* no zero */ new RDI_Line *[total_line_table_count];
  ...
  ProfBegin("Push");
  RDI_LineTable *line_tables_rdi = /* no zero */ new RDI_LineTable[total_line_table_count];
  ProfEnd();
                                     ...
                                     RDIB_SourceFileChunk **src_file_chunks)
{
  ...
  task.src_files_dst   = /* no zero */ new RDI_SourceFile[total_src_file_count];
                                     ...
                                     RDIB_InlineSiteChunk **inline_site_chunks)
{
  ...
  RDI_InlineSite *dst_arr = new RDI_InlineSite[total_inline_site_count];
...
rdib_init_input(Arena *arena)
{
  ...
  RDIB_InlineSite        *null_inline_site = rdib_inline_site_chunk_list_push_zero(arena, &input.inline_sites, 1);
  {
    ...
    null_unit->virt_ranges      = new Rng1U64[1];
...
#undef push_node
  ...
  {
    type_stats.udt_counts = new U64[all_types.count];
  ...
  RDIB_StringMap *string_map;
  {
    ...
    task.free_buckets           = new RDIB_StringMapBucket *[tp->worker_count];
    task.element_indices        = new U64[tp->worker_count];
  ...
  U64                   idx_run_bucket_count;
  {
    ...
    task.idx_run_map    = idx_run_map;
    task.free_buckets   = new RDIB_IndexRunBucket *[tp->worker_count];
  ...
  String8List rdi_data = {0};
  {
    ...
    RDI_Header  *rdi_header   = new RDI_Header[1];
    RDI_Section *rdi_sections = new RDI_Section[RDI_SectionKind_COUNT];
