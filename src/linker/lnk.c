...
internal uint128
lnk_blake3_hash_parallel(TP_Context *tp, uint64 chunk_count, String8 data)
{
  ProfBeginFunction();
  ...
  task.hashes           = push_array(scratch.arena, uint128, chunk_count);
...
lnk_merge_manifest_files(String8 mt_path, String8 out_name, String8List manifest_path_list)
{
  ...
  if (os_handle_match(mt_handle, os_handle_zero())) {
    ...
    os_process_join(mt_handle, max_uint64);
  ...
  for (; stack; ) {
    for (; stack->arr_idx < ArrayCount(stack->res_arr); stack->arr_idx += 1) {
      for (; stack->res_idx[stack->arr_idx] < stack->res_arr[stack->arr_idx].count; ) {
        uint64          res_idx = stack->res_idx[stack->arr_idx]++;
        ...
        case PE_ResDataKind_DIR: {
          ...
          // fill out named ids
          for (uint64 i = 0; i < named_array.count; i += 1) {
            PE_Resource            src = named_array.v[i];
            COFF_ResourceDirEntry *dst = &named_entries[i];

            // append resource name
            uint32     res_name_off = safe_cast_u32(rsrc1->data.total_size);
          ...
          // fill out number ids
          for (uint64 i = 0; i < id_array.count; i += 1) {
        ...
        case PE_ResDataKind_COFF_RESOURCE: {
          ...
          uint32 resdat_off = safe_cast_u32(rsrc2->data.total_size);
          ...
          uint64 apply_off   = rsrc1->data.total_size + OffsetOf(COFF_ResourceDataEntry, data_voff);
          uint32 apply_off32 = safe_cast_u32(apply_off);
                           ...
                           LNK_InputObjList    *input_obj_list)
{
  ...
  uint64 input_idx = Compose64Bit(lib->input_idx, symbol->member_offset);
...
#define state_list_pop(l) (l).first->state; SLLQueuePop((l).first, (l).last); (l).count -= 1
  ...
  HashTable             *export_ht                = hash_table_init(scratch.arena, max_uint16/2);
  ...
  uint64                   entry_point_lookup_attempts      = 0;
  ...
  for (;;) {
    while (state_list.count) {
      ...
      case State_InputObjs: {
        ...
        for (uint64 thin_input_idx = 0; thin_input_idx < thin_inputs_count; thin_input_idx += 1) {
        ...
        if (lnk_get_log_status(LNK_Log_InputObj)) {
          uint64 input_size = 0;
          for (uint64 i = 0; i < unique_obj_input_list.count; ++i) { input_size += input_obj_arr[i]->data.size; }
        ...
        if (config->machine == COFF_MachineType_Unknown) {
          for (uint64 obj_idx = 0; obj_idx < obj_node_arr.count; obj_idx += 1) {
            ...
            for (uint64 sect_idx = 0; sect_idx < obj->header.section_count_no_null; sect_idx += 1) {
              ...
              if (sect_header->flags & COFF_SectionFlag_LnkInfo) {
                ...
                if (str8_match(sect_name, str8_lit(".drectve"), 0)) {
                  ...
                  Rng1uint64 sect_range = rng_1u64(sect_header->foff, sect_header->foff + sect_header->fsize);
      ...
      case State_InputLibs: {
        ...
        for (uint64 input_source = 0; input_source < ArrayCount(input_libs); ++input_source) {
          ...
          if (lnk_get_log_status(LNK_Log_InputLib)) {
            if (libs.count > 0) {
              uint64 input_size = 0;
              for (uint64 i = 0; i < libs.count; ++i) { input_size += libs.v[i].data.data.size; }
      ...
      case State_InputLinkerObjs: {
        ...
        if (delayed_imports->count) {
          ...
          for (uint64 dll_idx = 0; dll_idx < delayed_imports->count; dll_idx += 1) {
        ...
        if (static_imports->count) {
          ...
          for (uint64 dll_idx = 0; dll_idx < static_imports->count; dll_idx += 1) {
            String8 import_debug_symbols = lnk_make_dll_import_debug_symbols(scratch.arena, config->machine, dll_names[dll_idx]);
            LNK_InputObj *input = lnk_input_obj_list_push(scratch.arena, &input_obj_list);
            input->input_idx = input_obj_list.count;
...
THREAD_POOL_TASK_FUNC(lnk_gather_symtab_fixups_task)
{
  ...
  LNK_Symbol **symlinks = push_array(scratch.arena, LNK_Symbol *, obj->header.section_count_no_null+1);
  {
    ...
    for (uint64 symbol_idx = 0; symbol_idx < obj->header.symbol_count; symbol_idx += (1 + symbol.aux_symbol_count)) {
      ...
      if (interp == COFF_SymbolValueInterp_Regular && symbol.aux_symbol_count == 0 && symbol.storage_class == COFF_SymStorageClass_External) {
        ...
        if (sect_header->flags & COFF_SectionFlag_LnkCOMDAT) {
          if (symlinks[symbol.section_number] == 0 || symbol.value == 0) {
            symlinks[symbol.section_number] = lnk_symbol_table_search(task->symtab, LNK_SymbolScope_Defined, symbol.name);
          }
        }
      }
    }
  ...
  LNK_SymbolTableFixupList fixups = {0};
  {
    ...
    for (uint64 symbol_idx = 0; symbol_idx < obj->header.symbol_count; symbol_idx += (1 + symbol.aux_symbol_count)) {
      ...
      if (interp == COFF_SymbolValueInterp_Regular) {
        LNK_Symbol *symlink = symlinks[symbol.section_number];
        if (symlink && symlink->u.defined.obj != obj) {
          if (symbol.storage_class == COFF_SymStorageClass_External) {
            ...
            fixup->obj_idx        = symlink->u.defined.obj->input_idx;
            fixup->obj_symbol_idx = symlink->u.defined.symbol_idx;
          } else {
            // COMDAT section may have static symbols which are now invalid to relocate against
            LNK_SymbolTableFixup *fixup = lnk_symbol_table_fixup_list_push(scratch.arena, &fixups);
            fixup->idx            = symbol_idx;
            fixup->obj_idx        = max_uint32;
            fixup->obj_symbol_idx = max_uint32;
            task->u.patch_symtabs.was_symbol_patched[obj_idx][symbol_idx] = 1;
          }
        }
      } else if (interp == COFF_SymbolValueInterp_Common) {
        ...
        uint64      lookup_symbol_idx = symbol_idx;
        for (;;) {
          ...
          // not external symbol -- symbol resolved
...
internal void
lnk_gc_sections(uint64 objs_count, LNK_Obj **objs, LNK_SymbolTableFixupArray *objs_fixups, Pairuint32 root)
{
  for (;;) {
    LNK_Obj                   *obj            = objs[root.v0];
    COFF_SectionHeader        *section_header = lnk_coff_section_header_from_section_number(obj, root.v1);
    ...
    LNK_SymbolTableFixupArray *fixups         = objs_fixups[root.v0];

    for (uint64 reloc_idx = 0; reloc_idx < reloc_info.count; reloc_idx += 1) {
      COFF_Reloc           *reloc = &relocs[reloc_idx];
      LNK_SymbolTableFixup *fixup = lnk_bsearch_symbol_table_fixup_array(fixups, reloc->isymbol);

      LNK_Obj *symbol_obj;
      uint64      symbol_idx;
      if (fixup == 0) {
        symbol_obj = obj;
        symbol_idx = reloc->isymbol;
      } else {
        symbol_obj = objs[fixup->obj_idx];
        symbol_idx = fixup->obj_symbol_idx;
      }

      COFF_ParsedSymbol   symbol                = lnk_parsed_symbol_from_coff_symbol_idx(symbol_obj, symbol_idx);
      COFF_SectionHeader *symbol_section_header = ;
    }
  }
}
*/
...
THREAD_POOL_TASK_FUNC(lnk_remove_associative_sections_task)
{
  ...
  for (uint64 sect_idx = 0; sect_idx < obj->header.section_count_no_null; sect_idx += 1) {
    ...
    uint32 head_sect_idx = max_uint32;
    for (uint64 current_sect_idx = sect_idx;;) {
      uint32 symbol_idx = obj->comdats[current_sect_idx];
      if (symbol_idx == max_uint32) {
        break;
      }
  ...
  for (uint64 sect_idx = 0; sect_idx < obj->header.section_count_no_null; sect_idx += 1) {
    ...
    if (~sect_header->flags & COFF_SectionFlag_LnkRemove && sect_header->fsize > 0) {
      ...
      if (sc_chunk) {
        ...
        uint16 sc_align = coff_align_size_from_section_flags(sect_header->flags);
...
THREAD_POOL_TASK_FUNC(lnk_patch_comdat_leaders_task)
{
  ...
  ProfBeginV("Set COMDAT Section Contribs");
  for (uint64 sect_idx = 0; sect_idx < obj->header.section_count_no_null; sect_idx += 1) {
    ...
    if (section_header->flags & COFF_SectionFlag_LnkCOMDAT) {
      LNK_Symbol *sym = symlinks[sect_idx+1];
      if (sym) {
        COFF_ParsedSymbol parsed_sym = lnk_parsed_symbol_from_coff_symbol_idx(sym->u.defined.obj, sym->u.defined.symbol_idx);
        task->sect_map[obj_idx][sect_idx] = task->sect_map[sym->u.defined.obj->input_idx][parsed_sym.section_number - 1];
      }
    }
  }
  ProfEnd();
    ...
    if (interp == COFF_SymbolValueInterp_Regular) {
      ...
      if (symlink && symlink->u.defined.obj != obj) {
        ...
        if (obj->header.is_big_obj) {
          ...
          symbol16->section_number = (uint16)section_number;
...
lnk_section_contrib_ptr_is_before(void *raw_a, void *raw_b)
{
  ...
  uint64 input_idx_a = Compose64Bit((*a)->u.obj_idx, (*a)->u.obj_sect_idx);
  uint64 input_idx_b = Compose64Bit((*b)->u.obj_idx, (*b)->u.obj_sect_idx);
...
THREAD_POOL_TASK_FUNC(lnk_patch_common_block_leaders_task)
{
  ...
  LNK_BuildImageTask *task          = raw_task;
  Rng1uint64             contrib_range = task->u.patch_symtabs.ranges[task_id];

  for (uint64 contrib_idx = contrib_range.min; contrib_idx < contrib_range.max; contrib_idx += 1) {
    ...
    uint64                     section_number = task->u.patch_symtabs.common_block_sect->sect_idx + 1;
...
internal uint64
lnk_compute_win32_image_header_size(LNK_Config *config, uint64 sect_count)
{
  uint64 image_header_size = 0;
  image_header_size += sizeof(PE_DosHeader) + pe_dos_program.size;
  image_header_size += sizeof(uint32); // PE_MAGIC
...
THREAD_POOL_TASK_FUNC(lnk_obj_reloc_patcher)
{
  ...
  for (uint64 sect_idx = 0; sect_idx < obj_header.section_count_no_null; sect_idx += 1) {
    ...
    Rng1uint64 section_frange = rng_1u64(section_header->foff, section_header->foff + section_header->fsize);
    ...
    for (uint64 reloc_idx = 0; reloc_idx < reloc_info.count; reloc_idx += 1) {
      ...
      uint64 reloc_voff = section_header->voff + reloc->apply_off;

      // compute symbol location values
      uint32 symbol_secnum = 0;
      uint32 symbol_secoff = 0;
      uint64 symbol_voff   = 0;
      {
        ...
        if (interp == COFF_SymbolValueInterp_Regular) {
          ...
          symbol_voff   = safe_cast_u32((uint64)task->image_section_table[symbol.section_number]->voff + (uint64)symbol_secoff);
        } else if (interp == COFF_SymbolValueInterp_Abs) {
          ...
          symbol_secnum = 0;
          symbol_secoff = 0;
          symbol_voff   = (uint64)symbol.value - (uint64)task->image_base;
        } else if (interp == COFF_SymbolValueInterp_Weak) {
          // unresolved weak
        } else if (interp == COFF_SymbolValueInterp_Undefined) {
      ...
      uint64 addend       = extend_sign64(raw_addend, reloc_value.size);
      uint64 reloc_result = reloc_value.value + addend;
...
lnk_section_definition_is_before(void *raw_a, void *raw_b)
{
  ...
  uint64 input_idx_a = Compose64Bit((*a)->obj->input_idx, (*a)->obj_sect_idx);
  uint64 input_idx_b = Compose64Bit((*b)->obj->input_idx, (*b)->obj_sect_idx);
  return u64_compar_is_before(&input_idx_a, &input_idx_b);
}
...
THREAD_POOL_TASK_FUNC(lnk_fill_out_common_block_contribs_task)
{
  LNK_BuildImageTask *task   = raw_task;
  LNK_SymbolTable    *symtab = task->symtab;
  uint64                 cursor = task->u.common_block.offsets[task_id];
      ...
      for (uint64 reloc_idx = 0; reloc_idx < reloc_info.count; reloc_idx += 1) {
        ...
        if (is_symbol_address) {
          ...
          if (is_addr32 || is_addr64) {
            uint64 reloc_voff = sect_header->voff + r->apply_off;
            uint64 page_voff  = AlignDownPow2(reloc_voff, task->page_size);
...
internal String8List
lnk_build_base_relocs(TP_Context *tp, TP_Arena *tp_arena, LNK_Config *config, uint64 objs_count, LNK_Obj **objs)
{
  ProfBeginFunction();
  ...
  if (page_arr.count) {
    ...
    for (uint64 page_idx = 0; page_idx < page_arr.count; ++page_idx) {
      ...
      uint32 *page_voff_ptr;
      uint32 *block_size_ptr;
      uint16 *reloc_arr_base;
      
      // push buffer
      uint64   buf_size = AlignPow2(sizeof(*page_voff_ptr) + sizeof(*block_size_ptr) + sizeof(*reloc_arr_base)*total_entry_count, sizeof(uint32));
      void *buf      = push_array_no_zero(arena, uint8, buf_size);
      
      // setup pointers into buffer
      page_voff_ptr  = buf;
      block_size_ptr = page_voff_ptr + 1;
      reloc_arr_base = (uint16*)(block_size_ptr + 1);
      
      // write 32-bit relocations
      uint16 *reloc_arr_ptr = reloc_arr_base;
      for (uint64Node *i = page->entries_addr32.first; i != 0; i = i->next) {
      ...
      for (uint64Node *i = page->entries_addr64.first; i != 0; i = i->next) {
...
internal String8List
lnk_build_win32_header(Arena *arena, LNK_SymbolTable *symtab, LNK_Config *config, LNK_SectionArray sects, uint64 expected_image_header_size)
{
  ProfBeginFunction();
  ...
  // PE magic
  //
  uint32 *pe_magic = push_array(arena, uint32, 1);
  *pe_magic = PE_MAGIC;
  ...
  //
  uint64 code_base            = 0;
  uint64 sizeof_code          = 0;
  uint64 sizeof_inited_data   = 0;
  uint64 sizeof_uninited_data = 0;
  uint64 sizeof_image         = 0;
  for (uint64 sect_idx = 0; sect_idx < sects.count; sect_idx += 1) {
  ...
  uint64                 coff_section_table_count = 0;
  {
    for (uint64 sect_idx = 0; sect_idx < sects.count; sect_idx += 1) {
      LNK_Section *sect = sects.v[sect_idx];
  ...
  // align image headers
  {
    uint64 image_headers_align_size = AlignPadPow2(result.total_size, config->file_align);
    uint8 *image_headers_align      = push_array(arena, uint8, image_headers_align_size);
  ...
  //
  {
    ...
    for (uint64 i = 1; i <= coff_section_table_count; i += 1) { section_table[i] = &coff_section_table[i-1]; }
...
lnk_build_image(TP_Arena *arena, TP_Context *tp, LNK_Config *config, LNK_SymbolTable *symtab, uint64 objs_count, LNK_Obj **objs)
{
  ProfBegin("Image");
  ...
  /*
  ProfBegin("Gather Symbol Tables Fixups");
  tp_for_parallel(tp, 0, objs_count, lnk_gather_symtab_fixups_task, &task);
  ProfEnd();
  */
...

  {
    ...
    for (uint64 worker_idx = 0; worker_idx < tp->worker_count; worker_idx += 1) task.u.gather_sects.defns[worker_idx] = hash_table_init(arena->v[0], 128);
    ProfEnd();
    ...
    uint64                     sect_defns_count;
    LNK_SectionDefinition **sect_defns;
    {
      for (uint64 worker_idx = 1; worker_idx < tp->worker_count; worker_idx += 1) {
        uint64                     sect_defns_count = task.u.gather_sects.defns[worker_idx]->count;
        ...
        for (uint64 defn_idx = 0; defn_idx < sect_defns_count; defn_idx += 1) {
    ...
    for (uint64 defn_idx = 0; defn_idx < sect_defns_count; defn_idx += 1) {
      ...
      for (LNK_SectionNode *sect_n = sectab->list.first; sect_n != 0; sect_n = sect_n->next) {
        if (str8_match(sect_n->data.name, sect_name, 0) && sect_n->data.flags != sect_defn->flags) {
          ...
          uint32                 sect_number        = sect_defn->obj_sect_idx + 1;
  ...
  uint64 expected_image_header_size;
  {
    ProfBegin("Alloc Section Map");
    task.sect_map = push_array(scratch.arena, LNK_SectionContrib **, objs_count);
    for (uint64 obj_idx = 0; obj_idx < objs_count; obj_idx += 1) task.sect_map[obj_idx] = push_array(scratch.arena, LNK_SectionContrib *, objs[obj_idx]->header.section_count_no_null);
    ProfEnd();
    ...
    uint64                     common_block_contribs_count;
    LNK_CommonBlockContrib *common_block_contribs;
    {
      ProfBegin("Build Common Block");

      ProfBegin("Count Contribs");
      task.u.common_block.counts = push_array(scratch.arena, uint64, tp->worker_count);
      ...
      if (common_block_contribs_count) {
        ...
        uint64 common_block_cursor = common_block_sect->vsize;

        // compute and assign offsets into the common block
        for (uint64 contrib_idx = 0; contrib_idx < common_block_contribs_count; contrib_idx += 1) {
          LNK_CommonBlockContrib *contrib = &common_block_contribs[contrib_idx];
          uint32 size  = contrib->u.size;
          uint32 align = Min(32, u64_up_to_pow2(size)); // link.exe caps align at 32 bytes
        ...
        common_block_sc->u.obj_idx              = max_uint32;
        common_block_sc->u.obj_sect_idx         = max_uint32;
    ...
    // finalize sections layouts
    {
      ...
      for (LNK_SectionNode *sect_n = sectab->merge_list.first; sect_n != 0; sect_n = sect_n->next) {
        ...
        uint64                 last_sc_size = lnk_size_from_section_contrib(last_sc);
    ...
    // patch symbol tables
    {
      ...
      for (uint64 obj_idx = 0; obj_idx < objs_count; obj_idx += 1) {
      ...
      ProfBegin("Flag Debug Symbols");
      tp_for_parallel(tp, 0, objs_count, lnk_flag_debug_symbols_task, &task);
      ProfEnd();

      ProfBegin("Patch COMDAT Leaders");
      tp_for_parallel(tp, 0, objs_count, lnk_patch_comdat_leaders_task, &task);
      ProfEnd();
      ...
      ProfBegin("Patch Common Block Symbols");
    ...
    uint64 voff_cursor = AlignPow2(expected_image_header_size + sizeof(COFF_SectionHeader), config->sect_align);
    for (uint64 i = 0; i < task.image_sects.count; i += 1) {
    ...
    // assign file space
    uint64 foff_cursor = AlignPow2(expected_image_header_size, config->file_align);
    for (uint64 i = 0; i < task.image_sects.count; i += 1) {
  ...
  String8 image_data = {0};
  {
    ...
    image_data.str  = push_array_no_zero(arena->v[0], uint8, image_size);
      ...
      if (~sect->flags & COFF_SectionFlag_CntUninitializedData) {
        ...
        for (LNK_SectionContribChunk *sc_chunk = sect->contribs.first; sc_chunk != 0; sc_chunk = sc_chunk->next) {
          for (uint64 sc_idx = 0; sc_idx < sc_chunk->count; sc_idx += 1) {
            ...
            uint64 fill_size = sc->u.off - prev_sc_opl;
        ...
        {
          uint64 fill_size = sect->fsize - prev_sc_opl;
          MemorySet(image_data.str + sect->foff + prev_sc_opl, fill_byte, fill_size);
        }
      }
    }
...

  {
    ...
    // patch load config
    {
      ...
      if (load_config_symbol) {
        uint64     load_config_foff   = lnk_file_off_from_symbol(image_section_table, load_config_symbol);
        String8 load_config_data   = str8_skip(image_data, load_config_foff);

        uint32 load_config_size = 0;
    ...
    // patch import and import addr
    {
      ...
      if (idata_sect && null_import_desc && null_thunk_data) {
        ...
        uint64                null_thunk_data_voff   = image_section_table[null_thunk_data_parsed.section_number]->voff + null_thunk_data_parsed.value;
        uint64                first_import_foff      = image_section_table[idata_first_contrib->u.sect_idx+1]->foff + idata_first_contrib->u.off;
    ...
    // patch TLS
    {
      ...
      if (tls_used_symbol) {
        ...
        // patch-in align
        uint64 tls_header_foff = lnk_file_off_from_symbol(image_section_table, tls_used_symbol);
    ...
    // patch debug
    {
      ...
      if (debug_dir_sect) {
        ...
        uint64 debug_begin_foff = lnk_foff_from_section_contrib(image_section_table, first_sc);
        uint64 debug_end_fopl   = lnk_fopl_from_section_contrib(image_section_table, last_sc);

        // patch file offsets to the debug directories
        for (uint64 cursor = debug_begin_foff; cursor + sizeof(PE_DebugDirectory) <= debug_end_fopl; cursor += sizeof(PE_DebugDirectory)) {
          PE_DebugDirectory *dir = str8_deserial_get_raw_ptr(image_data, cursor, sizeof(PE_DebugDirectory));
          for (uint64 section_number = 1; section_number < pe.section_count+1; section_number += 1) {
    ...
    // compute image guid, and patch PDB and RDI guids
    {
      ...
      if (guid_pdb_symbol || guid_rdi_symbol) {
        ...
        case Lnk_DebugInfoGuid_ImageBlake3: {
          ...
          uint128 hash = lnk_blake3_hash_parallel(tp, 128, image_data);
      ...
      if (guid_pdb_symbol) {
        uint64   cv_guid_foff = lnk_file_off_from_symbol(image_section_table, guid_pdb_symbol);
        Guid *cv_guid  = str8_deserial_get_raw_ptr(image_data, cv_guid_foff, sizeof(*cv_guid));
        *cv_guid = config->guid;
      }

      if (guid_rdi_symbol) {
        uint64   cv_guid_foff = lnk_file_off_from_symbol(image_section_table, guid_rdi_symbol);
        Guid *cv_guid  = str8_deserial_get_raw_ptr(image_data, cv_guid_foff, sizeof(*cv_guid));
        *cv_guid = config->guid;
      }
    }
...
internal Pairuint32 *
lnk_obj_sect_idx_from_section(Arena *arena, uint64 objs_count, LNK_Obj **objs, LNK_Section *sect, LNK_Config *config, uint64 *obj_sect_idxs_count_out)
{
  uint64 max_contribs = 0;
  for (LNK_SectionContribChunk *chunk = sect->contribs.first; chunk != 0; chunk = chunk->next) {
    max_contribs += chunk->count;
  }

  uint64      obj_sect_idxs_count = 0;
  Pairuint32 *obj_sect_idxs       = push_array(arena, Pairuint32, max_contribs);
  for (uint64 obj_idx = 0; obj_idx < objs_count; obj_idx += 1) {
  ...
  uint64 pop_size = (max_contribs - obj_sect_idxs_count) * sizeof(obj_sect_idxs[0]);
...
lnk_coff_section_header_from_obj_sect_idx_pair(LNK_Obj **objs, Pairuint32 p)
{
...
internal uint64
lnk_pair_u32_nearest_section(Pairuint32 *arr, uint64 count, LNK_Obj **objs, uint32 voff)
{
...
internal String8List
lnk_build_rad_map(Arena *arena, String8 image_data, LNK_Config *config, uint64 objs_count, LNK_Obj **objs, LNK_LibList lib_index[LNK_InputSource_Count], LNK_SectionTable *sectab)
{
  ProfBeginFunction();
  Temp scratch = scratch_begin(&arena, 1);
  ...
  for (LNK_SectionNode *sect_n = sectab->list.first; sect_n != 0; sect_n = sect_n->next) {
    ...
    uint64      obj_sect_idxs_count = 0;
    Pairuint32 *obj_sect_idxs       = lnk_obj_sect_idx_from_section(scratch.arena, objs_count, objs, sect, config, &obj_sect_idxs_count);
    g_rad_map_objs = objs;
    radsort(obj_sect_idxs, obj_sect_idxs_count, lnk_obj_sect_idx_is_before);

    uint64 global_sc_idx = 0;
    for (LNK_SectionContribChunk *sc_chunk = sect->contribs.first; sc_chunk != 0; sc_chunk = sc_chunk->next) {
      for (uint64 sc_idx = 0; sc_idx < sc_chunk->count; sc_idx += 1, global_sc_idx += 1) {
        Temp temp = temp_begin(scratch.arena);
        LNK_SectionContrib *sc = sc_chunk->v[sc_idx];

        uint64        file_off   = image_section_table[sc->u.sect_idx+1]->foff + sc->u.off;
        uint64        virt_off   = image_section_table[sc->u.sect_idx+1]->voff + sc->u.off;
        uint64        virt_size  = lnk_size_from_section_contrib(sc);
        uint64        file_size  = lnk_size_from_section_contrib(sc);
        String8    sc_data    = str8_substr(image_data, rng_1u64(file_off, file_off + virt_size));

        LNK_Obj *obj      = 0;
        uint32      sect_idx = 0;
        uint64 obj_sect_idx_idx = lnk_pair_u32_nearest_section(obj_sect_idxs, obj_sect_idxs_count, objs, virt_off);
        ...
        if (~sect->flags & COFF_SectionFlag_CntUninitializedData) {
          ...
          blake3_hasher_finalize(&hasher, (uint8 *)&sc_hash, sizeof(sc_hash));
        }
  ...
  ProfBegin("LIBS");
  for (uint64 input_source = 0; input_source < LNK_InputSource_Count; ++input_source) {
