...
lnk_blake3_hash_parallel(TP_Context *tp, U64 chunk_count, String8 data)
{
  ...
  task.hashes           = new U128[chunk_count];
...
lnk_serialize_pe_resource_tree(COFF_ObjWriter *obj_writer, PE_ResourceDir *root_dir)
{
  ...
  struct Stack *stack = new struct Stack[1];
  // init stack
  {
    PE_Resource *root_wrapper = new PE_Resource[1];
    ...
    COFF_ResourceDirEntry *root_dir = new COFF_ResourceDirEntry[1];
    ...
    for (; stack->arr_idx < ArrayCount(stack->res_arr); stack->arr_idx += 1) {
      for (; stack->res_idx[stack->arr_idx] < stack->res_arr[stack->arr_idx].count; ) {
        ...
        case PE_ResDataKind_DIR: {
          ...
          COFF_ResourceDirTable *dir_header = new COFF_ResourceDirTable[1];
          ...
          COFF_ResourceDirEntry *named_entries = new COFF_ResourceDirEntry[named_array.count];
          COFF_ResourceDirEntry *id_entries    = new COFF_ResourceDirEntry[id_array.count];
          ...
          struct Stack *frame      = new struct Stack[1];
        ...
        case PE_ResDataKind_COFF_RESOURCE: {
          ...
          COFF_ResourceDataEntry *coff_res = new COFF_ResourceDataEntry[1];
                 ...
                 String8           obj_name)
{
  ...
  // load res files
  PE_ResourceDir *root_dir       = new PE_ResourceDir[1];
  MD5Hash        *res_hash_array = new MD5Hash[res_data_list.node_count];
  U64 node_idx = 0;
                      ...
                      TP_TaskFunc     *task_func)
{
  ...
  task.result_arr        = new LNK_SymbolFinderResult[tp->worker_count];
...
#define state_list_push(a, l, s) do {                          \
  struct StateNode *node = new struct StateNode[1]; \
  node->state = s;                                             \
    ...
    while (state_list.count) {
      ...
      case State_InputImports: {
        ...
        for (LNK_InputImportNode *input = input_import_list.first; input != 0; input = input->next) {
          ...
          if (import_symbols == 0) {
            import_symbols = new String8List[1];
...
{
  LNK_SymbolTableFixupNode *node = new LNK_SymbolTableFixupNode[1];
...
{
  LNK_SymbolTableFixup *result = new LNK_SymbolTableFixup[list.count];
  U64 idx = 0;
...
THREAD_POOL_TASK_FUNC(lnk_gather_symtab_fixups_task)
{
  ...
  LNK_Symbol **symlinks = new LNK_Symbol *[obj->header.section_count_no_null+1];
  {
      ...
      if (interp == COFF_SymbolValueInterp_Regular) {
        ...
        if (symlink && symlink->u.defined.obj != obj) {
          if (symbol.storage_class == COFF_SymStorageClass_External) {
            ...
            LNK_SymbolTableFixup *fixup = lnk_symbol_table_fixup_list_push(scratch.arena, &fixups);
            fixup->idx            = symbol_idx;
            fixup->obj_idx        = max_U32;
            fixup->obj_symbol_idx = max_U32;
        ...
        for (;;) {
          ...
          if (lookup_free_list) {
            ...
            loc = new struct LookupLocation[1];
          }
          loc->obj                   = lookup_obj;
...
THREAD_POOL_TASK_FUNC(lnk_gather_section_definitions_task)
{
  ...
  for (U64 sect_idx = 0; sect_idx < obj->header.section_count_no_null; sect_idx += 1) {
    ...
    if (~sect_header->flags & COFF_SectionFlag_LnkRemove && sect_header->fsize > 0) {
      ...
      if (sect_defn == 0) {
        sect_defn = new LNK_SectionDefinition[1];
          ...
          if (is_addr32 || is_addr64) {
            ...
            LNK_BaseRelocPageNode *page;
            {
              ...
              if (is_page_present) {
                ...
                // fill out page
                page = new LNK_BaseRelocPageNode[1];
...
lnk_build_base_relocs(TP_Context *tp, TP_Arena *tp_arena, LNK_Config *config, U64 objs_count, LNK_Obj **objs)
{
  ...
  {
    LNK_BaseRelocPageList  *page_list_arr = new LNK_BaseRelocPageList[tp->worker_count];
    HashTable             **page_ht_arr   = /* no zero */ new HashTable *[tp->worker_count];
    for (U64 i = 0; i < tp->worker_count; ++i) {
    ...
    page_arr.count = 0;
    page_arr.v     = /* no zero */ new LNK_BaseRelocPage[main_page_list->count];
...
lnk_build_win32_header(Arena *arena, LNK_SymbolTable *symtab, LNK_Config *config, LNK_SectionArray sects, U64 expected_image_header_size)
{
  ...
  {
    PE_DosHeader *dos_header          = new PE_DosHeader[1];
  ...
  *pe_magic = PE_MAGIC;
  ...
  {
    directory_array = new PE_DataDirectory[config->data_dir_count];
  ...
  // COFF section table
  //
  COFF_SectionHeader *coff_section_table       = new COFF_SectionHeader[sects.count];
  ...
  // align image headers
  {
    ...
    U8 *image_headers_align      = new U8[image_headers_align_size];
  ...
  //
  {
    ...
    COFF_SectionHeader **section_table = new COFF_SectionHeader *[coff_section_table_count + 1];
...
lnk_build_image(TP_Arena *arena, TP_Context *tp, LNK_Config *config, LNK_SymbolTable *symtab, U64 objs_count, LNK_Obj **objs)
{
  ...
  */
  ...
  task.symtab             = symtab;
  task.sectab             = sectab;
  task.null_sc            = new LNK_SectionContrib[1];
  task.obj_symtab_fixups  = new LNK_SymbolTableFixupArray[1];
...

  {
    ...
    task.u.gather_sects.defns = new HashTable *[tp->worker_count];
  ...
  U64 expected_image_header_size;
  {
    ...
    task.sect_map = new LNK_SectionContrib **[objs_count];
    for (U64 obj_idx = 0; obj_idx < objs_count; obj_idx += 1) task.sect_map[obj_idx] = new LNK_SectionContrib *[objs[obj_idx]->header.section_count_no_null];
    ProfEnd();
    ...
    // ensure determinism by sorting section contribs in chunks by input index
    {
      ...
      LNK_SectionContribChunk **chunks            = 0;
      {
        ...
        U64 cursor = 0;
        chunks = new LNK_SectionContribChunk *[total_chunk_count];
        for (LNK_SectionNode *sect_n = sectab->list.first; sect_n != 0; sect_n = sect_n->next) {
    ...
    LNK_CommonBlockContrib *common_block_contribs;
    {
      ...
      task.u.common_block.counts = new U64[tp->worker_count];
      ...
      common_block_contribs       = new LNK_CommonBlockContrib[common_block_contribs_count];
      ProfEnd();
    ...
    // patch symbol tables
    {
      ...
      task.u.patch_symtabs.was_symbol_patched = new B8 *[objs_count];
      for (U64 obj_idx = 0; obj_idx < objs_count; obj_idx += 1) {
        task.u.patch_symtabs.was_symbol_patched[obj_idx] = new B8[objs[obj_idx]->header.symbol_count];
      }
  ...
  String8 image_data = {0};
  {
    ...
    image_data.str  = /* no zero */ new U8[image_size];
...
lnk_obj_sect_idx_from_section(Arena *arena, U64 objs_count, LNK_Obj **objs, LNK_Section *sect, LNK_Config *config, U64 *obj_sect_idxs_count_out)
{
  ...
  PairU32 *obj_sect_idxs       = new PairU32[max_contribs];
  for (U64 obj_idx = 0; obj_idx < objs_count; obj_idx += 1) {
...
lnk_run(TP_Context *tp, TP_Arena *tp_arena, LNK_Config *config)
{
  ...
  LNK_WriteThreadContext *image_write_ctx = new LNK_WriteThreadContext[1];
