...
internal LNK_ObjNodeArray      lnk_obj_list_push_parallel(TP_Context *tp, TP_Arena *tp_arena, LNK_ObjList *obj_list, COFF_MachineType machine, uint64 input_count, LNK_InputObj **inputs);
...
internal uint32 lnk_obj_get_features(LNK_Obj *obj);
internal uint32 lnk_obj_get_comp_id(LNK_Obj *obj);
internal uint32 lnk_obj_get_vol_md(LNK_Obj *obj);

// --- Symbol & Section Helpers ------------------------------------------------

internal COFF_ParsedSymbol    lnk_parsed_symbol_from_coff(LNK_Obj *obj, void *coff_symbol);
internal COFF_ParsedSymbol    lnk_parsed_symbol_from_coff_symbol_idx(LNK_Obj *obj, uint64 symbol_idx);
internal COFF_SectionHeader * lnk_coff_section_header_from_section_number(LNK_Obj *obj, uint64 section_number);
internal B32                  lnk_is_coff_section_debug(LNK_Obj *obj, uint64 sect_idx);

// --- Helpers ----------------------------------------------------------------- 

internal String8List * lnk_collect_obj_sections(TP_Context *tp, TP_Arena *arena, uint64 objs_count, LNK_Obj **objs, String8 name, B32 collect_discarded);
