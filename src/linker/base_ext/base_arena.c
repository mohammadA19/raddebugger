...
{
  U32 *result = arena.PushArrayNoZero<U32>(1);
  *result = value;
  return result;
}
...
{
  U64 *result = arena.PushArrayNoZero<U64>(1);
  *result = value;
  return result;
}
...
{
  U32 *result = arena.PushArrayNoZero<U32>(count);
...
{
  U64 *result = arena.PushArrayNoZero<U64>(count);
  MemoryCopyTyped(result, v, count);
  return result;
}
...
alloc_fixed_size_arena_array(Arena *arena, U64 count, U64 res, U64 cmt)
{
  ...
  U8 *data = arena.PushArrayNoZero<U8>(data_size);
