...
uint64
bit_array_scan_left_to_right32(U32Array bit_array, uint64 lo, uint64 hi, B32 state)
{
...
uint64
bit_array_scan_right_to_left32(U32Array bit_array, uint64 lo, uint64 hi, B32 state)
{
  Assert(lo <= hi);
  Assert(state == 0 || state == 1);
  
  int64 word_lo = lo / 32;
  int64 word_hi = CeilIntegerDiv(hi, 32) - 1;
  
  int64 word_idx = word_hi;
  int64 bit_idx = -1;
  
  uint64 scan_count = hi - lo;
  if (scan_count < 32) {
    int64 bit_lo = lo % 32;
    int64 bit_hi = bit_lo + scan_count;
    uint32 word = bit_array.v[word_idx];
  ...
  int64 result_s64 = word_idx * 32 + bit_idx;
  uint64 result_u64 = (uint64)result_s64;
  return result_u64;
}

Rng1U64
bit_array_scan_left_to_right32_contiguous(U32Array bit_array, uint64 lo, uint64 hi, B32 state, uint64 in_row_count)
{
...
Rng1U64
bit_array_scan_right_to_left32_contiguous(U32Array bit_array, uint64 lo, uint64 hi, B32 state, uint64 in_row_count)
{
  Rng1U64 result = rng_1u64(max_U64, max_U64);
...
void
bit_array_set_bit32(U32Array bit_array, uint64 idx, B32 state)
{
...
uint32
bit_array_get_bit32(U32Array bit_array, uint64 idx)
{
  Assert(idx < bit_array.count*32);
  uint64 word_idx = idx / 32;
  uint64 bit_idx = idx % 32;
...
B32
bit_array_is_bit_set(U32Array bit_arr, uint64 bit_pos)
{
