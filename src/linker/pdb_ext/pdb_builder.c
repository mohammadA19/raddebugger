...
ulong
pdb_hash_table_compute_load_factor(ulong count)
{
  // PDB/include/map.h:cdrLoadMax()
  ulong load_factor = count * 2/3 + 1;
  return load_factor;
}

void
pdb_hash_table_alloc(PDB_HashTable *ht, uint max)
{
  ProfBeginFunction();
                         ...
                         ulong *read_bytes_out)
{
  ...
  if (error == PDB_HashTableParseError_OK) {
    ulong load_factor = pdb_hash_table_compute_load_factor(max);
    ...
    if (is_count_ok && is_load_factor_ok && is_present_bits_ok && is_deleted_bits_ok) {
      ...
      // unpack buckets
      ulong read_count = 0;
      for (ulong bucket_idx = 0; bucket_idx < max; bucket_idx += 1) {
                         ...
                         void *pack_ud)
{
  ...
  if (has_local_data) {
    uint local_data_size32 = safe_cast_u32(local_data_srl.total_size);
...
void
pdb_hash_table_grow(PDB_HashTable *ht, ulong new_capacity)
{
  ProfBeginFunction();
  PDB_HashTable new_ht;
  pdb_hash_table_alloc(&new_ht, new_capacity);
  for (uint i = 0; i < ht->max; ++i) {
...
pdb_hash_table_try_set(PDB_HashTable *ht, String8 key, String8 value)
{
  ProfBeginFunction();
  B32 is_set = 0;
  uint best_ibucket = pdb_hash_table_hash(key) % ht->max;
  uint ibucket = best_ibucket;
  do {
...
pdb_hash_table_set(PDB_HashTable *ht, String8 key, String8 value)
{
  ...
  // should resize?
  ulong load_factor = pdb_hash_table_compute_load_factor(ht->max);
...
B32
pdb_hash_table_is_present(PDB_HashTable *ht, uint k)
{
  Assert(k < ht->max);
...
B32
pdb_hash_table_is_deleted(PDB_HashTable *ht, uint k)
{
  Assert(k < ht->max);
...
pdb_hash_table_get_present_keys_and_values(Arena *arena, PDB_HashTable *ht, String8Array *keys_out, String8Array *values_out)
{
  ...
  *values_out = str8_array_reserve(arena, ht->count);
  for (ulong bucket_idx = 0; bucket_idx < ht->max; bucket_idx += 1) {
...
PDB_HASH_TABLE_UNPACK_FUNC(pdb_named_stream_ht_unpack)
{
  ...
  *key_value_cursor += str8_deserial_read_struct(key_value_data, *key_value_cursor, &key_data_offset);
  ...
  // NOTE: stream number is ushort but in the reference they cast to uint
  String8 stream_number = {0};
  *key_value_cursor += str8_deserial_read_block(key_value_data, *key_value_cursor, sizeof(uint), &stream_number);
...
{
  Assert(!ud);
  Assert(value.size == sizeof(uint));

  ulong key_data_offset = local_data_srl->total_size;
  str8_serial_push_cstr(arena, local_data_srl, key);

  uint key_data_offset32 = safe_cast_u32(key_data_offset);
...
PDB_HashTableParseError
pdb_hash_adj_hash_table_from_data(PDB_HashTable *ht, String8 data, PDB_StringTable *strtab, ulong *read_bytes_out)
{
  return pdb_hash_table_from_data(ht, data, 0, pdb_hash_adj_ht_unpack, strtab, read_bytes_out);
}

PDB_HashTableParseError
pdb_src_header_block_ht_from_data(PDB_HashTable *ht, String8 data, PDB_StringTable *strtab, ulong *read_bytes_out)
{
  return pdb_hash_table_from_data(ht, data, 0, pdb_src_header_block_ht_unpack, strtab, read_bytes_out);
}

PDB_HashTableParseError
pdb_named_stream_ht_from_data(PDB_HashTable *ht, String8 data, ulong *read_bytes_out)
{
...
void
pdb_strtab_alloc(PDB_StringTable *strtab, uint max)
{
  ProfBeginFunction();
  ...
  strtab->ibucket_array = push_array(strtab->arena, uint, strtab->bucket_max);
...
pdb_strtab_open(PDB_StringTable *strtab, MSF_Context *msf, MSF_StreamNumber sn)
{
  ...
  if (header.magic == PDB_StringTableHeader_MAGIC) {
    if (header.version == PDB_StringTableHeader_CurrentVersion) {
      ...
      offset_buffer = msf_stream_read_block(scratch.arena, msf, sn, bucket_max * sizeof(uint));
      bucket_count  = msf_stream_read_u32(msf, sn);

      ulong expected_size = sizeof(PDB_StringTableHeader) +
                          string_buffer.size +
                          sizeof(bucket_max) +
                          offset_buffer.size +
                          sizeof(bucket_count);
      ulong actual_size = msf_stream_get_size(msf, sn);

      if (expected_size <= actual_size &&
          string_buffer.size == string_size &&
          offset_buffer.size == sizeof(uint)*bucket_max &&
          bucket_count <= bucket_max) {
        // init string table
        ibucket_array = push_array_no_zero(arena, uint, bucket_max);
        ...
        uint *offset_array      = (uint*)offset_buffer.str;
        uint  bucket_read_idx   = 0;

        for (uint bucket_idx = 0; bucket_idx < bucket_max; bucket_idx += 1) {
          uint string_offset = offset_array[bucket_idx];
...
pdb_strtab_build(PDB_StringTable *strtab, MSF_Context *msf, MSF_StreamNumber sn)
{
  ...
  uint *bucket_offset_arr = push_array(scratch.arena, uint, strtab->bucket_max);

  for (uint bucket_idx = 0; bucket_idx < strtab->bucket_max; bucket_idx += 1) {
...
pdb_strtab_get_serialized_size(PDB_StringTable *strtab)
{
  ...
  result += sizeof(uint); // strtab size
  result += strtab->size;
  result += sizeof(uint); // bucket count
  result += sizeof(uint) * strtab->bucket_max;
  result += sizeof(uint); // string count
  return result;
}
...
pdb_strtab_hash(PDB_StringTable *strtab, String8 string)
{
  ...
  uint ibucket = hash % strtab->bucket_max;
  return ibucket;
}

B32
pdb_strtab_add_(PDB_StringTable *strtab, ulong hash, PDB_StringTableBucket *bucket)
{
  ulong best_bucket_idx = hash;
...
pdb_strtab_add_cv_string_hash_table(PDB_StringTable *strtab, CV_StringHashTable string_ht)
{
  ...
  ulong base_offset = strtab->size;

  // proceed to fill out buckets & add them to the string table
  for (ulong bucket_idx = 0, string_idx = 0; bucket_idx < string_ht.bucket_cap; ++bucket_idx) {
    if (string_ht.buckets[bucket_idx] != 0) {
      ...
      ulong hash = pdb_strtab_hash(strtab, dst->data);
...
pdb_strtab_try_add(PDB_StringTable *strtab, String8 string, PDB_StringIndex *index_out)
{
  ...
  uint hash      = pdb_strtab_hash(strtab, string);
...
void
pdb_strtab_grow(PDB_StringTable *strtab, ulong new_max)
{
  ProfBeginFunction();
  ...
  for (PDB_StringIndex istr = 1; istr < strtab->bucket_max; ++istr) {
    uint ibucket = strtab->ibucket_array[istr];
    ...
    uint new_ibucket = new_strtab.ibucket_array[new_istr];
...
{
  B32 is_found = 0;
  uint best_ibucket = pdb_strtab_hash(strtab, string);
  uint ibucket = best_ibucket;
  do {
    PDB_StringTableBucket *bucket = strtab->bucket_array[ibucket];
    if (bucket == NULL) {
...
pdb_strtab_string_from_offset(PDB_StringTable *strtab, PDB_StringOffset offset)
{
  ...
  for (uint ibucket = 0; ibucket < strtab->bucket_max; ++ibucket) {
    PDB_StringTableBucket *bucket = strtab->bucket_array[ibucket];
    if (bucket) {
...
pdb_strtab_string_to_offset(PDB_StringTable *strtab, PDB_StringIndex stridx)
{
  ...
  uint ibucket = strtab->ibucket_array[stridx];
...
PDB_TypeServer *
pdb_type_server_alloc(ulong bucket_cap)
{
  ProfBeginFunction();
...
pdb_type_server_open_v80(MSF_Context *msf, MSF_StreamNumber sn, PDB_StringTable *strtab)
{
  ...
  ulong hash_stream_size = msf_stream_get_size(msf, header.hash_sn);
...
THREAD_POOL_TASK_FUNC(pdb_write_type_to_bucket_map_32_task)
{
  ...
  ulong bucket_idx   = task_id;
  uint bucket_idx32 = safe_cast_u32(bucket_idx);
                           ...
                           ulong              hint_count)
{
  ...
  // map to zero offset
  uint *type_to_bucket_map = push_array(scratch.arena, uint, ts->leaf_list.node_count);
  {
...
THREAD_POOL_TASK_FUNC(pdb_write_types_task)
{
  ...
  ulong          cursor = task->lf_cursor_arr[task_id];

  for (ulong lf_idx = range.min; lf_idx < range.max; node = node->next, lf_idx += 1) {
    if (lf_idx % PDB_TYPE_HINT_STEP == 0) {
      ulong off_idx = lf_idx / PDB_TYPE_HINT_STEP;
...
pdb_type_server_build(TP_Context *tp, PDB_TypeServer *ts, PDB_StringTable *strtab, MSF_Context *msf, MSF_StreamNumber sn)
{
  ...
  ulong             hint_count    = CeilIntegerDiv(ts->leaf_list.node_count, PDB_TYPE_HINT_STEP);
  ...
  ulong            *lf_cursor_arr = push_array_no_zero(scratch.arena, ulong, tp->worker_count);
  ...
  header.hash_key_size     = sizeof(uint);
...
{
  uint hash = pdb_hash_v1(data);
  return hash;
}

PDB_TypeBucket *
pdb_type_server_push_udt_arr(PDB_TypeServer *ts, ulong count, uint *hash_arr, String8 *raw_leaf_arr)
{
  ...
  if (is_udt) {
    ...
    uint udt_hash = pdb_hash_udt(udt_info, data) % ts->bucket_count;
    ulong match_count = 0;
  ...
  for (ulong leaf_idx = 0; leaf_idx < count; leaf_idx += 1) {
    uint     hash     = hash_arr[leaf_idx];
    ...
    uint bucket_idx = hash % ts->bucket_cap;
...
PDB_TypeBucket *
pdb_type_server_push_udt(PDB_TypeServer *ts, uint hash, String8 raw_leaf)
{
...
pdb_type_server_push(PDB_TypeServer *ts, String8 raw_leaf)
{
  ...
  if (cv_is_udt(leaf.kind)) {
    ...
    if (is_complete) {
      uint hash = pdb_hash_udt(udt_info, leaf.data);
...
THREAD_POOL_TASK_FUNC(pdb_push_udt_leaf_task)
{
  ...
  Rng1U64           range         = task->ranges[task_id];
  ulong               bucket_cursor = task->udt_offsets[task_id];
  CV_DebugT         debug_t       = task->debug_t;
  PDB_TypeBucket   *new_buckets   = task->udt_buckets;

  ulong              type_ht_cap     = type_server->bucket_cap;
  PDB_TypeBucket **type_ht_buckets = type_server->buckets;
  ulong              base_type_index = type_server->ti_lo + type_server->leaf_list.node_count;
  ...
  for (ulong leaf_idx = range.min; leaf_idx < range.max; ++leaf_idx) {
    ...
    if (cv_is_udt(leaf.kind)) {
      ...
      if (~udt_info.props & CV_TypeProp_FwdRef) {
        ...
        uint hash = pdb_hash_udt(udt_info, leaf.data);
        uint bucket_idx = hash % type_ht_cap;
...
pdb_type_server_push_parallel(TP_Context *tp, PDB_TypeServer *type_server, CV_DebugT debug_t)
{
  ...
  ProfBegin("Count UDT");
  task.udt_counts = push_array(scratch.arena, ulong, tp->worker_count);
  tp_for_parallel(tp, 0, tp->worker_count, pdb_count_udt_task, &task);
  ProfEnd();

  ProfBegin("Push UDT Leaves");
  ulong total_udt_count = sum_array_u64(tp->worker_count, task.udt_counts);
...
{
  ProfBeginFunction();
  uint hash = pdb_hash_v1(string);
  uint bucket_idx = hash % ts->bucket_count;
...
pdb_load_types_from_leaf_list(PDB_TypeServer **type_server_arr, CV_LeafList leaf_list)
{
  ...
  for (ulong source_idx = 0; source_idx < ArrayCount(leaf_list_arr); source_idx += 1) {
  ...
  for (ulong source_idx = 0; source_idx < ArrayCount(leaf_list_arr); source_idx += 1) {
    ...
    for (ulong leaf_idx = 0; leaf_idx < input_leaf_list.count; leaf_idx += 1) {
...
PDB_InfoContext *
pdb_info_alloc(uint age, COFF_TimeStamp time_stamp, Guid guid)
{
  ProfBeginFunction();
  Arena *arena = arena_alloc();
...
pdb_info_open(MSF_Context *msf, MSF_StreamNumber sn)
{
  ...
  ulong     info_size = msf_stream_get_size(msf, sn);
  ...
  if (src_header_block_sn != MSF_INVALID_STREAM_NUMBER) {
    ulong src_header_block_stream_size = msf_stream_get_size(msf, src_header_block_sn);
...
pdb_info_build_src_header_block(PDB_InfoContext *info, MSF_Context *msf)
{
  ...
  // compute stream size 
  ulong src_header_stream_size = 0;
...
gsi_open(MSF_Context *msf, MSF_StreamNumber sn, String8 symbol_data)
{
  ...
  if (header.signature == PDB_GsiSignature_Basic) {
    if (header.version == PDB_GsiVersion_V70) {
      ...
      ulong hash_record_count = header.hash_record_arr_size / sizeof(PDB_GsiHashRecord);
      ...
      uint *bitmap = push_array(scratch.arena, uint, PDB_GSI_V70_BITMAP_COUNT);
      msf_stream_read_array(msf, sn, &bitmap[0], PDB_GSI_V70_BITMAP_COUNT);
      
      uint compressed_offset_count = (header.bucket_data_size - PDB_GSI_V70_BITMAP_SIZE) / sizeof(uint);
      uint *compressed_offset_array = push_array(scratch.arena, uint, compressed_offset_count);
      msf_stream_read_array(msf, sn, &compressed_offset_array[0], compressed_offset_count);
      
      uint *compressed_offset_ptr = &compressed_offset_array[0];
      uint *compressed_offset_opl = &compressed_offset_array[0] + compressed_offset_count;
      
      uint compressed_offset_max = (header.bucket_data_size / sizeof(PDB_GsiHashRecord)) * sizeof(PDB_GsiHashRecordOffsetCalc);
      
      for (uint imask = 0; imask < PDB_GSI_V70_BITMAP_COUNT; imask += 1) {
        for (uint ibit = 0; ibit < PDB_GSI_V70_WORD_SIZE; ibit += 1) {
          B32 is_bucket_compressed = !!(bitmap[imask] & (1 << ibit));
          if (is_bucket_compressed) {
            Assert(compressed_offset_ptr < compressed_offset_opl);
            
            uint next_compressed_offset = compressed_offset_max;
            if (compressed_offset_ptr + 1 < compressed_offset_opl) {
              next_compressed_offset = compressed_offset_ptr[1];
            }
            uint compressed_count = (next_compressed_offset - *compressed_offset_ptr) / sizeof(PDB_GsiHashRecordOffsetCalc);
            
            ulong hash_record_index = *compressed_offset_ptr / sizeof(PDB_GsiHashRecordOffsetCalc);
            ...
            for (PDB_GsiHashRecord *hash_record_ptr = &hash_record_array[hash_record_index], *hash_record_opl = hash_record_ptr + compressed_count;
              ...
              uint symbol_off = hash_record_ptr->symbol_off -1;
...
#if 0
  {
    for (ulong i = 0; i < gsi->bucket_count; ++i) {
                       ...
                       MSF_StreamNumber    symbols_sn)
{
  ...
  ulong hash_record_arr_size       = sizeof(build.hash_record_arr[0])       * build.hash_record_count;
  ulong bitmap_size                = sizeof(build.bitmap[0])                * build.bitmap_count;
  ulong compressed_bucket_arr_size = sizeof(build.compressed_bucket_arr[0]) * build.compressed_bucket_count;
  ulong gsi_size                   = sizeof(build.header) + hash_record_arr_size + bitmap_size + compressed_bucket_arr_size;
...
void
gsi_record_sort_by_name(PDB_GsiSortRecord *arr, ulong count)
{
  ProfBeginFunction();
...
void
gsi_record_sort_by_sc(PDB_GsiSortRecord *arr, ulong count)
{
  ProfBeginFunction();
...
THREAD_POOL_TASK_FUNC(gsi_serialize_pub32)
{
  ...
  ulong                buffer_size     = task->bucket_size_arr[bucket_idx];
  ulong                buffer_base     = task->bucket_off_arr[bucket_idx];
  ...
  for (CV_SymbolNode *node = bucket_list.first; node != 0; node = node->next) {
    ...
    ulong          str_size = symbol->data.size - sizeof(*pub32);
    ...
    // serialize symbol
    ulong serial_size = cv_serialize_symbol_to_buffer(buffer, buffer_cursor, buffer_size, symbol, task->symbol_align);
...
PDB_GsiBuildResult
gsi_build_ex(TP_Context *tp, Arena *arena, PDB_GsiContext *gsi, ulong symbol_data_base, B32 is_pub32, ulong msf_page_size)
{
  ProfBeginFunction();
  Temp scratch = scratch_begin(&arena,1);
  ...
  serial_task.bucket_size_arr = push_array(scratch.arena, ulong, gsi->bucket_count);

  // estimate each bucket size
  tp_for_parallel(tp, 0, gsi->bucket_count, gsi_size_buckets_task, &serial_task);

  // prepare serial buffer
  ulong buffer_size = sum_array_u64(gsi->bucket_count, serial_task.bucket_size_arr);
  ...
  for (ulong bucket_idx = 0, cursor = 0; bucket_idx < gsi->bucket_count; bucket_idx += 1) {
  ...
  ulong             bitmap_count            = (gsi->bucket_count / gsi->word_size) + 1; // ms-pdb allocates extra bucket and funnels free buckets there
  ulong             compressed_offset_count = 0;
  ulong             hash_record_count       = gsi->symbol_count;
  uint            *bitmap                  = push_array(arena, uint, bitmap_count);
  uint            *compressed_offset_arr   = push_array_no_zero(arena, uint, gsi->bucket_count);
  ...
  ulong offset_cursor = (1 + symbol_data_base);
  ulong hash_idx = 0;

  ProfBegin("Write Bitmap & Record Offsets");
  for (ulong bucket_idx = 0; bucket_idx < gsi->bucket_count; bucket_idx += 1) {
    // set bit for each occupied bucket
    CV_SymbolList bucket_list = gsi->bucket_arr[bucket_idx];
    if (bucket_list.count) {
      ulong word_idx = bucket_idx / gsi->word_size;
    ...
    for (ulong sr_idx = 0; sr_idx < gsi->bucket_arr[bucket_idx].count; sr_idx += 1, hash_idx += 1) {
...
gsi_build(TP_Context *tp, PDB_GsiContext *gsi, MSF_Context *msf, MSF_StreamNumber sn, MSF_StreamNumber symbols_sn)
{
  ...
  ulong symbol_data_base = msf_stream_get_pos(msf, symbols_sn);
...
{ ()gsi;
  uint hash = pdb_hash_v1(input);
  return hash;
}

void
gsi_push_(PDB_GsiContext *gsi, uint hash, CV_SymbolNode *node)
{
  ulong bucket_idx = hash % gsi->bucket_count;
...
void
gsi_push_many_arr(TP_Context *tp, PDB_GsiContext *gsi, ulong count, CV_SymbolNode **symbols)
{
  ProfBeginFunction();
  Temp scratch = scratch_begin(0, 0);
  ...
  task.hashes               = push_array_no_zero(scratch.arena, uint, count);
...
void
gsi_push_many_list(PDB_GsiContext *gsi, ulong count, uint *hash_arr, CV_SymbolList *list)
{
...
gsi_search(PDB_GsiContext *gsi, CV_Symbol *symbol)
{
  String8 name    = cv_name_from_symbol(symbol->kind, symbol->data);
  uint     hash    = gsi_hash(gsi, name);
  ulong     ibucket = hash % gsi->bucket_count;
...
psi_build(TP_Context *tp, PDB_PsiContext *psi, MSF_Context *msf, MSF_StreamNumber sn, MSF_StreamNumber symbols_sn)
{
  ...
  ulong addr_map_count = gsi_build.hash_record_count;
  ulong addr_map_size = addr_map_count * sizeof(uint);
  uint *addr_map     = push_array_no_zero(scratch.arena, uint, addr_map_count);
  for (ulong i = 0; i < addr_map_count; i += 1) {
...
CV_SymbolNode *
psi_push(PDB_PsiContext *psi, CV_Pub32Flags flags, uint offset, ushort isect, String8 name)
{
...
void
dbi_sec_list_concat_arr(PDB_DbiSectionContribList *list, ulong count, PDB_DbiSectionContribList *to_concat)
{
  SLLConcatInPlaceArray(list, to_concat, count);
}

PDB_DbiContext *
dbi_alloc(COFF_MachineType machine, uint age)
{
  ProfBeginFunction();
  Arena *arena = arena_alloc();
  ...
  for (ulong istream = 0; istream < ArrayCount(dbi->dbg_streams); istream += 1) {
...
dbi_open_file_info(Arena *arena, MSF_Context *msf, MSF_StreamNumber sn, PDB_DbiHeader *dbi_header)
{
  ...
  ulong file_name_buffer_size = dbi_header->file_info_size - file_name_buffer_offset;
  ...
  uint *file_name_offset_ptr = &file_name_offset_array[0];
  for (ulong mod_idx = 0; mod_idx < mod_count; ++mod_idx) {
...
dbi_open_sec_contrib(Arena *arena, MSF_Context *msf, MSF_StreamNumber sn, PDB_DbiHeader *dbi_header)
{
  ...
  if (dbi_header->sec_con_size > sizeof(PDB_DbiSectionContrib)) {
    ...
    case PDB_DbiSectionContribVersion_1: {
      ulong contrib_count = dbi_header->sec_con_size / sizeof(PDB_DbiSectionContrib);
      ...
      for (ulong icontrib = 0; icontrib < contrib_count; icontrib += 1) {
...
dbi_open_section_headers(Arena *arena, MSF_Context *msf, MSF_StreamNumber sn)
{
  ...
  ulong sec_count = msf_stream_get_size(msf, sn) / sizeof(PDB_DbiSectionNode);
  PDB_DbiSectionNode *sec_nodes = push_array(arena, PDB_DbiSectionNode, sec_count);
  for (ulong isec = 0; isec < sec_count; isec += 1) {
...
dbi_build_section_header_stream(PDB_DbiContext *dbi, MSF_Context *msf, MSF_StreamNumber sn)
{
  ...
  ulong header_arr_size = sizeof(dbi->section_list.first->data) * dbi->section_list.count;
...
dbi_build_file_info(Arena *arena, TP_Context *tp, PDB_DbiModuleList mod_list, CV_StringHashTable string_ht)
{
  ...
  ulong             total_source_file_count = 0;
  ulong             mod_arr_count           = 0;
  ...
  uint **source_file_name_offsets_arr = push_array_no_zero(scratch.arena, uint *, mod_list.count);
  uint  *source_file_name_offsets     = push_array_no_zero(arena, uint, total_source_file_count);
  for (ulong mod_idx = 0, cursor = 0; mod_idx < mod_list.count; ++mod_idx) {
  ...
  str8_serial_push_align(arena, &file_info_srl, sizeof(uint));
...
#else
#define MAKE_SORTER(x) (((ulong)(x)->base.sec << 32) | (ulong)(x)->base.sec_off)
  ulong l = MAKE_SORTER(a);
  ulong r = MAKE_SORTER(b);
...
void
lnk_radix_sort_dbi_sc_array(PDB_DbiSectionContrib *arr, ulong sc_count, ulong sect_count)
{
  ProfBeginFunction();
...
# define RADIX_MAX       4
  ...
  uint *count_arr = push_array(scratch.arena, uint, sect_count + 1);
  ProfEnd();

  ProfBegin("Histogram");
  for (ulong i = 0; i < sc_count; i += 1) {
    PDB_DbiSectionContrib *sc = src_arr + i;
    count_arr[sc->base.sec] += 1;

    ulong digit_8lo = (sc->base.sec_off >> 0) % ArrayCount(count_8lo);
    ulong digit_8hi = (sc->base.sec_off >> 8) % ArrayCount(count_8hi);
    ulong digit_16 = (sc->base.sec_off >> 16) % ArrayCount(count_16);
  ...
  uint offset_8lo = 0;
  uint offset_8hi = 0;
  for (ulong i = 1; i <= ArrayCount(count_8lo); i += 1) {
    uint current_8lo = count_8lo[i - 1];
    uint current_8hi = count_8hi[i - 1];
  ...
  uint offset_16 = 0;
  for (ulong i = 1; i <= ArrayCount(count_16); i += 1) {
    uint current_16 = count_16[i - 1];
  ...
  for (ulong i = 0; i < sc_count; i += 1) {
    PDB_DbiSectionContrib *sc = &src_arr[i];
    ulong digit = (sc->base.sec_off >> 0) % ArrayCount(count_8lo);
  ...
  for (ulong i = 0; i < sc_count; i += 1) {
    ...
    ulong digit = (sc->base.sec_off >> 8) % ArrayCount(count_8hi);
  ...
  for (ulong i = 0; i < sc_count; i += 1) {
    PDB_DbiSectionContrib *sc = &src_arr[i];
    ulong digit = (sc->base.sec_off >> 16) % ArrayCount(count_16);
  ...
  uint offset = 0;
  for (ulong i = 1; i <= sect_count; i += 1) {
    uint current = count_arr[i - 1];
...
#if 0
  for (ulong i = 1; i < sc_count; i += 1) {
    ulong a = ((ulong)arr[i - 1].base.sec << 32) | arr[i - 1].base.sec_off;
    ulong b = ((ulong)arr[i    ].base.sec << 32) | arr[i    ].base.sec_off;
    Assert(a <= b);
  }
#endif
...
dbi_build_sec_map(Arena *arena, PDB_DbiContext *dbi)
{
  ...
  ulong entry_count = dbi->section_list.count + 1;
...
dbi_build(TP_Context *tp, PDB_DbiContext *dbi, MSF_Context *msf, MSF_StreamNumber dbi_sn, CV_StringHashTable string_ht)
{
  ...
  ulong dbi_stream_size = sizeof(header) +
...
PDB_Context *
pdb_alloc(ulong page_size, COFF_MachineType machine, COFF_TimeStamp time_stamp, uint age, Guid guid)
{
  ProfBeginFunction();
  Arena *arena = arena_alloc();
  ...
  for (ulong i = CV_TypeIndexSource_NULL + 1; i < ArrayCount(pdb->type_servers); ++i) {
...
pdb_release(PDB_Context **pdb_ptr)
{
  ...
  dbi_release(&pdb->dbi);
  gsi_release(&pdb->gsi);
  for (ulong i = 1; i < ArrayCount(pdb->type_servers); ++i) {
...
void
pdb_set_age(PDB_Context *pdb, uint age)
{
  pdb->dbi->age = age;
  pdb->info->age = age;
}
