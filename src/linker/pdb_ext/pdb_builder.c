                         ...
                         void *pack_ud)
{
  ...
  for (U64 i = 0; i < ht->count; i += 1) {
    string key   = key_arr.v[i];
    string value = value_arr.v[i];
  ...
  string data = str8_serial_end(arena, &srl);
...
internal U32
pdb_hash_table_hash(string key)
{
  return (U16)pdb_hash_v1(key);
}

internal B32
pdb_hash_table_try_set(PDB_HashTable *ht, string key, string value)
{
  ProfBeginFunction();
  B32 is_set = 0;
...
internal void
pdb_hash_table_set(PDB_HashTable *ht, string key, string value)
{
  ProfBeginFunction();
...
internal B32
pdb_hash_table_get(PDB_HashTable *ht, string key, string *value_out)
{
  ProfBeginFunction();
  B32 is_get_ok = 0;
  U32 best_ibucket = pdb_hash_table_hash(key) % ht->max;
...
internal void
pdb_hash_table_delete(PDB_HashTable *ht, string key)
{
  ProfBeginFunction();
  U32 best_ibucket = pdb_hash_table_hash(key) % ht->max;
  U32 ibucket = best_ibucket;
...
PDB_HASH_TABLE_UNPACK_FUNC(pdb_named_stream_ht_unpack)
{
  ...
  *key_value_cursor += str8_deserial_read_struct(key_value_data, *key_value_cursor, &key_data_offset);
  ...
  string stream_name = str8_cstring_capped(cstr_ptr, cstr_opl);

  // NOTE: stream number is U16 but in the reference they cast to U32
  string stream_number = {0};
...
PDB_HASH_TABLE_UNPACK_FUNC(pdb_hash_adj_ht_unpack)
{
  ...
  *key_value_cursor += str8_deserial_read_block(key_value_data, *key_value_cursor, sizeof(CV_TypeIndex), &type_index);
  ...
  string type_name = pdb_strtab_string_from_offset(strtab, string_offset);
...
PDB_HASH_TABLE_UNPACK_FUNC(pdb_src_header_block_ht_unpack)
{
  ...
  *key_value_cursor += str8_deserial_read_struct(key_value_data, *key_value_cursor, &path_offset);
  ...
  string src_header_block_entry = {0};
  *key_value_cursor += str8_deserial_read_block(key_value_data, *key_value_cursor, sizeof(PDB_SrcHeaderBlockEntry), &src_header_block_entry);
  ...
  PDB_StringTable *strtab = (PDB_StringTable*)ud;
  string path = pdb_strtab_string_from_offset(strtab, path_offset);
...
internal PDB_HashTableParseError
pdb_hash_adj_hash_table_from_data(PDB_HashTable *ht, string data, PDB_StringTable *strtab, U64 *read_bytes_out)
{
  return pdb_hash_table_from_data(ht, data, 0, pdb_hash_adj_ht_unpack, strtab, read_bytes_out);
}

internal PDB_HashTableParseError
pdb_src_header_block_ht_from_data(PDB_HashTable *ht, string data, PDB_StringTable *strtab, U64 *read_bytes_out)
{
  return pdb_hash_table_from_data(ht, data, 0, pdb_src_header_block_ht_unpack, strtab, read_bytes_out);
}

internal PDB_HashTableParseError
pdb_named_stream_ht_from_data(PDB_HashTable *ht, string data, U64 *read_bytes_out)
{
...
{
  string data = pdb_data_from_hash_table(arena, ht, 0, pdb_hash_adj_ht_pack, strtab);
  return data;
}
...
{
  string data = pdb_data_from_hash_table(arena, ht, 0, pdb_src_header_block_ht_pack, strtab);
  return data;
}
...
{
  string data = pdb_data_from_hash_table(arena, ht, 1, pdb_named_stream_ht_pack, 0);
  return data;
}
...
internal U32
pdb_strtab_hash(PDB_StringTable *strtab, string string)
{
  U32 hash = 0;
...
internal B32
pdb_strtab_try_add(PDB_StringTable *strtab, string string, PDB_StringIndex *index_out)
{
...
pdb_strtab_add(PDB_StringTable *strtab, string string)
{
...
internal B32
pdb_strtab_search(PDB_StringTable *strtab, string string, PDB_StringIndex *index_out)
{
  B32 is_found = 0;
...
pdb_type_server_parse_from_data_v80(string data, PDB_TypeServerParse *parse)
{
  ProfBeginFunction();
...
internal PDB_OpenTypeServerError
pdb_type_server_parse_from_data(string data, PDB_TypeServerParse *parse_out)
{
  PDB_OpenTypeServerError error = PDB_OpenTypeServerError_UNKNOWN;
...
pdb_type_server_open_v80(MSF_Context *msf, MSF_StreamNumber sn, PDB_StringTable *strtab)
{
  ...
  string types_data = msf_stream_read_block(ts->arena, msf, sn, header.leaf_data_size);
  ...
  for (U64 cursor = 0, leaf_idx = 0; 
    ...
    string raw_leaf = cv_debug_t_get_raw_leaf(debug_t, leaf_idx);
  ...
  string adjust_data = msf_stream_read_block(scratch.arena, msf, header.hash_sn, header.hash_adj.size);
                           ...
                           U64              hint_count)
{
  ...
  string hash_adj_data = pdb_data_from_hash_adj_hash_table(scratch.arena, &ts->hash_adj, strtab);
...
pdb_type_server_make_leaf(PDB_TypeServer *ts, CV_LeafKind kind, string data)
{
  ProfBeginFunction();

  string      leaf = cv_serialize_raw_leaf(ts->arena, kind, data, PDB_LEAF_ALIGN);
...
internal U32
pdb_type_server_hash(string data)
{
...
pdb_type_server_push_udt_arr(PDB_TypeServer *ts, U64 count, U32 *hash_arr, string *raw_leaf_arr)
{
  ...
  for (U64 leaf_idx = 0; leaf_idx < count; leaf_idx += 1) {
    ...
    string raw_leaf = raw_leaf_arr[leaf_idx];
...
internal PDB_TypeBucket *
pdb_type_server_push_udt(PDB_TypeServer *ts, U32 hash, string raw_leaf)
{
  return pdb_type_server_push_udt_arr(ts, 1, &hash, &raw_leaf);
}

internal void
pdb_type_server_push(PDB_TypeServer *ts, string raw_leaf)
{
  ProfBeginFunction();
...
pdb_type_server_leaf_from_string(PDB_TypeServer *ts, string string)
{
  ProfBeginFunction();
  ...
  for (PDB_TypeBucket *i = head_bucket; i != 0; i = i->next) {
    ...
    string leaf_name = cv_get_leaf_name(leaf->data.kind, leaf->data.data);
...
internal void
pdb_info_parse_from_data(string data, PDB_InfoParse *parse_out)
{
...
pdb_info_open(MSF_Context *msf, MSF_StreamNumber sn)
{
  ...
  string info_data = msf_stream_read_block(scratch.arena, msf, sn, info_size);
  ...
  if (src_header_block_sn != MSF_INVALID_STREAM_NUMBER) {
    ...
    string src_header_block_data = msf_stream_read_block(scratch.arena, msf, src_header_block_sn, src_header_block_stream_size);
...
pdb_info_build_src_header_block(PDB_InfoContext *info, MSF_Context *msf)
{
  ...
  string hash_table_data = pdb_data_from_src_header_block_ht(scratch.arena, &info->src_header_block_ht, &info->strtab);
...
pdb_info_build(PDB_InfoContext *info, MSF_Context *msf, MSF_StreamNumber sn)
{
  ...
  string named_stream_ht_data = pdb_data_from_named_stream_ht(scratch.arena, &info->named_stream_ht);
...
pdb_push_named_stream(PDB_HashTable *named_stream_ht, MSF_Context *msf, string name)
{
  ProfBeginFunction();
  Temp scratch = scratch_begin(0,0);
  MSF_StreamNumber sn = msf_stream_alloc(msf);
  string name_cstr = push_cstr(scratch.arena, name);
  U32 sn32 = (U32)sn;
...
internal MSF_StreamNumber
pdb_find_named_stream(PDB_HashTable *named_stream_ht, string name)
{
  ProfBeginFunction();
...
pdb_add_src(PDB_InfoContext *info, MSF_Context *msf, string file_path, string file_data, PDB_SrcCompType comp)
{
  Temp scratch = scratch_begin(0,0);
  ...
  if (comp == PDB_SrcComp_NULL) {
    ...
    if (!is_virt_path_present) {
      string stream_name = push_str8f(scratch.arena, "/src/files/%S", virt_path);
      ...
      if (is_name_free) {
        ...
        if (is_file_data_written) {
          ...
          string key = virt_path;
          string val = str8_struct(&entry);
...
internal PDB_GsiContext *
gsi_open(MSF_Context *msf, MSF_StreamNumber sn, string symbol_data)
{
  ProfBeginFunction();
...
#if 0
  {
    for (U64 i = 0; i < gsi->bucket_count; ++i) {
      ...
      for (CV_SymbolNode *prev = bucket->first, *curr = bucket->first ? bucket->first->next : NULL;
           curr != NULL;
           prev = curr, curr = curr->next) {
        string a = pdb_get_symbol_name(prev->symbol.kind, prev->symbol.data);
        string b = pdb_get_symbol_name(curr->symbol.kind, curr->symbol.data);
...
THREAD_POOL_TASK_FUNC(gsi_serialize_pub32)
{
  ...
  for (CV_SymbolNode *node = bucket_list.first; node != 0; node = node->next) {
    ...
    string      name     = str8(str_ptr, str_size);
...
internal U32
gsi_hash(PDB_GsiContext *gsi, string input)
{ (void)gsi;
...
THREAD_POOL_TASK_FUNC(gsi_symbol_hasher_task)
{
  ...
  for (U64 symbol_idx = range.min; symbol_idx < range.max; ++symbol_idx) {
    ...
    string        name   = cv_name_from_symbol(symbol->data.kind, symbol->data.data);
...
gsi_search(PDB_GsiContext *gsi, CV_Symbol *symbol)
{
  ...
  for (CV_SymbolNode *node = bucket_list.first; node != 0; node = node->next) {
    string that_name = cv_name_from_symbol(node->data.kind, node->data.data);
...
internal PDB_PsiContext *
psi_open(MSF_Context *msf, MSF_StreamNumber sn, string symbol_data)
{
  ProfBeginFunction();
...
internal CV_SymbolNode *
psi_push(PDB_PsiContext *psi, CV_Pub32Flags flags, U32 offset, U16 isect, string name)
{
...
dbi_open_file_info(Arena *arena, MSF_Context *msf, MSF_StreamNumber sn, PDB_DbiHeader *dbi_header)
{
  ...
  for (U64 mod_idx = 0; mod_idx < mod_count; ++mod_idx) {
    ...
    for (U16 ifile = 0; ifile < file_count; ifile += 1, file_name_offset_ptr += 1) {
      ...
      string file_path = str8_cstring(file_name_buffer + *file_name_offset_ptr);
...
dbi_open_module_info(Arena *arena, MSF_Context *msf, MSF_StreamNumber sn, PDB_DbiHeader *dbi_header, String8List *file_info)
{
  ...
  while (msf_stream_get_pos(msf, sn) < module_info_opl) { 
    ...
    msf_stream_read_struct(msf, sn, &header);
    string obj_path = msf_stream_read_string(arena, msf, sn);
    string lib_path = msf_stream_read_string(arena, msf, sn);
...
dbi_build_file_info(Arena *arena, TP_Context *tp, PDB_DbiModuleList mod_list, CV_StringHashTable string_ht)
{
  ...
  // pack strings
  string string_buffer = cv_pack_string_hash_table(arena, tp, string_ht);
...
dbi_push_module(PDB_DbiContext *dbi, string obj_path, string lib_path)
{
  // init module
...
pdb_open(string data)
{
  ...
  if (msf_err == MSF_Error_OK) {
    ...
    if (pdb->dbi) {
      ...
      string symbol_data = msf_stream_read_block(scratch.arena, pdb->msf, pdb->dbi->symbols_sn, sym_data_size);
