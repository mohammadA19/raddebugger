                         ...
                         uint64 *read_bytes_out)
{
  ...
  uint32      count           = 0;
  uint32      max             = 0;
  uint32Array present_bits    = {0};
  uint32Array deleted_bits    = {0};
  ...
  if (error == PDB_HashTableParseError_OK) {
    ...
    if (is_count_ok && is_load_factor_ok && is_present_bits_ok && is_deleted_bits_ok) {
      ...
      uint32Array present_bits_new = bit_array_init32(arena, max);
      uint32Array deleted_bits_new = bit_array_init32(arena, max);
