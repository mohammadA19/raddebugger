                         ...
                         uint64 *read_bytes_out)
{
  ...
  uint32      count           = 0;
  uint32      max             = 0;
  Sapn<uint32> present_bits    = {0};
  Sapn<uint32> deleted_bits    = {0};
  ...
  if (error == PDB_HashTableParseError_OK) {
    ...
    if (is_count_ok && is_load_factor_ok && is_present_bits_ok && is_deleted_bits_ok) {
      ...
      Sapn<uint32> present_bits_new = bit_array_init32(arena, max);
      Sapn<uint32> deleted_bits_new = bit_array_init32(arena, max);
...
PDB_HASH_TABLE_UNPACK_FUNC(pdb_named_stream_ht_unpack)
{
  ...
  uint32 key_data_offset = max_uint32;
...
internal PDB_HashTableParseError
pdb_hash_adj_hash_table_from_data(PDB_HashTable *ht, String8 data, PDB_StringTable *strtab, uint64 *read_bytes_out)
{
  return pdb_hash_table_from_data(ht, data, 0, pdb_hash_adj_ht_unpack, strtab, read_bytes_out);
}

internal PDB_HashTableParseError
pdb_src_header_block_ht_from_data(PDB_HashTable *ht, String8 data, PDB_StringTable *strtab, uint64 *read_bytes_out)
{
  return pdb_hash_table_from_data(ht, data, 0, pdb_src_header_block_ht_unpack, strtab, read_bytes_out);
}

internal PDB_HashTableParseError
pdb_named_stream_ht_from_data(PDB_HashTable *ht, String8 data, uint64 *read_bytes_out)
{
...
THREAD_POOL_TASK_FUNC(pdb_write_types_task)
{
  ...
  Rng1uint64      range  = task->lf_range_arr[task_id];
...
pdb_type_server_build(TP_Context *tp, PDB_TypeServer *ts, PDB_StringTable *strtab, MSF_Context *msf, MSF_StreamNumber sn)
{
  ...
  Rng1uint64        *lf_range_arr  = tp_divide_work(scratch.arena, ts->leaf_list.node_count, tp->worker_count);
...
pdb_type_server_push_udt_arr(PDB_TypeServer *ts, uint64 count, uint32 *hash_arr, String8 *raw_leaf_arr)
{
        ...
        if (str8_match(udt_info.name, this_udt_info.name)) {
          ...
          if (is_data_match) {
            ...
            if (is_not_head) {
              ...
              pdb_hash_table_set(&ts->hash_adj, udt_info.name, str8((uint8*)&curr->leaf->type_index, sizeof(curr->leaf->type_index)));
            }
...
internal PDB_TypeBucket *
pdb_type_server_push_udt(PDB_TypeServer *ts, uint32 hash, String8 raw_leaf)
{
...
internal CV_SymbolNode *
psi_push(PDB_PsiContext *psi, CV_Pub32Flags flags, uint32 offset, uint16 isect, String8 name)
{
...
dbi_build_file_info(Arena *arena, TP_Context *tp, PDB_DbiModuleList mod_list, CV_StringHashTable string_ht)
{
  ...
  uint16 total_source_file_count16    = Min(max_uint16, total_source_file_count);
  uint16 mod_count16                  = Min(max_uint16, mod_list.count);
...
dbi_build_module_info(Arena *arena, PDB_DbiContext *dbi, MSF_Context *msf)
{
  ...
  for (PDB_DbiModule *mod = dbi->module_list.first; mod != 0; mod = mod->next) {
    ...
    header->num_contrib_files    = Min(max_uint16, mod->source_file_list.node_count);
...
dbi_build_sec_con(Arena *arena, PDB_DbiContext *dbi)
{
  ...
  *version = PDB_DbiSectionContribVersion_1;
  ...
  str8_list_push(arena, &sec_con_list, str8((uint8*)version, sizeof(*version)));
  str8_list_push(arena, &sec_con_list, str8((uint8*)sc_array, sizeof(sc_array[0])*dbi->sec_contrib_list.count));
  ProfEnd();
...
dbi_build_sec_map(Arena *arena, PDB_DbiContext *dbi)
{
  ...
  str8_list_push(arena, &sec_map_list, str8((uint8*)header, sizeof(*header)));
  str8_list_push(arena, &sec_map_list, str8((uint8*)entry_array, sizeof(entry_array[0])*entry_count));
