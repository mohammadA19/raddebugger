...
pdb_hash_table_alloc(PDB_HashTable *ht, U32 max)
{
  ...
  ht->bucket_arr   = new PDB_HashTableBucket[max];
...
pdb_strtab_alloc(PDB_StringTable *strtab, U32 max)
{
  ...
  strtab->ibucket_array = new U32[strtab->bucket_max];
  MemorySet(strtab->ibucket_array, 0xff, sizeof(strtab->ibucket_array[0]) * strtab->bucket_max);
  strtab->bucket_array = new PDB_StringTableBucket *[strtab->bucket_max];
  ...
  if (header.magic == PDB_StringTableHeader_MAGIC) {
    if (header.version == PDB_StringTableHeader_CurrentVersion) {
      ...
      if (expected_size <= actual_size &&
        ...
        bucket_array  = /* no zero */ new PDB_StringTableBucket *[bucket_max];
...
pdb_strtab_build(PDB_StringTable *strtab, MSF_Context *msf, MSF_StreamNumber sn)
{
  ...
  U8  *string_buffer     = /* no zero */ new U8[strtab->size];
  U32 *bucket_offset_arr = new U32[strtab->bucket_max];
...
pdb_strtab_add_cv_string_hash_table(PDB_StringTable *strtab, CV_StringHashTable string_ht)
{
  ...
  PDB_StringTableBucket *buckets = /* no zero */ new PDB_StringTableBucket[string_ht.total_insert_count];
...
{
  PDB_StringTableBucket *bucket = new PDB_StringTableBucket[1];
...
pdb_type_server_open_v80(MSF_Context *msf, MSF_StreamNumber sn, PDB_StringTable *strtab)
{
  ...
  // read hash data
  U8 *hash_buffer = new U8[header.hash_vals.size];
  ...
  for (U64 cursor = 0, leaf_idx = 0; 
    ...
    // push bucket
    PDB_TypeBucket *bucket = new PDB_TypeBucket[1];
                           ...
                           U64              hint_count)
{
  ...
  // map to zero offset
  U32 *type_to_bucket_map = new U32[ts->leaf_list.node_count];
  {
...
pdb_type_server_build(TP_Context *tp, PDB_TypeServer *ts, PDB_StringTable *strtab, MSF_Context *msf, MSF_StreamNumber sn)
{
  ...
  PDB_TpiOffHint *hint_arr      = /* no zero */ new PDB_TpiOffHint[hint_count];
  String8Node   **lf_arr        = /* no zero */ new String8Node *[tp->worker_count];
  U64            *lf_cursor_arr = /* no zero */ new U64[tp->worker_count];
  ...
  write_types_task.lf_buf        = /* no zero */ new U8[lf_buf_size];
...
pdb_type_server_push_udt_arr(PDB_TypeServer *ts, U64 count, U32 *hash_arr, String8 *raw_leaf_arr)
{
  ...
  PDB_TypeBucket *bucket_arr = /* no zero */ new PDB_TypeBucket[count];
...
pdb_type_server_push_parallel(TP_Context *tp, PDB_TypeServer *type_server, CV_DebugT debug_t)
{
  ...
  ProfBegin("Count UDT");
  task.udt_counts = new U64[tp->worker_count];
  ...
  task.udt_buckets = /* no zero */ new PDB_TypeBucket[total_udt_count];
...
pdb_info_alloc(U32 age, COFF_TimeStamp time_stamp, Guid guid)
{
  ProfBeginFunction();
  Arena *arena = arena_alloc();
  PDB_InfoContext *info = new PDB_InfoContext[1];
  info->arena      = arena;
...
gsi_open(MSF_Context *msf, MSF_StreamNumber sn, String8 symbol_data)
{
  ...
  if (header.signature == PDB_GsiSignature_Basic) {
    if (header.version == PDB_GsiVersion_V70) {
      ...
      PDB_GsiHashRecord *hash_record_array = new PDB_GsiHashRecord[hash_record_count];
      msf_stream_read_array(msf, sn, &hash_record_array[0], hash_record_count);
      
      U32 *bitmap = new U32[PDB_GSI_V70_BITMAP_COUNT];
      ...
      U32 *compressed_offset_array = new U32[compressed_offset_count];
...
gsi_build_ex(TP_Context *tp, Arena *arena, PDB_GsiContext *gsi, U64 symbol_data_base, B32 is_pub32, U64 msf_page_size)
{
  ...
  serial_task.bucket_size_arr = new U64[gsi->bucket_count];
  ...
  // prepare GSI records
  serial_task.sort_record_arr_arr  = /* no zero */ new PDB_GsiSortRecord *[gsi->bucket_count];
  ...
  U32            *bitmap                  = new U32[bitmap_count];
...
gsi_push(PDB_GsiContext *gsi, CV_Symbol *symbol)
{
  ...
  CV_SymbolNode *node = /* no zero */ new CV_SymbolNode[1];
  node->next = 0;
  node->prev = 0;
  node->data = *symbol;
...
gsi_push_many_arr(TP_Context *tp, PDB_GsiContext *gsi, U64 count, CV_SymbolNode **symbols)
{
  ...
  task.hashes               = /* no zero */ new U32[count];
...
psi_alloc(void)
{
  ProfBeginFunction();
  Arena *arena = arena_alloc();
  PDB_PsiContext *psi = new PDB_PsiContext[1];
  psi->arena = arena;
  psi->gsi = gsi_alloc();
  ProfEnd();
  return psi;
...
psi_build(TP_Context *tp, PDB_PsiContext *psi, MSF_Context *msf, MSF_StreamNumber sn, MSF_StreamNumber symbols_sn)
{
  ...
  U32 *addr_map     = /* no zero */ new U32[addr_map_count];
...
dbi_alloc(COFF_MachineType machine, U32 age)
{
  ProfBeginFunction();
  Arena *arena = arena_alloc();
  PDB_DbiContext *dbi = new PDB_DbiContext[1];
  dbi->arena      = arena;
  dbi->age        = age;
  dbi->machine    = machine;
...
dbi_open_file_info(Arena *arena, MSF_Context *msf, MSF_StreamNumber sn, PDB_DbiHeader *dbi_header)
{
  ...
  CV_ModIndex *imod_array = new CV_ModIndex[mod_count];
  msf_stream_read_array(msf, sn, &imod_array[0], mod_count);
  
  U16 *mod_file_count = new U16[mod_count];
  ...
  U32 *file_name_offset_array = new U32[total_file_count];
  ...
  char *file_name_buffer = new char[file_name_buffer_size + 1];
  msf_stream_read_array(msf, sn, &file_name_buffer[0], file_name_buffer_size);
  
  String8List *file_info = new String8List[mod_count + 1];
...
dbi_open_module_info(Arena *arena, MSF_Context *msf, MSF_StreamNumber sn, PDB_DbiHeader *dbi_header, String8List *file_info)
{
  ...
  while (msf_stream_get_pos(msf, sn) < module_info_opl) { 
    ...
    PDB_DbiModule *mod    = new PDB_DbiModule[1];
    mod->next             = 0;
    mod->sn               = header.sn;
...
dbi_open_sec_contrib(Arena *arena, MSF_Context *msf, MSF_StreamNumber sn, PDB_DbiHeader *dbi_header)
{
  ...
  if (dbi_header->sec_con_size > sizeof(PDB_DbiSectionContrib)) {
    ...
    case PDB_DbiSectionContribVersion_1: {
      ...
      PDB_DbiSectionContrib *src_contrib_array = new PDB_DbiSectionContrib[contrib_count];
...
dbi_open_section_headers(Arena *arena, MSF_Context *msf, MSF_StreamNumber sn)
{
  ...
  PDB_DbiSectionNode *sec_nodes = new PDB_DbiSectionNode[sec_count];
...
dbi_build_file_info(Arena *arena, TP_Context *tp, PDB_DbiModuleList mod_list, CV_StringHashTable string_ht)
{
  ...
  U64             mod_arr_count           = 0;
  PDB_DbiModule **mod_arr                 = /* no zero */ new PDB_DbiModule *[mod_list.count];
  ...
  U32 **source_file_name_offsets_arr = /* no zero */ new U32 *[mod_list.count];
...
dbi_build_module_info(Arena *arena, PDB_DbiContext *dbi, MSF_Context *msf)
{
  ...
  for (PDB_DbiModule *mod = dbi->module_list.first; mod != 0; mod = mod->next) {
    // fill out header
    PDB_DbiCompUnitHeader *header = new PDB_DbiCompUnitHeader[1];
...
# define RADIX_MAX       4
  ...
  PDB_DbiSectionContrib *temp_arr = /* no zero */ new PDB_DbiSectionContrib[sc_count];
  ...
  U32 *count_arr = new U32[sect_count + 1];
  ProfEnd();
...
dbi_build_sec_con(Arena *arena, PDB_DbiContext *dbi)
{
  ...
  PDB_DbiSectionContribVersion *version = new PDB_DbiSectionContribVersion[1];
  *version = PDB_DbiSectionContribVersion_1;
  ...
  PDB_DbiSectionContrib *sc_array = /* no zero */ new PDB_DbiSectionContrib[dbi->sec_contrib_list.count];
...
dbi_build_sec_map(Arena *arena, PDB_DbiContext *dbi)
{
  ...
  PDB_DbiSecMapEntry *entry_array = new PDB_DbiSecMapEntry[entry_count];
  U64 isect = 0;
  ...
  // init header
  PDB_DbiSecMapHeader *header = new PDB_DbiSecMapHeader[1];
                                ...
                                COFF_SectionFlags flags)
{
  ...
  PDB_DbiSectionContribNode *node = /* no zero */ new PDB_DbiSectionContribNode[1];
  node->data = sc;
...
dbi_push_section(PDB_DbiContext *dbi, COFF_SectionHeader *hdr)
{
  ...
  PDB_DbiSectionNode *n = new PDB_DbiSectionNode[1];
  n->data = *hdr;
  n->next = 0;
...
pdb_alloc(U64 page_size, COFF_MachineType machine, COFF_TimeStamp time_stamp, U32 age, Guid guid)
{
  ProfBeginFunction();
  Arena *arena = arena_alloc();
  PDB_Context *pdb = new PDB_Context[1];
  pdb->arena = arena;
  ...
  pdb->gsi   = gsi_alloc();
  pdb->psi   = psi_alloc();
  pdb->type_servers[CV_TypeIndexSource_NULL] = new PDB_TypeServer[1];
