...
#define PDB_HASH_TABLE_UNPACK_FUNC(name) B32 name(void *ud, String8 local_data, String8 key_value_data, uint64 *key_value_cursor, String8 *key_out, String8 *value_out)
...
typedef struct PDB_HashTable
{
  ...
  Sapn<uint32>             present_bits;
  Sapn<uint32>             deleted_bits;
  uint32   			         max;
  uint32                  count;
} PDB_HashTable;
...
internal PDB_Context *    pdb_alloc(uint64 page_size, COFF_MachineType machine, COFF_TimeStamp time_stamp, uint32 age, Guid guid);
...
internal void             pdb_set_age(PDB_Context *pdb, uint32 age);
internal COFF_MachineType pdb_get_machine(PDB_Context *pdb);
internal COFF_TimeStamp   pdb_get_time_stamp(PDB_Context *pdb);
internal uint32              pdb_get_age(PDB_Context *pdb);
internal Guid             pdb_get_guid(PDB_Context *pdb);

////////////////////////////////
// Info

internal PDB_InfoContext * pdb_info_alloc(uint32 age, COFF_TimeStamp time_stamp, Guid guid);
...
internal PDB_GsiBuildResult gsi_build_ex(TP_Context *tp, Arena *arena, PDB_GsiContext *gsi, uint64 symbol_data_base, B32 export_symbol_ptr_arr, uint64 msf_page_size);
internal uint32                gsi_hash(PDB_GsiContext *gsi, String8 input);
internal CV_SymbolNode *    gsi_push(PDB_GsiContext *gsi, CV_Symbol *symbol);
internal void               gsi_push_many_arr(TP_Context *tp, PDB_GsiContext *gsi, uint64 count, CV_SymbolNode **symbol_arr);
internal void               gsi_push_many_list(PDB_GsiContext *gsi, uint64 count, uint32 *hash_arr, CV_SymbolList *list);
...
internal CV_SymbolNode *  psi_push(PDB_PsiContext *psi, CV_Pub32Flags flags, uint32 offset, uint16 isect, String8 name);
...
internal PDB_DbiContext *          dbi_alloc(COFF_MachineType machine, uint32 age);
...
internal void                      dbi_module_push_section_contrib(PDB_DbiContext *dbi, PDB_DbiModule *mod, ISectOff isect_off, uint32 size,  uint32 data_crc, uint32 reloc_crc, COFF_SectionFlags flags);
...
internal void                    pdb_hash_table_alloc(PDB_HashTable *ht, uint32 max);
internal void                    pdb_hash_table_release(PDB_HashTable *ht);
internal PDB_HashTableParseError pdb_hash_table_from_data(PDB_HashTable *ht, String8 data, B32 has_local_data, PDB_HashTableUnpackFunc *unpack_func, void *unpack_ud, uint64 *read_bytes_out);
...
internal B32                     pdb_hash_table_is_present(PDB_HashTable *ht, uint32 k);
internal B32                     pdb_hash_table_is_deleted(PDB_HashTable *ht, uint32 k);
internal uint32                     pdb_hash_table_hash(String8 key);
internal void                    pdb_hash_table_grow(PDB_HashTable *ht, uint64 new_capacity);
...
internal PDB_HashTableParseError pdb_hash_adj_hash_table_from_data(PDB_HashTable *ht, String8 data, PDB_StringTable *strtab, uint64 *read_bytes_out);
internal PDB_HashTableParseError pdb_src_header_block_ht_from_data(PDB_HashTable *ht, String8 data, PDB_StringTable *strtab, uint64 *read_bytes_out);
internal PDB_HashTableParseError pdb_named_stream_ht_from_data(PDB_HashTable *ht, String8 data, uint64 *read_bytes_out);
...
internal void                     pdb_strtab_alloc(PDB_StringTable *strtab, uint32 max);
...
internal uint32                      pdb_strtab_get_serialized_size(PDB_StringTable *strtab);
internal B32                      pdb_strtab_try_add(PDB_StringTable *strtab, String8 string, PDB_StringIndex *index_out);
internal void                     pdb_strtab_grow(PDB_StringTable *strtab, uint64 new_max);
internal uint32                      pdb_strtab_hash(PDB_StringTable *strtab, String8 string);
...
internal PDB_TypeServer *        pdb_type_server_alloc(uint64 bucket_count);
...
internal String8Node *           pdb_type_server_reserve(PDB_TypeServer *ts, uint64 count);
...
internal PDB_TypeHashStreamInfo  pdb_type_hash_stream_build(TP_Context *tp, PDB_TypeServer *ts, PDB_StringTable *strtab, MSF_Context *msf, PDB_TpiOffHint *hint_arr, uint64 hint_count);
