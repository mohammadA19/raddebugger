...
#define PDB_HASH_TABLE_UNPACK_FUNC(name) B32 name(void *ud, String8 local_data, String8 key_value_data, ulong *key_value_cursor, String8 *key_out, String8 *value_out)
...
#define PDB_GSI_V70_BITMAP_SIZE  (PDB_GSI_V70_BITMAP_COUNT * sizeof(uint))
...
typedef struct PDB_GsiBuildResult
{
  ...
  ulong                hash_record_count;
  PDB_GsiHashRecord *hash_record_arr;
  PDB_GsiSortRecord *sort_record_arr;
  ulong                bitmap_count;
  uint               *bitmap;
  ulong                compressed_bucket_count;
  uint               *compressed_bucket_arr;
  ulong                total_hash_size;
  String8            symbol_data;
} PDB_GsiBuildResult;
...
typedef struct PDB_GsiSerializeSymbolsTask
{
  ulong                  symbol_align;
  CV_SymbolList       *bucket_arr;
  ulong                 *bucket_size_arr;
  ulong                 *bucket_off_arr;
  byte                  *buffer;
...
typedef struct PDB_DbiModule
{
  ...
  ulong                   c11_data_size;
  ulong                   c13_data_size;
  ulong                   globrefs_size; // TODO: what is this for?
  String8               obj_path;
  String8               lib_path;
...
internal PDB_Context *    pdb_alloc(ulong page_size, COFF_MachineType machine, COFF_TimeStamp time_stamp, uint age, Guid guid);
...
internal void             pdb_set_age(PDB_Context *pdb, uint age);
internal COFF_MachineType pdb_get_machine(PDB_Context *pdb);
internal COFF_TimeStamp   pdb_get_time_stamp(PDB_Context *pdb);
internal uint              pdb_get_age(PDB_Context *pdb);
internal Guid             pdb_get_guid(PDB_Context *pdb);

////////////////////////////////
// Info

internal PDB_InfoContext * pdb_info_alloc(uint age, COFF_TimeStamp time_stamp, Guid guid);
...
internal PDB_GsiBuildResult gsi_build_ex(TP_Context *tp, Arena *arena, PDB_GsiContext *gsi, ulong symbol_data_base, B32 export_symbol_ptr_arr, ulong msf_page_size);
internal uint                gsi_hash(PDB_GsiContext *gsi, String8 input);
internal CV_SymbolNode *    gsi_push(PDB_GsiContext *gsi, CV_Symbol *symbol);
internal void               gsi_push_many_arr(TP_Context *tp, PDB_GsiContext *gsi, ulong count, CV_SymbolNode **symbol_arr);
internal void               gsi_push_many_list(PDB_GsiContext *gsi, ulong count, uint *hash_arr, CV_SymbolList *list);
...
internal CV_SymbolNode *  psi_push(PDB_PsiContext *psi, CV_Pub32Flags flags, uint offset, ushort isect, String8 name);

// TODO:
//internal CV_Symbol psi_neareset_symbol(PDB_PsiContext *psi, ushort isect, uint off);
//internal void      psi_push_thunk_map(PDB_PsiContext *psi, uint *thunk_map, uint thunk_count, uint thunk_size, PDB_SO *sect_map, uint sect_count, ISectOff thunk_table);

////////////////////////////////
// DBI

internal PDB_DbiContext *          dbi_alloc(COFF_MachineType machine, uint age);
...
internal void                      dbi_module_push_section_contrib(PDB_DbiContext *dbi, PDB_DbiModule *mod, ISectOff isect_off, uint size,  uint data_crc, uint reloc_crc, COFF_SectionFlags flags);
...
internal void                    pdb_hash_table_alloc(PDB_HashTable *ht, uint max);
internal void                    pdb_hash_table_release(PDB_HashTable *ht);
internal PDB_HashTableParseError pdb_hash_table_from_data(PDB_HashTable *ht, String8 data, B32 has_local_data, PDB_HashTableUnpackFunc *unpack_func, void *unpack_ud, ulong *read_bytes_out);
...
internal B32                     pdb_hash_table_is_present(PDB_HashTable *ht, uint k);
internal B32                     pdb_hash_table_is_deleted(PDB_HashTable *ht, uint k);
internal uint                     pdb_hash_table_hash(String8 key);
internal void                    pdb_hash_table_grow(PDB_HashTable *ht, ulong new_capacity);
...
internal PDB_HashTableParseError pdb_hash_adj_hash_table_from_data(PDB_HashTable *ht, String8 data, PDB_StringTable *strtab, ulong *read_bytes_out);
internal PDB_HashTableParseError pdb_src_header_block_ht_from_data(PDB_HashTable *ht, String8 data, PDB_StringTable *strtab, ulong *read_bytes_out);
internal PDB_HashTableParseError pdb_named_stream_ht_from_data(PDB_HashTable *ht, String8 data, ulong *read_bytes_out);
...
internal void                     pdb_strtab_alloc(PDB_StringTable *strtab, uint max);
...
internal uint                      pdb_strtab_get_serialized_size(PDB_StringTable *strtab);
internal B32                      pdb_strtab_try_add(PDB_StringTable *strtab, String8 string, PDB_StringIndex *index_out);
internal void                     pdb_strtab_grow(PDB_StringTable *strtab, ulong new_max);
internal uint                      pdb_strtab_hash(PDB_StringTable *strtab, String8 string);
...
internal PDB_TypeServer *        pdb_type_server_alloc(ulong bucket_count);
...
internal String8Node *           pdb_type_server_reserve(PDB_TypeServer *ts, ulong count);
...
internal PDB_TypeHashStreamInfo  pdb_type_hash_stream_build(TP_Context *tp, PDB_TypeServer *ts, PDB_StringTable *strtab, MSF_Context *msf, PDB_TpiOffHint *hint_arr, ulong hint_count);
