...
msf_get_fpm_page_bit_state(MSF_PageDataList page_data_list, MSF_UInt page_size, MSF_PageNumber active_fpm, MSF_PageNumber pn)
{
  ...
  Sapn<uint32> fpm_data = msf_fpm_data_from_pn(page_data_list, page_size, fpm_pn);
...
msf_alloc_pn_arr(Arena *arena, MSF_Context *msf, MSF_UInt alloc_count)
{
  ...
  for (MSF_UInt alloc_idx = 0; alloc_idx < alloc_count; ) {
    ...
    Sapn<uint32> fpm_data = msf_fpm_data_from_pn(msf->page_data_list, msf->page_size, fpm_pn);
...
internal B32
msf_stream_write_s8(MSF_Context *msf, MSF_StreamNumber sn, uint8 value)
{
  return msf_stream_write(msf, sn, &value, sizeof(value));
}

internal B32
msf_stream_write_s16(MSF_Context *msf, MSF_StreamNumber sn, uint16 value)
{
  return msf_stream_write(msf, sn, &value, sizeof(value));
}

internal B32
msf_stream_write_s32(MSF_Context *msf, MSF_StreamNumber sn, uint32 value)
{
  return msf_stream_write(msf, sn, &value, sizeof(value));
}

internal B32
msf_stream_write_s64(MSF_Context *msf, MSF_StreamNumber sn, uint64 value)
{
  return msf_stream_write(msf, sn, &value, sizeof(value));
}
