...
void
msf_set_page_data_list(Arena* arena, MSF_PageDataList* list, MSF_UInt page_size, StringView data)
{
  ProfBeginFunction();
...
msf_data_from_pn(MSF_PageDataList list, MSF_UInt page_size, MSF_PageNumber pn)
{
  ...
  StringView data = StringView(ptr, page_size);
  return data;
}
...
msf_write__(MSF_PageDataList page_data_list, MSF_UInt page_size, MSF_PageNode** page_ptr, MSF_UInt* pos_ptr, void* buffer, MSF_UInt buffer_size)
{
  ...
  while (*page_ptr) {
    ...
    StringView        page_bytes  = msf_data_from_pn(page_data_list, page_size, page_number);
...
THREAD_POOL_TASK_FUNC(msf_write_task)
{
  ...
  StringView  data     = str8_substr(task.data, range);
...
msf_stream_write_parallel(TP_Context* tp, MSF_Context* msf, MSF_StreamNumber sn, void* buffer, MSF_UInt buffer_size)
{
  ...
  if (is_write_ok) {
    ...
    if (mid_size > 0) {
      ...
      task.data           = StringView(mid_ptr, mid_size);
...
msf_stream_read_block(Arena* arena, MSF_Context* msf, MSF_StreamNumber sn, uint64 block_size)
{
  ...
  StringView block = StringView(block_buffer, block_size);
  return block;
}
...
msf_open_stream_table(Arena* arena, MSF_PageDataList page_data_list, MSF_UInt page_size, MSF_PageList st_page_list, MSF_UInt stream_table_size, MSF_StreamList* stream_list)
{
  ...
  // setup buffer reader
  StringView st_data = StringView(st_buffer, st_read_size);
  uint64 st_cursor = 0;
...
MSF_Error
msf_open(StringView data, MSF_Context** msf_out)
{
  ProfBeginFunction();
...
msf_build_stream_table_data(Arena* arena, MSF_StreamList* st, MSF_UInt page_size, MSF_UInt page_count)
{
  ...
  *stream_count_ptr = st.count;
  ...
  str8_list_push(arena, &st_data_list, StringView((uint8*)stream_count_ptr, sizeof(*stream_count_ptr)));
  str8_list_push(arena, &st_data_list, StringView((uint8*)stream_size_arr, sizeof(*stream_size_arr) * (*stream_count_ptr)));
  str8_list_push(arena, &st_data_list, StringView((uint8*)stream_pages_arr, sizeof(*stream_pages_arr) * stream_page_count));
...
msf_get_page_data_nodes(Arena* arena, MSF_Context* msf)
{
  ...
  for (MSF_PageDataNode* data_node = msf.page_data_list.first; data_node != 0; data_node = data_node.next) {
    ...
    StringView data = StringView(data_node.data, to_copy);
...
MSF_Error
msf_save_arena(Arena* arena, MSF_Context* msf, StringView* data_out)
{
  ProfBeginFunction();
  ...
  if (err == MSF_Error_OK) {
    ...
    if (is_saved) {
      *data_out = StringView(buffer, buffer_size);
    } else {
...
msf_bytedump_stream(char* file_name, MSF_Context* msf, MSF_StreamNumber sn, uint64 start, uint64 byte_count)
{
  ...
  os_write_file(str8_cstring(file_name), StringView(buffer, read_size));
...
test_size_limit()
{
  ...
  StringView msf_data = msf_save_arena(scratch.arena, msf);
