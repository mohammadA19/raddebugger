...
U64 msf_get_data_node_size(MSF_UInt page_size)
{
...
void msf_page_data_list_push(Arena *arena, MSF_PageDataList *list, MSF_UInt page_size, MSF_UInt count)
{
...
MSF_PageDataList msf_page_data_list_pop(MSF_PageDataList *list, MSF_UInt count)
{
...
void msf_page_data_list_concat_in_place(MSF_PageDataList *list, MSF_PageDataList *to_concat)
{
  DLLConcatInPlace(list, to_concat);
}

void msf_set_page_data_list(Arena *arena, MSF_PageDataList *list, MSF_UInt page_size, String8 data)
{
  ProfBeginFunction();
...
String8 msf_data_from_pn(MSF_PageDataList list, MSF_UInt page_size, MSF_PageNumber pn)
{
  U64 node_size = msf_get_data_node_size(page_size);
...
MSF_StreamNode* msf_stream_list_push(Arena *arena, MSF_StreamList *list)
{
...
void msf_stream_list_remove(MSF_StreamList *list, MSF_StreamNode *node)
{
  Assert(list->count > 0);
  DLLRemove(list->first, list->last, node);
  list->count -= 1;
...
void msf_page_list_push_node(MSF_PageList *list, MSF_PageNode *node)
{
  DLLPushBack(list->first, list->last, node);
  list->count += 1;
}

MSF_PageNode* msf_page_list_push(Arena *arena, MSF_PageList *list)
{
...
MSF_PageNode* msf_page_list_pop_last(MSF_PageList *list)
{
...
void msf_page_list_concat_in_place(MSF_PageList *list, MSF_PageList *to_concat)
{
  DLLConcatInPlace(list, to_concat);
}

MSF_PageNumber* msf_page_list_to_arr(Arena *arena, MSF_PageList list)
{
...
MSF_PageNode* msf_page_from_index(MSF_PageList page_list, MSF_UInt index)
{
  MSF_PageNode *page;
...
void msf_page_list_push_extant_page_arr(Arena *arena, MSF_PageList *list,
...
void msf_page_list_push_extant_page(Arena *arena, MSF_PageList *list, MSF_PageDataList page_data_list, MSF_UInt page_size, MSF_PageNumber pn)
{
  msf_page_list_push_extant_page_arr(arena, list, page_data_list, page_size, &pn, 1);
}

#if LNK_PARANOID
void msf_check_fpm_bits_for_page_list(MSF_PageDataList page_data_list, MSF_UInt page_size, MSF_PageNumber active_fpm, MSF_PageList page_list, MSF_UInt test_state)
{
...
U32Array msf_fpm_data_from_pn(MSF_PageDataList page_data_list, MSF_UInt page_size, MSF_PageNumber fpm_pn)
{
...
MSF_UInt msf_get_fpm_page_count(MSF_PageDataList page_data_list, MSF_UInt page_size, MSF_UInt fpm_interval)
{
  U64 node_size = msf_get_data_node_size(page_size);
...
MSF_PageNumberArray msf_get_fpm_page_arr(Arena *arena, MSF_PageDataList page_data_list, MSF_UInt page_size, MSF_UInt active_fpm)
{
...
MSF_PageNumber msf_get_fpm_from_page_number(MSF_UInt page_size, MSF_PageNumber active_fpm, MSF_PageNumber pn)
{
...
void msf_set_fpm_bit_(MSF_PageDataList page_data_list, MSF_UInt page_size, MSF_PageNumber active_fpm, MSF_PageNumber pn, B32 state)
{
  // fetch FPM
  MSF_PageNumber fpm_pn = msf_get_fpm_from_page_number(page_size, active_fpm, pn);
  U32Array fpm_data = msf_fpm_data_from_pn(page_data_list, page_size, fpm_pn);
...
B32 msf_grow(MSF_Context *msf, MSF_PageNumber new_page_count)
{
...
#if 0
B32 msf_shrink(MSF_Context *msf, MSF_PageNumber new_page_count)
{
  MSF_UInt fpm_interval_wrong = msf_get_fpm_interval_wrong(msf->page_size);
  MSF_UInt fpm_interval_correct = msf_get_fpm_interval_correct(msf->page_size);
...
MSF_PageNumber msf_find_max_pn_(MSF_PageDataList page_data_list, MSF_UInt page_size, MSF_PageNumberArray fpm_pn_arr)
{
...
MSF_PageNumber msf_find_max_pn(MSF_PageDataList page_data_list, MSF_UInt page_size)
{
  Temp scratch = scratch_begin(0, 0);
...
B32 msf_write__(MSF_PageDataList page_data_list, MSF_UInt page_size, MSF_PageNode **page_ptr, MSF_UInt *pos_ptr, void *buffer, MSF_UInt buffer_size)
{
...
MSF_UInt msf_read__(MSF_PageDataList page_data_list, MSF_UInt page_size, MSF_PageNode **page_ptr, MSF_UInt *pos_ptr, void *buffer, MSF_UInt buffer_size)
{
  MSF_UInt buffer_pos = 0;
  while (*page_ptr) {
    MSF_UInt page_offset = *pos_ptr % page_size;
...
MSF_UInt msf_stream_get_cap__(MSF_Context *msf, MSF_Stream *stream)
{
  return stream->page_list.count * msf->page_size;
}

MSF_UInt msf_stream_get_cap(MSF_Context *msf, MSF_StreamNumber sn)
{
  MSF_Stream *stream = msf_find_stream(msf, sn);
  MSF_UInt cap = 0;
  if (stream) {
...
MSF_UInt msf_stream_get_pos__(MSF_Context *msf, MSF_Stream *stream)
{
  return stream->pos;
}
...
B32 msf_stream_seek__(MSF_Context *msf, MSF_Stream *stream, MSF_UInt new_pos) 
{ (void)msf;
...
B32 msf_stream_write__(MSF_Context *msf, MSF_Stream *stream, void *buffer, MSF_UInt buffer_size)
{
  B32 is_write_ok = 0;
...
MSF_UInt msf_stream_reserve__(MSF_Context *msf, MSF_Stream *stream, MSF_UInt res)
{
...
B32 msf_stream_reserve(MSF_Context *msf, MSF_StreamNumber sn, MSF_UInt res)
{
  MSF_Stream *stream = msf_find_stream(msf, sn);
  B32 is_res_ok = 0;
  if (stream) {
...
B32 msf_stream_write_u8(MSF_Context *msf, MSF_StreamNumber sn, U8 value)
{
  return msf_stream_write(msf, sn, &value, sizeof(value));
}

B32 msf_stream_write_u16(MSF_Context *msf, MSF_StreamNumber sn, U16 value)
{
  return msf_stream_write(msf, sn, &value, sizeof(value));
}
...
B32 msf_stream_write_s8(MSF_Context *msf, MSF_StreamNumber sn, S8 value)
{
  return msf_stream_write(msf, sn, &value, sizeof(value));
}

B32 msf_stream_write_s16(MSF_Context *msf, MSF_StreamNumber sn, S16 value)
{
  return msf_stream_write(msf, sn, &value, sizeof(value));
}
...
B32 msf_stream_write_string_parallel(TP_Context *tp, MSF_Context *msf, MSF_StreamNumber sn, String8 string)
{
...
MSF_UInt msf_stream_read__(MSF_Context *msf, MSF_Stream *stream, void *buffer, MSF_UInt buffer_size)
{
...
MSF_UInt msf_stream_read(MSF_Context *msf, MSF_StreamNumber sn, void *buffer, MSF_UInt buffer_size)
{
  MSF_Stream *stream = msf_find_stream(msf, sn);
  if (stream) {
...
S8 msf_stream_read_s8(MSF_Context *msf, MSF_StreamNumber sn)
{
  S8 result = 0;
...
S16 msf_stream_read_s16(MSF_Context *msf, MSF_StreamNumber sn)
{
  S16 result = 0;
  msf_stream_read_struct(msf, sn, &result);
  return result;
...
U8 msf_stream_read_u8(MSF_Context *msf, MSF_StreamNumber sn)
{
  U8 result = 0;
  msf_stream_read_struct(msf, sn, &result);
  return result;
...
U16 msf_stream_read_u16(MSF_Context *msf, MSF_StreamNumber sn)
{
  U16 result = 0;
  msf_stream_read_struct(msf, sn, &result);
  return result;
...
MSF_Error msf_open_header(Arena *arena, MSF_PageDataList page_data_list, MSF_UInt page_size, MSF_PageList *page_list)
{
  ProfBeginFunction();
...
MSF_Error msf_open_root(Arena *arena, MSF_PageDataList page_data_list, MSF_UInt page_size, MSF_PageNumber root_pn, MSF_UInt stream_table_size, MSF_PageList *page_list)
{
  ProfBeginFunction();
  Temp scratch = scratch_begin(&arena, 1);
...
MSF_Error msf_open_stream_table_page_list(Arena *arena, MSF_PageDataList page_data_list, MSF_UInt page_size, MSF_PageList root_page_list, MSF_UInt stream_table_size, MSF_PageList *page_list)
{
  ProfBeginFunction();
  Temp scratch = scratch_begin(&arena, 1);
...
MSF_Error msf_open_stream_table(Arena *arena, MSF_PageDataList page_data_list, MSF_UInt page_size, MSF_PageList st_page_list, MSF_UInt stream_table_size, MSF_StreamList *stream_list)
{
  ProfBeginFunction();
  Temp scratch = scratch_begin(&arena, 1);
  MSF_Error error = MSF_Error_OK;
...
String8List msf_build_stream_table_data(Arena *arena, MSF_StreamList *sectab, MSF_UInt page_size, MSF_UInt page_count)
{
  ProfBeginFunction();
...
MSF_Error msf_build_stream_table(MSF_Context *msf, MSF_UInt *stream_table_size_out)
{
  ProfBeginFunction();
  Temp scratch = scratch_begin(0, 0);
...
MSF_Error msf_build_root_directory(MSF_Context *msf)
{
  ProfBeginFunction();
  Temp scratch = scratch_begin(0, 0);
...
MSF_Error msf_build_header(MSF_Context *msf, MSF_UInt stream_table_size)
{
  ProfBeginFunction();
  MSF_Error error = MSF_Error_OK;
...
void msf_bytedump_stream(char *file_name, MSF_Context *msf, MSF_StreamNumber sn, U64 start, U64 byte_count)
{
  Temp scratch = scratch_begin(0, 0);
...
void msf_hexdump_stream(FILE *file, MSF_Context *msf, MSF_StreamNumber sn, U64 start, U64 byte_count, U64 stride)
{
  Temp scratch = scratch_begin(0, 0);
...
void msf_hexdump_stream_to_file(char *name, MSF_Context *msf, MSF_StreamNumber sn, U64 start, U64 byte_count, U64 stride)
{
  FILE *f = fopen(name, "w");
