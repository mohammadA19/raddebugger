...
CV_DebugS *
lnk_parse_debug_s_sections(TP_Context* tp, TP_Arena* arena, uint64 obj_count, LNK_Obj** obj_arr, LNK_ChunkList* sect_list_arr)
{
  ProfBeginFunction();
...
CV_DebugT *
lnk_parse_debug_t_sections(TP_Context* tp, TP_Arena* arena, uint64 obj_count, LNK_Obj** obj_arr, LNK_ChunkList* debug_t_list_arr)
{
  ProfBeginFunction();
...
lnk_make_code_view_input(TP_Context* tp, TP_Arena* tp_arena, String8List lib_dir_list, LNK_ObjList obj_list)
{
  ...
  // obj list . array
  uint64       obj_count = obj_list.count;
...
lnk_leaf_bucket_array_sort(TP_Context* tp, LNK_LeafBucketArray arr, uint64 obj_count, uint64 type_server_count)
{
  ...
  if (arr.count > 140000) {
    ...
    task.loc_idx_bit_count_0   = Clamp(0, (int32)loc_idx_max_bits - 21, 11);
    task.loc_idx_bit_count_1   = Clamp(0, (int32)loc_idx_max_bits - 10, 11);
    task.loc_idx_bit_count_2   = Clamp(0, (int32)loc_idx_max_bits,      10);
                  ...
                  LNK_LeafHashTable*  leaf_ht_arr)
{
  ...
  uint64 max_ti_list_size = sizeof(CV_TypeIndexInfo) * (max_U16 / sizeof(CV_TypeIndex));
...
uint64
lnk_format_u128(uint8* buf, uint64 buf_max, uint64 length, U128 v)
{
  uint64 size = 0;
  if (length > 0 && buf_max > 0) {
    if (length <= 8) {
      uint64 mask = length == 8 ? max_U64 : (1ull << (length*8)) - 1;
      size = raddbg_snprintf((char*)buf, buf_max - 1, "%llX", (long long)(v.u64[0] & mask));
    } else {
      uint64 mask1 = length == 16 ? max_U64 : (1ull << ((length-8)*8)) - 1;
...
THREAD_POOL_TASK_FUNC(lnk_push_dbi_sec_contrib_task)
{
  ...
  for (uint64 chunk_idx = 0; chunk_idx < obj.sect_count; ++chunk_idx) {
  ...
  for (LNK_SymbolHashTrieChunk* chunk = chunk_list.first; chunk != 0; chunk = chunk.next) {
    ...
    for (uint64 i = 0, node_idx = 0; i < chunk.count; ++i) {
      ...
      if (defined_symbol.value_type == LNK_DefinedSymbolValue_Chunk) {
        ...
        uint64 symbol_off   = lnk_sect_off_from_symbol(sect_id_map, symbol);
        uint64 symbol_isect = lnk_isect_from_symbol(sect_id_map, symbol);

        uint32 symbol_off32   = safe_cast_u32(symbol_off);
        uint16 symbol_isect16 = safe_cast_u16(symbol_isect);
...
RDIB_Type *
lnk_type_from_itype(CV_TypeIndex itype, Rng1U64 tpi_itype_range, RDIB_Type** tpi_itype_map, LNK_Obj* obj, CV_SymKind symbol_kind, uint64 symbol_offset)
{
  RDIB_Type* type = 0;
...
uint64
lnk_virt_off_from_sect_off(uint64 sect_idx, uint64 sect_off, LNK_SectionArray image_sects, LNK_Obj* obj, CV_SymKind symbol_kind, uint64 symbol_offset)
{
  uint64 virt_off = 0;
...
Rng1U64
lnk_virt_range_from_sect_off_size(uint64 sect_idx, uint64 sect_off, uint64 size, LNK_SectionArray image_sects, LNK_Obj* obj, CV_SymKind symbol_kind, uint64 symbol_offset)
{
  Rng1U64 virt_range = {0};
  if (sect_idx < image_sects.count) {
    uint64 virt_off = image_sects.v[sect_idx].virt_off + sect_off;
  ...
  for (CV_SymbolNode* symbol_n = symbols_input.symbol_list.first; symbol_n != 0; symbol_n = symbol_n.next) {
    ...
    case CV_SymKind_REGREL32: {
      if (~scope_stack.proc_flags & CV_ProcFlag_OptDbgInfo) {
        ...
        uint32         value_size = 8;
        uint32         value_pos  = 0;
        rdib_push_location_addr_reg_off(arena, &local.locations, arch_rdi, reg_code, value_size, value_pos, (int64)regrel32.reg_off, is_ref, scope_stack.scope.ranges);
    ...
    case CV_SymKind_DEFRANGE_FRAMEPOINTER_REL: {
      ...
      rdib_push_location_addr_reg_off(arena, &scope_stack.defrange_target.locations, arch_rdi, fp_reg_rdi, value_size, value_pos, (int64)defrange_fprel.off, 0, ranges);
    } break;
    ...
    case CV_SymKind_DEFRANGE_FRAMEPOINTER_REL_FULL_SCOPE: {
      ...
      rdib_push_location_addr_reg_off(arena, &scope_stack.defrange_target.locations, arch_rdi, fp_reg_rdi, value_size, value_pos, (int64)defrange_fprelfs.off, 0, ranges);
    } break;
...
THREAD_POOL_TASK_FUNC(lnk_collect_obj_virtual_ranges_task)
{
  ...
  for (uint64 chunk_idx = 0; chunk_idx < obj.sect_count; ++chunk_idx) {
    ...
    uint64 chunk_voff = lnk_virt_off_from_chunk_ref(task.sect_id_map, chunk.ref);
    uint64 chunk_size = lnk_virt_size_from_chunk_ref(task.sect_id_map, chunk.ref);
                         ...
                         CV_DebugT                 types[CV_TypeIndexSource_COUNT])
{
  ...
  ProfBegin("Top Level Info");
  {
    ...
    for (uint64 sect_idx = 0; sect_idx < image_sects.count; sect_idx++) {
      ...
      uint64 sect_virt_size = lnk_virt_size_from_chunk_ref(sect_id_map, sect.root.ref);
      uint64 sect_voff_max  = sect.virt_off + sect_virt_size;
  ...
  ProfBegin("Sections");
  {
    ...
    for (uint64 sect_idx = 0; sect_idx < image_sects.count; ++sect_idx) {
      ...
      uint64 sect_virt_size = lnk_virt_size_from_chunk_ref(sect_id_map, src.root.ref);
      uint64 sect_file_size = lnk_file_size_from_chunk_ref(sect_id_map, src.root.ref);
