...
THREAD_POOL_TASK_FUNC(lnk_check_debug_t_sig_and_get_data_task)
{
  ...
  for (string *data_ptr = &data_arr.v[0], *data_opl = data_arr.v + data_arr.count;
       data_ptr < data_opl;
       ++data_ptr) {
    if (data_ptr.size == 0) {
      continue;
...
lnk_setup_pch(Arena *arena, U64 obj_count, LNK_Obj *obj_arr, CV_DebugT *debug_t_arr, CV_DebugT *debug_p_arr, CV_SymbolListArray *parsed_symbols)
{
  ...
  for (U64 obj_idx = 0; obj_idx < obj_count; ++obj_idx) {
    ...
    if (debug_p.count) {
      string obj_path = obj_arr[obj_idx].path;      
  ...
  for (U64 obj_idx = 0; obj_idx < obj_count; ++obj_idx) {
    ...
    if (cv_debug_t_is_pch(debug_t)) {
      ...
      string obj_path = path_absolute_dst_from_relative_dst_src(scratch.arena, precomp.obj_name, work_dir);
...
lnk_make_code_view_input(TP_Context *tp, TP_Arena *tp_arena, LNK_IO_Flags io_flags, String8List lib_dir_list, U64 obj_count, LNK_Obj **obj_arr)
{
  ...
  String8List *debug_s_list_arr = lnk_collect_obj_sections(tp, tp_arena, obj_count, obj_arr, (".debug$S"), collect_discarded_flag);
  String8List *debug_p_list_arr = lnk_collect_obj_sections(tp, tp_arena, obj_count, obj_arr, (".debug$P"), collect_discarded_flag);
  String8List *debug_t_list_arr = lnk_collect_obj_sections(tp, tp_arena, obj_count, obj_arr, (".debug$T"), collect_discarded_flag);
  ProfEnd();
  ...
  U64List     *ts_to_obj_arr     = push_array(tp_arena.v[0], U64List, external_count);
  {
    ...
    for (U64 obj_idx = 0; obj_idx < external_count; ++obj_idx) {
      ...
      if (match_list.node_count == 0) {
        string file_name = str8_skip_last_slash(ts.name);
      ...
      string path = match_list.first.string;
      {
        struct HT_Value {
    ...
    ts_path_arr.v                  = push_array(tp_arena.v[0], string, ts_info_list.count);
    ...
    // read type servers from disk in parallel
    {
      ...
      for (U64 ts_idx = 0; ts_idx < msf_data_arr.count; ++ts_idx) {
        ...
        if (!msf_parse) {
          ...
          if (!MemoryMatchStruct(&info_parse.guid, &ts_info_arr[ts_idx].sig)) {
            Temp scratch = scratch_begin(0,0);
            string expected_sig_str = string_from_guid(scratch.arena, ts_info_arr[ts_idx].sig);
            string on_disk_sig_str  = string_from_guid(scratch.arena, info_parse.guid);
      ...
      for (U64 ts_idx = 1; ts_idx < msf_data_arr.count; ++ts_idx) {
        if (task.is_corrupted[ts_idx]) {
          ...
          for (U64Node *obj_idx_node = obj_idx_list.first; obj_idx_node != 0; obj_idx_node = obj_idx_node.next) {
            string obj_path = external_obj_arr[obj_idx_node.data].path;
            str8_list_pushf(scratch.arena, &unopen_type_server_list, "\t\t\t%S\n", obj_path);
          }
        }
      }
      if (unopen_type_server_list.node_count) {
        ...
        string error_msg = str8_list_join(scratch.arena, &error_msg_list, 0);
  ...
  if (u128_match(a_hash, b_hash)) {
...
#if 0
    {
      ...
      for (U64 i = 0; i < a_raw_data_arr.count; ++i) {
        string a_chunk = a_raw_data_arr.v[i];
        string b_chunk = b_raw_data_arr.v[i];
...
lnk_match_leaf_ref_deep(Arena *arena, LNK_CodeViewInput *input, LNK_LeafHashes *hashes, LNK_LeafRef a, LNK_LeafRef b)
{
  ...
  if (u128_match(a_hash, b_hash)) {
    string a_raw_leaf = lnk_data_from_leaf_ref(input, a);
    string b_raw_leaf = lnk_data_from_leaf_ref(input, b);
  ...
  for (CV_TypeIndexInfo *ti_n = ti_info_list.first; ti_n != 0; ti_n = ti_n.next) {
    ...
    if (sub_ti >= ti_ranges[ti_n.source].min) {
      ...
      if (sub_ti < curr_ti) {
        ...
        Temp scratch = scratch_begin(0,0);
        string leaf_kind_str = cv_string_from_leaf_kind(leaf.kind);
        string leaf_info     = push_str8f(scratch.arena, "LF_%S(type_index: 0x%x) forward refs member type index 0x%x (leaf struct offset: 0x%llx)", leaf_kind_str, curr_ti, sub_ti, ti_n.offset);
        if (loc_type == LNK_LeafLocType_Internal) {
...
lnk_leaf_bucket_array_sort(TP_Context *tp, LNK_LeafBucketArray arr, U64 obj_count, U64 type_server_count)
{
  ...
  string leaf_count_string        = str8_from_count(scratch.arena, arr.count);
  string obj_count_string         = str8_from_count(scratch.arena, obj_count);
  string type_server_count_string = str8_from_count(scratch.arena, type_server_count);
...
THREAD_POOL_TASK_FUNC(lnk_patch_leaves_task)
{
  ...
  for (U64 bucket_idx = range.min; bucket_idx < range.max; ++bucket_idx) {
    ...
    string         raw_leaf = lnk_data_from_leaf_ref(task.input, bucket.leaf_ref);
...
THREAD_POOL_TASK_FUNC(lnk_unbucket_raw_leaves_task)
{
  ...
  Rng1U64                    range = task.range_arr[task_id];
  for (U64 i = range.min; i < range.max; ++i) {
    string raw_leaf = lnk_data_from_leaf_ref(task.input, task.bucket_arr[i]->leaf_ref);
...
#if PROFILE_TELEMETRY
    string count_string = str8_from_count(scratch.arena, input.internal_count);
...
#if PROFILE_TELEMETRY
  string obj_count_string = str8_from_count(tp_temp.v[0], input.internal_count);
  string tpi_count_string = str8_from_count(tp_temp.v[0], internal_per_source_count[CV_TypeIndexSource_TPI]);
  string ipi_count_string = str8_from_count(tp_temp.v[0], internal_per_source_count[CV_TypeIndexSource_IPI]);
...
THREAD_POOL_TASK_FUNC(lnk_process_c13_data_task)
{
  ...
  string string_data = cv_string_table_from_debug_s(debug_s);
  ...
  string        mod_c13_data = dbi_module_read_c13_data(scratch.arena, task.msf, mod);
...
THREAD_POOL_TASK_FUNC(lnk_gsi_hash_cv_list_task)
{
  ...
  for (U64 symbol_idx = range.min; symbol_idx < range.max; ++symbol_idx) {
    ...
    string name = cv_name_from_symbol(symbol.kind, symbol.data);
              ...
              CV_DebugT                 types[CV_TypeIndexSource_COUNT])
{
  ...
  ProfBegin("Build NatVis");
  {
    ...
    for (U64 i = 0; i < natvis_file_data_arr.count; ++i) {
      string natvis_file_path = natvis_file_path_arr.v[i];
      string natvis_file_data = natvis_file_data_arr.v[i];
      ...
      string ext = str8_skip_last_dot(natvis_file_path);
...
internal U64
lnk_udt_name_hash_table_hash(string string)
{
    ...
    if (cv_is_udt(leaf.kind)) {
      ...
      if (~udt_info.props & CV_TypeProp_FwdRef) {
        if (!cv_is_udt_name_anon(udt_info.name)) {
          string name       = cv_name_from_udt_info(udt_info);
    ...
    case CV_LeafKind_FIELDLIST: {
      ...
      for (U64 cursor = 0; cursor + sizeof(CV_LeafKind) <= src.data.size; ) {
        ...
        case CV_LeafKind_MEMBER: {
          ...
          string          name        = str8_cstring_capped(src.data.str + cursor + sizeof(CV_LeafMember) + offset.encoded_size, src.data.str + src.data.size);
        ...
        case CV_LeafKind_STMEMBER: {
          ...
          string         name      = str8_cstring_capped(st_member + 1, src.data.str + src.data.size);
        ...
        case CV_LeafKind_METHOD: {
          ...
          string        name   = str8_cstring_capped(method + 1, src.data.str + src.data.size);
        ...
        case CV_LeafKind_ONEMETHOD: {
          ...
          string name = str8_cstring_capped(src.data.str + cursor, src.data.str + src.data.size);
          cursor += name.size + 1;
        ...
        case CV_LeafKind_NESTTYPE: {
          ...
          string          name      = str8_cstring_capped(nest_type + 1, src.data.str + src.data.size);
        ...
        case CV_LeafKind_NESTTYPEEX: {
          ...
          string            name         = str8_cstring_capped(nest_type_ex + 1, src.data.str + src.data.size);
        ...
        case CV_LeafKind_ENUMERATE: {
          ...
          string           name      = str8_cstring_capped(src.data.str + cursor + sizeof(CV_LeafEnumerate) + value.encoded_size, src.data.str + src.data.size);
...
internal U64
lnk_src_file_hash_cv(string normal_full_path, CV_C13ChecksumKind checksum_kind, string checksum)
{
  XXH3_state_t state;
...
THREAD_POOL_TASK_FUNC(lnk_insert_src_files_task)
{
  ...
  string               string_table = cv_string_table_from_debug_s(debug_s);
  ...
  for (String8Node *raw_chksms_n = raw_chksms_list.first; raw_chksms_n != 0; raw_chksms_n = raw_chksms_n.next) {
    for (U64 cursor = 0; cursor + sizeof(CV_C13Checksum) <= raw_chksms_n.string.size; ) {
      ...
      // grab checksum
      string checksum = str8_substr(raw_chksms_n.string, rng_1u64(cursor + sizeof(CV_C13Checksum), 
                                                                    cursor + sizeof(CV_C13Checksum) + header.len));

      // grab file path
      Assert(header.name_off < string_table.size);
      string file_path = str8_cstring_capped(string_table.str + header.name_off, string_table.str + string_table.size);

      // normalize file path
      string normal_path = lnk_normalize_src_file_path(arena, file_path);
...
lnk_find_container_type(string name, Rng1U64 tpi_itype_range, LNK_UDTNameBucket **udt_name_buckets, U64 udt_name_buckets_cap, RDIB_Type **tpi_itype_map)
{
  ...
  if (delim_pos > 0) {
    ...
    string container_name      = str8_prefix(name, container_name_size);
...
THREAD_POOL_TASK_FUNC(lnk_convert_line_tables_to_rdi_task)
{
  ...
  // find sub sections
  string     raw_string_table = cv_string_table_from_debug_s(debug_s);
  string     raw_file_chksms  = cv_file_chksms_from_debug_s(debug_s);
  ...
  for (String8Node *raw_lines_node = raw_lines_list.first; raw_lines_node != 0; raw_lines_node = raw_lines_node.next) {
    ...
    for (CV_C13LinesHeaderNode *lines_node = parsed_list.first; lines_node != 0; lines_node = lines_node.next) {
      ...
      string file_path      = str8_cstring_capped(raw_string_table.str + checksum_header.name_off, raw_string_table.str + raw_string_table.size);
      string checksum_bytes = str8((U8 *) (checksum_header + 1), checksum_header.len);
      ...
      string               normal_path     = lnk_normalize_src_file_path(scratch.arena, file_path);
...
#define push_scope_frame() do { \
  ...
  for (CV_SymbolNode *symbol_n = symbols_input.symbol_list.first; symbol_n != 0; symbol_n = symbol_n.next) {
    ...
    case CV_SymKind_LDATA32: {
      ...
      string       name           = str8_cstring_capped(data32 + 1, symbol.data.str + symbol.data.size);
    ...
    case CV_SymKind_GTHREAD32: {
      ...
      string         name           = str8_cstring_capped(thread32 + 1, symbol.data.str + symbol.data.size);
    ...
    case CV_SymKind_GPROC32: {
      CV_SymProc32 *proc32     = (CV_SymProc32 *) symbol.data.str;
      string       name       = str8_cstring_capped(proc32 + 1, symbol.data.str + symbol.data.size);
    ...
    case CV_SymKind_THUNK32: {
      ...
      string        name       = str8_cstring_capped(thunk32 + 1, symbol.data.str + symbol.data.size);
    ...
    case CV_SymKind_REGREL32: {
      if (~scope_stack.proc_flags & CV_ProcFlag_OptDbgInfo) {
        ...
        string         name     = str8_cstring_capped(regrel32 + 1, symbol.data.str + symbol.data.size);
    ...
    case CV_SymKind_LOCAL: {
      ...
      string      name      = str8_cstring_capped(sym_local + 1, symbol.data.str + symbol.data.size);
    ...
    case CV_SymKind_FILESTATIC: {
      ...
      string           name        = str8_cstring_capped(file_static + 1, symbol.data.str + symbol.data.size);
    ...
    case CV_SymKind_INLINESITE: {
      ...
      string           binary_annots   = str8_skip(symbol.data, sizeof(*sym_inline_site));
                         ...
                         CV_DebugT                 types[CV_TypeIndexSource_COUNT])
{
  ...
  ProfBegin("Sections");
  {
    ...
    for (U64 sect_idx = 0; sect_idx < image_sects.count; ++sect_idx) {
      ...
      string sect_name = coff_name_from_section_header(image_strtab, src);
