...
CV_DebugS *
lnk_parse_debug_s_sections(TP_Context *tp, TP_Arena *arena, ulong obj_count, LNK_Obj **obj_arr, LNK_ChunkList *sect_list_arr)
{
  ProfBeginFunction();
...
CV_DebugT *
lnk_parse_debug_t_sections(TP_Context *tp, TP_Arena *arena, ulong obj_count, LNK_Obj **obj_arr, LNK_ChunkList *debug_t_list_arr)
{
  ProfBeginFunction();
...
LNK_PchInfo *
lnk_setup_pch(Arena *arena, ulong obj_count, LNK_Obj *obj_arr, CV_DebugT *debug_t_arr, CV_DebugT *debug_p_arr, CV_SymbolListArray *parsed_symbols)
{
  ...
  for (ulong obj_idx = 0; obj_idx < obj_count; ++obj_idx) {
...
void
lnk_do_debug_info_discard(CV_DebugS *debug_s_arr, CV_SymbolListArray *parsed_symbols, ulong obj_idx)
{
  // remove symbols
  for (ulong i = 0; i < parsed_symbols[obj_idx].count; ++i) {
...
CV_DebugT *
lnk_merge_debug_t_and_debug_p(Arena *arena, ulong obj_count, CV_DebugT *debug_t_arr, CV_DebugT *debug_p_arr)
{
...
lnk_make_code_view_input(TP_Context *tp, TP_Arena *tp_arena, String8List lib_dir_list, LNK_ObjList obj_list)
{
  ...
  // obj list -> array
  ulong       obj_count = obj_list.count;
  ...
  if (lnk_get_log_status(LNK_Log_Debug) || PROFILE_TELEMETRY) {
    ulong total_debug_s_size = 0, total_debug_t_size = 0, total_debug_p_size = 0;
    for (ulong obj_idx = 0; obj_idx < obj_count; ++obj_idx) {
  ...
  ulong external_count = 0, internal_count = 0;
  ...
  for (ulong obj_idx = 0; obj_idx < obj_count; ++obj_idx) {
    ...
    if (is_type_server) {
      ...
      ulong slot_idx = (obj_count - external_count - 1);
      ++external_count;
  ...
  ulong internal_total_symbol_input_count = 0;
  ulong external_total_symbol_input_count = 0;
  for (ulong obj_idx = 0; obj_idx < internal_count; ++obj_idx) {
  ...
  for (ulong obj_idx = 0; obj_idx < external_count; ++obj_idx) {
  ...
  ulong                       total_symbol_input_count = internal_total_symbol_input_count + external_total_symbol_input_count;
  ...
  CV_SymbolListArray       *parsed_symbols           = push_array_no_zero(tp_arena->v[0], CV_SymbolListArray,       obj_count);
  {
    ...
    for (ulong obj_idx = 0, input_idx = 0; obj_idx < obj_count; ++obj_idx) {
  ...
  ulong         *obj_to_ts_idx_arr = push_array_no_zero(tp_arena->v[0], ulong, external_count);
  U64List     *ts_to_obj_arr     = push_array(tp_arena->v[0], U64List, external_count);
  {
    ...
    for (ulong obj_idx = 0; obj_idx < external_count; ++obj_idx) {
      ...
      String8 path = match_list.first->string;
      {
        ...
        if (is_path_queued) {
          ...
          ulong ts_idx = ts_info_list.count;
    ...
    // read type servers from disk in parallel
    {
      ...
      for (ulong ts_idx = 0; ts_idx < msf_data_arr.count; ++ts_idx) {
      ...
      for (ulong ts_idx = 1; ts_idx < msf_data_arr.count; ++ts_idx) {
...
LNK_LeafRef
lnk_leaf_ref(uint enc_loc_idx, uint enc_leaf_idx)
{
  LNK_LeafRef ref;
...
LNK_LeafRef
lnk_obj_leaf_ref(uint obj_idx, uint leaf_idx)
{
  return lnk_leaf_ref(obj_idx, leaf_idx);
}

LNK_LeafRef
lnk_ts_leaf_ref(CV_TypeIndexSource ti_source, uint ts_idx, uint leaf_idx)
{
...
LNK_LeafLocType
lnk_loc_type_from_obj_idx(LNK_CodeViewInput *input, ulong obj_idx)
{
...
ulong
lnk_loc_idx_from_obj_idx(LNK_CodeViewInput *input, ulong obj_idx)
{
  if (input->external_obj_range.min <= obj_idx && obj_idx < input->external_obj_range.max) {
...
lnk_ti_lo_from_leaf_ref(LNK_CodeViewInput *input, LNK_LeafRef leaf_ref)
{
  ...
  case LNK_LeafLocType_External: {
    ulong                ts_idx    = leaf_ref.enc_loc_idx & ~LNK_LeafRefFlag_LocIdxExternal;
...
CV_TypeIndex
lnk_ti_lo_from_loc(LNK_CodeViewInput *input, LNK_LeafLocType loc_type, ulong loc_idx, CV_TypeIndexSource ti_source)
{
...
lnk_data_from_leaf_ref(LNK_CodeViewInput *input, LNK_LeafRef leaf_ref)
{
  ...
  switch (loc_type) {
  case LNK_LeafLocType_Internal: {
    uint       obj_idx  = leaf_ref.enc_loc_idx & ~LNK_LeafRefFlag_LocIdxExternal;
    uint       leaf_idx = leaf_ref.enc_leaf_idx;
  ...
  case LNK_LeafLocType_External: {
    ulong                ts_idx    = leaf_ref.enc_loc_idx & ~LNK_LeafRefFlag_LocIdxExternal;
    ulong                leaf_idx  = leaf_ref.enc_leaf_idx & ~LNK_LeafRefFlag_LeafIdxIPI;
...
lnk_hash_from_leaf_ref(LNK_LeafHashes *hashes, LNK_LeafRef leaf_ref)
{
  ...
  uint loc_idx  = leaf_ref.enc_loc_idx & ~LNK_LeafRefFlag_LocIdxExternal;
  uint leaf_idx = leaf_ref.enc_leaf_idx & ~LNK_LeafRefFlag_LeafIdxIPI;
...
lnk_match_leaf_ref(LNK_CodeViewInput *input, LNK_LeafHashes *hashes, LNK_LeafRef a, LNK_LeafRef b)
{
  ...
  if (u128_match(a_hash, b_hash)) {
...
#if 0
    {
      ...
      for (ulong i = 0; i < a_raw_data_arr.count; ++i) {
    ...
    if (a_header->kind == b_header->kind && a_header->size == b_header->size) {
      ...
      for (CV_TypeIndexInfo *ti_info = ti_info_list.first; ti_info != 0; ti_info = ti_info->next) {
        ...
        if (*a_ti_ptr >= a_ti_lo && *b_ti_ptr >= b_ti_lo) {
          ...
          ulong a_loc_idx = a.enc_loc_idx & ~LNK_LeafRefFlag_LocIdxExternal;
          ulong b_loc_idx = b.enc_loc_idx & ~LNK_LeafRefFlag_LocIdxExternal;
                 ...
                 CV_TypeIndexInfoList ti_info_list)
{
  ...
  // hash bytes around indices
  {
    ...
    for (ulong i = 0; i < raw_data_arr.count; ++i) {
  ...
  while (stack) {
    while (stack->ti_info) {
      ...
      if (*ti_ptr >= ti_ranges[curr_ti_info->source].min) {
        ...
        ulong ti_idx = (*ti_ptr - ti_ranges[curr_ti_info->source].min);
    ...
    if (!stack->ti_info) {
      ...
      if (stack != root_frame) {
        ...
        ulong leaf_idx = stack->ti - ti_ranges[stack->ti_source].min;
...
lnk_leaf_hash_table_insert_or_update(LNK_LeafHashTable *leaf_ht, LNK_CodeViewInput *input, LNK_LeafHashes *hashes, U128 new_hash, LNK_LeafBucket *new_bucket)
{
  ...
  ulong best_idx = u128_mod64(new_hash, leaf_ht->cap);
  ulong idx      = best_idx;
...
lnk_leaf_hash_table_search(LNK_LeafHashTable *ht, LNK_CodeViewInput *input, LNK_LeafHashes *hashes, LNK_LeafRef leaf_ref)
{
  ...
  ulong  best_bucket_idx = u128_mod64(hash, ht->cap);
  ulong  bucket_idx      = best_bucket_idx;
...
THREAD_POOL_TASK_FUNC(lnk_count_per_source_leaf_task)
{
  ...
  for (LNK_LeafRange *leaf_range = leaf_range_list.first; leaf_range != 0; leaf_range = leaf_range->next) {
    ...
    for (ulong leaf_idx = leaf_range->range.min; leaf_idx < leaf_range->range.max; ++leaf_idx) {
...
void
lnk_cv_debug_t_count_leaves_per_source(TP_Context *tp, ulong count, CV_DebugT *debug_t_arr, ulong per_source_count_arr[CV_TypeIndexSource_COUNT])
{
  ProfBeginFunction();
  Temp scratch = scratch_begin(0,0);

  ProfBegin("Compute Per Task Ranges");
  ulong                per_task_leaf_count  = 10000;
  LNK_LeafRangeList *leaf_ranges_per_task = push_array(scratch.arena, LNK_LeafRangeList, tp->worker_count);
  for (ulong i = 0, task_weight = 0, task_id = 0; i < count; ++i) {
    CV_DebugT *debug_t = &debug_t_arr[i];
    for (ulong k = 0; k < debug_t->count; k += per_task_leaf_count) {
      ulong cap = per_task_leaf_count - task_weight;
  ...
  for (ulong i = 0; i < CV_TypeIndexSource_COUNT; ++i) {
...
THREAD_POOL_TASK_FUNC(lnk_hash_debug_t_task)
{
  ...
  for (ulong ti_source = 0; ti_source < ArrayCount(ti_ranges); ++ti_source) {
    ti_ranges[ti_source] = rng_1u64(task->input->pch_arr[obj_idx].ti_lo, task->input->pch_arr[obj_idx].ti_hi + debug_t.count);
  }

  for (ulong leaf_idx = 0; leaf_idx < debug_t.count; ++leaf_idx) {
...
THREAD_POOL_TASK_FUNC(lnk_hash_type_server_leaves_task)
{
  ...
  ulong                ts_idx         = input->external_obj_to_ts_idx_arr[obj_idx];
  ...
  for (ulong i = 0; i < parsed_symbols.count; ++i) {
...
THREAD_POOL_TASK_FUNC(lnk_leaf_dedup_external_task)
{
  ...
  ulong                leaf_count = dim_1u64(input->external_ti_ranges[ts_idx][task->dedup_ti_source]);

  LNK_LeafBucket *bucket = 0;

  for (ulong leaf_idx = 0; leaf_idx < leaf_count; ++leaf_idx) {
...
THREAD_POOL_TASK_FUNC(lnk_count_present_buckets_task)
{
  ...
  for (ulong bucket_idx = task->range_arr[task_id].min; bucket_idx < task->range_arr[task_id].max; ++bucket_idx) {
...
THREAD_POOL_TASK_FUNC(lnk_get_present_buckets_task)
{
  ...
  ulong                cursor = task->offset_arr[task_id];
  LNK_LeafHashTable *ht     = task->ht;

  for (ulong bucket_idx = range.min; bucket_idx < range.max; ++bucket_idx) {
...
lnk_present_bucket_array_from_leaf_hash_table(TP_Context *tp, Arena *arena, LNK_LeafHashTable *ht)
{
  ...
  task.ht                        = ht;
  task.count_arr                 = push_array(scratch.arena, ulong, tp->worker_count);
...
THREAD_POOL_TASK_FUNC(lnk_leaf_ref_histo_task)
{
  ...
  uint                   *counts_ptr = task->counts_arr[task_id];

  uint loc_idx_bit_count_0 = task->loc_idx_bit_count_0;
  uint loc_idx_bit_count_1 = task->loc_idx_bit_count_1;
  uint loc_idx_bit_count_2 = task->loc_idx_bit_count_2;
  ...
  case 0: {
    for (ulong i = range.min; i < range.max; ++i) {
      ...
      ulong leaf_digit0 = BitExtract(bucket->leaf_ref.enc_leaf_idx, 10, 0);
  ...
  case 1: {
    for (ulong i = range.min; i < range.max; ++i) {
      LNK_LeafBucket *bucket = task->src[i];
      ulong leaf_digit1 = BitExtract(bucket->leaf_ref.enc_leaf_idx, 11, 10);
  ...
  case 2: {
    for (ulong i = range.min; i < range.max; ++i) {
      LNK_LeafBucket *bucket = task->src[i];
      ulong leaf_digit2 = BitExtract(bucket->leaf_ref.enc_leaf_idx, 11, 21 - 1); // don't take into account IPI flag
  ...
  case 3: {
    for (ulong i = range.min; i < range.max; ++i) {
      LNK_LeafBucket *bucket = task->src[i];
      ulong digit0 = BitExtract(bucket->leaf_ref.enc_loc_idx, loc_idx_bit_count_0, 0);
      ++counts_ptr[digit0];
    }
  } break;
  case 4: {
    for (ulong i = range.min; i < range.max; ++i) {
      LNK_LeafBucket *bucket = task->src[i];
      ulong digit1 = BitExtract(bucket->leaf_ref.enc_loc_idx, loc_idx_bit_count_1, loc_idx_bit_count_0);
      ++counts_ptr[digit1];
    }
  } break;
  case 5: {
    for (ulong i = range.min; i < range.max; ++i) {
      LNK_LeafBucket *bucket = task->src[i];
      ulong digit2 = BitExtract(bucket->leaf_ref.enc_loc_idx, loc_idx_bit_count_2, loc_idx_bit_count_0 + loc_idx_bit_count_1);

      ulong loc_bit = !!(bucket->leaf_ref.enc_loc_idx & LNK_LeafRefFlag_LocIdxExternal);
...
void
lnk_leaf_bucket_array_sort(TP_Context *tp, LNK_LeafBucketArray arr, ulong obj_count, ulong type_server_count)
{
  Temp scratch = scratch_begin(0,0);
  ...
  if (arr.count > 140000) {
    ...
    uint loc_idx_max_bits = 32 - clz32(Max(obj_count, type_server_count));
    ...
    task.counts_arr = push_array_no_zero(scratch.arena, uint *, tp->worker_count);
    for (ulong i = 0; i < tp->worker_count; ++i) {
      // zero-out happens in histogram step
      task.counts_arr[i] = push_array_no_zero(scratch.arena, uint, task.counts_max);
    }
    ProfEnd();
    ...
    for (task.pass_idx = 0; task.pass_idx < 6; ++task.pass_idx) {
      ...
      for (ulong task_id = 0; task_id < tp->worker_count; ++task_id) {
      ...
      {
        ulong digit_cursor = 0;
        for (ulong digit_idx = 0; digit_idx < task.counts_max; ++digit_idx) {
          for (ulong task_id = 0; task_id < tp->worker_count; ++task_id) {
            ulong count = task.counts_arr[task_id][digit_idx];
...
THREAD_POOL_TASK_FUNC(lnk_patch_symbols_task)
{
  ...
  ulong             loc_idx  = lnk_loc_idx_from_obj_idx(task->input, symbol_input.obj_idx);
                  ...
                  LNK_LeafHashTable  *leaf_ht_arr)
{
  ...
  ulong max_ti_list_size = sizeof(CV_TypeIndexInfo) * (max_U16 / sizeof(CV_TypeIndex));
...
THREAD_POOL_TASK_FUNC(lnk_patch_inlines_task)
{
  ...
  ulong             loc_idx          = lnk_loc_idx_from_obj_idx(task->input, task_id);
...
THREAD_POOL_TASK_FUNC(lnk_patch_leaves_task)
{
  ...
  for (ulong bucket_idx = range.min; bucket_idx < range.max; ++bucket_idx) {
    ...
    ulong             loc_idx  = bucket->leaf_ref.enc_loc_idx & ~LNK_LeafRefFlag_LocIdxExternal;
    ...
    if (symbol->kind == CV_SymKind_LPROC32_ID || symbol->kind == CV_SymKind_GPROC32_ID || symbol->kind == CV_SymKind_LPROC32_DPC) {
      ...
      if (proc32->itype >= task->ipi_min_type_index) {
        if ((proc32->itype - task->ipi_min_type_index) < task->ipi_types.count) {
          ulong     leaf_idx = proc32->itype - task->ipi_min_type_index;
...
lnk_import_types(TP_Context *tp, TP_Arena *tp_temp, LNK_CodeViewInput *input)
{
  ...
  LNK_LeafHashes *hashes = push_array(tp_temp->v[0], LNK_LeafHashes, 1);
  {
    ...
    for (ulong obj_idx = 0; obj_idx < input->internal_count; ++obj_idx) {
      CV_DebugT debug_t = input->merged_debug_t_p_arr[obj_idx];
      ...
      for (ulong ti_source = 0; ti_source < CV_TypeIndexSource_COUNT; ++ti_source) {
    ...
    for (ulong ts_idx = 0; ts_idx < input->type_server_count; ++ts_idx) {
      hashes->external_hashes[ts_idx] = push_array_no_zero(tp_temp->v[0], U128Array, CV_TypeIndexSource_COUNT);
      for (ulong ti_source = 0; ti_source < CV_TypeIndexSource_COUNT; ++ti_source) {
        ulong leaf_count = dim_1u64(input->external_ti_ranges[ts_idx][ti_source]);
  ...
  ulong internal_per_source_count[CV_TypeIndexSource_COUNT] = { 0 };
  ulong external_per_source_count[CV_TypeIndexSource_COUNT] = { 0 };
  {
...
ulong
lnk_format_u128(byte *buf, ulong buf_max, ulong length, U128 v)
{
  ulong size = 0;
  if (length > 0 && buf_max > 0) {
    if (length <= 8) {
      ulong mask = length == 8 ? max_U64 : (1ull << (length*8)) - 1;
      size = raddbg_snprintf((char*)buf, buf_max - 1, "%llX", (long long)(v.u64[0] & mask));
    } else {
      ulong mask1 = length == 16 ? max_U64 : (1ull << ((length-8)*8)) - 1;
...
THREAD_POOL_TASK_FUNC(lnk_replace_type_names_with_hashes_lenient_task)
{
  ...
  ulong                   hash_length = task->hash_length;
  ...
  ulong hash_max_chars = hash_length*2;
  byte  temp[128];

  for (ulong leaf_idx = range.min; leaf_idx < range.max; ++leaf_idx) {
    CV_Leaf leaf = cv_debug_t_get_leaf(debug_t, leaf_idx);
    if (leaf.kind == CV_LeafKind_STRUCTURE || leaf.kind == CV_LeafKind_CLASS) {
      ...
      if ((udt_info.props & CV_TypeProp_HasUniqueName) &&
        ...
        ulong numeric_size = cv_read_numeric(leaf.data, sizeof(CV_LeafStruct), &dummy);

        String8 lambda_prefix = str8_lit("<lambda_");
        ulong     colon_pos     = str8_find_needle_reverse(udt_info.name, 0, lambda_prefix, 0);
        B32     is_lambda     = colon_pos != 0;

        if (is_lambda) {
          ulong size = lnk_format_u128(temp, sizeof(temp), hash_length, name_hash);
...
void
lnk_replace_type_names_with_hashes(TP_Context *tp, TP_Arena *arena, CV_DebugT debug_t, LNK_TypeNameHashMode mode, ulong hash_length, String8 map_name)
{
  ProfBeginFunction();
...
THREAD_POOL_TASK_FUNC(lnk_process_sym_data_task)
{
  ...
  ulong buffer_size = sizeof(MODULE_SYMBOL_SIGNATURE);
  for (ulong i = 0; i < parsed_symbols.count; ++i) {
    CV_SymbolList list = parsed_symbols.v[i];
    ulong data_size = cv_patch_symbol_tree_offsets(list, buffer_size, PDB_SYMBOL_ALIGN);
...
lnk_process_c11_data(TP_Context *tp, TP_Arena *arena, ulong obj_count, CV_DebugS *debug_s_arr, ulong string_data_base_offset, CV_StringHashTable string_ht, MSF_Context *msf, PDB_DbiModule **mod_arr)
{
...
THREAD_POOL_TASK_FUNC(lnk_process_c13_data_task)
{
  ...
  ulong          checksum_base            = mod_checksum_data->total_size;
...
lnk_process_c13_data(TP_Context *tp, TP_Arena *arena, ulong obj_count, CV_DebugS *debug_s_arr, ulong string_data_base_offset, CV_StringHashTable string_ht, MSF_Context *msf, PDB_DbiModule **mod_arr)
{
  ProfBeginFunction();
...
THREAD_POOL_TASK_FUNC(lnk_write_module_data_task)
{
  ...
  uint sym_data_size32 = safe_cast_u32(sym_data.total_size);
  uint c11_data_size32 = safe_cast_u32(c11_data.total_size);
  uint c13_data_size32 = safe_cast_u32(c13_data.total_size);
  uint globrefs_size32 = safe_cast_u32(globrefs.total_size);
...
THREAD_POOL_TASK_FUNC(lnk_cv_symbol_ptr_array_hasher)
{
  ...
  Rng1U64                     range = task->range_arr[task_id];
  for (ulong symbol_idx = range.min; symbol_idx < range.max; ++symbol_idx) {
...
lnk_hash_cv_symbol_ptr_arr(TP_Context *tp, Arena *arena, CV_SymbolPtrArray arr)
{
  ...
  task.hash_arr                   = push_array_no_zero(arena, ulong, arr.count);
  task.arr                        = arr.v;
...
THREAD_POOL_TASK_FUNC(lnk_push_dbi_sec_contrib_task)
{
  ...
  for (ulong chunk_idx = 0; chunk_idx < obj->sect_count; ++chunk_idx) {
    ...
    ulong          chunk_size = lnk_file_size_from_chunk_ref(sect_id_map, chunk->ref);
    
    // compute chunk CRC
    uint data_crc  = update_crc32(0, chunk_data.str, chunk_data.size);
    uint reloc_crc = 0; // TODO: compute CRC for relocations block
  ...
  if (sc_count > 0) {
    for (ulong sc_idx = 0; sc_idx < sc_count; ++sc_idx) {
...
THREAD_POOL_TASK_FUNC(lnk_build_pdb_public_symbols_defined_task)
{
  ...
  for (LNK_SymbolHashTrieChunk *chunk = chunk_list.first; chunk != 0; chunk = chunk->next) {
    ...
    for (ulong i = 0, node_idx = 0; i < chunk->count; ++i) {
      ...
      if (defined_symbol->value_type == LNK_DefinedSymbolValue_Chunk) {
        ...
        ulong symbol_off   = lnk_sect_off_from_symbol(sect_id_map, symbol);
        ulong symbol_isect = lnk_isect_from_symbol(sect_id_map, symbol);

        uint symbol_off32   = safe_cast_u32(symbol_off);
              ...
              CV_DebugT                 types[CV_TypeIndexSource_COUNT])
{
  ...
  ulong string_data_base_offset = pdb->info->strtab.size;
  ...
  ProfBegin("Build DBI Modules");
  {
    TP_Temp temp = tp_temp_begin(tp_arena);
    {
      ...
      for (ulong obj_idx = 0; obj_idx < obj_count; ++obj_idx) {
        ...
        uint mod_size32 = safe_cast_u32(mod_size);
  ...
  ProfBegin("Build DBI Section Headers");
  {
    ...
    ulong                 coff_sect_count        = coff_sect_chunk_data.size / sizeof(COFF_SectionHeader);
  ...
  ProfBegin("Build NatVis");
  {
    ...
    for (ulong i = 0; i < natvis_file_data_arr.count; ++i) {
...
THREAD_POOL_TASK_FUNC(lnk_build_udt_name_hash_table_task)
{
  ...
  for (ulong leaf_idx = task->ranges[task_id].min; leaf_idx < task->ranges[task_id].max; ++leaf_idx) {
    ...
    if (cv_is_udt(leaf.kind)) {
      ...
      if (~udt_info.props & CV_TypeProp_FwdRef) {
        if (!cv_is_udt_name_anon(udt_info.name)) {
          ...
          ulong     hash       = lnk_udt_name_hash_table_hash(name);
          ulong     best_idx   = hash % task->buckets_cap;
          ulong     bucket_idx = best_idx;
...
LNK_UDTNameBucket *
lnk_udt_name_hash_table_lookup(LNK_UDTNameBucket **buckets, ulong cap, String8 name)
{
  ulong hash       = lnk_udt_name_hash_table_hash(name);
...
CV_TypeIndex
lnk_udt_name_hash_table_lookup_itype(LNK_UDTNameBucket **buckets, ulong cap, String8 name)
{
...
lnk_push_basic_itypes(Arena *arena, RDIB_DataModel data_model, RDIB_Type **itype_map, RDIB_TypeChunkList *rdib_types_list)
{
  ...
  for (ulong i = 0; i < ArrayCount(table); ++i) {
    ulong builtin_size;
...
lnk_rdib_type_from_itype(LNK_ConvertTypesToRDI *task, CV_TypeIndex itype)
{
  ...
  if (itype < tpi_range.max) {
    ...
    if (itype >= tpi_range.min) {
      ulong     leaf_idx = itype - tpi_range.min;
...
THREAD_POOL_TASK_FUNC(lnk_convert_types_to_rdi_task)
{
  ...
  ulong leaf_count = dim_1u64(task->ranges[task_id]);
  rdib_type_chunk_list_reserve(arena, &task->rdib_types_lists[task_id], leaf_count);

  for(ulong leaf_idx = task->ranges[task_id].min; leaf_idx < task->ranges[task_id].max; ++leaf_idx) {
    ulong     itype = task->itype_ranges[CV_TypeIndexSource_TPI].min + leaf_idx;
    ...
    case CV_LeafKind_POINTER: {
      ...
      uint             ptr_size = CV_PointerAttribs_Extract_Size(ptr->attribs);
      ()ptr_kind;
      ...
      for (next_itype = ptr->itype; task->itype_ranges[CV_TypeIndexSource_TPI].min <= next_itype && next_itype < task->itype_ranges[CV_TypeIndexSource_TPI].max;) {
        ulong     next_leaf_idx = next_itype - task->itype_ranges[CV_TypeIndexSource_TPI].min;
    ...
    case CV_LeafKind_ARGLIST: {
      ...
      for (ulong param_idx = 0; param_idx < arglist->count; ++param_idx) {
    ...
    case CV_LeafKind_FIELDLIST: {
      ...
      for (ulong cursor = 0; cursor + sizeof(CV_LeafKind) <= src.data.size; ) {
        CV_LeafKind field_kind = *(CV_LeafKind *) (src.data.str + cursor);
        cursor += sizeof(field_kind);

        // do we have bytes to read?
        ulong header_size = cv_header_struct_size_from_leaf_kind(field_kind);
        ...
        case CV_LeafKind_METHOD: {
          ...
          if (contains_1u64(task->itype_ranges[CV_TypeIndexSource_TPI], method->list_itype)) {
            ulong     method_list_leaf_idx = method->list_itype - task->itype_ranges[CV_TypeIndexSource_TPI].min;
            ...
            if (method_list_leaf.kind == CV_LeafKind_METHODLIST) {
              for (ulong cursor = 0; cursor + sizeof(CV_LeafMethodListMember) <= method_list_leaf.data.size; ) {
        ...
        case CV_LeafKind_BCLASS: {
          ...
          ulong offset64 = cv_u64_from_numeric(&offset);
...
LNK_SourceFileBucket *
lnk_src_file_insert_or_update(LNK_SourceFileBucket **buckets, ulong cap, ulong hash, LNK_SourceFileBucket *new_bucket)
{
...
THREAD_POOL_TASK_FUNC(lnk_count_source_files_task)
{
  ...
  for (String8Node *raw_chksms_n = raw_chksms_list.first; raw_chksms_n != 0; raw_chksms_n = raw_chksms_n->next) {
    for(ulong cursor = 0; cursor + sizeof(CV_C13Checksum) <= raw_chksms_n->string.size; ) {
      // parse header
...
THREAD_POOL_TASK_FUNC(lnk_insert_src_files_task)
{
  ...
  for (String8Node *raw_chksms_n = raw_chksms_list.first; raw_chksms_n != 0; raw_chksms_n = raw_chksms_n->next) {
    for (ulong cursor = 0; cursor + sizeof(CV_C13Checksum) <= raw_chksms_n->string.size; ) {
      ...
      // insert bucket
      ulong                   normal_path_hash = lnk_src_file_hash_cv(normal_path, header->kind, checksum);
...
RDIB_Type *
lnk_find_container_type(String8 name, Rng1U64 tpi_itype_range, LNK_UDTNameBucket **udt_name_buckets, ulong udt_name_buckets_cap, RDIB_Type **tpi_itype_map)
{
  CV_TypeIndex container_itype = 0;

  String8 delim     = str8_lit("::");
  ulong     delim_pos = str8_find_needle_reverse(name, 0, delim, 0);
  if (delim_pos > 0) {
    ulong     container_name_size = delim_pos - delim.size;
...
RDIB_Type *
lnk_type_from_itype(CV_TypeIndex itype, Rng1U64 tpi_itype_range, RDIB_Type **tpi_itype_map, LNK_Obj *obj, CV_SymKind symbol_kind, ulong symbol_offset)
{
  RDIB_Type *type = 0;
...
ulong
lnk_virt_off_from_sect_off(ulong sect_idx, ulong sect_off, LNK_SectionArray image_sects, LNK_Obj *obj, CV_SymKind symbol_kind, ulong symbol_offset)
{
  ulong virt_off = 0;
...
Rng1U64
lnk_virt_range_from_sect_off_size(ulong sect_idx, ulong sect_off, ulong size, LNK_SectionArray image_sects, LNK_Obj *obj, CV_SymKind symbol_kind, ulong symbol_offset)
{
  Rng1U64 virt_range = {0};
  if (sect_idx < image_sects.count) {
    ulong virt_off = image_sects.v[sect_idx].virt_off + sect_off;
...
THREAD_POOL_TASK_FUNC(lnk_find_obj_compiler_info_task)
{
  ...
  for (ulong symbol_list_idx = 0; symbol_list_idx < parsed_symbols.count; ++symbol_list_idx) {
  ...
  // fill out unit info
  ulong unit_chunk_idx = task_id / task->unit_chunk_cap;
  ulong local_unit_idx = task_id - unit_chunk_idx * task->unit_chunk_cap;
...
THREAD_POOL_TASK_FUNC(lnk_convert_line_tables_to_rdi_task)
{
  ...
  ulong        unit_chunk_idx = unit_idx / task->unit_chunk_cap;
  ulong        local_unit_idx = unit_idx - unit_chunk_idx * task->unit_chunk_cap;
  RDIB_Unit *dst            = &task->units[unit_chunk_idx].v[local_unit_idx];
  ...
  for (String8Node *raw_lines_node = raw_lines_list.first; raw_lines_node != 0; raw_lines_node = raw_lines_node->next) {
    ...
    for (CV_C13LinesHeaderNode *lines_node = parsed_list.first; lines_node != 0; lines_node = lines_node->next) {
      ...
      ulong                   src_file_hash   = lnk_src_file_hash_cv(normal_path, checksum_header->kind, checksum_bytes);
...
#define push_scope_frame() do { \
  ...
  for (CV_SymbolNode *symbol_n = symbols_input.symbol_list->first; symbol_n != 0; symbol_n = symbol_n->next) {
    ...
    case CV_SymKind_LDATA32: {
      ...
      ulong           data_voff      = lnk_virt_off_from_sect_off(data32->sec, data32->off, task->image_sects, obj, symbol.kind, symbol.offset);
      ...
      RDIB_LocationList locations = {0};
      {
        ...
        ulong data_size = rdib_size_from_type(type);
        if (data_size == 0) {
          data_size = max_U64;
        }
    ...
    case CV_SymKind_REGREL32: {
      if (~scope_stack->proc_flags & CV_ProcFlag_OptDbgInfo) {
        ...
        if (scope_stack->regrel32_idx < scope_stack->param_count) {
          ...
          if (type != 0) { 
            ulong byte_size = rdib_size_from_type(type);
    ...
    case CV_SymKind_DEFRANGE_REGISTER: {
      ...
      ulong                     gap_count    = (symbol.data.size - sizeof(*defrange_reg)) / sizeof(*gaps);
    ...
    case CV_SymKind_DEFRANGE_FRAMEPOINTER_REL: {
      ...
      ulong                            gap_count      = (symbol.data.size - sizeof(*defrange_fprel)) / sizeof(gaps[0]);
      ...
      Rng1U64List           ranges         = cv_make_defined_range_list_from_gaps(arena, defrange, gaps, gap_count);
      uint                   value_pos      = 0;
      uint                   value_size     = rdi_addr_size_from_arch(arch_rdi);
    ...
    case CV_SymKind_DEFRANGE_SUBFIELD_REGISTER: {
      ...
      ulong                             gap_count                  = (symbol.data.size - sizeof(*defrange_subfield_register)) / sizeof(gaps[0]);
      RDI_RegCode                     reg_rdi                    = rdi_reg_code_from_cv(comp_info.arch, defrange_subfield_register->reg);
      uint                             value_pos                  = CV_DefrangeSubfieldRegister_Extract_ParentOffset(defrange_subfield_register->field_offset);
      uint                             value_size                 = cv_size_from_reg(comp_info.arch, defrange_subfield_register->reg) - value_pos;
    ...
    case CV_SymKind_DEFRANGE_FRAMEPOINTER_REL_FULL_SCOPE: {
      ...
      CV_Reg                                  fp_reg           = cv_decode_fp_reg(comp_info.arch, encoded_fp_reg);
      RDI_RegCode                             fp_reg_rdi       = rdi_reg_code_from_cv(comp_info.arch, fp_reg);
      uint                                     value_size       = cv_size_from_reg(comp_info.arch, fp_reg);
      uint                                     value_pos        = 0;
    ...
    case CV_SymKind_DEFRANGE_REGISTER_REL: {
      ...
      ulong                        gap_count             = (symbol.data.size - sizeof(*defrange_register_rel)) / sizeof(gaps[0]);
      RDI_RegCode                reg_rdi               = rdi_reg_code_from_cv(comp_info.arch, defrange_register_rel->reg);
      ulong                        value_size            = cv_size_from_reg(comp_info.arch, defrange_register_rel->reg);
      ulong                        value_pos             = 0;
    ...
    case CV_SymKind_INLINESITE: {
      ...
      if (task->ipi_itype_range.min <= sym_inline_site->inlinee && sym_inline_site->inlinee < task->ipi_itype_range.max) {
        ulong     leaf_idx = sym_inline_site->inlinee - task->tpi_itype_range.min;
...
THREAD_POOL_TASK_FUNC(lnk_convert_inline_site_line_tables_task)
{
  ...
  for (ulong i = 0; i < chunk->count; ++i) {
    ...
    ulong           lines_count = inline_site->convert_ref.ud1;
    ulong           obj_idx     = inline_site->convert_ref.ud2;
    ...
    for (ulong file_idx = 0; file_idx < lines_count; ++file_idx) {
...
THREAD_POOL_TASK_FUNC(lnk_collect_obj_virtual_ranges_task)
{
  ...
  for (ulong chunk_idx = 0; chunk_idx < obj->sect_count; ++chunk_idx) {
    ...
    ulong chunk_voff = lnk_virt_off_from_chunk_ref(task->sect_id_map, chunk->ref);
    ulong chunk_size = lnk_virt_size_from_chunk_ref(task->sect_id_map, chunk->ref);
                         ...
                         CV_DebugT                 types[CV_TypeIndexSource_COUNT])
{
  ...
  {
    ulong image_vsize = 0;
    for (ulong sect_idx = 0; sect_idx < image_sects.count; sect_idx++) {
      LNK_Section *sect  = &image_sects.v[sect_idx];
      ulong sect_virt_size = lnk_virt_size_from_chunk_ref(sect_id_map, sect->root->ref);
      ulong sect_voff_max  = sect->virt_off + sect_virt_size;
  ...
  ProfBegin("Sections");
  {
    ...
    for (ulong sect_idx = 0; sect_idx < image_sects.count; ++sect_idx) {
      ...
      ulong sect_virt_size = lnk_virt_size_from_chunk_ref(sect_id_map, src->root->ref);
      ulong sect_file_size = lnk_file_size_from_chunk_ref(sect_id_map, src->root->ref);
  ...
  for (ulong i = 0; i < ArrayCount(itype_ranges); ++i) {
  ...
  LNK_SourceFileBucket **src_file_buckets;
  {
    ...
    task.src_file_buckets_cap = (ulong)(task.total_src_file_count * 1.3);
  ...
  for (ulong bucket_idx = 0; bucket_idx < src_file_buckets_cap; ++bucket_idx) {
