...
{
  U32 *result = arena.PushArrayNoZero<U32>(ht->count);
...
{
  U64 *result = arena.PushArrayNoZero<U64>(ht->count);
  for (U64 bucket_idx = 0, cursor = 0; bucket_idx < ht->cap; ++bucket_idx) {
    for (BucketNode *n = ht->buckets[bucket_idx].first; n != 0; n = n->next) {
      Assert(cursor < ht->count);
...
{
  KeyValuePair *pairs = arena.PushArrayNoZero<KeyValuePair>(ht->count);
  for (U64 bucket_idx = 0, cursor = 0; bucket_idx < ht->cap; ++bucket_idx) {
    for (BucketNode *n = ht->buckets[bucket_idx].first; n != 0; n = n->next) {
      Assert(cursor < ht->count);
      pairs[cursor++] = n->v;
