...
lnk_file_search(Arena *arena, String8List dir_list, string file_path)
{
  ...
  if (os_file_path_exists(file_path)) {
    string str = push_str8_copy(arena, file_path);
  ...
  if (is_relative) {
    for (String8Node *i = dir_list.first; i != 0; i = i.next) {
      ...
      string path = str8_path_list_join_by_style(scratch.arena, &path_list, PathStyle_SystemAbsolute);
      ...
      if (file_exists) {
        ...
        if (is_unique) {
          string str = push_str8_copy(arena, path);
          str8_list_push(arena, &match_list, str);
        }
      }
    }
...
internal OS_Handle
lnk_file_open_with_rename_permissions(string path)
{
...
internal B32
lnk_file_rename(OS_Handle handle, string new_name)
{
  Temp scratch = scratch_begin(0,0);
#if OS_WINDOWS
...
internal void
lnk_log_read(string path, U64 size)
{
...
THREAD_POOL_TASK_FUNC(lnk_data_size_from_file_path_task)
{
  ...
  string         path = task.path_arr.v[task_id];
...
lnk_read_data_from_file_path_parallel(TP_Context *tp, Arena *arena, LNK_IO_Flags io_flags, String8Array path_arr)
{
  ...
  if (io_flags & LNK_IO_Flags_MemoryMapFiles) {
    ...
    reader.data_arr.v     = push_array(arena, string, path_arr.count);
...
internal void
lnk_write_data_to_file_path(string path, string temp_path, string data)
{
  Temp scratch = scratch_begin(0,0);
