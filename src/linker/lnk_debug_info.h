...
internal CV_DebugS *       lnk_parse_debug_s_sections(TP_Context *tp, TP_Arena *arena, ulong obj_count, LNK_Obj **obj_arr, LNK_ChunkList *sect_list_arr);
internal CV_DebugT *       lnk_parse_debug_t_sections(TP_Context *tp, TP_Arena *arena, ulong obj_count, LNK_Obj **obj_arr, LNK_ChunkList *debug_t_list_arr);
internal CV_SymbolList *   lnk_cv_symbol_list_arr_from_debug_s_arr(TP_Context *tp, TP_Arena *arena, ulong obj_count, CV_DebugS *debug_s_arr);
internal LNK_PchInfo *     lnk_setup_pch(Arena *arena, ulong obj_count, LNK_Obj *obj_arr, CV_DebugT *debug_t_arr, CV_DebugT *debug_p_arr, CV_SymbolListArray *parsed_symbols);
...
internal LNK_LeafRef      lnk_leaf_ref(uint idx, uint leaf_idx);
internal LNK_LeafRef      lnk_obj_leaf_ref(uint obj_idx, uint leaf_idx);
internal LNK_LeafRef      lnk_ts_leaf_ref(CV_TypeIndexSource ti_source, uint ts_idx, uint leaf_idx);
...
internal LNK_LeafLocType  lnk_loc_type_from_obj_idx(LNK_CodeViewInput *input, ulong obj_idx);
internal ulong              lnk_loc_idx_from_obj_idx(LNK_CodeViewInput *input, ulong obj_idx);
internal CV_TypeIndex     lnk_ti_lo_from_loc(LNK_CodeViewInput *input, LNK_LeafLocType loc_type, ulong loc_idx, CV_TypeIndexSource ti_source);
...
internal LNK_LeafRef      lnk_leaf_ref_from_loc_idx_and_ti(LNK_CodeViewInput *input, LNK_LeafLocType loc_type, CV_TypeIndexSource ti_source, ulong loc_idx, CV_TypeIndex obj_ti);
...
internal U128             lnk_hash_cv_leaf(Arena *arena, LNK_CodeViewInput *input, LNK_LeafHashes *hashes, LNK_LeafLocType loc_type, uint loc_idx, Rng1U64 *ti_ranges, CV_TypeIndex curr_ti, CV_Leaf leaf, CV_TypeIndexInfoList ti_info_list);
internal void             lnk_hash_cv_leaf_deep(Arena *arena, LNK_CodeViewInput *input, Rng1U64 *ti_ranges, CV_DebugT *leaves, LNK_LeafHashes *hashes, LNK_LeafLocType loc_type, uint loc_idx, CV_TypeIndexInfoList ti_info_list, String8 data);
...
internal void                lnk_cv_debug_t_count_leaves_per_source(TP_Context *tp, ulong count, CV_DebugT *debug_t_arr, ulong *per_source_count_arr);
internal void                lnk_hash_debug_t_arr(TP_Context *tp, Arena *arena, ulong obj_count, CV_DebugT *debug_t_arr, U128Array *hash_arr_arr);
...
internal void                lnk_leaf_bucket_array_sort_radix_subset_parallel(TP_Context *tp, ulong bucket_count, ulong loc_idx_max, LNK_LeafBucket **dst, LNK_LeafBucket **src);
internal void                lnk_leaf_bucket_array_sort_radix_parallel(TP_Context *tp, LNK_LeafBucketArray arr, ulong obj_count, ulong type_server_count);
...
internal void                lnk_patch_inlines(TP_Context *tp, LNK_CodeViewInput *input, LNK_LeafHashes *hashes, LNK_LeafHashTable *leaf_ht_arr, ulong obj_count, CV_DebugS *debug_s_arr);
...
internal void lnk_replace_type_names_with_hashes(TP_Context *tp, TP_Arena *arena, CV_DebugT debug_t, LNK_TypeNameHashMode mode, ulong hash_length, String8 map_name);

////////////////////////////////
// RAD Debug info

internal ulong                  lnk_udt_name_hash_table_hash(String8 string);
internal LNK_UDTNameBucket ** lnk_udt_name_hash_table_from_debug_t(TP_Context *tp, TP_Arena *arena, CV_DebugT debug_t, ulong *buckets_cap_out);
internal LNK_UDTNameBucket *  lnk_udt_name_hash_table_lookup(LNK_UDTNameBucket **buckets, ulong cap, String8 name);
internal CV_TypeIndex *       lnk_build_udt_fwdmap(TP_Context *tp, Arena *arena, CV_DebugT debug_t, CV_TypeIndex ti_lo, LNK_UDTNameBucket **udt_name_buckets, ulong udt_name_buckets_cap);
...
internal LNK_SourceFileBucket * lnk_src_file_hash_table_lookup_slot(LNK_SourceFileBucket **src_file_buckets, ulong src_file_buckets_cap, ulong hash, String8 file_path, CV_C13ChecksumKind checksum_kind, String8 checksum_bytes);
...
internal LNK_ProcessedCodeViewC11Data lnk_process_c11_data(TP_Context *tp, TP_Arena *arena, ulong obj_count, CV_DebugS *debug_s_arr, ulong string_data_base_offset, CV_StringHashTable string_ht, MSF_Context *msf, PDB_DbiModule **mod_arr);
internal LNK_ProcessedCodeViewC13Data lnk_process_c13_data(TP_Context *tp, TP_Arena *arena, ulong obj_count, CV_DebugS *debug_s_arr, ulong string_data_base_offset, CV_StringHashTable string_ht, MSF_Context *msf, PDB_DbiModule **mod_arr);
internal ulong *                        lnk_hash_cv_symbol_ptr_arr(TP_Context *tp, Arena *arena, CV_SymbolPtrArray arr);
internal CV_SymbolPtrArray            lnk_dedup_gsi_symbols(TP_Context *tp, Arena *arena, PDB_GsiContext *gsi, ulong obj_count, CV_SymbolList *symbol_list_arr);
