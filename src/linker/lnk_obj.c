...
THREAD_POOL_TASK_FUNC(lnk_obj_initer)
{
  ...
  // extract COFF info
  //
  string raw_coff_section_table = str8_substr(input.data, header.section_table_range);
  string raw_coff_symbol_table  = str8_substr(input.data, header.symbol_table_range);
  string raw_coff_string_table  = str8_substr(input.data, header.string_table_range);
  ...
  for (U64 sect_idx = 0; sect_idx < header.section_count_no_null; sect_idx += 1) {
    ...
    // read name
    string sect_name = coff_name_from_section_header(raw_coff_string_table, coff_sect_header);
  ...
  //
  {
    ...
    string string_table = str8_substr(input.data, header.string_table_range);
    string symbol_table = str8_substr(input.data, header.symbol_table_range);
  ...
  if (header.machine == COFF_MachineType_X64) {
    ...
    for (U64 sect_idx = 0; sect_idx < obj.header.section_count_no_null; sect_idx += 1) {
      ...
      string name = str8_cstring_capped_reverse(sect_header.name, sect_header.name+sizeof(sect_header.name));
      if (str8_match(name, (".debug$S"), 0)) {
        Temp temp = temp_begin(scratch.arena);
        string debug_s_data = str8_substr(input.data, rng_1u64(sect_header.foff, sect_header.foff+sect_header.fsize));
...
lnk_obj_match_symbol(LNK_Obj *obj, string match_name)
{
  ...
  string raw_coff_symbol_table = str8_substr(obj.data, coff_info.symbol_table_range);
  string raw_coff_string_table = str8_substr(obj.data, coff_info.string_table_range);
...
lnk_is_coff_section_debug(LNK_Obj *obj, U64 sect_idx)
{
  ...
  string full_name = coff_name_from_section_header(string_table, section_header);
  string name, postfix;
...
THREAD_POOL_TASK_FUNC(lnk_collect_obj_chunks_task)
{
  ...
  for (U32 sect_idx = 0; sect_idx < obj.header.section_count_no_null; sect_idx += 1) {
    ...
    if (str8_match(section_name, task.name, 0)) {
      string section_data = str8_substr(obj.data, rng_1u64(section_header.foff, section_header.foff + section_header.fsize));
...
lnk_raw_directives_from_obj(Arena *arena, LNK_Obj *obj)
{
  ...
  for (U64 sect_idx = 0; sect_idx < obj.header.section_count_no_null; sect_idx += 1) {
    ...
    if (sect_header.flags & COFF_SectionFlag_LnkInfo) {
      string sect_name = str8_cstring_capped(sect_header.name, sect_header.name + sizeof(sect_header.name));
