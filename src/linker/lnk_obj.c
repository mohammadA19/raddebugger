...
THREAD_POOL_TASK_FUNC(lnk_obj_initer)
{
  ...
  for (uint64 sect_idx = 0; sect_idx < header.section_count_no_null; sect_idx += 1) {
    ...
    if (~coff_sect_header->flags & COFF_SectionFlag_CntUninitializedData) {
      if (coff_sect_header->fsize > 0) {
        Rng1uint64 sect_range = rng_1u64(coff_sect_header->foff, coff_sect_header->foff + coff_sect_header->fsize);
  ...
  //
  {
    ...
    for (uint64 symbol_idx = 0; symbol_idx < header.symbol_count; symbol_idx += (1 + symbol.aux_symbol_count)) {
  ...
  uint32 *comdats;
  {
    comdats = push_array_no_zero(arena, uint32, header.section_count_no_null);
  ...
  //
  {
    ...
    for (uint64 sect_idx = 0; sect_idx < header.section_count_no_null; sect_idx += 1) {
      for (uint32 curr_section = sect_idx;;) {
        uint32 symbol_idx = comdats[curr_section];
...
lnk_obj_match_symbol(LNK_Obj *obj, String8 match_name)
{
  ...
  COFF_ParsedSymbol symbol;
  for (uint64 symbol_idx = 0; symbol_idx < coff_info.symbol_count; symbol_idx += (1 + symbol.aux_symbol_count)) {
    void *symbol_ptr;
...
lnk_coff_section_header_from_section_number(LNK_Obj *obj, uint64 section_number)
{
...
internal B32
lnk_is_coff_section_debug(LNK_Obj *obj, uint64 sect_idx)
{
...
internal COFF_ParsedSymbol
lnk_parsed_symbol_from_coff_symbol_idx(LNK_Obj *obj, uint64 symbol_idx)
{
  String8 string_table = str8_substr(obj->data, obj->header.string_table_range);
...
THREAD_POOL_TASK_FUNC(lnk_collect_obj_chunks_task)
{
  ...
  COFF_SectionHeader *section_table = (COFF_SectionHeader *)str8_substr(obj->data, obj->header.section_table_range).str;
  String8             string_table  = str8_substr(obj->data, obj->header.string_table_range);
  for (uint32 sect_idx = 0; sect_idx < obj->header.section_count_no_null; sect_idx += 1) {
...
lnk_collect_obj_sections(TP_Context *tp, TP_Arena *arena, uint64 objs_count, LNK_Obj **objs, String8 name, B32 collect_discarded)
{
...
lnk_parse_msvc_linker_directive(Arena *arena, LNK_Obj *obj, LNK_DirectiveInfo *directive_info, String8 buffer)
{
  ...
  String8 to_parse;
  {
    local_persist const uint8 bom_sig[]   = { 0xEF, 0xBB, 0xBF };
    local_persist const uint8 ascii_sig[] = { 0x20, 0x20, 0x20 };
