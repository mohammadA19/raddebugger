...
internal LNK_Reloc *
lnk_reloc_list_reserve(Arena *arena, LNK_RelocList *list, ulong count)
{
  LNK_Reloc *arr = NULL;
  if (count) {
...
internal void
lnk_reloc_list_concat_in_place_arr(LNK_RelocList *list, LNK_RelocList *arr, ulong count)
{
  SLLConcatInPlaceArray(list, arr, count);
}

internal LNK_RelocList **
lnk_make_reloc_list_arr_arr(Arena *arena, ulong slot_count, ulong per_count)
{
...
internal LNK_RelocList
lnk_reloc_list_from_coff_reloc_array(Arena *arena, COFF_MachineType machine, LNK_Chunk *chunk, LNK_SymbolArray symbol_array, COFF_Reloc *reloc_v, ulong reloc_count)
{
  ...
  for (; reloc_ptr < reloc_opl; ++reloc_ptr, ++coff_reloc_ptr) {
    ...
    ulong            apply_off   = coff_reloc_ptr->apply_off;
...
lnk_ext_reloc_type_from_coff(COFF_MachineType machine, uint type)
{
