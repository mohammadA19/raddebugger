...
lnk_lib_list_reserve(Arena *arena, LNK_LibList *list, ulong count)
{
  LNK_LibNode *arr = 0;
  if (count) {
...
internal void
lnk_lib_symbol_array_sort(LNK_LibSymbol *arr, ulong count)
{
  Assert(count >= 2);
...
lnk_lib_from_data(Arena *arena, String8 data, String8 path)
{
  ...
  if (parse.second_member.member_count) {
    ...
    member_off_arr = push_array_no_zero(arena, uint, symbol_count);
    
    // decompress member offsets
    for (ulong symbol_idx = 0; symbol_idx < symbol_count; symbol_idx += 1) {
      ushort off_number = second_member.symbol_indices[symbol_idx];
  ...
  // and lld-link with /DLL emits only first member
    ...
    for (uint offset_idx = 0; offset_idx < symbol_count; offset_idx += 1) {
...
lnk_lib_list_push_parallel(TP_Context *tp, TP_Arena *arena, LNK_LibList *list, String8Array data_arr, String8Array path_arr)
{
  ...
  ulong lib_count = data_arr.count;
...
lnk_lib_writer_push_obj(LNK_LibWriter *writer, LNK_Obj *obj)
{
  ...
  ulong member_idx = writer->member_list.count;
...
internal void
lnk_lib_writer_push_export(LNK_LibWriter *writer, COFF_MachineType machine, ulong time_stamp, String8 dll_name, LNK_Export *exp)
{
  ProfBeginFunction();
...
lnk_coff_archive_from_lib_build(Arena *arena, LNK_LibBuild *lib, B32 emit_second_member, COFF_TimeStamp time_stamp, uint mode)
{
  ProfBeginFunction();
  
  Temp scratch = scratch_begin(&arena, 1);

  ulong            symbol_count = lib->symbol_count - 2;
  LNK_LibSymbol *symbol_arr   = lib->symbol_array + 1;

  HashTable   *name_ht          = hash_table_init(scratch.arena, 1024);
  ulong         *member_off_arr   = push_array_no_zero(scratch.arena, ulong, lib->member_count);
  String8List  long_names_list  = {0};
  String8List  member_data_list = {0};
  
  for (ulong member_idx = 0; member_idx < lib->member_count; member_idx += 1) {
    LNK_LibMember *member = &lib->member_array[member_idx];
    
    // make member name
    String8 name;
    ulong name_with_slash_size = member->name.size + 1;
  ...
  if (long_names_list.total_size > 0) {
    ...
    ulong member_offset = member_data_list.total_size + data.size + header.size;
  ...
  byte *name_buffer = push_array_no_zero(scratch.arena, byte, name_buffer_size);
  {
    ulong name_cursor = 0;
    for (LNK_LibSymbol *ptr = &symbol_arr[0], *opl = ptr + symbol_count; ptr < opl; ptr += 1) {
  ...
  ulong member_base_off;
  {
    ulong sizeof_first_header  = sizeof(COFF_ArchiveMemberHeader) + sizeof(uint) + sizeof(uint) * symbol_count + name_buffer_size;
    ulong sizeof_second_header = sizeof(COFF_ArchiveMemberHeader) + sizeof(uint) + sizeof(uint) * lib->member_count + sizeof(uint) + sizeof(ushort) * symbol_count + name_buffer_size;
    ulong sizeof_long_names    = sizeof(COFF_ArchiveMemberHeader) + long_names_list.total_size;
  ...
  if (emit_second_member) {
    uint member_count32 = safe_cast_u32(lib->member_count);
    uint symbol_count32 = safe_cast_u32(symbol_count);

    uint *member_off32_arr = push_array_no_zero(scratch.arena, uint, lib->member_count);
    ushort *member_idx16_arr = push_array_no_zero(scratch.arena, ushort, symbol_count);

    // write member offset array
    for (ulong member_idx = 0; member_idx < lib->member_count; member_idx += 1) {
      ulong member_off = member_base_off + member_off_arr[member_idx];
      uint member_off32 = safe_cast_u32(member_off);
    ...
    for (ulong symbol_idx = 0; symbol_idx < symbol_count; symbol_idx += 1) {
      ...
      ulong member_idx = symbol_arr[symbol_idx].member_idx + 1;
      ushort member_idx16 = safe_cast_u16(member_idx);
    ...
    ulong member_offset = member_data_list.total_size + member_data.size + member_header.size;
    str8_list_push_pad_front(arena, &member_data_list, member_offset, COFF_Archive_MemberAlign);
  ...
  {
    uint  symbol_count_be  = from_be_u32(symbol_count);
    uint *member_off32_arr = push_array_no_zero(scratch.arena, uint, symbol_count);

    for (ulong symbol_idx = 0; symbol_idx < symbol_count; symbol_idx += 1) {
      LNK_LibSymbol *symbol = &symbol_arr[symbol_idx];

      // write big endian member offset
      ulong member_off = member_base_off + member_off_arr[symbol->member_idx];
      uint member_off32 = from_be_u32(safe_cast_u32(member_off));
    ...
    ulong member_offset = sizeof(g_coff_archive_sig) + member_header.size + member_data.size;
    str8_list_push_pad_front(arena, &member_data_list, member_offset, COFF_Archive_MemberAlign);
    str8_list_push_front(arena, &member_data_list, member_data);
    str8_list_push_front(arena, &member_data_list, member_header);
  }
...
lnk_build_import_entry_obj(Arena *arena, String8 dll_name, COFF_MachineType machine)
{
  ...
  ulong string_table_base = list.total_size;
  uint *string_table_size_ptr = push_array(arena, uint, 1);
  ...
  // import descriptor
  {
    ...
    ulong symbol_name_off = (list.total_size - string_table_base);
...
lnk_build_null_import_descriptor_obj(Arena *arena, COFF_MachineType machine)
{
  ...
  ulong null_import_data_size = 20;
  byte *null_import_data      = push_array(arena, byte, null_import_data_size);
  ulong null_import_data_off  = list.total_size;
...
lnk_build_null_thunk_data_obj(Arena *arena, String8 dll_name, COFF_MachineType machine)
{
  ...
  ulong lookup_entry_data_size = 8;
  byte *lookup_entry_data      = push_array(arena, byte, lookup_entry_data_size);
  ulong lookup_entry_data_off  = list.total_size;
  str8_list_push(arena, &list, str8(lookup_entry_data, lookup_entry_data_size));
  
  ulong null_thunk_data_size = 8;
  byte *null_thunk_data      = push_array(arena, byte, null_thunk_data_size);
  ulong null_thunk_data_off  = list.total_size;
...
internal String8
lnk_build_lib_member_header(Arena *arena, String8 name, COFF_TimeStamp time_stamp, ushort user_id, ushort group_id, ushort mode, uint size)
{
  ProfBeginFunction();
  ...
  //ulong sizeof_member_header = /* name */ 16 + /* time */ 12 + /* user_id */ 6 + /* group id */ 6 + /* mode */ 8 + /* size */ 10;
...
lnk_build_import_lib(TP_Context *tp, TP_Arena *arena, COFF_MachineType machine, COFF_TimeStamp time_stamp, String8 lib_name, String8 dll_name, LNK_ExportTable *exptab)
{
  ...
  for (ulong i = 0; i < ArrayCount(import_obj_array); ++i) {
